...
function enterTeacherGreet(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_rel ?? 0)?.['A26'] > 20  &&  ((s as any).pcs_hotcat ?? 0) >= 5) {
    ...
    if (((s as any).pantyworntype ?? 0) !== 'none') {
      ...
      scene.text(`"${((s as any).pcs_firstname || '')}, you really should squat down instead of bending over like that. One of the boys might take advantage!" he says with a smile.`);
    } else {
    ...
    scene.text(`"Miss ${((s as any).pcs_lastname || '')}! Please cover up!" he says when he notices that you're not wearing any panties. He quickly runs up to you and pulls your skirt down, covering up your naked ass.`);
    ...
    scene.text(`"Miss ${((s as any).pcs_lastname || '')}! Please cover up!" he says before he rushes to you and pulls your skirt down, covering your ass.`);
    scene.text('He gives you a lecture on how to properly pick up books from the floor before you both leave, not saying a word to each other.');
    scene.actions([
      { label: 'Continue', goto: ['gschool_lessons', 'morning'] },
    ]);
    ...
    scene.text(`"Yes, ${((s as any).pcs_firstname || '')}, their behavior is often bad," he says. Before leaving, he gently runs his hand over your back.`);
    ...
    if ((Math.floor(Math.random() * 5) + 0) > 0) {
      ...
      scene.text(`"Miss ${((s as any).pcs_lastname || '')}! Are you okay? Did you hurt yourself?"`);
      ...
      scene.text(`"You need to be more careful, ${((s as any).pcs_firstname || '')}! You have very beautiful legs and it would be a shame if you badly injured them," he gently whispers to you "It doesn't look bad, but you should go to the infirmary so they can check it out."`);
  ...
  scene.actions([
    ...
    if (((s as any).npc_rel ?? 0)?.['A26'] < 10) {
      if (((s as any).pcs_hotcat ?? 0) < 5) {
        ...
        scene.text(`"Hello ${((s as any).pcs_firstname || '')}. I also look forward to such a wonderful student like you being in my class," he replies while not looking you in the eye, but somewhere on the level of your hips.`);
        qspCall(s, 'npc_relationship', 'modify', 'A26', 1);
      }
    } else {
      scene.img('images/characters/shared/headshots_main/big26.jpg');
      if ((!(Math.floor(Math.random() * 2) + 0))) {
        ...
        scene.text(`"Hello ${((s as any).pcs_firstname || '')}. I'm expecting your homework today. Are you prepared, or should I not ask you for it?" he asks in a friendly manner.`);
    ...
    if (((s as any).npc_rel ?? 0)?.['A26'] < 10) {
      if (((s as any).pcs_hotcat ?? 0) < 5) {
        if ((!(Math.floor(Math.random() * 2) + 0))) {
          ...
          scene.text(`"Miss ${((s as any).pcs_lastname || '')}, you shouldn't be saying things like that," he replies, but still smiles back at you in response.`);
        } else {
        ...
        if ((!(Math.floor(Math.random() * 2) + 0))) {
          ...
          scene.text(`"Thank you, Miss ${((s as any).pcs_lastname || '')}. I'd pay you a compliment, but we're not supposed to have such informal relationships with students," he replies, but still smiles back.`);
          qspCall(s, 'npc_relationship', 'modify', 'A26', 1);
        } else {
      ...
      if ((!(Math.floor(Math.random() * 2) + 0))) {
        ...
        scene.text(`"Thank you, ${((s as any).pcs_firstname || '')}. I bought it myself with my last paycheck," he replies.`);
        qspCall(s, 'npc_relationship', 'modify', 'A26', 1);
      }
    }
    scene.actions([
    ...
    if (((s as any).npc_rel ?? 0)?.['A26'] < 10) {
      if (((s as any).pcs_hotcat ?? 0) < 5) {
        ...
        scene.text(`"Good morning, Miss ${((s as any).pcs_lastname || '')}. No time to say hello to your teacher?" he asks.`);
      ...
      scene.text(`"Good morning, Miss ${((s as any).pcs_lastname || '')}. Why don't you say hello? A teacher can't punish or reward such actions. You've still got this year's exam and should behave more kindly," he jokes.`);
      scene.actions([
...
function enterBreakEvents(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).break_rand ?? 0) === 1  &&  ((s as any).pcs_hotcat ?? 0) > 5) {
    ...
    scene.text(`The moment the hand makes contact with your ass, you jump mid-stride and squeal at the stinging pain. You whip around and glare furiously at ${((s as any).rand_boy || '')}, who is standing right behind you with a shit eating grin on his face.`);
    ...
    scene.text(`You turn to face ${((s as any).rand_boy || '')}, who is standing there with a huge grin on his face. You step towards him and, with a sultry smile, move your lips up to his ear.`);
    ...
    scene.text(`Your heart is pounding as you let them guide you along. You know perfectly well that they won't let you go now, no matter what you say. When you arrive at the men's restroom, one of the gopniks stays by the door to keep watch while the others shove you in. The walls are covered in all sorts of crude text. A lot of it involves you. Across the entire wall, a large text reads "${((s as any).pcs_firstname || '')} ${((s as any).pcs_lastname || '')} = ${((s as any).gnikname || '')}".`);
    ...
    scene.text(`"That was great, ${((s as any).pcs_firstname || '')}. I see you've certainly earned your reputation. We'll definitely be doing this more often, maybe next time I'll make sure the other boys can have a turn as well. We'll come find you when we want you. Understood?"`);
    ...
    scene.text(`"Hey ${((s as any).pcs_nickname || '')}. Somebody took the artwork off your locker, so I added it back for you. No need to thank me."`);
    ...
    scene.text(`"Oh, hey ${((s as any).pcs_nickname || '')}. Somebody took the artwork off your locker, so I added it back for you. No need to thank me." She gives you a dangerous look despite the calm tone in her voice.`);
...
function enterAfterschoolEvents(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).PCloSkirt ?? 0) > 0  &&  ((s as any).christinaQW ?? 0)?.['subpath'] === 0  &&  ((s as any).christinaQW ?? 0)?.['blackmail'] === 0  &&  ((s as any).christinaQW ?? 0)?.['fight'] === 0  &&  (Math.floor(Math.random() * 4) + 1) === 4) {
    ...
    scene.text(`She stumbles away, but calls back. "I gotta go. See you around, ${((s as any).pcs_nickname || '')}!"`);
    scene.actions([
      { label: 'Leave', goto: ['gschool_grounds', 'main'] },
    ]);
  } },
    ...
    scene.text(`You walk up to her and she turns to face you, giving you a full view of everything she has. "What's up, ${((s as any).pcs_nickname || '')}?"`);
    ...
    if (((s as any).fame ?? 0)?.['pav_slut'] > 250) {
      ...
      scene.text(`"<b>${((s as any).pcs_nickname || '')} ${((s as any).pcs_lastname || '')} = ${((s as any).gnikname || '')}"</b> is scrawled across one of the walls in large letters.`);
    }
    scene.actions([
    ...
    scene.text(`You kneel down and wrap your ${((s as any).pc_desc ?? 0)?.['lips'] ?? ''} lips around ${((s as any).boydesc || '')}'s hard cock and start sucking. Not forgetting the others, you spend some time alternating between all the cocks, stroking the ones you're not sucking with your hands.`);
    // TODO-QSP: dynamic text: Finally, you feel his cum hit the back of your throat and <<$boydesc>> moans. "T...
    scene.text(`Finally, you feel his cum hit the back of your throat and ${((s as any).boydesc || '')} moans. "That's right bitch! Swallow it all like a good little slut." ${((s as any).boydesc || '')} seems satisfied and tucks his cock back in his pants.`);
    ...
    scene.text(`He moves aside as well and you finish off the last two guys. As you get up off your knees, you hear one of them laugh. "You're the best cock sucker we got, ${((s as any).pcs_nickname || '')}!" they say as they leave you alone in the bathroom, more horny than when you started.`);
