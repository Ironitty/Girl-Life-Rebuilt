...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).mey_vika ?? 0)?.['mey_vika_qw'] >= 32) {
    ...
    ((st as any).mey_tamara = (st as any).mey_tamara ?? {})['help_day'] = ((st as any).daystart ?? 0);
    ((st as any).mey_tamara = (st as any).mey_tamara ?? {})['help_count'] = ((st as any).mey_tamara['help_count'] ?? 0) + (1);
    (st as any).minut = ((st as any).minut ?? 0) + ((Math.floor(Math.random() * 21) + 50));
    qspCall(st, 'sweat', 'add', 3);
    qspCall(st, 'mood', 'lower', 'small');
    qspCall(st, 'stat', '');
    if (((st as any).mey_vika ?? 0)?.['bath_qw'] === 0  &&  ((st as any).mey_vika ?? 0)?.['bath_qw_day'] !== ((st as any).daystart ?? 0)) {
      qspCall(st, 'mey_vika_events', 'bath_qw0');
    } else {
