...
function enterCityDate1Sporthub(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).kickbox ?? 0)?.['amateurWin'] + ((s as any).kickbox ?? 0)?.['amateurLoss'] + ((s as any).kickbox ?? 0)?.['amateurDraw'] > 0  &&  ((s as any).kickbox ?? 0)?.['amateur_fight_day'] !== ((s as any).daystart ?? 0)) {
    if (((s as any).pcs_stam ?? 0) < ((s as any).stammax ?? 0) / 5) {
      scene.actions([
        { label: 'Enter the amateur fight', handler: (st: GameState) => { scene.text('<br>You are too exhausted to do this. Recover your stamina.'); } },
      ]);
    } else {
      if (((s as any).pcs_energy ?? 0) < 20) {
        scene.actions([
          { label: 'Enter the amateur fight', handler: (st: GameState) => { scene.text('<br>You feel too hungry to do this.'); } },
        ]);
      } else {
        if (((s as any).pcs_hydra ?? 0) < 20) {
          scene.actions([
            { label: 'Enter the amateur fight', handler: (st: GameState) => { scene.text('<br>You feel too thirsty to do this.'); } },
          ]);
        } else {
          if ((!((s as any).PSport ?? 0))) {
            scene.actions([
              { label: 'Enter the amateur fight', handler: (st: GameState) => { scene.text('<br>You need to wear your sport clothes.'); } },
            ]);
          } else {
            scene.actions([
    ...
    if (((s as any).pcs_run ?? 0) >= 20  &&  ((s as any).runnerQW ?? 0)?.['comp_day'] !== ((s as any).daystart ?? 0)  &&  ((s as any).runnerQW ?? 0)?.['joined_team'] > 0) {
      if (((s as any).pcs_stam ?? 0) < ((s as any).stammax ?? 0) / 5) {
        scene.actions([
          { label: '<b>Enter competition race</b>', handler: (st: GameState) => { scene.text('<br>You are too exhausted to do this. Recover your stamina before entering the race.'); } },
        ]);
      } else {
        if (((s as any).pcs_energy ?? 0) < 20) {
          scene.actions([
            { label: '<b>Enter competition race</b>', handler: (st: GameState) => { scene.text('<br>You feel too hungry to do this.'); } },
          ]);
        } else {
          if (((s as any).pcs_hydra ?? 0) < 20) {
            scene.actions([
              { label: '<b>Enter competition race</b>', handler: (st: GameState) => { scene.text('<br>You feel too thirsty to do this.'); } },
            ]);
          } else {
            if (((s as any).apparel ?? 0)?.['status'] !== 'sport') {
              scene.actions([
                { label: '<b>Enter competition race</b>', handler: (st: GameState) => { scene.text('<br>You need to wear your sport clothes.'); } },
              ]);
            } else {
              scene.actions([
    ...
    st.scene = { ...st.scene, mainText: String(qspFunc(st, 'wrap', 'neg', '<br>You need to wear your normal clothes.') || ''), curActs: [] };
  } },
        ]);
      } else {
        scene.actions([
...
function enterCityDate1Mall(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).temp_city_work = undefined;
    if (((st as any).runnerQW ?? 0)?.['joined_team'] > 0  &&  (((st as any).runnerQW ?? 0)?.['bronze_medals'] + ((st as any).runnerQW ?? 0)?.['silver_medals'] + ((st as any).runnerQW ?? 0)?.['gold_medals']) > 0) {
      (st as any).temp_city_work = [...((st as any).temp_city_work ?? []), 'runner'];
    }
    if (((st as any).photography_start ?? 0) > 0) {
      (st as any).temp_city_work = [...((st as any).temp_city_work ?? []), 'photographer'];
    }
    if (((st as any).job_status ?? 0)?.['city_salon_masseuse'] === 'employed') {
      (st as any).temp_city_work = [...((st as any).temp_city_work ?? []), 'masseuse'];
    }
    if (((st as any).job_status ?? 0)?.['city_aphrodite_model'] === 'employed') {
      (st as any).temp_city_work = [...((st as any).temp_city_work ?? []), 'model'];
    }
...
function enterCityDate2Morningsex(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).NatbelQW ?? 0)?.['horny'] >= 10000) {
    ...
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['horny'] = 0;
    (st as any).temp_last_used = undefined;
    (st as any).minut = ((st as any).minut ?? 0) + 10;
