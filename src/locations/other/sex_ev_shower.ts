...
function enterBeforeBorrowShower(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if ((Math.floor(Math.random() * 10) + 1) < ((s as any).npc_sexdrive ?? 0)?.[String((s as any).npcID ?? 0)]) {
      ...
      { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterBorrowShower2(s, scene); (s as any).locArgs = __savedLocArgs; }
    } else {
      scene.text('"Okay," he snickers back.');
      { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterBorrowShower3(s, scene); (s as any).locArgs = __savedLocArgs; }
    }
  } },
...
function enterBorrowShower3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`${((s as any).npcdesc || '')} locks eyes with you for a split second before both of you rush for the bathroom, peeling off each other's clothes along the way.`);
    scene.actions([
      { label: 'Shower together', goto: ['sex_ev_shower', 'shower_together1'] },
    ]);
  } },
...
function enterBeforeAlone(s: GameState, scene: SceneBuilder): void {
  ...
  if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['start_shower'] = 1;
  qspCall(s, 'stat', '');
  ...
  scene.actions([
    ...
    scene.text(`A few minutes later, you turn off the water and step out, getting practically attacked by ${((s as any).npcdesc || '')} as soon as you open the door.`);
...
function enterAfterAloneShower(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`After spending a few minutes enjoying the heat, you borrow some of ${((s as any).npcdesc || '')}'s soap and do a quick scrub before turning off the water and stepping out, clean and dripping.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterAfterAloneTowel(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enterAfterAloneTowel(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['shower'] = 1;
  // TODO-QSP: end
  scene.actions([
    ...
    scene.text(`You quickly wrap yourself in a spare towel from the cupboard. ${((s as any).npcdesc || '')} is still laying in bed, only covered by a sheet.`);
    scene.text('"Thanks," you smile at him.');
    qspCall(s, 'sex_ev_after', 'after_sex2');
  } },
    { label: 'Wrap it around your head', handler: (st: GameState) => {
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['towel'] = 1;
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['towel_head'] = 1;
    ...
    scene.text(`You pull a towel from the cupboard and dry your body with it, only pausing to wrapping it around your hair before opening the door. Upon stepping out of the bathroom, ${((s as any).npcdesc || '')} eyes light up at the sight of your body, completely naked except for the towel turban atop your head.`);
    scene.text('"Thanks," you smile at him.');
    qspCall(s, 'sex_ev_after', 'after_sex2');
  } },
    ...
    scene.text(`You give your hair a quick wipe with ${((s as any).npcdesc || '')}'s towel to leave it merely damp instead of soaked and exit the bathroom.`);
...
function enterMorningAlone(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).sex_ev ?? 0)?.['boy_asleep'] === 1  &&  (Math.floor(Math.random() * 10) + 1) < ((s as any).npc_sexdrive ?? 0)?.[String((s as any).npcID ?? 0)]  &&  ((s as any).sex_ev ?? 0)?.['morning_fuck'] === 0) {
      ...
      scene.text(`A few minutes later, you hear the door open and ${((s as any).npcdesc || '')} comes clambering into the shower after you.`);
      scene.actions([
...
function enterMorningLook1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You wait a few minutes waiting outside the door for ${((s as any).npcdesc || '')} before you hear the water shuts off and he opens the door, his hair wet and a towel wrapped around his waist.`);
    ...
    scene.text(`"Sure," ${((s as any).npcdesc || '')} replies, smirking as he shamelessly ogles your nude body.`);
...
function enterShowerJoin1(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['join_shower'] = 1;
...
function enterShowerJoin2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You slip in tight, under the water right beside ${((s as any).npcdesc || '')}.`);
    if (((s as any).sex_ev ?? 0)?.['join_shower_offer'] === 1) {
      if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['join_shower_offer'] = 0;
    ...
    if (((s as any).sex_ev ?? 0)?.['join_shower_offer'] === 1) {
      ...
      scene.text(`"Couldn't resist after all, huh?" ${((s as any).npcdesc || '')} smirks as his soft shaft twitches in your grip.`);
    } else {
      // TODO-QSP: dynamic text: <<$npcdesc>> doesn't say anything but you hear his breathing grow heavy and his ...
      scene.text(`${((s as any).npcdesc || '')} doesn't say anything but you hear his breathing grow heavy and his soft shaft twitches in your grip.`);
    }
...
function enterBoyShowerJoin1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You just manage to put your arm across your breasts and turn your hips in a way that covers all your naughty bits as ${((s as any).npcdesc || '')} comes into view.`);
...
function enterBoyShowerJoinResponse1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Ah. Well. Let me know if you change your mind," ${((s as any).npcdesc || '')} says and leaves.`);
    ...
    scene.text(`"Sure," you smirk and ${((s as any).npcdesc || '')} moves to get into the shower with you. "I wouldn't mind a little bit of company..."`);
...
function enterShowerTogether1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerSexMenu(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enterWashBoy1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerReachCock(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterShowerReachCock(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You allow the soap to slip from your fingers, instead snaking your hand down to wrap them around ${((s as any).npcdesc || '')}'s cock.`);
    if (((s as any).sex_ev ?? 0)?.['cum_count'] >= 5 + ((s as any).sex_ev ?? 0)?.['extra_cum']  ||  (Math.floor(Math.random() * 10) + 1) > ((s as any).npc_sexdrive ?? 0)?.[String((s as any).npcID ?? 0)]) {
...
function enterShowerTogetherMakeout1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Things start getting hot and heavy under the shower. ${((s as any).npcdesc || '')}'s lips find yours and you kiss him back, enjoying the moment. Moans are released into each other's mouths as hands roam bodies with unrestricted access.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerSexMenu(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
...
function enterShowerTogetherJerkoff1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You reach back for ${((s as any).npcdesc || '')}'s cock as he keeps groping your tits from behind. Both your breathing intensifies as you jerk his shaft and he squeezes your nipples.`);
...
function enterTogetherBj1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Without a word, you kneel down, taking ${((s as any).npcdesc || '')}'s wet member in your mouth and noisily lick, suck, and slurp it as water rains down on your head.`);
    if (((s as any).npc_fav_pos ?? 0)?.[String((s as any).npcID ?? 0)] === 'blowjob') {
      ...
      scene.text(`"You're fucking incredible," ${((s as any).npcdesc || '')} groans beneath the ministrations of your lips and tongue. He's rock hard between your lips, somehow seeming to grow even harder by the second.`);
    } else {
      // TODO-QSP: dynamic text: <<$npcdesc>> groans softly beneath the ministrations of your lips and tongue, oc...
      scene.text(`${((s as any).npcdesc || '')} groans softly beneath the ministrations of your lips and tongue, occasionally letting his hand rest on your hair as a gentle guide to let you know you're doing well.`);
    }
    ...
    scene.text(`"What?" ${((s as any).npcdesc || '')} moans breathlessly. You let his cock slip from your mouth with a <i>pop</i>.`);
...
function enterTogetherFuckMenu(s: GameState, scene: SceneBuilder): void {
  if (((s as any).stat ?? 0)?.['think_virgin'] === 0  ||  ((s as any).sex_ev ?? 0)?.['fuck_count'] > 0) {
    ...
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['shower_sex'] = ((s as any).sex_ev['shower_sex'] ?? 0) + (1);
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['orgasm'] = ((s as any).orgasm ?? 0);
    scene.actions([
...
function enterFuckCondomCheck(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_free_creampies ?? 0)?.[String((s as any).npcID ?? 0)] !== 'at_will'  &&  ((s as any).sex_ev ?? 0)?.['condom_setup'] !== 1) {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', 'together_fuck_' + qspUntranslated(s, "sex_ev['position']>", { location: "sex_ev_shower" }) + '2']; enterDefault(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
        ]);
      } else {
    ...
    scene.text(`Your reply is interrupted by a small squeak when ${((s as any).npcdesc || '')} pushes his cock between your folds.`);
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterTogetherFuckBareback(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
        ]);
      } else {
        if (((s as any).stat ?? 0)?.['preg_risk'] === 'prob_safe') {
    ...
    scene.text(`"Don't forget to pull- <i>Nngh~!</i>" Your warning is interrupted by a sharp breath when ${((s as any).npcdesc || '')} pushes his cock balls deep into your pussy.`);
    ...
    scene.text(`"We'll need to use a condom after this. But for now just don't forget to pull- <i>Nngh~!</i>" Your warning is interrupted by a sharp breath when ${((s as any).npcdesc || '')} pushes his cock balls deep into your pussy.`);
    scene.text('"... out," you finish with a sigh.');
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', 'together_fuck_' + qspUntranslated(s, "sex_ev['position']>", { location: "sex_ev_shower" }) + '2']; enterDefault(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
      ]);
...
function enterTogetherFuckMiss1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['position'] === 'handjob') {
    ...
    if (((s as any).sex_ev ?? 0)?.['position'] === 'blowjob') {
      // TODO-QSP: dynamic text: You pull your lips off <<$npcdesc>>'s cock with a pop and stand to face him. Wit...
...
function enterTogetherFuckDoggy2(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: <<$npcdesc>>'s hips slap against your ass as he eagerly fucks your pussy, helpin...
...
function enterShowerCumAsk(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['cock_inserted'] === 1) {
    ...
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['not_inside'] = 1;
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerTogetherJerkoffCum(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
      ]);
    }
    scene.actions([
...
function enterShowerTogetherJerkoffCum(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['last_cum'] = 'jerkoff';
...
function enterShowerCumMouth1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['position'] !== 'blowjob') {
    // TODO-QSP: dynamic text: With extreme urgency, you hop off of <<$npcdesc>>'s cock and kneel down on the p...
  ...
  // TODO-QSP: dynamic text: "That's it-!" <<$npcdesc>> grunts when you swallow it whole, lips kissing the ba...
  scene.text(`"That's it-!" ${((s as any).npcdesc || '')} grunts when you swallow it whole, lips kissing the base of his shaft. "I'm gonna-!"`);
  // TODO-QSP: dynamic text: He doesn't even manage to finish his warning before he groans loudly and his coc...
...
function enterShowerCumMouth2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You gulp down ${((s as any).npcdesc || '')}'s load.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerTogetherEnd(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
...
function enterShowerCreampie1(s: GameState, scene: SceneBuilder): void {
  ...
  if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['shower_creampie'] = ((s as any).sex_ev['shower_creampie'] ?? 0) + (1);
...
function enterShowerCreampie2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).sex_ev ?? 0)?.['position'] === 'miss') {
      scene.img('images/shared/sex/shower/leg_up2.mp4');
      // TODO-QSP: dynamic text: The first spurt of <<$npcdesc>>'s cum into you flips your switch and you're imme...
      scene.text(`The first spurt of ${((s as any).npcdesc || '')}'s cum into you flips your switch and you're immediately shuddering, muscles clenching, one leg tightening around his hip while the other wobbles beneath you, both on the verge of cramping in the throes of orgasm while he empties his balls into your cunt.`);
    } else {
      if (((s as any).sex_ev ?? 0)?.['position'] === 'doggy') {
        scene.img('images/shared/sex/shower/doggy_orgasm1.mp4');
        // TODO-QSP: dynamic text: The first spurt of <<$npcdesc>>'s cum into you flips your switch and you're imme...
        scene.text(`The first spurt of ${((s as any).npcdesc || '')}'s cum into you flips your switch and you're immediately shuddering in orgasm. Your legs go weak and you nearly slip off his cock even as he pumps load after load into your cunt, emptying his balls inside you.`);
      }
    }
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerCreampieAfter(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ...
    if (((s as any).sex_ev ?? 0)?.['orgasm'] < ((s as any).orgasm ?? 0)) {
      // TODO-QSP: dynamic text: <<$npcdesc>>'s cum in your cunt immediately makes you go stiff, ruining your org...
      scene.text(`${((s as any).npcdesc || '')}'s cum in your cunt immediately makes you go stiff, ruining your orgasm and cutting it short right after it started. Disgust and revulsion overwhelm you as he pours load after load into you, but it's too late now. You give a silent sigh, grimacing internally as you wait for him to finish emptying his balls.`);
    } else {
      // TODO-QSP: dynamic text: <<$npcdesc>>'s cum in your cunt immediately makes you stiffen up. Disgust and re...
      scene.text(`${((s as any).npcdesc || '')}'s cum in your cunt immediately makes you stiffen up. Disgust and revulsion overwhelm you as he pours load after load into you, but it's too late to do anything more about it. You give a silent sigh, grimacing internally as you wait for him to finish emptying his balls.`);
    }
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerCreampieAfter(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
...
function enterShowerCreampieAfter(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).sex_ev ?? 0)?.['orgasm'] < ((s as any).orgasm ?? 0)) {
      ...
      scene.text(`${((s as any).npcdesc || '')} slips his cock from your pussy and you shiver as a hot glob of cum immediately seeps out and dribbles down your leg.`);
    } else {
      // TODO-QSP: dynamic text: <<$npcdesc>> slips his cock from your pussy and a hot glob of cum immediately se...
      scene.text(`${((s as any).npcdesc || '')} slips his cock from your pussy and a hot glob of cum immediately seeps out and dribbles down your leg.`);
    }
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerTogetherEnd(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
...
function enterShowerCreampieSurprise(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['creampie_allowance'] === 0  &&  ((s as any).npc_free_creampies ?? 0)?.[String((s as any).npcID ?? 0)] !== 'at_will') {
    ...
    scene.text(`"Asshole!" you shout, pushing ${((s as any).npcdesc || '')} away from you. But it's too late and you feel a hot liquid that isn't water running down your leg.`);
    ...
    scene.text(`"Wait!" ${((s as any).npcdesc || '')} protests, clumsily wrapping a towel around his waist and following you.`);
    ...
    scene.text(`After a few more moments of anger, you deflate. You pull ${((s as any).npcdesc || '')} close, pressing your naked skin against his.`);
    ...
    scene.text(`You stand there holding each other in silence for several more minutes before ${((s as any).npcdesc || '')} breaks it.`);
    scene.text('"So... what should we do now?"');
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerWhatNow(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ...
  scene.actions([
    ...
    scene.text(`You gasp in surprise, but hold on to ${((s as any).npcdesc || '')}, allowing him to empty his balls into your cunt.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerCreampieSurpriseReaction(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    { label: 'Creampie induced orgasm', handler: (st: GameState) => {
    if (((s as any).sex_ev ?? 0)?.['orgasm'] === ((s as any).orgasm ?? 0)) {
      // TODO-QSP: gs 'arousal', 'vaginal',1, 'no_orgasm_msg', $sex_ev['prostitution_flag']
    ...
    scene.text(`You gasp in surprise, feeling ${((s as any).npcdesc || '')} pulse inside you. You were already close yourself and his orgasm sends you over the edge. He holds you tight and you clench on to him with all your might, both of you riding out your orgasms completely intertwined.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerCreampieSurpriseReaction(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
...
function enterShowerCreampieSurpriseReaction(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).sex_ev ?? 0)?.['orgasm'] < ((s as any).orgasm ?? 0)) {
      ...
      scene.text(`When your orgasms finally subside, ${((s as any).npcdesc || '')} pulls away, eliciting shudders from you as his cum gushes from your pussy.`);
    } else {
      // TODO-QSP: dynamic text: Once <<$npcdesc>>'s orgasm finally subsides, he pulls out of your pussy with a p...
      scene.text(`Once ${((s as any).npcdesc || '')}'s orgasm finally subsides, he pulls out of your pussy with a pop, releasing a gush of cum that dribbles down your leg.`);
    }
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerTogetherEnd(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ...
    scene.text(`"Yeah..." ${((s as any).npcdesc || '')} smiles back at you.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterShowerTogetherEnd(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
...
function enterShowerTogetherEnd(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['sex_over'] === 0) {
    ...
    scene.text(`You and ${((s as any).npcdesc || '')} stand there under the hot water together, breathing hard.`);
...
function enterShowerTogetherEndingKiss(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).sex_ev ?? 0)?.['orgasm'] < ((s as any).orgasm ?? 0)) {
      ...
      scene.text(`Still panting as you wind down from your orgasm, you give ${((s as any).npcdesc || '')} one final parting kiss, pouring all of your enjoyment and satisfaction into the brief smack of your lips together.`);
    } else {
      if (((s as any).sex_ev ?? 0)?.['position'] === 'makeout') {
        // TODO-QSP: dynamic text: All good things must come to an end. <<$npcdesc>> shuts off the water and you gi...
        scene.text(`All good things must come to an end. ${((s as any).npcdesc || '')} shuts off the water and you give him one final parting kiss before stepping out of the shower together.`);
      } else {
        if (((s as any).sex_ev ?? 0)?.['position'] === 'blowjob'  &&  ((s as any).sex_ev ?? 0)?.['last_cum'] !== 'facial') {
          // TODO-QSP: dynamic text: You smile at <<$npcdesc>> as you get off your knees and stand, giving him a smal...
          scene.text(`You smile at ${((s as any).npcdesc || '')} as you get off your knees and stand, giving him a small kiss just before he turns off the faucet and you step out of the shower together.`);
        } else {
          if (((s as any).sex_ev ?? 0)?.['last_cum'] === 'facial') {
            ...
            scene.text(`As the last of the cum is cleaned from your face, you turn and give ${((s as any).npcdesc || '')} a small kiss of satisfaction before he turns off the faucet and you step out of the shower together.`);
          } else {
            // TODO-QSP: dynamic text: With a smile, you give <<$npcdesc>> a small kiss of satisfaction just before he ...
            scene.text(`With a smile, you give ${((s as any).npcdesc || '')} a small kiss of satisfaction just before he turns off the faucet and you step out of the shower together.`);
          }
        }
      }
    }
...
function enterShowerTogetherTowel(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npcID ?? 0) === 'A34') {
    scene.actions([{ label: 'Continue', goto: ['sex_ev_A34', 'kolka_shower_end'] }]);
  }
  ...
  if (((s as any).sex_ev ?? 0)?.['loc'] === 'pc_home') {
    ...
    scene.text(`You grab your towel off the rack and pull one out of the cupboard for ${((s as any).npcdesc || '')}.`);
    ...
    scene.text(`You grab your towel off the rack and hand it to ${((s as any).npcdesc || '')}.`);
    ...
    scene.text(`You give your hair a quick wring and exit the bathroom naked, ${((s as any).npcdesc || '')} following behind you with his towel around his waist.`);
    qspCall(s, 'sex_ev_after', 'after_sex2');
  } },
      ]);
    } else {
