...
function enterAfterAlone(s: GameState, scene: SceneBuilder): void {
  ...
  if ((Math.floor(Math.random() * 10) + 1) < ((s as any).npc_sexdrive ?? 0)?.[String((s as any).npcID ?? 0)]) {
    scene.actions([
      { label: 'Continue', goto: ['sex_ev_shower', 'boy_shower_join1'] },
    ]);
  } else {
...
function enterBoyShowerJoin1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('"Well, I got to thinking and thought maybe you might ' + (((Math.floor(Math.random() * 2) + 1) === 1) ? ('need someone to wash your back."') : ('be feeling lonely and need some company in here."')));
...
function enterShowerCumAsk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('iif(sex_ev[\'orgasm\'] < orgasm, \'"Me too-!" you gasp.\', \'"Mmmm-!" you hum back.\')');
...
function enterShowerCreampieAfter(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    scene.text(`iif($sex_ev['position'] = 'miss','<center><video autoplay loop ${((st as any).set_imgh ?? '')} src="images/shared/sex/shower/creampie1.mp4"></video></center>','<center><img ${((st as any).set_imgh ?? '')} src="images/shared/sex/shower/creampie_after1.jpg"></video></center>')`);
    if (((st as any).sex_ev ?? 0)?.['orgasm'] < ((st as any).orgasm ?? 0)) {
...
function enterBoyShowerFinishActs(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('"Did you have a good shower?" you ask' + ((((st as any).sex_ev ?? 0)?.['drawer_rummage'] === 1) ? (' innocently putting your hands behind your back as if you hadn\'t just been snooping through his things.') : ('.')));
    if (((st as any).sex_ev ?? 0)?.['join_shower_offer'] === 1) {
