...
function enterPosterRepeat(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: '', labelFn: (s: GameState) => String(((((s as any).cheerleadingQW ?? 0)?.['try_count'] > 0) ? ('Decide to try one more time') : ('Decide to give it a try this time')) ?? ''), handler: (st: GameState) => {
    ((st as any).cheerleadingQW = (st as any).cheerleadingQW ?? {})['quest_stage'] = 1;
    if (((st as any).grupTipe ?? 0) === 1  ||  ((st as any).grupTipe ?? 0) === 2) {
      scene.text('"I\'m one of them. They would be mad to not at least consider me! Those nerds and their rumors are just jealous losers!" you think to yourself as you decide to attend the tryouts and show them that you deserve that spot.');
    } else {
    ...
    { label: '', labelFn: (s: GameState) => String(((((s as any).cheerleadingQW ?? 0)?.['try_count'] > 0) ? ('It\'s not worth the hassle ') : ('Ignore the poster ')) ?? ''), handler: (st: GameState) => {
...
function enterSkippedEv(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).cheerleadingQW ?? 0)?.['skipped'] === 1) {
    ...
    scene.text(`"Where were you yesterday, ${((s as any).pcs_nickname ?? '')}? ` + ((((s as any).cheerleadingQW ?? 0)?.['skipped_ev'] === 1) ? ('You missed learning about and practicing the new parts we added to our routine. You really let the team down."') : ('You missed the game and it was really hard adjusting the routine. You seriously disappointed us, ' + ((s as any).pcs_nickname ?? '') + '."')));
