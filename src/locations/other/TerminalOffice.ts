...
function enterTerminalOfficeScreen(s: GameState, scene: SceneBuilder): void {
  scene.text(qspFunc(s, 'wrap', 'header b center', ((s as any).locArgs?.[1] ?? '')));
  scene.text(`<center><img ${((s as any).set_imgh ?? '')} src="images/locations/city/industrial/terminal/` + ((s as any).locArgs?.[2] ?? '') + '.jpg"></center>');
  scene.build();
}
...
function enter11BuyGoods(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).DoNotBuy ?? 0) === 2) {
    ...
    (st as any).MaxCost = ((st as any).BuyQuantity ?? 0) * 100;
    if (qspFunc(s, 'money', 'can_afford', ((st as any).MaxCost ?? 0), 'cash') === 0) {
      scene.text('You don\'t have enough money on you. You will have to withdraw money before proceeding with the purchase.');
      if (qspFunc(s, 'money', 'can_afford', ((st as any).MaxCost ?? 0)) === 1) {
      ...
      if (((st as any).BuyQuantity ?? 0) >= ((st as any).MaxQuantityHome ?? 0)) {
        ...
        if (((st as any).YouCanGar ?? 0) > 0) {
          scene.text(`L 'You'll store ${((st as any).MaxQuantityHome ?? '')} trickets at home.'`);
        }
      } else {
        if (((st as any).YouCanGar ?? 0) > 0) {
          scene.text(`L 'You'll store ${((st as any).BuyQuantity ?? '')} trinkets at home.'`);
        }
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 13;
    (st as any).BuyQuantity = ((st as any).MaxQuantityHome ?? 0) + ((st as any).MaxQuantityGarage ?? 0);
    (st as any).MaxCost = ((st as any).BuyQuantity ?? 0) * 100;
    if (qspFunc(s, 'money', 'can_afford', ((st as any).MaxCost ?? 0), 'cash') === 0) {
      scene.text('You don\'t have enough money on you. You will have to withdraw money before proceeding with the purchase.');
      if (qspFunc(s, 'money', 'can_afford', ((st as any).MaxCost ?? 0)) === 1) {
...
function enter21(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).TerminalOfficeBuhWorkTime ?? 0))) {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['TerminalTask'] = 2;
    qspCall(st, 'stat', '');
