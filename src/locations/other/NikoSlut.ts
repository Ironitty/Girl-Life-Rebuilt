...
function enterCustomer1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The man looks you in the eyes. "Thank you so much, ${((s as any).NikoSlutName || '')}. You're such a perfect girl and I would gladly pay you every day for your services." The man puts his pants on and walks out of the restroom as you follow behind.`);
    scene.actions([
...
function enterCustomer2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The man's breathing suddenly stops as he starts pumping his warm seed into your pussy. Once he's finished, the man pulls out of you, causing his cum to leak out. "That was amazing! You're one sexy girl ${((s as any).NikoSlutName || '')}. I can't wait to use your services again."`);
...
function enterCustomer3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"So ${((s as any).NikoSlutName || '')}, are you ready to get your pussy fucked?" he asks with a grin.`);
    scene.actions([
    ...
    scene.text(`"That was incredible, ${((s as any).NikoSlutName || '')}! You're one sexy girl," he says after catching his breath. He then kisses you on the cheek before getting dressed and leaving the restroom.`);
    scene.actions([
...
function enterCustomer4(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Well done ${((s as any).pcs_nickname || '')}. You should probably get washed up for the next customer."`);
    scene.actions([
      { label: 'Wash up first', handler: (st: GameState) => {
    qspCall(st, 'outfit', 'restore');
...
function enterCustomer7(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You remove your pants as the trucker lays on the ground before mounting him and guiding his dick into your pussy. You start riding him at a moderate pace before he suddenly lifts you up by the legs, pushing his dick deeper inside you as he whispers in your ear. "How does it feel, ${((s as any).NikoSlutName || '')}? Good?"`);
...
function enterCustomer8(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The man takes a few seconds to catch his breath. "You were outstanding, ${((s as any).NikoSlutName || '')}. I look forward to seeing you again soon." He then buttons his pants before leaving the restroom.`);
    scene.actions([
      { label: 'Stand up', goto: ['NikoSlut', 'Mens Restroom'] },
    ]);
  } },
...
function enterCustomer9(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).VK ?? 0) === 1) {
    ...
    scene.text(`After a few minutes, the trucker groans, filling the condom with his warm seed. You focus on the condom expanding inside you until the man finishes. "You were great ${((s as any).NikoSlutName || '')}, I'll definitely be visiting you again." He then pulls up his pants and leaves the restroom.`);
    scene.actions([
      { label: 'Head back to Niko', goto: ['NikoSlut', 'Customers'] },
    ]);
  } },
    ...
    scene.text(`After a few minutes, the trucker groans, filling your womb with his warm seed. As soon as the man finishes, he squeezes your breast. "You were great ${((s as any).NikoSlutName || '')}, I'll definitely be visiting you again." He then pulls up his pants and leaves the restroom.`);
    scene.actions([
      { label: 'Wash up first', goto: ['NikoSlut', 'Mens Restroom'] },
      { label: 'Head back to Niko', goto: ['NikoSlut', 'Customers'] },
    ]);
...
function enterDinerInterior(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { label: 'Order some eggs', handler: (st: GameState) => {
