...
function enterCustomer1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'cum_call', 'face', 'Biker');
    qspCall(st, 'arousal', 'bj', 5, 'dom');
    ...
    qspCall(st, 'cum_call', 'mouth_swallow', 'Biker');
    ...
    qspCall(st, 'arousal', 'anal', 10, 'dom');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'dina', 'prezik');
    qspCall(st, 'cum_call', 'anus', 'Biker', 0, 3);
    qspCall(st, 'arousal', 'anal', 5, 'dom');
    qspCall(st, 'arousal', 'end');
    ...
    scene.actions([
      ...
      scene.actions([
        { label: 'Make him cum', handler: (st: GameState) => {
    qspCall(st, 'cum_call', 'anus', 'Biker');
    qspCall(st, 'arousal', 'anal', 5, 'dom');
    qspCall(st, 'arousal', 'end');
...
function enterCustomer2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'cum_call', 'mouth_swallow', 'Trucker');
    qspCall(st, 'arousal', 'bj', 5, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
    ...
    qspCall(st, 'arousal', 'bj', 10, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
    scene.img('images/characters/pavlovsk/school/boy/niko/nikomisc/cum/swallow7.mp4');
    scene.text('He starts squirming and groaning as he fills your mouth with his cum. The man fully milks his dick in your mouth before pulling up his pants. "Damn girl, you really got me off quickly. Great work."');
    ...
    scene.actions([
      ...
      { label: 'Anal', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'anal', 10, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
    ...
    if (((st as any).mc_inventory ?? 0)?.['equipped_condoms'] >= 1  &&  (!((st as any).preziktype ?? 0))) {
      scene.actions([
        { label: 'Make him cum', handler: (st: GameState) => {
    qspCall(st, 'dina', 'prezik');
    qspCall(st, 'cum_call', 'anus', 'Trucker', 0, 3);
    qspCall(st, 'arousal', 'anal', 5, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
    ...
    scene.actions([
      ...
      scene.actions([
        { label: 'Make him cum', handler: (st: GameState) => {
    qspCall(st, 'cum_call', 'anus', 'Trucker');
    qspCall(st, 'arousal', 'anal', 5, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
    scene.img('images/shared/sex/cum/analcreampie/cumanal1.mp4');
...
function enterCustomer5(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.actions([
      { label: 'Make him cum (Swallow)', handler: (st: GameState) => {
    qspCall(st, 'cum_call', 'mouth_swallow', 'Keith');
    qspCall(st, 'arousal', 'bj', 5, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
    ...
    qspCall(st, 'dina', 'prezik');
    qspCall(st, 'cum_call', 'anus', 'Keith', 0, 3);
    qspCall(st, 'arousal', 'anal', 5, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
    ...
    scene.actions([
      ...
      scene.actions([
        { label: 'Rock your hips', handler: (st: GameState) => {
    qspCall(st, 'cum_call', 'anus', 'Keith');
    qspCall(st, 'arousal', 'anal', 5, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'end');
...
function enterCustomer6(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).VK ?? 0) === 1) {
    ...
    scene.actions([
      { label: 'Make him cum', handler: (st: GameState) => {
    qspCall(st, 'cum_call', 'mouth', 'Trucker');
    qspCall(st, 'arousal', 'bj', 5, 'sub');
    qspCall(st, 'arousal', 'end');
...
function enterBikerAttack(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).npc_grupTipe = (st as any).npc_grupTipe ?? {})['A189'] = 600;
    (st as any).NikoEv = 21;
    (st as any).NikoSlut = 0;
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
...
function enterDinerInterior(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) === 18) {
    if (((s as any).YurikEv ?? 0)?.['Name'] === 'Yurik') {
      scene.text('You see <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027NikoSlut/u0027, /u0027Yurik In/u0027); return false;">Yurik</a> sitting in the corner of the bar eating some dinner.');
    } else {
      scene.text('You see <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027NikoSlut/u0027, /u0027Yurik In/u0027); return false;">a burly bearded man</a> sitting in the corner of the bar eating some dinner.');
    }
  }
  // TODO-QSP: end
  scene.actions([
...
function enterMensRestroom(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('This restroom has a <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027NikoSlut/u0027, /u0027Wash/u0027); return false;">sink</a>, 3 urinals and a <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027mirror/u0027, /u0027start/u0027); return false;">mirror</a>. A pretty funky smell fills the room and judging by the stains on the wall, the guys who use this restroom aren\'t very good at aiming.');
...
function enterLadiesRestroom(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('This restroom has 5 <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027NikoSlut/u0027, /u0027Wash/u0027); return false;">sink</a>, 5 <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027mirror/u0027, /u0027start/u0027); return false;">mirrors</a> and 3 stalls. It\'s very clean and looks pretty unused, likely because very few women visit the diner.');
  if (((s as any).NikoSlut ?? 0) === 2) {
    scene.actions([
      { label: 'Return to Niko', goto: ['NikoSlut', 'Customers'] },
    ]);
