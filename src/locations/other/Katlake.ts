...
function enterZnakomstvo(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_apprnc ?? 0) < 56) {
    ...
    scene.text(`"${((s as any).pcs_nickname ?? 0)}", you exhale with relief. Feeling a lot more at ease, you have a nice conversation with her for another half hour while you enjoy the warmth of the sun together.`);
...
function enterKey(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Take it easy, ${((s as any).pcs_nickname ?? 0)}!" you hear Kat's voice in the darkness. "You're safe, just relax!"`);
