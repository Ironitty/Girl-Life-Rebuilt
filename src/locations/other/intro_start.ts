...
function enterTgAdultShared(s: GameState, scene: SceneBuilder): void {
  if ((!((s as any).locArgs?.[1] ?? 0))) {
    ...
    qspCall(s, 'intro_start', 'tg_adult_shared_set_mikael_stats');
    scene.actions([
    ...
    scene.actions([
      { label: 'Fight', goto: ['fight', 'start'] },
        ...
        if (((s as any).locArgs?.[1] ?? 0) === 3) {
          qspCall(s, 'intro_start', 'tg_adult_shared_reset_mikael_stats');
          scene.img('images/system/1_openings/shared/npc_tatiana.jpg');
...
function enterSgTg(s: GameState, scene: SceneBuilder): void {
  if ((!((s as any).locArgs?.[1] ?? 0))) {
    ...
    if (((s as any).locArgs?.[1] ?? 0) === 1) {
      ...
      scene.text(`Her name was ${((s as any).pcs_firstname ?? 0)} ${((s as any).pcs_lastname ?? 0)}, or ${((s as any).pcs_nickname ?? 0)} for short. You don't know why, but she seemed to liked you. Maybe she was as lonely as you were. You couldn't believe your luck.`);
    ...
    // TODO-QSP: gt 'intro_start', 'sg_tg', 1
      ...
      if (((s as any).locArgs?.[1] ?? 0) === 2) {
        ...
        scene.text(`Her mother's sister, Aunt Luda, lives on the top floor of the same building as her. Her stepfather's best friend is called Uncle Misha, who lives across the hall. Her mother's old friend Sergey Shulgin lives on the floor above them, and ${((s as any).pcs_nickname ?? 0)}'s old childhood friend Vasily is his son.`);
        ...
        scene.text(`You did the best you could to repress any resentment towards ${((s as any).pcs_nickname ?? 0)} about how little she appreciated what she had.`);
        scene.actions([
          { label: 'Continue', handler: (st: GameState) => {
    ...
    scene.text(`A few months passed. You and ${((s as any).pcs_nickname ?? 0)} continued to talk, and you learned even more about her life. As summer break approaches, you bring up the idea of meeting up. ${((s as any).pcs_nickname ?? 0)} was excited at the prospect and you agreed to meet at the café where her mother works.`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    ...
    scene.text(`On the train ride to Pavlovsk, you get a message from ${((s as any).pcs_nickname ?? 0)}. Her parents have taken her to spend time with her grandparents in Gadukino. She was unable to get out of the trip, since her mother has been upset at how she has been spending so much time on her phone and not with her family.`);
    ...
    scene.text(`You get off the train next to a small road in the middle of nowhere. A dirt road leads to a small village made up of rickety wooden houses and even more ruined ones. Unsure which house belongs to ${((s as any).pcs_nickname ?? 0)}'s family, you pull out your phone to text her.`);
