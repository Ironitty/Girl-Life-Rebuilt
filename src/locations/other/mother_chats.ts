...
function enterRapeTalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`After a very brief pause, your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} adds her immediate thought. "Erm… ${((s as any).pcs_nickname || '')}, I take it you've taken the morning after pill? If not, then you need to do that as you don't want to end up pregnant. I can get one for you."`);
    ...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you. "${((s as any).pcs_nickname || '')}, ff you want me to go with you please just let me know; you don't have to do that alone."`);
...
function enterMiscarriageTalk1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"You've been very brave, ${((s as any).pcs_nickname || '')}. Even if you didn't feel ready, you would have been a wonderful mother. But hush now. Just let it all out."`);
    ...
    scene.text(`"I'm so sorry ${((s as any).pcs_nickname || '')}," she murmurs and pulls back to hold your face directly in front of hers. "I know what it's like to want to do anything and everything to protect your baby. I can't imagine the pain of what you're going through right now…"`);
    scene.text('Tears well up in your eyes again and she pulls you back into her arms and cuddles you, holding you tight as you both let out all your pain and anguish.');
...
function enterLostVirginityTalk(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).momslut ?? 0))) {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterLostVirginLeaveAlone(s, scene); (s as any).locArgs = __savedLocArgs; }
    scene.actions([
    ...
    ((s as any).mother = (s as any).mother ?? {})['kickedout_timer'] = ((s as any).daystart ?? 0);
    scene.img('images/characters/pavlovsk/resident/mom/walking.jpg');
...
function enterLostVirginityTalk1(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).momslut ?? 0))) {
    ...
    // TODO-QSP: dynamic text: Your <<$npc_nickname['A29']>> just glares at you and opens her mouth, but all th...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} just glares at you and opens her mouth, but all that comes out is a frustrated growl. She shoos you away with her arms and turns away from you.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterLostVirginLeaveAlone1(s, scene); (s as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Leave home (You will not be permanently banned from parents home)', handler: (st: GameState) => {
    qspCall(s, 'homes_properties', 'block_access', 'parents_home');
    ((s as any).mother = (s as any).mother ?? {})['kickedout_timer'] = ((s as any).daystart ?? 0);
...
function enterReconciliationTalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((s as any).mother = (s as any).mother ?? {})['kickedout_timer'] = 0;
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    ...
    scene.text(`She responds with a sad smile as she looks at you. "Never mind that, ${((s as any).pcs_nickname || '')}. I asked around as I haven't stopped loving you and you're very important to me."`);
    ...
    ((s as any).mother = (s as any).mother ?? {})['kickedout_pending'] = 1;
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    qspCall(s, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/mom/stop_talk.jpg');
    ...
    scene.text(`"Okay ${((s as any).pcs_nickname || '')}," she sighs. "I guess I'll just have to say it now... I just wanted to say I'm sorry that things went so far and to tell you that I miss you. In fact, we all miss you and you're welcome to return home. I do love you ${((s as any).pcs_nickname || '')}, I hope you know that!"`);
    ...
    scene.text(`"${((s as any).pcs_nickname || '')}… I'm sorry about my behaviour at the clinic. I was very angry and…" She pauses before continuing. "You were right. I overreacted and… I'm sorry for that."`);
    ...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} gets up and rushes around the table to give you a hug, nearly knocking her tea over in the process. "Thank you, ${((s as any).pcs_nickname || '')}!"`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterReconciliationReplies(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enterKnock(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).mother ?? 0)?.['kickedout_visit'] === 0) {
      ((s as any).mother = (s as any).mother ?? {})['kickedout_visit'] = 1;
      ...
      scene.text(`She just nods and hugs you again. "Okay ${((s as any).pcs_nickname || '')}, I guess that'll have to do for now. Come in and I'll make us some tea."`);
      ...
      scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks sad. "Oh... Well we all miss you, ${((s as any).pcs_nickname || '')}. Please... Wouldn't you reconsider?"`);
      // TODO-QSP: dynamic text: "No <<$npc_nickname['A29']>>, I'm happy where I am; here's the address so you ca...
      scene.text(`"No ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''}, I'm happy where I am; here's the address so you can visit if you want. However, I'd really like it if we could at least <i>try</i> and talk?"`);
      // TODO-QSP: dynamic text: She just nods and hugs you again. "Okay <<$pcs_nickname>>, I guess that'll have ...
      scene.text(`She just nods and hugs you again. "Okay ${((s as any).pcs_nickname || '')}, I guess that'll have to do for now. Come in and I'll make us some tea."`);
    ...
    ((s as any).mother = (s as any).mother ?? {})['kickedout_pending'] = 0;
    qspCall(s, 'homes_properties', 'give_access', 'parents_home');
    qspCall(s, 'npc_relationship', 'modify', 'A29', 10);
    (s as any).minut = ((s as any).minut ?? 0) + 25;
    qspCall(s, 'stat', '');
    ...
    scene.text(`"Yes, of course ${((s as any).pcs_nickname || '')}, welcome home! Let me make a cup of tea and we can chat before you go to your room and unpack."`);
    // TODO-QSP: dynamic text: You follow your <<$npc_nickname['A29']>> into the kitchen and chat with her over...
    ...
    scene.text(`She smiles at you and hugs you tightly. "I'm so happy you decided to come home, ${((s as any).pcs_nickname || '')}!"`);
    scene.actions([
...
function enterNatbel(s: GameState, scene: SceneBuilder): void {
  if (((s as any).loc ?? 0) === 'kuhrPar'  &&  qspFunc(s, 'homes_properties', 'is_current_home', 'parents_home')) {
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 10;
    ((s as any).NatbelQW = (s as any).NatbelQW ?? {})['mother_talk'] = 1;
    ((s as any).NatbelQW = (s as any).NatbelQW ?? {})['mother_talkday'] = ((s as any).daystart ?? 0);
    ...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} interjects. "What are you telling me, ${((s as any).pcs_nickname || '')}? It can't be just that you've got a good friend who's clever?"`);
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    ((s as any).NatbelQW = (s as any).NatbelQW ?? {})['mother_talk'] = 2;
    qspCall(s, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/mom/prost2.jpg');
    ...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} turns to you and sighs. "Okay ${((s as any).pcs_nickname || '')}. I guess we should clear the air."`);
    ...
    scene.text(`"I know, ${((s as any).pcs_nickname || '')}. I also know that it would have been very difficult for you to tell me this, but thank you for having the courage to do so."`);
