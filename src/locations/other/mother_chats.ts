...
function enterRapeTalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: You shake your head slowly. "No <<$npc_nickname[''A29'']>>, I just wanted to tel...
    scene.text(`You shake your head slowly. "No ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}, I just wanted to tell you. I guess to get it off my chest. Thank you for listening."`);
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> looks at you with a sad smile. "Whatever you wan...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you with a sad smile. "Whatever you want darling, just let me know. I haven't really done anything."`);
    // TODO-QSP: dynamic text: You smile at her. "Yes, you did <<$npc_nickname[''A29'']>>! You listened and gav...
    scene.text(`You smile at her. "Yes, you did ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}! You listened and gave me a hug, which was what I needed."`);
    ...
    // TODO-QSP: dynamic text: As you eventually calm down, your <<$npc_nickname[''A29'']>> looks at you as she...
    scene.text(`As you eventually calm down, your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you as she strokes your face and smiles. "I'm glad you spoke to me about this rather than carrying the burden by yourself. If you want to talk more about this or anything else, I'm here for you; I love you and will support you. <i>Always</i>!"`);
    ...
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> looks at you with concern. "Well, did he do anyt...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you with concern. "Well, did he do anything else? I mean… Sorry, this isn't coming out right."`);
    ...
    ((st as any).mc_inventory = (st as any).mc_inventory ?? {})['morning_after_pill'] = ((st as any).mc_inventory['morning_after_pill'] ?? 0) + (1);
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
    ...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you with mounting concern. "I guess we should also report it to the police and most importantly get you checked over at the clinic as you don't know if he's got anything unpleasant. I'll come with you if you want so you don't have to go through that on your own."`);
    scene.actions([
    ...
    scene.text('Tears well up in your eyes again and she pulls you back into her arms and cuddles you, holding you tight as you both let out all your pain and anguish.');
    // TODO-QSP: dynamic text: As you calm down, your <<$npc_nickname[''A29'']>> looks at you as she strokes yo...
    scene.text(`As you calm down, your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you as she strokes your face and smiles. "At least you talked to me about this rather than keeping it all bottled up inside."`);
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> makes you both a cup of tea and shuts the kitche...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} makes you both a cup of tea and shuts the kitchen door. As you drink it, she spends some time focusing on and comforting you.`);
    ...
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> looks as though she''s going to say something, b...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks as though she's going to say something, but has second thoughts and just shakes her head sadly.`);
    ...
    // TODO-QSP: dynamic text: As you calm down, your <<$npc_nickname[''A29'']>> looks at you as she strokes yo...
    scene.text(`As you calm down, your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you as she strokes your face and smiles. "Okay, how about you get yourself freshened up and I'll wait here for you. I'll go to the clinic with you so you don't need to do all this by yourself."`);
    scene.actions([
    ...
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> looks at you and smiles. "Of course, if that''s ...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you and smiles. "Of course, if that's what you want, darling."`);
    // TODO-QSP: dynamic text: A little while later, you return to your <<$npc_nickname[''A29'']>> and tell her...
    scene.text(`A little while later, you return to your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} and tell her that, while the doctor didn't find anything, you have to come back soon for a second check.`);
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> looks at you. "If you want me to go with you, pl...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you. "If you want me to go with you, please just let me know."`);
    // TODO-QSP: dynamic text: "Thanks <<$npc_nickname[''A29'']>>, but I think I''d rather just go by myself. T...
    scene.text(`"Thanks ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}, but I think I'd rather just go by myself. The embarrassment of letting the doctor examine me down there after what happened…"`);
    scene.text('Tears well up in your eyes again and she pulls you back into her arms and cuddles you, holding you tight as you both let out all your pain and anguish.');
    // TODO-QSP: dynamic text: As you calm down, your <<$npc_nickname[''A29'']>> looks at you as she strokes yo...
    scene.text(`As you calm down, your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks at you as she strokes your face and smiles. "At least you talked to me about this rather than carrying the burden by yourself. Let's go home."`);
    scene.actions([
    ...
    // TODO-QSP: dynamic text: You both go return home, where your <<$npc_nickname[''A29'']>> leads you into th...
    scene.text(`You both go return home, where your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} leads you into the kitchen, shuts the door and makes you both a cup of tea. As you drink it, she spends some time focusing on and comforting you.`);
...
function enterMiscarriageTalk1(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "<<$npc_nickname[''A29'']>>," you start hesitantly. "I need to tell you somethin...
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 10;
    (st as any).will_counter = ((st as any).will_counter ?? 0) + (20);
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/mom/hug_sad1.jpg');
...
function enterLostVirginityTalk(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "I wanted the best for you and your sister!" your <<$npc_nickname[''A29'']>> sho...
  ...
  if ((!((s as any).momslut ?? 0))) {
    ...
    ((st as any).mother = (st as any).mother ?? {})['kickedout_timer'] = ((st as any).daystart ?? 0);
    scene.img('images/characters/pavlovsk/resident/mom/walking.jpg');
    scene.text('"If you don\'t want me at home, then I won\'t be!" you scream.');
    qspCall(st, 'mother', 'get_belongings');
    // TODO-QSP: dynamic text: You turn and leave, slamming the door behind you. You''ll need to find somewhere...
    scene.text(`You turn and leave, slamming the door behind you. You'll need to find somewhere to live until your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} calms down. Maybe Vlad or Anya will have a word with her when they see you aren't home? You could try your uncle upstairs or the hotel for now.`);
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'mother', 'get_belongings');
    // TODO-QSP: dynamic text: You turn and leave, slamming the door behind you. You''ll need to find somewhere...
    scene.text(`You turn and leave, slamming the door behind you. You'll need to find somewhere to live at least until your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} calms down. Maybe Vlad or Anya will have a word with her when they see you aren't home? You could try your uncle upstairs or the hotel for now.`);
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Return to Five Eight', goto: ['pav_complex', 'start'] },
...
function enterReconciliationTalk(s: GameState, scene: SceneBuilder): void {
  // TODO-QSP: end
  scene.actions([
    { label: '', labelFn: (s: GameState) => 'Approach your ' + String(((s as any).npc_nickname ?? 0)?.['A29'] ?? '' ?? ''), handler: (st: GameState) => {
    ((st as any).mother = (st as any).mother ?? {})['kickedout_timer'] = 0;
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'npc_relationship', 'modify', 'A29', 'love');
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/mom/stop_talk.jpg');
    ...
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> picks a table in a corner of the café and waits ...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} picks a table in a corner of the café and waits for you to sit down. You notice that she looks very sad as you walk up to her.`);
    ...
    // TODO-QSP: dynamic text: There''s another pause as your <<$npc_nickname[''A29'']>> reflects on your respo...
    scene.text(`There's another pause as your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} reflects on your response.`);
    ...
    qspCall(st, 'homes_properties', 'give_access', 'parents_home');
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'npc_relationship', 'modify', 'A29', 'love');
    qspCall(st, 'stat', '');
    ...
    scene.text(`You stand up and grab your bag. "I'm glad you apologised ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}, but I don't know if I want to come back home since I can see something similar happening again in the future. Thanks for the tea."`);
    // TODO-QSP: dynamic text: You turn and silenty leave your unfinished tea and your <<$npc_nickname[''A29'']...
    scene.text(`You turn and silenty leave your unfinished tea and your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} behind.`);
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
  } },
...
function enterReconciliationReplies(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: "Don''t think this means that I''m going to stop having sex though, <<$npc_nickn...
    scene.text(`"Don't think this means that I'm going to stop having sex though, ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}," you say, your voice muffled by her tight hug. She pulls back suddenly, staring at you with wide eyes. "I'm never going to be able to go back to being a virgin, so you're going to have to accept that and understand that sex is a part of my life now."`);
...
function enterKnock(s: GameState, scene: SceneBuilder): void {
  if (((s as any).university ?? 0)?.['student'] === 1) {
    qspGoto(s, 'mother_chats', 'knock_uni');
  }
  (s as any).minut = ((s as any).minut ?? 0) + 5;
  qspCall(s, 'stat', '');
  ...
  scene.actions([
    ...
    if (((st as any).mother ?? 0)?.['kickedout_visit'] === 0) {
      ((st as any).mother = (st as any).mother ?? {})['kickedout_visit'] = 1;
      qspCall(st, 'npc_relationship', 'modify', 'A29', 10);
      // TODO-QSP: dynamic text: "I''m not moving back in, <<$npc_nickname[''A29'']>>. I have my own place now an...
      scene.text(`"I'm not moving back in, ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}. I have my own place now and just wanted to visit to see if we can at least get back on talking terms."`);
      // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> looks sad. "Oh... Well, wouldn''t you reconsider...
      scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} looks sad. "Oh... Well, wouldn't you reconsider? I... No, <i>we</i> miss you."`);
      ...
      scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} gives you another hug before you leave and you promise to visit again soon before you depart.`);
    } else {
    ...
    ((st as any).mother = (st as any).mother ?? {})['kickedout_pending'] = 0;
    qspCall(st, 'homes_properties', 'give_access', 'parents_home');
    qspCall(st, 'npc_relationship', 'modify', 'A29', 10);
    (st as any).minut = ((st as any).minut ?? 0) + 25;
    qspCall(st, 'stat', '');
...
function enterKnockUni(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "Thanks <<$npc_nickname[''A29'']>>," you reply as you get up and leave the kitch...
...
function enterLoverTalks(s: GameState, scene: SceneBuilder): void {
  if (((s as any).NatbelQW ?? 0)?.['FriendLover'] >= 8) {
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterNatbel(s, scene); (s as any).locArgs = __savedLocArgs; }
  }
  // TODO-QSP: end
  scene.build();
}
...
function enterNatbel(s: GameState, scene: SceneBuilder): void {
  if (((s as any).loc ?? 0) === 'kuhrPar'  &&  qspFunc(s, 'homes_properties', 'is_current_home', 'parents_home')) {
    if (((s as any).NatBelQW ?? 0)?.['mother_talk'] === 0) {
      scene.actions([
        { label: '', labelFn: (s: GameState) => 'Tell your ' + String(((s as any).npc_nickname ?? 0)?.['A29'] ?? '' ?? '') + ' about Natasha', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 10;
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['mother_talk'] = 1;
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['mother_talkday'] = ((st as any).daystart ?? 0);
    ...
    scene.text(`You decide that you need to get this over with since the longer it goes on, the harder it'll get and the more likely your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} will find out by accident.`);
    // TODO-QSP: dynamic text: "<<$npc_nickname[''A29'']>>, I need to speak to you about something..." you mumb...
    scene.text(`"${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}, I need to speak to you about something..." you mumble slightly.`);
    // TODO-QSP: dynamic text: Your <<$npc_nickname[''A29'']>> doesn''t reply, so you continue. "I need to tell...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} doesn't reply, so you continue. "I need to tell you something that's important to me; I've become very good friends with a girl who lives upstairs. She's been helping me with my studies and homework. You might have noticed my grades have improved recently..."`);
    // TODO-QSP: dynamic text: You pause and your <<$npc_nickname[''A29'']>> smiles at you, obviously pleased t...
    ...
    scene.text(`Your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} explodes. "WHAT?! NO!!! After all we've done to bring you up properly, you go off and spoil yourself... And with a girl! Aaargh! Just... Get out of my sight! I can't speak to you right now or I'll say something I'll regret!"`);
    ...
    scene.actions([
      ...
      if (((s as any).NatbelQW ?? 0)?.['mother_talk'] === 1  &&  ((s as any).daystart ?? 0) > ((s as any).NatbelQW ?? 0)?.['mother_talkday']) {
        scene.actions([
          { label: '', labelFn: (s: GameState) => String(((s as any).npc_nickname ?? 0)?.['A29'] ?? '' ?? '') + ' reconciliation', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['mother_talk'] = 2;
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/mom/prost2.jpg');
    ...
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/mom/prost1.jpg');
    // TODO-QSP: dynamic text: You smie, happy that your <<$npc_nickname[''A29'']>> is no longer so angry about...
    scene.text(`You smie, happy that your ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''} is no longer so angry about the situation.`);
    // TODO-QSP: dynamic text: "Thanks <<$npc_nickname[''A29'']>>. It''s still early days in our relationship, ...
    scene.text(`"Thanks ${((st as any).npc_nickname ?? 0)?.['A29'] ?? ''}. It's still early days in our relationship, but I didn't want to keep it a secret from you in case you found out from someone else, and that would have only made matters worse."`);
