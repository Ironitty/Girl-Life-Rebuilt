...
function enterOldman(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    dynamicGoto(st, 'prevLoc', 'prevArg');
      ...
      { label: 'Agree', handler: (st: GameState) => {
    qspCall(st, 'npcgeneratec', '', 0, 'Old man', (Math.floor(Math.random() * 16) + 52));
    qspCall(st, 'boyStat', '', ((st as any).npclastgenerated ?? 0));
    qspCall(st, 'pain', '', 3, 'vaginal', 'ache');
    qspCall(st, 'arousal', 'vaginal', 15, 'rough', 'unknown');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'cum_call', 'labia', 'Old guy');
    qspCall(st, 'cum_call', 'butt', 'Old guy');
    qspCall(st, 'cum_call', 'legs', 'Old guy');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Return to the station', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
...
function enterFit(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal_finger', (-15), 'unknown');
    scene.actions([
    ...
    qspCall(st, 'pain', '', 1, 'legL', 'ache');
    qspCall(st, 'pain', '', 1, 'legR', 'ache');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'end');
    qspCall(st, 'cum_call', 'mouth_swallow', 'Fit guy');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Return to the station', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
...
function enterYoung(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'npcgeneratec', '', 0, 'Well dress guy', (Math.floor(Math.random() * 4) + 18));
    qspCall(st, 'boyStat', '', ((st as any).npclastgenerated ?? 0));
    ...
    qspCall(st, 'arousal', 'anal', 10, 'sub', 'unknown');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Take his cum', handler: (st: GameState) => {
    ...
    qspCall(st, 'arousal', 'end');
    qspCall(st, 'cum_call', 'mouth_swallow', 'Well dress guy');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterBreastfeed(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).lactation ?? 0)?.['active'] > 0  &&  ((s as any).pcs_inhib ?? 0) >= 30) {
    ...
    dynamicGoto(st, 'prevLoc', 'prevArg');
      ...
      if (((st as any).vidage ?? 0) > 16  &&  ((st as any).vidage ?? 0) < 20) {
    ...
    dynamicGoto(st, 'prevLoc', 'prevArg');
          ...
          if (((st as any).tits ?? 0) === 3) {
            ...
            if (((st as any).tits ?? 0) === 4  ||  ((st as any).tits ?? 0) === 5) {
...
function enterBreastfeed1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.img('images/locations/shared/train/breastfeeding/train_breastfeeding.mp4');
    if (((st as any).temp_var ?? 0) > 15) {
      (st as any).milkedvolume = qspFunc(s, 'lact_lib', '$get_breastmilk', 2, 15);
      ...
      if (((st as any).milkedvolume ?? 0) >= 50000) {
...
function enterWeed(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if ((Math.floor(Math.random() * 100) + 0) < 5) {
      qspGoto(st, 'train_incidental', 'events');
    }
    qspGoto(st, 'train', 'center');
  } },
    ]);
  } },
    ]);
    ...
    qspCall(st, 'npcgeneratec', '', 0, 'Stoner', (Math.floor(Math.random() * 7) + 18));
    qspCall(st, 'boyStat', '', ((st as any).npclastgenerated ?? 0));
    ...
    qspCall(st, 'arousal', 'end');
    qspCall(st, 'cum_call', 'face', 'Stoner');
    qspCall(st, 'cum_call', 'mouth_swallow', 'Stoner');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Board the train to the city', handler: (st: GameState) => {
    ((st as any).temp_transportVars = (st as any).temp_transportVars ?? {})['timecost'] = qspFunc(s, 'transport_functions', 'get_train_timecost', 'pavlovsk', 'center');
...
function enterPrivate(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', (-10), 'prostitution', 'sub', 'unknown');
    ...
    qspCall(st, 'arousal', 'vaginal', (-15), 'prostitution', 'sub', 'unknown');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'anal', (-5), 'prostitution', 'sub', 'unknown');
    qspCall(st, 'dinSex', 'std_trigger');
    scene.actions([
      { label: 'Finished', handler: (st: GameState) => {
    qspCall(st, 'money', 'earn', 500);
    qspCall(st, 'cum_call', 'anus', ((st as any).boy ?? 0), 0, '', '', 20);
...
function enterGopnik(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'npcgeneratec', '', 0, 'Gopnik', (Math.floor(Math.random() * 8) + 18));
    qspCall(st, 'boyStat', '', ((st as any).npclastgenerated ?? 0));
    ...
    qspCall(st, 'arousal', 'foreplay', 5, 'unknown', 'exhibitionism');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'cum_call', 'anus', 'Gopnik');
    qspCall(st, 'stat', '');
    scene.actions([
