import { qspCall, qspFunc, dynamicGoto, qspGoto } from '../_shared/qspBridge';

// AUTO-GENERATED FILE — DO NOT EDIT, fix the transpiler (scripts/qsp-transpile)
import type { GameState, ActionDef, LocationDef } from '../../core/types';
import type { SceneBuilder } from '../../core/scene';

function enterDefault(s: GameState, scene: SceneBuilder): void {
  scene.build();
}

function enterStart(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'stat', '');
  scene.img('images/shared/home/bedroom/bed2.jpg');
  scene.text('<center><h4>Enter the time using the number pad and click on set to change alarm times, don\'t forget to turn your alarm on.</h></center>');
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterBuildTable(s, scene); (s as any).locArgs = __savedLocArgs; }
  scene.actions([
    { label: 'Finish', handler: (st: GameState) => {
    (st as any).bud1 = 0;
    (st as any).bud2 = 0;
    (st as any).bud3 = 0;
    (st as any).bud4 = 0;
    (st as any).numbud = 0;
    qspGoto(st, 'alarmclock', 'fin');
  } },
    { label: 'Clear time display', handler: (st: GameState) => {
    (st as any).bud1 = 0;
    (st as any).bud2 = 0;
    (st as any).bud3 = 0;
    (st as any).bud4 = 0;
    (st as any).numbud = 0;
    qspGoto(st, 'alarmclock', 'start');
  } },
  ]);
  scene.build();
}

function enterAlarmDisplay(s: GameState, scene: SceneBuilder): void {
  (s as any).result = '<b>' + qspFunc(s, 'time', 'get_time_string', ((s as any).locArgs?.[1] ?? 0), ((s as any).locArgs?.[2] ?? 0), (((s as any).cheatVars ?? 0)?.['time_format'])) + '</b>';
  scene.build();
}

function enterGetWeekDisplay(s: GameState, scene: SceneBuilder): void {
  (s as any).result = '<b>' + qspFunc(s, 'time', 'get_time_string', (((s as any).alarmVars ?? 0)?.['timerH']), (((s as any).alarmVars ?? 0)?.['timerM']), (((s as any).cheatVars ?? 0)?.['time_format'])) + '</b>';
  scene.build();
}

function enterGetWeekendDisplay(s: GameState, scene: SceneBuilder): void {
  (s as any).result = '<b>' + qspFunc(s, 'time', 'get_time_string', (((s as any).alarmVars ?? 0)?.['timerEndH']), (((s as any).alarmVars ?? 0)?.['timerEndM']), (((s as any).cheatVars ?? 0)?.['time_format'])) + '</b>';
  scene.build();
}

function enterBaseAlarmclockText(s: GameState, scene: SceneBuilder): void {
  (s as any).result = 'Your <a href="#" onclick="window.__gameStore.getState().doGoto(\u0027\u0027alarmclock\u0027\u0027, \u0027\u0027start\u0027\u0027); return false;">alarm clock</a> is set to ' + qspFunc(s, 'alarmclock', 'get_week_display') + ' during the week and ' + qspFunc(s, 'alarmclock', 'get_weekend_display') + ' during the weekend. It is currently turned \' + iif(alarmVars[\'alarmOn\'] = 0, \'<a href="#" onclick="window.__gameStore.setState((s) => { /* TODO-QSP: alarmVars[\u0027\u0027alarmOn\u0027\u0027] = 1 */ return s; }); window.__gameStore.getState().doGoto(window.__gameStore.getState().prevLoc, window.__gameStore.getState().prevArg); return false;">OFF</a>.\', \'<a href="#" onclick="window.__gameStore.setState((s) => { /* TODO-QSP: alarmVars[\u0027\u0027alarmOn\u0027\u0027] = 0 */ return s; }); window.__gameStore.getState().doGoto(window.__gameStore.getState().prevLoc, window.__gameStore.getState().prevArg); return false;">ON</a>.\'';
  scene.build();
}

function enterClear(s: GameState, scene: SceneBuilder): void {
  (s as any).bud1 = 0;
  (s as any).bud2 = 0;
  (s as any).bud3 = 0;
  (s as any).bud4 = 0;
  (s as any).numbud = 0;
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enterTime(s: GameState, scene: SceneBuilder): void {
  ((s as any).alarmVars = (s as any).alarmVars ?? {})['alarmOn'] = 1;
  if (((s as any).bud_weekend ?? 0) === 1) {
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerEndH'] = ((s as any).bud1 ?? 0) * 10 + ((s as any).bud2 ?? 0);
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerEndM'] = ((s as any).bud3 ?? 0) * 10 + ((s as any).bud4 ?? 0);
    if (((s as any).alarmVars ?? 0)?.['timerEndH'] > 23) {
      ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerEndH'] = 0;
    }
    if (((s as any).alarmVars ?? 0)?.['timerEndM'] > 60) {
      ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerEndM'] = 0;
    }
  } else {
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerH'] = ((s as any).bud1 ?? 0) * 10 + ((s as any).bud2 ?? 0);
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerM'] = ((s as any).bud3 ?? 0) * 10 + ((s as any).bud4 ?? 0);
    if (((s as any).alarmVars ?? 0)?.['timerH'] > 23) {
      ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerH'] = 0;
    }
    if (((s as any).alarmVars ?? 0)?.['timerM'] > 60) {
      ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerM'] = 0;
    }
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter1(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 1;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 1;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 1;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 1;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter2(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 2;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 2;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 2;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 2;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter3(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 3;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 3;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 3;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 3;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter4(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 4;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 4;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 4;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 4;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter5(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 5;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 5;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 5;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 5;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter6(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 6;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 6;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 6;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 6;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter7(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 7;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 7;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 7;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 7;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter8(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 8;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 8;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 8;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 8;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter9(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 9;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 9;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 9;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 9;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enter0(s: GameState, scene: SceneBuilder): void {
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).bud4 = 0;
    (s as any).numbud = 4;
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).bud3 = 0;
    (s as any).numbud = 3;
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).bud2 = 0;
    (s as any).numbud = 2;
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).bud1 = 0;
    (s as any).numbud = 1;
  }
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enterReset(s: GameState, scene: SceneBuilder): void {
  if (((s as any).bud_weekend ?? 0) === 1) {
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerEndH'] = 0;
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerEndM'] = 0;
  } else {
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerH'] = 0;
    ((s as any).alarmVars = (s as any).alarmVars ?? {})['timerM'] = 0;
  }
  (s as any).bud1 = 0;
  (s as any).bud2 = 0;
  (s as any).bud3 = 0;
  (s as any).bud4 = 0;
  (s as any).numbud = 0;
  qspGoto(s, 'alarmclock', 'start');
  scene.build();
}

function enterFin(s: GameState, scene: SceneBuilder): void {
  dynamicGoto(s, 'menu_loc', 'menu_arg');
  scene.build();
}

function enterBuildTable(s: GameState, scene: SceneBuilder): void {
  (s as any).num_html = '<td align="center"><b><font size="+2">';
  (s as any).html_num = '</a></font></b></td>';
  (s as any).more_html = '<center><table width="20%" cellspacing=0 cellpadding=3 align="center" border=1><tr align="center"><th COLSPAN=3 align="center"><font size="+3">';
  (s as any).even_more_html = '</font></th></tr>';
  if (((s as any).numbud ?? 0) === 4) {
    (s as any).alarm_html = '' + ((s as any).bud1 ?? 0) + '' + ((s as any).bud2 ?? 0) + ' : ' + ((s as any).bud3 ?? 0) + '' + ((s as any).bud4 ?? 0) + '';
  }
  if (((s as any).numbud ?? 0) === 3) {
    (s as any).alarm_html = '' + ((s as any).bud1 ?? 0) + '' + ((s as any).bud2 ?? 0) + ' : ' + ((s as any).bud3 ?? 0) + '<u>' + ((s as any).bud4 ?? 0) + '</u>';
  }
  if (((s as any).numbud ?? 0) === 2) {
    (s as any).alarm_html = '' + ((s as any).bud1 ?? 0) + '' + ((s as any).bud2 ?? 0) + ' : <u>' + ((s as any).bud3 ?? 0) + '</u>' + ((s as any).bud4 ?? 0) + '';
  }
  if (((s as any).numbud ?? 0) === 1) {
    (s as any).alarm_html = '' + ((s as any).bud1 ?? 0) + '<u>' + ((s as any).bud2 ?? 0) + '</u> : ' + ((s as any).bud3 ?? 0) + '' + ((s as any).bud4 ?? 0) + '';
  }
  if ((!((s as any).numbud ?? 0))) {
    (s as any).alarm_html = '<u>' + ((s as any).bud1 ?? 0) + '</u>' + ((s as any).bud2 ?? 0) + ' : ' + ((s as any).bud3 ?? 0) + '' + ((s as any).bud4 ?? 0) + '';
  }
  (s as any).numbud_table = '<tr><td colspan=3 align="center"><b><font size="+2"><a href="#" onclick="window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027clear\u0027); return false;">Clear</a></font></b></td></tr>';
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('<tr>' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'1\'">1' + ((s as any).html_num ?? 0) + '');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'2\'">2' + ((s as any).html_num ?? 0) + '');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'3\'">3' + ((s as any).html_num ?? 0) + '</tr>');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('<tr>' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'4\'">4' + ((s as any).html_num ?? 0) + '');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'5\'">5' + ((s as any).html_num ?? 0) + '');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'6\'">6' + ((s as any).html_num ?? 0) + '</tr>');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('<tr>' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'7\'">7' + ((s as any).html_num ?? 0) + '');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'8\'">8' + ((s as any).html_num ?? 0) + '');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('' + ((s as any).num_html ?? 0) + '<a href="exec:gt \'alarmclock\', \'9\'">9' + ((s as any).html_num ?? 0) + '</tr>');
  (s as any).numbud_table = ((s as any).numbud_table ?? 0) + ('<tr><td colspan=3 align="center"><b><font size="+2"><a href="#" onclick="window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u00270\u0027); return false;">0</a></font></b></td></tr></table></center>');
  (s as any).alarmVars_table = '<center><table border=0 cellpadding=0 cellspacing=5><font size="+1"<b>';
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<tr><td><b><a href="#" onclick="window.__gameStore.setState((s) => { s.bud_weekend = 0; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027Reset\u0027); return false;">Reset weekday alarm</a></b></td>');
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<td><b><a href="#" onclick="window.__gameStore.setState((s) => { s.bud_weekend = 0; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027Time\u0027); return false;">Set weekday alarm</a></b></td>');
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<td><b>Currently set for ' + qspFunc(s, 'time', 'get_time_string', (((s as any).alarmVars ?? 0)?.['timerH']), (((s as any).alarmVars ?? 0)?.['timerM']), (((s as any).cheatVars ?? 0)?.['time_format'])) + '</b></td></tr>');
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<tr><td><b><a href="#" onclick="window.__gameStore.setState((s) => { s.bud_weekend = 1; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027Reset\u0027); return false;">Reset weekend alarm</a></b></td>');
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<td><b><a href="#" onclick="window.__gameStore.setState((s) => { s.bud_weekend = 1; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027Time\u0027); return false;">Set weekend alarm</a></b></td>');
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<td><b>Currently set for ' + qspFunc(s, 'time', 'get_time_string', (((s as any).alarmVars ?? 0)?.['timerEndH']), (((s as any).alarmVars ?? 0)?.['timerEndM']), (((s as any).cheatVars ?? 0)?.['time_format'])) + '</b></td></tr>');
  if (((s as any).start_type ?? 0)?.['loc'] === 'sg') {
    (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<tr><td colspan="3"><b>Use weekend alarm for holidays - ' + ((((s as any).alarmVars ?? 0)?.['alarm_holiday'] === 0) ? ('<a href="#" onclick="window.__gameStore.setState((s) => { (s.alarmVars ??= {})\u0027alarm_holiday\u0027 = 1; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027start\u0027); return false;">OFF</a>') : ('<a href="#" onclick="window.__gameStore.setState((s) => { (s.alarmVars ??= {})\u0027alarm_holiday\u0027 = 0; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027start\u0027); return false;">ON</a>')) + '</b></td></tr>');
  }
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('<tr><td colspan="3"><b>Your alarm is currently turned ' + ((((s as any).alarmVars ?? 0)?.['alarmOn'] === 0) ? ('<a href="#" onclick="window.__gameStore.setState((s) => { (s.alarmVars ??= {})\u0027alarmOn\u0027 = 1; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027start\u0027); return false;">OFF</a>') : ('<a href="#" onclick="window.__gameStore.setState((s) => { (s.alarmVars ??= {})\u0027alarmOn\u0027 = 0; return s; }); window.__gameStore.getState().doGoto(\u0027alarmclock\u0027, \u0027start\u0027); return false;">ON</a>')) + '.</b></td></tr>');
  if (((s as any).alarmVars ?? 0)?.['alarmOn'] === 1) {
    (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('</table></center><center><table border=0 cellpadding=0 cellspacing=10><tr><td><b>Your Alarm is ' + ((((s as any).alarmVars ?? 0)?.['timerH'] <= 0  &&  ((s as any).alarmVars ?? 0)?.['timerM'] <= 0) ? ('not set') : ('set to ' + qspFunc(s, 'time', 'get_time_string', (((s as any).alarmVars ?? 0)?.['timerH']), (((s as any).alarmVars ?? 0)?.['timerM'])))) + ' for the weekdays ' + ((((s as any).alarmVars ?? 0)?.['timerEndH'] <= 0  &&  ((s as any).alarmVars ?? 0)?.['timerEndM'] <= 0) ? ('and not set') : ('and set to ' + qspFunc(s, 'time', 'get_time_string', (((s as any).alarmVars ?? 0)?.['timerEndH']), (((s as any).alarmVars ?? 0)?.['timerEndM'])))) + ' for the weekend' + ((((s as any).alarmVars ?? 0)?.['alarm_holiday'] === 1) ? (' and holidays') : ('')) + '.</b></td></tr>');
  }
  (s as any).alarmVars_table = ((s as any).alarmVars_table ?? 0) + ('</b></font></table></center>');
  scene.text(`${((s as any).more_html ?? '')}${((s as any).alarm_html ?? '')}${((s as any).even_more_html ?? '')}${((s as any).numbud_table ?? '')}`);
  scene.text(`${((s as any).alarmVars_table ?? '')}`);
  (s as any).num_html = undefined;
  (s as any).html_num = undefined;
  (s as any).alarm_html = undefined;
  (s as any).numbud_table = undefined;
  (s as any).more_html = undefined;
  (s as any).even_more_html = undefined;
  (s as any).alarmVars_table = undefined;
  scene.build();
}

function enter(s: GameState, scene: SceneBuilder): void {
  const arg = s.locArg;
  switch (arg) {
    case 'start':
      enterStart(s, scene);
      break;
    case 'alarm_display':
      enterAlarmDisplay(s, scene);
      break;
    case 'get_week_display':
      enterGetWeekDisplay(s, scene);
      break;
    case 'get_weekend_display':
      enterGetWeekendDisplay(s, scene);
      break;
    case 'base_alarmclock_text':
      enterBaseAlarmclockText(s, scene);
      break;
    case 'clear':
      enterClear(s, scene);
      break;
    case 'Time':
      enterTime(s, scene);
      break;
    case '1':
      enter1(s, scene);
      break;
    case '2':
      enter2(s, scene);
      break;
    case '3':
      enter3(s, scene);
      break;
    case '4':
      enter4(s, scene);
      break;
    case '5':
      enter5(s, scene);
      break;
    case '6':
      enter6(s, scene);
      break;
    case '7':
      enter7(s, scene);
      break;
    case '8':
      enter8(s, scene);
      break;
    case '9':
      enter9(s, scene);
      break;
    case '0':
      enter0(s, scene);
      break;
    case 'Reset':
      enterReset(s, scene);
      break;
    case 'fin':
      enterFin(s, scene);
      break;
    case 'build_table':
      enterBuildTable(s, scene);
      break;
    default:
      enterDefault(s, scene);
      break;
  }
}

export const alarmclock: LocationDef = {
  name: 'alarmclock',
  title: '<center><h4>Enter the time using the number pad and click on',
  region: 'other',
  enter: enter,
};
