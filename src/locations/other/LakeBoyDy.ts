...
function enterRide(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).npc_QW ?? 0)?.['A113'] === 0  &&  (!((s as any).belgangday ?? 0))) {
      ...
      scene.text(`You're still a bit intimidated by his presence, and just say your name: "${((s as any).pcs_nickname || '')}."`);
    }
...
function enterSlap(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You're wincing in pain now, and are at a loss for words. Yes, you do think you're better than him, but obviously you can't say that. Meanwhile, he digs through your personal items and finds your passport: "${((s as any).pcs_nickname || '')} ${((s as any).pcs_lastname || '')}, living in Pavlovsk… interesting." He pockets your passport. Then he turns back to you and says in a menacing voice: "You know what, ${((s as any).pcs_nickname || '')}… you remind me of this girl I met the other day. Right here, on this same beach. She was being rude to me, not even as much as you are now. I think I broke her hand by accident."`);
    scene.actions([
    ...
    scene.text(`"Excellent. I'll have you know, ${((s as any).pcs_nickname || '')}: if you do not pay me ${qspFunc(s, 'money', 'string_debt_addition', ((s as any).belgangPayWeekAmount || ''))} each week until you paid of your debts, you're going to earn them for me one way or the other. I know just the place. For your sake: make sure you bring the money."`);
    ...
    scene.text(`The bald guy looks at you indifferently and yawns: "That's really not my problem, ${((s as any).pcs_nickname || '')}. But you know what, I'm a nice guy," he says with a fake smile. "I will reduce it to ${qspFunc(s, 'money', 'string_debt_addition', ((s as any).belgangPayWeekAmount || ''))} each week and I'll get you started."`);
    ...
    // TODO-QSP: dynamic text: The bald guy looks at you indifferently and yawns: "That's really not my problem...
    scene.text(`The bald guy looks at you indifferently and yawns: "That's really not my problem, ${((s as any).pcs_nickname || '')}. But you know what, I'll get you started."`);
    scene.text('He pulls down his trunks, revealing his flaccid cock. His associate grabs you by the hair and pushes your head down towards the fat man\'s groin: "You want some money, bitch? Go ahead… !"');
    scene.actions([
      { label: 'Suck him off', goto: ['LakeBoyDy', 'sucksasha'] },
    ]);
