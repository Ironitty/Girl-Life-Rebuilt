...
function enterSlap(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    ((st as any).npc_QW = (st as any).npc_QW ?? {})['A113'] = 1;
    (st as any).daybelisex = ((st as any).daystart ?? 0);
    qspCall(st, 'money', 'debt_add', 'belgangPay', Math.max(((st as any).money ?? 0) * 5, 50000));
    qspCall(st, 'stat', '');
    if (((st as any).week ?? 0) === 7) {
      (st as any).belfirstweek = 0;
    } else {
      if (((st as any).week ?? 0) >= 1  &&  ((st as any).week ?? 0) <= 2) {
        (st as any).belfirstweek = 0;
      } else {
        (st as any).belfirstweek = 1;
      }
    }
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedorev/Strela/vadimbely.jpg');
    ...
    qspCall(st, 'stat', '');
    (st as any).belgangday = ((st as any).daystart ?? 0);
    (st as any).belgangPayWeekAmount = 5000;
    (st as any).belgangPayWeek = 0;
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    qspCall(st, 'stat', '');
    (st as any).belgangPayWeekAmount = 4000;
    (st as any).belgangPayWeek = 0;
    (st as any).belgangPayextension = 0;
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedorev/Strela/vadimbely.jpg');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 2;
    (st as any).belgangPayWeekAmount = 3000;
    (st as any).belgangPayWeek = 0;
    (st as any).belgangPayextension = 0;
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedorev/Strela/vadimbely.jpg');
