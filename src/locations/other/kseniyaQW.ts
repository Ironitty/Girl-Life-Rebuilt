...
function enterEvent1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).trait_vars ?? 0)?.['exhibitionist'] < 1) {
    (s as any).trait_vars['exhibitionist_exp'] = qspUntranslated(s, "max(trait_vars['exhibitionist_exp'], 10)", { location: "kseniyaQW" });
...
function enterEvent2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).trait_vars ?? 0)?.['exhibitionist'] < 2) {
    (s as any).trait_vars['exhibitionist_exp'] = qspUntranslated(s, "max(trait_vars['exhibitionist_exp'], 50)", { location: "kseniyaQW" });
...
function enterEvent3_1(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "It's OK <<$pcs_firstname>>, I can take the pictures on my phone and message the...
  scene.text(`"It's OK ${((s as any).pcs_firstname ?? 0)}, I can take the pictures on my phone and message them to you. What's your number?"`);
  ...
  scene.actions([
    ...
    scene.text('You look at the card again, "Simply Salacious" just off Pushkin Okhlopkov Square. Open Mon-Sat, \'+func(\'time\', \'get_time_string\', 8, 0)+\' - \'+func(\'time\', \'get_time_string\', 18, 0)+\'');
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    dynamicGoto(st, 'loc', 'loc_arg');
  } },
