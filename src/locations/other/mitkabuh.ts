...
function enterDefault(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sunWeather ?? 0) === 1) {
    ...
    if (((st as any).locat ?? 0)?.['A60_loc'] === 'mitkabuh_group') {
      qspGoto(st, 'mitkabuh', 'partywithMira');
    } else {
      qspGoto(st, 'mitkabuh', 'partyalone');
    }
  } },
    ]);
  } else {
...
function enterPartyalone(s: GameState, scene: SceneBuilder): void {
  (s as any).loc = 'mitkabuh';
  (s as any).loc_arg = 'partyalone';
  ...
  if (((s as any).alko ?? 0) < 10) {
    ...
    if (((st as any).GadBoy ?? 0)?.['first_drink'] === 1) {
      ((st as any).GadBoy = (st as any).GadBoy ?? {})['first_drink'] = 2;
    ...
    if (((s as any).npc_QW ?? 0)?.['A63'] > 0  &&  ((s as any).GadBoy ?? 0)?.['river_gang'] < 3) {
      if (((s as any).GadBoy ?? 0)?.['drunk_event'] === 0) {
        ...
        if (((s as any).npc_QW ?? 0)?.['A63'] < 10) {
          qspGoto(s, 'mitkabuh', 'solo_1to9');
        } else {
          if (((s as any).npc_QW ?? 0)?.['A63'] === 10) {
            qspGoto(s, 'mitkabuh', 'solo_10');
          } else {
            (s as any).temp_rand = (Math.floor(Math.random() * 10) + 1);
            if (((s as any).temp_rand ?? 0) < 4) {
              qspGoto(s, 'mitkabuh', 'solo_11andup_1boy');
            } else {
              if (((s as any).temp_rand ?? 0) < 10) {
                qspGoto(s, 'mitkabuh', 'solo_11andup_2boys');
              } else {
                qspGoto(s, 'mitkabuh', 'solo_11andup_3boys');
              }
            }
          }
        }
...
function enterPartywithMira(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).MiraVars = (s as any).MiraVars ?? {})['follow_time'] = 3;
  ...
  if (((s as any).npc_drunk ?? 0)?.['A60'] >= 10  &&  ((s as any).alko ?? 0) >= 10  &&  ((s as any).GadBoy ?? 0)?.['drunk_event'] === 1  &&  ((s as any).MiraVars ?? 0)?.['drunk_event'] === 1) {
    ...
    if (((s as any).npc_drunk ?? 0)?.['A60'] >= 10  &&  ((s as any).MiraVars ?? 0)?.['drunk_event'] === 0) {
      ...
      if (((s as any).MiraVars ?? 0)?.['QW'] === 5) {
        qspGoto(s, 'mitkabuh', 'mira_5');
      } else {
        if (((s as any).MiraVars ?? 0)?.['QW'] === 7) {
          qspGoto(s, 'mitkabuh', 'mira_7');
        } else {
          if (((s as any).MiraVars ?? 0)?.['QW'] === 8) {
            qspGoto(s, 'mitkabuh', 'mira_8');
          } else {
            if (((s as any).MiraVars ?? 0)?.['QW'] === 9) {
              qspGoto(s, 'mitkabuh', 'mira_9');
            } else {
              if (((s as any).MiraVars ?? 0)?.['QW'] >= 11) {
                qspGoto(s, 'mitkabuh', 'mira_11andup');
              } else {
                scene.img('images/locations/gadukino/village/drinking.jpg');
    ...
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
                    ...
                    scene.actions([
                      { label: 'Leave', handler: (st: GameState) => {
    ((st as any).npc_drunk = (st as any).npc_drunk ?? {})['A60'] = 0;
    (st as any).minut = ((st as any).minut ?? 0) + 15;
    qspCall(st, 'willpower', 'drink', 'resist');
    qspCall(st, 'willpower', 'pay', 'resist');
    qspCall(st, 'stat', '');
...
function enterSolo_1to9(s: GameState, scene: SceneBuilder): void {
  ...
  if ((((s as any).temp_rand ?? 0) === 1  ||  (((s as any).temp_rand ?? 0) < 5  &&  ((s as any).stat ?? 0)?.['think_virgin'] === 0))  &&  (!((s as any).mesec ?? 0))) {
    ...
    if (((st as any).temp_randB ?? 0) === 5) {
      ...
      qspCall(st, 'cum_call', 'breasts', 'A63', 1);
    } else {
      if (((st as any).temp_randB ?? 0) >= 3) {
        ...
        qspCall(st, 'cum_call', 'face', 'A63', 1);
        qspCall(st, 'cum_call', 'mouth_swallow', 'A63', 1);
        qspCall(st, 'cum_call', 'hair', 'A63', 1);
      }
    }
    qspCall(st, 'arousal', 'end');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterSetContinueActs(st, scene); (st as any).locArgs = __savedLocArgs; }
...
function enterSolo_10(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A63', 1);
    qspCall(st, 'npc_relationship', 'modify', 'A61', (-5));
    ((st as any).npc_QW = (st as any).npc_QW ?? {})['A63'] = 11;
    qspCall(st, 'boyStat', 'A63');
    ((st as any).npc_had_sex = (st as any).npc_had_sex ?? {})['A63'] = 1;
...
function enterSolo_11andup_3boys(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A61', 1);
    if (((st as any).npc_QW ?? 0)?.['A63'] < 20) {
      ((st as any).npc_QW = (st as any).npc_QW ?? {})['A63'] = ((st as any).npc_QW['A63'] ?? 0) + (1);
    }
    qspCall(st, 'boyStat', 'A63');
    ...
    ((st as any).npc_had_sex = (st as any).npc_had_sex ?? {})['A62'] = 1;
    qspCall(st, 'boyStat', 'A61');
    ((st as any).npc_had_sex = (st as any).npc_had_sex ?? {})['A61'] = 1;
    ((st as any).stat = (st as any).stat ?? {})['gangbang_count'] = ((st as any).stat['gangbang_count'] ?? 0) + (1);
    ...
    qspCall(st, 'arousal', 'hj', 10, 'sub', 'gangbang', 'rough');
    qspCall(st, 'arousal', 'bj', 10, 'sub', 'gangbang', 'rough');
    if ((!((st as any).mesec ?? 0))) {
      qspCall(st, 'arousal', 'vaginal', 10, 'sub', 'gangbang', 'rough');
    } else {
      qspCall(st, 'arousal', 'anal', 10, 'sub', 'gangbang', 'rough');
    }
    qspCall(st, 'cum_call', 'breasts', 'A62', 1);
    qspCall(st, 'cum_call', 'stomach', 'A62', 1);
...
function enterMira_5(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'voyeur_sex', 15, 'sub');
    qspCall(st, 'stat', '');
...
function enterMira_7(s: GameState, scene: SceneBuilder): void {
  (s as any).minut = ((s as any).minut ?? 0) + 5;
  ((s as any).MiraVars = (s as any).MiraVars ?? {})['QW'] = 8;
  qspCall(s, 'stat', '');
  qspCall(s, 'miroslava', 'miraclothes');
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'erotic_nudity', 15, 'sub');
    qspCall(st, 'stat', '');
...
function enterMira_8(s: GameState, scene: SceneBuilder): void {
  (s as any).minut = ((s as any).minut ?? 0) + 5;
  ((s as any).MiraVars = (s as any).MiraVars ?? {})['QW'] = 9;
  qspCall(s, 'stat', '');
  qspCall(s, 'miroslava', 'miraclothes');
  scene.text('"I\'m bored!" Mitka yells. "Mira get your ass down and give us a show to remember!" he snorts like a pig. "Show us how how loose your holes are whore…"');
...
function enterMira_9(s: GameState, scene: SceneBuilder): void {
  (s as any).minut = ((s as any).minut ?? 0) + 5;
  ((s as any).MiraVars = (s as any).MiraVars ?? {})['QW'] = 10;
  qspCall(s, 'stat', '');
  qspCall(s, 'miroslava', 'miraclothes');
  ...
  scene.actions([
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'voyeur_sex', 10, 'sub');
    qspCall(st, 'stat', '');
    ...
    scene.actions([
      { label: 'Talk to Mira', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).MiraVars = (st as any).MiraVars ?? {})['QW'] = 11;
    qspCall(st, 'stat', '');
    qspCall(st, 'miroslava', 'miraclothes');
...
function enterMira_11andup(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!(Math.floor(Math.random() * 2) + 0))) {
    ...
    if (((s as any).temp_rand ?? 0) === 1) {
      scene.actions([
        { label: 'Watch her', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'voyeur_sex', 15, 'sub', 'group');
    qspCall(st, 'stat', '');
    ...
    if (((st as any).sunWeather ?? 0) === 1) {
      ...
      if (((s as any).temp_rand ?? 0) === 2) {
        scene.actions([
          { label: 'Watch her', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'voyeur_sex', 15, 'sub', 'goup');
    qspCall(st, 'stat', '');
