...
function enter10(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Soon the popcorn is long forgotten, as ${((s as any).telsob ?? 0)} works her way down between your legs. Her expert tongue along with the thrill of doing this in a movie theater brings you to orgasm almost immediately.`);
...
function enter11(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You rise up from your chair, lifting ${((s as any).telsob ?? 0)}'s shirt as you go. Your mouth finds one of her nipples, while you work your hand down her pants. After a minute of working your hand, ${((s as any).telsob ?? 0)} lets out a load moan, soaking your hand with her juices.`);
...
function enter12(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You slide down in you seat, and motion ${((s as any).telsob ?? 0)} upward. She quickly realizes what you want, and straddles your face. You raise your tongue up to meet her dripping wet pussy, and begin to lick.`);
...
function enter13(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You kneel down between ${((s as any).telsob ?? 0)}'s legs, and gently lap at her folds. As you reach her clitoris, thinking she might like the same thing, you gently begin to insert fingers one by one. Eventually she is stretched enough to envelope your entire hand. This brings a loud cry of pleasure from her, as her body shakes with an orgasm.`);
  qspCall(s, 'arousal', 'cuni_give', 10, 'lesbian');
  qspCall(s, 'arousal', 'vaginal_finger_give', (-10), 'lesbian');
  qspCall(s, 'stat', '');
  scene.actions([
...
function enter17(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You gradually work your fingers into her pussy, while you continue to lick her backside. Finger by finger you stretch her until your entire hand is inside her. ${((s as any).telsob ?? 0)} almost seems overwhelmed by all this attention. Her cries of pleasure are loud enough that you fear an usher might come in to see what's going. This thought gives you a wicked little thrill, but it's probably better to finish up.`);
  qspCall(s, 'arousal', 'cuni_give', 10, 'lesbian');
  qspCall(s, 'arousal', 'vaginal_finger_give', (-10), 'lesbian');
  qspCall(s, 'stat', '');
  scene.actions([
...
function enter2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).pcs_hotcat ?? 0) >= 5) {
      ...
      if ((!((s as any).kendra_theater ?? 0))) {
        if (((s as any).meet_kendra ?? 0) === 1) {
          ...
          scene.text(`While she is gone, Kendra plops right down in the chair next to you. She starts eating your popcorn and places a hand on your knee. Too stunned to say anything, you just sit there a few moments as she rubs your knee and eats the popcorn. "Hello again ${((s as any).pcs_nickname ?? 0)}, so what are you doing." She says as she looks at you like a piece of property.`);
        } else {
    ...
    if (((s as any).meet_kendra ?? 0) === 1) {
      ...
      scene.text(`Before she can say more ${((s as any).telsob ?? 0)} comes back and grabs her by the arm and pulls her up out of the chair. "She's with me, get out of here."`);
      // TODO-QSP: dynamic text: The Kendra jerks her arm free and then laughs at <<$telsob>>, before walking off...
      scene.text(`The Kendra jerks her arm free and then laughs at ${((s as any).telsob ?? 0)}, before walking off. ${((s as any).telsob ?? 0)} is very agitated for a while as the two of you watch the movie complaining about the black girl that was just here, eventually she calms down and the two of you watch the rest of the movie in peace.`);
    } else {
      scene.text('You explain you are on a date. "Uh… sorry but I am already on a date. She should be back any moment."');
      ...
      scene.text(`The black girl jerks her arm free and then laughs at ${((s as any).telsob ?? 0)}, before walking off. ${((s as any).telsob ?? 0)} is very agitated for a while as the two of you watch the movie complaining about the black girl that was just here, eventually she calms down and the two of you watch the rest of the movie in peace.`);
    }
    scene.actions([
      { label: 'Finish', goto: ['kinosvid', '3'] },
    ]);
    ...
    if (((s as any).meet_kendra ?? 0) === 1) {
      ...
      scene.text(`"${((s as any).pcs_nickname ?? 0)}." You tell her and then explain you are on a date. "Uh… sorry but I am already sorta on a date. She should be back any moment."`);
    ...
    scene.text(`Before you can answer ${((s as any).telsob ?? 0)} comes back and grabs Kendra by the arm and pulls her up out of the chair. "She's with me, get out of here."`);
    // TODO-QSP: dynamic text: The Kendra jerks her arm free and then laughs at <<$telsob>>, before walking off...
    scene.text(`The Kendra jerks her arm free and then laughs at ${((s as any).telsob ?? 0)}, before walking off. ${((s as any).telsob ?? 0)} is very agitated for a while as the two of you watch the movie complaining about the black girl that was just here, eventually she calms down and the two of you watch the rest of the movie in peace.`);
    scene.actions([
      { label: 'Finish', goto: ['kinosvid', '3'] },
    ...
    scene.text(`Just as it is starting to really feel good, ${((s as any).telsob ?? 0)} shows up and grabs Kendra by the arm yanking her up. Kendra doesn't put up a fight, and even starts laughing as she goes. Kendra calls back to you before she walks off: "If you ever get a taste for dark chocolate come see me at the dorm, ask for me by name. I know you want to." She leaves licking her still wet fingers.`);
    // TODO-QSP: dynamic text: Furious, <<$telsob>> throws her a parting shot: "Get the fuck out of here you cr...
    scene.text(`Furious, ${((s as any).telsob ?? 0)} throws her a parting shot: "Get the fuck out of here you crazy black bitch! ${((s as any).pcs_nickname ?? 0)} is my girl." She says the last part so only you can hear, as she sits back down.`);
    scene.actions([
      { label: 'Wait', goto: ['kinosvid', '23'] },
        ...
        // TODO-QSP: dynamic text: While she is gone, Kendra plops right down in the chair next to you. She starts ...
        scene.text(`While she is gone, Kendra plops right down in the chair next to you. She starts eating your popcorn and places a hand on your knee. Too stunned to say anything, you just sit there a few moments as she rubs your knee and eats the popcorn. "Hello again ${((s as any).pcs_nickname ?? 0)}, I bet your all wet for me again." She says as she looks at you like a piece of property.`);
        scene.actions([
          { label: 'On a date', handler: (st: GameState) => {
    scene.img('images/characters/city/university/girl/kendra/theater/theater5.jpg');
    ...
    scene.text(`You pulls your clothes aside until you can bare your pussy to her. As she looks down at your pussy she licks her lips. "Oh, are you all wet for me ${((s as any).pcs_nickname ?? 0)}, or are you hot for that white bitch?" After a few moments she reaches over and starts to rub your thigh gently. Her hand starts sliding up your thigh to your pussy.`);
    qspCall(s, 'arousal', 'flash', 2, 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Let her', handler: (st: GameState) => {
...
function enter23(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`${((s as any).telsob ?? 0)} gives you a slow deep kiss, as she works two fingers into your pussy. She curls her fingers up, and gives a firm tug. "This is mine", she whispers against your lips.`);
    ...
    scene.text(`You obediently roll over to your knees, when ${((s as any).telsob ?? 0)} tells you to. She gets to work, stretching your little pussy with finger after finger. She isn't rough, but she leaves no doubt about who is in charge.`);
    // TODO-QSP: dynamic text: "I want you to cum for me now, <<$pcs_nickname>>", she says, in a firm voice.
    scene.text(`"I want you to cum for me now, ${((s as any).pcs_nickname ?? 0)}", she says, in a firm voice.`);
    scene.text('You do.');
    ...
    scene.text(`${((s as any).telsob ?? 0)} gets into position, and guides your hand between her legs. "Do it to me now, sweetie", she tells you. You slowly do the same thing that she has just done to you, slowly stretching her until you get four fingers inside her. After a short time ${((s as any).telsob ?? 0)} bucks wildly against your hand, as she climaxes. When she recovers she gives you a little kiss on the cheek, and goes back to watching the movie, without a word.`);
...
function enter21(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`When you are dressed and watching the movie again, ${((s as any).telsob ?? 0)} turns to you saying: "Oh, OK apology accepted."`);
    ...
    scene.text(`You lie back, breathing heavy, basking in the afterglow, and the thought of ${((s as any).telsob ?? 0)} no longer being mad about the incident.`);
    qspCall(s, 'arousal', 'cuni', 10, 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Finish', goto: ['kinosvid', '3'] },
...
function enter3(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).telsob ?? 0) === 'Alla'  ||  ((s as any).telsob ?? 0) === 'Masha'  ||  ((s as any).telsob ?? 0) === 'Kate') {
    ...
    scene.text(`When the movie ends, ${((s as any).telsob ?? 0)} says she has get home. She leaves you with a deep kiss, leaving you wanting more.`);
    ...
    scene.text(`When the movie ends, ${((s as any).telsob ?? 0)} looks at you and waits for your next move.`);
    if (((s as any).home ?? 0)?.['town'] === 'pavlovsk') {
      scene.actions([
