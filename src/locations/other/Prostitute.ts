  ...
  if (((s as any).hour ?? 0) >= 16  &&  ((s as any).hour ?? 0) <= 23) {
    ...
    qspGoto(s, 'fight', 'start');
          ...
          if (((s as any).proseventrand ?? 0) <= 8) {
            ...
            if (((s as any).proseventrand ?? 0) <= 14) {
...
function enterWork2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'foreplay', 5, 'prostitution', 'unknown');
    qspCall(s, 'stat', '');
    ...
    if (((s as any).locArgs?.[1] ?? 0) === 'blow job') {
      ...
      qspCall(s, 'arousal', 'bj', 5, 'prostitution', 'unknown');
    ...
    if ((!(Math.floor(Math.random() * 2) + 0))) {
      ...
      qspCall(s, 'arousal', 'vaginal', 15, 'prostitution', 'unknown', 'rough');
      qspCall(s, 'stat', '');
    } else {
    ...
    // TODO-QSP: gs 'money', 'earn', ProsMoney * 100, 'cash'
    qspCall(s, 'stat', '');
    (s as any).dickK = ((s as any).dick ?? 0) * 10 / 100;
    if (((s as any).dickK ?? 0) < 1) {
      (s as any).dickK = 1;
    }
    ...
    if (((s as any).locArgs?.[1] ?? 0) === 'blow job') {
      ...
      if (((s as any).bjrand ?? 0) <= 50) {
        if (((s as any).stat ?? 0)?.['bj'] <= 15) {
          ...
          qspCall(s, 'arousal', 'bj', 10, 'prostitution', 'unknown');
          qspCall(s, 'stat', '');
        } else {
          (s as any).spafinloc = 12;
          qspCall(s, 'cum_manage', '');
    ...
    if (((s as any).prostRand ?? 0) <= 50) {
      ...
      qspCall(s, 'arousal', 'vaginal', 10, 'prostitution', 'unknown');
      qspCall(s, 'stat', '');
    } else {
      if (((s as any).prostRand ?? 0) <= 75) {
        ...
        qspCall(s, 'arousal', 'vaginal', 10, 'prostitution', 'unknown', 'rough');
        qspCall(s, 'stat', '');
      } else {
        (s as any).pose = 0;
    ...
    qspCall(s, 'arousal', 'foreplay', 1, 'prostitution', 'unknown');
    qspCall(s, 'stat', '');
    if (((s as any).pcs_ass ?? 0) < ((s as any).dick ?? 0)) {
    ...
    qspCall(s, 'arousal', 'bj', 5, 'prostitution', 'unknown', 'sub', 'humiliation');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    qspCall(s, 'arousal', 'vaginal', 5, 'prostitution', 'unknown', 'sub', 'humiliation');
    qspCall(s, 'arousal', 'end');
    scene.actions([
