...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).mc_inventory ?? 0)?.['cigarettes'] > 0) {
    ...
    if (((s as any).i ?? 0) <= 25) {
      ...
      if (((s as any).i ?? 0) <= 40) {
        qspGoto(s, 'gschool_boys_bathroom', 'caughtb');
      } else {
        if (((s as any).i ?? 0) <= 65) {
          (s as any).demerit = ((s as any).demerit ?? 0) + (Math.floor(Math.random() * 2) + 1);
          qspGoto(s, 'gschool_boys_bathroom', 'companyb');
        } else {
          (s as any).demerit = ((s as any).demerit ?? 0) + (Math.floor(Math.random() * 2) + 1);
          scene.actions([
...
function enterBoy2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_hotcat ?? 0) >= 6) {
    ...
    scene.text(`"I was just playing girl, no need to get all bitchy! If you don't want to play, there are other bitches dying to take a ride. Catch you later, ${((s as any).pcs_nickname || '')}." He pauses to check himself in the mirror and straighten up his hair before walking out like nothing happened.`);
    if ((!((s as any).school_bunk ?? 0))) {
      scene.actions([
        { label: 'Continue to play on phone', goto: ['gschool_boys_bathroom', 'bathroom2'] },
      ]);
...
function enterBoy2Sub(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`He lets out a relaxed sigh. "Fuck, I needed that. You're a great lay, ${((s as any).pcs_nickname || '')}." He steps over to the sink and cleans himself up, wiping off the sweat and giving his cock a good wash. You climb down from the counter, his cum flowing from your gaping asshole and running down your thigh, and do the same.`);
...
function enterBoy3Kiss(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Hey ${((s as any).pcs_nickname || '')}, did you do the math homework?" he suddenly asks. You're taken off-guard by the unexpected question - it just doesn't seem to fit in with everything that's just happened - and it takes you a moment to reply. "Yeah, why?"`);
...
function enterBoy4(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).anal_slip = ((s as any).anal_slip ?? 0) + (4);
    (s as any).orgasm_txt = 'He picks up the pace, fucking you fast and hard through your orgasm and prolonging it to the point where it almost turns into pain. You can barely see anything, blinded by the sensory overload, but you can hear him panting and grunting.';
...
function enterBoy5(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_hotcat ?? 0) >= 5) {
    ...
    scene.text(`You slide off the toilet, down onto all fours, and crawl over to them on your hands and knees, sitting up once you're between the two. You take Fedor's ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} cock into your mouth first and start sucking on it while you use your hand to stroke Ivan's dick.`);
    (s as any).orgasm_or = 'no';
    ...
    scene.text(`Once you're settled after their impressive, if strange, display of strength, Ivan kneels behind you and starts rubbing the tip of his wet dick against your anus. It doesn't take much guesswork to figure out what he has in mind, so you try to relax as he coats your asshole with the saliva you left on him. When he shoves his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} cock into your ass, your moan is loud, a mixture of pain and pleasure; You take a moment to relish the tightness and adjust to it before both boys start fucking you, leaving you feeling completely filled. You moan loudly as they both pound you in tandem, your pleasure overtaking your pain.`);
...
function enterBoy6(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).grupTipe ?? 0) !== 4  &&  ((s as any).pcs_hotcat ?? 0) >= 5) {
    ...
    if (((s as any).runout ?? 0) === 1) {
      ...
      scene.text(`Afraid of what they might do, you use the element of surprise, break through their circle and run out of the bathroom, hearing them curse behind you. You run down the hall, past several classrooms; you don't even really know where you're going, you just want to put some distance between them and you. You think you're almost in the clear when you hear a loud, commanding voice in the hall behind you. "Where do you think you're going in such a hurry, Miss ${((s as any).pcs_lastname || '')}?" Busted. Sighing, you turn around and see a teacher glaring at you.`);
      // TODO-QSP: dynamic text: "Well, you see, I…" They interrupt you before you can even properly begin an exc...
      scene.text(`"Well, you see, I…" They interrupt you before you can even properly begin an excuse. "I know you have class this period and you're late, Miss ${((s as any).pcs_lastname || '')}. You aren't cutting class now, are you?"`);
    ...
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
        ...
        scene.actions([
          { label: 'Not today', handler: (st: GameState) => {
    qspCall(s, 'willpower', 'pay', 'resist');
    qspGoto(s, 'gschool_boys_bathroom', 'boy6_not');
  } },
        ]);
      }
      scene.actions([
...
function enterCompanyb1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).orgasm_txt = 'You have to bite your lip to keep from crying out as an orgasm shoots through your body and causes you to convulse on the toilet.';
    (s as any).orgasm_or = 'custom';
...
function enterCompanyb2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).orgasm_txt = 'You have to bite your lip to keep from crying out as an orgasm shoots through your body and causes you to convulse on the toilet seat.';
    (s as any).orgasm_or = 'custom';
...
function enterJoinThem(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_rel ?? 0)?.['A144'] >= 50  &&  ((s as any).anushkaQW ?? 0)?.['sex'] >= 10  &&  ((s as any).pcs_hotcat ?? 0) >= 6) {
    ...
    ((s as any).anushkaQW = (s as any).anushkaQW ?? {})['toilet'] = 1;
    ...
    (s as any).orgasm_txt = 'Between the two of them, your own climax builds up fast, and only a few seconds later, you\'re moaning into Anushka\'s mouth and gripping Arkadi\'s cock like a vice, twitching and shaking like crazy.';
    (s as any).orgasm_or = 'custom';
    ...
    (s as any).orgasm_txt = 'You have to bite your lip to keep from crying out as an orgasm shoots through your body and causes you to convulse.';
    (s as any).orgasm_or = 'custom';
    qspCall(s, 'arousal', 'vaginal_finger', 10, 'masturbate');
    qspCall(s, 'arousal', 'end');
    ...
    scene.text(`He lifts one of her legs off the ground and turns her sideways. Her leg is barely up in the air when his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} cock penetrates her pussy. You would be impressed with his stamina if you weren't so occupied with enjoying this.`);
    scene.text('Anushka starts to scream as her body is rocked by muscle spasms as she has an orgasm. Arkadi just fucks her right through her orgasm before pulling out and pushing her down. She quickly turns around and drops to her knees, opening her mouth and sticking out her tongue. He gives his cock a few jerks before his cum starts flying all over her face and into her mouth. Once he finishes, she sucks his dick, sucking out the last few drops of cum from him. Afterwards, as he gets dressed, she uses her fingers to clean her face, sucking the cum from her fingers before she gets dressed as well. As she finishes getting dressed, she looks over at you and winks. "Hope you enjoyed the show." The two of them then leave.');
...
function enterCaughtb1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspGoto(s, 'gschool_office', 'principal');
  } },
    ...
    // TODO-QSP: dynamic text: Without a word, he walks over to one of the sinks and washes his hands; he seems...
    scene.text(`Without a word, he walks over to one of the sinks and washes his hands; he seems to be ignoring you, but you can see that he's giving you sideways glances. Finally, he smiles. "Feel free to cut class any time you like, ${((s as any).pcs_firstname || '')}. I'll happily keep your secret."`);
...
function enterCaughtb2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).Ivanov_toilet ?? 0) === 1) {
    ...
    qspGoto(s, 'gschool_office', 'principal');
      ...
      scene.text(`Mr. Ivanov gives you a long, hard stare. You can see a vein twitch at his temple and he seems to be struggling with himself but, eventually, he nods curtly. "Okay," he hisses."I'll let you off with a warning today, Miss ${((s as any).pcs_lastname || '')}, but I expect you to be at the top of my class and, so help me god, if I find out you're cutting class again…" he trails off meaningfully. "Do we understand each other?"`);
    ...
    qspGoto(s, 'gschool_office', 'principal');
      ...
      scene.text(`He looks down at you before silently unzipping his pants and pulling his dick out. It's not even hard yet, but he seems to have made his choice and so you stroke his dick a few times until it's erect, then lean forward and start licking it; you lick up and down the shaft and run your tongue around the head while looking up at him to gauge his nonexistent reaction before you take his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} cock into your mouth. You wrap your ${((s as any).pc_desc ?? 0)?.['lips'] ?? ''} lips tightly around his dick when you feel him grab the back of your head and shove your head down, forcing his dick deep into your mouth and down your throat. You start gagging, but he doesn't relent - he pulls out briefly, only to shove his cock right back in.`);
    ...
    scene.text(`You do as you're told and stroke his dick a few times until it's fully hard, then lean forward and start licking it; you lick up and down the shaft and run your tongue around the head while looking up at him to gauge his nonexistent reaction, before you take his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} cock into your mouth. You wrap your ${((s as any).pc_desc ?? 0)?.['lips'] ?? ''} lips tightly around his dick and start bobbing your head, stroking his cock with your lips, your tongue still making its rounds. He rests one hand on the top of your head, but he lets you control the action.`);
    ...
    scene.text(`When he feels you're sufficiently prepared, he rubs the tip of his dick against your backdoor and uses one of his hands to spread your ass cheeks wide. Once he has your ass cheeks spread wide and his cock lined up, he shoves his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} cock into your ass. You groan loudly in pleasure and pain. In a few uncomfortable thrusts, he shoves his cock all the way into your bowels until he's balls deep inside. You cry out, the pain too intense to keep quiet, and he doesn't pause to give your ass a little time to adjust; he just starts giving you a proper hard fucking.`);
    ...
    (s as any).orgasm_txt = 'Despite the rough treatment he\'s giving your ass, you feel an orgasm roll in and when it hits with loud screams, Mr. Ivanov fucks you even harder. He pounds your ass all the way through your orgasm.';
    (s as any).orgasm_or = 'custom';
...
function enterCaughtb2Slut(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`He places his hand at the back of your head to keep you from pulling away as he jams his dick down your throat. As you gag on his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} dick, you feel his balls slap against your chin and your slobber running out of your mouth as he gags you over and over, violently deepthroating you before he finally pulls his dick out of your mouth and pulls you up to your feet by your hair. His slips one hand between your legs to your pussy and slides his fingers inside you.`);
...
function enterBoy7(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).IgorQW ?? 0)?.['strap'] >= 1  &&  ((s as any).mc_inventory ?? 0)?.['strapon'] > 0  &&  ((s as any).strapNumber ?? 0) > 0) {
    ...
    scene.text(`He does what you ask without hesitation. Once his pants are halfway down his thighs, you grab him by the hips and pull him back towards you. Once he's close enough, you reach down and line up the dildo against his asshole. Using one hand, you pull his hips back while the other guides your ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} dildo into his ass. He moans in pleasure and pain as your dildo slides into his ass and you slowly start fucking him.`);
    ...
    scene.text(`He lets out a slight sigh of relief. "Thanks, ${((s as any).pcs_firstname || '')}. I just don't want anyone to know…"`);
