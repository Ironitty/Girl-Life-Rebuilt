...
function enterChrisSub(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).LariskaQW ?? 0)?.['story'] === 3) {
    ...
    ((st as any).LariskaQW = (st as any).LariskaQW ?? {})['story'] = 4;
    (st as any).sauna_stripped = 0;
    qspCall(st, 'outfit', 'strip_all');
    ((st as any).christinaQW = (st as any).christinaQW ?? {})['subpath'] = 2;
    ((st as any).christinaQW = (st as any).christinaQW ?? {})['submit'] = 1;
    ...
    if (((st as any).cheatVars ?? 0)?.['doormat_disabled'] === 0) {
      ((st as any).trait_vars = (st as any).trait_vars ?? {})['doormat_exp'] = ((st as any).trait_vars['doormat_exp'] ?? 0) + (2);
      if (((st as any).trait_vars ?? 0)?.['doormat'] < 1) {
        qspCall(st, 'traits', 'level', 'doormat', 1);
      }
    }
    scene.actions([
...
function enterShowers(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).christinaQW ?? 0)?.['subpath'] === 3) {
      if ((!((st as any).zverhumiliation ?? 0))) {
        qspGoto(st, 'Zvereva_sub_1', '0');
      } else {
        if (((st as any).zverhumiliation ?? 0) === 1) {
          qspGoto(st, 'Zvereva_sub_1', '1');
        } else {
          if (((st as any).zverhumiliation ?? 0) === 2) {
            qspGoto(st, 'Zvereva_sub_1', '2');
          } else {
            if (((st as any).zverhumiliation ?? 0) === 3) {
              qspGoto(st, 'Zvereva_sub_1', '3');
            } else {
              if (((st as any).zverhumiliation ?? 0) === 4) {
                qspGoto(st, 'Zvereva_sub_1', '4');
              } else {
                if (((st as any).zverhumiliation ?? 0) === 5) {
                  qspGoto(st, 'Zvereva_sub_1', '5');
                } else {
                  if (((st as any).zverhumiliation ?? 0) === 6) {
                    qspGoto(st, 'Zvereva_sub_1', '6');
                  } else {
                    qspGoto(st, 'Zvereva_sub_1', 'default');
                  }
                }
              }
            }
      ...
      if (((st as any).stat ?? 0)?.['think_virgin'] === 1) {
        (st as any).gKristinaZvtemp = (Math.floor(Math.random() * 2) + 14);
      } else {
        (st as any).gKristinaZvtemp = (Math.floor(Math.random() * 9) + 7);
      }
      if (((st as any).gKristinaZvtemp ?? 0) === 7  &&  ((st as any).stat ?? 0)?.['think_virgin'] === 0) {
        qspGoto(st, 'Zvereva_sub_1', '7');
      } else {
        if (((st as any).gKristinaZvtemp ?? 0) === 8  &&  ((st as any).stat ?? 0)?.['think_virgin'] === 0) {
          qspGoto(st, 'Zvereva_sub_1', '8');
        } else {
          if (((st as any).gKristinaZvtemp ?? 0) === 9  &&  ((st as any).stat ?? 0)?.['think_virgin'] === 0) {
            qspGoto(st, 'Zvereva_sub_1', '9');
          } else {
            if (((st as any).gKristinaZvtemp ?? 0) === 10  &&  ((st as any).stat ?? 0)?.['think_virgin'] === 0) {
              qspGoto(st, 'Zvereva_sub_1', '10');
            } else {
              if (((st as any).gKristinaZvtemp ?? 0) === 11  &&  ((st as any).stat ?? 0)?.['think_virgin'] === 0) {
                qspGoto(st, 'Zvereva_sub_1', '11');
              } else {
                if (((st as any).gKristinaZvtemp ?? 0) === 12  &&  ((st as any).stat ?? 0)?.['think_virgin'] === 0) {
                  qspGoto(st, 'Zvereva_sub_1', '12');
                } else {
                  if (((st as any).gKristinaZvtemp ?? 0) === 13  &&  ((st as any).stat ?? 0)?.['think_virgin'] === 0) {
                    qspGoto(st, 'Zvereva_sub_1', '13');
                  } else {
                    if (((st as any).gKristinaZvtemp ?? 0) === 14) {
                      qspGoto(st, 'Zvereva_sub_1', '14');
                    } else {
                      qspGoto(st, 'Zvereva_sub_1', 'default');
                    }
                  }
...
function enter0(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'dildo_suck', 10, 'sub', 'humiliation');
    qspCall(st, 'arousal', 'end');
    (st as any).zverhumiliation = 1;
    scene.actions([
...
function enter1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'dildo_suck', 10, 'sub', 'humiliation', 'deepthroat');
    qspCall(st, 'arousal', 'end');
    (st as any).zverhumiliation = 2;
    scene.actions([
      { label: 'Continue', goto: ['gschool_lessons4', 'postphys2'] },
    ]);
  } },
  ...
  scene.actions([
    ...
    if (((st as any).stat ?? 0)?.['think_virgin'] === 1) {
      if (((st as any).pcs_ass ?? 0) < 15) {
        ...
        scene.text('She grabs her bag and throws a dildo your way. ' + ((st as any).OpenInnerThought ?? '') + 'Wait. She expects me to put this in my butt! It will never fit!' + ((st as any).CloseInnerThought ?? ''));
        ...
        if (((st as any).pcs_ass ?? 0) >= 15  &&  ((st as any).pcs_ass ?? 0) < 25) {
          ...
          scene.text(`You lean backwards and after you spit on the dildo a few times, move it against your backside. All of the girls from your class watch you attentively as you slowly work the head in, and then begin to fuck your ass with it. Christina smiles and says "You're quite good at taking stuff up your butt, ${((st as any).pcs_nickname ?? '')}. How is this so easy for you? Do you practice on a daily basis or something?"`);
          scene.text('After a while Christina has seen enough, and allows you to change.');
          qspCall(st, 'arousal', 'anal_dildo', 10, 'sub', 'humiliation');
          qspCall(st, 'arousal', 'end');
        } else {
          ...
          qspCall(st, 'arousal', 'anal_fist', 10, 'sub', 'humiliation');
          qspCall(st, 'arousal', 'end');
        }
      }
    } else {
...
function enter10(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Christina spends some time watching you, and asks the girls if they have any more fun ideas for next time. While some of them have grown to like the locker room entertainment, no one really suggests anything. "Hmm, sounds like it's all up to me then. ${((st as any).pcs_nickname ?? '')}, you can stop," she mutters.`);
...
function enter12(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`After a while, Christina nods. "Alright, that's enough. You're turning into a great little locker room whore, ${((st as any).pcs_nickname ?? '')}. Maybe I should take you to the guys' locker room next time."`);
    (st as any).orgasm_or = 'no';
...
function enterFinished(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    { label: 'Continue', handler: (st: GameState) => {
    (st as any).zverhumiliation = undefined;
    (st as any).gKristinaZvtemp = undefined;
...
function enterWs_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The quiet murmurs of your classmates make you realize that all the other girls in your class had been watching the whole time! Christina notices them too and asks: "Does anyone else need to go? No one? Okay… ${((st as any).pcs_nickname ?? '')}, you can get dressed then. See you next time."`);
    qspCall(st, 'arousal', 'pee', 5, 'sub', 'humiliation', 'lesbian');
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
