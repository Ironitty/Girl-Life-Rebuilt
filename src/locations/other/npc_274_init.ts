...
function enterGraveyardMemorial(s: GameState, scene: SceneBuilder): void {
  ...
  if (Number((s as any).locArgs?.[1] ?? 0) === 'graveyard_wait') {
    ...
    // TODO-QSP: dynamic text: You look up, hearing footsteps, and it''s <<$npc_firstname[''A274'']>> coming ba...
  ...
  scene.actions([
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A274', 'like');
    ((st as any).npc_trust = (st as any).npc_trust ?? {})['A274'] = ((st as any).npc_trust['A274'] ?? 0) + (5);
    qspCall(st, 'stat', '');
    ...
    scene.text(`You step in, pulling her close to you. You smell the soft, floral fragrance she wears, and she starts to cry. You just hold her waiting for the storm of tears to pass. You reach into your purse to take out a tissue and hand it to ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}.`);
    scene.actions([
    ...
    scene.text(`"I'm not usually this bad," ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} replies. "It's just that getting a place on this course is a major achievement for me. Do you mind and give me a moment alone, please?" She looks vulnerable as she makes the request.`);
    // TODO-QSP: dynamic text: "Sure, <<$npc_firstname[''A274'']>>. I''ll be beside those trees."
    scene.text(`"Sure, ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}. I'll be beside those trees."`);
    scene.actions([
    ...
    scene.text(`You pick up your coat and start walking to the bench near the main entrance brushing the grass and leaves from your coat. You sit down and look towards ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} but can't see her. You pull out your phone and play a game to distract yourself.`);
    // TODO-QSP: dynamic text: You look up, hearing footsteps, and it''s <<$npc_firstname[''A274'']>> coming ba...
    scene.text(`You look up, hearing footsteps, and it's ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} coming back. Her eyes were red and puffy from crying. You stand up to meet her, "You look a fright, ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}. Let me fix your makeup."`);
    // TODO-QSP: dynamic text: Before she can say anything, you pull her to the bench and start cleaning her up...
...
function enterPostGrave(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).mayaqw ?? 0)?.['grave'] === 2) {
    ...
    scene.text('<center><h2>Residential Bedroom</h2></center>');
    scene.img('images/locations/pushkin/ballet_residence/bedroom.jpg');
    // TODO-QSP: dynamic text: You enter your room together and see that <<$npc_firstname[''A274'']>> is still ...
    ...
    scene.text(`You make some strong tea and head back to your room. ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} is laying on your bed with her eyes closed.`);
...
function enterSummerSchool(s: GameState, scene: SceneBuilder): void {
  ...
  if (Number((s as any).locArgs?.[1] ?? 0) === 'piano') {
    ...
    scene.text(`You are led downstairs to where ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''} lives in the warden's apartment. As you enter you are greeted by a grey cat who meows a greeting to ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''} and starts to entwine himself between her legs.`);
    // TODO-QSP: dynamic text: “Meet Fyodor. He likes to think he is an opera singer with his meows.” <<$npc_fi...
    scene.text(`“Meet Fyodor. He likes to think he is an opera singer with his meows.” ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''} says with a small smile.`);
    ...
    scene.text(`You continue to look blankly at ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''}. `);
    // TODO-QSP: dynamic text: “Never mind, you obviously missed that part of your education. We are here to ta...
    scene.text(`“Never mind, you obviously missed that part of your education. We are here to talk about ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}. Would you like some tea?”`);
    // TODO-QSP: dynamic text: “Now, where to start?” <<$npc_firstname[''A286'']>> muses to herself as she star...
    scene.text(`“Now, where to start?” ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''} muses to herself as she stares out through the apartment window composing her thoughts. As you sit waiting Fyodor comes over to you and starts to rub against your legs. You reach down and start to scritch him behind the ears, and he starts to purr loudly.`);
    if (((st as any).mayaqw ?? 0)?.['grave'] >= 3) {
      // TODO-QSP: dynamic text: “You know <<$npc_firstname[''A274'']>> lost her mother six years ago? I saw you ...
      scene.text(`“You know ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} lost her mother six years ago? I saw you leave with her to visit her grave.”`);
      scene.text('“Yes, I went with her but I am not sure why she asked me?”');
    } else {
      // TODO-QSP: dynamic text: “I am not sure how to put this, but <<$npc_firstname[''A274'']>> lost her mother...
      scene.text(`“I am not sure how to put this, but ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} lost her mother six years ago. She often plays the piano as a way to escape her grief.”`);
      scene.text('“You said her mother died six years ago? Isn\'t that a long time?”');
      // TODO-QSP: dynamic text: “Yes, but <<$npc_firstname[''A274'']>> and her mother had a falling out over wha...
      scene.text(`“Yes, but ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} and her mother had a falling out over what career ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} wanted to follow, and when her mother died suddenly she never got the chance to repair the rift that had opened between them.”`);
    }
    // TODO-QSP: dynamic text: “But why was <<$npc_firstname[''A274'']>> chosen as my dance partner?”
    scene.text(`“But why was ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} chosen as my dance partner?”`);
    if (((st as any).start_type ?? 0)?.['magic'] === 'nomagic') {
      ...
      scene.text(`“You both have an otherworldly grace to your dance style. You are very alike in that you draw the audience's focus whenever you dance. It's almost magical how you both do this, and when you both dance together it's both entrancing and captivating. You are both destined to be great ballerinas, but you still have a lot to learn. However, with ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} you will catch up quickly if you apply yourself.”`);
    }
    // TODO-QSP: dynamic text: “I think when you start performing with <<$npc_firstname[''A274'']>> or solo you...
    scene.text(`“I think when you start performing with ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} or solo your performances will be in great demand, and if you ever need support the School will be here for you, just like ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} when she lost her mother. I will not go into her story but suffice it to say the School has a vested interest in ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}. She had all but given up on being a ballet dancer and instead she wanted to become a gymnast, but after her mother died she almost gave up everything until we encouraged her to become a ballerina again. Her grandparents travel in Europe, they have little contact with ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}, so we made her a ward.”`);
    scene.text('“What has this to do with me and what happened to her father?”');
    // TODO-QSP: dynamic text: “<<$npc_firstname[''A274'']>> may learn to trust you, and we would like you to r...
    scene.text(`“${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} may learn to trust you, and we would like you to respect that trust. You are one of the few people that she seems to be opening up to.” There is a hint of a threat in ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''}'s voice but you are unsure why. “We still do not know who ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}'s father is. We have not been able to find out who, but the little we do know leads us to suspect he was a composer that Anitchka fell in love with when she was touring. Unfortunately we have no details or other information, despite our best efforts.”`);
    // TODO-QSP: dynamic text: “OK, but I am not sure about <<$npc_firstname[''A274'']>> – or even if a friends...
    scene.text(`“OK, but I am not sure about ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} – or even if a friendship will develop?”`);
    scene.text('“If it does, it does, but remember she is our ward and under our protection if any harm comes to her.”');
    if (((st as any).start_type ?? 0)?.['magic'] !== 'nomagic') {
      // TODO-QSP: dynamic text: “One other bit of advice, be wary of <<$npc_firstname[''A283'']>>. He has taken ...
      scene.text(`“One other bit of advice, be wary of ${((st as any).npc_firstname ?? 0)?.['A283'] ?? ''}. He has taken a personal interest in you both.”`);
      scene.text('“Why?”');
      // TODO-QSP: dynamic text: “It''s rare for <<$npc_firstname[''A283'']>> to take an interest in students, es...
      scene.text(`“It's rare for ${((st as any).npc_firstname ?? 0)?.['A283'] ?? ''} to take an interest in students, especially those who have the most potential. Since he was injured, he has become increasingly bitter over the years – I have said too much, just be aware that his attention might not be in your own or ${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''}'s best interests.” With this ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''} stands up, and you get the hint it's time to leave.`);
    } else {
      // TODO-QSP: dynamic text: “<<$npc_firstname[''A274'']>> has to deal with a lot of grief, and remember she'...
      scene.text(`“${((st as any).npc_firstname ?? 0)?.['A274'] ?? ''} has to deal with a lot of grief, and remember she's been made a ward of the school because of her mother.”`);
    ...
    // TODO-QSP: dynamic text: With that <<$npc_firstname[''A286'']>> moves away from the doorway and you are l...
    scene.text(`With that ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''} moves away from the doorway and you are left wondering what just occurred, and why Bronya was watching you in the first place.`);
    scene.actions([
      { label: 'Accept cup of tea', handler: (st: GameState) => {
    // TODO-QSP: dynamic text: “Thank you <<$npc_firstname[''A286'']>>, just milk please. You wait a few moment...
    scene.text(`“Thank you ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''}, just milk please. You wait a few moments as ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''} busies herself with pouring you a cup of tea.”`);
  } },
    ...
    scene.text(`You hold up your hand to decline the offer of tea and give a small smile to ${((st as any).npc_firstname ?? 0)?.['A286'] ?? ''}. “No thanks, I'm fine thank you.”`);
  } },
