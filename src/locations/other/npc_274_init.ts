...
function enterPostGrave(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).mayaqw ?? 0)?.['grave'] === 2) {
    ...
    scene.text(`She opens her eyes and gives you a small smile, "Yes, ${((s as any).pcs_nickname || '')}. You brought the tea, thank you. You've been too good to me tonight."`);
    // TODO-QSP: dynamic text: <<$npc_firstname['A274']>> settles down and starts to talk. "<<$pcs_nickname>>, ...
    scene.text(`${((s as any).npc_firstname ?? 0)?.['A274'] ?? ''} settles down and starts to talk. "${((s as any).pcs_nickname || '')}, how much do you know about me?"`);
...
function enterSummerSchool(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'piano') {
    ...
    scene.text(`“Don't ${((s as any).pcs_nickname || '')}, she won't appreciate it.” ${((s as any).npc_firstname ?? 0)?.['A286'] ?? ''}'s tone is firm and she's insistent that you listen to her.`);
