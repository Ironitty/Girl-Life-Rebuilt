...
function enterIraLenaFun(s: GameState, scene: SceneBuilder): void {
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 'girls') {
    ...
    qspCall(st, 'arousal', 'foreplay', 5, ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'foreplay', (-5), ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Move away', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
    ...
    qspCall(st, 'arousal', 'foreplay', 5, 'lesbian', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'cuni', 2, ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'cuni', 3, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'cuni_give', 3, ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'cuni_give', 2, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger', 2, ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger', 3, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger_give', (-5), ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger_give', (-5), ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Cum', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).iralenasleep = ((st as any).iralenasleep ?? 0) + (1);
    scene.text('<center><b>Ira and Lena</b></center>');
    ...
    qspCall(st, 'arousal', 'vaginal_finger', 3, ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger', 2, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger_give', (-5), ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger_give', (-5), ((st as any).npcID1 ?? 0), 'lesbian', 'group');
...
function enterIraLenaRex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You go with ${((s as any).boydesc ?? '')}, Lena and Ira follow a little ways behind and without waiting for them, he puts you down on your knees.`);
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 5, 'sub', 'unknown', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Lie on your back', handler: (st: GameState) => {
    qspCall(st, 'arousal_funcs', 'stretch', 'vaginal');
    qspCall(st, 'cum_call', 'precum', ((st as any).npcID ?? 0), 1);
    scene.text('<center><b>Ira and Lena</b></center>');
    ...
    scene.text(`${((st as any).boydesc ?? '')} takes advantage of Ira's position and slides his cock inside of her and begins to fuck her roughly.`);
    scene.text(`Ira bouncing her hips back and forth, fucking him as much as he was fucking her as she keeps licking your pussy, Lena starts kissing ${((st as any).boydesc ?? '')}, while her hand caress his balls.`);
    scene.text(`Time flies fast, Ira, Lena and yourself have all switched positions many times, letting him fuck all three of you, ${((st as any).boydesc ?? '')} can take no more.`);
    qspCall(st, 'arousal', 'cuni', 5, 'sub', 'unknown', 'lesbian');
    ...
    scene.text(`You bend over in front of him, Lena and Ira obediently bend over in front of ${((st as any).boydesc ?? '')} as well. One of them to each side of you, giving him a lovely view of all three of your asses.`);
    scene.text(`${((st as any).boydesc ?? '')} enjoys the spectacular view of your graceful asses. He fucks you first, while he slaps the girls on the ass, as he pounds into you.`);
    scene.text('You got the feeling he meant to take turns fucking each of you, but he grunts and can no longer hold back.');
    qspCall(st, 'arousal', 'vaginal', 5, 'sub', 'unknown', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Cum', goto: ['rex_party_sexEvents', 'ira_lena_rex_cum'] },
    ]);
...
function enterIraLenaRexCum(s: GameState, scene: SceneBuilder): void {
  ...
  if ((Math.floor(Math.random() * 2) + 1) === 1) {
    ...
    scene.text(`${((s as any).boydesc ?? '')} pulls out of you and blows his load all over your ass, covering it in his spunk.`);
...
function enterIraFunBoys(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 5, 'sub', 'group', 'unknown');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterIraBoysFuck(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'cum_call', 'stomach', ((st as any).npcID1 ?? 0));
    qspCall(st, 'cum_call', 'precum', ((st as any).npcID2 ?? 0));
    qspCall(st, 'cum_call', 'stomach', ((st as any).npcID2 ?? 0));
    (st as any).iralenasleep = ((st as any).iralenasleep ?? 0) + (1);
    scene.text('<center><b>Ira and Lena</b></center>');
...
function enterRexToiletBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Rex drags you to the toilet, sporting a huge erection. You quickly squat down and eagerly plunge his ${((s as any).dick ?? '')}cm ${((s as any).dick_girth ?? '')} cock into your mouth. Rex exclaims excitedly, "Wow, you are really unpredictable, and for that I love you." With that, he fucks you in the mouth, while you suck on him passionately. Soon you can tell he is about to blow his load.`);
  scene.actions([
    { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'bj', 8);
    qspCall(st, 'arousal', 'end');
...
function enterSisterFun(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"${((s as any).pcs_nickname ?? '')}, let's dance like there's no tomorrow!" Pulling her top off, she helps you remove yours.`);
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'kiss', 5, 'lesbian', 'incest');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'foreplay', 5, 'lesbian', 'incest');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Lick Anya\'s pussy', handler: (st: GameState) => {
    (st as any).sister_party_dom = 1;
    scene.text('<center><b>Anya</b></center>');
    ...
    scene.text(`"Mmm ${((st as any).pcs_nickname ?? '')}, you taste so sweet," Anya murmurs. You can't help but smile at the skill of her tongue and again silently question her claims that she isn't into girls.`);
...
function enterSisterHand(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if ((Math.floor(Math.random() * 2) + 1) === 1) {
      ...
      qspCall(st, 'arousal', 'vaginal_finger', 5, 'lesbian', 'incest');
      qspCall(st, 'arousal', 'cuni_give', (-5), 'lesbian', 'incest');
      qspCall(st, 'arousal', 'cuni', 5, 'lesbian', 'incest');
      qspCall(st, 'stat', '');
      scene.actions([
    ...
    qspCall(st, 'arousal', 'trib', 5, 'lesbian', 'incest');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Cum', goto: ['rex_party_sexEvents', 'sister_cum'] },
    ]);
...
function enterDjSuck(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).rex_love ?? 0))) {
    ...
    scene.text(`You find you don't want to stop and passionately start sucking his ${((st as any).dick ?? '')}cm ${((st as any).dick_girth ?? '')} dick right there in front of everyone.`);
    ...
    scene.text(`He abruptly pushes his ${((st as any).dick ?? '')}cm ${((st as any).dick_girth ?? '')} dick inside of you and starts fucking you with a fast pace.`);
...
function enterToiletBoysBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).i = (Math.floor(Math.random() * 2) + 1);
    if (((st as any).i ?? 0) === 1) {
      scene.text(`<center><video autoplay loop ${((st as any).set_imgh ?? '')} src="images/locations/pavlovsk/resident/rekshome/party/party_girls_party_gost_12_1.mp4"></video></center>`);
    }
    if (((st as any).i ?? 0) === 2) {
      scene.text(`<center><img ${((st as any).set_imgh ?? '')} src="images/locations/pavlovsk/resident/rekshome/party/party_girls_party_gost_12_3.jpg"></center>`);
    }
    ...
    qspCall(st, 'arousal', 'bj', 5, ((st as any).npcID ?? 0), 'group', 'gangbang');
    qspCall(st, 'arousal', 'clit_finger', (-5), ((st as any).npcID1 ?? 0), 'masturbate');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'cum_call', 'mouth', ((st as any).npcID ?? 0), 1, '', '', 20);
    qspCall(st, 'cum_call', 'face', ((st as any).npcID ?? 0), 1, '', '', 20);
    qspCall(st, 'cum_call', 'clothes', ((st as any).npcID1 ?? 0), 1, '', '', 20);
    qspCall(st, 'cum_call', 'hair', ((st as any).npcID1 ?? 0), 1, '', '', 20);
    qspCall(st, 'stat', '');
...
function enterGirlOrgy(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'kiss', 5, 'lesbian', 'group');
    qspCall(st, 'arousal', 'cuni', 5, 'lesbian', 'group');
    qspCall(st, 'arousal', 'cuni_give', 5, 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger', 5, 'lesbian', 'group');
    qspCall(st, 'arousal', 'vaginal_finger_give', 5, 'lesbian', 'group');
    qspCall(st, 'arousal', 'end');
    scene.actions([
...
function enterOrgy(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!(Math.floor(Math.random() * 2) + 0))) {
    ...
    qspCall(st, 'arousal', 'bj', (-10), ((st as any).npcID ?? 0), 'group', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 10, ((st as any).npcID1 ?? 0), 'group', 'gangbang');
    qspCall(st, 'arousal', 'bj', (-10), ((st as any).npcID1 ?? 0), 'group', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 10, ((st as any).npcID2 ?? 0), 'group', 'gangbang');
    qspCall(st, 'arousal', 'bj', (-10), ((st as any).npcID2 ?? 0), 'group', 'gangbang');
    qspCall(st, 'arousal', 'end');
...
function enterGangbangOutdoor(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).temp_rand ?? 0))) {
    ...
    scene.text(`Together they get you out to the street and one of the guys pushes you down to your knees and pull his cock out. "Come on, ${((s as any).pcs_nickname ?? '')}, start sucking." You put you hand on his member and start to lick the whole length before wrapping your lips around the head. You keep sucking, taking turns on the guy's dicks, until they are all hard and ready. Then one of them tells you to bend over.`);
    ...
    scene.text(`"Bend over, ${((st as any).pcs_nickname ?? '')}, and show us your tight, wet hole." You get in a comfortable position and the guys take turns fucking your wet pussy. You really enjoy yourself as you move your hips towards their rock-hard members, making them glide in and out of your pussy, not wanting to wait their turn they start to fuck your ass as well. The guys finish on your ass and back, except the last one who groans and tells you: "Ah, ${((st as any).pcs_nickname ?? '')}, open up, I want to cum in your mouth!"`);
    qspCall(st, 'arousal', 'vaginal', 5, ((st as any).npcID1 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID1 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 5, ((st as any).npcID2 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID2 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 5, ((st as any).npcID3 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID3 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    scene.text(`You turn around and kneel in front of him, playfully sticking out your tongue. The drunk guy gives a little jerk and sprays of sperm jets into your mouth. "Ah, ${((st as any).pcs_nickname ?? '')}, you are truly the best bitch at this party!" the guys all laugh. You try to join in, but your laughter is stifled by the mouthful of sperm.`);
    qspCall(st, 'cum_call', 'back', ((st as any).npcID1 ?? 0));
    qspCall(st, 'cum_call', 'butt', ((st as any).npcID2 ?? 0));
    qspCall(st, 'cum_call', 'mouth', ((st as any).npcID3 ?? 0));
    qspCall(st, 'cum_call', 'face', ((st as any).npcID3 ?? 0));
    qspCall(st, 'arousal', 'end');
    scene.actions([
    ...
    if (((s as any).temp_rand ?? 0) === 1) {
      ...
      scene.actions([
        { label: 'Bend over', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[0] = 'vagina';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[1] = 'labia';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[2] = 'anus';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[3] = 'butt';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[4] = 'back';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[5] = 'legs';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[6] = 'arms';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[7] = 'face';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[8] = 'mouth';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[9] = 'hands';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[10] = 'stomach';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[11] = 'breasts';
    ((st as any).temp_cumloc = (st as any).temp_cumloc ?? {})[12] = 'hair';
    ...
    qspCall(st, 'arousal', 'vaginal', 5, ((st as any).npcID1 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-5), ((st as any).npcID1 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 5, ((st as any).npcID2 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-5), ((st as any).npcID2 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 5, ((st as any).npcID3 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-5), ((st as any).npcID3 ?? 0), 'sub', 'gangbang');
    if (((st as any).i2 ?? 0) === 1) {
      qspCall(st, 'cuminsidereact', 'one of the guys from the party');
    }
    qspCall(st, 'arousal', 'end');
    (st as any).i2 = undefined;
    (st as any).temp_rand = undefined;
    (st as any).temp_cumloc = undefined;
    scene.actions([
      { label: 'Fall asleep', goto: ['rex_party_smallEvents', 'sleep'] },
    ]);
  } },
    ...
    qspCall(st, 'arousal', 'bj', 3, ((st as any).npcID1 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 3, ((st as any).npcID1 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID1 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'bj', 3, ((st as any).npcID2 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 3, ((st as any).npcID2 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID2 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'bj', 3, ((st as any).npcID3 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'vaginal', 3, ((st as any).npcID3 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID3 ?? 0), 'sub', 'gangbang');
    qspCall(st, 'stat', '');
    if (((st as any).i2 ?? 0) === 1) {
      qspCall(st, 'cuminsidereact', 'one of the guys from the party');
...
function enterGangbangIndoor(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', 3, ((st as any).npcID1 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID1 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'bj', 3, ((st as any).npcID2 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'vaginal', 3, ((st as any).npcID2 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID2 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'bj', 3, ((st as any).npcID3 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'vaginal', 3, ((st as any).npcID3 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'anal', (-3), ((st as any).npcID3 ?? 0), 'sub', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'cum_call', 'face', ((st as any).npcID3 ?? 0), 0, 0, 20);
    qspCall(st, 'stat', '');
    if ((Math.floor(Math.random() * 2) + 1) === 1) {
...
function enterBoobsYes(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You gladly kneel in front of him to take a closer look at his erect member. "Come on ${((st as any).pcs_nickname ?? '')}, don't be shy."`);
    scene.actions([
    ...
    (st as any).spafinloc = 11;
    qspCall(st, 'cum_manage', '');
    (st as any).party_boys_in = ((st as any).party_boys_in ?? 0) + (1);
    (st as any).spafinloc = 12;
    qspCall(st, 'cum_manage', '');
    qspCall(st, 'stat', '');
...
function enterBoysSwalow(s: GameState, scene: SceneBuilder): void {
  ...
  do {
    ...
    scene.text(`"Uhh ${((s as any).pcs_nickname ?? '')}! You're still the best cocksucker around. If you want more sperm to swallow, call us."`);
    scene.text('The guys help you back into the apartment and into one of the bedrooms as you are too drunk to do it yourself.');
    scene.actions([
      { label: 'Fall asleep', goto: ['rex_party_smallEvents', 'sleep'] },
    ]);
  } while (((s as any).i ?? 0) > 0);
  scene.build();
}
...
function enterBoysSpit(s: GameState, scene: SceneBuilder): void {
  ...
  do {
    ...
    scene.text(`"What's wrong ${((s as any).pcs_nickname ?? '')}? Didn't you like it? I thought all girls loved the taste of cum." one of the guys asked you with a laugh.`);
    scene.text('The guys help you back into the apartment and into one of the bedrooms as you are too drunk to do it yourself.');
    scene.actions([
      { label: 'Fall asleep', goto: ['rex_party_smallEvents', 'sleep'] },
    ]);
...
function enterToiletSexEvents(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).temp ?? 0) === 1) {
    ...
    (st as any).toiletEvent_Rex = ((st as any).daystart ?? 0);
    ...
    scene.text(`As you jerk him off, he moans and sperm starts squirting all over your hands. "Ooh ${((st as any).pcs_nickname ?? '')}, you saved me. But next time, use your mouth." he says with a wink.`);
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Move away', goto: ['rex_party', 'Toilet'] },
    ]);
    ...
    scene.text(`You feel Rex's hands on your shoulders gently pushing you down. You instantly obey and drop to your knees. Once you're level with Rex's ${((st as any).dick ?? '')}cm ${((st as any).dick_girth ?? '')} dick, you take his head into your mouth and wrap your lips tightly around his shaft as you begin to suck him off.`);
    ...
    (st as any).spafinloc = 11;
    qspCall(st, 'cum_manage', '');
    (st as any).i = (Math.floor(Math.random() * 8) + 11);
    if (((st as any).i ?? 0) === 11) {
      scene.img(`images/locations/pavlovsk/resident/rekshome/party/reksparty_party_toilet_5_${((st as any).i ?? '')}.mp4`);
    } else {
      scene.img(`images/locations/pavlovsk/resident/rekshome/party/reksparty_party_toilet_5_${((st as any).i ?? '')}.jpg`);
    }
    ...
    (st as any).toiletEvent_Sis = ((st as any).daystart ?? 0);
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'npcStat', 'A56');
    (st as any).sexvolume = 5;
    (st as any).spafinloc = 12;
    qspCall(st, 'cum_manage', '');
    qspCall(st, 'stat', '');
    (st as any).i = (Math.floor(Math.random() * 4) + 2);
    if (((st as any).i ?? 0) === 2  ||  ((st as any).i ?? 0) === 4) {
      scene.img(`images/locations/pavlovsk/resident/rekshome/party/reksparty_party_toilet_6_${((st as any).i ?? '')}.mp4`);
    } else {
      scene.img(`images/locations/pavlovsk/resident/rekshome/party/reksparty_party_toilet_6_${((st as any).i ?? '')}.jpg`);
    }
    ...
    scene.text(`"Oh, it's you ${((st as any).pcs_nickname ?? '')}. Glad it is not some guy. We do not know what we would have done with him."`);
    ...
    scene.text(`"Well ${((st as any).pcs_nickname ?? '')}, did you like it?"`);
    ...
    scene.actions([
      ...
      { label: 'Go to the living room', goto: ['rex_party', 'LivingRoom'] },
              ...
              if (((s as any).temp ?? 0) === 7) {
                ...
                if (((s as any).i ?? 0) === 2) {
                  scene.img(`images/locations/pavlovsk/resident/rekshome/party/reksparty_party_toilet_8_${((s as any).i ?? '')}.mp4`);
                } else {
                  scene.img(`images/locations/pavlovsk/resident/rekshome/party/reksparty_party_toilet_8_${((s as any).i ?? '')}.jpg`);
                }
...
function enterToiletMast(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).i = (Math.floor(Math.random() * 3) + 1);
  if (((s as any).i ?? 0) < 3) {
    scene.text(`<center><img ${((s as any).set_imgh ?? '')} src="images/locations/pavlovsk/resident/rekshome/party/reksparty_party_toilet_1_${((s as any).i ?? '')}.jpg"></center>`);
  }
  if (((s as any).i ?? 0) === 3) {
