...
function enterEnterNatashaRoom(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).soniaQW ?? 0)?.['encourage'] === 0  ||  ((s as any).soniaQW ?? 0)?.['help'] >= ((s as any).soniaQW ?? 0)?.['encourage']) {
    ...
    scene.text(`She pauses and takes another deep breath then continues. "That wasn't nice, ${((s as any).pcs_nickname || '')}! I think at the very least you should go and apologise to her. It won't undo what happened but it might help her to know you are sorry."`);
    ...
    scene.text(`Natasha looks relieved. "Thank you ${((s as any).pcs_nickname || '')}." After a pause she continues. "Also, I want to discuss something with you..."`);
    scene.actions([
      { label: 'Discuss your relationship status', goto: ['natbel_uni_dates_2', 'discuss_relationship_status'] },
    ]);
  } },
...
function enterDiscussRelationshipStatus(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Natasha breaks out into a huge grin. "You've just made my day ${((s as any).pcs_nickname || '')}; I simply couldn't be happier right now!"`);
    scene.actions([
    ...
    scene.text(`She shakes her head. "No ${((s as any).pcs_nickname || '')}, let me buy them also as a bit of a "thank you" for all you've done for me."`);
...
function enterDesignsChat(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (qspFunc(s, 'money', 'can_afford', 500) === 0) {
      ...
      scene.text(`Natasha is embarrassed and looks downcast. "I know, I'm sorry ${((s as any).pcs_nickname || '')}."`);
      ...
      scene.text(`"Oh, okay ${((s as any).pcs_nickname || '')}, yes that's a good idea. There's a stationery and arts shop not far from here."`);
...
function enterDesignsDormGift(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`She's unsure. "I don't know ${((s as any).pcs_nickname || '')}; I'm really not sure if they're good enough."`);
    ...
    scene.text(`"Okay ${((s as any).pcs_nickname || '')}, I don't mind, but it'll probably be a wasted trip."`);
...
function enterVisitAphrodite(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).job_status ?? 0)?.['city_aphrodite_model'] !== 'employed') {
      ...
      scene.text(`You approach the manager, who greets you. "Hello ${((s as any).pcs_nickname || '')}, how can I help you today?" You return his greeting and briefly tell him the purpose of your visit.`);
    }
    ...
    scene.text(`After taking another look he nods and tells you. "Okay ${((s as any).pcs_nickname || '')}, I'm going to get one of our photographers to take a look as he's much more qualified to give an opinion than me."`);
    ...
    scene.text(`With that you stand and reach over to collect Natasha's drawings. Before you can pick them up the manager continues. "You haven't wasted our time ${((s as any).pcs_nickname || '')}. I tell you what, let's go over the road and get a coffee and continue this discussion."`);
    scene.actions([
...
function enterVisitTailor(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"No, that's okay ${((s as any).pcs_nickname || '')}, you can pay me when you collect them. Come back in about a month and they should be ready."`);
    scene.actions([
...
function enterCollectTailor(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Natasha looks at you and motions for you to do a twirl. "New dress ${((s as any).pcs_nickname || '')}? It's lovely; where did you get it?"`);
    ...
    scene.text(`Natasha looks so happy as well as being about ready to burst into tears as she grabs you and hugs you close. "Thank you so much ${((s as any).pcs_nickname || '')}."`);
    scene.actions([
    ...
    scene.text(`Natasha has an idea about the photos. "${((s as any).pcs_nickname || '')}, about the photos — instead of going to Aphrodite as that'd be expensive, what about Julia? I've seen some of her work and it's really good."`);
    ...
    scene.text(`"Yes, of course ${((s as any).pcs_nickname || '')}. I'm just happy and content with life right now. I'm really enjoying university and spending time with you. This has been pretty full on and it'd be nice to take a step back for a while and concentrate on the day to day stuff, especially our coursework."`);
