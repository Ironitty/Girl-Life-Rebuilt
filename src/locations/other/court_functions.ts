...
function enterArrestFor(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).policeQW) (s as any).policeQW = {}; (s as any).policeQW['' + String((s as any).$ARGS[1] || '') + '_booked'] = ((s as any).policeQW['' + String((s as any).$ARGS[1] || '') + '_booked'] ?? 0) + (1);
  if (!(s as any).policeQW) (s as any).policeQW = {}; (s as any).policeQW['crime_flag'] = ((s as any).locArgs?.[1] ?? 0);
  // TODO-QSP: end
  scene.build();
}
...
function enterAddFine(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).policeQW ?? 0)?.['fine_deadline'] === 0) {
    ...
    if (!(s as any).policeQW) (s as any).policeQW = {}; (s as any).policeQW['fine_deadline'] = Math.max(((s as any).daystart ?? 0), ((s as any).policeQW ?? 0)?.['fine_deadline']) + 14 - Math.min(7, 2 * ((s as any).temp_current_missed ?? 0) + ((s as any).temp_tot_missed ?? 0) / 4);
  }
