...
function enterToysPaddleOptions(s: GameState, scene: SceneBuilder): void {
  if (((s as any).pcs_inhib ?? 0) >= 30) {
    ...
    qspCall(st, 'stat', '');
    ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['sub'] = ((st as any).AnnaQW['sub'] ?? 0) + (2);
    ...
    // TODO-QSP: dynamic text: "It was a pleasure <<$pcs_nickname>>. We''ll continue another time." You put on ...
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: "OK <<$pcs_nickname>>, it''s time for you to go, we can continue another time." ...
    ...
    // TODO-QSP: dynamic text: "OK <<$pcs_nickname>>, just try as hard as you can. It''s a ''bit'' difficult to...
    scene.text(`"OK ${((st as any).pcs_nickname || '')}, just try as hard as you can. It's a 'bit' difficult to leave marks on me, ok?"`);
    scene.actions([
    ...
    if (((st as any).annaToy ?? 0) !== 2) {
      ...
      scene.text(`"Good effort, I really felt it but my butt will only show marks from a really heavy beating ${((st as any).pcs_nickname || '')}. That's because I'm well trained and my skin has gotten a bit tough. Still, the main thing is that you get the idea and understand the process. That's all for today come find me when you want to learn more." Anna leads you to the door.`);
    } else {
      // TODO-QSP: dynamic text: "Mmmh, it seems mommy <<$pcs_nickname>> will need a lot of practice. Hehehe. Unf...
      scene.text(`"Mmmh, it seems mommy ${((st as any).pcs_nickname || '')} will need a lot of practice. Hehehe. Unfortunately for you i've got a steel butt from plenty of practice. Still, the main thing is that you get the idea and understand the process. That's all for today come find me when you want to learn more." Anna leads you to the door.`);
    }
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    qspCall(st, 'hotel_anna_gear', 'toys_images_clear');
...
function enterToysVacuum2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: Anna smiles, "Thank you it comes with experience. OK <<$pcs_nickname>> that''s a...
...
function enterToysWheel(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "Wattenburg wheel uh? Well, it''s a medical pinwheel to stimulate or cause a tin...
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    // TODO-QSP: $anna_toys_image5
    // TODO-QSP: dynamic text: "That''s great <<$pcs_nickname>>…honestly I think that practice is always more i...
    scene.text(`"That's great ${((st as any).pcs_nickname || '')}…honestly I think that practice is always more instructive. Here try the wheel on your hand for the moment, and try to see what kind of sensation you can have from that. You can use the multiwheel on your arm…"`);
    scene.actions([
      { label: 'Ok', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['sub'] = ((st as any).AnnaQW['sub'] ?? 0) + (1);
    ...
    // TODO-QSP: $anna_toys_image2
    ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['dom'] = ((st as any).AnnaQW['dom'] ?? 0) + (1);
    // TODO-QSP: dynamic text: "Hehehe <<$pcs_nickname>>, you are right but the answer is: no. There''s no poin...
    scene.text(`"Hehehe ${((st as any).pcs_nickname || '')}, you are right but the answer is: no. There's no point for me trying the wheel on myself; it's a matter of you understanding what this tool can offer. You can get the right idea using the multiwheel on your arm… unless you're scared."`);
    scene.actions([
...
function enterToysWand(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['trust'] = ((st as any).AnnaQW['trust'] ?? 0) + (1);
    // TODO-QSP: dynamic text: "Interesting… A friend of mine would be really interested in that… it''s not eve...
    ...
    qspCall(st, 'stat', '');
    // TODO-QSP: $anna_toys_image2
    (st as any).Anna_trust = ((st as any).Anna_trust ?? 0) - (1);
    ...
    qspCall(st, 'stat', '');
    // TODO-QSP: $anna_toys_image2
    (st as any).Anna_see_abrat = ((st as any).Anna_see_abrat ?? 0) + (1);
    ...
    scene.text(`"Well ${((st as any).pcs_nickname || '')} of course I can… The question is: What do you get out of that? You are the one who's supposed to know more about the violet wand… Maybe you are scared by this little tool and what it can do" She says with a smile.`);
    scene.actions([
      { label: 'I\'m not scared! Give it to me!…', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    (st as any).BDSM_Knowledge = ((st as any).BDSM_Knowledge ?? 0) + (1);
    ...
    (st as any).Anna_see_abrat = ((st as any).Anna_see_abrat ?? 0) + (1);
    // TODO-QSP: $anna_toys_image2
    // TODO-QSP: dynamic text: "Such bravery. <<$pcs_nickname>>… don''t be silly… if that was enough to move yo...
    ...
    scene.text(`Anna moves toward the control panel and turns it on. "${((st as any).pcs_nickname || '')}, please, would you be so kind to turn it to max? I can't do that alone… well I can but it's more difficult… I want to hold my puppies."`);
    scene.actions([
...
function enterToysImages(s: GameState, scene: SceneBuilder): void {
  if (((s as any).annaToy ?? 0) === 1) {
    (s as any).anna_toys_image2 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy2a.jpg"></center>';
    (s as any).anna_toys_image3 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy3a.jpg"></center>';
    (s as any).anna_toys_image4 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy4a.jpg"></center>';
    (s as any).anna_toys_image5 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy5a.jpg"></center>';
    (s as any).anna_toys_image6 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy6a.jpg"></center>';
    (s as any).anna_toys_image7 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy7a.jpg"></center>';
  } else {
    if (((s as any).annaToy ?? 0) === 2  ||  ((s as any).annaToy ?? 0) === 4) {
      (s as any).anna_toys_image2 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy2b.jpg"></center>';
      (s as any).anna_toys_image3 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy3b.jpg"></center>';
      (s as any).anna_toys_image4 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy4b.jpg"></center>';
      (s as any).anna_toys_image5 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy5b.jpg"></center>';
      (s as any).anna_toys_image6 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy6b.jpg"></center>';
      (s as any).anna_toys_image7 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy7b.jpg"></center>';
    } else {
      if (((s as any).annaToy ?? 0) === 3) {
        (s as any).anna_toys_image2 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy2c.jpg"></center>';
        if (((s as any).annaToy ?? 0) === 2) {
          (s as any).anna_toys_image3 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy3ca.jpg"></center>';
          (s as any).anna_toys_image4 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy4cb.jpg"></center>';
        } else {
          (s as any).anna_toys_image3 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy3ca.jpg"></center>';
          (s as any).anna_toys_image4 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy4cb.jpg"></center>';
        }
        (s as any).anna_toys_image5 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy5c.jpg"></center>';
        (s as any).anna_toys_image6 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy6c.jpg"></center>';
        (s as any).anna_toys_image7 = '<center><img ' + ((s as any).set_imgh ?? 0) + ' src="images/characters/pavlovsk/resident/Anna/sessionhotel/anna1toy7c.jpg"></center>';
      }
    }
  }
  // TODO-QSP: end
...
function enterRestraintsRope(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['dom'] = ((st as any).AnnaQW['dom'] ?? 0) + (1);
    (st as any).BDSM_Knowledge = ((st as any).BDSM_Knowledge ?? 0) + (1);
    qspGoto(st, 'hotel_anna_gear', 'restraints_rope_dom');
  } },
    ...
    scene.text(`"Don't worry ${((st as any).pcs_nickname || '')}, you are free to learn at your own pace and only do what you are comfortable with. Next time something a bit more exciting, if I can get it arranged in time."`);
...
function enterRestraintsRopeDom(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'BDSM_give', 5);
    qspCall(st, 'stat', '');
    ...
    // TODO-QSP: dynamic text: "Don''t worry, we''ll get there." Anna says reassuringly as she puts her clothes...
    scene.text(`"Don't worry, we'll get there." Anna says reassuringly as she puts her clothes back on, and after a brief chat she leads you to the door. "Next time I hope to have something a little more exciting if I can get it arranged, see you then ${((st as any).pcs_nickname || '')}."`);
    scene.text('"Thank you Anna, goodbye."');
    scene.actions([
      { label: 'Leave', goto: ['pav_hotel', ''] },
    ]);
...
function enterRestraintsHarness(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Great! Here let me help you." Anna helps you disrobe. "Wow! Stunning. You have a great figure ${((st as any).pcs_nickname || '')}."</font>`);
    scene.actions([
    ...
    scene.text(`"Nonsense. You have a beautiful body ${((st as any).pcs_nickname || '')} and this harness is made to show some skin as I said just turn around so I can see your back."</font>`);
    scene.actions([
    ...
    scene.text(`You slowly turn swinging your back to tease Anna. "Mmmm… there's no need to put a show on… but… ok: if that make you feel comfortable by all means… go on. Just remember, we don't have all day ${((st as any).pcs_nickname || '')}."`);
    ...
    { label: 'On me?', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    (st as any).Anna_see_aswitch = ((st as any).Anna_see_aswitch ?? 0) + (1);
    (st as any).BDSM_Knowledge = ((st as any).BDSM_Knowledge ?? 0) + (1);
    scene.img('images/characters/pavlovsk/resident/Anna/sessionhotel/anna2restr5.jpg');
    ...
    scene.text(`"You have a beautiful figure ${((st as any).pcs_nickname || '')}."`);
    scene.actions([
      { label: 'Thanks…', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'foreplay', 5);
    qspCall(st, 'stat', '');
    ...
    scene.text(`"You have a beautiful body my dear ${((st as any).pcs_nickname || '')} and this harness is made to show that now turn around so I can see the back."`);
    scene.actions([
      { label: 'Turn', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'foreplay', 5);
    qspCall(st, 'arousal', 'end');
    ...
    scene.text(`You slowly turn as to put a show on for Anna. "As I said ${((st as any).pcs_nickname || '')} you have a great body. But we are not here to make you blush or feel uncomfortable. Now, do you feel how nice the leather feels against your body? Just the smell of leather can be arousing to some you know. Nothing quite beats leather. I think we'll end it there and continue next time. You can take the harness off now and get dressed." You put your clothes back on and talk a bit about leather harnesses before you say goodbye and go outside.`);
    scene.actions([
      { label: 'Leave', goto: ['pav_hotel', ''] },
    ]);
  } },
...
function enterRestraintsCuff(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"So ${((st as any).pcs_nickname || '')}, are you willing to try on some cuffs?"`);
    scene.actions([
    ...
    scene.text(`"Exactly, cuffs can be used to made subs assume revealing positions. We could talk about all the different positions, but that would take all night. Anyway this should help you have an idea… So, immpressions?… Feelings?… Sensations?.. .Tell me ${((st as any).pcs_nickname || '')}… how is it?"`);
    scene.actions([
    ...
    qspCall(st, 'arousal', 'end');
    scene.img('images/characters/pavlovsk/resident/Anna/sessionhotel/cuff1.jpg');
    // TODO-QSP: dynamic text: "Sure <<$pcs_nickname>> Just lay still and I''ll unlock the cuffs." Anna starts ...
    scene.text(`"Sure ${((st as any).pcs_nickname || '')} Just lay still and I'll unlock the cuffs." Anna starts freeing your ankles and wrists.`);
    ...
    scene.text(`"Exactly, cuffs can be used to made subs assume a desired position. I could talk for hours about all the different positions but we don't have that much time. Anyway this should help you have a general idea… So Immpressions?… Feelings?… Sensations?… Tell me ${((st as any).pcs_nickname || '')}… what are your feelings right now?"`);
    scene.actions([
      { label: 'I-i…', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    if (((st as any).Anna_see_asub ?? 0) >= 10) {
      ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['trust'] = ((st as any).AnnaQW['trust'] ?? 0) + (1);
    }
    ...
    scene.text(`"Of course ${((st as any).pcs_nickname || '')} wait a sec. Just lay still so I can unlock the cuffs." Anna starts freeing your ankles and wrists. "I want you to remember that I won't push this on you… the main thing that you learn, and I'd say you've learned some today, now get dressed." She smiles at you in a reassuring way… Once free and with your panties back on, you thank Anna for tonight and say goodbye.`);
    scene.actions([
      { label: 'Leave', goto: ['pav_hotel', ''] },
    ]);
  } },
...
function enterRestraintsLeash(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: "Ok <<$pcs_nickname>>… let''s go for a walk? Just a few steps around the room, n...
    scene.text(`"Ok ${((st as any).pcs_nickname || '')}… let's go for a walk? Just a few steps around the room, nothing serious. First, can you show me what it is to be submissive?"`);
    scene.actions([
    ...
    scene.text(`"How is it going so far ${((st as any).pcs_nickname || '')}?"`);
    scene.actions([
      { label: '"Fine, I think"', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/Anna/sessionhotel/leashnakfine.jpg');
    // TODO-QSP: dynamic text: "I''m happy to hear it… anyway we should stop for now. The walk ends. "We should...
    scene.text(`"I'm happy to hear it… anyway we should stop for now. The walk ends. "We shouldn't get carried away, this is a practice session after all, so ${((st as any).pcs_nickname || '')} I hope it has been instructive for you, you should have a better idea about the collar and the leash. You can get dressed now, and we'll continue next time"`);
    ...
    scene.text(`"I think we should end it here, you should get dressed ${((st as any).pcs_nickname || '')}."`);
    scene.text('"Y-yes…"');
    ...
    scene.text(`No answer comes from you. "Uh?…" Anna immediately stops moving worried by the lack of answer. "Is there a problem ${((st as any).pcs_nickname || '')}?"`);
    scene.text('"Y-yes."');
    ...
    scene.actions([
      { label: 'Follow Anna\'s lead…', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    if (((st as any).Anna_see_adom ?? 0) >= 10) {
      ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['switch'] = ((st as any).AnnaQW['switch'] ?? 0) + (1);
    }
    scene.img('images/characters/pavlovsk/resident/Anna/sessionhotel/leash.mp4');
    // TODO-QSP: dynamic text: Anna lets you walk in the room a little; understanding the aspects of being on d...
    scene.text(`Anna lets you walk in the room a little; understanding the aspects of being on display like this, like a pet. "How is it going so far ${((st as any).pcs_nickname || '')}?"`);
    scene.actions([
      { label: 'Fine… I think…', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/Anna/sessionhotel/leashnakfine.jpg');
    // TODO-QSP: dynamic text: "I''m happy to hear it… But we should stop for now." The walk ends. "We shouldn'...
    scene.text(`"I'm happy to hear it… But we should stop for now." The walk ends. "We shouldn't drag this on too long, this is for you to learn after all, so ${((st as any).pcs_nickname || '')} I hope it has been a learning experience for you, hopefully you should have a better idea about the collar and the leash. You can get dressed now, and we'll follow up on this next time"`);
    scene.text('"Thank you Anna."You get dressed and chat a little about collars before saying goodbye and leaving.');
    scene.actions([
      { label: 'Leave', goto: ['pav_hotel', ''] },
    ]);
    ...
    scene.text(`You shake your head. "Well ${((st as any).pcs_nickname || '')}, in that case we should end it for today. Hopefully I've given you some things to think about and we've made some steps forward." She unclips the leash from your collar.`);
    ...
    // TODO-QSP: dynamic text: "Well <<$pcs_nickname>>… in that case we''ll call it a day. The main thing is th...
    scene.text(`"Well ${((st as any).pcs_nickname || '')}… in that case we'll call it a day. The main thing is that you learn… and we've made progress." She removes the leash from your collar.`);
    scene.text('"Move your hair so I can get the collar off." Anna removes the collar and you get dressed.');
    scene.text('You chat a little with Anna and she reassures you that learning about BSDM practices can sometimes mean being submissive or Dominant even if that is not in your nature.');
    scene.text('After your chat you say your thanks and head out to the hotel lobby.');
    scene.actions([
    ...
    // TODO-QSP: dynamic text: "There''s no reason to be worried <<$pcs_nickname>>… let''s go for a walk a few ...
    scene.text(`"There's no reason to be worried ${((st as any).pcs_nickname || '')}… let's go for a walk a few steps in the room, nothing serious can you do that?. I won't ask you to show me submission… even if this is a learning experience without any consequence and it could be really instructive."`);
    scene.actions([
    ...
    scene.text(`"Exactly! That is a good sign a slave can show to show their obedience ${((st as any).pcs_nickname || '')}. Shall we go for a walk?"</font>…`);
    scene.actions([
      { label: 'Follow Anna\'s lead…', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    ((st as any).AnnaQW = (st as any).AnnaQW ?? {})['trust'] = ((st as any).AnnaQW['trust'] ?? 0) + (1);
    ...
    scene.text(`Anna lets you walk in the room for a little bit; you can understand the meaning of being the submissive to a dominant partner. "How are you doing ${((st as any).pcs_nickname || '')}?"`);
    scene.actions([
    ...
    // TODO-QSP: dynamic text: "I''m happy to hear it wasn''t a problem for you. Ok <<$pcs_nickname>> I hope it...
    scene.text(`"I'm happy to hear it wasn't a problem for you. Ok ${((st as any).pcs_nickname || '')} I hope it was an instructive session; you should have a better idea about collar, leash, submission and petplay. now we'll end it here for today, let me remove the collar and get your clothes." You get dressed and after a brief chat you leave Anna's apartment.`);
    scene.actions([
      { label: 'Leave', goto: ['pav_hotel', ''] },
    ]);
  } },
    ...
    scene.text(`"Sometimes it can get a bit too much, let me get this collar off and get your clothes ${((st as any).pcs_nickname || '')} we're done for today."`);
    ...
    scene.text(`Anna lets you walk in the room a little; you start to understand what she means about being the submissive. "How are you doing ${((st as any).pcs_nickname || '')}, everything okay so far?"`);
    scene.actions([
    ...
    scene.text(`Your body freezes completely, feeling the collar yank against your neck. "What?" Anna drops the leash immediately letting it fall. "Are you okay ${((st as any).pcs_nickname || '')}? I think we've gone far enough for today."`);
    scene.text('"I shouldn\'t have done this."');
    ...
    // TODO-QSP: dynamic text: "I''m only trying to teach you <<$pcs_nickname>>. But I think it''s time to stop...
    scene.text(`"I'm only trying to teach you ${((st as any).pcs_nickname || '')}. But I think it's time to stop here for today then. Hopefully you'll have learned something from this." Anna removes the collar and gives you your clothes. "There we go." After you've gotten dressed you talk a little with Anna, she seems to be understanding of your feeling and says that it's perfectly normal to feel like this. After you've finished talking you thank her and leave.`);
  } },
    ]);
  } },
      { label: 'Change your mind', handler: (st: GameState) => {
    ...
    scene.text(`"So ${((st as any).pcs_nickname || '')}, how does it feel to be walking your slave around naked, it's exhilarating isn't it?"`);
