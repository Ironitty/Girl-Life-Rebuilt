...
function enterDefault(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).temp_camVars ?? 0)?.['vaginal_available']) {
    ...
    if (((st as any).temp_camVars ?? 0)?.['anal_vibe_available']) {
      qspCall(st, 'komp_cam_MFC_requests_anal', 'set_vibr_act');
    }
    if (((st as any).temp_camVars ?? 0)?.['anal_small_available']) {
      qspCall(st, 'komp_cam_MFC_requests_anal', 'set_small_act');
    }
    if (((st as any).temp_camVars ?? 0)?.['anal_normal_available']) {
      qspCall(st, 'komp_cam_MFC_requests_anal', 'set_normal_act');
    }
    if (((st as any).temp_camVars ?? 0)?.['anal_big_available']) {
      qspCall(st, 'komp_cam_MFC_requests_anal', 'set_big_act');
    }
    if (((st as any).temp_camVars ?? 0)?.['anal_large_available']) {
      qspCall(st, 'komp_cam_MFC_requests_anal', 'set_large_act');
    }
    if (((st as any).temp_camVars ?? 0)?.['anal_huge_available']) {
      qspCall(st, 'komp_cam_MFC_requests_anal', 'set_huge_act');
    }
    if (((st as any).temp_camVars ?? 0)?.['anal_enormous_available']) {
      qspCall(st, 'komp_cam_MFC_requests_anal', 'set_enormous_act');
    }
    ...
    if (((st as any).pcs_vag ?? 0) < 15) {
      ...
      if (((st as any).pcs_horny ?? 0) < 100) {
        ...
        (st as any).orgasm_txt = 'You can\'t stop yourself even if you wanted to! You orgasm eagerly in front of the camera, leaving a slight blush on your face. "I wish that were you!" you wink at the person who posted the request, who readily donates some extra tokens.';
        (st as any).orgasm_or = 'custom';
      }
...
function enterSetVibrVagAct(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).pcs_horny ?? 0) < 100) {
      ...
      (st as any).orgasm_txt = 'You orgasm violently from the sudden stimulation, which is met with a lot of enthusiasm in your chat.';
      (st as any).orgasm_or = 'custom';
    }
    qspCall(st, 'arousal', 'vaginal_dildo', (-5), 'dick_length', 'masturbation');
    qspCall(st, 'stat', '');
...
function enterSetSmallVagAct(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).pcs_horny ?? 0) < 100) {
      ...
      (st as any).orgasm_txt = 'Feeling overwhelmed by the sudden sensations the dildo is giving you, you orgasm right in front of the camera. It\'s only a short distance away from your sopping pussy, and all your viewers get an excellent view on your pussy spasming and squeezing the dildo tightly.';
      (st as any).orgasm_or = 'custom';
    }
    qspCall(st, 'arousal', 'vaginal_dildo', (-5), 'dick_length', 'masturbation');
    qspCall(st, 'stat', '');
...
function enterSetNormalVagAct(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).pcs_horny ?? 0) < 100) {
      ...
      (st as any).orgasm_txt = 'You cum within the minute, your body rocking and spasming while you try your best to stay in the camera frame. Your spectators loved it, and shower you with comments in the chat afterwards.';
      (st as any).orgasm_or = 'custom';
    }
    qspCall(st, 'arousal', 'vaginal_dildo', (-5), 'dick_length', 'masturbation');
    qspCall(st, 'stat', '');
  ...
  scene.actions([
    ...
    if (((st as any).pcs_vag ?? 0) < 30) {
      ...
      if (((st as any).pcs_horny ?? 0) < 100) {
        ...
        (st as any).orgasm_txt = 'You decide to give it all you have and ride the dildo carefully, bringing yourself to an unexpected orgasm. Your pussy lips are stretched quite far around the dildo\'s very wide shaft, which gets you several compliments from your more dedicated viewers.';
        (st as any).orgasm_or = 'custom';
      }
    } else {
      scene.text('You smile at the camera confidently as you grab hold of the enormous dildo and spit on the suction cup before you slap it onto the floor in front of your bed.');
      ...
      if (((st as any).pcs_horny ?? 0) < 100) {
        ...
        (st as any).orgasm_txt = 'You decide to give it all you have and ride the dildo furiously, bringing yourself to an unexpected orgasm. Your pussy lips are stretched quite far around the dildo\'s very wide shaft, which gets you several compliments from your more dedicated viewers.';
        (st as any).orgasm_or = 'custom';
      }
    }
    qspCall(st, 'arousal', 'vaginal_dildo', (-5), 'dick_length', 'masturbation');
  ...
  scene.actions([
    ...
    if (((st as any).pcs_vag ?? 0) < 35) {
      ...
      if (((st as any).pcs_horny ?? 0) < 100) {
        ...
        (st as any).orgasm_or = 'custom';
        (st as any).orgasm_txt = 'Then you realize: "How am I fucking myself with two dildos at once!?" and orgasm almost immediately after. If you didn\'t know you were a size queen before this, you definitely realize it now!<br>The chat loves their size queen, and showers you with donations while your body is still quivering on the bed.';
      }
    }
    qspCall(st, 'arousal', 'vaginal_dildo', (-5), 'dick_length', 'masturbation');
    qspCall(st, 'stat', '');
