...
function enterManager1(s: GameState, scene: SceneBuilder): void {
  ...
  if (qspFunc(s, 'car_funcs', 'has_car')) {
    if (qspFunc(s, 'car_funcs', 'is_here_area')) {
      scene.text('The shifty-looking salesman circles around your car a couple of times, making exaggerated displeased noises all the while and pointing out practically every speck of dust as if it is major damage. When he turns to you, he states that the best he can do is ' + qspFunc(s, 'money', 'string_profit', ((s as any).bupay ?? '')) + ' - while he claims that he\'s taking a risk with that price, you\'re pretty sure he\'s ripping you off.');
      scene.actions([
        { label: '', labelFn: (s: GameState) => String('Sell your car (' + qspFunc(s, 'money', 'string_profit', ((s as any).bupay ?? '')) + ') (1:00)' ?? ''), handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 60;
    qspCall(st, 'money', 'earn', ((st as any).bupay ?? 0));
    (st as any).car = undefined;
    (st as any).bupay = undefined;
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
      ...
      if (qspFunc(s, 'money', 'can_afford', 3000) === 1) {
        scene.actions([
          { label: '', labelFn: (s: GameState) => String('Pay for towing service to the car market (' + qspFunc(s, 'money', 'string_price', 3000) + ') (1:00)' ?? ''), handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 60;
...
function enterInspect(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`'${(((s as any).used_car ?? 0)?.[String(((s as any).autotraidF_carnum ?? 0)) + '_condition_desc'] ?? '')} The car salesman wants ' + $func('money', 'string_price', used_car['${((s as any).autotraidF_carnum ?? '')}_price']) + ' for it.'`);
  ...
  scene.actions([
    ...
    (st as any).autotraidF_carname = undefined;
    (st as any).autotraidF_carnum = undefined;
    (st as any).autotraidF_benz = undefined;
...
function enterBuyCar(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('You agree and before you know it, the car salesman and you have signed all the necessary documents. You pay the ' + qspFunc(s, 'money', 'string_price', (((s as any).used_car ?? 0)?.[String(((s as any).autotraidF_carnum ?? 0)) + '_price'] ?? '')) + ', and the car is yours.');
  ...
  (s as any).autotraidF_carname = undefined;
  (s as any).autotraidF_carnum = undefined;
  (s as any).autotraidF_benz = undefined;
  scene.actions([
...
function enterBuyWreck(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('You agree and before you know it, the car salesman and you have signed all the necessary documents. You pay ' + qspFunc(s, 'money', 'string_price', (((s as any).used_car ?? 0)?.['wrek_price'] ?? '')) + ', and the car is yours.');
