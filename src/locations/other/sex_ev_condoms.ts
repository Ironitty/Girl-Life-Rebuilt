...
function enterCondomWait(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['no_condom'] !== 1  &&  ((s as any).sex_ev ?? 0)?.['skip_condom'] !== 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDay(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
...
function enterPcNeedCondoms(s: GameState, scene: SceneBuilder): void {
  if (((s as any).mc_inventory ?? 0)?.['normal_condoms'] > 0  ||  (((s as any).mc_inventory ?? 0)?.['equipped_condoms'] > 0  &&  ((s as any).preziktype ?? 0) === 0)) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcCondoms2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  scene.build();
...
function enterPcCondoms2(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_no_condoms ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
    ...
    dynamicGoto(st, 'sex_ev_' + String((((st as any).sex_ev ?? {})['position'])), String((((st as any).sex_ev ?? {})['position'])) + '_goto');
  } },
      ]);
    }
  }
...
function enterPcNoCondomsLeft(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcNoCondomsLeft2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
  ...
  scene.actions([
    ...
    if (((st as any).npcID ?? 0) === 'A34') {
      ...
      if ((((st as any).npc_cum_pref ?? 0)?.[String((st as any).npcID ?? 0)] === 'pullout'  ||  ((st as any).npc_arrogant ?? 0)?.[String((st as any).npcID ?? 0)] > 0  ||  ((st as any).npc_abusive ?? 0)?.[String((st as any).npcID ?? 0)] > 0  ||  ((st as any).npc_risktaker ?? 0)?.[String((st as any).npcID ?? 0)] > 0)  &&  ((st as any).npc_condom_conscious ?? 0)?.[String((st as any).npcID ?? 0)] < 1  &&  ((st as any).npc_know_bc ?? 0)?.[String((st as any).npcID ?? 0)] === 0) {
        scene.text(`"Don't worry," ${((st as any).npcdesc ?? '')} says. "I've got pullout game."`);
        ((st as any).npc_pullout_game = (st as any).npc_pullout_game ?? {})[String((st as any).npcID ?? 0)] = 1;
        { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNpcPulloutGame(st, scene); (st as any).locArgs = __savedLocArgs; }
      } else {
        if (((st as any).npc_no_condoms ?? 0)?.[String((st as any).npcID ?? 0)] > 0) {
          { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNpcDoesntUseCondoms(st, scene); (st as any).locArgs = __savedLocArgs; }
        } else {
          { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcCondomAsk3(st, scene); (st as any).locArgs = __savedLocArgs; }
        }
      }
    }
  } },
...
function enterPcSkipTheCondom(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPulledOutLastTime(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Just this once?', handler: (st: GameState) => {
    scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
    scene.text('"Maybe we can skip it..." you smile. "Just this once?"');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcSkipTheCondom2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
...
function enterBulkCondoms(s: GameState, scene: SceneBuilder): void {
  if (qspFunc(s, 'money', 'can_afford', 2000) !== 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNpcCondomPutOn(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  scene.actions([
...
function enterNpcOutOfCondoms(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).mc_inventory ?? 0)?.['normal_condoms'] > 0  ||  (((s as any).mc_inventory ?? 0)?.['equipped_condoms'] > 0  &&  ((s as any).preziktype ?? 0) === 0)) {
    ...
    qspCall(st, 'sex_ev_stats', 'pc_condom_stats');
    scene.img('images/shared/sex/misc/condom_put.jpg');
    scene.text('iif(mc_inventory[\'normal_condoms\'] = 1, \'"That\'s okay, I\'ve got one."\', \'"That\'s okay, I\'ve got some."\')');
...
function enterNpcPulloutGame(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_no_creampie_mistakes ?? 0)?.[String((s as any).npcID ?? 0)] <= ((s as any).npc_pullout_failure ?? 0)?.[String((s as any).npcID ?? 0)]  &&  ((s as any).npc_pullout_failure ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPulledOutLastTime(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterDidntPullOutLastTime(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDayCondomInsist2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ...
    scene.text('"<i>Most</i> of the time," you scowl. "But it only takes one fuck up to get me pregnant.' + ((((st as any).npc_pullout_failure_last_time ?? 0)?.[String((st as any).npcID ?? 0)] === 1) ? (' And I seem to recall you fucking up <i>majorly</i> last time..."') : ('"')));
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPulledOutLastTime(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterDidntPullOutLastTime(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoCondomAllowAnnoyed(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: '<i>Need</i> a condom', handler: (st: GameState) => {
    scene.text(`"Sorry, ${(((st as any).npc_lovername ?? 0)?.[String((st as any).npcID ?? 0)] ?? '')}," you smirk. "Gotta use a rubber this time."`);
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDayCondomInsist2(st, scene); (st as any).locArgs = __savedLocArgs; }
    ...
    scene.text('"<i>Most</i> of the time," you say with a meaningful smirk.' + ((((st as any).npc_pullout_failure_last_time ?? 0)?.[String((st as any).npcID ?? 0)] === 1) ? (' "I seem to recall something white and sticky filling me up last time..."') : (' ')));
    if (((st as any).npc_pay_for_ma_pill ?? 0)?.[String((st as any).npcID ?? 0)] === 1) {
      scene.text(`"At least I paid for it, right?" ${((st as any).npcdesc ?? '')} grins back.`);
    } else {
      if ((((st as any).npc_selfish ?? 0)?.[String((st as any).npcID ?? 0)] > 0  ||  ((st as any).npc_abusive ?? 0)?.[String((st as any).npcID ?? 0)] > 0)  &&  ((st as any).npc_caretaker ?? 0)?.[String((st as any).npcID ?? 0)] < 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPulledOutLastTime(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterDidntPullOutLastTime(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoCondomAllowAmused(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: '<i>Need</i> a condom', handler: (st: GameState) => {
    scene.text(`"Sorry, ${(((st as any).npc_lovername ?? 0)?.[String((st as any).npcID ?? 0)] ?? '')}," you smirk. "Gotta use a rubber this time."`);
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDayCondomInsist2(st, scene); (st as any).locArgs = __savedLocArgs; }
  ...
  scene.actions([
    ...
    if (((st as any).sex_ev ?? 0)?.['condom_count'] > 0  &&  ((st as any).sex_ev ?? 0)?.['using_pc_condoms'] !== 1) {
      ...
      if (((st as any).sex_ev ?? 0)?.['using_pc_condoms'] === 1) {
        ...
        { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcUseCondoms(st, scene); (st as any).locArgs = __savedLocArgs; }
      } else {
        ...
        if (((st as any).mc_inventory ?? 0)?.['normal_condoms'] === 0  &&  (((st as any).mc_inventory ?? 0)?.['equipped_condoms'] === 0)) {
          ...
          { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoSexOnly(st, scene); (st as any).locArgs = __savedLocArgs; }
          { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcSkipTheCondom(st, scene); (st as any).locArgs = __savedLocArgs; }
        } else {
          { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcUseCondoms(st, scene); (st as any).locArgs = __savedLocArgs; }
        }
      }
...
function enterNoCondomPulloutAllow(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).sex_ev ?? 0)?.['position'] === '') {
      ...
      dynamicGoto(st, 'sex_ev_' + String((((st as any).sex_ev ?? {})['position'])), '' + (((st as any).sex_ev ?? 0)?.['position']) + '_goto');
    }
  } },
...
function enterNoCondom1(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_latex_allrg ?? 0)?.[String((s as any).npcID ?? 0)] !== 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoCondomLeave(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
...
function enterNoCondom2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if ((Math.floor(Math.random() * 10) + 1) < 8) {
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoCondom2_1(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      ((st as any).sex_ev = (st as any).sex_ev ?? {})['dont_need_condom'] = 1;
...
function enterDisableCondoms(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_bareback ?? 0)?.[String((s as any).npcID ?? 0)] === ''  &&  ((s as any).sex_ev ?? 0)?.['condom'] === 0) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterDisableCondomsOptions2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      ]);
    }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterCatholicCondoms(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterDisableCondomsOptions1(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ]);
...
function enterDisableCondomsOptions1(s: GameState, scene: SceneBuilder): void {
  if (((s as any).knowpreg ?? 0) === 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNpcNoCondomReact(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } else {
    if (((s as any).birth_control ?? 0)?.['think_safe'] === 1) {
...
function enterCatholicCondoms(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterCatholicCondomReact(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterCatholicCondomReact(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_condom_conscious ?? 0)?.[String((s as any).npcID ?? 0)] === 2  ||  (((s as any).npc_condom_conscious ?? 0)?.[String((s as any).npcID ?? 0)] === 1  &&  ((Math.floor(Math.random() * (100 - ((s as any).npc_willpwr ?? 0)?.[String((s as any).npcID ?? 0)] + 1)) + (((s as any).npc_willpwr ?? 0)?.[String((s as any).npcID ?? 0)])) - ((s as any).npc_rel ?? 0)?.[String((s as any).npcID ?? 0)]) > ((s as any).pcs_persuas ?? 0))) {
    ...
    ((st as any).sex_ev = (st as any).sex_ev ?? {})['preg_risk'] = 'danger';
    ((st as any).sex_ev = (st as any).sex_ev ?? {})['promise_no_creampie'] = 1;
    scene.text('"Just don\'t come inside me, okay?" you say as you get into position. ' + ((((st as any).ovulate ?? 0) > 0) ? ('"I\'m ovulating right now. I\'ll get pregnant."') : ('"I\'m ovulating soon. I\'ll get pregnant."')));
    qspGoto(st, 'sex_ev_sex', 'position_choose');
  } },
      ]);
    }
    scene.actions([
      { label: 'Continue', goto: ['sex_ev_sex', 'position_choose'] },
...
function enterReenableCondomsCode(s: GameState, scene: SceneBuilder): void {
  ((s as any).npc_bareback = (s as any).npc_bareback ?? {})[String((s as any).npcID ?? 0)] = '';
...
function enterReenableCondoms1(s: GameState, scene: SceneBuilder): void {
  if (((s as any).pc_know_npc_latex_allrg ?? 0)?.[String((s as any).npcID ?? 0)] !== 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterReenableCondoms1StopBc(st, scene); (st as any).locArgs = __savedLocArgs; }
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTryingForBaby(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterReenableCondoms2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      ]);
    }
  } },
...
function enterReenableCondoms3(s: GameState, scene: SceneBuilder): void {
  if (((s as any).pc_know_npc_latex_allrg ?? 0)?.[String((s as any).npcID ?? 0)] !== 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterCondomCompromise(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Insist', handler: (st: GameState) => {
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterReenableCondomsCode(st, scene); (st as any).locArgs = __savedLocArgs; }
    ...
    qspGoto(st, 'sex_ev_sex', 'position_choose');
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcCondoms(st, scene); (st as any).locArgs = __savedLocArgs; }
    }
  } },
    ]);
  } },
...
function enterTempDisableCondoms(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_bareback ?? 0)?.[String((s as any).npcID ?? 0)] === ''  &&  ((s as any).sex_ev ?? 0)?.['condom'] === 0) {
    ...
    ((st as any).sex_ev = (st as any).sex_ev ?? {})['pc_skip_the_condom'] = 1;
    scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
    scene.text('iif($sex_ev[\'type\'] ! \'hookup\', \'"Let\'s skip the condom today," you smile.\', \'"We don\'t need to use condoms," you smile.\')');
    ...
    ((st as any).sex_ev = (st as any).sex_ev ?? {})['must_fuck'] = 1;
    scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
    scene.text('"We don\'t need to use condoms," you smile. ' + ((((st as any).tabletkishot ?? 0) > 0) ? ('"I\'m on the shot."') : ('"I\'m on the pill."')));
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNpcTempNoCondomReact(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      ]);
    } else {
      if (((st as any).mesec ?? 0) > 0) {
    ...
    scene.actions([
      { label: 'I trust you', handler: (st: GameState) => {
    scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
    scene.text(((((st as any).sex_ev ?? 0)?.['type'] !== 'hookup') ? ('"Let\'s skip the condom today," you smile. ') : ('"We don\'t need to use condoms," you smile. ')) + '"I trust you."');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNpcTempNoCondomReact(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      { label: 'It\'ll feel better', handler: (st: GameState) => {
    scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
    scene.text(((((st as any).sex_ev ?? 0)?.['type'] !== 'hookup') ? ('"Let\'s skip the condom today," you smile. ') : ('"We don\'t need to use condoms," you smile. ')) + '"It\'ll feel better bareback."');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNpcTempNoCondomReact(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
...
function enterNpcNoCondomReact(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_condom_conscious ?? 0)?.[String((s as any).npcID ?? 0)] === 2  ||  (((s as any).npc_condom_conscious ?? 0)?.[String((s as any).npcID ?? 0)] === 1  &&  ((Math.floor(Math.random() * (100 - ((s as any).npc_willpwr ?? 0)?.[String((s as any).npcID ?? 0)] + 1)) + (((s as any).npc_willpwr ?? 0)?.[String((s as any).npcID ?? 0)])) - ((s as any).npc_rel ?? 0)?.[String((s as any).npcID ?? 0)]) > ((s as any).pcs_persuas ?? 0))) {
    ...
    if (((s as any).sex_ev ?? 0)?.['temp_bareback'] === 1) {
      ...
      ((s as any).npc_bareback = (s as any).npc_bareback ?? {})[String((s as any).npcID ?? 0)] = 'when_safe';
    }
  ...
  scene.actions([
    ...
    if ((((st as any).npc_cum_pref ?? 0)?.[String((st as any).npcID ?? 0)] === 'pullout'  ||  ((st as any).npc_arrogant ?? 0)?.[String((st as any).npcID ?? 0)] > 0  ||  ((st as any).npc_abusive ?? 0)?.[String((st as any).npcID ?? 0)] > 0)  &&  ((st as any).npc_condom_conscious ?? 0)?.[String((st as any).npcID ?? 0)] < 1) {
      ...
      if (((st as any).sex_ev ?? 0)?.['condom_count'] > ((st as any).sex_ev ?? 0)?.['condoms_used']) {
        ...
        { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoCondom2(st, scene); (st as any).locArgs = __savedLocArgs; }
      }
    }
  } },
    ]);
...
function enterPcDangerousDay(s: GameState, scene: SceneBuilder): void {
  if (((s as any).stat ?? 0)?.['preg_risk'] === 'danger'  &&  ((s as any).birth_control ?? 0)?.['think_safe'] !== 1  &&  ((s as any).succubusflag ?? 0) !== 1  &&  ((s as any).sex_ev ?? 0)?.['preg_risk'] === '') {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDayCantCumInside(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDayPullout(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDayCondomInsist(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  scene.build();
...
function enterPcDangerousDayPullout(s: GameState, scene: SceneBuilder): void {
  if (((s as any).stat ?? 0)?.['preg_risk'] === 'danger'  &&  ((s as any).birth_control ?? 0)?.['think_safe'] !== 1  &&  ((s as any).succubusflag ?? 0) !== 1) {
    ...
    scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
    if (((st as any).ovulate ?? 0) > 0) {
      scene.text(`"I'm ovulating right now," you warn ${((st as any).npcdesc ?? '')}. ` + ((((st as any).sex_ev ?? 0)?.['pullout_game'] === 1) ? ('"So you better live up to that."') : ('"Don\'t forget to pull out."')));
    } else {
      scene.text(`"I'm ovulating soon," you warn ${((st as any).npcdesc ?? '')}. ` + ((((st as any).sex_ev ?? 0)?.['pullout_game'] === 1) ? ('"So you better live up to that if you don\'t want me to get pregnant."') : ('"Don\'t forget to pull out if you don\'t want to risk getting me pregnant."')));
    }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterPcDangerousDay2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
...
function enterStartedBc1(s: GameState, scene: SceneBuilder): void {
  if (((s as any).birth_control ?? 0)?.['think_safe'] === 1) {
    ...
    if (((st as any).sex_ev ?? 0)?.['pullout_game'] === 1) {
      scene.text('"Why bother?" you ask in a mischievous tone. ' + ((((st as any).tabletkishot ?? 0) > 0) ? ('"I\'m on the shot') : ('"I\'m on the pill')) + ((((st as any).npc_fuck_times ?? 0)?.[String((st as any).npcID ?? 0)] > 1) ? (' now."') : ('."')));
    } else {
      scene.text('"Don\'t worry about pulling out," you say in a mischievous tone. ' + ((((st as any).tabletkishot ?? 0) > 0) ? ('"I\'m on the shot') : ('"I\'m on the pill')) + ((((st as any).npc_fuck_times ?? 0)?.[String((st as any).npcID ?? 0)] > 1) ? (' now."') : ('."')));
    }
    ...
    qspCall(st, 'sex_ev_stats', 'birth_control_know');
    if (((st as any).sex_ev ?? 0)?.['pullout_game'] === 1) {
      scene.text('"You better not," you say in a mischievous tone. ' + ((((st as any).tabletkishot ?? 0) > 0) ? ('"I\'m on the shot') : ('"I\'m on the pill')) + ((((st as any).npc_fuck_times ?? 0)?.[String((st as any).npcID ?? 0)] > 1) ? (' now."') : ('."')));
    } else {
      scene.text('"Don\'t you dare pull out," you warn in a mischievous tone. ' + ((((st as any).tabletkishot ?? 0) > 0) ? ('"I\'m on the shot') : ('"I\'m on the pill')) + ((((st as any).npc_fuck_times ?? 0)?.[String((st as any).npcID ?? 0)] > 1) ? (' now."') : ('."')));
    }
    scene.text(`${((st as any).npcdesc ?? '')}'s eyes light up.`);
...
function enterNoMoreFreeCreampies1(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_free_creampies ?? 0)?.[String((s as any).npcID ?? 0)] !== '') {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoMoreFreeCreampies2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  scene.build();
...
function enterNoMoreFreeCreampies2(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_free_creampies ?? 0)?.[String((s as any).npcID ?? 0)] === 'at_will') {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoMoreFreeCreampies3(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterCondoms(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
...
function enterNoMoreFreeCreampies3(s: GameState, scene: SceneBuilder): void {
  if ((((s as any).birth_control ?? 0)?.['think_safe'] === 0  &&  ((s as any).pillcon2 ?? 0) > 0)  ||  (((s as any).birth_control ?? 0)?.['remind_hour'] === 0  &&  ((s as any).tabletkicheck ?? 0) === 0)) {
    ...
    scene.text('"I\'m not on birth control anymore."');
    scene.text('"Huh? How come?"');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoMoreFreeCreampies3_bc(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  if (((s as any).stat ?? 0)?.['boyfriends_current'] > 0) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTryingForBaby(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNoMoreFreeCreampies4(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  scene.actions([
...
function enterVirginCondomAsk1(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).sex_ev ?? 0)?.['condom_setup'] !== 1) {
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterSetup(st, scene); (st as any).locArgs = __savedLocArgs; }
    }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterCheckCount(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
    ...
    if (((st as any).npc_condom_conscious ?? 0)?.[String((st as any).npcID ?? 0)] === 1) {
      ...
      if (((st as any).npc_no_condoms ?? 0)?.[String((st as any).npcID ?? 0)] > 0) {
        ...
        ((st as any).npc_no_condoms_know = (st as any).npc_no_condoms_know ?? {})[String((st as any).npcID ?? 0)] = 1;
        { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterVirginCondomAsk2(st, scene); (st as any).locArgs = __savedLocArgs; }
        { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterVirginNoCondomShock1(st, scene); (st as any).locArgs = __savedLocArgs; }
      } else {
        if (((st as any).sex_ev ?? 0)?.['condom_count'] <= 0) {
          ...
          if ((Math.floor(Math.random() * 2) + 1) === 2) {
            ...
            { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterVirginCondomAsk3(st, scene); (st as any).locArgs = __savedLocArgs; }
          }
        }
      }
    }
...
function enterVirginCondomAsk2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterVirginCondom2(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
...
function enterVirginCondomAsk3(s: GameState, scene: SceneBuilder): void {
  if (((s as any).birth_control ?? 0)?.['think_safe'] === 0) {
    ...
    if (((st as any).npc_no_condoms_know ?? 0)?.[String((st as any).npcID ?? 0)] === 1) {
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterVirginNpcDoesntCondom(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      ((st as any).sex_ev = (st as any).sex_ev ?? {})['pc_insist_condom'] = 1;
...
function enterVirginCondom2(s: GameState, scene: SceneBuilder): void {
  if (((s as any).mc_inventory ?? 0)?.['morning_after_pill'] > 0) {
    ...
    if (((st as any).LudaQW ?? 0)?.['free_condoms'] === 1  &&  ((st as any).LudaQW ?? 0)?.['luda_ma_pill'] === 0) {
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterVirginCondom3(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      scene.actions([
        { label: 'No', handler: (st: GameState) => {
