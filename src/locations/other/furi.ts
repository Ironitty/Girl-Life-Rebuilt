...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    scene.img('images/locations/highway/furi.jpg');
    (st as any).furi_recog = (Math.floor(Math.random() * 200) + 1);
    (st as any).furi_driver = (Math.floor(Math.random() * 20) + 1);
    if ((!((st as any).nroad ?? 0))) {
      scene.text('"Excuse me, sir?" you ask the driver. "I need a ride, could you give me a lift?"');
      (st as any).furi_recog_city = (Math.floor(Math.random() * 200) + 1);
    } else {
      if (((st as any).nroad ?? 0) === 10) {
        scene.text('"Excuse me, sir?" you ask the driver. "I need a ride, could you give me a lift?"');
        (st as any).furi_recog_gad = (Math.floor(Math.random() * 200) + 1);
      } else {
        if (((st as any).nroad ?? 0) === 20) {
          (st as any).furi_recog_pav = (Math.floor(Math.random() * 200) + 1);
          scene.text('"Excuse me, sir?" you ask the driver. "I need a ride, could you give me a lift?"');
        }
      }
    }
    if (((st as any).furibj ?? 0) < ((st as any).furi_recog ?? 0)) {
    ...
    if (((st as any).mc_inventory ?? 0)?.['equipped_condoms'] > 0  ||  ((st as any).mc_inventory ?? 0)?.['normal_condoms'] > 0) {
      scene.actions([
    ...
    qspCall(st, 'stat', '');
    if ((((st as any).furiswallow ?? 0) / 10) >= ((st as any).furi_driver ?? 0)) {
    ...
    // TODO-QSP: gt 'furisex', 100
      ...
      if ((((st as any).furibj ?? 0) / 10) >= ((st as any).furi_driver ?? 0)) {
        scene.text('As you start to walk over he glances up and grins at you.');
        if (((st as any).furibj ?? 0) < 50) {
