...
{ label: 'Nod', handler: (st: GameState) => {
    ...
    qspCall(st, 'stat', '');
    // TODO-QSP: dynamic text: In less than half an hour you see that your hair''s original color has been chan...
...
function enterPier(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'stat', '');
  // TODO-QSP: dynamic text: Since you agreed to adjust your piercings to fit your <<$npcrelat>>''s preferenc...
...
{ label: 'Continue', goto: ['homes_properties', 'go_home'] },
    ...
    if (((st as any).pcs_piercings ?? 0)?.['pussy'] > 0  &&  ((st as any).pcs_piercings ?? 0)?.['wearing'] > ((st as any).pierPrefTmpMax ?? 0)) {
      // TODO-QSP: dynamic text: <<$npcdesc>> gets close to your ear, "Don''t make a scene okay?", then he non ch...
...
{ label: 'Continue', goto: ['homes_properties', 'go_home'] },
    ...
    if (((st as any).pcs_piercings ?? 0)?.['pussy'] < 0  &&  ((st as any).pcs_piercings ?? 0)?.['wearing'] < ((st as any).pierPrefTmpMin ?? 0)) {
      // TODO-QSP: dynamic text: <<$npcdesc>> gets close to your ear, "Don''t make a scene okay?", then <<$Xe>> n...
...
{ label: 'Continue', goto: ['homes_properties', 'go_home'] },
    ...
    return;
        ...
        if (((st as any).pierPrefTmpMin ?? 0) > 1) {
          // TODO-QSP: dynamic text: You can tell <<$Xe>> likes a girl with a more piercings and you can''t help but ...
...
function enterLips(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: Even though you don''t know what <<$npcdesc>> said to the surgeon, you still jum...
...
function enterFigure(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (qspFunc(s, 'lover_pref', 'check_bmi') === 0  &&  ((st as any).lc_counter ?? 0) < 10) {
    ...
    if (((st as any).figurePrefMin ?? 0) < 19) {
      ...
      if (((st as any).figurePrefMin ?? 0) < 25) {
        // TODO-QSP: dynamic text: You can tell that <<$npcdesc>> likes a girl that''s not too fat, but thankfully ...
