...
function enterInit(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).date_ev = (s as any).date_ev ?? {})[''] = 'home_date';
...
function enterGetReady2(s: GameState, scene: SceneBuilder): void {
  if (((s as any).date_loc ?? 0) === 'player_home') {
    if (((s as any).player_home_has_living ?? 0) === 1) {
      scene.actions([
        { label: 'Settle down on the couch', goto: ['date_chill', 'settle_down1'] },
      ]);
    } else {
      scene.actions([
        { label: 'Settle down on the bed', goto: ['date_chill', 'settle_down1'] },
      ]);
    }
...
function enterNetflixMovie1(s: GameState, scene: SceneBuilder): void {
  scene.text(String(qspFunc(s, 'date_ev', 'watching_image') || ''));
