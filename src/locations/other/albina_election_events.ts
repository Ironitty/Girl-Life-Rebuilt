...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Miss ${((s as any).pcs_lastname ?? 0)}, you are a member of the front that was pretending to be Miss Barlovskaya's dance entourage. These criminal activities are about to be put on hold permanently, with or without your collaboration."`);
    ...
    scene.text(`He places a hand gently on your shoulder. "We're finally going to put him behind bars where he belongs, but we could use your help, Miss ${((s as any).pcs_lastname ?? 0)}, however small it seems."`);
    scene.actions([
...
function enterGiveStatement(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"You've really helped us here, Miss ${((s as any).pcs_lastname ?? 0)}. Thank you."`);
