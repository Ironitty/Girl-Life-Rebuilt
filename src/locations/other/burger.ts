...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('There is a <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027butoilet/u0027, /u0027/u0027); return false;">WC</a> for visitors.');
  if (((s as any).BurgerQW ?? 0)?.['BossSlut'] > 0) {
    scene.text('<a href="#" onclick="window.__gameStore.getState().doGoto(/u0027burger/u0027, /u0027boss/u0027); return false;">The Office of the Director</a> Anatoly Borisovich');
  }
  ...
  if (((s as any).job_status ?? 0)?.['city_diner_secretary'] !== 'employed') {
    if (((s as any).job_status ?? 0)?.['city_diner_dishwasher'] === 'employed'  &&  ((s as any).job_last_work_day ?? 0)?.['city_diner_dishwasher'] !== ((s as any).daystart ?? 0)  &&  qspFunc(s, 'jobs', 'is_arrival_time', 'city_diner_dishwasher') === 1) {
      scene.actions([
        { label: '', labelFn: (s: GameState) => 'Wash the dishes for ' + String(qspFunc(s, 'money', 'string_profit', 70) ?? '') + ' (1:00)', goto: ['burger', 'dishes'] },
      ]);
    }
    if (((s as any).job_status ?? 0)?.['city_diner_floor_washer'] === 'employed'  &&  ((s as any).job_last_work_day ?? 0)?.['city_diner_floor_washer'] !== ((s as any).daystart ?? 0)  &&  qspFunc(s, 'jobs', 'is_arrival_time', 'city_diner_floor_washer') === 1) {
      scene.actions([
        { label: '', labelFn: (s: GameState) => 'Wash the floor for ' + String(qspFunc(s, 'money', 'string_profit', 70) ?? '') + ' (1:00)', goto: ['burger', 'floor'] },
      ]);
    }
  }
  // TODO-QSP: end
...
function enterOffice(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).BurgerQW ?? 0)?.['IlyQWEvidence'] === 1) {
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQW'] = 6;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWTatianaHelp'] = 0;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWEvidence'] = 0;
    qspCall(st, 'stat', '');
  ...
  scene.actions([
    { label: 'Leave', goto: ['burger', 'start'] },
    { label: 'Repay the debt', handler: (st: GameState) => {
    if (qspFunc(s, 'money', 'can_afford', 650000) === 0) {
      s.scene = { ...s.scene, mainText: String((st as any).noMoney || ''), curActs: [] };
    } else {
      (st as any).minut = ((st as any).minut ?? 0) + 5;
      qspCall(st, 'money', 'pay', 650000);
      ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQW'] = 5;
      qspCall(st, 'stat', '');
...
function enterApply(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).location_type = 'public_indoors';
  (s as any).minut = ((s as any).minut ?? 0) + 5;
  // TODO-QSP: dynamic text: You go into the manager''s office where you meet a smart girl. She begins to tel...
  ...
  scene.actions([
    ...
    qspCall(st, 'jobs', 'set_employed', 'city_diner_dishwasher');
...
function enterManager(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'jobs', 'set_employed', 'city_diner_floor_washer');
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['sergeyQWServiced1'] = 0;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['sergeyQWServiced2'] = 0;
    scene.text('You agree to work.');
    scene.actions([
      { label: 'Leave', goto: ['burger', 'start'] },
    ]);
...
function enterDishes(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).BurgerQW ?? 0)?.['Level'] === 10) {
    ((s as any).BurgerQW = (s as any).BurgerQW ?? {})['Level'] = 0;
  }
  ...
  if (((s as any).pcs_apprnc ?? 0) >= 80  &&  (((s as any).job_shifts_total ?? 0)?.['city_diner_dishwasher'] + ((s as any).job_shifts_total ?? 0)?.['city_diner_floor_washer']) >= 7  &&  ((s as any).BurgerQW ?? 0)?.['Level'] < 10) {
    if (((s as any).BurgerQW ?? 0)?.['Level'] === 1) {
      ...
      scene.text(`Out of the his office comes the owner of the bistro smiling at you. "${((s as any).pcs_nickname ?? '')}. My offer still stands. Would you like to go to a restaurant?"`);
    } else {
      if (((s as any).BurgerQW ?? 0)?.['Level'] === 0) {
        ((s as any).BurgerQW = (s as any).BurgerQW ?? {})['Level'] = 1;
    ...
    qspGoto(st, 'burger', 'start');
  } },
      ]);
    }
    scene.actions([
{ label: 'Agree', handler: (st: GameState) => {
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 10;
    ...
    qspCall(st, 'npcStat', 'A74');
    scene.actions([
      { label: 'Get in the car', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 15;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 20;
    qspCall(st, 'stat', '');
    ...
    scene.text(`"I think you should take this one. It is the most expensive one. And I think, ${((st as any).pcs_nickname ?? '')}, you should be very thankful for such a gift."`);
    ...
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Thank you very much. (Deny)', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 50;
    (st as any).guy = ((st as any).guy ?? 0) + (1);
    qspCall(st, 'clothing', 'add_item', 'cats_dress', 23);
    ...
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/shared/car.jpg');
    // TODO-QSP: dynamic text: "Whew, <<$pcs_nickname>>. I''m a little tired. Let''s go to a restaurant some ot...
    scene.text(`"Whew, ${((st as any).pcs_nickname ?? '')}. I'm a little tired. Let's go to a restaurant some other time." Anatoly tells you, taking you by the hand as he leads you to his car. You quickly get to your home and Anatoly says goodbye to you.`);
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 50;
    (st as any).guy = ((st as any).guy ?? 0) + (1);
    qspCall(st, 'clothing', 'add_item', 'cats_dress', 24);
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'bj', 2, 'sub', 'deepthroat');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 4, 'sub', 'humiliation');
    qspCall(st, 'npcStat', 'A74');
    qspCall(st, 'cum_call', 'mouth_swallow', 'A74', 1, 0, 10000, 13);
    qspCall(st, 'cum_call', 'face', 'A74', 1, 0, 10000, 27);
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    scene.text(`You walk out of the boutique with Anatoly Borisovich. "Whew, ${((st as any).pcs_nickname ?? '')}. I'm a little tired. Let's go to a restaurant some other time." Anatoly tells you, taking you by the hand as he leads you to his car. You quickly get to your home and Anatoly says goodbye to you.`);
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Out of the car', goto: ['city_residential', ''] },
    ]);
    ...
    if (((s as any).BurgerQW ?? 0)?.['Level'] === 20) {
      ...
      if (((s as any).BurgerQW ?? 0)?.['Level'] === 56) {
        // TODO-QSP: dynamic text: When you''re finished you are suddenly approached by the manager Ilyushkin. "Oka...
        scene.text(`When you're finished you are suddenly approached by the manager Ilyushkin. "Okay ${((s as any).pcs_nickname ?? '')}. I'm sorry. I was wrong about you. I do not know what got into me. We are currently looking for an Assistant Manager. I think you can do it. You have the nature for it. Well? What do you say?"`);
        scene.actions([
{ label: 'Yes', handler: (st: GameState) => {
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 57;
    qspCall(st, 'stat', '');
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 111;
    qspCall(st, 'stat', '');
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlySecurityCamDesc'] = 'You look into the envelope. Then you close the safe and head to the exit.';
    qspCall(st, 'stat', '');
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWDay'] = ((st as any).daystart ?? 0);
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQW'] = 1;
    qspCall(st, 'jobs', 'set_fired', 'city_diner_dishwasher');
    if (((st as any).job_status ?? 0)?.['city_diner_floor_washer'] === 'employed') {
      qspCall(st, 'jobs', 'set_fired', 'city_diner_floor_washer');
    }
    qspCall(st, 'stat', '');
    ...
    scene.actions([
      ...
      { label: 'Take the envelope to Ilyushkin', handler: (st: GameState) => {
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlySecurityCamDesc'] = 'You take the envelope, close the safe and head out.';
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWDay'] = ((st as any).daystart ?? 0);
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQW'] = 1;
    qspCall(st, 'jobs', 'set_fired', 'city_diner_dishwasher');
    if (((st as any).job_status ?? 0)?.['city_diner_floor_washer'] === 'employed') {
...
{ label: 'No', handler: (st: GameState) => {
    ...
    scene.actions([
      { label: 'Leave', goto: ['burger', 'start'] },
        ...
        if (((s as any).BurgerQW ?? 0)?.['Level'] === 50) {
          ...
          scene.text(`During your work, you learned that Anatoly Borisovich is away on business affairs and Ilyushkin is replacing him as the manager. When you are getting ready to go to work Ilyushkin comes out of the manager's office and says in a conspiratorial tone, "Well ${((s as any).pcs_nickname ?? '')}, you obviously do not want to work for a couple of pennies? Come to me for special work. Special work for special Salary."`);
          scene.actions([
            { label: 'Go to the manager\'s office', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 15;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 55;
    ...
    qspCall(st, 'willpower', 'pay', 'resist');
    qspCall(st, 'stat', '');
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 56;
    scene.img('images/locations/city/citycenter/diner/sex/bj1.jpg');
    ...
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'That\'s bullshit!', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
    ...
    qspCall(st, 'arousal', 'bj', 3, 'sub', 'deepthroat', 'prostitution');
    qspCall(st, 'stat', '');
    scene.actions([
...
{ label: 'Bend over', handler: (st: GameState) => {
    ...
    qspCall(st, 'arousal', 'bj', 3, 'sub', 'prostitution');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    return;
          ...
          if (((s as any).BurgerQW ?? 0)?.['Level'] === 55) {
            ...
            scene.actions([
              { label: 'Go to the manager\'s office', handler: (st: GameState) => {
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 60;
    ...
    // TODO-QSP: dynamic text: Afterwards, Ilyushkin takes out <<$func(''money'', ''string_profit'', 500)>> and...
    ...
    (st as any).guy = ((st as any).guy ?? 0) + (1);
    qspCall(st, 'npcStat', 'A75');
    qspCall(st, 'arousal', 'bj', 3, 'sub');
    qspCall(st, 'cum_call', 'mouth_swallow', 'A75', 1);
    ...
    // TODO-QSP: dynamic text: "Guys would you like to blow off some steam on this nymph? Don''t worry, she enj...
    scene.text(`"Guys would you like to blow off some steam on this nymph? Don't worry, she enjoys it. She is here to serve. Am I right ${((st as any).pcs_nickname ?? '')}?`);
    qspCall(st, 'willpower', 'group', 'resist', 'medium');
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Refuse', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
      ...
      scene.actions([
        { label: 'Refuse', handler: (st: GameState) => {
    qspCall(st, 'willpower', 'pay', 'resist');
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 61;
    qspGoto(st, 'burger', 'start');
  } },
      ]);
    }
    scene.actions([
      { label: 'Yes', handler: (st: GameState) => {
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['Level'] = 70;
    ...
    qspCall(st, 'arousal', 'hj', (-1), ((st as any).npcID1 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'hj', (-1), ((st as any).npcID2 ?? 0), 'sub', 'group');
    qspCall(st, 'arousal', 'hj', (-1), ((st as any).npcID3 ?? 0), 'sub', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
...
{ label: 'Use your mouth', handler: (st: GameState) => {
    ...
    qspCall(st, 'npcStat', 'A75');
    qspCall(st, 'arousal', 'bj', 3, 'sub', 'prostitution');
    qspCall(st, 'arousal', 'hj', (-3), 'sub', 'prostitution');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'money', 'earn', 300);
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['sergeyQWServiced2'] = ((st as any).BurgerQW['sergeyQWServiced2'] ?? 0) + (1);
    if (((st as any).BurgerQW ?? 0)?.['sergeyQWServiced2'] === 10) {
      ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['sergeyQWServiced1'] = 3;
    }
    ...
    scene.actions([
      { label: 'Leave', goto: ['burger', 'start'] },
                ...
                if ((((s as any).BurgerQW ?? 0)?.['Level'] === 57  &&  ((s as any).BurgerQW ?? 0)?.['BossSlut'] === 0)  ||  (((s as any).BurgerQW ?? 0)?.['Level'] >= 65  &&  ((s as any).BurgerQW ?? 0)?.['BossSlut'] === 0  &&  ((s as any).BurgerQW ?? 0)?.['sergeyQWServiced1'] === 3)) {
                  ((s as any).BurgerQW = (s as any).BurgerQW ?? {})['BossSlut'] = 1;
                  if (((s as any).BurgerQW ?? 0)?.['sergeyQWServiced1'] === 3) {
                    ((s as any).BurgerQW = (s as any).BurgerQW ?? {})['sergeyQWServiced1'] = 4;
                  }
                  // TODO-QSP: dynamic text: After you finish, you see Anatoly Borisovich. He nods and tells you "Hello <<$pc...
                  scene.text(`After you finish, you see Anatoly Borisovich. He nods and tells you "Hello ${((s as any).pcs_nickname ?? '')}, when you're done, come to me."`);
                  scene.actions([
{ label: 'Leave', goto: ['burger', 'start'] },
]);
                  return;
...
function enterBoss(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).BurgerQW ?? 0)?.['BossSlut'] === 10) {
    qspGoto(s, 'burger', 'bossSlut_10');
  } else {
    if (((s as any).BurgerQW ?? 0)?.['BossSlut'] === 1) {
      qspGoto(s, 'burger', 'bossSlut_01');
    } else {
      if (((s as any).BurgerQW ?? 0)?.['BossSlut'] === 2  &&  ((s as any).BurgerQW ?? 0)?.['BossSlutLastServed'] === ((s as any).daystart ?? 0)) {
        ...
        if (((s as any).BurgerQW ?? 0)?.['BossSlut'] === 2) {
          ((s as any).BurgerQW = (s as any).BurgerQW ?? {})['BossSlutLastServed'] = ((s as any).daystart ?? 0);
    ...
    qspCall(st, 'money', 'earn', 300);
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['BossSlutLastServed'] = ((st as any).daystart ?? 0);
    ...
    qspCall(st, 'npcStat', 'A74');
    qspCall(st, 'arousal', 'bj', 5, 'sub', 'prostitution');
...
function enterBossSlut_01(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['BossSlut'] = 10;
    qspCall(st, 'stat', '');
    if (((st as any).BurgerQW ?? 0)?.['Level'] === 57) {
      ((st as any).mc_inventory = (st as any).mc_inventory ?? {})['cosmetics'] = ((st as any).mc_inventory['cosmetics'] ?? 0) + (200);
      ...
      if (((st as any).license ?? 0)?.['secretary'] === 1) {
        scene.actions([
    ...
    scene.actions([
      { label: 'Leave', goto: ['burger', 'start'] },
        ...
        if (((st as any).license ?? 0)?.['secretary'] === 0  &&  ((st as any).license ?? 0)?.['secretary_enrolled'] > 0) {
          scene.actions([
    ...
    scene.actions([
      ...
      if (((st as any).BurgerQW ?? 0)?.['Level'] >= 65) {
    ...
    qspGoto(st, 'burger', 'start');
        ...
        scene.actions([
          { label: 'Get on your knees', handler: (st: GameState) => {
    qspCall(st, 'money', 'earn', 300);
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['BossSlut'] = 2;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['BossSlutLastServed'] = ((st as any).daystart ?? 0);
    scene.img('images/characters/city/anatoly/sex/bj2.jpg');
    scene.text('You kneel down and take the money from the floor. Anatoly Borisovich pulls his already hard member out and holds it in front of your face. You wrap your lips around it and begin to suck as the hard cock fills your mouth. Anatoly Borisovich grits his teeth with pleasure "Suck it slut and don\'t forget to swallow every last drop!" You feel his sperm hit the inside of your mouth and begin to quickly swallow his cum to prevent yourself from choking. After you finish, Anatoly Borisovich puts his dick back in his pants. "Now get out."');
    qspCall(st, 'npcStat', 'A74');
...
function enterBossSlut_10(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).job_status ?? 0)?.['city_diner_secretary'] === 'fired') {
    if (qspFunc(s, 'jobs', 'is_arrival_time', 'city_diner_secretary') === 1) {
      // TODO-QSP: dynamic text: Anatoly Borisovich looks at you sternly, "<<$pcs_nickname>>, you''re fired. Go a...
      scene.text(`Anatoly Borisovich looks at you sternly, "${((s as any).pcs_nickname ?? '')}, you're fired. Go away, we're done, I love my wife more than you."`);
    }
  } else {
    if (((s as any).job_status ?? 0)?.['city_diner_secretary'] === 'employed') {
      if (qspFunc(s, 'jobs', 'is_arrival_time', 'city_diner_secretary') === 1) {
        if ((!((s as any).POffice ?? 0))) {
          // TODO-QSP: dynamic text: Anatoly Borisovich looks at you sternly, "<<$pcs_nickname>>, why aren''t you dre...
          scene.text(`Anatoly Borisovich looks at you sternly, "${((s as any).pcs_nickname ?? '')}, why aren't you dressed properly? Go home and come back tomorrow dressed appropriately, you're not working today!"`);
        } else {
          scene.actions([
            { label: 'Work', goto: ['burgerWork', 'start'] },
            { label: 'Resign', handler: (st: GameState) => {
    qspCall(st, 'jobs', 'set_terminated', 'city_diner_secretary');
    ...
    if (((st as any).job_status ?? 0)?.['city_diner_secretary'] === ''  &&  qspFunc(s, 'jobs', 'check_employment_possible', 'city_diner_secretary') === 1) {
      scene.actions([
