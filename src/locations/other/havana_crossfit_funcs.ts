...
function enterSetupWod(s: GameState, scene: SceneBuilder): void {
  (s as any).crossfit_wods = undefined;
  (s as any).wod_min_score = undefined;
  (s as any).wod_max_score = undefined;
  (s as any).wod_type = undefined;
  (s as any).wod_video = undefined;
...
function enterAddWod(s: GameState, scene: SceneBuilder): void {
  (s as any).crossfit_wods = [...((s as any).crossfit_wods ?? []), ((s as any).locArgs?.[1] ?? 0)];
  ((s as any).wod_min_score = (s as any).wod_min_score ?? {})[((s as any).locArgs?.[1] ?? 0)] = ((s as any).locArgs?.[2] ?? 0);
  ((s as any).wod_max_score = (s as any).wod_max_score ?? {})[((s as any).locArgs?.[1] ?? 0)] = ((s as any).locArgs?.[3] ?? 0);
  ((s as any).wod_type = (s as any).wod_type ?? {})[((s as any).locArgs?.[1] ?? 0)] = ((s as any).locArgs?.[4] ?? 0);
  ((s as any).wod_video = (s as any).wod_video ?? {})[((s as any).locArgs?.[1] ?? 0)] = ((s as any).locArgs?.[5] ?? 0);
  scene.build();
}
    ...
    if (((s as any).crossfit_record_it ?? 0) < Object.keys((s as any).crossfit_wods ?? {}).length) {
      ...
      if (((s as any).box_wod_record ?? 0)?.[String((s as any).crossfit_wod ?? 0)] === 0  ||  ((s as any).box_wod_record ?? 0)?.[String((s as any).crossfit_wod ?? 0)] > ((s as any).new_record_wod_time ?? 0)) {
        ...
        if ((Math.floor(Math.random() * 3) + 0) === 0  ||  ((s as any).box_wod_record_holder ?? 0)?.[String((s as any).crossfit_wod ?? 0)] === ''  ||  ((s as any).box_wod_record_holder ?? 0)?.[String((s as any).crossfit_wod ?? 0)] === ((s as any).pcs_nickname ?? 0)) {
          ((s as any).box_wod_record_holder = (s as any).box_wod_record_holder ?? {})[String((s as any).crossfit_wod ?? 0)] = qspFunc(s, 'npcrnamefile', 'rusFemale');
        }
      }
