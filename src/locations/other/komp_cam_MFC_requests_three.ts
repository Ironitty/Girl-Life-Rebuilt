...
function enterShowPussy(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'komp_cam_functions', 'camming', 5, 'erotic', 'request');
    qspCall(st, 'stat', '');
  ...
  scene.actions([
    ...
    if (((st as any).analPlugIn ?? 0) === 1) {
      ...
      if (qspFunc(s, 'pcs_has_attr', 'sex_virgin')) {
        (st as any).img_source = 18;
      } else {
        if (((st as any).pcs_vag ?? 0) <= 10) {
          (st as any).img_source = 13;
        } else {
          if (((st as any).pcs_vag ?? 0) <= 15) {
            (st as any).img_source = 14;
          } else {
            if (((st as any).pcs_vag ?? 0) <= 25) {
              (st as any).img_source = 15;
            } else {
              if (((st as any).pcs_vag ?? 0) <= 35) {
                (st as any).img_source = 16;
              }
            }
          }
        }
