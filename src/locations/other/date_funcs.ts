...
function enterDateMood(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === 'improve') {
    if (((s as any).date_ev ?? 0)?.['talk_mood'] === 'awkward') {
      if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['talk_mood'] = 'good';
    } else {
      if (((s as any).date_ev ?? 0)?.['talk_mood'] === 'good') {
        if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['talk_mood'] = 'great';
      } else {
        if (((s as any).date_ev ?? 0)?.['talk_mood'] === 'great') {
          if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['talk_mood'] = 'amazing';
        } else {
          if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['talk_mood'] = 'good';
        }
      }
    ...
    if (((s as any).locArgs?.[1] ?? 0) === 'worsen') {
      if (((s as any).date_ev ?? 0)?.['talk_mood'] === 'awkward') {
        if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['talk_mood'] = 'bad';
      } else {
        if (((s as any).date_ev ?? 0)?.['talk_mood'] === 'good') {
...
function enterFirstTime(s: GameState, scene: SceneBuilder): void {
  if ((String(((s as any).npc_date_locs ?? 0)?.[String((s as any).npcID ?? 0)]).indexOf(String(';" + $ARGS[1] + ";'))) + 1 <= 0) {
    if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['first_time'] = 1;
  }
  // TODO-QSP: end
  scene.build();
}
...
function enterDateTracker(s: GameState, scene: SceneBuilder): void {
  if (((s as any).date_ev ?? 0)?.['tracker'] !== ((s as any).locArgs?.[1] ?? 0)) {
    if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['loc'] = ((s as any).locArgs?.[1] ?? 0);
...
function enterSetContinue(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['cont_loc'] = ((s as any).locArgs?.[1] ?? 0);
  if (!(s as any).date_ev) (s as any).date_ev = {}; (s as any).date_ev['cont_arg'] = ((s as any).locArgs?.[2] ?? 0);
  // TODO-QSP: end
  scene.build();
}

function enterFlashStats(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).npc_date_flash) (s as any).npc_date_flash = {}; (s as any).npc_date_flash[String((s as any).npcID ?? 0)] = ((s as any).npc_date_flash[String((s as any).npcID ?? 0)] ?? 0) + (1);
