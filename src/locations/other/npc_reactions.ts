...
function enterStat(s: GameState, scene: SceneBuilder): void {
  return;
  // TODO-QSP: end
  if (Number((s as any).locArgs?.[1] ?? 0) === 'cikl') {
    if (((s as any).week ?? 0) === 1) {
      // TODO-QSP: killvar 'npc_reaction_weekly'
    }
    return;
  }
  if (((s as any).npc_preferences ?? 0)[Number((s as any).locArgs?.[1] ?? 0)] !== '') {
  }
...
function enterGeneral(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_reaction_first_time ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_tan'] === 1  &&  (!((s as any).pcs_tan ?? 0))) {
    ((s as any).npc_reaction_first_time = (s as any).npc_reaction_first_time ?? {})[String(((s as any).locArgs?.[1] ?? 0)) + '_tan'] = 0;
  }
  if (qspFunc(s, 'pcs_has_attr', 'cum_face')  &&  ((s as any).npc_reaction_hourly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_cum_on_face'] === 0) {
    (s as any).result = qspFunc(s, 'npc_reactions', 'cum_on_face', ((s as any).locArgs?.[1] ?? 0), ((s as any).locArgs?.[2] ?? 0));
  } else {
    if (qspFunc(s, 'pcs_has_attr', 'cum_clothes')  &&  ((s as any).npc_reaction_hourly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_cum_on_clothes'] === 0) {
      (s as any).result = qspFunc(s, 'npc_reactions', 'cum_on_clothes', ((s as any).locArgs?.[1] ?? 0), ((s as any).locArgs?.[2] ?? 0));
    } else {
      if (qspFunc(s, 'pcs_has_attr', 'body_pregnant')  &&  ((s as any).npc_reaction_pregnant ?? 0)[Number((s as any).locArgs?.[1] ?? 0)] === 0) {
        (s as any).result = qspFunc(s, 'npc_reactions', 'pregnant', ((s as any).locArgs?.[1] ?? 0), ((s as any).locArgs?.[2] ?? 0));
      } else {
        if (((s as any).npc_reaction_weekly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_fit'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'body_fit')) {
            // TODO-QSP: $npc_reaction_choice[] = 'fit'
          }
        }
        if (((s as any).npc_reaction_weekly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_piercing'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'cosmetics_piercing_visible')) {
            // TODO-QSP: $npc_reaction_choice[] = 'piercing'
          }
        }
        if (((s as any).npc_reaction_weekly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_strong'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'stats_strength_strong')) {
            // TODO-QSP: $npc_reaction_choice[] = 'strong'
          }
        }
        if (((s as any).npc_reaction_first_time ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_tan'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'body_tan')) {
            // TODO-QSP: $npc_reaction_choice[] = 'tan'
          }
        }
        if (((s as any).npc_reaction_first_time ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_tattoo'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'cosmetics_tattoo_visible')) {
            // TODO-QSP: $npc_reaction_choice[] = 'tattoo'
          }
        }
        if (((s as any).npc_reaction_weekly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_bimbo'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'clothes_style_bimbo')) {
            // TODO-QSP: $npc_reaction_choice[] = 'bimbo_look'
          }
        }
        if (((s as any).npc_reaction_weekly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_goth'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'clothes_style_goth')) {
            // TODO-QSP: $npc_reaction_choice[] = 'goth_look'
          }
        }
        if (((s as any).npc_reaction_weekly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_punk'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'clothes_style_punk')) {
            // TODO-QSP: $npc_reaction_choice[] = 'punk_look'
          }
        }
        if (((s as any).npc_reaction_daily ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_short_skirt'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'clothes_skirt_short')) {
            // TODO-QSP: $npc_reaction_choice[] = 'short_skirt'
          }
        }
        if (((s as any).npc_reaction_daily ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_clothes_thinnes'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'clothes_thin_high')) {
            // TODO-QSP: $npc_reaction_choice[] = 'clothes_thinnes'
          }
        }
        if (((s as any).npc_reaction_weekly ?? 0)[Number((s as any).locArgs?.[1] ?? 0) + '_makeup'] === 0) {
          if (qspFunc(s, 'pcs_has_attr', 'cosmetics_makeup_any')) {
            // TODO-QSP: $npc_reaction_choice[] = 'makeup'
          }
        }
        (s as any).npc_reaction_chosen = ((s as any).npc_reaction_choice ?? 0)?.[((s as any).rand ?? 0)(0, ((s as any).arrsize ?? 0)('npc_reaction_choice') - 1)];
...
function enterBimboLook(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_nr_pref_values ?? 0)?.['clothes_style_bimbo'] > 0  ||  ((s as any).npc_reactions_temp_preferences ?? 0)?.['bimbo'] === 1) {
    if (((s as any).npc_rel ?? 0)[Number((s as any).locArgs?.[1] ?? 0)] > 80  ||  Number((s as any).locArgs?.[2] ?? 0) === 'love') {
      (s as any).result = '"You\'re gorgeous! I adore your fashion sense!" ' + ((s as any).nr_temp_npc_name ?? 0) + ' says';
    } else {
      if (((s as any).npc_rel ?? 0)[Number((s as any).locArgs?.[1] ?? 0)] > 60  ||  Number((s as any).locArgs?.[2] ?? 0) === 'like') {
        (s as any).result = '"That\'s a great look. It should never be changed!" ' + ((s as any).nr_temp_npc_name ?? 0) + ' says';
      } else {
        if (((s as any).npc_rel ?? 0)[Number((s as any).locArgs?.[1] ?? 0)] > 40  ||  Number((s as any).locArgs?.[2] ?? 0) === 'indifferent') {
          (s as any).result = '"At the very least, you know how to dress correctly" ' + ((s as any).nr_temp_npc_name ?? 0) + ' says';
        } else {
          if (((s as any).npc_rel ?? 0)[Number((s as any).locArgs?.[1] ?? 0)] > 20  ||  Number((s as any).locArgs?.[2] ?? 0) === 'dislike') {
