...
function enterWhatToWatchPav(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTicketMenu(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
...
function enterCityCinema(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterBoyFilmSuggestReact(s, scene); (st as any).locArgs = __savedLocArgs; }
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterSuggestFilm3(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      ]);
    }
    if (((st as any).date_ev ?? 0)?.['film_suggest'] !== 'horror') {
...
function enterSuggestFilm3(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_argumentative ?? 0)?.[String((s as any).npcID ?? 0)] > 0  &&  ((s as any).npc_assertive ?? 0)?.[String((s as any).npcID ?? 0)] < 1) {
    ...
    // TODO-QSP: dynamic text: "Why''d you ask what I wanted if you just wanted to watch that one anyways?" <<$...
...
function enterAskedToPayStart(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('"Why me?"');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterAskedToPay(s, scene); (st as any).locArgs = __savedLocArgs; }
  ...
  scene.actions([
    ...
    if (((st as any).npc_humor ?? 0)?.[String((st as any).npcID ?? 0)] === 'perverted'  ||  ((st as any).npc_misogynist ?? 0)?.[String((st as any).npcID ?? 0)] > 0  ||  ((st as any).npc_rel_goal ?? 0)?.[String((st as any).npcID ?? 0)] === 'sex'  ||  ((st as any).npc_sexdrive ?? 0)?.[String((st as any).npcID ?? 0)] > (Math.floor(Math.random() * 6) + 5)) {
      if (((st as any).npc_fav_pos ?? 0)?.[String((st as any).npcID ?? 0)] === 'blowjob') {
        // TODO-QSP: dynamic text: "Oh yeah?" <<$npcdesc>>''s attention suddenly seems more focused on you as he gr...
...
function enterMovieInterlude(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_fav_date ?? 0)?.[String((s as any).npcID ?? 0)] !== 'movie_date'  &&  (Math.floor(Math.random() * 11) + 0) < ((s as any).npc_sexdrive ?? 0)?.[String((s as any).npcID ?? 0)]) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTheaterBjMenu(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      ]);
    } else {
...
function enterBoredWatching(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterReachCock(s, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Second half', handler: (st: GameState) => {
    // TODO-QSP: gt 'date_movie', 'watch_<<$date_ev[''film_decide'']>>_movie2...
  } },
  ...
  scene.actions([
    ...
    if (((st as any).npc_fav_date ?? 0)?.[String((st as any).npcID ?? 0)] === 'movie_date') {
      ...
      if ((Math.floor(Math.random() * 11) + 0) < ((st as any).npc_sexdrive ?? 0)?.[String((st as any).npcID ?? 0)]) {
        ...
        { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterMovieInterludeEnd(s, scene); (st as any).locArgs = __savedLocArgs; }
      }
    } else {
...
function enterTheaterBjDistracted(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspGoto(st, 'date_movie', 'theater_bj_cum_pre');
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTheaterBjCumPre(s, scene); (st as any).locArgs = __savedLocArgs; }
    }
  } },
  ]);
  scene.build();
...
function enterFilmTalkPc(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterFilmTalkBoy(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
...
function enterFilmTalkBoy(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).date_ev ?? 0)?.['npc_opinion'] !== '') {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterDateEnd(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
...
function enterDateEnd(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Continue', handler: (st: GameState) => {
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterDateEndStats(s, scene); (st as any).locArgs = __savedLocArgs; }
