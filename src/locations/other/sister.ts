  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'anya') {
    ...
    dynamicGoto(st, 'loc', 'loc_arg');
          ...
          if (((s as any).alko ?? 0) >= 3) {
            ...
            scene.text(`"${((s as any).pcs_nickname ?? 0)}, how the hell did you get so drunk already? I was going to invite you to a party, but you look like you've already been to one! Go home and sleep it off!"`);
            scene.actions([
              { label: 'Move away', handler: (st: GameState) => {
    dynamicGoto(st, 'loc', 'loc_arg');
  } },
    ...
    if (((s as any).locArgs?.[1] ?? 0) === 'rex') {
      if (((s as any).fame ?? 0)?.['pav_slut'] >= 250  ||  ((s as any).cumloc ?? 0)[11] === 1  ||  (((s as any).cumloc ?? 0)[6] === 1  ||  ((s as any).cumloc ?? 0)[7] === 1)) {
        ...
        if (((s as any).fame ?? 0)?.['pav_slut'] >= 250) {
          ...
          scene.text(`"${((s as any).pcs_nickname ?? 0)}, everyone's saying you're a fucking whore. I thought we were an item and things were getting serious, but you're fucking everything that moves?"`);
    ...
    dynamicGoto(st, 'loc', 'loc_arg');
          ...
          if (((s as any).alko ?? 0) >= 3) {
            ...
            scene.text(`"${((s as any).pcs_nickname ?? 0)}, are you drunk? I wanted to invite you to the party, but you look like you just left one! Go home and sleep it off."`);
            scene.actions([
              { label: 'Move away', handler: (st: GameState) => {
    dynamicGoto(st, 'loc', 'loc_arg');
  } },
...
function enterScene1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_QW ?? 0)?.['A33'] === 1) {
    ...
    scene.text(`"${((s as any).pcs_nickname ?? 0)}, you wanted to meet my boyfriend, right? Well, this is Roma. Roma, this is my sister, ${((s as any).pcs_nickname ?? 0)}."`);
  ...
  if (((s as any).reksLike ?? 0) === 1  &&  ((s as any).toiletRexSex ?? 0) === 0  &&  ((s as any).sisterQW ?? 0)?.['partycount'] > 0  &&  (!((s as any).rex_breakup ?? 0))) {
    ...
    scene.actions([
      ...
      if (((s as any).alko ?? 0) > 2  &&  ((s as any).temp ?? 0) <= 20  &&  ((s as any).npc_rel ?? 0)?.['A57'] <= 60) {
        ...
        scene.text(`Already pretty tipsy, Rex approaches you. "Let me refill your glass, ${((s as any).pcs_nickname ?? 0)}…" However, he spills some on you, seemingly by accident. "Oh sorry! Here, I'll clean that up for you."`);
    ...
    scene.actions([
      { label: 'Continue to drink beer', goto: ['sister', 'scene2'] },
        ...
        if (((s as any).rexPark ?? 0) === 1  &&  ((s as any).rexParkDay ?? 0) < ((s as any).daystart ?? 0)  &&  ((s as any).rexSisTalk ?? 0) === 2) {
          ...
          scene.text(`Rex comes up to you. "${((s as any).pcs_nickname ?? 0)}, I looked like a real chump waiting for you in the park yesterday! That wasn't very nice…"`);
          scene.actions([
            { label: 'Continue to drink beer', goto: ['sister', 'scene2'] },
          ]);
        } else {
          if (((s as any).rexCar ?? 0) === 1  &&  ((s as any).rexCarDay ?? 0) < ((s as any).daystart ?? 0)  &&  ((s as any).rexSisTalk ?? 0) === 3) {
            ...
            scene.text(`Rex comes up to you. "${((s as any).pcs_nickname ?? 0)}, I was waiting at your door for almost two hours last night! It wasn't very nice to ditch me like that…"`);
            scene.actions([
              { label: 'Continue to drink beer', goto: ['sister', 'scene2'] },
            ]);
          } else {
            if (((s as any).rexCar ?? 0) === 1  &&  ((s as any).rexCarDay ?? 0) < ((s as any).daystart ?? 0)  &&  ((s as any).rexSisTalk ?? 0) === 4) {
              ...
              scene.text(`Rex comes up to you. "${((s as any).pcs_nickname ?? 0)}, you could have at least told me you didn't want to go out! Last night was a disaster! Next time, please just tell me if you can't make it."`);
              scene.actions([
                { label: 'Continue to drink beer', goto: ['sister', 'scene2'] },
              ]);
            } else {
    ...
    scene.text('"Awesome! It\'ll be fun. Just be here before \' + func(\'time\', \'get_time_string\', 18, 0) + \', otherwise we\'ll leave without you."');
