...
function enterWeek1(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).SMSTree) (s as any).SMSTree = {}; (s as any).SMSTree['0'] = '"Hey, what up? Just got up, it was a long night last night. Did our first gig at this little club in Pskov, it was pretty cool. Stayed up drinking afterwards, have a hell of a headache. Hope you are having half as much fun as I am bitch." Her text is followed by emojis aligned in a lewd way.';
  if (!(s as any).SMSTree) (s as any).SMSTree = {}; (s as any).SMSTree['a1'] = 'Good luck!';
...
function enterWeek2(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).SMSTree) (s as any).SMSTree = {}; (s as any).SMSTree['0'] = '"Got to Smolensk, been playing most nights in local clubs. Going to do a little site seeing today, the boys are all out still. Was a hell of a party last night, so many bitches just throwing themselves at the boys and boy do they love it." Her text is followed by emojis aligned in a lewd way.';
  if (((s as any).locArgs?.[1] ?? 0) === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    // TODO-QSP: gs 'SMStext_builder', 'receive', $SMSTree['0']
...
function enterWeek4(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).SMSTree) (s as any).SMSTree = {}; (s as any).SMSTree['0'] = '"We made Kyiv in time for Atlas music festival, they also had a beer festival and a lot of tattoo artists. I was going to get a new tattoo, but the wait was too long. Val got so drunk we had to carry him back to the hotel."';
  if (((s as any).locArgs?.[1] ?? 0) === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    // TODO-QSP: gs 'SMStext_builder', 'receive', $SMSTree['0']
...
function enterWeek6(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).SMSTree) (s as any).SMSTree = {}; (s as any).SMSTree['0'] = '"There was some watermelon festival going on here at Rostov, Arkadi entered into the eating contest and lost, we took some with us for our next drive. Played at a lot of small clubs while here."';
  if (((s as any).locArgs?.[1] ?? 0) === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    // TODO-QSP: gs 'SMStext_builder', 'receive', $SMSTree['0']
...
function enterWeek9(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).SMSTree) (s as any).SMSTree = {}; (s as any).SMSTree['0'] = '"Moscow is huge and amazing, had a blast here. Getting sick of the boys, after two months with them I am ready to strangle them. At least we ditched the bitch, we just left her passed out in the hotel room."';
  if (((s as any).locArgs?.[1] ?? 0) === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    // TODO-QSP: gs 'SMStext_builder', 'receive', $SMSTree['0']
...
function enterWeek11(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).SMSTree) (s as any).SMSTree = {}; (s as any).SMSTree['0'] = '"Back home, can\'t believe it but I actually missed this place. Got to start packing tomorrow to move to the city and get ready for Uni."';
  if (((s as any).locArgs?.[1] ?? 0) === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    // TODO-QSP: gs 'SMStext_builder', 'receive', $SMSTree['0']
