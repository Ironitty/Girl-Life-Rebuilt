...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ((s as any).anushkaQW = (s as any).anushkaQW ?? {})['num_tour_SMS_send'] = ((s as any).anushkaQW['num_tour_SMS_send'] ?? 0) + (1);
  ((s as any).anushkaQW = (s as any).anushkaQW ?? {})['tour_SMS_send'] = ((s as any).daystart ?? 0);
...
function enterWeek1(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"Hey, what up? Just got up, it was a long night last night. Did our first gig at this little club in Pskov, it was pretty cool. Stayed up drinking afterwards, have a hell of a headache. Hope you are having half as much fun as I am bitch." Her text is followed by emojis aligned in a lewd way.';
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['a1'] = 'Good luck!';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    ...
    qspCall(s, 'SMStext_builder', 'add_reply', (((s as any).SMSTree ?? 0)?.['a1']), 'band_tour_anushka_SMS', 'Week1', 'Choice_a');
...
function enterWeek2(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"Got to Smolensk, been playing most nights in local clubs. Going to do a little site seeing today, the boys are all out still. Was a hell of a party last night, so many bitches just throwing themselves at the boys and boy do they love it." Her text is followed by emojis aligned in a lewd way.';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    qspCall(s, 'SMStext_builder', 'receive', (((s as any).SMSTree ?? 0)?.['0']));
...
function enterWeek4(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"We made Kyiv in time for Atlas music festival, they also had a beer festival and a lot of tattoo artists. I was going to get a new tattoo, but the wait was too long. Val got so drunk we had to carry him back to the hotel."';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    qspCall(s, 'SMStext_builder', 'receive', (((s as any).SMSTree ?? 0)?.['0']));
...
function enterWeek6(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"There was some watermelon festival going on here at Rostov, Arkadi entered into the eating contest and lost, we took some with us for our next drive. Played at a lot of small clubs while here."';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    qspCall(s, 'SMStext_builder', 'receive', (((s as any).SMSTree ?? 0)?.['0']));
...
function enterWeek7(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"Had some big gigs in Volgograd, followed by a two day party at some girls place. I don\'t remember half of it, but I woke up in the van, somehow we picked up two bitches who are now with us, the guys take turns fucking them in the back, as we drive." Followed by several lewd emojis for sex';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    qspCall(s, 'SMStext_builder', 'receive', (((s as any).SMSTree ?? 0)?.['0']));
...
function enterWeek8(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"Had some great gigs here in Voronezh, one of the bitches took off, getting a bus ride home. Anna stayed, she seems to love it when I drive, so the boys can triple team her. The rest of the time she spends passed out from drinking too much, she is starting to get on my nerves."';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    qspCall(s, 'SMStext_builder', 'receive', (((s as any).SMSTree ?? 0)?.['0']));
...
function enterWeek9(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"Moscow is huge and amazing, had a blast here. Getting sick of the boys, after two months with them I am ready to strangle them. At least we ditched the bitch, we just left her passed out in the hotel room."';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    qspCall(s, 'SMStext_builder', 'receive', (((s as any).SMSTree ?? 0)?.['0']));
...
function enterWeek11(s: GameState, scene: SceneBuilder): void {
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['0'] = '"Back home, can\'t believe it but I actually missed this place. Got to start packing tomorrow to move to the city and get ready for Uni."';
  if (String((s as any).locArgs?.[1] ?? '') === 'Add SMS') {
    qspCall(s, 'SMStext_builder', 'start');
    qspCall(s, 'SMStext_builder', 'receive', (((s as any).SMSTree ?? 0)?.['0']));
