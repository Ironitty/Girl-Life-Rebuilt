...
function enterWalkingHomeWithAGuy(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['QWstage'] === 5) {
    ...
    scene.text('You sniff your lines at the same time. Katja ' + ((((st as any).katjaQW ?? 0)?.['coke_stage'] === 2) ? ('has gotten used to it and it doesn\'t itch her nostrils anymore.') : ('is still getting itching nostrils and has to concentrate on not sneezing.')));
    ...
    scene.text('"Okay. Make some lines for me," she replies' + ((((st as any).katjaQW ?? 0)?.['coke_stage'] === 3) ? (' hesitantly.') : ('.')));
...
function enterOnGuySex1(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'npcStat', 'A14', 'a');
  scene.img('images/characters/pavlovsk/school/girl/katja/uni/city/sex/man_home_sex2.jpg');
  scene.text('"This is a really comfortable bed. Why don\'t you join us?" ' + ((qspFunc(s, 'katja_procedural', 'willing_to_be_naughty') > 80  &&  ((s as any).katjaQW ?? 0)?.['horny'] >= (Math.floor(Math.random() * 31) + 70)) ? ('Katja asks.') : ('you ask.')));
...
function enterGoingHome(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['QWstage'] === 5) {
    ...
    scene.text('iif(katjaQW[\'horny\'] = 0, \'"It was great, even better than I imagined," she says with a big smile. \', \'"It was okay, but I had imagined more," she says. \')+iif(npc_vag[\'A14\'] < dick, \'"It did hurt though"\', \'"I was surprised that it didn\'t hurt at all"\')+\', she continues.\'');
    scene.text('"Does this mean you want to try again?" you ask.');
    scene.text(((((s as any).katjaQW ?? 0)?.['horny'] === 0) ? ('"Definitely,"') : ('"Probably,"')) + ' she answers. You continue your small talk until you reach the metro station.');
  } else {
...
function enterBlindfoldSexStart(s: GameState, scene: SceneBuilder): void {
  scene.text('As you walk through his apartment, he leans into Katja and whispers something to her. She looks' + ((((s as any).katjaQW ?? 0)?.['bondage_sex'] > 0) ? (' quite excited by what he says, and immediately nods her head enthusiastically.') : (' intrigued and thinks about what he said for a moment before timidly nodding her head.')));
...
function enterBlindfoldSex2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['bondage_sex'] = (((st as any).katjaQW = (st as any).katjaQW ?? {})['bondage_sex'] ?? 0) + (1);
...
function enterTwoGuysDp(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).katjaQW ?? 0)?.['dp'] === 0) {
      ...
      scene.text(((((st as any).katjaQW ?? 0)?.['anal_quest'] < 3) ? ('"You haven\'t tried anal before tonight and that went well, so why don\'t you try this also?"') : ('"You\'ll only know if you try, so why don\'t you?"')) + ` ${(((st as any).npc_nickname ?? 0)?.[String((st as any).npcID1 ?? 0)] ?? '')} says.`);
