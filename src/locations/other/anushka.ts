...
function enterPlan(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A158', (-2));
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 5, ((st as any).npcID1 ?? 0), 'rough', 'group');
    scene.actions([
...
function enterHerTurn(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).stat ?? 0)?.['think_virgin'] === 0) {
    ...
    qspCall(st, 'arousal', 'cuni', (-15), ((st as any).npcID ?? 0), 'group', 'lesbian');
    ...
    qspCall(st, 'arousal', 'cuni', 10, ((st as any).npcID ?? 0), 'group', 'lesbian');
    qspCall(st, 'arousal', 'breasts', (-10), ((st as any).npcID1 ?? 0), 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    scene.text('<center><b>Detention</b></center>');
...
function enterWedgieFight(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'foreplay', (-20), 'humiliation');
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    if (((st as any).anushkaQW ?? 0)?.['first_visit'] === 0) {
      (st as any).AnushkaLoc = 5;
      qspGoto(st, 'anushapt', 'livingroom');
    } else {
      (st as any).AnushkaLoc = 2;
      qspGoto(st, 'anush_bedroom', 'anushroom');
    }
  } },
    ]);
  } },
...
function enterChatwhilewaiting(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).anushLimitedQuestions ?? 0) > 0) {
    ...
    (st as any).anushTalkedofDolls = 1;
    scene.text('<center><b>Classroom</b></center>');
    ...
    if (((st as any).rex_love ?? 0) === 1  ||  ((st as any).artemQW ?? 0)?.['bf'] === 1  ||  ((st as any).kotovLoveQW ?? 0) === 1  ||  ((st as any).fedorKozlovQW ?? 0)  ||  ((st as any).NikoVolkovQW ?? 0) >= 5  ||  (Array.isArray((st as any).pcs_rel_type) ? ((st as any).pcs_rel_type as any[]).indexOf('boyfriend') : -1) >= 0) {
      scene.actions([
