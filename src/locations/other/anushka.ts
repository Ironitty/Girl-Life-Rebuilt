...
function enterPlan(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`She walks up to the side of the teacher's desk where Mr ${((s as any).rand_teacher || '')} is sitting, his unguarded drink conveniently located at the opposite end. You causally stroll over and sit down on the edge of the table, using your body to block his drink from view from the rest of the room as Mr ${((s as any).rand_teacher || '')} looks at the two of you questioningly. "Can I help you girls?"`);
...
function enterHerTurn(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).stat ?? 0)?.['think_virgin'] === 0) {
    ...
    scene.text(`She's right… you're sopping wet and ready. As you lower yourself down, she guides his cock into your pussy. You feel his ${((s as any).dick1 || '')}cm ${((s as any).dick_girth1 || '')} cock slowly filling and stretching you open, moaning softly as it does.`);
    ...
    scene.text(`You feel him rubbing the tip of his dick against your asshole before pushing the tip of his ${((s as any).dick1 || '')}cm ${((s as any).dick_girth1 || '')} cock into your anus.`);
    ...
    scene.text(`When he's finished, she diligently licks you clean and you return the favor. When all the cum has been collected, she gives you a deep and passionate kiss, both of you making sure to share the cum you licked up together. With that done, the three of you get dressed. As Valentin unlocks the lounge door, he gives you a smirk. "Okay, I admit it, you're not as much of a killjoy as I thought, ${((s as any).pcs_nickname || '')}. Nush was right; you're a pretty cool chick, not to mention a damn good fuck!" You blush and give Anushka a sideways glance to find her grinning at you.`);
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: When he's finished, she diligently licks you clean and you return the favor. Whe...
    scene.text(`When he's finished, she diligently licks you clean and you return the favor. When all the cum has been collected, she gives you a deep and passionate kiss, both of you making sure to share the cum you licked up together. With that done, the three of you get dressed. As Valentin unlocks the lounge door, he gives you a smirk. "Okay, I admit it. You're not as much of a killjoy as I thought, ${((s as any).pcs_nickname || '')}. Nush was right; you're a pretty cool chick. Not to mention you really know how to suck cock." You give Anushka a sideways glance and find her grinning at you.`);
    qspCall(s, 'arousal', 'kiss', 5, ((s as any).npcID ?? 0));
    qspCall(s, 'arousal', 'end');
    scene.text('Going back to the library, you see that the teacher is still out like a light and all the other students are gone. There\'s less than an hour of detention left, so the three of you leave the school. Once outside, you each go your separate ways.');
    scene.actions([
...
function enterWedgieFight(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The principal seems a little surprised by this. "Miss Konstantinov! I've come to expect this kind of behavior from you, but <b>you</b> Miss ${((s as any).pcs_lastname || '')}? I'd hoped you would know better. It seems that hope was in vain." She nods to the teacher. "I'll deal with them." He leaves the office and closes the door behind him.`);
    scene.actions([
    ...
    scene.text(`You quickly gather up your clothes and get dressed again. As the two of you head out, Anushka turns to you. "Hey ${((s as any).pcs_nickname || '')}, you want to come by my place?"`);
    scene.actions([
...
function enterTemptation(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You look around. When you're sure that there is no one watching, you get up and quickly slip in behind one of the shelves, making your way over to where you saw Anushka. As you get close, you spot her much further back in the library, silently waving at you. You follow her to the very back of the library, far away from everyone else, where she gives you a grin that you're all too aware means she's cooking up trouble. "${((s as any).pcs_nickname || '')}! Let's have some fun."`);
    scene.actions([
    ...
    if (((s as any).trait_vars ?? 0)?.['exhibitionist'] >= 3) {
      ...
      if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
        scene.actions([
          { label: 'I would like to, but…', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
        ]);
      } else {
    ...
    scene.text(`She can see your reluctance, but also your willingness. She steps up close and starts pulling your shirt up. "You worry too much, ${((s as any).pcs_nickname || '')}. You've got to live in the moment. Besides, it'll be so much fun."`);
...
function enterChatwhilewaiting(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).anushLimitedQuestions ?? 0) > 0) {
    ...
    scene.text(`She laughs. "Which one? Where I'm a nympho? A slut? A whore? A lesbian? Or do you mean that I'm a party girl that spends all her time getting drunk or stoned and wasting her life away? Or maybe how I'm the band whore? You'll need to be a bit more specific there, ${((s as any).pcs_nickname || '')}."`);
    ...
    if (((s as any).npc_grupTipe ?? 0)?.['A5'] === 5) {
      ...
      if (((s as any).pcs_hotcat ?? 0) >= 6) {
        ...
        scene.text(`She snorts. "Yeah sure he is… Uh-huh. Be honest ${((s as any).pcs_nickname || '')}, it's because he has a really big dick, isn't it?" she asks with a grin while pretending to hold a really big imaginary dick between her legs.`);
    ...
    scene.actions([
      { label: 'Talk about something else', goto: ['anushka', 'chatwhilewaiting'] },
        ...
        // TODO-QSP: dynamic text: She snorts. "Yeah sure he is… Uh-huh. Be honest <<$pcs_nickname>>, it's because ...
        scene.text(`She snorts. "Yeah sure he is… Uh-huh. Be honest ${((s as any).pcs_nickname || '')}, it's because he has a really big dick, isn't it?" she says with a grin while pretending to hold a really big imaginary dick between her legs.`);
        ...
        scene.text(`She laughs. "I knew it!" She pretends to hump the air doggystyle while talking in a gruff voice trying to sound like Fedor. "Yeah, take my big dick ${((s as any).pcs_nickname || '')}! Take it!" She switches to a girly voice and tries to sound like you. "Yes… yes… Fedor fuck me with your big dick… YES!" She bursts out laughing and you can't help but to join in.`);
        scene.actions([
          { label: 'Talk about something else', goto: ['anushka', 'chatwhilewaiting'] },
        ]);
      }
