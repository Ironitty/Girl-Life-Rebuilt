...
function enterEvents(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).lesbiQW ?? 0) === 2) {
    ...
    qspCall(st, 'arousal', 'erotic_nudity', 15, 'dom', 'lesbian', 'exhibitionism');
...
function enterEvent2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'erotic_nudity', 5, 'dom', 'lesbian', 'exhibitionism');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'erotic_nudity', 20, 'dom', 'lesbian', 'exhibitionism');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'vaginal_strap_give', 30, 'dom', 'lesbian', 'rough');
    qspCall(st, 'arousal', 'end');
    scene.actions([
...
function enterEvent3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'foreplay', 20, 'dom', 'lesbian');
    qspCall(st, 'stat', '');
    ...
    (st as any).orgasm_or = 'yes';
    qspCall(st, 'arousal', 'cuni', 15, 'dom', 'lesbian');
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Leave', goto: ['city_residential', ''] },
    ]);
...
function enterEvent3_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'anal_strap_give', 15, 'dom', 'lesbian');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'foreplay', 30, 'dom', 'lesbian', 'maso');
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Leave the apartment', goto: ['city_residential', ''] },
    ]);
    ...
    (st as any).orgasm_or = 'yes';
    qspCall(st, 'arousal', 'anal_strap_give', 10, 'dom', 'lesbian');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Leave the apartment', goto: ['city_residential', ''] },
    ]);
...
function enterEvent4(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).mistressspank ?? 0) !== ((s as any).daystart ?? 0)) {
    ...
    qspCall(st, 'sweat', 'add', 10);
    (st as any).mistressspank = ((st as any).daystart ?? 0);
    if (((st as any).mistendbound ?? 0) <= 50) {
      (st as any).mistendspank = ((st as any).mistendspank ?? 0) + (1);
    }
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'foreplay', 15, 'dom', 'maso', 'lesbian');
    qspCall(st, 'stat', '');
    scene.actions([
  ...
  if (((s as any).pcs_energy ?? 0) < 20) {
    ...
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + ((Math.floor(Math.random() * 11) + 10));
    if (((st as any).mistanalfist ?? 0) >= 25  &&  (!(Math.floor(Math.random() * 10) + 0))) {
      (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + ((Math.floor(Math.random() * 21) + 30));
  ...
  if (((s as any).pcs_sweat ?? 0) >= 40) {
    ...
    if (((st as any).pcs_horny ?? 0) >= 90  &&  ((st as any).mistbath ?? 0) !== ((st as any).daystart ?? 0)) {
      scene.actions([
        { label: 'Have her lick your pussy', handler: (st: GameState) => {
    (st as any).mistbath = ((st as any).daystart ?? 0);
  ...
  if ((!((s as any).mistsexshop ?? 0))) {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).mistsexshop = 1;
    if ((!((st as any).slavejobQW ?? 0))) {
      (st as any).slavejobdayexp = ((st as any).daystart ?? 0);
      (st as any).slavejobQW = 1;
    }
  ...
  if (((s as any).mistsexshop ?? 0) === 1  &&  ((s as any).slavepay ?? 0) !== ((s as any).daystart ?? 0)) {
    ...
    qspCall(st, 'stat', '');
    scene.img('images/characters/city/natalya/sex/dom12.jpg');
    // TODO-QSP: dynamic text: Your slave has earned <<$func(''money'', ''string_profit'', 300*cocsuckedonday)>...
    scene.text(`Your slave has earned ${qspFunc(s, 'money', 'string_profit', 300*((st as any).cocsuckedonday ?? ''))} for you at the gloryholes.`);
    scene.actions([
  ...
  if (((s as any).mistsexshop ?? 0) === 2  &&  ((s as any).slavepay ?? 0) !== ((s as any).daystart ?? 0)) {
    ...
    qspCall(st, 'stat', '');
    scene.img('images/characters/city/natalya/sex/dom12.jpg');
    // TODO-QSP: dynamic text: Your slave has earned <<$func(''money'', ''string_profit'', 900*cocsuckedonday)>...
    scene.text(`Your slave has earned ${qspFunc(s, 'money', 'string_profit', 900*((st as any).cocsuckedonday ?? ''))} for you while whoring herself out.`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    (st as any).slavepay = ((st as any).daystart ?? 0);
  }, goto: ['lesbisubhouse', 'event4'] },
  ...
  if (((s as any).mistpiss ?? 0) >= 20) {
    ...
    qspCall(st, 'arousal', 'pee_give', 10, 'dom', 'lesbian');
    qspCall(st, 'arousal', 'end');
  ...
  scene.actions([
    ...
    if (((st as any).mistendspank ?? 0) <= 50) {
      (st as any).mistendbound = ((st as any).mistendbound ?? 0) + (1);
    }
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'foreplay', 15, 'dom', 'bound', 'lesbian');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Continue', goto: ['lesbisubhouse', 'bound'] },
    ]);
...
function enterBound(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_horny ?? 0) >= 80) {
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      if (((st as any).mistressfist ?? 0) === 4) {
        ...
        qspCall(st, 'arousal', 'vaginal_strap_give', 30, 'dom', 'rough', 'lesbian', 'bound');
        qspCall(st, 'arousal', 'end');
      } else {
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      if (((st as any).mistressfist ?? 0) === 5) {
        ...
        (st as any).orgasm_or = 'no';
        qspCall(st, 'arousal', 'anal_strap_give', 30, 'dom', 'lesbian', 'bound');
        qspCall(st, 'arousal', 'end');
      } else {
        ...
        (st as any).orgasm_or = 'yes';
        qspCall(st, 'arousal', 'anal_strap_give', 30, 'dom', 'lesbian', 'rough', 'bound');
        qspCall(st, 'arousal', 'end');
      }
    } else {
      if (((st as any).mistressfist ?? 0) === 5) {
    ...
    if (((st as any).pcs_vag ?? 0) < 10) {
      if (((st as any).vgape ?? 0) < 3) {
        (st as any).vgape = ((st as any).vgape ?? 0) + (1);
      }
    }
    ...
    (st as any).orgasm_or = 'yes';
    qspCall(st, 'arousal', 'vaginal_dildo', 30, 'lesbian', 'dom', 'humiliation', 'bound');
    qspCall(st, 'arousal', 'end');
    if (((st as any).mistpiss ?? 0) >= 20) {
      scene.actions([
        { label: 'Enough for now', handler: (st: GameState) => {
    ...
    if (((st as any).mistressfist ?? 0) === 1) {
      (st as any).mistressfist = 4;
    }
    if ((!((st as any).mistressfist ?? 0))) {
      (st as any).mistressfist = 1;
    }
    if (((st as any).mistressfist ?? 0) === 2) {
      (st as any).mistressfist = 3;
    }
    (st as any).mistanalfist = ((st as any).mistanalfist ?? 0) - (1);
    qspCall(st, 'stat', '');
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      if (((st as any).mistressfist ?? 0) === 4) {
        ...
        qspCall(st, 'arousal', 'vaginal_fist_give', 30, 'dom', 'lesbian', 'bound');
        qspCall(st, 'arousal', 'end');
      } else {
    ...
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (40);
    if (((st as any).mistressfist ?? 0) === 2) {
      (st as any).mistressfist = 5;
    }
    if ((!((st as any).mistressfist ?? 0))) {
      (st as any).mistressfist = 2;
    }
    if (((st as any).mistressfist ?? 0) === 1) {
      (st as any).mistressfist = 3;
    }
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      if (((st as any).mistressfist ?? 0) === 5) {
        ...
        qspCall(st, 'arousal', 'anal_fist_give', 20, 'dom', 'lesbian', 'bound');
        qspCall(st, 'arousal', 'end');
      } else {
...
function enterSex(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_horny ?? 0) >= 80) {
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      ...
      qspCall(st, 'arousal', 'anal_strap_give', 30, 'dom', 'rough', 'lesbian', 'bound');
      qspCall(st, 'stat', '');
    } else {
    ...
    if (((st as any).pcs_ass ?? 0) < 10) {
      if (((st as any).agape ?? 0) < 3) {
        (st as any).agape = ((st as any).agape ?? 0) + (1);
      }
    }
    ...
    (st as any).orgasm_or = 'yes';
    qspCall(st, 'arousal', 'anal_dildo', 30, 'lesbian', 'dom', 'humiliation', 'bound');
    qspCall(st, 'arousal', 'end');
    if (((st as any).mistpiss ?? 0) >= 20) {
      scene.actions([
        { label: 'Enough for now', handler: (st: GameState) => {
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      ...
      qspCall(st, 'arousal', 'foreplay', 15, 'dom', 'exhibitionism', 'lesbian');
      qspCall(st, 'arousal', 'end');
    } else {
    ...
    (st as any).mistressfist = 4;
    qspCall(st, 'stat', '');
    (st as any).temp_lesbisubhouse = (Math.floor(Math.random() * 5) + 0);
    if ((!((st as any).temp_lesbisubhouse ?? 0))) {
      ...
      if (((st as any).temp_lesbisubhouse ?? 0) === 1) {
        ...
        if (((st as any).temp_lesbisubhouse ?? 0) === 2) {
          ...
          if (((st as any).temp_lesbisubhouse ?? 0) === 3) {
            ...
            if (((st as any).temp_lesbisubhouse ?? 0) === 4) {
    ...
    scene.actions([
      ...
      { label: 'Surprise me', handler: (st: GameState) => {
    if (((st as any).mistanalfist ?? 0) < 50) {
      (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + ((Math.floor(Math.random() * 21) + 30));
    }
    if (((st as any).mistanalfist ?? 0) >= 50) {
      (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (80);
    }
    if (((st as any).mistanalfist ?? 0) >= 25  &&  ((st as any).mistanalfist ?? 0) < 50) {
      if (((st as any).mistressfist ?? 0) === 2) {
        (st as any).mistressfist = 5;
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      ...
      qspCall(st, 'arousal', 'anal_fist_give', 20, 'dom', 'lesbian');
      qspCall(st, 'arousal', 'end');
    } else {
    ...
    if (((st as any).mistskold ?? 0) === 1) {
      ...
      qspCall(st, 'arousal', 'vaginal_fist_give', 20, 'dom', 'lesbian');
      qspCall(st, 'arousal', 'end');
    }
...
function enterGetFucked(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'boyStat', 'A178');
    qspCall(st, 'boyStat', 'D2');
    scene.img('images/characters/city/natalya/sex/dom401.jpg');
    qspCall(st, 'dinsex2', 'd_strapon_vag', 30, 'dom', 'lesbian');
    qspCall(st, 'stat', '');
    if (((st as any).pcs_horny ?? 0) > 20) {
      scene.actions([
    ...
    qspCall(st, 'arousal', 'foreplay', 15, 'dom', 'lesbian', 'maso');
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Enough for now', handler: (st: GameState) => {
    (st as any).mistressfist = 0;
    ...
    qspCall(st, 'dinSex', 'girl_wants_anal', '', 'lubri');
    qspCall(st, 'dinsex2', 'd_strapon_anal', 30, 'dom', 'lesbian');
    qspCall(st, 'arousal', 'end');
    if (((st as any).mistpiss ?? 0) >= 20) {
      scene.actions([
        { label: 'Enough for now', handler: (st: GameState) => {
