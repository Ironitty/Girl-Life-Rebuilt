...
function enterAnalStart(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['initiative'] === 'girl') {
    ...
    scene.actions([
      { label: 'Anal (doggy)', goto: ['sex_ev_anal', 'anal_present_ass'] },
    ]);
  } else {
...
function enterAnalDoggySwitch(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['initiative'] === 'girl') {
    if (((s as any).stat ?? 0)?.['anal'] === 0) {
      ...
      scene.text('iif(stat[\'no_sex_swearing\'] = 0, \'"That\'s enough of that. Time for you to shove it up my ass.\', \'That\'s enough of that. Time for you to put it in my butthole."\')');
    }
    scene.actions([
...
function enterAnalPresentAss(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).stat ?? 0)?.['anal'] === 0) {
    ...
    if (((s as any).sex_ev ?? 0)?.['anal_count'] > 0) {
      scene.text('iif(stat[\'no_sex_swearing\'] = 0, \'"That\'s enough of that. Time for you to shove it in my ass again.\', \'That\'s enough of that. Time for you to put it back in my butthole."\')');
    } else {
      scene.text('iif(stat[\'no_sex_swearing\'] = 0, \'"That\'s enough of that. Time for you to shove it in my ass again.\', \'That\'s enough of that. Time for you to put it in my butthole."\')');
    }
  }
...
function enterAnalNoAsk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Wait for him to put it in', goto: ['sex_ev_anal', 'anal_doggy_insert_slow'] },
...
function enterAnalDoggyInsertSlow(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(String(qspFunc(s, 'sex_ev', 'anal_insert_desc1') || ''));
...
function enterAnal3(s: GameState, scene: SceneBuilder): void {
  ...
  qspCall(s, 'sex_ev_stats', 'position_tracker', 'anal', 2);
  qspCall(s, 'arousal', 'anal', 1, 'no_orgasm_msg', (((s as any).sex_ev ?? 0)?.['prostitution_flag']), 'rough');
...
function enterAnalVirgNorm1_1(s: GameState, scene: SceneBuilder): void {
  ((s as any).sex_ev = (s as any).sex_ev ?? {})['anal_virgin_fuck'] = 1;
  qspCall(s, 'sex_ev_anal', '' + (((s as any).sex_ev ?? 0)?.['virgin_react']) + '_react1');
  scene.actions([
    { label: 'Continue', handler: (st: GameState) => {
    scene.img('images/shared/sex/anal/doggy/insert2.mp4');
    qspCall(st, 'sex_ev_anal', '' + (((st as any).sex_ev ?? 0)?.['virgin_react']) + '_react2');
    ...
    scene.text(String(qspFunc(s, 'sex_ev', 'virgin_media') || ''));
...
function enterAnalVirgHard1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Scream', handler: (st: GameState) => {
    (st as any).orgasm_or = 'no';
    qspCall(st, 'arousal', 'ass', 1, 'rough', 'no_orgasm_msg', (((st as any).sex_ev ?? 0)?.['prostitution_flag']));
    qspCall(st, 'stat', '');
    scene.img('images/shared/sex/anal/doggy/hard2.mp4');
...
function enterAnalVirgHard2(s: GameState, scene: SceneBuilder): void {
  ...
  qspCall(s, 'arousal', 'anal', (-15), 'no_orgasm_msg', (((s as any).sex_ev ?? 0)?.['prostitution_flag']));
  scene.img('images/shared/sex/anal/doggy/med1.mp4');
...
function enterVirginEnjoyment(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['speed_change'] === 1) {
    ...
    (st as any).orgasm_or = '';
    qspCall(st, 'sex_ev_sex', 'fuck_arousal_code');
    qspCall(st, 'arousal', 'anal', (-15), 'no_orgasm_msg', (((st as any).sex_ev ?? 0)?.['prostitution_flag']));
    qspCall(st, 'stat', '');
