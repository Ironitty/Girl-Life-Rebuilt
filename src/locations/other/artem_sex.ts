...
function enterHj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Wow, that was amazing, ${((s as any).pcs_nickname || '')}! You're the best!" he blurts out.`);
...
function enterSlutBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"I think we're done for the day, ${((s as any).pcs_nickname || '')}. I have some things I need to do, so can you please leave now?" he asks as he grabs a tissue to clean up his dick, so you gather your clothes and get dressed.`);
...
function enterAnushka_3someCleanup(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).artemQW ?? 0)?.['rough_anal_nush'] === 1) {
    ...
    scene.text(`She snorts. "Oh, I'll give it to you that for the last few minutes, you started to actually fuck me like a man, but that was only after ${((s as any).pcs_nickname || '')} and I encouraged you several times." She steps up close to him and taps him on the chest. "Which means you have it in you, but where was that earlier, hmm? The last few minutes don't make up for the rest."`);
