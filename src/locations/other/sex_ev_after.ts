...
function enterFuckedOut(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).sex_ev ?? 0)?.['cock_inserted'] === 0) {
      scene.text(String(qspFunc(s, 'sex_ev', 'bed_room') || ''));
      scene.text(String(qspFunc(s, 'sex_ev', 'end_comment') || ''));
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterAfterSex1(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
...
function enterPostCigaretteBoy(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_smoker ?? 0)?.[String((s as any).npcID ?? 0)] > 0  &&  ((s as any).sex_ev ?? 0)?.['cock_inserted'] === 1) {
    ...
    scene.text('You both lay back, basking in the afterglow of your fuck together. ' + ((((s as any).npc_smoker ?? 0)?.[String((s as any).npcID ?? 0)] > 0) ? ('After about a minute, ' + ((s as any).npcdesc ?? '') + ' reaches over and grabs a pack of cigarettes.') : (' ')));
  }
...
function enterAfterSex2(s: GameState, scene: SceneBuilder): void {
      ...
      if (((s as any).sex_ev ?? 0)?.['loc'] === 'pc_home') {
        ...
        if (((s as any).sex_ev ?? 0)?.['type'] === 'hookup'  &&  ((s as any).sex_ev ?? 0)?.['leaving'] === 0) {
          if (((s as any).sex_ev ?? 0)?.['mad'] < 1  &&  ((s as any).sex_ev ?? 0)?.['angry_after'] < 1) {
            scene.actions([
              { label: '', labelFn: (s: GameState) => 'Thanks for the ' + String((((s as any).sex_ev ?? 0)?.['fun'] ?? '') ?? ''), goto: ['sex_ev_hookup_leave', 'thanks_for_fun'] },
            ]);
          }
          scene.actions([
            { label: 'I should go', goto: ['sex_ev_hookup_leave', 'i_should_go'] },
          ]);
        } else {
          scene.actions([
            { label: 'Prepare to leave', goto: ['sex_ev_leave', 'leave_router'] },
          ]);
        }
      }
    }
  ...
  if (((s as any).sex_ev ?? 0)?.['cigarette'] === 0  &&  (((s as any).mc_inventory ?? 0)?.['cigarettes'] > 0  ||  ((s as any).npc_smoker ?? 0)?.[String((s as any).npcID ?? 0)] > 0)  &&  ((s as any).sex_ev ?? 0)?.['npc_late'] !== 1) {
    scene.actions([
      { label: 'Smoke a cigarette', goto: ['sex_ev_after', 'smoke_ciga1'] },
    ]);
  }
...
function enterRelaxTogether(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Stop relaxing', goto: ['sex_ev_after', 'after_sex2'] },
...
function enterDrinkWater1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).npc_gymrat ?? 0)?.[String((st as any).npcID ?? 0)] > 0) {
      scene.text('iif(sex_ev[\'rough_fuck\'] > 0, \'"It was practically a workout," he agrees with no small amount of amusement.\', \'"Important to stay hydrated after cardio," he chuckles.\')');
    } else {
      scene.text('iif(rand(1, 2), \'"That\'s a good idea," he nods.\', \'"Me too," he huffs tiredly.\')');
    }
    if (((st as any).cum_loc ?? 0)?.['mouth'] > 0) {
...
function enterPlanB(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 0  &&  ((s as any).mc_inventory ?? 0)?.['morning_after_pill'] > 0  &&  ((s as any).sex_ev ?? 0)?.['ma_pill'] !== 2  &&  ((s as any).birth_control ?? 0)?.['think_safe'] === 0  &&  ((s as any).sex_ev ?? 0)?.['angry_after'] === 0  &&  (!((s as any).mesec ?? 0))) {
    scene.actions([
      { label: 'Take a morning after pill', goto: ['sex_ev_after', 'plan_b_take'] },
    ]);
  }
  scene.build();
}
...
function enterSmokeLink(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['smoke_time'] + 5 > ((s as any).totminut ?? 0)  &&  ((s as any).sex_ev ?? 0)?.['boy_smoke_time'] + 5 > ((s as any).totminut ?? 0)) {
    scene.text(`You and ${((s as any).npcdesc ?? '')} lay together, ` + ((((s as any).sex_ev ?? 0)?.['smoke_react'] === 0) ? ('<a href="#" onclick="window.__gameStore.getState().doGoto(\u0027sex_ev_after\u0027, \u0027smoke_ciga_react\u0027); return false;">puffing away on your cigarettes.</a>') : ('puffing away on your cigarettes.')));
  } else {
    if (((s as any).sex_ev ?? 0)?.['smoke_time'] + 5 > ((s as any).totminut ?? 0)) {
      scene.text(`${((s as any).npcdesc ?? '')} lays next to you while you ` + ((((s as any).sex_ev ?? 0)?.['smoke_react'] === 0) ? ('<a href="#" onclick="window.__gameStore.getState().doGoto(\u0027sex_ev_after\u0027, \u0027smoke_ciga_react\u0027); return false;">puff away on your cigarette.</a>') : ('puff away on your cigarette.')));
    } else {
      if (((s as any).sex_ev ?? 0)?.['boy_smoke_time'] + 5 > ((s as any).totminut ?? 0)) {
        scene.text(`You lay next to ${((s as any).npcdesc ?? '')} while he ` + ((((s as any).sex_ev ?? 0)?.['smoke_react'] === 0) ? ('<a href="#" onclick="window.__gameStore.getState().doGoto(\u0027sex_ev_after\u0027, \u0027boy_smoking\u0027); return false;">puffs away on his cigarette.</a>') : ('puffs away on his cigarette.')));
      }
    }
  }
  scene.build();
...
function enterSmokeCigaAct(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).sex_ev ?? 0)?.['boy_in_shower'] !== 1  &&  ((st as any).sex_ev ?? 0)?.['boy_asleep'] !== 1) {
      ...
      scene.img('images/shared/sex/after/bed_smoke1.jpg');
      if (((st as any).sex_ev ?? 0)?.['cigarette'] === 1) {
        scene.text('Smoke fills your lungs and you hold it there, savouring it, before exhaling slowly. The smoke trails hazily around the room as you enjoy another cigarette' + ((((st as any).sex_ev ?? 0)?.['boy_asleep'] === 0) ? ('.') : (' while ' + ((st as any).npcdesc ?? '') + ' snores softly beside you.')));
      } else {
        scene.text('Smoke fills your lungs and you hold it there, savouring it, before exhaling slowly. As the smoke blows past your lips, it\'s like all your stress goes with it and you sigh happily, relaxing into the bed' + ((((st as any).sex_ev ?? 0)?.['boy_asleep'] === 0) ? ('.') : (' while ' + ((st as any).npcdesc ?? '') + ' snores softly beside you.')));
      }
