...
function enterMagic(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"My favorite discipline! Ask away ${((s as any).pcs_lastname ?? 0)}."`);
  if (((s as any).hour ?? 0) > 19) {
    scene.actions([
      { label: 'The clock strikes', goto: ['alexandriaChat', 'late'] },
    ]);
    ...
    if (((s as any).npc_rel ?? 0)?.['A241'] === 1) {
      ...
      scene.text(`"The Succubae… Yes, your new <i>sisterhood</i>. I'm not happy about that development ${((s as any).pcs_lastname ?? 0)}, but remember my apprentice, they are the ones that need you."`);
    } else {
      // TODO-QSP: dynamic text: "The Succubae… Yes, your new <i>sisterhood</i>. I'm not happy about that develop...
      scene.text(`"The Succubae… Yes, your new <i>sisterhood</i>. I'm not happy about that development ${((s as any).pcs_lastname ?? 0)}, but… I didn't have much hope in you anyway."`);
    }
...
function enterPeople(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"So, it is gossiping time. What a <i>marvellous</i> experience ${((s as any).pcs_lastname ?? 0)}…"`);
  if (((s as any).hour ?? 0) > 19) {
    scene.actions([
      { label: 'The clock strikes', goto: ['alexandriaChat', 'late'] },
    ]);
    ...
    scene.text(`"${((s as any).pcs_lastname ?? 0)}, <i>Est potentia ex testamento</i>, was the motto from the honorably disbandment <i>Council Army Forces</i>, a force that I have the privilege to command."`);
    ...
    scene.text(`"I must say ${((s as any).pcs_lastname ?? 0)}, that that girl, has committed a serious mistake taking our legacy in vain, as even now, decades after our disbandment, those words - <i>Est potentia ex testamento</i> - carry a lot of weight in our corner of the world."`);
    scene.actions([
    ...
    scene.text(`"Ah, the parahuman. I must admit that he is the only acquaintance of yours that I don't have problems with, ${((s as any).pcs_lastname ?? 0)}. A remarkable achievement on its own."`);
    ...
    scene.text(`"What do you think about your <i>handler's</i> actions ${((s as any).pcs_lastname ?? 0)}?"`);
    scene.actions([
      { label: 'Continue talking about people', goto: ['alexandriaChat', 'people'] },
      { label: 'Talk about other things', goto: ['alexandriaHome', 'study'] },
    ]);
...
function enterTraining(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Remember ${((s as any).pcs_lastname ?? 0)}, this is only a first step into a larger world."`);
  if (((s as any).hour ?? 0) > 19) {
    scene.actions([
      { label: 'The clock strikes', goto: ['alexandriaChat', 'late'] },
    ]);
  ...
  if (((s as any).AlexandriaQW ?? 0) === 7) {
    ...
    if (((s as any).AlexandriaQW ?? 0) < 12  ||  ((s as any).AlexandriaQW ?? 0) > 19  &&  ((s as any).AlexandriaQW ?? 0) < 23) {
      ...
      if (((s as any).AlexandriaQW ?? 0) === 12) {
        ...
        scene.text(`"That ${((s as any).pcs_lastname ?? 0)}, is a Pattern 1796 Heavy Cavalry Sword, that until recently was in a Bratva's Pakhan possession. Fortunately for me, it seems that his stinginess with bribes has finally made the police move against him, so that this beautiful piece of history, can end in more deserving hands."`);
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
        ...
        if (((s as any).AlexandriaQW ?? 0) < 16) {
          ...
          scene.text(`"${((s as any).pcs_lastname ?? 0)}, aren't you supposed to be looking for my sword?"`);
          scene.actions([
            { label: 'Talk about other things', goto: ['alexandriaHome', 'study'] },
          ]);
        } else {
          if (((s as any).pregChem ?? 0) > 1800  &&  (!((s as any).knowpreg ?? 0))) {
            ...
            scene.text(`Aleksei is silent at your question, looking at you with curiosity "You dodn't know… Isn't it ${((s as any).pcs_lastname ?? 0)}?"`);
            ...
            scene.text(`An amused smirk creeps into Aleksei's face as he deigns to answer "${((s as any).pcs_lastname ?? 0)}, that depends on your point of view at the prospect of being pregnant."`);
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
            ...
            if (((s as any).npc_rel ?? 0)?.['A241'] === 0  &&  ((s as any).pregChem ?? 0) > 1800  &&  ((s as any).knowpreg ?? 0) === 1) {
              ...
              scene.text(`"${((s as any).pcs_lastname ?? 0)}, taking into consideration the nature of our <i>training lessons</i>, I think is better to not submit your <i>pregnant</i> body to that stress level."`);
              scene.actions([
                { label: 'Talk about other things', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
  }, goto: ['alexandriaHome', 'study'] },
              ...
              if (((s as any).npc_rel ?? 0)?.['A241'] === 0  &&  ((s as any).pcs_sweat ?? 0) >= 20) {
                ...
                scene.text(`"${((s as any).pcs_lastname ?? 0)}, at risk to embarrass you, I think is better if first you take a shower."`);
                scene.actions([
                  { label: 'Talk about other things', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
  }, goto: ['alexandriaHome', 'study'] },
                ...
                scene.text(`"${((s as any).pcs_lastname ?? 0)}, you aren't ready for the next lesson(PLACEHOLDER until I write the next part)."`);
                scene.actions([
                  { label: 'Talk about other things', goto: ['alexandriaHome', 'study'] },
                ]);
              }
...
function enterSword(s: GameState, scene: SceneBuilder): void {
  if (((s as any).AlexandriaQW ?? 0) === 14) {
    ...
    scene.text(`"Interesting, ${((s as any).pcs_lastname ?? 0)}. Please, explain to me in detail what happened."`);
    ...
    if (((s as any).AlexandriaQW ?? 0) === 16) {
      ...
      scene.text(`"Yes, ${((s as any).pcs_lastname ?? 0)}, let's talk about this sword, that that little nuisance has sent me with a detailed and <i>colurfull</i> description of your actions."`);
      // TODO-QSP: dynamic text: You freeze, as Aleksei fixes his gaze into yours… He seems <i>superficially</i> ...
      scene.text(`You freeze, as Aleksei fixes his gaze into yours… He seems <i>superficially</i> calm, but if eyes are a window to the soul… Well… It has been good to know you ${((s as any).pcs_nickname ?? 0)}.`);
      // TODO-QSP: dynamic text: "Let's be clear <<$pcs_lastname>>. I am old, I know betrayal in all forms and co...
      scene.text(`"Let's be clear ${((s as any).pcs_lastname ?? 0)}. I am old, I know betrayal in all forms and colors, and even with that, it always surprises me, human pettiness… It seems that putting my confidence in you was… unwise. In time you will help me to find those who are behind this travesty, but until then, our <i>study methodology</i> must be re-evaluated."`);
      ...
      scene.text(`"Let's start now ${((s as any).pcs_lastname ?? 0)}."`);
      scene.actions([
      ...
      if (((s as any).AlexandriaQW ?? 0) > 16) {
        ...
        scene.text(`"This, ${((s as any).pcs_lastname ?? 0)}, is war. Let me explain something to you, people like me are just plain simply too powerful. When there is some kind of irreparable rift between parties, we can't just sort this out with a backstreet brawl… too much collateral damage… That is a reason for The Council continued existence, but not all of us accept their arbitration… and some of the most imaginative of our brethren, resort to these acts of pettiness to settle any perceived grievance."`);
    ...
    scene.text(`"Now ${((s as any).pcs_lastname ?? 0)}, I must search the miasma that are my so-called <i>enemies</i>, looking for the idiot that has decided to waste our precious time on this childish <i>attack</i>."`);
    ...
    scene.text(`"${((s as any).pcs_lastname ?? 0)}, that isn't the question, no, take this lesson to heart… When you are as old and powerful as I am, the problem is to crib through the cesspit of incompetence that rise regularly against you… is not that I don't have an idea about who is behind this, no… my problem is that I have, <i>too many ideas</i>, but… let's leave this for later, we must talk about your actions aftermatch."`);
    ...
    if (((s as any).npc_rel ?? 0)?.['A241'] === 0  &&  ((s as any).AlexandriaMissionQW ?? 0) >= 12) {
      ...
      scene.text(`"As I told you ${((s as any).pcs_lastname ?? 0)}, <i>you lack the maturity and will that are required to master the high arts</i>, but… seeing your performance in this last endeavor, I am inclined to rethink our relationship."`);
      ...
      scene.text(`"You have done well my new <i>apprentice</i>. Now, ${((s as any).pcs_lastname ?? 0)}, come with me, is time that I teach you the secrets of the Lighting spell."`);
      ...
      if (((s as any).npc_rel ?? 0)?.['A241'] === 1  &&  ((s as any).AlexandriaQW ?? 0) === 19) {
        ...
        scene.text(`"${((s as any).pcs_lastname ?? 0)}, at risk of sounding ill-bred, I want to ask you a question… Why you decided to lie with the little nuisance?"`);
        ...
        scene.text(`"Ehm… ${((s as any).pcs_lastname ?? 0)}, maybe you find my question… <i>inpropper</i>… is not like I want to be privy of your <i>affairs</i>, but that woman is an agent for, at the moment, a unknow enemy, whose objectives are at best nebulous."`);
        ...
        scene.text(`"Is evident, that I must to up your training ${((s as any).pcs_lastname ?? 0)}, first teaching you the secrets of the Lighting spell, and then helping you to take away the idea of another dalliance with that <i>woman</i>."`);
        scene.actions([
          { label: 'Follow your master', goto: ['alexandriaSex', 'nice2'] },
        ]);
      } else {
        if (((s as any).npc_rel ?? 0)?.['A241'] === 1  &&  ((s as any).AlexandriaQW ?? 0) === 18) {
          ...
          scene.text(`"${((s as any).pcs_lastname ?? 0)}, at risk to sound ill-bred, I want to ask you a question… Why you didn't lie with the little nuisance?"`);
          ...
          scene.text(`"Of course no! ${((s as any).pcs_lastname ?? 0)} you did the correct! I just thought that…" He stops in his tracks and seems to consider his answer before continuing. "Well ${((s as any).pcs_lastname ?? 0)}, it doesn't matter, is evident that as my apprentice you have the <i>Gravitas</i>, to do the correct and ignore the siren's call from some loose moral woman, that obviously is working for one of my so-called enemies."`);
          ...
          scene.text(`"Is evident, that I must to up your training ${((s as any).pcs_lastname ?? 0)}, teaching you the secrets of the Lighting spell, but first… I think a little shopping is necessary."`);
          scene.actions([
    ...
    scene.text(`"I admit, ${((s as any).pcs_lastname ?? 0)}, that I wasn't expecting for me to come to this establishment, but… you need a recompense, and sadly, my curse acting through the <i>apprentice link</i>, will, without doubt, end in a more <i>girly</i> outcome so… we will need to ride the wave and find something of good taste that you can wear, without embarrassing both of us."`);
    ...
    if (((s as any).loop_counter ?? 0) === 300) {
      ...
      scene.text(`"Sorry ${((s as any).pcs_lastname ?? 0)}, that was a waste of our valuable time, that could have been used for more worthy endeavors… sigh… let get that for late and indulge us on something more worthy of our time."`);
      ...
      if (qspFunc(s, 'clothing', 'is_owned', 'moncheri_gown', ((s as any).numrand ?? 0))) {
        ...
        scene.img(`images/pc/items/moncheri/gown/${((s as any).numrand ?? 0)}.jpg`);
      }
    }
    ...
    scene.text(`"That was… interesting ${((s as any).pcs_lastname ?? 0)}, but sadly a necessity, as is inescapable that you, as my apprentice, will be required to attempt certain societal gatherings where… so-called <i>masters</i> will judge you based only on your appearance… sigh… let get that for late and indulge us on something more worthy of our time."`);
    scene.text('The rest of the trip towards his home, go without problems, and when you arrive, Aleksei don\'t lose time in taking you to his room.');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 13;
          ...
          if (((s as any).npc_rel ?? 0)?.['A241'] === 1  &&  ((s as any).AlexandriaQW ?? 0) === 17) {
            ...
            scene.img('images/characters/city/alexandria/alexandria3.jpg');
            // TODO-QSP: dynamic text: "Is evident, that I must to up your training <<$pcs_lastname>>, teaching you the...
            scene.text(`"Is evident, that I must to up your training ${((s as any).pcs_lastname ?? 0)}, teaching you the secrets of the Lighting spell."`);
            scene.text('Aleksei, then walk towards you, put his hands on your shoulders, and push you towards the studio\'s exit.');
            scene.actions([
        ...
        scene.text(`"And… What is impeding you to find this child ${((s as any).pcs_lastname ?? 0)}? Sigh… She will be around the city center during the weekend, and the challenge will be physical, so… you better dress accordingly."`);
