        ...
        do {
          ...
          do {
            ...
            if (((st as any).obekt ?? 0)?.['numberOSDM'] < 0  ||  ((st as any).obekt ?? 0)?.['numberOSDM'] >= 9  ||  ((st as any).obekt ?? 0)?.['numberOSDM'] !== ((st as any).obekt ?? 0)?.['numdmr']) {
              alert(qspUntranslated(s, "(\"Silence, nothing has changed\")", { location: "obekt" }));
            } else {
              ((st as any).obekt = (st as any).obekt ?? {})['openstateDM1'] = 1;
              alert(qspUntranslated(s, "(\"In the door that just clicked\")", { location: "obekt" }));
            }
            qspGoto(st, 'obekt', 'dungeon2');
          } while (((st as any).obekt ?? 0)?.['i'] < 5);
        } while (((st as any).obekt ?? 0)?.['DMN'] === ((String(((st as any).obekt ?? 0)?.['DMN']).slice((2)-1))) + ((String(((st as any).obekt ?? 0)?.['DMN']).slice((1)-1, ((1)-1)+(1)))) || ((st as any).obekt ?? 0)?.['DMN'] === '8809' || ((st as any).obekt ?? 0)?.['DMN'] === '7111' || ((st as any).obekt ?? 0)?.['DMN'] === '2172' || ((st as any).obekt ?? 0)?.['DMN'] === '3213' || ((st as any).obekt ?? 0)?.['DMN'] === '7662' || ((st as any).obekt ?? 0)?.['DMN'] === '9312' || ((st as any).obekt ?? 0)?.['DMN'] === '8193' || ((st as any).obekt ?? 0)?.['DMN'] === '8096' || ((st as any).obekt ?? 0)?.['DMN'] === '7756' || ((st as any).obekt ?? 0)?.['DMN'] === '6855' || ((st as any).obekt ?? 0)?.['DMN'] === '9881' || ((st as any).obekt ?? 0)?.['DMN'] === '5531');
      }
    }
  } },
  ]);
...
function enterDungeon4(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).obekt ?? 0)?.['openstateDM2'] === 1) {
    ...
    if (((st as any).obekt ?? 0)?.['numberOSDM2'] === 1415) {
      ...
      alert(qspUntranslated(s, "(\"Outside that it rattled.\")", { location: "obekt" }));
    } else {
      alert(qspUntranslated(s, "(\"Silence, nothing has changed\")", { location: "obekt" }));
    }
