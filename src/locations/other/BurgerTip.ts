...
{ label: 'Go to the boss\'s office', handler: (st: GameState) => {
    ...
    qspCall(st, 'stat', '');
    if (((st as any).stat ?? 0)?.['think_virgin'] === 0  ||  ((st as any).pcs_ass ?? 0) > 0) {
      scene.actions([
    ...
    (st as any).bosstipsexrand = (Math.floor(Math.random() * 10) + 1);
    ...
    qspCall(st, 'stat', '');
    if (((st as any).neighborQW ?? 0)?.['stage'] === 0) {
      ...
      if (((st as any).neighborQW ?? 0)?.['stage'] > 0  &&  ((st as any).neighborQW ?? 0)?.['knows_work'] > 0) {
        ((st as any).neighborQW = (st as any).neighborQW ?? {})['knows_work'] = 1;
...
{ label: 'Oh, I suppose', handler: (st: GameState) => {
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['sergeyQW'] = 1;
    qspCall(st, 'stat', '');
...
function enterIlyWork(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWBlackmail'] = 1;
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/citycenter/diner/work5.jpg');
...
function enterIlyWork2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    qspCall(st, 'stat', '');
    qspGoto(st, 'BurgerTip', 'ily_work3');
  } },
    ]);
  }
  // TODO-QSP: end
