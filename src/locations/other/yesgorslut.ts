...
function enter(s: GameState, scene: SceneBuilder): void {
  if (((s as any).fame ?? 0)?.['pav_slut'] >= 250) {
    if (((s as any).npc_grupTipe ?? 0)['A' + ((s as any).numnpc ?? 0)] === 5) {
      ...
      // TODO-QSP: dynamic text: You have a chat with <<$npc_firstname[''A<<numnpc>>'']>>, who happily listens to...
      ...
      if (((s as any).npc_grupTipe ?? 0)['A' + ((s as any).numnpc ?? 0)] === 0) {
        if (((s as any).npc_gender ?? 0)['A' + ((s as any).numnpc ?? 0)] === 0) {
          // TODO-QSP: dynamic text: You''re doing your best to engage in conversation with the guy, but <<$npc_first...
          ...
          // TODO-QSP: dynamic text: You''re doing your best to engage in conversation with the girl, but <<$npc_firs...
