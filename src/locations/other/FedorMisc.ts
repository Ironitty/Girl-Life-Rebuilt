...
function enterFedorChat(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).npc_rel ?? 0)?.['A5'] < 20) {
      ...
      if (((s as any).npc_rel ?? 0)?.['A5'] < 80  &&  ((s as any).npc_rel ?? 0)?.['A5'] >= 20) {
...
function enterFedorWorkout(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'workout_date') {
  ...
  scene.actions([
    ...
    scene.text(`Fedor begins doing push ups as you do a few yourself until you look up and see him grinning at you as he says, "Would you like to see me do 45 push ups in 2 minutes?" You respond, "Only if I can ride your back as you do it." Fedor tilts his head as his grin widens. "Challenge accepted." you then climb on his back and he lift you both in repeated cycles. You can feel his muscles tighten as he lifts and his heartbeat gradually increase as he lifts you on his back. Fedor finally reaches 45 push ups then lowers his body to the ground where he lays with you still on his back. "So ${((s as any).pcs_nickname ?? 0)}… impressed?" You reply "Yes very, I'm a lucky girl to have such a strong man to myself." Fedor winks then gets up and walks toward his bag.`);
...
function enterFedorWorkout2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).VK ?? 0) === 1) {
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedormisc/gym/chat\'+rand(1, 3)+\'.jpg');
    ...
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedormisc/gym/selfie\'+rand(1, 3)+\'.jpg');
...
function enterStrength(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).FedorDW ?? 0) === 1) {
    ...
    scene.text(`You take a well needed stretch before turning back to see Fedor collecting his things, and placing them in his gym bag before he looks to you and says, "Hey ${((s as any).pcs_nickname ?? 0)} would you like to come back to my place to hang out, maybe watch some tv?"`);
    scene.actions([
...
function enterOutcastIntro(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).FedorLove ?? 0))) {
    ...
    scene.text(`You slowly step toward Fedor and say, "Look Fedor… I never meant for any of this to happen." Fedor shouts "YOU AGREED TO GO OUT WITH VIKTOR WHILE DATING ME! How can you expect me to believe that you cared for me at all… Just go away ${((s as any).pcs_firstname ?? 0)}. I have nothing for you." you step away from Fedor knowing that nothing you say will console him while thinking to yourself "If only he trusted me more, then maybe he would believe that my apology is sincere."`);
    scene.actions([
      { label: 'Walk away', goto: ['pav_park', 'start'] },
    ]);
  } },
      { label: 'Mock', handler: (st: GameState) => {
    (s as any).FedorOutcast = (-2);
    qspCall(s, 'npc_relationship', 'set', 'A5', 0);
    ...
    scene.text(`As you approach Fedor, he turns to you with a distraught look on his face. "${((s as any).pcs_firstname ?? 0)}… What are you doing here. I bet Viktor will be upset with us both if he sees you with me." You reply, "Viktor doesn't own me. I came here to check on you." Fedor responds, "How did you know to find me here." You give Fedor a sweet smile as you reply, "On our first date since hooking up, you told me that this is where you go when you are feeling down." Fedor returns the smile. "You remembered? I always knew you were one of the good ones."`);
    scene.actions([
      { label: 'Cheer him up', handler: (st: GameState) => {
    (s as any).fedorKozlovQW = (-12);
    qspCall(s, 'npc_relationship', 'modify', 'A5', 10);
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    (s as any).FedorOutcast = 1;
    (s as any).fedormasha = 0;
    qspCall(s, 'stat', '');
    scene.text(`<center><b>${'Fyodor [Fedor] Kozlov'}</b></center>`);
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedormisc/Outcast/outcast2.jpg');
    // TODO-QSP: dynamic text: You walk up to Fedor then place your hand on his shoulder. "You may be an outcas...
    scene.text(`You walk up to Fedor then place your hand on his shoulder. "You may be an outcast for some but you will always be a friend to me." Fedor downcasts his eyes as a tear escapes them. "${((s as any).pcs_firstname ?? 0)} you truly are an angel. I'm so lucky to have a friend like you. Thank you." You give Fedor a hug as you say, "I know things are really hard for you but I need you to stay strong. Can you do that for me?" Fedor returns the hug. "I won't let what happened keep me down. I promise." You respond, "I expected nothing less from you." you then give him a kiss on the cheek.`);
...
function enterOutcast(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).VK ?? 0) === 1) {
    scene.text(`<center><b>${'Fyodor [Fedor] Kozlov'}</b></center>`);
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedormisc/Outcast/parkalone\'+rand(1, 3)+\'.jpg');
    scene.text('You walk toward the train tracks to see Fedor, he seems to be lost in thought.');
  } else {
    scene.text(`<center><b>${'Fyodor [Fedor] Kozlov'}</b></center>`);
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedormisc/Outcast/parkalone\'+rand(4, 5)+\'.jpg');
  ...
  if (((s as any).fedorKozlovQW ?? 0) === -12) {
    ...
    scene.text(`You approach Fedor and his eyes light up as he asks "${((s as any).pcs_nickname ?? 0)} would you like to spend some time together or is there something else you want?"`);
    scene.actions([
...
function enterFedorVsDimka(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'anal_finger', 5, 'sub', 'rough');
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 10;
    ...
    // TODO-QSP: dynamic text: You feel Dimka's hands on your ass cheeks, spreading them open. You suddenly fee...
    scene.text(`You feel Dimka's hands on your ass cheeks, spreading them open. You suddenly feel a sharp pain as Dimka shoves his dick inside your asshole, causing you to shriek in searing pain before hearing some thumping behind you. You turn around to see Fedor sprinting at the both of you. Dimka quickly pulls off of you as he begins zipping his pants, he then angrily shouts "What the fu…" Fedor punches him in his face, knocking Dimka a meter away. Dimka quickly looks up at Fedor. "What the fuck is wrong with you!?" Fedor begins kicking him in his ribs while shouting "<b>How dare you fuck with ${((s as any).pcs_firstname ?? 0)}! She is my girl and no one fucks with my girl!</b>" Fedor continues kicking Dimka in his ribs.`);
    scene.text(`<center><b>${'Fyodor [Fedor] Kozlov'}</b></center>`);
    if (((s as any).fedorKozlovQW ?? 0) === -15) {
      scene.img('images/characters/pavlovsk/school/boy/fedor/fedormisc/Outcast/outcast2.jpg');
    } else {
    ...
    // TODO-QSP: dynamic text: You quickly rush to Fedor's side then wrap your hands around his waist. You begi...
    scene.text(`You quickly rush to Fedor's side then wrap your hands around his waist. You begin pulling Fedor off of Dimka. Fedor looks to you then his rage turns to sorrow as he sees you naked, realizing that he was too late. "${((s as any).pcs_firstname ?? 0)} I am so sorry. I thought that you were heading to the other side of the school." You give Fedor a firm hug as you reply, "It's ok Fedor. The important thing is that you came for me." You flash Fedor an innocent smile as you continue hugging him. Dimka, still squirming in pain, looks toward Fedor then says, "I didn't know ${((s as any).pcs_firstname ?? 0)} was yours but there is something you should know about her." He then pulls his phone out of his pocket.`);
    scene.actions([
      { label: 'Snatch the phone', handler: (st: GameState) => {
    (s as any).dimaFilm = 0;
    (s as any).DimaRudeBlock = 1;
    (s as any).FedorvsDimka = 2;
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    qspCall(s, 'stat', '');
    scene.text(`<center><b>${'Fyodor [Fedor] Kozlov'}</b></center>`);
    if (((s as any).fedorKozlovQW ?? 0) === -15) {
...
function enterDimkaParty2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You push Dimka away from you while shouting "hey! Beat it! Are you crazy or something?" but he doesn't let go and begins dragging you into the next room when you both hear "Take your hands off her!" You both turn to the corner of the room to see Fedor in the shadows. Dimka laughs as he says, "Well look who it is… the cocksucker Masha. If you want to suck my dick then you better get lost because I have ${((s as any).pcs_firstname ?? 0)} for that." after 2 seconds of silence Fedor charges Dimka and begins punching him repeatedly.`);
