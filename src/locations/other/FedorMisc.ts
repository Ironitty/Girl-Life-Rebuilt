...
function enterFedorChat(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_hairbsh ?? 0) < 1) {
    scene.text(`Fedor notices your messy hair and pulls out his <a href="#" onclick="window.__gameStore.getState().doGoto(/u0027FedorMisc/u0027, /u0027Comb/u0027, ((s as any).locArgs?.[1] ?? /u0027/u0027)); return false;">comb</a>.`);
  }
...
function enterOutcast(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).fedorKozlovQW ?? 0) === -12) {
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    scene.text(`<center><b><h4><font color=#FF00FF>${'' + ((st as any).pcs_firstname ?? '') + ' [' + ((st as any).pcs_nickname ?? '') + '] ' + ((st as any).pcs_lastname ?? '') + ''}</font></h4></b></center>`);
    scene.img(`${qspFunc(s, '$face_image', '')}`);
