...
function enterIntroPavlovsk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29']} shoots you a sharp look and you know to fix your attitude or else. "Well, that depends on what you plan to do in the morning. There's breakfast, showering and brushing your teeth, doing your hair and makeup, and shaving if you really need to. If I remember correctly, Anya used to wake up at ' + func('time', 'get_time_string', 6, 0) + ' for school."`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
    qspCall(s, 'stat', '');
    ...
    scene.text('"You want me to wake up that early?!" you huff. "Can\'t I wake up at like… \' + func(\'time\', \'get_time_string\', 7, 0) + \'?"');
...
function enterIntroGadukino(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).month ?? 0) === 6) {
      ...
      scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29']} shoots you a sharp look, and you know to fix your attitude or else. "Well, depends on what you plan to do in the morning. There's showering and brushing your teeth, doing your hair and makeup, and shaving if you really need to. If I remember correctly, Anya used to wake up at ' + func('time', 'get_time_string', 6, 0) + ' when she used to sleep here. Besides, while staying at your grandparents' house, you should be up extra early helping them tend to the farm. This isn't a holiday camp!"`);
    } else {
      // TODO-QSP: dynamic text: Your <<$npc_nickname['A29']>> shoots you a sharp look, and you know to fix your ...
      scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29']} shoots you a sharp look, and you know to fix your attitude or else. "Well, depends on what you plan to do in the morning. There's showering and brushing your teeth, doing your hair and makeup, and shaving if you really need to. If I remember correctly, Anya used to wake up at ' + func('time', 'get_time_string', 6, 0) + ' for school. Besides, while staying at your grandparents' house, you should be up extra early helping them tend to the farm. This isn't a holiday camp!"`);
    }
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
...
function enterIntroPushkin(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"No excuses, ${((s as any).pcs_nickname ?? 0)}! Your stepfather only has this morning free to drive you and your stuff over to Pushkin. You can sleep when you get there," your ${((s as any).npc_nickname ?? 0)?.['A29']} says as she stares at you expectantly.`);
    scene.actions([
      { label: 'Get up', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
    qspCall(s, 'stat', '');
...
function enterIntroSharing(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"No excuses, ${((s as any).pcs_firstname ?? 0)}! You have 5 minutes to pack up and get out before I drag you out of here, dressed or not. It's not like the whole of Pavlosk hasn't already seen you naked anyway!"`);
    scene.actions([
      { label: 'Get up', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
    qspCall(s, 'stat', '');
