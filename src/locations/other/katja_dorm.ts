...
function enterTalk(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['dress_borrowed'] === 1) {
    ((s as any).katjaQW ?? {})['dress_borrowed'] = 0;
      ...
      if (((s as any).locat ?? 0)?.['katja'] === 29) {
        ...
        if (((s as any).week ?? 0) === 4) {
          ...
          if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
            scene.actions([
              { label: 'Convince Katja to stay here and watch a movie [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
...
function enterChat(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 6  &&  (((s as any).katjaQW ?? 0)?.['lazer_anal'] + ((s as any).katjaQW ?? 0)?.['lazer_pussy']) > 0  &&  ((s as any).week ?? 0) < 6  &&  ((s as any).katjaQW ?? 0)?.['horny'] > 80  &&  ((s as any).katjaQW ?? 0)?.['Lazar_day'] !== ((s as any).daystart ?? 0)  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] === 0) {
    ...
    ((s as any).katjaQW ?? {})['Lazar_day'] = ((s as any).daystart ?? 0);
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    qspCall(s, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    ...
    if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 6  &&  ((s as any).katjaQW ?? 0)?.['anal_quest'] >= 3  &&  (!(Math.floor(Math.random() * 7) + 0))) {
      // TODO-QSP: dynamic text: You call him, and he tells you that Ivan is over at his place, but ' + iif(katja...
      scene.text('You call him, and he tells you that Ivan is over at his place, but \' + iif(katjaQW[\'Lazar_Ivan_4some\'] = 0, \'he\'s sure that Ivan would like to join the fun as long as you\'re willing to take it in the ass\', \'you had a lot of fun last time and he\'s sure Ivan is up for it again\') + \'.');
      if (((s as any).katjaQW ?? 0)?.['Lazar_Ivan_4some'] === 0) {
        if (((s as any).katjaQW ?? 0)?.['simultanous_boys'] >= 2  ||  ((s as any).katjaQW ?? 0)?.['slut'] >= 80  ||  (((s as any).katjaQW ?? 0)?.['ivan_anal'] === 1  &&  ((s as any).katjaQW ?? 0)?.['slut'] >= 70)) {
          (s as any).minut = ((s as any).minut ?? 0) + 5;
          // TODO-QSP: dynamic text: "Sounds fun," Katja says. "' + iif(katjaQW['simultanous_boys'] >= 2, 'It'll be f...
          scene.text('"Sounds fun," Katja says. "\' + iif(katjaQW[\'simultanous_boys\'] >= 2, \'It\'ll be fun to have sex with several boys again\', \'I think it\'s time for me to try sex with two boys at once\') + \'."');
  ...
  scene.actions([
    ...
    scene.text(`<center><b>${((s as any).npc_firstname ?? 0)?.['A14']} ${((s as any).npc_lastname ?? 0)?.['A14']}</b></center>`);
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    if (((Math.floor(Math.random() * 101) + 0) > ((s as any).katjaQW ?? 0)?.['horny'] + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 2, 50))  ||  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] === 2) {
      ...
      qspCall(s, 'katja_sex_talk', 'Sextalk');
    }
  } },
...
function enterSofiaMeet(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "Sofia, this is <<$pcs_nickname>>, my ' + iif($university['enrolled_in'] = 'teac...
  scene.text(`"Sofia, this is ${((s as any).pcs_nickname ?? 0)}, my ' + iif($university['enrolled_in'] = 'teaching_studies', 'classmate and ', ') + 'friend from school who was just about to leave," Katja replies before she turns to you. "${((s as any).pcs_nickname ?? 0)}, this is Sofia, my roommate."`);
  // TODO-QSP: dynamic text: "Hi <<$pcs_nickname>>, it's so nice to meet Katja's friends since she doesn't se...
  scene.text(`"Hi ${((s as any).pcs_nickname ?? 0)}, it's so nice to meet Katja's friends since she doesn't seem to have a lot of them," Sofia says, which clearly makes Katja blush. "So ' + iif($university['enrolled_in'] = 'teaching_studies', 'you're also studying to become a teacher like Katja', 'what are you studying') + '?" she asks.`);
  // TODO-QSP: dynamic text: ' + iif( $university['enrolled_in'] = 'teaching_studies', '"yes" ', '"I'm ' + ii...
  scene.text('\' + iif( $university[\'enrolled_in\'] = \'teaching_studies\', \'"yes" \', \'"I\'m \' + iif(university[\'student\'] = 1, \'in the \' + $func(\'uni_programs\', \'get_program_name\') + \' program\', \'not studying at the university\') + \'"\') + \'you answer.');
  // TODO-QSP: dynamic text: "Cool. I'm ' + iif($university['enrolled_in'] = 'nursing', 'also studying to bec...
  scene.text('"Cool. I\'m \' + iif($university[\'enrolled_in\'] = \'nursing\', \'also studying to become a nurse. I think I\'ve seen you in class now that I think about it.\', \'studying to become a nurse.\') + \' You must tell me all about yourself and what naughty things Katja was up to in school," she says, making Katja blush even more.');
  // TODO-QSP: dynamic text: "I'm sorry, but <<$pcs_nickname>> was just leaving," Katja says, looking at you ...
  scene.text(`"I'm sorry, but ${((s as any).pcs_nickname ?? 0)} was just leaving," Katja says, looking at you as if trying to say she's sorry.`);
...
function enterLazarPussyTalk(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['lazer_pussy_talk'] + ((s as any).katjaQW ?? 0)?.['lazer_anal_talk'] === 0  &&  ((s as any).katjaQW ?? 0)?.['lazer_anal'] === 0) {
    ((s as any).katjaQW ?? {})['lazer_pussy_talk'] = 1;
    if (((s as any).katjaQW ?? 0)?.['QWstage'] < 6  &&  ((s as any).katjaQW ?? 0)?.['anal_quest'] < 3) {
      ...
      scene.text(`"You know, ${((s as any).pcs_nickname ?? 0)}, meeting Lazar at the café got me thinking," Katja says.`);
    ...
    if (((s as any).katjaQW ?? 0)?.['QWstage'] < 5) {
      ((s as any).katjaQW ?? {})['QWstage'] = 5;
              ...
              { label: 'Refuse', handler: (st: GameState) => {
    ((s as any).katjaQW ?? {})['lazer_pussy_talk'] = 2;
    // TODO-QSP: gt 'katja_dorm', 'lazar_refuse', 'pussy'
  } },
            ]);
          } else {
    ...
    // TODO-QSP: dynamic text: "No. I've never had ' + iif(stat['anal'] = 0, 'sex', 'vaginal sex') + ' with a g...
    scene.text('"No. I\'ve never had \' + iif(stat[\'anal\'] = 0, \'sex\', \'vaginal sex\') + \' with a guy," you say.');
    ...
    // TODO-QSP: gt 'katja_dorm', 'lazar_refuse', 'pussy'
      ...
      if (((s as any).katjaQW ?? 0)?.['QWstage'] < 6) {
        ...
        scene.text(`"You know, ${((s as any).pcs_nickname ?? 0)}, meeting Lazar at the cafe got me thinking," Katja says.`);
        if (((s as any).katjaQW ?? 0)?.['QWstage'] === 4) {
    ...
    // TODO-QSP: gt 'katja_dorm', 'lazar_refuse', 'pussy'
              ...
              if (((s as any).stat ?? 0)?.['vaginal'] === 0) {
                scene.actions([
                  { label: 'Tell her you haven\'t', handler: (st: GameState) => {
    // TODO-QSP: dynamic text: "No. I've never had ' + iif(stat['anal'] = 0, 'sex', 'vaginal sex') + 'with a gu...
    scene.text('"No. I\'ve never had \' + iif(stat[\'anal\'] = 0, \'sex\', \'vaginal sex\') + \'with a guy," you say.');
    ...
    // TODO-QSP: gt 'katja_dorm', 'lazar_refuse', 'pussy'
        ...
        scene.text(`"You know ${((s as any).pcs_nickname ?? 0)}, when we talked with Lazar at the café, he seemed really into us," Katja says. "Maybe we should call him and see if he's up to having some fun with us?"`);
        scene.actions([
          { label: 'Agree', handler: (st: GameState) => {
    if (((s as any).katjaQW ?? 0)?.['QWstage'] < 5) {
      ((s as any).katjaQW ?? {})['QWstage'] = 5;
    ...
    if (((s as any).katjaQW ?? 0)?.['lazer_pussy_talk'] === 2) {
      ...
      scene.text('"I know you said earlier that you didn\'t want to have vaginal sex with Lazar\' + iif(katjaQW[\'lazer_anal\'] > 0, \', but we\'ve had anal sex with him; \', \' \') + \'so would you please reconsider? I really want to try with a guy, and I want you to be there. Please?" Katja pleads, looking at you with her big eyes.');
      scene.actions([
...
function enterLazarRefuse(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).stat ?? 0)?.['vaginal'] === 0  &&  ((s as any).locArgs?.[1] ?? 0) === 'pussy') {
    scene.actions([
  ...
  if (((s as any).stat ?? 0)?.['anal'] === 0  &&  ((s as any).locArgs?.[1] ?? 0) === 'anal') {
    scene.actions([
...
function enterLazarAgree(s: GameState, scene: SceneBuilder): void {
  ...
  scene.img('images/characters/shared/headshots_main/big14.jpg');
  if (((s as any).katjaQW ?? 0)?.['QWstage'] === 5) {
    // TODO-QSP: dynamic text: "Okay. We should let Lazar be ' + iif(stat['vaginal'] > 0, 'your', 'our') + ' fi...
    scene.text('"Okay. We should let Lazar be \' + iif(stat[\'vaginal\'] > 0, \'your\', \'our\') + \' first guy," you agree.');
    scene.text('Katja looks a little embarrassed, but has a naughty gleam in her eyes.');
  } else {
    if (((s as any).katjaQW ?? 0)?.['QWstage'] > 5) {
      // TODO-QSP: dynamic text: "Okay. ' + iif(katjaQW['ivan_anal'] + katjaQW['marcus_pussy'] + katjaQW['marcus_...
      scene.text('"Okay. \' + iif(katjaQW[\'ivan_anal\'] + katjaQW[\'marcus_pussy\'] + katjaQW[\'marcus_anal\'] > 0, \'We can include Lazar in our harem of hot guys\', \'We Having a threesome with Lazar do sounds fun\') + \'" you say, which causes Katja to start laughing.');
  ...
  if (((s as any).katjaQW ?? 0)?.['anal_quest'] === 2) {
    if (((s as any).katjaQW ?? 0)?.['lazer_anal_talk'] === 1) {
      if (((s as any).katjaQW ?? 0)?.['QWstage'] > 5) {
        ...
        // TODO-QSP: dynamic text: "Okay. We should let Lazar be the first guy to fuck ' + iif(stat['anal'] > 0, 'y...
        scene.text('"Okay. We should let Lazar be the first guy to fuck \' + iif(stat[\'anal\'] > 0, \'your\', \'our\') + \' ass," you agree.');
        scene.text('Katja looks a little embarrassed, but has a naughty gleam in her eyes.');
      }
    } else {
      ((s as any).katjaQW ?? {})['lazer_anal_talk'] = 1;
  ...
  if (((s as any).katjajQW ?? 0)?.['slut'] > 60  &&  ((s as any).katjaQW ?? 0)?.['anal_quest'] === 0) {
    ...
    scene.text(`You take out your phone and call Lazar. "What up, ${((s as any).pcs_nickname ?? 0)}?" Lazar says when he answers, almost immediately.`);
    ...
    if (((s as any).katjaQW ?? 0)?.['QWstage'] === 5) {
      if (((s as any).katjaQW ?? 0)?.['anal_quest'] === 2) {
        // TODO-QSP: dynamic text: "Well ' + iif(stat['vaginal'] > 0, 'Katja has a pussy', 'we have pussies') + ' a...
        scene.text('"Well \' + iif(stat[\'vaginal\'] > 0, \'Katja has a pussy\', \'we have pussies\') + \' and \' + iif(stat[\'anal\'] > 0, \'Katja has a butt\', \'we have butts\') + \' that have never tried a cock, and thought that you would be the man for doing something about it?" you say.');
      } else {
        // TODO-QSP: dynamic text: "Well ' + iif(stat['vaginal'] > 0, 'Katja has a pussy that has', 'we have pussie...
        scene.text('"Well \' + iif(stat[\'vaginal\'] > 0, \'Katja has a pussy that has\', \'we have pussies that have\') + \' never tried a cock, and thought that you would be the man for doing something about it?" you say.');
      }
      ...
      if (((s as any).katjaQW ?? 0)?.['anal_quest'] === 2) {
        // TODO-QSP: dynamic text: "Well, ' + iif( stat['anal'] > 0, 'Katja has a butt that has', 'we have butts th...
        scene.text('"Well, \' + iif( stat[\'anal\'] > 0, \'Katja has a butt that has\', \'we have butts that have\') + \' never tried a cock, and thought that you would be the man for doing something about that?" you say.');
        scene.text('You hear Lazar gasp at your brazenness and see Katja\'s face turning completely red.');
        scene.text('After getting himself together, he answers, "Of course, I am the right man for that. Come on over."');
      } else {
    ...
    // TODO-QSP: dynamic text: "We will be right over," you say' + iif(katjaQW['lazer_anal'] + katjaQW['lazer_p...
    scene.text('"We will be right over," you say\' + iif(katjaQW[\'lazer_anal\'] + katjaQW[\'lazer_pussy\'] = 0, \' and he gives you his address \', \' \') + \'before you hang up.');
    scene.actions([
...
function enterLazarAnalTalk(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['lazer_pussy_talk'] + ((s as any).katjaQW ?? 0)?.['lazer_anal_talk'] === 0  &&  ((s as any).katjaQW ?? 0)?.['lazer_pussy'] === 0) {
    ...
    ((s as any).katjaQW ?? {})['anal_quest'] = 2;
            ...
            { label: 'Refuse', handler: (st: GameState) => {
    ((s as any).katjaQW ?? {})['lazer_anal_talk'] = 2;
    // TODO-QSP: gt 'katja_dorm', 'lazar_refuse', 'anal'
  } },
          ]);
        } else {
          scene.text('"Have you ever…" she continues.');
          ...
          if (((s as any).stat ?? 0)?.['anal'] === 0) {
            scene.actions([
              { label: 'Tell her you haven\'t', handler: (st: GameState) => {
    // TODO-QSP: dynamic text: "No. I've never had ' + iif(stat['vaginal'] = 0, 'sex', 'anal sex') + ' with a g...
    scene.text('"No. I\'ve never had \' + iif(stat[\'vaginal\'] = 0, \'sex\', \'anal sex\') + \' with a guy," you say.');
    scene.text('"Me neither," Katja says. "Do you want to try?" she asks hesitantly.');
    ...
    ((s as any).katjaQW ?? {})['anal_quest'] = 2;
            ...
            { label: 'Refuse', handler: (st: GameState) => {
    ((s as any).katjaQW ?? {})['lazar_anal_talk'] = 2;
    ((s as any).katjaQW ?? {})['horny'] = (((s as any).katjaQW ?? {})['horny'] ?? 0) - (5);
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    ...
    // TODO-QSP: gt 'katja_dorm', 'lazar_refuse', 'anal'
            ...
            if (((s as any).stat ?? 0)?.['anal'] === 0) {
              scene.actions([
                { label: 'Tell her you haven\'t', handler: (st: GameState) => {
    // TODO-QSP: dynamic text: "No. I've never had ' + iif(stat['vaginal'] = 0, 'sex', 'anal sex') + ' with a g...
    scene.text('"No. I\'ve never had \' + iif(stat[\'vaginal\'] = 0, \'sex\', \'anal sex\') + \' with a guy," you answer.');
    scene.text('"Me neither," Katja says. "Do you want to try?" she asks hesitantly.');
    scene.text('"You want Lazar to be the first guy fucking our butts?" you ask, surprised. Katja just bites her lips and slowly nods.');
    scene.actions([
      { label: 'Agree', handler: (st: GameState) => {
    ...
    // TODO-QSP: gt 'katja_dorm', 'lazar_refuse', 'anal'
        ...
        scene.text(`"You know, ${((s as any).pcs_nickname ?? 0)}, when we talked with Lazar at the café, he seemed really into us," Katja says. "Maybe we should call him and see if he's up for having some fun with us."`);
        scene.actions([
          { label: 'Agree', goto: ['katja_dorm', 'lazar_agree'] },
...
function enterMovieTalk1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).week ?? 0) > 4  ||  ((s as any).katjaQW ?? 0)?.['slut'] > 75) {
    ...
    if (((s as any).week ?? 0) === 4  &&  ((s as any).university ?? 0)?.['exam_week'] === 1) {
      ...
      if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
        scene.actions([
          { label: 'Convince her to go out [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
        ]);
      } else {
...
function enterWatchMovie(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!(Math.floor(Math.random() * 3) + 0))) {
    ((s as any).katjaQW ?? {})['movie_temp'] = 2 + Math.max(((s as any).rand ?? 0)(-2, 2) + Math.min((((s as any).katjaQW ?? {})?.['slut'] + ((s as any).katjaQW ?? {})?.['horny']) / 50, 2), 0);
  } else {
    ((s as any).katjaQW ?? {})['movie_temp'] = Math.floor(Math.random() * 2) + 0;
  }
  if (((s as any).katjaQW ?? 0)?.['movie_temp'] === 4) {
    (s as any).t = Math.floor(Math.random() * 4) + 1;
    ((s as any).katjaQW ?? {})['horny'] = (((s as any).katjaQW ?? {})['horny'] ?? 0) + (((s as any).t ?? 0) * 10);
    ...
    if (((s as any).npc_had_sex ?? 0)?.['A14']  &&  ((s as any).katjaQW ?? 0)?.['horny'] + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 3, 34) >= 100  &&  (Math.floor(Math.random() * 4) + 0) < 3  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
      ...
      if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 3  &&  Math.min(((s as any).katjaQW ?? 0)?.['horny'], 100) + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 2, 50) >= 115  &&  ((s as any).katjaQW ?? 0)?.['masturbates'] > 0  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
        ...
        (s as any).minut = ((s as any).minut ?? 0) + 10;
        qspCall(s, 'stat', '');
        // TODO-QSP: dynamic text: You spend some time discussing the film once it's finished. You agree that it wa...
        scene.text('You spend some time discussing the film once it\'s finished. You agree that it was \' + iif(rand(0, 1) = 1, \'good,\', \'bad, but\') + \' with \' + iif(rand(0, 1) = 1, \'good\', \'okay\') + \' acting.');
        // TODO-QSP: dynamic text: Katja ' + iif(t > 2, 'keeps coming back to the sex scenes and how good the actor...
        scene.text('Katja \' + iif(t > 2, \'keeps coming back to the sex scenes and how good the actors looked in them.\', \'didn\'t find the scenes that believable and clearly didn\'t like the look of the main actress.\') + \' Katja then says she wants to go to sleep and you say your goodbyes.');
        scene.actions([
    ...
    if (((s as any).katjaQW ?? 0)?.['movie_temp'] === 3) {
      ...
      ((s as any).katjaQW ?? {})['horny'] = (((s as any).katjaQW ?? {})['horny'] ?? 0) - (((s as any).t ?? 0) * 5);
      ...
      if (((s as any).npc_had_sex ?? 0)?.['A14']  &&  ((s as any).katjaQW ?? 0)?.['horny'] + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 3, 34) >= 100  &&  (Math.floor(Math.random() * 4) + 0) < 3  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
        ...
        if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 3  &&  Math.min(((s as any).katjaQW ?? 0)?.['horny'], 100) + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 2, 50) >= 115  &&  ((s as any).katjaQW ?? 0)?.['masturbates'] > 0  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
          ...
          scene.text('Once the movie is finished, you \' + iif(rand(0, 1) = 1, \'spend some time crying before drying your eyes\', \' dry your eyes\') + \' and start discussing it. You agree that it was \' + iif(rand(0, 1) = 1, \'good,\', \'sad, but\') + \' with \' + iif(rand(0, 1) = 1, \'an extremely touching\', \'a somehow moving\') + \' story.');
          // TODO-QSP: dynamic text: Katja thinks the ending ' + iif(rand(0, 1) = 1, 'was too depressing.', 'made up ...
          scene.text('Katja thinks the ending \' + iif(rand(0, 1) = 1, \'was too depressing.\', \'made up for all the bleakness in the story.\') + \' She then says she wants to go to sleep and you say your goodbyes.');
          scene.actions([
            { label: 'Go to the hallway', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 2;
  }, goto: ['uni_dorm', 'second_floor'] },
      ...
      if (((s as any).katjaQW ?? 0)?.['movie_temp'] === 2) {
        ...
        if (((s as any).npc_had_sex ?? 0)?.['A14']  &&  ((s as any).katjaQW ?? 0)?.['horny'] + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 3, 34) >= 100  &&  (Math.floor(Math.random() * 4) + 0) < 3  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
          ...
          if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 3  &&  Math.min(((s as any).katjaQW ?? 0)?.['horny'], 100) + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 2, 50) >= 115  &&  ((s as any).katjaQW ?? 0)?.['masturbates'] > 0  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
            ...
            qspCall(s, 'stat', '');
            // TODO-QSP: dynamic text: You spend some time discussing the film once it's finished. You agree that it wa...
            scene.text('You spend some time discussing the film once it\'s finished. You agree that it was a \' + iif(rand(0, 1) = 1, \'good movie,\', \'bad movie, but\') + \' with \' + iif(rand(0, 1) = 1, \'good\', \'mediocre\') + \' acting.');
            // TODO-QSP: dynamic text: Katja thinks that the overall plot was ' + iif(rand(0, 1) = 1, 'quite interestin...
            scene.text('Katja thinks that the overall plot was \' + iif(rand(0, 1) = 1, \'quite interesting.\', \'too complicated.\') + \' She then says she wants to go to sleep and you say your goodbyes.');
            scene.actions([
              { label: 'Go to the hallway', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 2;
  }, goto: ['uni_dorm', 'second_floor'] },
        ...
        if (((s as any).katjaQW ?? 0)?.['movie_temp'] === 1) {
          ...
          ((s as any).katjaQW ?? {})['horny'] = (((s as any).katjaQW ?? {})['horny'] ?? 0) + (((s as any).t ?? 0) * 6);
          ...
          if (((s as any).npc_had_sex ?? 0)?.['A14']  &&  ((s as any).katjaQW ?? 0)?.['horny'] + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 3, 34) >= 100  &&  (Math.floor(Math.random() * 4) + 0) < 3  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
            ...
            if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 3  &&  Math.min(((s as any).katjaQW ?? 0)?.['horny'], 100) + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 2, 50) >= 115  &&  ((s as any).katjaQW ?? 0)?.['masturbates'] > 0  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
              ...
              qspCall(s, 'stat', '');
              // TODO-QSP: dynamic text: You spend some time discussing the film once it's finished. You agree that it wa...
              scene.text('You spend some time discussing the film once it\'s finished. You agree that it was a \' + iif(rand(0, 1) = 1, \'good movie,\', \'bad movie, but\') + \' with \' + iif(rand(0, 1) = 1, \'a funny\', \'a cringe\') + \' romance plot.');
              // TODO-QSP: dynamic text: Katja ' + iif(t > 3, 'keeps coming back to how great the chemistry between main ...
              scene.text('Katja \' + iif(t > 3, \'keeps coming back to how great the chemistry between main actors was and how hot they looked together.\', \'thinks that the overall plot was full of holes.\') + \' She then says she wants to go to sleep and you say your goodbyes.');
              scene.actions([
                { label: 'Go to the hallway', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 2;
  }, goto: ['uni_dorm', 'second_floor'] },
          ...
          (s as any).t = ((s as any).rand ?? 0)(-2, 6);
          ((s as any).katjaQW ?? {})['horny'] = (((s as any).katjaQW ?? {})['horny'] ?? 0) + (Math.max(((s as any).t ?? 0) * 5, 0));
          ...
          if (((s as any).npc_had_sex ?? 0)?.['A14']  &&  ((s as any).katjaQW ?? 0)?.['horny'] + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 3, 34) >= 100  &&  (Math.floor(Math.random() * 4) + 0) < 3  &&  ((s as any).braworntype ?? 0) !== 'none'  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
            ...
            if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 3  &&  Math.min(((s as any).katjaQW ?? 0)?.['horny'], 100) + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 2, 50) >= 115  &&  ((s as any).katjaQW ?? 0)?.['masturbates'] > 0  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
              ...
              qspCall(s, 'stat', '');
              // TODO-QSP: dynamic text: You spend some time discussing the film once it's finished. You agree that it wa...
              scene.text('You spend some time discussing the film once it\'s finished. You agree that it was a \' + iif(rand(0, 1) = 1, \'good movie,\', \'bad movie, but\') + \' with \' + iif(rand(0, 1) = 1, \'great\', \'mediocre\') + \' action scenes.');
              // TODO-QSP: dynamic text: Katja ' + iif(t > 3, 'keeps coming back to how hot the actors and actresses were...
              scene.text('Katja \' + iif(t > 3, \'keeps coming back to how hot the actors and actresses were.\', \'thinks the acting of the main character was good.\') + \' She then says she wants to go to sleep and you say your goodbyes.');
              scene.actions([
                { label: 'Go to the hallway', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 2;
  }, goto: ['uni_dorm', 'second_floor'] },
...
function enterAfterMovieStrip1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Oh.. Um.. Bye ${((s as any).pcs_nickname ?? 0)}. See you later…" she says without moving from the bed.`);
...
function enterRelationshipTalk(s: GameState, scene: SceneBuilder): void {
  ((s as any).locat ?? {})['katja_save2'] = 20;
  ((s as any).katjaQW ?? {})['relationship_talk'] = 1;
  if (((s as any).hour ?? 0) < 20) {
  ...
  scene.actions([
    ...
    ((s as any).katjaQW ?? {})['sex_clossedness'] = 2;
    ...
    if (((s as any).katjaQW ?? 0)?.['QWstage'] >= 6  ||  ((s as any).katjaQW ?? 0)?.['anal_quest'] >= 3  ||  ((s as any).katjaQW ?? 0)?.['simultanous_girls'] > 1) {
      // TODO-QSP: dynamic text: "But we have also had sex with other ' + iif(katjaQW['boy_block'] = 1, 'girls', ...
      scene.text('"But we have also had sex with other \' + iif(katjaQW[\'boy_block\'] = 1, \'girls\', \'people\') + \'. How does that fit with us being in a relationship?" Katja asks.');
    } else {
    ...
    ((s as any).katjaQW ?? {})['sex_clossedness'] = 1;
    ((s as any).katjaQW ?? {})['faithful'] = 1;
    if (((s as any).katjaQW ?? 0)?.['liberated'] === 1) {
      ((s as any).katjaQW ?? {})['liberated'] = 0;
    }
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    qspCall(s, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    ...
    scene.text(`"Okay. ' + iif(katjaQW['QWstage'] >= 6 or katjaQW['anal_quest'] >= 3 or katjaQW['simultanous_girls'] > 1, 'I do like it when we have fun with somebody else and want that to continue.', 'I guess we might try that sometime.') + ' But no sex with anyone else if I'm not there," Katja replies. "I'm glad we settled that, ${((s as any).pcs_nickname ?? 0)}, but now it's time for bed," she says, getting up and giving you a quick kiss.`);
    scene.text('"Sleep tight, my girlfriend," you say as you leave leave her room.');
    scene.text('"Good night, my love," Katja replies.');
    scene.actions([
      { label: 'Go to the hallway', handler: (st: GameState) => {
    ...
    if (((s as any).katjaQW ?? 0)?.['liberated'] === 0) {
      ...
      // TODO-QSP: dynamic text: "' + iif(katjaQW['QWstage'] >= 6 or katjaQW['anal_quest'] >= 3 or katjaQW['simul...
      scene.text('"\' + iif(katjaQW[\'QWstage\'] >= 6 or katjaQW[\'anal_quest\'] >= 3 or katjaQW[\'simultanous_girls\'] > 1, \'I do love it when we have fun with somebody else.\', \'I guess we should try to have sex with somebody else together first.\') + \' You have given me some things to think about, but now I need to go to bed," Katja says, getting up and giving you a quick kiss.');
    } else {
...
function enterCocainOffer(s: GameState, scene: SceneBuilder): void {
  ((s as any).katjaQW ?? {})['coke_day_ask'] = ((s as any).daystart ?? 0);
  (s as any).minut = ((s as any).minut ?? 0) + 2;
  qspCall(s, 'stat', '');
  scene.img('images/characters/shared/headshots_main/big14.jpg');
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    scene.actions([
      { label: 'Convince Katja not to take the cocaine [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
    ]);
  } else {
    scene.actions([
      { label: 'Convince Katja that you should not do drugs right now [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 3;
    qspCall(s, 'npc_relationship', 'modify', 'A14', 'dislike');
    qspCall(s, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    ...
    // TODO-QSP: dynamic text: "No, I don't think it is. Taking drugs when you are just sitting around a normal...
    scene.text('"No, I don\'t think it is. Taking drugs when you are just sitting around a normal \' + iif(hour > 18, \'evening\', \'afternoon\') + \' isn\'t healthy" you respond.');
