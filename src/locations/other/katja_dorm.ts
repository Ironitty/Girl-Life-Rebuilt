...
function enterSofiaMeet(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('' + ((((s as any).university ?? 0)?.['enrolled_in'] === 'teaching_studies') ? ('"yes" ') : ('"I\'m ' + ((((s as any).university ?? 0)?.['student'] === 1) ? ('in the ' + qspFunc(s, 'uni_programs', 'get_program_name') + ' program') : ('not studying at the university')) + '"')) + 'you answer.');
...
function enterLazarKiss(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['horny'] = (((st as any).katjaQW = (st as any).katjaQW ?? {})['horny'] ?? 0) - (20);
  ...
  if (((s as any).katjaQW ?? 0)?.['movie_temp'] === 4) {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 2;
        ...
        if (((s as any).katjaQW ?? 0)?.['movie_temp'] === 1) {
          ...
          scene.text('You watch a romantic comedy while eating your popcorn. ' + ((((s as any).t ?? 0) > 3) ? ('The movie centers around some very good looking actors and includes some sex scenes, which have a clear effect on Katja.') : ('')));
          if (((s as any).npc_had_sex ?? 0)?.['A14']  &&  ((s as any).katjaQW ?? 0)?.['horny'] + Math.min(((s as any).katjaQW ?? 0)?.['slut'] / 3, 34) >= 100  &&  (Math.floor(Math.random() * 4) + 0) < 3  &&  ((s as any).katjaQW ?? 0)?.['sex_clossedness'] < 2) {
            scene.actions([
              { label: 'Finish the movie', goto: ['katja_dorm', 'after_movie_strip2'] },
            ]);
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 2;
          ...
          scene.text('You watch an action movie while eating your popcorn. ' + ((((s as any).t ?? 0) > 3) ? ('The movie includes a lot of men without there shirts on and women in very tight clothes, and it is clear that this is having an effect upon Katja.') : ('')));
...
function enterRelationshipTalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    scene.text('"Since you\'re now my girlfriend, I want to be with you and only you," you answer' + ((((st as any).katjaQW ?? 0)?.['QWstage'] === 6  ||  ((st as any).katjaQW ?? 0)?.['anal_quest'] === 3  ||  ((st as any).katjaQW ?? 0)?.['simultanous_girls'] > 1) ? ('. "We should stop having sex with others, both alone and together from now on," you continue.') : ('.')));
