...
function enterBreakFun_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You enter the girls restroom to find that nobody is inside. As you walk further in, you hear Albina. "In here, ${((s as any).pcs_nickname || '')}."`);
    ...
    scene.text(`She justs laughs at your question as she stands and walks up to you. "Come on ${((s as any).pcs_nickname || '')}, you're not that stupid. I was thinking about the times we've had some fun and how good your tongue felt on my pussy."`);
...
function enterLunchFun_1_2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Fuck Julia, you're an absolute pro! Only ${((s as any).pcs_nickname || '')} has come close to eating my pussy as good as you did!" She then sits up and looks down at the meek looking nerd, who's clearly aroused and in need of a release. "Get up here. It's your turn."`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
...
function enterLunchFun_2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`She pulls you up to your feet and you practically rip each other's clothes off and make out before she lies back on a desk and spreads her legs. "Get that magic tongue of yours over here and make me cum, ${((s as any).pcs_nickname || '')}!"`);
    ...
    scene.text(`"${((s as any).pcs_nickname || '')}!!!" she says while looking back and biting her lip. "You know I'd love it if you continued, but we need to get dressed before someone finds us."`);
    ...
    scene.text(`"Ooohh ${((s as any).pcs_nickname || '')}..." she moans softly. "While I'd love a round two, we should get dressed before someone finds us."`);
