...
function enterBreakFun_2(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!(Math.floor(Math.random() * 2) + 0))) {
    ...
    (st as any).orgasm_txt = 'Albina notices this and starts swirling her tongue over your engorged clit even faster, quickly bringing you to an explosive climax.';
...
function enterLunchFun_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Accept', handler: (st: GameState) => {
    scene.img('images/characters/shared/headshots_main/big23.jpg');
    if ((Math.floor(Math.random() * 3) + 0) === 0  &&  ((st as any).AlbinaQW ?? 0)?.['julia_sex'] === 0) {
...
function enterLunchFun_1_2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).orgasm_txt = 'A few minutes of this is enough to bring you to an explosive orgasm and you muffle your screams as you cum over Albina\'s face.';
    (st as any).orgasm_or = 'custom';
    qspCall(st, 'arousal', 'cuni', (-15), 'lesbian');
    qspCall(st, 'stat', '');
...
function enterLunchFun_2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'finger', (-5), 'lesbian', 'no_orgasm_msg');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
