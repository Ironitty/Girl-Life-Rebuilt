...
function enterFedorHelp(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Fedor gently rubs your back. "Please ${((s as any).pcs_nickname ?? 0)}, tell me what's wrong."`);
...
function enterFedorHelp2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`After turning the corner, Niko suddenly recoils as Fedor's fist makes contact, causing Niko to drop to the ground holding his face. "HOW DARE YOU TOUCH ${((s as any).pcs_nickname ?? 0)}! I'LL SHOW YOU WHAT HAPPENS WITH SHITHEADS LIKE YOU!" Fedor screams as he starts punching Niko, who tries to block the blows before Fedor is suddenly tackled.`);
    scene.actions([
    ...
    scene.text(`Fedor takes a few seconds to catch his breath before turning to you. "Are you okay ${((s as any).pcs_nickname ?? 0)}? You took quite a hit there."`);
...
function enterIvanHelp(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Ivan's face saddens as he wraps his arms around you. "What's wrong ${((s as any).pcs_nickname ?? 0)}? You can tell me."`);
  scene.actions([
    ...
    scene.text(`Ivan angrily slams his fist on one of the lockers. "That slimy little shit! This won't continue ${((s as any).pcs_nickname ?? 0)}, I'll put an end to it. You don't need to worry."`);
...
function enterNushHelp(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Anushka glares at him. "${((s as any).pcs_nickname ?? 0)} told us what you've been doing to her. It ends now."`);
...
function enterVitekHelp(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Vitek interrupts him. "Hold that thought. ${((s as any).pcs_nickname ?? 0)}, we're having guy talk right now. We can chat later."`);
  // TODO-QSP: dynamic text: You try to speak up, but Vasily intentionally interrupts you. "You need to cheer...
  scene.text(`You try to speak up, but Vasily intentionally interrupts you. "You need to cheer up ${((s as any).pcs_nickname ?? 0)}, you look so glum. Here, I'll tell you a joke."`);
  ...
  scene.actions([
    ...
    scene.text(`Vitek continuously kicks Niko in the ribs as Vasily joins him. "We'll show you what happens to those who fuck with ${((s as any).pcs_nickname ?? 0)}!!"`);
    ...
    scene.text(`Vitek gives your ass a light smack. "You know it ${((s as any).pcs_nickname ?? 0)}. Now get going, I have some business to finish here."`);
...
function enterVitekHelp2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You approach Vitek and he smiles. "Hey ${((s as any).pcs_nickname ?? 0)}, want a drink?"`);
  ...
  scene.actions([
    ...
    scene.text(`Niko immediately tries to run away, but Vasily drives his fist into Niko's face. "You don't fuck with ${((s as any).pcs_nickname ?? 0)} without dealing with us."`);
    ...
    scene.text(`Vasily approaches you. "You're safe now ${((s as any).pcs_firstname ?? 0)}. We'll make sure that Niko never touches you again."`);
