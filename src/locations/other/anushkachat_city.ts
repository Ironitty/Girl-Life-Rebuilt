...
function enterChat(s: GameState, scene: SceneBuilder): void {
  ...
  if ((Math.floor(Math.random() * 10) + 1) === 1  &&  ((s as any).anushkaQW ?? 0)?.['dreams'] < 3  &&  ((s as any).anushkaQW ?? 0)?.['dreams_daystart'] > ((s as any).daystart ?? 0) + 7) {
    ...
    if (((st as any).artemQW ?? 0)?.['rough_anal_nush'] === 2  ||  ((st as any).anushkaQW ?? 0)?.['artem_dom'] === 1) {
      ((st as any).artemQW = (st as any).artemQW ?? {})['nush3some'] = 3;
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).gopnikbandQW = (st as any).gopnikbandQW ?? {})['practice_invite_city'] = 1;
    scene.text('<center><b>Anushka\'s Room</b></center>');
    scene.img('images/characters/pavlovsk/school/girl/anushka/bedroom/bedroomtalk.jpg');
    ...
    scene.text('<center><b>Anushka\'s Room</b></center>');
    scene.img('images/characters/pavlovsk/school/girl/anushka/bedroom/bedroomtalk.jpg');
    if (((st as any).home ?? 0)?.['current'] === 'university_dorm') {
    ...
    if (((st as any).artemQW ?? 0)?.['bf'] === 1  ||  ((st as any).fedorKozlovQW ?? 0)  ||  ((st as any).NikoVolkovQW ?? 0) >= 5  ||  ((st as any).stat ?? 0)?.['boyfriends_current'] > 0) {
      scene.actions([
