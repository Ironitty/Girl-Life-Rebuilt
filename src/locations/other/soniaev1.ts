...
function enterEatsonia(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).orgasm_txt = 'You soon feel your body start to spasm as well as your own orgasm rocks your body. It feels like an eternity before it fades.';
    qspCall(s, 'arousal', 'cuni', (-2), 'lesbian');
    qspCall(s, 'arousal', 'cuni_give', 2, 'lesbian');
...
function enterStraponAnal(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Not so hard, ${((s as any).pcs_nickname || '')}!" she cries out as you start to really pound her ass.`);
    qspCall(s, 'arousal', 'anal_strap_give', 2, ((s as any).npcID1 ?? 0), 'lesbian');
    qspCall(s, 'stat', '');
    qspCall(s, 'willpower', 'anal', 'force');
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
...
function enterStraponEaten(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).orgasm_txt = 'As she starts to orgasm, it\'s too much for you. You feel your own body shake and rock as an orgasm courses through it.';
    qspCall(s, 'arousal', 'cuni', (-2), 'lesbian');
    qspCall(s, 'arousal', 'cuni_give', 2, 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.text(`Once you're both dressed and cleaned up, she walks you to her front door. "Goodnight, ${((s as any).pcs_nickname || '')}."`);
...
function enterStraponMagicBedroomAnal(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"OH FUCK! Harder, ${((s as any).pcs_nickname || '')}! Fuck my ass harder!" she cries out as you start to really pound her ass.`);
    qspCall(s, 'arousal', 'magic_anal', 2, ((s as any).npcID1 ?? 0), 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
