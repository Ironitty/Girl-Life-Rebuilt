...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You arrive at the cafe and sit at a table. ${((s as any).npcdesc ?? 0)} orders food and wine. While you wait for the food to arrive, ${((s as any).npcdesc ?? 0)} tells you an anecdote:<br>`);
  qspCall(s, 'anekdot', '');
...
function enterB(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Music begins to play in the cafe and ${((s as any).npcdesc ?? 0)} asks if you would like to dance.`);
...
function enterDance(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You go with ${((s as any).npcdesc ?? 0)} to the dance floor. He puts his arms around you and you start to dance.`);
  if (((s as any).npc_gentle ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
    // TODO-QSP: dynamic text: <<$npcdesc>> is very reserved in his dance. He holds you a bit awkwardly and apo...
    scene.text(`${((s as any).npcdesc ?? 0)} is very reserved in his dance. He holds you a bit awkwardly and apologizes.`);
    scene.actions([
...
function enterSex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You pull his dick out of his pants and start masturbating him. ${((s as any).npcdesc ?? 0)} puts on a condom, then makes you stand over the toilet and pushes his ${((s as any).dick ?? 0)} centimeter member in your quivering pussy.`);
...
function enterBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You pull his dick out of his pants and start masturbating him, then squat in front of him and take him in your mouth, slowly sucking his ${((s as any).dick ?? 0)} centimeter member.`);
  // TODO-QSP: dynamic text: Soon <<$npcdesc>> can not resist any longer and finishes in your mouth, filling ...
  scene.text(`Soon ${((s as any).npcdesc ?? 0)} can not resist any longer and finishes in your mouth, filling it with his tart semen. You try to swallow it all, but cum still pours out of your mouth, running down your lips and chin.`);
