...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 10, 'sub', 'lesbian', 'rough');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Refuse to talk', handler: (st: GameState) => {
    ...
    qspCall(st, 'arousal', 'vaginal_strap_give', 10, 'dom', 'lesbian', 'rough');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Wait', handler: (st: GameState) => {
    (st as any).gnewQW = 3;
    (st as any).hour = ((st as any).hour ?? 0) + (20);
    scene.img('images/locations/city/industrial/police/c1.jpg');
