...
function enterPark(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).flag_KissEV ?? 0) === 3) {
    ...
    if (((st as any).month ?? 0)===1  ||  ((st as any).month ?? 0)===2  ||  ((st as any).month ?? 0)===12  ||  ((st as any).month ?? 0)===11) {
...
function enterProhorovSex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    if (((st as any).katjaQW ?? 0)?.['horny'] >= 50  &&  ((st as any).katjaQW ?? 0)?.['slut'] >= 50) {
      scene.actions([
    ...
    qspCall(st, 'stat', '');
    if (((st as any).katjaQW ?? 0)?.['horny'] >= 60  &&  ((st as any).katjaQW ?? 0)?.['slut'] >= 60) {
      scene.actions([
    ...
    qspCall(st, 'arousal', 'anal_finger', 2);
    qspCall(st, 'stat', '');
    ...
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Tell Ivan to just watch', handler: (st: GameState) => {
...
function enterMarcusWait(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).npc_vag ?? 0)?.['A14'] + 5 < ((st as any).npc_dick ?? 0)?.['A146']) {
      qspGoto(st, 'katja_sex', 'marcus_first');
    } else {
      qspGoto(st, 'katja_sex', 'marcus_threesome');
    }
  } },
    ]);
  } },
...
function enterMarcusFirst(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal_finger_give', 3, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    ...
    qspCall(st, 'arousal', 'cuni_give', 3, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'voyeur_sex', (-3), 'group');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', 3, 'lesbian', 'inhibition');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', 3, ((st as any).npcID ?? 0), 'group');
    qspCall(st, 'arousal', 'kiss', (-3), ((st as any).npcID1 ?? 0), 'lesbian', 'group', 'inhibition');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'hj', 1, ((st as any).npcID ?? 0), 'group');
    qspCall(st, 'arousal', 'end');
...
function enterMarcusThreesome(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal_finger_give', 3, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'voyeur_sex', (-3), ((st as any).npcID ?? 0), 'group');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterKatnatsex1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['simultanous_girls'] < 2) {
    ...
    ((s as any).katjaQW = (s as any).katjaQW ?? {})['simultanous_girls'] = 2;
  } else {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'cuni_give', 10, ((st as any).npcID ?? 0), 'group');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'stat', '');
    qspCall(st, 'willpower', 'cuni', 'force', 'hard');
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
    ...
    scene.text('"Oh! Katja! I mean, oh! Fuck! Natasha I\'m cumming!"');
    qspCall(st, 'arousal', 'cuni', 10, ((st as any).npcID1 ?? 0), 'lesbian', 'group');
    qspCall(st, 'arousal', 'kiss', (-5), ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'kiss', 10, ((st as any).npcID ?? 0), 'lesbian', 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Cuddles', goto: ['katja_sex', 'katnatsex2'] },
    ]);
    ...
    qspCall(st, 'arousal', 'cuni_give', 10, ((st as any).npcID ?? 0), 'group');
    qspCall(st, 'stat', '');
    qspCall(st, 'willpower', 'kiss', 'self', 'hard');
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
...
function enterFirstFisting(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal_fist_give', 5, 'lesbian');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'vaginal_fist_give', 2, 'lesbian');
    qspCall(st, 'stat', '');
    ...
    if (((st as any).npc_vag ?? 0)?.['A14'] < 33) {
      ((st as any).npc_vag = (st as any).npc_vag ?? {})['A14'] = ((st as any).npc_vag['A14'] ?? 0) + (3);
    } else {
      ((st as any).npc_vag = (st as any).npc_vag ?? {})['A14'] = 36;
    }
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['horny'] = 0;
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['slut'] = ((st as any).katjaQW['slut'] ?? 0) + (5);
    if (((st as any).katjaQW ?? 0)?.['fisted_vag'] === 0) {
      ((st as any).katjaQW = (st as any).katjaQW ?? {})['fisted_vag'] = 1;
    }
    qspCall(st, 'stat', '');
