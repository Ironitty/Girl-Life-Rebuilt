...
function enterMarcusWait(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).npc_vag ?? 0)?.['A14'] + 5 < ((s as any).npc_dick ?? 0)?.['A146']) {
      qspGoto(s, 'katja_sex', 'marcus_first');
    } else {
      qspGoto(s, 'katja_sex', 'marcus_threesome');
    }
  } },
    ]);
  } },
...
function enterKatnatsex1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Hm? Don't worry ${((s as any).pcs_nickname || '')}, Natasha is in good hands."`);
    ...
    scene.text(`"Ouch! ${((s as any).pcs_nickname || '')}!"`);
    ...
    scene.text(`"Shh. Don't worry Natasha, you are too nervous. Let ${((s as any).pcs_nickname || '')} and I take care of that, ok?"`);
    ...
    scene.text(`"Shh… Don't worry ${((s as any).pcs_nickname || '')}, Natasha and I know what to do."`);
    ...
    scene.text(`"Hmpf… ${((s as any).pcs_nickname || '')}… Muach… Is Katja's… Lick…"`);
...
function enterFirstFisting(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).katjaQW ?? 0)?.['fisted_vag'] === 0) {
      ((s as any).katjaQW = (s as any).katjaQW ?? {})['fisted_vag'] = 1;
    }
    qspCall(s, 'stat', '');
