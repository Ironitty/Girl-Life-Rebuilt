...
function enterStation(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).DjibrilQW ?? 0)?.['taxi'] === 1) {
    ...
    if ((!((st as any).first_taxi ?? 0))) {
      (st as any).first_taxi = 1;
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A55', 1);
    qspCall(st, 'npc_relationship', 'modify', 'A82', 1);
    qspCall(st, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big55.jpg');
    if (((st as any).OluQW ?? 0)?.['sex'] === 1) {
...
function enterDjibrilTaxiSexAnal1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'anal', 2, 'rough');
    qspCall(st, 'cum_call', 'butt', 'A82');
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 3;
