...
function enterArt_102Model(s: GameState, scene: SceneBuilder): void {
  scene.actions([{ label: 'Continue', goto: ['uni_lessons_electives_art1', 'art_102_model'] }]);
  scene.build();
}

function enterArt_102ModelIgor(s: GameState, scene: SceneBuilder): void {
  scene.actions([{ label: 'Continue', goto: ['uni_lessons_electives_art1', 'art_102_model_igor'] }]);
  scene.build();
}

function enterArt_102ModelFeofan(s: GameState, scene: SceneBuilder): void {
  scene.actions([{ label: 'Continue', goto: ['uni_lessons_electives_art1', 'art_102_model_feofan'] }]);
  scene.build();
}

function enterArt_102ModelNatasha(s: GameState, scene: SceneBuilder): void {
  scene.actions([{ label: 'Continue', goto: ['uni_lessons_electives_art1', 'art_102_model_natasha'] }]);
  scene.build();
}

function enterArt_102ModelErmias(s: GameState, scene: SceneBuilder): void {
  scene.actions([{ label: 'Continue', goto: ['uni_lessons_electives_art1', 'art_102_model_ermias'] }]);
  // TODO-QSP: end
  scene.build();
}
