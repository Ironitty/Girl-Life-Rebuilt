...
function enterGetRandomChoreAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).daystart ?? 0) !== ((s as any).grandmaQW ?? 0)?.['last_day_helped']) {
    ...
    scene.text(`"There is nothing to do today, ${((s as any).pcs_nickname ?? 0)}. Take the day off.`);
    scene.actions([
      { label: 'Continue', goto: ['gp_elene', 'talk'] },
    ]);
  } },
...
function enterSetReportChoresActs(s: GameState, scene: SceneBuilder): void {
  ...
  qspCall(s, 'gp_elene', 'set_report_cleanfloor_act');
  qspCall(s, 'gp_elene', 'set_report_washclothes_act');
  qspCall(s, 'gp_elene', 'set_report_milkcow_act');
  qspCall(s, 'gp_elene', 'set_report_groceries_act');
  qspCall(s, 'gp_elene', 'set_report_feedchickens_act');
  qspCall(s, 'gp_elene', 'set_report_checkplants_act');
  qspCall(s, 'gp_elene', 'set_report_waterplants_act');
  qspCall(s, 'gp_elene', 'set_report_collectstrawberries_act');
  qspCall(s, 'gp_elene', 'set_report_collectfruit_act');
  qspCall(s, 'gp_elene', 'set_report_harvestgarden_act');
  qspCall(s, 'gp_elene', 'set_report_gathermushrooms_act');
  qspCall(s, 'gp_elene', 'set_report_gatherberries_act');
  qspCall(s, 'gp_elene', 'set_report_gatherboth_act');
  scene.build();
}
...
function enterSetReportWashclothesAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_wash_clothes'] === 2) {
    ...
    scene.text(`"Thank you, ${((s as any).pcs_nickname ?? 0)}. I have no idea how that man gets so dirty!"`);
...
function enterSetReportMilkcowAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_milk_cow'] === 2) {
    ...
    scene.text(`"Thank you, ${((s as any).pcs_nickname ?? 0)}. She's such a sweet thing, isn't she?" your grandmother asks fondly.`);
...
function enterSetReportGroceriesAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_groceries'] === 2) {
    ...
    scene.text(`"Thank you, ${((s as any).pcs_nickname ?? 0)}. He's a nice young man. I've known him since he was a child, you know."`);
    ...
    scene.text(`"No, that is all, ${((s as any).pcs_nickname ?? 0)}," she says and gives you a gentle pat on the back.`);
    scene.actions([
      { label: 'Continue', goto: ['gp_elene', 'talk'] },
    ]);
  } },
...
function enterSetHelpCanMushroomsAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_can_mushrooms'] === 1) {
    ...
    scene.text(`"Thank you, ${((s as any).pcs_nickname ?? 0)}," she says. "It is important to save food up for the hard winter months. Sure, now you can walk to the store and get whatever you want, but it wasn't always that way!"`);
    scene.actions([
...
function enterSetHelpCanVeggiesAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_can_veggies'] === 1) {
    ...
    scene.text(`"You know, ${((s as any).pcs_nickname ?? 0)}, if you know how to can your own food, you can survive years of poor harvests," she says.`);
...
function enterSetReportFeedchickensAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_feed_chickens'] === 2) {
    ...
    scene.text(`"Good, thank you, ${((s as any).pcs_nickname ?? 0)}," she says. "A fat chicken is a happy chicken, and happy chickens lay the best eggs."`);
...
function enterSetReportCheckplantsAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_work_in_garden'] === 2) {
    ...
    scene.text(`"Good, thank you, ${((s as any).pcs_nickname ?? 0)}," she says. "Did you pull out all the weeds?"`);
    ...
    scene.text(`"Good, good," she nods. "It is crucial to take care of your garden, ${((s as any).pcs_nickname ?? 0)}. It is how we provide for ourselves when no one else will."`);
    ...
    scene.text(`"Anyway," she continues, "thank you for doing that for me, ${((s as any).pcs_nickname ?? 0)}. My old bones complain when I do it."`);
...
function enterSetReportWaterplantsAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_water_garden'] === 2) {
    ...
    scene.text(`"Thank you, ${((s as any).pcs_nickname ?? 0)}," she says. "A healthy plant is always thirsty. So you should have some water, too, if you want to be a healthy girl."`);
...
function enterSetReportCollectstrawberriesAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_collect_strawberries'] === 2) {
    ...
    scene.text(`"Oh, ${((s as any).pcs_nickname ?? 0)}," she exclaims, "don't these just look wonderful? They are plump and juicy this year."`);
...
function enterSetReportCollectfruitAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_collect_fruit'] === 2) {
    ...
    scene.text(`"Thank you, ${((s as any).pcs_nickname ?? 0)}," she says. "I cannot reach or bend like I used to, so you have done me a great favor."`);
...
function enterSetReportHarvestgardenAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_harvest_garden'] === 2) {
    ...
    scene.text(`"Oh, don't worry yourself, little ${((s as any).pcs_nickname ?? 0)}," she says, cupping your cheek with her hand. "Forgive an old woman of her thoughtful ways. We are lucky to have such a kind granddaughter to help us bring in the harvest."`);
...
function enterSetReportGatherberriesAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_gather_berries'] === 2  &&  ((s as any).bilberry ?? 0) >= ((s as any).grandmaQW ?? 0)?.['chore_berry_quantity']  &&  ((s as any).hour ?? 0) >= 6  &&  ((s as any).hour ?? 0) < 20) {
    ...
    scene.text(`"Oh, thank you, ${((s as any).pcs_nickname ?? 0)}," Grandma replied. "Those look fine if a little late. They will have to go down in the root cellar until we are ready to can them."`);
    ...
    scene.text(`"Oh, thank you, ${((s as any).pcs_nickname ?? 0)}," Grandma replied. "Those look perfect. They will have to go down in the root cellar until we are ready to can them."`);
...
function enterSetReportGatherbothAct(s: GameState, scene: SceneBuilder): void {
  if (((s as any).grandmaQW ?? 0)?.['chore_gather_both'] === 2  &&  ((s as any).boletus ?? 0) >= ((s as any).grandmaQW ?? 0)?.['chore_mushroom_quantity']  &&  ((s as any).bilberry ?? 0) >= ((s as any).grandmaQW ?? 0)?.['chore_berry_quantity']  &&  ((s as any).hour ?? 0) >= 6  &&  ((s as any).hour ?? 0) < 20) {
    ...
    scene.text(`"Oh, thank you, ${((s as any).pcs_nickname ?? 0)}. I was starting to think you had forgotten about me. They will have to go down in the root cellar until we are ready to can them," your Grandma replied.`);
    ...
    scene.text(`"Oh, thank you very much, ${((s as any).pcs_nickname ?? 0)}. They will have to go down in the root cellar until we are ready to can them," your Grandma replied.`);
    scene.text('"Do I need to go out on another run, or do you think that\'s enough?"');
    scene.text('"Well, little one," your Grandma says, "There is a difference between enough and enough for today."');
    scene.text('"Oh…" you say, unsure what to make of her response.');
    scene.text('She smiles gently and pats your head. "You\'ve done enough for today, child."');
