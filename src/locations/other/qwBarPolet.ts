...
function enterOrdering(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).cumloc ?? 0)[7] > 0  ||  ((s as any).cumloc ?? 0)[10] > 0  ||  ((s as any).cumloc ?? 0)[11] > 0  ||  ((s as any).cumloc ?? 0)[13] > 0  ||  ((s as any).cumloc ?? 0)[16] > 0) {
    ...
    if (((s as any).alko ?? 0) >= 6) {
      ...
      scene.text(`"You can barely stand-up straight, ' + iif(knowMartin=1, '${((s as any).pcs_firstname ?? 0)}', 'girl') + '. And I sure as hell don't want a young woman like yourself to pass out in my bar. Go home, or at least sober up a bit."`);
      scene.actions([
        { label: 'Can I have some water? [+$func(\'money\', \'get_cost_string\', 35)]', handler: (st: GameState) => {
    ...
    if (((s as any).DrinkingRand ?? 0) === 2) {
      ...
      if (((s as any).DrinkingRand ?? 0) === 3) {
        ...
        if (((s as any).DrinkingRand ?? 0) === 4) {
          ...
          scene.img(`images/locations/city/industrial/bar/bartalk${((s as any).pic_rand ?? 0)}.jpg`);
