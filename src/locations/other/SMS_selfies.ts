...
function enterSelfieMenu(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).SMSSelfieVars['text'] = ((s as any).SMSSelfieVars['text'] ?? 0) + ('<a href="exec: gs \'SMS_selfies\', \'exit\', ' + qspUntranslated(s, "ARGS[2]>", { location: "SMS_selfies" }) + '">Exit</a></td></tr></table>');
  qspCall(s, 'SMS_selfies', 'show_sms');
  scene.build();
}

function enterShowlocation(s: GameState, scene: SceneBuilder): void {
  (s as any).SMSSelfieVars['text'] = '<table width=80%><tr><th><font color="maroon">' + qspUntranslated(s, "selfieLocDesc[ARGS[1]]>", { location: "SMS_selfies" }) + ' selfies</font></th></tr>';
