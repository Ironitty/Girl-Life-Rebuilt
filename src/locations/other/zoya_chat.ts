...
function enterBreakfastChat(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Good morning, ${((s as any).pcs_firstname ?? 0)}. I hope you slept well?"`);
...
function enterLoungeChat(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Good evening, ${((s as any).pcs_firstname ?? 0)}. Please take a seat."`);
...
function enterBreakfastChat1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"The pleaure was mine, ${((s as any).pcs_firstname ?? 0)}," she smiles. "Feel free to stop and chat whenever you wish."`);
    ...
    scene.text(`She smiles at your compliment. "Thank you ${((s as any).pcs_firstname ?? 0)}, but sadly having a trophy wife wasn't good enough for him. He would regularly seduce other young women for sex and even hire escorts, and he was just as abusive to them as he was towards to me."`);
    ...
    scene.text(`She smiles at your compliment, and you think she even blushes a little. "You're too kind ${((s as any).pcs_firstname ?? 0)}, but I can assure you that I'm well past my modelling years. I'll be 40 before I know it!"`);
    ...
    scene.text(`"Oh. Oh, I'm sorry for putting you on the spot like that, ${((s as any).pcs_firstname ?? 0)}!" she replies. "My advice in that case would be to make sure you're using both condoms <i>and</i> birth control once you're active."`);
    scene.actions([
      { label: 'Talk about something else', goto: ['zoya_chat', 'breakfast_chat1'] },
      { label: 'Leave', goto: ['albinahome', 'kitchen'] },
    ]);
    ...
    scene.text(`"The pull-" she gasps. "${((s as any).pcs_firstname ?? 0)}! He doesn't have to 'finish' inside you for his sperm to get into your vagina! Not to mention the risk of catching an STI!"`);
    ...
    scene.text(`She smiles. "I appreciate that, ${((s as any).pcs_firstname ?? 0)}, but I don't want her to feel like she's being manipulated, so don't overstep your bounds as her friend. ' + iif(npc_had_sex['A23'], 'Or whatever your relationship with her might be.', ') + '"`);
    if (((s as any).npc_had_sex ?? 0)?.['A23']) {
      scene.actions([
    ...
    scene.text(`"I'm still a bit young for that, ${((s as any).pcs_firstname ?? 0)}!" she laughs. "Yes, I would, but like I said, I want Albina to live her life first and get a good job before she has children. I don't want her being tied down with a child at such a young age."`);
