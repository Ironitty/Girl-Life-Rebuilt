...
function enterStarlets(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).AlbinaQW ?? 0)?.['ShowTriggerCount'] >= 6) {
    ...
    if (((s as any).AlbinaQW ?? 0)?.['StarletsCustomerCount'] === 1) {
      ...
      scene.text(`Albina avoids eye contact. "Nothing, it's just… If you go with this person and be… 'affectionate'," she says hesitantly, "you'll get ${qspFunc(s, 'money', 'string_profit', ((s as any).AlbinaQW ?? {})?.['StarletPay']*1000, 1)}."`);
      ...
      scene.text(`Albina avoids eye contact. "Nothing, it's just… See those guys?" she asks hesitantly. "If you go with them and be…'affectionate', you'll get ${qspFunc(s, 'money', 'string_profit', ((s as any).AlbinaQW ?? {})?.['StarletPay']*1000, 1)}."`);
      // TODO-QSP: dynamic text: She looks you in the eye, the fear within her apparently growing. "Please <<$pcs...
      scene.text(`She looks you in the eye, the fear within her apparently growing. "Please ${((s as any).pcs_nickname ?? 0)}, I need you to do this for me. Just… please."`);
    }
...
function enterStarletsEnd(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You enter the locker room and immediately see the disapproving looks on the girls faces. Albina walks over to you and, looking severely disappointed, shakes her head. "Why ${((s as any).pcs_nickname ?? 0)}? You used to be cool, but now you've gone and done this to yourself. I'm sorry, but I just can't have someone like you being associated with us anymore."`);
...
function enterStarletsPracticeMissed(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).starlet_practice_skipped ?? 0) === 1) {
    ...
    scene.text(`You walk in and see an annoyed Albina waiting for you, her arms crossed as she glares at you. "Where the fuck were you ${((s as any).pcs_nickname ?? 0)}?! And don't fucking lie to me!"`);
...
function enterQuit(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You enter the hall and look around for Albina, spotting her as she's doing some stretching exercises. You walk over to her and she gives you a puzzled stare. "Hurry up and get dressed ${((s as any).pcs_nickname ?? 0)}, we haven't got all day to wait for you!" she barks. You meekly smile at her in response. "Well? Hurry up and spit it out!" she says while motioning for you to hurry up.`);
