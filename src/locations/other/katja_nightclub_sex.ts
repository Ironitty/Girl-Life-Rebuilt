...
function enterPrivateRoom2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_hotcat ?? 0) + ((s as any).locArgs?.[1] ?? 0) < (Math.floor(Math.random() * 9) + 6)) {
    ...
    scene.actions([
      { label: '', labelFn: (s: GameState) => 'Have sex with ' + String(((s as any).npcdesc || '') ?? ''), handler: (st: GameState) => {
...
function enterPrivateBooth_FFM(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).katjaQW ?? 0)?.['horny'] >= 100) {
      ((s as any).katjaQW = (s as any).katjaQW ?? {})['horny'] = ((s as any).katjaQW['horny'] ?? 0) - (100);
      if (((s as any).npc_had_sex ?? 0)?.['A14'] === 0  &&  ((s as any).katjaQW ?? 0)?.['first_time_in_nightclub'] < 2) {
        ((s as any).katjaQW = (s as any).katjaQW ?? {})['first_time_in_nightclub'] = 2;
        qspCall(s, 'npc_relationship', 'modify', 'A14', 'adore');
      }
    ...
    qspCall(s, 'stat', '');
    scene.actions([
      { label: '', labelFn: (s: GameState) => 'Suck ' + String(((s as any).npcdesc || '') ?? '') + '\'s cock', handler: (st: GameState) => {
    ...
    qspCall(s, 'stat', '');
    scene.actions([
      { label: '', labelFn: (s: GameState) => 'Get on top of ' + String(((s as any).npcdesc || '') ?? ''), handler: (st: GameState) => {
...
function enterLoneBartender(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).loc_arg = 'lonebartender_drinking';
    ...
    qspGoto(s, 'katja_nightclub_sex', 'bartender_bj');
  } },
      ]);
    } else {
...
function enterBartenderBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).npc_sex_stamina ?? 0)?.[String((s as any).npclastgenerated ?? 0)] <= (Math.floor(Math.random() * (Math.min(((s as any).stat ?? 0)?.['bj'] / 25 + 3, 10) - Math.min(((s as any).stat ?? 0)?.['bj'] / 100, 5) + 1)) + (Math.min(((s as any).stat ?? 0)?.['bj'] / 100, 5)))) {
      ...
      qspGoto(s, 'katja_nightclub_sex', 'bartender_bj_katja');
    }
  } },
...
function enterBartenderSex1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 2;
    ((s as any).katjaQW = (s as any).katjaQW ?? {})['slut'] = ((s as any).katjaQW['slut'] ?? 0) + (Math.floor(Math.random() * 4) + 0);
    ((s as any).katjaQW = (s as any).katjaQW ?? {})['horny'] = ((s as any).katjaQW['horny'] ?? 0) + (Math.floor(Math.random() * 16) + 5);
    qspCall(s, 'arousal', 'cuni', 3, ((s as any).npcID1 ?? 0), 'group', 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    (s as any).loc = 'katja_nightclub_sex';
    (s as any).loc_arg = 'lonebartender_drinking2';
    qspCall(s, 'food_menu', '');
  } },
    ]);
  } },
...
function enterCheatingConversation(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['cheat_warning'] === 0) {
    ...
    if (((s as any).katjaQW ?? 0)?.['cheat_times'] > 0) {
      ((s as any).katjaQW = (s as any).katjaQW ?? {})['cheat_times'] = ((s as any).katjaQW['cheat_times'] ?? 0) - (Math.floor(Math.random() * 2) + 0);
    }
    qspCall(s, 'willpower', 'pay', 'force');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    if (((s as any).locArgs?.[1] ?? 0) === 'bartender') {
...
function enterPrivateBooth_FFFF(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'stat', '');
    scene.actions([
      { label: '', labelFn: (s: GameState) => 'See what ' + String(((s as any).npcdesc1 || '') ?? '') + ' does', handler: (st: GameState) => {
    ...
    ((s as any).katjaQW = (s as any).katjaQW ?? {})['horny'] = 0;
    ((s as any).katjaQW = (s as any).katjaQW ?? {})['slut'] = ((s as any).katjaQW['slut'] ?? 0) + (4);
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Your turn', goto: ['katja_nightclub_sex', 'private_booth_FFFF_you'] },
    ]);
...
function enterPrivateBooth_FFFFYou(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'stat', '');
    scene.actions([
      { label: '', labelFn: (s: GameState) => String(((s as any).npcdesc2 || '') ?? '') + '\'s turn', goto: ['katja_nightclub_sex', 'private_booth_FFFF_girls', 'dp'] },
    ]);
  } },
