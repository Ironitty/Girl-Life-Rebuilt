...
function enterVirginAsk(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).virgin_stats ?? 0)?.['taker_ID'] === 'A16') {
    ...
    scene.text(`Anya clearly didn't expect your outburst. "Sorry ${((s as any).pcs_nickname || '')}, but sex with a girl doesn't count. You'll meet the right guy one day…"`);
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterFirstTime(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enterFirstTime(s: GameState, scene: SceneBuilder): void {
  if (((s as any).stat ?? 0)?.['vaginal'] >= 1) {
    ...
    scene.text(`"Don't worry ${((s as any).pcs_nickname || '')}. They all said that it gets a lot better after the first time. You just need a little more pussy practice."`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterSexTalkContinue(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enterBlackSex(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).DjibrilQW ?? 0)?.['anya_gangbang_count'] !== 0) {
    ...
    ((s as any).OluQW = (s as any).OluQW ?? {})['anya_inv'] = 1;
    scene.img('images/characters/pavlovsk/resident/anya/home/sister_chat.jpg');
    ...
    ((s as any).DjibrilQW = (s as any).DjibrilQW ?? {})['anya_inv'] = 1;
    scene.img('images/characters/pavlovsk/resident/anya/home/sister_chat.jpg');
    ...
    scene.actions([
      { label: 'Ask if she wants to join you', handler: (st: GameState) => {
    ((s as any).DjibrilQW = (s as any).DjibrilQW ?? {})['anya_inv'] = 2;
    scene.img('images/characters/pavlovsk/resident/anya/home/sister_chat.jpg');
    scene.text('"Would you like to come up with me next time I go?" you ask.');
    scene.text('She thinks about it for a moment, then grins and nods. "Yes, I think I would. You know, just to try it…" she says while blushing.');
