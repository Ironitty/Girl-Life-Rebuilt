...
function enterPhoneIntro(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('"Hello?"');
  // TODO-QSP: dynamic text: "<<$pcs_firstname>>, it''s Tatiana," she greets you. "I need you to come by the ...
  scene.text(`"${((s as any).pcs_firstname || '')}, it's Tatiana," she greets you. "I need you to come by the lab. How soon can you be here?"`);
...
function enterPhoneMission(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Hey ${((s as any).pcs_firstname || '')}," the young mage greets you. "Could you come by the lab soon?"`);
...
function enterIntro(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"What did you expect, dragons?" Tatiana laughs. "The magic is all around you, ${((st as any).pcs_firstname || '')}. Most of the books in this part of the library are books on magic, ranging from magic theory and philosophical works to spellbooks on geomancy and shamanism. It's one of the biggest collections of its kind in this country, I've been told."`);
...
function enterMission1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).hour ?? 0) < 9  ||  ((st as any).hour ?? 0) > 19) {
    ...
    // TODO-QSP: dynamic text: "Don''t worry, we''ll definitely return," Tatiana promises. "Won''t we, <<$pcs_f...
    scene.text(`"Don't worry, we'll definitely return," Tatiana promises. "Won't we, ${((st as any).pcs_firstname || '')}?"`);
...
function enterMission2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`As you approach Tatiana to find out exactly what your new, "exciting work" involves, she flashes you a Cheshire smile and raises her smartphone. "Okay, ${((s as any).pcs_nickname || '')}. Do you recognize her?"`);
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: "Yes, <<$pcs_nickname>>, <i>HIC SUNT DRACONES!</i> Not Salvatore''s sissyfied on...
    scene.text(`"Yes, ${((st as any).pcs_nickname || '')}, <i>HIC SUNT DRACONES!</i> Not Salvatore's sissyfied ones, I mean Tolkien's kingdom-destroying Smaug version! I will tell you about them some other day." She gives you another cheeky smile and continues.`);
    ...
    if (((st as any).pcs_splcstng ?? 0) >=50) {
      // TODO-QSP: dynamic text: Tatiana grabs some papers and passes them to you. "Read this, <<$pcs_nickname>>!...
      scene.text(`Tatiana grabs some papers and passes them to you. "Read this, ${((st as any).pcs_nickname || '')}!"`);
      ...
      scene.text(`"Yes, ${((st as any).pcs_nickname || '')}, they are transforming into scatter-brained bimbos! The interesting part is that they are copying some of my mannerisms and views. Following this lead, I consulted some curse experts. In summary, they think that there must be a personal item of mine at its center, and that it's what made the curse repel me."`);
    } else {
      // TODO-QSP: dynamic text: Tatiana grabs some papers and passes them to you. "Read this, <<$pcs_nickname>>!...
      scene.text(`Tatiana grabs some papers and passes them to you. "Read this, ${((st as any).pcs_nickname || '')}!"`);
      ...
      scene.text(`"Well, ${((st as any).pcs_nickname || '')}, the magical anomaly seems centered around a personal item of mine, and it's the reason for the repel effect."`);
    }
    scene.actions([
    ...
    if (((st as any).pfilmNO ?? 0) === 1) {
      ...
      // TODO-QSP: dynamic text: The sorceress shrugs. "Don''t worry, <<$pcs_nickname>>, I have Gustav for those ...
      scene.text(`The sorceress shrugs. "Don't worry, ${((st as any).pcs_nickname || '')}, I have Gustav for those sorts of jobs." I know a way to help you infiltrate the place so you can look for the item in question. When the moment feels right, go to Aphrodite Model Agency and give me a call, but remember, only when the building is open. The curse seems to go dormant when there are no people around. Don't worry, I will pay you for your time."`);
    } else {
      if (((st as any).job_status ?? 0)?.['city_aphrodite_model'] === 'employed') {
        ...
        if (((st as any).pcs_hotcat ?? 0) <= 5) {
          ...
          scene.text(`The sorceress looks you over and smiles. "Nothing that dramatic, ${((st as any).pcs_nickname || '')}. Just look for work at the agency. Once inside, you need to look for the item. Talk to the models, eavesdrop, walk around and look for something wrong. It will be good training, and when you finally find whatever is at its center, bring it to me so that I can end the curse. Don't worry, I will pay you for your time."`);
        }
      }
    }
    scene.text('Saying this, Tatiana turns towards her studies, leaving you to your own devices.');
...
function enterMission2A(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
    if (((st as any).arch_vars ?? 0)?.['main_active'] === 'bimbo') {
      ...
      if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
        scene.actions([
          { label: 'Kick Vintage Tatiana\'s ass!', handler: (st: GameState) => {
    ...
    scene.actions([
      ...
      if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
        scene.actions([
          { label: 'Talk to Vintage Tatiana', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
        ]);
      } else {
    ...
    qspCall(st, 'willpower', 'pay', 'self');
    qspCall(st, 'stat', '');
    qspGoto(st, 'tatiana_missions', 'mission2C');
  } },
        ]);
      }
      scene.actions([
    ...
    scene.text(`"Are you ready for your massage ${((st as any).pcs_nickname || '')}?"`);
    ...
    // TODO-QSP: dynamic text: "Sorry, <<$pcs_nickname>>! It''s just that, like, you don''t have much in your h...
    scene.text(`"Sorry, ${((st as any).pcs_nickname || '')}! It's just that, like, you don't have much in your head."`);
    ...
    (st as any).pcs_sleep = ((st as any).pcs_sleep ?? 0) + (25);
    ...
    qspCall(st, 'npc_relationship', 'modify_exact', 'A241', (-1));
    (st as any).AlexandriaQW = 5;
    qspCall(st, 'stat', '');
    scene.actions([
      ...
      scene.text(`"Hmm… I think it's better if we talk somewhere else, ${((st as any).pcs_nickname || '')}."`);
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    qspCall(st, 'stat', '');
    if (((st as any).succubusQW ?? 0) !== 5) {
      qspGoto(st, 'tatiana_missions', 'mission2sex');
    } else {
      scene.img('images/characters/city/btatiana/ST1.jpg');
      scene.text('You find yourself lying face down, wearing only panties in what appears to be a photography set. \'Tatiana\' kneels by your side in a somewhat diaphanous robe.');
      // TODO-QSP: dynamic text: "Are you ready for your massage, <<$pcs_nickname>>?"
      ...
      scene.text(`You call ${((st as any).sucself1 || '')} up from its resting place.`);
      ...
      if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
        scene.actions([
          { label: 'Make her submit', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
        ]);
      } else {
    ...
    qspCall(st, 'npc_relationship', 'modify_exact', 'A241', 2);
    qspCall(st, 'stat', '');
    qspGoto(st, 'tatiana_missions', 'mission2E');
  } },
        ]);
      }
      scene.actions([
        { label: 'Kick her ass!', handler: (st: GameState) => {
    (st as any).boydesc = 'Sword-wielding bitch';
    scene.img('images/characters/city/btatiana/ST1.jpg');
    qspCall(st, 'fight', 'initFight');
    qspCall(st, 'fight_npcdata', 'btatiana');
...
function enterMission2B(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Come on, ${((s as any).pcs_nickname || '')}, this way!"`);
  ...
  scene.text(`Smiling, she answers, "Your new temporary identity! Now comes the hard part. Listen, ${((s as any).pcs_nickname || '')}, I can temporarily change your appearance so you can enter agency and look for the curse's focus. Additionally, that will set your magical senses in tune with the curse, making it easier to look for it. So… are you ready?"`);
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).pain = (st as any).pain ?? {})['head'] = ((st as any).pain['head'] ?? 0) + (360);
    qspCall(st, 'stat', '');
    ...
    scene.text(`"Sorry, ${((st as any).pcs_nickname || '')}! Three drops of your blood must be put in the potion!" You look at Tatiana like she's mad, and she pinches her nose tiredly. "Not a Raziel's fan are you? Damn… sorry, that was inappropriate. Look, ${((st as any).pcs_nickname || '')}, I can change your appearance superficially, but your magic will still try to revert any changes I make, that's the reason why it hurt. Now I need to do a more extensive transformation, and it will probably hurt a lot more-- but don't worry! It will pass, and the changes are only temporary."`);
    ...
    scene.text(`She gives you a tired half-smile, before handing you a pocket mirror so that you can see the stranger you've become. "Pretty neat, isn't it, ${((st as any).pcs_nickname || '')}?" Tatiana says, as she looks over your shoulder.`);
...
function enterMission2D(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"…${((s as any).pcs_lastname || '')}, ${((s as any).pcs_firstname || '')} ${((s as any).pcs_lastname || '')}."`);
  if (((s as any).start_type ?? 0)?.['magic'] === 'tg') {
    qspCall(s, 'npc_relationship', 'modify_exact', 'A241', 1);
    // TODO-QSP: dynamic text: "Isn''t it Mr. Ivanov?" You blink, surprised at her words, and she smiles at you...
    scene.text(`"Isn't it Mr. Ivanov?" You blink, surprised at her words, and she smiles at your reaction. "Yes, ${((s as any).pcs_lastname || '')}, I know who you <i>really</i> are."`);
  } else {
    // TODO-QSP: dynamic text: "So here we are, <<$pcs_lastname>>."
    scene.text(`"So here we are, ${((s as any).pcs_lastname || '')}."`);
  }
...
function enterMission2F(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "<i>Shh,</i> it''s okay, <<$pcs_nickname>>! Don''t fight it! You won''t remember...
  scene.text(`"<i>Shh,</i> it's okay, ${((s as any).pcs_nickname || '')}! Don't fight it! You won't remember this!"`);
  ...
  scene.text(`"<i>Mmm…</i> So good! What a wonderful taste, ${((s as any).pcs_nickname || '')}!"`);
  // TODO-QSP: end
  scene.actions([
...
function enterMission2sex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('You find yourself lying face down, wearing only panties in what appears to be a photography set. Tatiana kneels by your side in a somewhat diaphanous robe.');
  // TODO-QSP: dynamic text: "Are you ready for your massage, <<$pcs_nickname>>?"
  scene.text(`"Are you ready for your massage, ${((s as any).pcs_nickname || '')}?"`);
  ...
  scene.text(`"Okay! Let's start with the oil, ${((s as any).pcs_nickname || '')}!"`);
  ...
  // TODO-QSP: dynamic text: "Hey! Look, <<$pcs_nickname>>! The guys are gone! Tell me… Doesn''t this give yo...
  scene.text(`"Hey! Look, ${((s as any).pcs_nickname || '')}! The guys are gone! Tell me… Doesn't this give you ideas?"`);
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    scene.actions([
      { label: 'Try to remember!', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
    ]);
  } else {
    ...
    scene.text(`"Tee hee, ${((st as any).pcs_nickname || '')}. Do you like this? Am I making your panties wet? Don't worry, no girl has left feeling unsatisfied after playing with me!"`);
    ...
    // TODO-QSP: dynamic text: "<<$pcs_nickname>>, please! I''m not going to hurt you! Just enjoy it like all t...
    scene.text(`"${((st as any).pcs_nickname || '')}, please! I'm not going to hurt you! Just enjoy it like all the other girls!"`);
    ...
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Counterattack!', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
    ...
    scene.text(`"Hey, ${((st as any).pcs_nickname || '')}! This isn't how-- <i>mmph?!</i>"`);
    ...
    scene.text(`"${((st as any).pcs_nickname || '')}! Please! Stop!"`);
    ...
    // TODO-QSP: dynamic text: "Please, <<$pcs_nickname>>! Don''t do that! I''m not supposed to work like this!...
    scene.text(`"Please, ${((st as any).pcs_nickname || '')}! Don't do that! I'm not supposed to work like this! I… don't know what will happen if you continue! Please…"`);
    ...
    scene.text(`"${((st as any).pcs_nickname || '')}… <i>mmm…</i> There is something I must tell you!"`);
    ...
    scene.text(`"Umm… Look ${((st as any).pcs_nickname || '')}. I have done girls, but… I have never let girls do me. What I mean is… umm… I'm like… a virgin?"`);
    scene.text('You rest your head against one of her sexy legs and ponder her words for a moment.');
    if (((st as any).stat ?? 0)?.['lesbian_count'] === 0) {
    ...
    scene.text(`"Oh, fuck, ${((st as any).pcs_nickname || '')}! I love being your little slut!"`);
    ...
    scene.text(`"Yes, ${((st as any).pcs_nickname || '')}! Treat me like your bitch! Fuck my cunt!"`);
    ...
    scene.text(`"Oh my fucking God! Yes! ${((st as any).pcs_nickname || '')}! I never want to get out of this bed!"`);
    ...
    scene.text(`"Fuck, ${((st as any).pcs_nickname || '')}! That was weird… like eating backward? I mean, I don't care if we do that again and again, but give a girl time to recover."`);
    ...
    scene.text('"No! You can call me Aleksei, and you are?"');
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    qspCall(st, 'npc_relationship', 'modify_exact', 'A241', 3);
    (st as any).AlexandriaQW = 4;
    qspCall(st, 'stat', '');
    scene.actions([
      { label: '"I am…"', goto: ['tatiana_missions', 'mission2D'] },
  ...
  scene.actions([
    ...
    scene.text(`"You know, ${((st as any).pcs_nickname || '')}, there have been other girls, but they were like… I dunno… Vodka shots? Yeah, that! Quick and dirty fun! But you? You're my 'Billion Bucks Vodka!' So, I was thinking… want to be my special girl? You know, like… my morning girl? The one that must be tasted slow and with care?`);
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    (st as any).cuni = ((st as any).cuni ?? 0) + (1);
    qspCall(st, 'stat', '');
    ...
    // TODO-QSP: dynamic text: "Ahhh… <<$pcs_nickname>>, I knew it! I made you so wet! Damn! You''re a slut! Te...
    scene.text(`"Ahhh… ${((st as any).pcs_nickname || '')}, I knew it! I made you so wet! Damn! You're a slut! Tee-hee! You have no idea how much I love that!"`);
    ...
    scene.text(`"Tee-hee. There it is, ${((st as any).pcs_nickname || '')}! From now on, you are marked as my girl!"`);
    ...
    if (((st as any).stat ?? 0)?.['think_virgin'] === 1) {
      ...
      // TODO-QSP: dynamic text: "Oh… okay. Sorry, <<$pcs_nickname>>, I won''t take that from you. I… understand ...
      scene.text(`"Oh… okay. Sorry, ${((st as any).pcs_nickname || '')}, I won't take that from you. I… understand that you want to gift it to someone special."`);
    } else {
      ...
      scene.text(`"Yeah… Do you like this, ${((st as any).pcs_nickname || '')}? Tee-hee. I just love how you squirm! <i>Muach!</i> Don't worry, I'm going to make you my little slutty girl!"`);
    }
    ...
    qspCall(st, 'archetypes', 'gain', 'bimbo', 'large', 'Tatiana mission');
    qspCall(st, 'stat', '');
    scene.img('images/characters/city/btatiana/HT1.jpg');
    ...
    // TODO-QSP: dynamic text: "Oh… No, no, no! <<$pcs_nickname>>! Please just enjoy it! I''m going to… What?! ...
    scene.text(`"Oh… No, no, no! ${((st as any).pcs_nickname || '')}! Please just enjoy it! I'm going to… What?! No, wait! Please! N--"`);
