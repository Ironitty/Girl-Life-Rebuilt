...
function enterIntro(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"What did you expect, dragons?" Tatiana laughs. "The magic is all around you, ${((s as any).pcs_firstname || '')}. Most of the books in this part of the library are books on magic, ranging from magic theory and philosophical works to spellbooks on geomancy and shamanism. It's one of the biggest collections of its kind in this country, I've been told."`);
...
function enterMission1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Don't worry, we'll definitely return," Tatiana promises. "Won't we, ${((s as any).pcs_firstname || '')}?"`);
...
function enterMission2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Yes, ${((s as any).pcs_nickname || '')}, <i>HIC SUNT DRACONES!</i> Not Salvatore's sissyfied ones, I mean Tolkien's kingdom-destroying Smaug version! I will tell you about them some other day." She gives you another cheeky smile and continues.`);
    ...
    if (((s as any).pcs_splcstng ?? 0) >=50) {
      ...
      scene.text(`Tatiana grabs some papers and passes them to you. "Read this, ${((s as any).pcs_nickname || '')}!"`);
      ...
      scene.text(`"Yes, ${((s as any).pcs_nickname || '')}, they are transforming into scatter-brained bimbos! The interesting part is that they are copying some of my mannerisms and views. Following this lead, I consulted some curse experts. In summary, they think that there must be a personal item of mine at its center, and that it's what made the curse repel me."`);
    } else {
      // TODO-QSP: dynamic text: Tatiana grabs some papers and passes them to you. "Read this, <<$pcs_nickname>>!...
      scene.text(`Tatiana grabs some papers and passes them to you. "Read this, ${((s as any).pcs_nickname || '')}!"`);
      ...
      scene.text(`"Well, ${((s as any).pcs_nickname || '')}, the magical anomaly seems centered around a personal item of mine, and it's the reason for the repel effect."`);
    }
    scene.actions([
    ...
    if (((s as any).pfilmNO ?? 0) === 1) {
      ...
      if (((s as any).job_status ?? 0)?.['city_aphrodite_model'] === 'employed') {
        ...
        if (((s as any).pcs_hotcat ?? 0) <= 5) {
          ...
          scene.text(`The sorceress looks you over and smiles. "Nothing that dramatic, ${((s as any).pcs_nickname || '')}. Just look for work at the agency. Once inside, you need to look for the item. Talk to the models, eavesdrop, walk around and look for something wrong. It will be good training, and when you finally find whatever is at its center, bring it to me so that I can end the curse. Don't worry, I will pay you for your time."`);
        }
      }
    }
    scene.text('Saying this, Tatiana turns towards her studies, leaving you to your own devices.');
...
function enterMission2A(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Sorry, ${((s as any).pcs_nickname || '')}! It's just that, like, you don't have much in your head."`);
    ...
    scene.actions([
      ...
      scene.text(`"Hmm… I think it's better if we talk somewhere else, ${((s as any).pcs_nickname || '')}."`);
...
function enterMission2B(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Sorry, ${((s as any).pcs_nickname || '')}! Three drops of your blood must be put in the potion!" You look at Tatiana like she's mad, and she pinches her nose tiredly. "Not a Raziel's fan are you? Damn… sorry, that was inappropriate. Look, ${((s as any).pcs_nickname || '')}, I can change your appearance superficially, but your magic will still try to revert any changes I make, that's the reason why it hurt. Now I need to do a more extensive transformation, and it will probably hurt a lot more-- but don't worry! It will pass, and the changes are only temporary."`);
    ...
    scene.text(`She gives you a tired half-smile, before handing you a pocket mirror so that you can see the stranger you've become. "Pretty neat, isn't it, ${((s as any).pcs_nickname || '')}?" Tatiana says, as she looks over your shoulder.`);
...
function enterMission2sex(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    scene.text(`"Tee hee, ${((s as any).pcs_nickname || '')}. Do you like this? Am I making your panties wet? Don't worry, no girl has left feeling unsatisfied after playing with me!"`);
    ...
    scene.text(`"${((s as any).pcs_nickname || '')}, please! I'm not going to hurt you! Just enjoy it like all the other girls!"`);
    ...
    scene.text(`"Hey, ${((s as any).pcs_nickname || '')}! This isn't how-- <i>mmph?!</i>"`);
    ...
    scene.text(`"${((s as any).pcs_nickname || '')}! Please! Stop!"`);
    ...
    scene.text(`"Please, ${((s as any).pcs_nickname || '')}! Don't do that! I'm not supposed to work like this! I… don't know what will happen if you continue! Please…"`);
    ...
    scene.text(`"${((s as any).pcs_nickname || '')}… <i>mmm…</i> There is something I must tell you!"`);
    ...
    scene.text(`"Umm… Look ${((s as any).pcs_nickname || '')}. I have done girls, but… I have never let girls do me. What I mean is… umm… I'm like… a virgin?"`);
    ...
    scene.text(`"Oh, fuck, ${((s as any).pcs_nickname || '')}! I love being your little slut!"`);
    ...
    scene.text(`"Yes, ${((s as any).pcs_nickname || '')}! Treat me like your bitch! Fuck my cunt!"`);
    ...
    scene.text(`"Oh my fucking God! Yes! ${((s as any).pcs_nickname || '')}! I never want to get out of this bed!"`);
    ...
    scene.text(`"Fuck, ${((s as any).pcs_nickname || '')}! That was weird… like eating backward? I mean, I don't care if we do that again and again, but give a girl time to recover."`);
    ...
    scene.text(`"${((s as any).pcs_nickname || '')}! Do you remember the tale 'Beauty and The Beast'? Well, in it, The Serpent ends up screwing the girl pretty hard, and she is like, "You know, you must marry me!" So, I mean… you're like The Serpent, and I'm The Beauty, so… can I, like, come and live with you? I don't want to go with the other Tatiana! We can even…"`);
  ...
  scene.actions([
    ...
    scene.text(`"You know, ${((s as any).pcs_nickname || '')}, there have been other girls, but they were like… I dunno… Vodka shots? Yeah, that! Quick and dirty fun! But you? You're my 'Billion Bucks Vodka!' So, I was thinking… want to be my special girl? You know, like… my morning girl? The one that must be tasted slow and with care?`);
    ...
    scene.text(`"Ahhh… ${((s as any).pcs_nickname || '')}, I knew it! I made you so wet! Damn! You're a slut! Tee-hee! You have no idea how much I love that!"`);
    ...
    scene.text(`"Tee-hee. There it is, ${((s as any).pcs_nickname || '')}! From now on, you are marked as my girl!"`);
    ...
    if (((s as any).stat ?? 0)?.['think_virgin'] === 1) {
      ...
      scene.text(`"Oh… okay. Sorry, ${((s as any).pcs_nickname || '')}, I won't take that from you. I… understand that you want to gift it to someone special."`);
    } else {
      ...
      scene.text(`"Yeah… Do you like this, ${((s as any).pcs_nickname || '')}? Tee-hee. I just love how you squirm! <i>Muach!</i> Don't worry, I'm going to make you my little slutty girl!"`);
    }
    ...
    scene.text(`"Oh… No, no, no! ${((s as any).pcs_nickname || '')}! Please just enjoy it! I'm going to… What?! No, wait! Please! N--"`);
