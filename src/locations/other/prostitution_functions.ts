...
function enterWlBlock(s: GameState, scene: SceneBuilder): void {
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 'block') {
...
function enterUpdateProstitutionLocations(s: GameState, scene: SceneBuilder): void {
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterSetPavlovskProstitute(s, scene); (s as any).locArgs = __savedLocArgs; }
  if (((s as any).prostitute ?? 0)?.['gadukino'] === 0) {
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterSetGadukinoProstitute(s, scene); (s as any).locArgs = __savedLocArgs; }
  }
  if (((s as any).prostitute ?? 0)?.['road'] === 0) {
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterSetHighwayProstitute(s, scene); (s as any).locArgs = __savedLocArgs; }
  }
  scene.build();
}
...
function enterProstituteOutfitAtHome(s: GameState, scene: SceneBuilder): void {
  if (qspFunc(s, 'homes_properties', 'is_current_home')  &&  ((s as any).prostitute ?? 0)?.['active']) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterSetDefaultOutfit(st, scene); (st as any).locArgs = __savedLocArgs; }
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterChangeIntoRegularClothes(st, scene); (st as any).locArgs = __savedLocArgs; }
    dynamicGoto(st, 'prevLoc', 'prevArg');
  } },
      ]);
    } else {
    ...
    alert(qspFunc(st, 'wrap', 'neg', 'Unavailable as your outfit is too small!'));
  } },
          ]);
        } else {
          if (((s as any).temp_not_wear_reason ?? 0) === 'too_large') {
            scene.actions([
              { label: 'Change into prostitute outfit', handler: (st: GameState) => {
    alert(qspFunc(st, 'wrap', 'neg', 'Unavailable as your outfit is too large!'));
  } },
            ]);
          } else {
    ...
    alert(0);
                ...
                scene.actions([
                  { label: 'Change into prostitute outfit', handler: (st: GameState) => {
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterChangeIntoProstituteOutfit(st, scene); (st as any).locArgs = __savedLocArgs; }
    dynamicGoto(st, 'prevLoc', 'prevArg');
  } },
                ]);
              }
        ...
        (s as any).temp_not_wear_reason = undefined;
      }
    }
  }
  scene.build();
...
function enterChangeToWork(s: GameState, scene: SceneBuilder): void {
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 'car') {
    scene.text('iif($loc_desc = \', \'You quickly change into your working outfit in your car.\', $loc_desc)');
  }
  if (String((s as any).locArgs?.[1] ?? '') === 'restroom') {
    scene.text('iif($loc_desc = \', \'You slip into a stall and quickly change into your working outfit, trying to avoid your clothes touching the floor or, really, anything before you place them safely in your bag.\', $loc_desc)');
  }
  if (String((s as any).locArgs?.[1] ?? '') === 'road_side') {
    scene.text('iif($loc_desc = \', \'You quickly change into your working outfit behind some bushes by the road.\', $loc_desc)');
  }
...
function enterWillUnprotected(s: GameState, scene: SceneBuilder): void {
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 'resist') {
    ...
    if (((s as any).pro_stats ?? 0)?.['unprotected'] <= 90) {
      (s as any).will_cost = (100 - (((s as any).pro_stats ?? {})?.['unprotected'] ?? 0) + ((s as any).will_calc ?? 0)) / 10;
    } else {
...
function enterWillRimming(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'willpower', 'calc');
  if (String((s as any).locArgs?.[1] ?? '') === 'self') {
    ...
    if (String((s as any).locArgs?.[1] ?? '') === 'force') {
      ...
      if (((s as any).stat ?? 0)?.['rimming_give'] <= 90) {
        (s as any).will_cost = (10 + (((s as any).stat ?? {})?.['rimming_give'] ?? 0) + ((s as any).will_calc ?? 0)) / 10;
      } else {
        (s as any).will_cost = (100 + ((s as any).will_calc ?? 0)) / 10;
      }
    }
...
function enterWillAtm(s: GameState, scene: SceneBuilder): void {
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 'self') {
    ...
    if (String((s as any).locArgs?.[1] ?? '') === 'force') {
      ...
      if (((s as any).pro_stats ?? 0)?.['atm'] <= 90) {
        (s as any).will_cost = (10 + (((s as any).pro_stats ?? {})?.['atm'] ?? 0) + ((s as any).will_calc ?? 0)) / 10;
      } else {
        (s as any).will_cost = (100 + ((s as any).will_calc ?? 0)) / 10;
      }
    }
...
function enterStdCheck(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).pro_std_check_rand = undefined;
  (s as any).pro_std_rand = undefined;
  scene.build();
}
...
function enterHisCondom(s: GameState, scene: SceneBuilder): void {
  (s as any).bwa_boy = ((String((s as any).locArgs?.[1] ?? '') === '') ? (((s as any).boydesc ?? 0)) : (((s as any).locArgs?.[1] ?? 0)));
  (s as any).protect = 1;
  (s as any).sexcontra = 3;
...
function enterClientEffort(s: GameState, scene: SceneBuilder): void {
  (s as any).client_type = ((String((s as any).locArgs?.[1] ?? '') === '') ? ('normal') : (((s as any).locArgs?.[1] ?? 0)));
...
function enterDailyEffects(s: GameState, scene: SceneBuilder): void {
  if (((s as any).prostitute ?? 0)?.['age_mod'] >= 100) {
    ...
    (s as any).temp_age_mod_remain = undefined;
  }
  if (((s as any).prostitute ?? 0)?.['skin_penalty'] >= 100) {
    ...
    (s as any).temp_skin_remain = undefined;
  }
  scene.build();
}

function enterPayment(s: GameState, scene: SceneBuilder): void {
  if (String((s as any).locArgs?.[1] ?? '') === 'pav') {
  ...
  if (((s as any).klismaday ?? 0) !== ((s as any).daystart ?? 0)  &&  ((s as any).prostitute ?? 0)?.['client_scene'] === 'Anal') {
    if (((s as any).prostitute ?? 0)?.['dirty_anal_mod'] === 0) {
      if (String((s as any).locArgs?.[1] ?? '') === 'condom_no') {
        ...
        if (String((s as any).locArgs?.[1] ?? '') === 'condom_yes') {
          ((s as any).prostitute = (s as any).prostitute ?? {})['payment_mod'] = ((s as any).prostitute['payment_mod'] ?? 0) - (300);
        }
      }
    } else {
...
function enterRandomName(s: GameState, scene: SceneBuilder): void {
  ((s as any).pro_boyname = (s as any).pro_boyname ?? {})[1] = '';
  ((s as any).pro_boyname = (s as any).pro_boyname ?? {})[2] = '';
  while (true) {
    ...
    if (((s as any).pro_boyname ?? 0)[1] === '') {
      ((s as any).pro_boyname = (s as any).pro_boyname ?? {})[1] = ((s as any).pro_temp ?? 0);
      break;
    } else {
      if (((s as any).pro_boyname ?? 0)[2] === '') {
        ((s as any).pro_boyname = (s as any).pro_boyname ?? {})[2] = ((s as any).pro_temp ?? 0);
      }
    }
...
function enter(s: GameState, scene: SceneBuilder): void {
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[0] = 'pav_commcenter';
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[1] = 'pav_residential';
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[2] = 'pav_industrial';
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[3] = 'pav_commercial';
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[4] = 'pav_market';
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[5] = 'pav_complex';
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[6] = 'pav_park';
  ((s as any).solicitation_locations = (s as any).solicitation_locations ?? {})[7] = 'pushkin';
  const arg = s.locArg;
  switch (arg) {
