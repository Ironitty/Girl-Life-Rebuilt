...
function enterGoSeeOlu(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).OluQW = (st as any).OluQW ?? {})['nat_inv'] = 1;
    ((st as any).OluQW = (st as any).OluQW ?? {})['nat_count'] = ((st as any).OluQW['nat_count'] ?? 0) + (1);
    qspCall(st, 'boystat', 'A55');
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['Debt'] = ((st as any).NatbelQW['Debt'] ?? 0) - (500);
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    if (((st as any).NatbelQW ?? 0)?.['QWstage'] === 9) {
      ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['QWstage'] = 10;
    }
    qspCall(st, 'stat', '');
    ...
    scene.text(`He looks at you, that perpetual smile still on his face. "Hi, ${((st as any).pcs_nickname ?? '')}! Who's your friend?" He says in badly accented Russian`);
    ...
    (st as any).Olu_pay = ((st as any).Olu_pay ?? 0) + (1);
    qspCall(st, 'stat', '');
    ...
    scene.text(`You help Natasha get undressed and then get her down on all fours in front of Olu. While he lubes up his dick and her ass, you get undressed as well. No reason he should have all the fun. You knee down beside Natasha and grab her butt cheeks with your hands and spread them wide for Olu. Not needing any more invitation than that, he kneels down behind her and lines his cock up, she squeals loudly as he forces ${((st as any).dick ?? '')}cm ${((st as any).dick_girth ?? '')} cock into her ass.`);
    // TODO-QSP: dynamic text: He starts fucking her slowly at first but you encourage him to go faster and dee...
    scene.text(`He starts fucking her slowly at first but you encourage him to go faster and deeper, pulling on her hips. You feel your pussy getting wet listening to Natasha cries and watching the ${((st as any).dick ?? '')}cm ${((st as any).dick_girth ?? '')} cock rip up her ass. You start rubbing your clit while watching.`);
    ...
    qspCall(st, 'arousal', 'voyeur_sex', 5);
    qspCall(st, 'arousal', 'cuni', (-5), 'dom', 'lesbian');
