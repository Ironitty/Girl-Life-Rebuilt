...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).PCloStyle2 ?? 0) === 4  ||  ((s as any).PCloSchool ?? 0) === 1) {
    ...
    scene.text(`"Hi! My name is ${((s as any).pcs_nickname || '')}." you answer. "School just ended so I was trying to figure out what to do for the rest of the day."`);
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterDateChoice(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ...
    if (((s as any).region ?? 0) === 'city_center'  ||  ((s as any).region ?? 0) === 'city'  ||  ((s as any).region ?? 0) === 'city'  ||  ((s as any).region ?? 0) === 'city_island'  ||  ((s as any).region ?? 0) === 'city_lake'  ||  ((s as any).region ?? 0) === 'city_park') {
      ...
      if (((s as any).region ?? 0) === 'pav'  ||  ((s as any).region ?? 0) === 'pav_park'  ||  ((s as any).region ?? 0) === 'pav') {
    ...
    scene.text(`You give him a contemptuous look. "Oh, you have us all figured out do you? My name is ${((s as any).pcs_nickname || '')}, not that it is any of your business."`);
    if (((s as any).loc ?? 0) === 'pav_residential'  ||  ((s as any).loc ?? 0) === 'pav_commercial') {
    ...
    scene.text(`You break out in a smile telling him, "My name is ${((s as any).pcs_nickname || '')}."`);
    ...
    scene.text(`You start blushing as ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} compliments you… "So what do you say ${((s as any).pcs_nickname || '')} would you like to go on a date with me?"`);
    scene.actions([
      { label: 'Decline', goto: ['dateM', 'decline'] },
      { label: 'I would love to', handler: (st: GameState) => {
    scene.img(`${((s as any).npc_pic ?? 0)?.[String((s as any).boy ?? 0)] ?? ''}`);
...
function enterDatepark(s: GameState, scene: SceneBuilder): void {
  ...
  qspCall(s, 'stat', '');
  scene.img('images/locations/shared/park/walk_\'+ rand(1, 2) +\'.jpg');
  ...
  scene.actions([
    ...
    scene.text(`The two of you sit down and begin commenting on the people walking by. You laugh while ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} makes up stories about their backgrounds and quirks.`);
    // TODO-QSP: dynamic text: There are a few times you almost get caught as you point and laugh. As you're si...
    scene.text(`There are a few times you almost get caught as you point and laugh. As you're sitting down an ice cream pushcart appears, generating quite a buzz around it. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} asks you, "I sure could go for some ice cream. Would you also like some ice cream?"`);
    scene.actions([
    ...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} runs away to the pushcart and comes back with two ice creams. "Here you go, ${((s as any).pcs_nickname || '')} I hope you'll enjoy it."`);
    // TODO-QSP: dynamic text: You nod and thank him. The two of you continue talking while eating the ice crea...
    scene.text(`You nod and thank him. The two of you continue talking while eating the ice cream. As you finish ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} asks, "Shall we move on?"`);
    scene.actions([
    ...
    scene.text(`You don't even notice that the two of you strayed off the beaten path, all of a sudden ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} stops and whispers in your ear, "Now that we're at a secluded spot, you know what would make this date even better?"`);
    scene.actions([
    ...
    scene.text(`Before you're even able to respond ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} leans in and kisses you on the lips.`);
    scene.actions([
    ...
    scene.text(`Feeling frisky you kiss ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} back. Not before long the two of you are having a make out session, french kissing each other.`);
    scene.text('You feel yourself getting more and more aroused, maybe you should stop before the situation escalates too much…');
    qspCall(s, 'arousal', 'kiss', 3, 'exhibitionism');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} pulls you in closer and slowly begins rubbing your wet slit through your clothes. You let out a soft whimper as he's touching you.`);
    qspCall(s, 'arousal', 'kiss', 4, 'exhibitionism');
    qspCall(s, 'stat', '');
    // TODO-QSP: dynamic text: As you're pressed against <<$npc_firstname[$boy]>> you feel his bulge through hi...
    scene.text(`As you're pressed against ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} you feel his bulge through his pants. The two of you quickly remove your clothes, playing with each other's sensitive parts.`);
    scene.actions([
    ...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} lets out a groan and you feel his cock growing bigger and bigger. After a minute or so his cock is pulsating and you're ready to move on.`);
    // TODO-QSP: dynamic text: You keep teasing him on as you lick his penis head and penis shaft. <<$npc_first...
    scene.text(`You keep teasing him on as you lick his penis head and penis shaft. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''}'s cock twitches as you keep licking him.`);
    qspCall(s, 'arousal', 'hj', 4, 'exhibitionism');
    qspCall(s, 'arousal', 'bj', 4, 'exhibitionism');
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Give blowjob', handler: (st: GameState) => {
    scene.img('images/locations/shared/date/parkbj.jpg');
    // TODO-QSP: dynamic text: "Enough teasing, I want you to blow me <<$pcs_nickname>>, I don't want to cum be...
    scene.text(`"Enough teasing, I want you to blow me ${((s as any).pcs_nickname || '')}, I don't want to cum before you've had a taste of my cock."`);
    ...
    qspCall(s, 'arousal', 'bj', 6, 'exhibitionism');
    qspCall(s, 'stat', '');
    // TODO-QSP: dynamic text: "Damn, <<$pcs_nickname>>, you're really a good little cocksucker. How about you ...
    scene.text(`"Damn, ${((s as any).pcs_nickname || '')}, you're really a good little cocksucker. How about you get on all fours so I can enjoy that little pussy of yours?"`);
    scene.actions([
    ...
    scene.text(`Even ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} is groaning and grimacing badly by now ready to cum.`);
    scene.actions([
      { label: 'Finish', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 10;
    qspCall(s, 'cum_call', 'labia', ((s as any).boy ?? 0));
    qspCall(s, 'stat', '');
    scene.img('images/locations/shared/date/parkfinish.jpg');
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> can't contain himself anymore, he's loudly groaning and...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} can't contain himself anymore, he's loudly groaning and as soon he pulls out of you he cums coating your pussy lips.`);
    ...
    scene.text(`"That was great ${((s as any).pcs_nickname || '')}. We'll have to do this again." You get up and clean yourself, straightening out your clothes the best you can. "Shall we head back?" you ask.`);
    scene.actions([
      { label: 'Walk back', goto: ['dateM', 'walk_back'] },
    ]);
  } },
    ...
    scene.text(`He shakes his head. "No, not at all. I'm not going to make you do something you don't want." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} answers.`);
    scene.actions([
      { label: 'Walk back', goto: ['dateM', 'walk_back'] },
    ]);
  } },
...
function enterDatecinema(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"How about this one?" ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} points at a movie.`);
    ...
    scene.text(`You and ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} stand in line waiting to approach the box office.`);
    // TODO-QSP: dynamic text: As you've bought the tickets, <<$npc_firstname[$boy]>> asks, "Would you like som...
    scene.text(`As you've bought the tickets, ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} asks, "Would you like something from the concession stand?"`);
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 45;
    qspCall(s, 'stat', '');
    scene.img('images/locations/shared/cinema/kino_\'+ rand(1, 3) +\'.jpg');
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 15;
    scene.img('images/locations/shared/cinema/kino_\'+ rand(1, 3) +\'.jpg');
    // TODO-QSP: dynamic text: As you're about to grab some popcorn from the tub, <<$npc_firstname[$boy]>> unkn...
    scene.text(`As you're about to grab some popcorn from the tub, ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} unknowingly goes to grab some popcorn too, touching your hand.`);
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> nervously smiles at you. "Sorry it wasn't my intentions...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} nervously smiles at you. "Sorry it wasn't my intentions to make you feel uncomfortable."`);
    ...
    scene.text(`You look around checking if everyone else is concentrating on the movie and then quickly remove your top. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} not knowing how to respond looks in awe. Not wanting to wait on him anymore you decide to take action.`);
    ...
    scene.text(`You go down on your knees removing your shirt and bra showing your body to ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''}. You begin playing with his rock-hard penis through the pants.`);
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> looks at you with lust. His eyes are quite revealing, h...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} looks at you with lust. His eyes are quite revealing, he wants you to please him with your mouth and he wants it now.`);
    // TODO-QSP: dynamic text: You unbutton his pants and pull down his briefs. The rock-hard cock pops out whi...
    scene.text(`You unbutton his pants and pull down his briefs. The rock-hard cock pops out while you look at ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} smiling. "Keep quiet now we don't want to get caught."`);
    // TODO-QSP: dynamic text: You grab <<$npc_firstname[$boy]>> by the shaft and your lips embrace the top of ...
    scene.text(`You grab ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} by the shaft and your lips embrace the top of his cock. After a while you pick up the pace moving the head up and down while ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} is trying his best to keep quiet.`);
    ...
    scene.text(`You lay down on the empty seats next to you and spread your legs. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} by now very aroused dives in and begins playing with your wet clit.`);
    ...
    scene.text(`You make ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} sit down again and plant yourself on top of his cock, letting out a quiet whimper as you don't want the guy in front of you to find out what the two of you are doing.`);
    // TODO-QSP: dynamic text: As you control the pace you switch it up multiple times bouncing up and down as ...
    scene.text(`As you control the pace you switch it up multiple times bouncing up and down as you see this as a payback for not letting you orgasm before. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} is clearly having some trouble holding himself as his cock is growing inside you.`);
    qspCall(s, 'arousal', 'vaginal', 5, 'dom', 'exhibitionism');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.text(`By now ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} is almost drained and the smallest touch would make him cum. You decide to give him a breather as you once again lay down spreading your legs once again while your pussy juice drips all over the seat.`);
    // TODO-QSP: dynamic text: As soon as <<$npc_firstname[$boy]>> enters you he's already heavily panting. You...
    scene.text(`As soon as ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} enters you he's already heavily panting. You whisper angrily to him "Don't you dare cum yet. I'm still not pleased." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} does his best and keeps thrusting for a minute or two not being able to hold on anymore.`);
    qspCall(s, 'arousal', 'vaginal', 5, 'dom', 'exhibitionism');
    qspCall(s, 'stat', '');
    ...
    scene.text(`While the two of you are returning to your senses all of a sudden someone flashes a light at you. It's the usher… You're totally caught as you're still laying down naked with ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} cum splattered all over you.`);
    // TODO-QSP: dynamic text: "What the hell are you two doing!" the usher yells. The guy that sat in front of...
    scene.text(`"What the hell are you two doing!" the usher yells. The guy that sat in front of you stands up noticing you laying naked quickly picks up the phone and tries taking some pictures. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} pulls up his pants and gets dressed, leaving you all naked to fend for yourself.`);
    ...
    // TODO-QSP: dynamic text: You're blemishing red as you quickly get dressed, grabbing your stuff, running o...
    ...
    scene.text(`"Wow, that's never happened to me before." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} tells you.`);
    ...
    scene.text(`After a while ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} leans towards you and whispers if you're enjoying yourself.`);
    ...
    scene.text(`The movie finishes and the lights get turned on. You and ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} walk outside where you talk about how funny the movie was.`);
...
function enterDatecafe(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"This is a nice spot. So tell me about yourself ${((s as any).pcs_nickname || '')}?" ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} asks as soon as you sit down.`);
    // TODO-QSP: dynamic text: You tell him a little about your family, where you're from and the likes. All wh...
    scene.text(`You tell him a little about your family, where you're from and the likes. All while ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} carefully listens to your every word just like he's analyzing you in his head.`);
    ...
    scene.text(`She scribbles down your order, as she re-reads the order, she manages to stammer through your order but when it comes to ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} she messes up.`);
    // TODO-QSP: dynamic text: "What is wrong with you? Can't you write down a simple order?" <<$npc_firstname[...
    scene.text(`"What is wrong with you? Can't you write down a simple order?" ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} reacts.`);
    ...
    scene.text(`"Hope everything is in order." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} says. Feeling a bit irritated you kick him under the table, while thanking her.`);
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> looks at you in despair, "What? Did I do anything wrong...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} looks at you in despair, "What? Did I do anything wrong?"`);
    ...
    scene.text(`Not liking the way ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} treated the waitress you go on the attack against ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''}. You're appalled by how he behaves. With such poor first-impression you decide you don't ever want to see this bottom-feeder again.`);
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> looks at you in fear as he wasn't expecting you to reac...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} looks at you in fear as he wasn't expecting you to react like that.`);
    ...
    scene.text('The poor waitress returns after a while with your order. You look at her with an apologetic look.');
    // TODO-QSP: dynamic text: "Hope everything is in order." <<$npc_firstname[$boy]>> says. You're not confide...
    scene.text(`"Hope everything is in order." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} says. You're not confident enough commenting on how an unknown person is behaving so you keep your mouth shut.`);
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> looks at with confidence, "What? She had it coming. I g...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} looks at with confidence, "What? She had it coming. I get irritated by weak people."`);
    ...
    scene.text(`"Hope everything is in order." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} says. "Yeah, it would be a shame if we needed to talk with the manager." you sourly tell her.`);
    // TODO-QSP: dynamic text: The waitress asks for forgiveness once again before going away. <<$npc_firstname...
    scene.text(`The waitress asks for forgiveness once again before going away. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} looks at you impressed, "I must say, the way you handled her was perfect. I'm getting so turned on."`);
...
function enterDatebar(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`As you sit down ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} asks you, "Would you like to drink something?"`);
    scene.actions([
      { label: 'I\'ll have a beer, thanks', handler: (st: GameState) => {
    qspCall(s, 'drugs', 'alcohol', 'beer');
    scene.img('images/locations/shared/date/barorder_\'+ rand(1, 2) +\'.jpg');
    // TODO-QSP: dynamic text: "My kind of girl." <<$npc_firstname[$boy]>> tells you.
    scene.text(`"My kind of girl." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} tells you.`);
    scene.actions([
...
function enterBartalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: The band steps out and starts playing. You're amazed over how well they play and...
    scene.text(`The band steps out and starts playing. You're amazed over how well they play and turn to ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} "They are pretty good don't you think?"`);
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> nods his head. "Yeah they are, do you want another drin...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} nods his head. "Yeah they are, do you want another drink?"`);
    scene.actions([
      { label: 'Sure', handler: (st: GameState) => {
    qspCall(s, 'drugs', 'alcohol', 'vodka');
    (s as any).minut = ((s as any).minut ?? 0) + 20;
    ...
    // TODO-QSP: dynamic text: "Well, I'm having a great time so why not." <<$npc_firstname[$boy]>> goes over t...
    scene.text(`"Well, I'm having a great time so why not." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} goes over to the counter and returns with two new drinks.`);
    ...
    scene.text(`"Would you like to dance?" you ask ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''}.`);
    ...
    scene.text(`You get up from the chair and walk up near the stage and begin moving sensually, teasing ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''}.`);
    scene.actions([
    ...
    scene.text(`"Damn ${((s as any).pcs_nickname || '')} you really know how to move. I bet all that dancing made you even thirstier."`);
    ...
    scene.text(`Recognizing ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} voice, you come to your senses and open the door, quickly pulling him in.`);
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname[$boy]>> doesn't seem to mind your brashness. He begins grabbing...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} doesn't seem to mind your brashness. He begins grabbing you all over your body, adding even more to your arousal.`);
    ...
    scene.text(`"Get down on your knees ${((s as any).pcs_nickname || '')}."`);
    ...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} arrives shortly after you to the table. The whole situation is too awkward now and you begin to blush as you're really ashamed of your actions. You're starting to panic a bit…`);
    scene.actions([
    ...
    scene.text(`You devour his flaccid penis with your mouth, swallowing it whole with ease. ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} is all warmed up, his cock is twitching and he's right at the edge as you stop, "Oh, no we're not done yet."`);
    ...
    scene.text(`You tell him to rest for a moment while you get ready. You pull down your panties exposing your wet cunt, you play with yourself as ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} catches his breath. "That's enough rest." you tell him.`);
    ...
    qspCall(s, 'stat', '');
    // TODO-QSP: dynamic text: It doesn't take long before <<$npc_firstname[$boy]>> is panting heavily and just...
    scene.text(`It doesn't take long before ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} is panting heavily and just as you're about to say not to cum inside you, you feel a warm stream shoot up deep inside of you.`);
    scene.actions([
    ...
    scene.text(`"That was great ${((s as any).pcs_nickname || '')}." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} compliments you. "You were not bad yourself" you tell him.`);
    scene.actions([
      { label: 'Finish the date', goto: ['dateM', 'barend'] },
    ]);
  } },
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 20;
    qspCall(s, 'stat', '');
    scene.img('images/locations/shared/date/bardiscuss_\'+ rand(1, 4) +\'.jpg');
    // TODO-QSP: dynamic text: You and <<$npc_firstname[$boy]>> continue talking even if it's hard to hear what...
    scene.text(`You and ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)] ?? ''} continue talking even if it's hard to hear what he says at some times because of the loud music.`);
...
function enterBarend(s: GameState, scene: SceneBuilder): void {
  ...
  qspCall(s, 'stat', '');
  scene.img('images/locations/shared/date/bardiscuss_\'+ rand(1, 4) +\'.jpg');
  // TODO-QSP: dynamic text: You tell <<$npc_firstname[$boy]>>, "I've had a great time but we should be on ou...
