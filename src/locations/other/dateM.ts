...
function enterDatepark(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)]} runs away to the pushcart and comes back with two ice creams. "Here you go, ${((s as any).pcs_nickname ?? 0)} I hope you'll enjoy it."`);
    ...
    scene.text(`"Enough teasing, I want you to blow me ${((s as any).pcs_nickname ?? 0)}, I don't want to cum before you've had a taste of my cock."`);
    ...
    scene.text(`"Damn, ${((s as any).pcs_nickname ?? 0)}, you're really a good little cocksucker. How about you get on all fours so I can enjoy that little pussy of yours?"`);
    scene.actions([
    ...
    scene.text(`"That was great ${((s as any).pcs_nickname ?? 0)}. We'll have to do this again." You get up and clean yourself, straightening out your clothes the best you can. "Shall we head back?" you ask.`);
    scene.actions([
      { label: 'Walk back', goto: ['dateM', 'walk_back'] },
    ]);
  } },
...
function enterDatecafe(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"This is a nice spot. So tell me about yourself ${((s as any).pcs_nickname ?? 0)}?" ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)]} asks as soon as you sit down.`);
...
function enterBartalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Damn ${((s as any).pcs_nickname ?? 0)} you really know how to move. I bet all that dancing made you even thirstier."`);
    ...
    scene.text(`"Get down on your knees ${((s as any).pcs_nickname ?? 0)}."`);
    qspCall(s, 'arousal', 'foreplay', 2);
    qspCall(s, 'stat', '');
    qspCall(s, 'willpower', 'bj', 'resist');
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Decide to stop [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
    ...
    scene.text(`"That was great ${((s as any).pcs_nickname ?? 0)}." ${((s as any).npc_firstname ?? 0)?.[String((s as any).boy ?? 0)]} compliments you. "You were not bad yourself" you tell him.`);
    scene.actions([
      { label: 'Finish the date', goto: ['dateM', 'barend'] },
    ]);
  } },
