...
function enterDaytimeAfterMenu(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) < 14) {
    if ((Array.isArray((s as any).date_ev) ? ((s as any).date_ev as any[]).indexOf('lunch_date') : -1) < 0) {
      // TODO-QSP: act 'Get lunch': gt 'date_ev', 'lunch_after_ask'
    }
  } else {
    if (((s as any).hour ?? 0) < 21) {
      if ((Array.isArray((s as any).date_ev) ? ((s as any).date_ev as any[]).indexOf('dinner_date') : -1) < 0) {
  ...
  scene.actions([
    ...
    if (((s as any).date_ev ?? 0)?.['at_home'] === 0) {
      if (((s as any).home ?? 0)?.['region'] !== ((s as any).region ?? 0)  &&  ((s as any).npc_residence ?? 0)?.[String((s as any).npcID ?? 0)] !== 'region') {
        ...
        scene.actions([
          { label: 'Go to the nearest hotel', handler: (st: GameState) => {
    // TODO-QSP: gt 'sex_ev_start', 'initiate_pre', $npcID, 'hotel'
  } },
        ]);
      } else {
    ...
    // TODO-QSP: gt 'sex_ev_start', 'initiate_pre', $npcID, 'hotel'
            ...
            if (((s as any).sex_ev ?? 0)?.['loc'] === 'hotel_room') {
              ...
              scene.text('"Yeah."');
              scene.actions([
                { label: 'Rush back', handler: (st: GameState) => {
    // TODO-QSP: gt 'sex_ev_start', 'initiate_pre', $npcID, 'hotel'
  } },
              ]);
            } else {
    ...
    scene.actions([
      { label: 'Rush home', handler: (st: GameState) => {
    // TODO-QSP: gt 'sex_ev_start', 'initiate_pre', $npcID, 'pc_home'
  } },
    ]);
  } },
                ]);
...
function enterHangoutAsk(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    scene.actions([
      { label: 'Go to <<$npcdesc>>\'s', handler: (st: GameState) => {
    // TODO-QSP: gt 'date_hangout', 'start', 'together'
  } },
    ]);
  } },
...
function enterStreetGoodbye(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: xgt 'date_after', 'leaving_dialogue_choices', 'outside'
  scene.build();
}
...
function enterNpcHomeLeave(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: xgt 'date_after', 'leaving_dialogue_choices', 'inside'
  scene.build();
}
