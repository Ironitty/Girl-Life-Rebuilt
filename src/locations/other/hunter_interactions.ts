...
function enterChatWithHunters(s: GameState, scene: SceneBuilder): void {
  if (((s as any).hunterVars ?? 0)?.['sexual_comfort'] > 40  &&  ((s as any).alko ?? 0) > 4  &&  ((s as any).clothingworntype ?? 0) !== 'nude'  &&  (Math.floor(Math.random() * 5) + 1) === 1) {
    // TODO-QSP: gt 'hunter_interactions', 'chat_with_hunters_theme', 'entertainment'
  } else {
    if (((s as any).boy_talk ?? 0) === 0  &&  ((s as any).hunterVars ?? 0)?.['drink'] === 1  &&  (Math.floor(Math.random() * 10) + 1) === 1) {
      // TODO-QSP: gt 'hunter_interactions', 'chat_with_hunters_theme', 'boy_talk'
    } else {
      if (((s as any).boy_talk ?? 0) === 1  &&  ((s as any).virginity_talk ?? 0) === 0  &&  ((s as any).hunterVars ?? 0)?.['drink'] === 1  &&  (Math.floor(Math.random() * 10) + 1) === 1) {
        // TODO-QSP: gt 'hunter_interactions', 'chat_with_hunters_theme', 'virginity_talk'
      } else {
        if (((s as any).PCloSkirt ?? 0) > 0  &&  ((s as any).pantyworntype ?? 0) === 'none'  &&  (Math.floor(Math.random() * 9) + 1) === 1) {
          // TODO-QSP: gt 'hunter_interactions', 'chat_with_hunters_theme', 'pantiless_flash'
        } else {
          if (((s as any).clothingworntype ?? 0) === 'nude'  &&  ((s as any).pantyworntype ?? 0) === 'none'  &&  (Math.floor(Math.random() * 3) + 1) === 1) {
            // TODO-QSP: gt 'hunter_interactions', 'chat_with_hunters_theme', 'nude'
          } else {
            // TODO-QSP: gt 'hunter_interactions', 'chat_with_hunters_theme', 'generic'
          }
        }
      }
    }
...
function enterChatWithHuntersTheme(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'generic') {
    ...
    if (((s as any).locArgs?.[1] ?? 0) === 'pantiless_flash') {
      ...
      if (((s as any).locArgs?.[1] ?? 0) === 'boy_talk') {
        ...
        scene.text(`"So, ${((s as any).pcs_nickname ?? 0)}, anyone special in your life?" Igor asks, slurring noticeably`);
        (s as any).boy_talk = 1;
        qspCall(s, 'stat', '');
        scene.actions([
    ...
    if (((s as any).hunterVars ?? 0)?.['sexual_comfort'] < 35  &&  ((s as any).hunterVars ?? 0)?.['collective_opinion'] > 40) {
      ...
      if (((s as any).hunterVars ?? 0)?.['sexual_comfort'] > 50  &&  ((s as any).clothingworntype ?? 0) !== 'nude') {
        ...
        qspCall(s, 'willpower', 'resist', 'bj', ((((s as any).hunterVars ?? 0)?.['collective_opinion'] > 40) ? ('easy') : ('hard')));
        if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
          scene.actions([
            { label: 'Maybe another time boys [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    ...
    scene.actions([
      { label: 'Finish chatting', goto: ['gad_swamp_yard', 'campfire'] },
        ...
        if (((s as any).locArgs?.[1] ?? 0) === 'virginity_talk') {
          scene.img('images/locations/gadukino/hunters/fireside_drink.jpg');
          // TODO-QSP: dynamic text: "So, <<$pcs_nickname>>, have you ever done it with a boy" Andrei asks, grinning ...
          scene.text(`"So, ${((s as any).pcs_nickname ?? 0)}, have you ever done it with a boy" Andrei asks, grinning mischievously.`);
      ...
      scene.actions([
        { label: 'Maybe another time boys, not feeling it now', goto: ['gad_swamp_yard', 'start'] },
          ...
          if (((s as any).locArgs?.[1] ?? 0) === 'nude') {
            ...
            if (((s as any).locArgs?.[1] ?? 0) === 'entertainment') {
              scene.img('images/locations/gadukino/hunters/hanters1.jpg');
              // TODO-QSP: dynamic text: "Hey, <<$pcs_nickname>>, can you do us a favor?" says Andrei, seeing you approac...
              scene.text(`"Hey, ${((s as any).pcs_nickname ?? 0)}, can you do us a favor?" says Andrei, seeing you approach them. "We are in desperate need of some entertainment. Say, why don't you help us with that?"`);
...
function enterJoinDrinking(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hunterVars ?? 0)?.['collective_opinion'] > 40) {
    if (((s as any).hunterVars ?? 0)?.['sexual_comfort'] > 40) {
      ...
      scene.text(`"Sure, ${((s as any).pcs_nickname ?? 0)}, help yourself for as much as you want. Drink irresponsibly." says Andrei with a wolfish grin.`);
      scene.actions([
        { label: 'Further', goto: ['hunter_interactions', 'drink_with_hunters'] },
      ]);
    } else {
      if (((s as any).hunterVars ?? 0)?.['sexual_comfort'] > 20) {
        if (((s as any).alko ?? 0) < 3) {
          scene.actions([{ label: 'Continue', goto: ['hunter_interactions', 'drink_with_hunters'] }]);
        } else {
          // TODO-QSP: dynamic text: "I don't know, <<$pcs_nickname>>." says Andrei. "What if you turn out to be a me...
          scene.text(`"I don't know, ${((s as any).pcs_nickname ?? 0)}." says Andrei. "What if you turn out to be a mean drunk? You should perform a dare for us to prove we can safely have you wasted."`);
    ...
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'pick', 'hardcore'
        ...
        if (((s as any).alko ?? 0) < 3) {
          ...
          scene.text(`"No ${((s as any).pcs_nickname ?? 0)}, go away" says Sergei. "It is not good for you to get blackout drunk so young. You have had enough for now"`);
          scene.actions([
            { label: 'Further', goto: ['gad_swamp_yard', 'campfire'] },
          ]);
        }
    ...
    if (((s as any).hunterVars ?? 0)?.['sexual_comfort'] > 30) {
      if (((s as any).hunterVars ?? 0)?.['collective_opinion'] > 20) {
        ...
        scene.text(`"You see, ${((s as any).pcs_nickname ?? 0)}" says Andrei. "We had to work hard to make the money for this booze here. You should work for it just like us. Say, how about you provide some entertainment for us?"`);
    ...
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'pick', 'hardcore'
      ...
      scene.text(`"Go away ${((s as any).pcs_nickname ?? 0)}" says Igor. "We do not want you to join us, got better things to do than babysit a drunk teen"`);
      if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
        scene.actions([
          { label: 'Ask again and flash your tits [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
        ]);
      } else {
        scene.actions([
          { label: 'Ask again and flash your tits [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'flash_tits'
  } },
        ]);
      }
      scene.actions([
...
function enterBoozeDare(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === ''  ||  ((s as any).locArgs?.[1] ?? 0) === 'pick') {
    if (((s as any).locArgs?.[2] ?? 0) === 'innocent') {
      (s as any).minut = ((s as any).minut ?? 0) + 5;
      qspCall(s, 'willpower', 'voyeur', 'exhib');
      qspCall(s, 'stat', '');
      if (((s as any).clothingworntype ?? 0) === 'nude') {
        ...
        scene.actions([
          { label: 'Dance naked by the bonfire', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'nude_dance'
  } },
        ]);
      } else {
        ...
        if (((s as any).pantyworntype ?? 0) !== 'none') {
          if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
            scene.actions([
              { label: 'Show him your panties [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
            ]);
          } else {
            scene.actions([
              { label: 'Show him your panties [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'flash_thong'
  } },
            ]);
          }
        } else {
    ...
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
              ...
              { label: 'Let them see your pussy [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
            ]);
          } else {
            ...
            scene.actions([
              { label: 'Show them your unclad ass [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'flash_ass'
  } },
              { label: 'Let them see your pussy [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'flash_pussy'
  } },
            ]);
          }
        }
    ...
    if (((s as any).locArgs?.[2] ?? 0) === 'strip') {
      ...
      if (((s as any).clothingworntype ?? 0) !== 'nude') {
        ...
        if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
          scene.actions([
            { label: 'Strip off your clothes [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
          ]);
        } else {
          scene.actions([
            { label: 'Strip off your clothes [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'strip_outfit'
  } },
          ]);
        }
      } else {
    ...
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
            ...
            scene.actions([
              { label: 'Take off your panties [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'strip_panties'
  } },
            ]);
          }
        } else {
          ...
          scene.actions([
            { label: 'Dare to be an erotic model', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'present_pussy'
  } },
          ]);
        }
      }
    }
    if (((s as any).locArgs?.[2] ?? 0) === 'softcore') {
      (s as any).minut = ((s as any).minut ?? 0) + 5;
      ...
      if (((s as any).clothingworntype ?? 0) !== 'nude'  ||  ((s as any).pantyworntype ?? 0) !== 'none') {
        ...
        if (((s as any).alko ?? 0) < 4) {
          ...
          if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
            scene.actions([
              { label: 'Oblige him [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
            ]);
          } else {
            scene.actions([
              { label: 'Oblige him [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'present_tits'
  } },
            ]);
          }
        } else {
          // TODO-QSP: dynamic text: "So, <<$pcs_nickname>>, I am scientifically curious again. Can you show us how y...
          scene.text(`"So, ${((s as any).pcs_nickname ?? 0)}, I am scientifically curious again. Can you show us how you jill off? Just a little demonstration, and you get your booze."`);
          if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
            scene.actions([
              { label: 'Give them a teaser [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
            ]);
          } else {
            scene.actions([
              { label: 'Give them a teaser [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'masturbate_1'
  } },
            ]);
          }
        }
    ...
    if (((s as any).locArgs?.[2] ?? 0) === 'hardcore') {
      (s as any).minut = ((s as any).minut ?? 0) + 5;
      qspCall(s, 'stat', '');
      if (((s as any).clothingworntype ?? 0) !== 'nude') {
        ...
        if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
          scene.actions([
            { label: 'Kneel down in front of Sergei [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
          ]);
        } else {
          scene.actions([
            { label: 'Kneel down in front of Sergei [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'blow_clothed'
  } },
          ]);
        }
      } else {
        ...
        if (((s as any).pantyworntype ?? 0) !== 'none') {
          ...
          if (((s as any).hunterVars ?? 0)?.['collective_opinion'] > 40) {
            ...
            if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
              scene.actions([
                { label: 'Lay down on the blanket [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
              ]);
            } else {
              scene.actions([
                { label: 'Lay down on the blanket [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'blanket_fuck'
  } },
              ]);
            }
          } else {
    ...
    // TODO-QSP: gt 'hunter_interactions', 'booze_dare', 'chair_fuck'
  } },
            ]);
          }
        }
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'flash_thong') {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'flash_ass') {
    ...
    scene.text(`"You have a cute butt, ${((s as any).pcs_nickname ?? 0)}" comments Igor. You blush slightly at his remark.`);
    (s as any).minut = ((s as any).minut ?? 0) + 1;
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'flash_tits') {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'flash_pussy') {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'nude_dance') {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'strip_outfit') {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'strip_panties') {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'present_pussy') {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'pussy_spread') {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'present_tits') {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'masturbate_1') {
    ...
    scene.actions([
      { label: 'Further', handler: (st: GameState) => {
    qspCall(s, 'arousal', 'clit_finger', 3, 'masturbate', 'exhibitionism', 'sub');
    qspCall(s, 'arousal', 'foreplay', 2, 'exhibitionism', 'sub');
    qspCall(s, 'stat', '');
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'masturbate_2') {
    qspCall(s, 'arousal', 'clit_finger', 4, 'masturbate', 'exhibitionism', 'sub');
    qspCall(s, 'stat', '');
    ...
    scene.actions([
      { label: 'Further', handler: (st: GameState) => {
    qspCall(s, 'arousal', 'clit_finger', 5, 'masturbate', 'exhibitionism', 'sub');
    qspCall(s, 'stat', '');
    ...
    qspCall(s, 'arousal', 'clit_finger', 4, 'masturbate', 'exhibitionism', 'sub', 'humiliation');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Grab a drink', handler: (st: GameState) => {
    (s as any).hunterVars['sexual_comfort'] = ((s as any).hunterVars['sexual_comfort'] ?? 0) + (4);
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'blow_clothed') {
    ...
    // TODO-QSP: dynamic text: You feel Sergei's cock throbbing more and more intensely before he suddenly pull...
    scene.text(`You feel Sergei's cock throbbing more and more intensely before he suddenly pulls out, spurting his jizz all over your face. "Nice makeup ${((s as any).pcs_nickname ?? 0)}!" says Andrei, after you blink the cum out of your eyes and make eye contact with him watching the whole thing.`);
    qspCall(s, 'boyStat', 'A174');
    qspCall(s, 'arousal', 'bj', 5, 'sub', 'rough', 'deepthroat');
    qspCall(s, 'cum_call', 'face', 'A174', 1);
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Grab a drink', handler: (st: GameState) => {
    (s as any).hunterVars['sexual_comfort'] = ((s as any).hunterVars['sexual_comfort'] ?? 0) + (10);
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'chair_fuck') {
    ...
    qspCall(s, 'arousal', 'vaginal', 8, 'sub', 'exhibitionism');
    qspCall(s, 'cum_call', '', '', 'A172', 1);
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Grab a drink', handler: (st: GameState) => {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'blanket_fuck') {
    ...
    qspCall(s, 'arousal', 'vaginal_finger', (-3), 'exhibitionism');
    qspCall(s, 'arousal', 'cuni', (-3), 'exhibitionism');
    ...
    qspCall(s, 'arousal', 'vaginal', 8, 'exhibitionism', 'rough');
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Further', handler: (st: GameState) => {
    ...
    qspCall(s, 'arousal', 'vaginal', 5, 'exhibitionism', 'dom');
    qspCall(s, 'arousal', 'clit_finger', (-5), 'exhibitionism', 'dom');
    qspCall(s, 'cum_call', '', '', 'A174', 1);
    qspCall(s, 'cum_call', 'labia', 'A174', 1);
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Grab a drink', handler: (st: GameState) => {
...
function enterDrunkenBlackout(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).clothingworntype ?? 0) === 'nude'  &&  ((s as any).pantyworntype ?? 0) === 'none'  &&  ((s as any).hunterVars ?? 0)?.['collective_opinion'] < 30  &&  ((s as any).hunterVars ?? 0)?.['sexual_comfort'] > 30) {
      (s as any).minut = ((s as any).minut ?? 0) + 23;
      (s as any).pcs_health = ((s as any).pcs_health ?? 0) - (10);
      qspCall(s, 'stat', '');
      scene.actions([{ label: 'Continue', goto: ['hunter_interactions', 'nighttime_gangbang'] }]);
    } else {
      (s as any).pcs_health = ((s as any).pcs_health ?? 0) - (15);
      qspCall(s, 'mood', 'lower', 'medium');
      qspCall(s, 'sleep_simple', 'forced', 176);
...
function enterMeetHunters(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Hi, I'm ${((s as any).pcs_nickname ?? 0)}" you introduce yourself meekly to one of the men`);
...
function enterMeetHuntersAfterRescue(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"${((s as any).pcs_nickname ?? 0)}, well, you can call me ${((s as any).pcs_nickname ?? 0)}." you answer.`);
  scene.text('"I\'m Andrei," he replied, "and these are my friends, from the city, Igor and Sergei."');
  // TODO-QSP: dynamic text: "Is your lastname possibly <<$pcs_lastname>>?" You look at him surprised for a m...
  scene.text(`"Is your lastname possibly ${((s as any).pcs_lastname ?? 0)}?" You look at him surprised for a moment before nodding.`);
  scene.text('"You look a lot like your mother that\'s how I knew. I am also from Gadukino."');
...
function enterShootingBet(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).tirand = ((s as any).rand ?? 0)(((s as any).tirkoef ?? 0) - ((s as any).tirmin ?? 0), ((s as any).tirkoef ?? 0) + ((s as any).tirmin ?? 0));
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'lose') {
    ...
    scene.actions([
      { label: 'Walk away', goto: ['gad_swamp_yard', 'start'] },
          ...
          { label: 'Let him lead you to the trees', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_favors', 'yard_bondage', 'lost_bet'
  } },
        ]);
      } else {
    ...
    qspCall(s, 'clothing', 'strip', 'gad_swamphouse');
    qspCall(s, 'underwear', 'remove');
    qspCall(s, 'arousal', 'flash', 10, 'exhibitionism', 'sub');
    qspCall(s, 'arousal', 'end');
  }, goto: ['gad_swamp_yard', 'start'] },
          { label: 'Refuse', handler: (st: GameState) => {
    (s as any).hunterVars['collective_opinion'] = ((s as any).hunterVars['collective_opinion'] ?? 0) + (5);
  ...
  scene.actions([
    ...
    if (((s as any).tirand ?? 0) > 70) {
      // TODO-QSP: gt 'hunter_interactions', 'shooting_bet', 'win'
    } else {
      // TODO-QSP: gt 'hunter_interactions', 'shooting_bet', 'lose'
    }
  } },
  ]);
  scene.build();
...
function enterAndreiQuickFuck(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'dinsex', 'vaginal_sex', (-30), 'sub', 'rough');
    scene.actions([
    ...
    qspCall(s, 'arousal', 'end');
    qspCall(s, 'cum_call', 'breasts', 'A172', 1);
    qspCall(s, 'cum_call', 'stomach', 'A172', 1);
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Further', handler: (st: GameState) => {
    if (((s as any).loc ?? 0) === 'gad_swamp_woods') {
      // TODO-QSP: gt 'gad_swamp_woods', 'shooting_practice', 'practice'
    } else {
...
function enterIgorDemonstration(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'flash', 25, 'dom', 'exhibitionism');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Further', goto: ['gad_swamp_yard', 'start'] },
    ]);
    ...
    qspCall(s, 'arousal', 'foreplay', 5, 'dom', 'exhibitionism');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Further', goto: ['gad_swamp_yard', 'start'] },
    ]);
  ...
  if (((s as any).hunterVars ?? 0)?.['KnowSlut'] === 0  &&  ((s as any).clothingworntype ?? 0) === 'nude'  &&  ((s as any).hour ?? 0) < 23  &&  ((s as any).hour ?? 0) >= 7) {
    ...
    if (((s as any).hunterVars ?? 0)?.['sexual_comfort'] < 40) {
      ...
      if (((s as any).hunterVars ?? 0)?.['collective_opinion'] > 45) {
        ...
        scene.text(`"You sure do look good, ${((s as any).pcs_nickname ?? 0)}" says Igor before pulling you in for a kiss.`);
        scene.actions([
    ...
    qspCall(s, 'arousal', 'fondling', 5, 'dom');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Further', goto: ['gad_swamp_yard', 'start'] },
    ]);
    ...
    qspCall(st, 'hunter_interactions', 'igor_demonstration');
          ...
          scene.actions([{ label: 'Continue', goto: ['hunter_interactions', 'andrei_quick_fuck'] }]);
        }
      }
      scene.actions([
        { label: 'Further', goto: ['gad_swamp_yard', 'start'] },
...
function enterNighttimeGangbang(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === '') {
    ...
    qspCall(s, 'arousal', 'vaginal', (-5), 'gangbang', 'rough');
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 3;
    qspCall(s, 'cum_call', 'mouth', 'A172', 1);
    qspCall(s, 'stat', '');
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 4;
    qspCall(s, 'arousal', 'vaginal', (-5), 'gangbang', 'exhibitionism', 'sub');
    qspCall(s, 'stat', '');
    ...
    qspCall(s, 'arousal', 'bj', (-5), 'gangbang', 'exhibitionism', 'sub');
    qspCall(s, 'arousal', 'vaginal', (-5), 'gangbang', 'exhibitionism', 'sub');
    qspCall(s, 'stat', '');
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(s, 'arousal', 'bj', 5, 'gangbang', 'exhibitionism', 'sub', 'deepthroat');
    qspCall(s, 'arousal', 'vaginal', 5, 'gangbang', 'exhibitionism', 'sub');
    qspCall(s, 'stat', '');
    ...
    qspCall(s, 'arousal', 'vaginal', (-10), 'gangbang', 'exhibitionism', 'sub');
    qspCall(s, 'stat', '');
    ...
    scene.text(`Sergei starts stroking himself while Igor keeps drilling your cunt. After a few minutes he grunts "${((s as any).pcs_nickname ?? 0)} I'm going to cum"`);
    scene.actions([
    ...
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Take care of Sergei', handler: (st: GameState) => {
    // TODO-QSP: gt 'hunter_interactions', 'nighttime_gangbang', 'finish'
  } },
    ]);
  } },
...
function enterStriptease(s: GameState, scene: SceneBuilder): void {
  if (((s as any).alko ?? 0) >= 8  &&  ((s as any).hunterVars ?? 0)?.['KnowSlut'] > 0) {
    ...
    qspCall(s, 'arousal', 'striptease', 10);
    (s as any).inhib_exp = ((s as any).inhib_exp ?? 0) + (Math.floor(Math.random() * 2) + 0);
    qspCall(s, 'stat', '');
    scene.img('images/locations/gadukino/hunters/dance1.\'+rand(1, 2)+\'.jpg');
...
function enterHuntersmokBJ(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).temp_sh = Math.floor(Math.random() * 2) + 1;
  qspCall(s, 'stat', '');
  scene.img('images/locations/gadukino/sex/hunter/hantersmokbj1.\'+rand(1, 3)+\'.jpg');
  ...
  if (((s as any).temp_sh ?? 0) === 2) {
    ...
    // TODO-QSP: dynamic text: "You're really good at this, <<$pcs_nickname>>," said a pleased Andrei. "You're ...
    scene.text(`"You're really good at this, ${((s as any).pcs_nickname ?? 0)}," said a pleased Andrei. "You're the perfect stress reliever.`);
...
function enterCreeksideGangbang(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'boyStat', 'A174');
    qspCall(s, 'oral', 'start', 1);
    qspCall(s, 'boyStat', 'A172');
    qspCall(s, 'oral', 'start', 1);
    qspCall(s, 'boyStat', 'A173');
    qspCall(s, 'dinsex', 'vaginal_sex', 5);
    qspCall(s, 'arousal', 'bj', 4, 'sub', 'gangbang');
    qspCall(s, 'arousal', 'vaginal', 4, 'sub');
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Further', handler: (st: GameState) => {
    ...
    qspCall(s, 'dinsex', 'sexcumsk');
    // TODO-QSP: dynamic text: You feel <<$boydesc>>'s cock twitch inside your pussy and realize he just came
    scene.text(`You feel ${((s as any).boydesc ?? 0)}'s cock twitch inside your pussy and realize he just came`);
    qspCall(s, 'arousal', 'bj', 3, 'sub', 'gangbang', 'deepthroat');
    qspCall(s, 'arousal', 'vaginal', 3, 'sub', 'gangbang');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.text(`After a few minutes of ${((s as any).boydesc ?? 0)} vigorously fucking your pussy, you feel him cumming inside you`);
    qspCall(s, 'cum_call', '', '', ((s as any).boy ?? 0), 1);
    scene.actions([
