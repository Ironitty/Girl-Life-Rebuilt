...
function enterV3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 3, 'deepthroat', 'rough');
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'vaginal', 6, 'rough', 'sub', 'humiliation');
    ...
    qspCall(st, 'arousal', 'foreplay', 1, 'sub', 'humiliation');
    qspCall(st, 'arousal', 'end');
    (st as any).sexvolume = 10;
    (st as any).spafinloc = 12;
    qspCall(st, 'cum_manage', '');
    (st as any).sexvolume = 18;
    (st as any).spafinloc = 11;
    qspCall(st, 'cum_manage', '');
    (st as any).sexvolume = 12;
