...
function enterReward(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You do your best to hold back from gagging, not wanting to interrupt Niko's orgasm. Once he's finished, he catches his breath. "You're simply divine, ${((s as any).pcs_firstname || '')}."`);
    ...
    scene.text(`You start coughing erratically as Niko holds you in his arms. "${((s as any).pcs_firstname || '')}! Please tell me that you're okay?" He gently rubs your back as you continue coughing and Niko wipes away your tears. "I'm so sorry, ${((s as any).pcs_firstname || '')}. My brother is such a brutish oaf. I'll find a way to protect you from him, I promise."`);
...
function enterLunchChat(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"I heard some things about you, ${((s as any).pcs_nickname || '')}. Are they true?" Petia asks.`);
    ...
    scene.text(`You look at Niko, who has a wide smile on his face as he nods at you. "No need to be shy ${((s as any).pcs_nickname || '')}, tell him."`);
    ...
    scene.text(`You take a deep breaths as Petia chuckles. "Don't be shy ${((s as any).pcs_nickname || '')}. I know that you've always wanted to be with me. Now's your chance."`);
    ...
    scene.text(`You look up to Niko, who's counting the money. "You were outstanding, ${((s as any).pcs_firstname || '')}. We'll have the money we need in no time." He then kisses you on your forehead. "Do you think that you can give me a quickie before we head back?"`);
    ...
    scene.text(`Niko looks down at you. "Oh ${((s as any).pcs_firstname || '')}, you're the most perfect being that could ever exist."`);
...
function enterLunchChat2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Niko shoots his cum onto your ass before taking a few seconds to catch his breath. "You were amazing ${((s as any).pcs_firstname || '')}. So fucking hot."`);
...
function enterDiner1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: "<<$func('money', 'string_profit', 2000)>>," Niko replies and the biker scratche...
    ...
    scene.text(`You see the biker shake hands with Niko and give him the money. Niko counts the money and gives the biker an approving nod before turning to you. "Good job ${((s as any).pcs_nickname || '')}. We're making some good progre…"`);
    scene.actions([
      { label: 'Turn around', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    qspCall(s, 'stat', '');
    ...
    // TODO-QSP: dynamic text: Niko chuckles. "She's my girl and it will be <<$func('money', 'string_profit', 2...
    ...
    if (((s as any).sexcontra ?? 0) >= 3  ||  ((s as any).protect ?? 0) >= 2) {
...
function enterDinerExit(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: He hands you <<$func('money', 'string_profit', 2400)>> and you look up at him. "...
  ...
  scene.actions([
    ...
    scene.text(`Niko wraps his arm around your neck. "No need to apologize. I will always look after you. I love you ${((s as any).pcs_firstname || '')}."`);
...
function enterReward2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Niko places his hand on your face. "You're so beautiful ${((s as any).pcs_firstname || '')}, the perfect girl." You can feel your heart beating faster at his words.`);
    scene.actions([
      { label: 'Suck his dick', handler: (st: GameState) => {
    qspCall(s, 'arousal', 'bj', 5);
    qspCall(s, 'stat', '');
    scene.img('images/characters/pavlovsk/school/boy/fedor/fedorev/home/bj.mp4');
    // TODO-QSP: dynamic text: You start sucking on Niko's dick and he tilts his head back as he lets out a lig...
    scene.text(`You start sucking on Niko's dick and he tilts his head back as he lets out a light moan before looking down at you. "Come here, ${((s as any).pcs_firstname || '')}." He beckons you to come closer.`);
    scene.actions([
    ...
    // TODO-QSP: dynamic text: Niko has a guilty look on his face. "I love you too <<$pcs_firstname>>. You're v...
    scene.text(`Niko has a guilty look on his face. "I love you too ${((s as any).pcs_firstname || '')}. You're very precious to me."`);
...
function enterDiner2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: You give Niko a nervous smile as he places his hands on your cheeks. "Don't worr...
    scene.text(`You give Niko a nervous smile as he places his hands on your cheeks. "Don't worry ${((s as any).pcs_nickname || '')}. I'll make sure that the customer only go one, or two at a time. No more group activities unless you're cool with it."`);
    ...
    scene.text(`"${((s as any).pcs_firstname || '')}. Very cute name," he says. "It really suits you."`);
    ...
    scene.text(`"${((s as any).NikoSlutName || '')} huh?" he replies. "That is is a pretty name."`);
    scene.text('You give the men a friendly smile as one of them approaches Niko.');
    scene.actions([
      { label: 'Continue', goto: ['NikoSlut', 'Customer 1'] },
    ]);
...
function enterDinerExit2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You finally make it to your apartment, where Niko grabs onto your shoulder and flips you around before giving you a very passionate kiss before whispering in your ear. "I love you ${((s as any).pcs_firstname || '')}. Never forget it."`);
...
function enterNikoSearch(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: As you're leaving the cafeteria, you walk toward the restroom to check up on Nik...
  ...
  scene.actions([
    ...
    if (((s as any).kotovLoveQW ?? 0) < 0) {
      ...
      // TODO-QSP: dynamic text: "Come on <<$pcs_nickname>>, don't forget about me." You alternate between suckin...
      scene.text(`"Come on ${((s as any).pcs_nickname || '')}, don't forget about me." You alternate between sucking their dicks as they talk to each other about you.`);
    }
    scene.actions([
      { label: '', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 5;
...
function enterNikoSearch2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).kotovLoveQW ?? 0) < 0) {
      // TODO-QSP: dynamic text: "We can always fetch <<$pcs_firstname>>," Vitek suggests. "She won't dare say no...
      scene.text(`"We can always fetch ${((s as any).pcs_firstname || '')}," Vitek suggests. "She won't dare say no."`);
    } else {
      // TODO-QSP: dynamic text: "You can always look for <<$pcs_firstname>>," Vitek suggests. "She seems to know...
      scene.text(`"You can always look for ${((s as any).pcs_firstname || '')}," Vitek suggests. "She seems to know her place."`);
    }
    ...
    scene.text(`You wait for a few seconds after the boys leave and quietly exit the stall, peeking out of the restroom door to see no one around. You let out a quick sigh of relief as you walk toward the girl's restroom to wait out recess when you suddenly hear someone behind you. "Hey ${((s as any).pcs_nickname || '')}, just the girl we're looking for."`);
