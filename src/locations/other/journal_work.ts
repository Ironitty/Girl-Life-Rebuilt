...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).audition_ev ?? 0)?.['day'] === ((s as any).daystart ?? 0)) {
    ...
    if (((s as any).audition_ev ?? 0)?.['day'] - ((s as any).daystart ?? 0) > 1) {
      ...
      scene.text(`You are auditioning for the role of ${((s as any).role ?? 0)} in ${((s as any).audition_ev ?? 0)?.['day'] - ((s as any).daystart ?? 0)} days.`);
    }
    if (((s as any).actress_cast ?? 0) === 1) {
      if (((s as any).acting_ev ?? 0)?.['shoot_day'] === ((s as any).daystart ?? 0)) {
        ...
        scene.text(`You are scheduled to play the part of ${((s as any).role ?? 0)} ${((s as any).acting_ev ?? 0)?.['shoot_day'] - ((s as any).daystart ?? 0)} days from now.`);
