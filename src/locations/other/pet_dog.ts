...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Your dog's name is ${(((s as any).rex ?? 0)?.['name'] ?? '')} and taking care of him is your responsibility. He needs to be fed and walked two times per day and bathed once per week.` + (((!(((s as any).rex ?? 0)?.['count_walk'] + ((s as any).rex ?? 0)?.['count_feed'] + ((s as any).rex ?? 0)?.['count_bath']))) ? (' You have done all necessary chores.') : ('')));
  ...
  if (((s as any).rex ?? 0)?.['relationship'] < 10) {
    scene.text(`${(((s as any).rex ?? 0)?.['name'] ?? '')} ignores you when you approach. ` + ((((s as any).objects ?? 0)?.['chew'] === 0) ? ('His old chew toy has seen better days. Maybe it\'s time to buy him a new one?') : ('')));
  } else {
    if (((s as any).rex ?? 0)?.['relationship'] < 30) {
      scene.text(`${(((s as any).rex ?? 0)?.['name'] ?? '')} looks up when you approach. ` + ((((s as any).objects ?? 0)?.['chew'] === 0) ? ('His old chew toy has seen better days. Maybe it\'s time to buy him a new one?') : ('')));
    } else {
      if (((s as any).rex ?? 0)?.['relationship'] < 60) {
        scene.text(`${(((s as any).rex ?? 0)?.['name'] ?? '')} looks up and wags his tail` + ((((s as any).rex ?? 0)?.['relationship'] > 45) ? (',') : (' a bit')) + ' when you approach. ' + ((((s as any).objects ?? 0)?.['chew'] === 0) ? ('His old chew toy has seen better days. Maybe it\'s time to buy him a new one?') : ('')));
      } else {
        scene.text(`${(((s as any).rex ?? 0)?.['name'] ?? '')} jumps up and` + ((((s as any).rex ?? 0)?.['relationship'] > 80) ? (' happily ') : (' ')) + 'wags his tail when you approach. ' + ((((s as any).objects ?? 0)?.['chew'] === 0) ? ('His old chew toy has seen better days. Maybe it\'s time to buy him a new one?') : ('')));
      }
    }
  }
...
function enterExercise(s: GameState, scene: SceneBuilder): void {
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 'walk') {
    ...
    if (((s as any).daystage ?? 0) === 5) {
      scene.text(`<center><img ${((s as any).set_imgh ?? '')} src="images/characters/shared/rex/walk_night_` + (Math.floor(Math.random() * 3) + 0) + '.jpg"></center>');
      ...
      if ((!((s as any).sunWeather ?? 0))) {
        ...
        scene.text(`<center><img ${((s as any).set_imgh ?? '')} src="images/characters/shared/rex/walk_day_` + (Math.floor(Math.random() * 5) + 0) + '.jpg"></center>');
    ...
    qspCall(st, 'stat', '');
    scene.text(`<center><img ${((st as any).set_imgh ?? '')} src="images/characters/shared/rex/play_outside_` + (Math.floor(Math.random() * 4) + 0) + '.jpg"></center>');
    ...
    scene.actions([
      { label: 'Return home', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg', 'check');
  } },
    ]);
  } else {
    ((s as any).rex = (s as any).rex ?? {})['timer_walk'] = ((s as any).hour ?? 0);
