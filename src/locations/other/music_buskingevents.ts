...
function enterBusking(s: GameState, scene: SceneBuilder): void {
  ...
  qspCall(s, 'stat', '');
  scene.text('' + ((s as any).locArgs?.[1] ?? 0) + '_music += rand(0,3)');
  ...
  (s as any).ml_buskingtime = undefined;
  scene.actions([
...
function enterCalculateTips(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).ml_location_mod = undefined;
  (s as any).ml_time_mod = undefined;
  (s as any).ml_tipsmax = undefined;
  (s as any).ml_time_loc_mod = undefined;
  scene.build();
}
