...
function enterComputer(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).fuckornot ?? 0) >= 3) {
    ...
    (st as any).view_location = 'school_lesson';
  } },
    ]);
  }
  ...
  scene.actions([
    ...
    scene.text(`You get Vasily's attention by smiling at him when he looks in your direction before you lean back in your chair and stretch your arms back over your head, causing your ${((st as any).titsize ?? '')} breasts to thrust out as you do. ` + ((qspFunc(s, 'pcs_has_attr', 'body_tits_small')) ? ('Your uniform stretches around your small breasts to accent what little you do have.') : (((qspFunc(s, 'pcs_has_attr', 'body_tits_average')) ? ('Your uniform stretches around your ample breasts, displaying their size.') : ('Your uniform stretches to its limits, threatening to pop buttons.')))));
    ...
    if (((st as any).grupTipe ?? 0) === 3) {
      ...
      scene.text('He looks at the screen and doesn\'t react at first. He walks back to the front of the class, looks at the image a final time then puts the phone in his desk while shaking his head.' + ((((st as any).npc_rel ?? 0)?.['A69'] >+50) ? (' "You\'re better than this, Miss ' + ((st as any).pcs_lastname ?? '') + '. I urge you not to start down this path," he says before he continues his lesson.') : (' "I expected nothing less from you, ' + ((st as any).pcs_lastname ?? '') + '. You never fail to fall to your lowest potential," he says in disgust before he continues his lesson.')));
    }
    scene.actions([
      { label: 'Wait for the end of the lesson', handler: (st: GameState) => {
    qspCall(st, 'gschool_lessonsev1', '');
...
function enterMusic(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.img('images/locations/pavlovsk/school/classroom/tease.jpg');
    scene.text(`You get Lavrenti's attention by smiling at him when he looks in your direction before you lean back in your chair and stretch your arms back over your head, causing your ${((st as any).titsize ?? '')} breasts to thrust out as you do. ` + ((qspFunc(s, 'pcs_has_attr', 'body_tits_small')) ? ('Your uniform stretches around your small breasts to accent what little you do have.') : (((qspFunc(s, 'pcs_has_attr', 'body_tits_average')) ? ('Your uniform stretches around your ample breasts, displaying their size.') : ('Your uniform stretches to its limits, threatening to pop buttons.')))));
    scene.text('He watches you intently while you bite your lower lip looking all coy and seductive to him. Once you finish your stretch, you pretend to act all innocent, like you didn\'t do anything.');
