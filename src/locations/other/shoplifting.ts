...
function enterCity(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    qspCall(s, 'stat', '');
    scene.img(`images/locations/city/shared/shoplift/steal${((s as any).picrand ?? 0)}.jpg`);
    ...
    scene.text(`"Okay ${((s as any).pcs_nickname ?? 0)}, just act normal…" you sigh quietly as you step towards the exit, trying to not draw any attention towards yourself.`);
    ...
    if (((s as any).pcs_run ?? 0) > 70) {
      ...
      ((s as any).policeQW ?? {})['shoplift_caught'] = (((s as any).policeQW ?? {})['shoplift_caught'] ?? 0) + (1);
      scene.text('You keep running while the security guard follows close behind… You turn a corner and then quickly take another corner as you try to shake him off…');
...
function enterSecurityroom(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).policeQW ?? {})['shoplift_value'] = 1300;
  qspCall(s, 'stat', '');
...
function enterShow(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 10;
    (s as any).policeCalled = Math.floor(Math.random() * 13) + 0;
    qspCall(s, 'stat', '');
...
function enterBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'cum_call', 'breasts', 'Security guard');
    (s as any).spafinloc = 15;
    qspCall(s, 'arousal', 'hj', 2, 'dom');
    qspCall(s, 'stat', '');
    ...
    if (((s as any).policeCalled ?? 0) <= 3) {
      ((s as any).policeQW ?? {})['toldonguard'] = 2;
      scene.text('To your dismay, the security guard has not only called the manager but there\'s even a police officer standing there with them.');
      scene.text('The security guard smugly proclaims, "Here she is officer I had her locked up the whole time…" He turns to the manager, "Don\'t worry sir, I\'ve already recovered the video evidence…"');
      scene.text('The manager nods while the police officer approaches you resolutely, telling you to stand and turn around so he can handcuff you.');
      scene.text('You obediently turn around, and the police officer tightens his cuffs so they sit real tight. As he finishes, he grabs you by the arm and leads you away…');
...
function enterBribe(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: All of a sudden, he changes his posture, "I don't usually do this, but if you gi...
    scene.text('All of a sudden, he changes his posture, "I don\'t usually do this, but if you give me \' + $func(\'money\', \'string_price\', 5000) + \', then we\'re all good."');
...
function enterPunish(s: GameState, scene: SceneBuilder): void {
  ((s as any).policeQW ?? {})['toldonguard'] = 0;
  (s as any).minut = ((s as any).minut ?? 0) + 10;
  qspCall(s, 'stat', '');
  ...
  // TODO-QSP: dynamic text: <i>Fuck it's the same guard, now you've done it, <<$pcs_nickname>>, get prepared...
  scene.text(`<i>Fuck it's the same guard, now you've done it, ${((s as any).pcs_nickname ?? 0)}, get prepared for whatever awaits you… your fate is in his hands…</i>`);
  ...
  scene.actions([
    ...
    qspCall(s, 'pain', '', 4, 'breasts', 'twist');
    ...
    qspCall(s, 'npcgeneratec', '', 0, 'Security Guard', 42);
    qspCall(s, 'boyStat', '', ((s as any).npclastgenerated ?? 0));
    qspCall(s, 'arousal', 'bj', 5, 'sub', 'humiliation', 'deepthroat');
    qspCall(s, 'stat', '');
    ...
    scene.actions([
      { label: 'Comply', handler: (st: GameState) => {
    qspCall(s, 'pain', '', 7, 'vaginal', 'tear');
    qspCall(s, 'arousal', 'vaginal', 10, 'sub', 'humiliation', 'rough');
    qspCall(s, 'stat', '');
    ...
    qspCall(s, 'cum_call', 'breasts', 'Security guard');
    (s as any).spafinloc = 15;
    qspCall(s, 'arousal', 'vaginal', 5, 'sub', 'humiliation', 'rough');
    qspCall(s, 'stat', '');
...
function enterPav(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    { label: 'Attempt to steal makeup', handler: (st: GameState) => {
    qspCall(s, 'item_cart', 'shopping_var_clear');
    (s as any).observ_exp = ((s as any).observ_exp ?? 0) + (5);
    (s as any).picrand = Math.floor(Math.random() * 8) + 0;
    qspCall(s, 'stat', '');
    scene.img(`images/locations/city/shared/shoplift/steal${((s as any).picrand ?? 0)}.jpg`);
    scene.text('Standing in front of the make-up section of the supermarket, you nervously look over your shoulder to make sure none of the employees are able to spot you.');
...
function enterBjPav(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).policeQW ?? {})['securitybj_pav'] = (((s as any).policeQW ?? {})['securitybj_pav'] ?? 0) + (1);
  scene.actions([
    { label: 'Take picture', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 2;
    qspCall(s, 'stat', '');
