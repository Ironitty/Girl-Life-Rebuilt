...
function enter(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === 'loc_change') {
    ...
    if (((s as any).arrsize ?? 0)('trace_locations') === 0) {
      // TODO-QSP: exit
    }
    if (((s as any).arrpos ?? 0)("((s as any).trace_locations ?? 0)", ((s as any).locArgs?.[2] ?? 0)) === -1) {
      // TODO-QSP: exit
    }
  ...
  if (((s as any).debug ?? 0)?.['trace_index'] < ((s as any).arrsize ?? 0)("((s as any).trace_args ?? 0)")) {
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 'trace_list_locs') {
    ...
    if (((s as any).tmp_i ?? 0) < ((s as any).arrsize ?? 0)('trace_locations')) {
