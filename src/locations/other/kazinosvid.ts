...
function enter1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`While playing with ${((s as any).telsob ?? 0)}, two guys walk up your table.`);
  ...
  scene.actions([
    ...
    scene.img(`images/characters/city/boyfriend/sex/event/cards/${((s as any).picrand ?? 0)}.jpg`);
    ...
    if (((s as any).randkazinv ?? 0) === 1) {
      ...
      scene.text(`These guys didn't stand a chance. You won ${qspFunc(s, 'money', 'string_profit', ((s as any).randkazinvsum ?? 0))}. As you and ${((s as any).telsob ?? 0)} are about to leave, the guys beg you stay: "Come on girls, you can't just take our money and run. Sit down have another drink. We have entertainments other than cards to offer."<br>From the bulge is his pants, It's pretty clear what kind of entertainment he has in mind.`);
      ...
      if (((s as any).telsobnastr ?? 0) < 4) {
        ...
        scene.text(`No, it's time we were on our way," says ${((s as any).telsob ?? 0)}. She takes you by the hand, and leads you out of the room.`);
        scene.actions([
        ...
        scene.text(`"I wouldn't mind consoling the losers," says ${((s as any).telsob ?? 0)}, with a lustful look in her eye. One of the men starts to unbutton his fly.`);
        scene.actions([
    ...
    // TODO-QSP: xgt 'sexdvanadva', 'var'
      ...
      scene.text(`Those guys were better card players than they looked. You lost ${qspFunc(s, 'money', 'string_price', ((s as any).randkazinvsum ?? 0))} to them.`);
      if (qspFunc(s, 'money', 'can_afford', ((s as any).randkazinvsum ?? 0), 'cash') === 0) {
        ...
        scene.text(`You and ${((s as any).telsob ?? 0)} don't even have enough cash to pay them. It looks like you have no choice other than spreading your legs for them.`);
        scene.actions([
    ...
    // TODO-QSP: xgt 'sexdvanadva', 'var'
        ...
        if (((s as any).telsobnastr ?? 0) < 4) {
          ...
          scene.text(`"Here's your money, we are leaving," says ${((s as any).telsob ?? 0)}. She takes you by the hand, and leads you out of the room.`);
          scene.actions([
            { label: 'Finish', goto: ['kazinosvid', '3'] },
          ]);
        } else {
          // TODO-QSP: dynamic text: "I wouldn't mind an alternative payment plan," says <<$telsob>>, smiling lascivi...
          scene.text(`"I wouldn't mind an alternative payment plan," says ${((s as any).telsob ?? 0)}, smiling lasciviously.`);
...
function enter2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`While you are playing with ${((s as any).telsob ?? 0)}, you attract the attention of a man.`);
  ...
  scene.actions([
    ...
    scene.text(`${((s as any).telsob ?? 0)} giggled, "You may end up naked and ashamed, ${((s as any).pcs_nickname ?? 0)} and I haven't lost a hand yet."`);
    scene.text('"I have nothing to be ashamed of," the man grins.');
    qspCall(s, 'willpower', 'misc', 'self', 'medium');
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Take <<$telsob>> and leave [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
    ...
    scene.text(`It's not until you and ${((s as any).telsob ?? 0)} are down to your panties that you think this guy might have been sandbagging. He still has all his clothes!`);
    // TODO-QSP: dynamic text: <<$telsob>> pouts, "I know we agreed, but you have to cheating."
    scene.text(`${((s as any).telsob ?? 0)} pouts, "I know we agreed, but you have to cheating."`);
    ...
    scene.text(`"Fine," ${((s as any).telsob ?? 0)} says, with an exaggerated sigh. "${((s as any).pcs_nickname ?? 0)} help me remove my panties."`);
    scene.actions([
