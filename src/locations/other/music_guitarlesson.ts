...
function enterEnquiry(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You shift on your feet "I'm ${((s as any).pcs_firstname ?? 0)}, and yes, I think. Can you please tell me a bit more?"`);
  ...
  scene.actions([
    ...
    scene.text(`The teacher nods "That's understandable. Just come in when you know and we can set up the lesson" He smiles at you "It was a pleasure to meet you, ${((s as any).pcs_firstname ?? 0)}, and I'm looking forward to seeing you again."`);
    scene.actions([
...
function enterGoodbye(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).ml_guitarlesson ?? 0)?.['lessonday'] < 8) {
    ...
    scene.text(`You agree to meet for your next lesson on ${((s as any).ml_guitarlesson ?? 0)?.['lessonday']} at ${((s as any).ml_guitarlesson ?? 0)?.['lessonhour']}:00 next week, on ${((s as any).newday ?? 0)}/${((s as any).newmonth ?? 0)}. The teacher enters it in the diary and you say goodbye before you leave.`);
