...
function enterClothesStart1(s: GameState, scene: SceneBuilder): void {
  ...
  if ((Math.floor(Math.random() * 2) + 0) ===1) {
    ...
    (s as any).store_choice = undefined;
...
function enterClothesStart2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if ((Math.floor(Math.random() * 2) + 0) === 1) {
      ...
      (st as any).store_choice = undefined;
      scene.text('"Where do you want to go?" Katja asks.');
      scene.actions([
        { label: 'Go to Pussy-Cats', goto: ['katja_city', 'Pussy-Cats'] },
      ]);
...
function enterPussy_Cats(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('You look at the price tag and see that it costs ' + qspFunc(s, 'money', 'string_price', ((st as any).price ?? '')) + '.');
...
function enterPussyCatOutfit1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    dynamicGoto(st, 'prevLoc', 'prevArg');
      ...
      scene.text('"Come on, turn around and let me see you from behind," you say.');
      scene.text(`iif(katjaQW['slut'] >= 80, '"Of course," she says. "I'm sure you'll enjoy the view!"', '"Really, ${((st as any).pcs_nickname ?? '')}?!"')`);
      ...
      scene.text(((((st as any).katjaQW ?? 0)?.['slut'] >= 80) ? ('"Thanks!" she says. "These will really make me the center of attention at a party."') : ('"Don\'t say that! It\'s really annoying how they creep on my butt!"')) + ' She then turns around to face you again.');
      scene.actions([
...
function enterPussyCatOutfit2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    dynamicGoto(st, 'prevLoc', 'prevArg');
      ...
      scene.text('"So do your panties match the bra?" you ask.');
      scene.text(`iif(katjaQW['slut'] >= 80, '"Are you assuming I'm wearing panties?" she asks slyly.', '"Really, ${((st as any).pcs_nickname ?? '')}?! You don't ask a good girl such a question!" she says with a wink.')`);
      ...
      if (((st as any).katjaQW ?? 0)?.['pantiesQWstage'] >= 5) {
        ...
        scene.text(((((st as any).pantyworntype ?? 0) === 'none') ? ('"You should. It\'s very liberating," you tell her as you flash her your naked pussy') : ('"Why am I not completely convinced that you\'re telling the truth?" you say')) + ' and her face turns even redder.');
      }
