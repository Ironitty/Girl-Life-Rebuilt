...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).PCloStyle2 ?? 0) === 4  ||  ((s as any).PCloSchool ?? 0) === 1) {
    ...
    scene.text(`"Oh really." After a moment of silence ${((s as any).npcdesc || '')} finally speaks up; "Don't take this the wrong way but what school do you attend?" she asks.`);
    scene.actions([
    ...
    scene.text(`${((s as any).npcdesc || '')} clearly upset by your question just turns around and walks away without saying another word.`);
    ...
    if (((s as any).loc ?? 0) === 'pav_residential'  ||  ((s as any).loc ?? 0) === 'pav_commercial') {
      ...
      scene.text(`Staying quiet for a moment, ${((s as any).npcdesc || '')} is contemplating what she should answer, finally deciding, "Sure, whatever you want, I can't say no to a girl in a hot school outfit…"`);
      // TODO-QSP: dynamic text: You tell <<$npcdesc>> what school you attend. "I know of that school, it's known...
      scene.text(`You tell ${((s as any).npcdesc || '')} what school you attend. "I know of that school, it's known for its beautiful girls." You blush a little as she's definitely flirting with you. "May I entertain you for the rest of the day?" she asks.`);
    } else {
    ...
    scene.text(`${((s as any).npcdesc || '')} looks at you disappointed. "Oh, that's a shame. Could I get-…"`);
    ...
    scene.text(`"Great!" answers ${((s as any).npcdesc || '')} "So, what do you want to do?".`);
    ...
    scene.text(`"I say that's a great idea. I'll be in touch." ${((s as any).npcdesc || '')} adds your number to her contact list.`);
    ...
    scene.text(`You give her a contemptuous look. "Oh, you have me all figured out do you? My name is ${((s as any).pcs_nickname || '')}, not that it is any of your business."`);
    if (((s as any).loc ?? 0) === 'pav_residential'  ||  ((s as any).loc ?? 0) === 'pav_commercial') {
    ...
    scene.text(`She laughs at your response. "Thanks. I'll try to make sure to call you when you're really bored." ${((s as any).npcdesc || '')} adds your number to her contact list.`);
    ...
    scene.text(`${((s as any).npcdesc || '')} looks at you in shock as she was not ready for you to react that way. "Well, I thought we could just have a little fun together…"`);
    // TODO-QSP: dynamic text: As you're getting more annoyed you give her a shove making her back up a couple ...
    scene.text(`As you're getting more annoyed you give her a shove making her back up a couple of steps. "Get the fuck out of here before I kick your ass!" ${((s as any).npcdesc || '')} seeing she's getting nowhere moves away quickly.`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
  } },
    ...
    scene.text(`${((s as any).npcdesc || '')} quickly scribbles it down not knowing you've just given her a totally unknown number.`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
  } },
    ...
    scene.text(`You think ${((s as any).npcdesc || '')} is cute-looking and even though you're not sure about this you decide grasping her hand.`);
    ...
    scene.text(`You break out in a smile telling her, "My name is ${((s as any).pcs_nickname || '')}."`);
    // TODO-QSP: dynamic text: "What a beautiful name and a loving smile." <<$npcdesc>> tells you.
    scene.text(`"What a beautiful name and a loving smile." ${((s as any).npcdesc || '')} tells you.`);
    // TODO-QSP: dynamic text: You start blushing as <<$npcdesc>> compliments you… "So what do you say <<$pcs_n...
    scene.text(`You start blushing as ${((s as any).npcdesc || '')} compliments you… "So what do you say ${((s as any).pcs_nickname || '')} would you like to go on a date with me?"`);
    scene.actions([
      { label: 'Decline', goto: ['dateF', 'decline'] },
      { label: 'I would love to', handler: (st: GameState) => {
    scene.img(`${((s as any).npc_pic ?? 0)?.[String((s as any).npcID ?? 0)] ?? ''}`);
    ...
    scene.text(`"Yeah, you heard me." said ${((s as any).npcdesc || '')}. "Don't act all that surprised, you're good-looking, I'm good-looking we would be the perfect couple, so what do you say?"`);
    ...
    scene.text(`${((s as any).npcdesc || '')} noticing that you're having some trouble, smiles and tells you, "I'll ask you one more time, I don't want to come off as desperate." she says laughing.`);
    ...
    scene.text(`"${((s as any).npcdesc || '')}! And yours?" she asks you confidently, seeing a glimmer of hope.`);
    ...
    scene.text(`"Just as I expected a beautiful name for a beautiful girl…" ${((s as any).npcdesc || '')} quickly replies.`);
    scene.actions([
      { label: 'Decline', handler: (st: GameState) => {
    scene.img(`${((s as any).npc_pic ?? 0)?.[String((s as any).npcID ?? 0)] ?? ''}`);
    ...
    scene.text(`${((s as any).npcdesc || '')} looks at you disappointed. "Oh well, I didn't think I had a chance but you can't blame me for trying."`);
    ...
    scene.text(`"Great!" answers ${((s as any).npcdesc || '')} "So, what do you want to do gorgeous?".`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterDateChoice(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
      { label: 'I would love to but can\'t right now', handler: (st: GameState) => {
    qspCall(s, 'lover', 'add_girlfriend', ((s as any).npcID ?? 0));
...
function enterDatepark(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The two of you sit down and begin commenting on the people walking by. You laugh while ${((s as any).npcdesc || '')} makes up stories about their backgrounds and quirks.`);
    // TODO-QSP: dynamic text: There are a few times you almost get caught as you point and laugh. As you're si...
    scene.text(`There are a few times you almost get caught as you point and laugh. As you're sitting down an ice cream pushcart appears, generating quite a buzz around it. ${((s as any).npcdesc || '')} asks you, "I sure could go for some ice cream. Would you also like some ice cream?"`);
    scene.actions([
    ...
    scene.text(`${((s as any).npcdesc || '')} runs away to the pushcart and comes back with two ice creams. "Here you go, ${((s as any).pcs_nickname || '')} I hope you'll enjoy it."`);
    // TODO-QSP: dynamic text: You nod and thank her. The two of you continue talking while eating the ice crea...
    scene.text(`You nod and thank her. The two of you continue talking while eating the ice cream. As you finish ${((s as any).npcdesc || '')} asks, "Shall we move on?"`);
    scene.actions([
    ...
    scene.text(`You don't even notice that the two of you strayed off the beaten path, all of a sudden ${((s as any).npcdesc || '')} stops and whispers in your ear, "Now that we're at a secluded spot, you know what would make this date even better?"`);
    scene.actions([
      { label: 'What?', handler: (st: GameState) => {
    scene.img('images/locations/shared/datef/park5.jpg');
    // TODO-QSP: dynamic text: Before you're even able to respond <<$npcdesc>> leans in and kisses you on the l...
    scene.text(`Before you're even able to respond ${((s as any).npcdesc || '')} leans in and kisses you on the lips.`);
    scene.actions([
    ...
    scene.text(`Feeling frisky you kiss ${((s as any).npcdesc || '')} back. Not before long the two of you are having a make out session, french kissing each other.`);
    ...
    // TODO-QSP: dynamic text: She shakes her head. "No, not at all. I'm not going to make you do something you...
    scene.text(`She shakes her head. "No, not at all. I'm not going to make you do something you don't want." ${((s as any).npcdesc || '')} answers.`);
    scene.actions([
      { label: 'Walk back', goto: ['dateF', 'walk_back'] },
    ]);
  } },
...
function enterWalkBack(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: You give <<$npcdesc>> your number. "Now I really hope you don't forget to call m...
    scene.text(`You give ${((s as any).npcdesc || '')} your number. "Now I really hope you don't forget to call me." you tell her. "Oh, don't worry about that I'll be in touch." she says.`);
    ...
    scene.text(`${((s as any).npcdesc || '')} disappointingly looks at you, "Well at least I gave it my best shot who knows maybe I'll meet you once again."`);
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
  } },
...
function enterDatebar(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`As you sit down ${((s as any).npcdesc || '')} asks you, "Would you like to drink something?"`);
    scene.actions([
    ...
    scene.text(`"My kind of girl." ${((s as any).npcdesc || '')} tells you.`);
    scene.actions([
...
function enterBartalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The band steps out and starts playing. You're amazed over how well they play and turn to ${((s as any).npcdesc || '')} "They are pretty good don't you think?"`);
    // TODO-QSP: dynamic text: <<$npcdesc>> nods her head. "Yeah they are, do you want another drink?"
    scene.text(`${((s as any).npcdesc || '')} nods her head. "Yeah they are, do you want another drink?"`);
    scene.actions([
      { label: 'Sure', handler: (st: GameState) => {
    qspCall(s, 'drugs', 'alcohol', 'vodka');
    (s as any).minut = ((s as any).minut ?? 0) + 20;
    ...
    // TODO-QSP: dynamic text: "Well, I'm having a great time so why not." <<$npcdesc>> goes over to the counte...
    scene.text(`"Well, I'm having a great time so why not." ${((s as any).npcdesc || '')} goes over to the counter and returns with two new drinks.`);
    ...
    scene.text(`"Would you like to dance?" you ask ${((s as any).npcdesc || '')}.`);
    ...
    scene.text(`You get up from the chair and walk up near the stage and begin moving sensually, teasing ${((s as any).npcdesc || '')}.`);
    scene.actions([
    ...
    scene.text(`Recognizing ${((s as any).npcdesc || '')} voice, you come to your senses and open the door, quickly pulling her in.`);
    ...
    // TODO-QSP: dynamic text: <<$npcdesc>> doesn't seem to mind your brashness. She begins grabbing you all ov...
    scene.text(`${((s as any).npcdesc || '')} doesn't seem to mind your brashness. She begins grabbing you all over your body, adding even more to your arousal.`);
    ...
    scene.text(`${((s as any).npcdesc || '')} arrives shortly after you to the table. The whole situation is too awkward now and you begin to blush as you're really ashamed of your actions. You're starting to panic a bit…`);
    scene.actions([
    ...
    qspCall(s, 'stat', '');
    scene.img('images/locations/shared/datef/bar2.jpg');
    // TODO-QSP: dynamic text: You and <<$npcdesc>> continue talking even if it's hard to hear what she says at...
    scene.text(`You and ${((s as any).npcdesc || '')} continue talking even if it's hard to hear what she says at some times because of the loud music.`);
