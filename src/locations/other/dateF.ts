...
function enterDatepark(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`The two of you sit down and begin commenting on the people walking by. You laugh while ${((s as any).npcdesc ?? 0)} makes up stories about their backgrounds and quirks.`);
    // TODO-QSP: dynamic text: There are a few times you almost get caught as you point and laugh. As you're si...
    scene.text(`There are a few times you almost get caught as you point and laugh. As you're sitting down an ice cream pushcart appears, generating quite a buzz around it. ${((s as any).npcdesc ?? 0)} asks you, "I sure could go for some ice cream. Would you also like some ice cream?"`);
    scene.actions([
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} runs away to the pushcart and comes back with two ice creams. "Here you go, ${((s as any).pcs_nickname ?? 0)} I hope you'll enjoy it."`);
    // TODO-QSP: dynamic text: You nod and thank her. The two of you continue talking while eating the ice crea...
    scene.text(`You nod and thank her. The two of you continue talking while eating the ice cream. As you finish ${((s as any).npcdesc ?? 0)} asks, "Shall we move on?"`);
    scene.actions([
    ...
    scene.text(`You don't even notice that the two of you strayed off the beaten path, all of a sudden ${((s as any).npcdesc ?? 0)} stops and whispers in your ear, "Now that we're at a secluded spot, you know what would make this date even better?"`);
    scene.actions([
    ...
    scene.text(`Before you're even able to respond ${((s as any).npcdesc ?? 0)} leans in and kisses you on the lips.`);
    scene.actions([
    ...
    scene.text(`Feeling frisky you kiss ${((s as any).npcdesc ?? 0)} back. Not before long the two of you are having a make out session, french kissing each other.`);
    ...
    scene.text(`She shakes her head. "No, not at all. I'm not going to make you do something you don't want." ${((s as any).npcdesc ?? 0)} answers.`);
    scene.actions([
      { label: 'Walk back', goto: ['dateF', 'walk_back'] },
    ]);
  } },
...
function enterWalkBack(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You give ${((s as any).npcdesc ?? 0)} your number. "Now I really hope you don't forget to call me." you tell her. "Oh, don't worry about that I'll be in touch." she says.`);
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} disappointingly looks at you, "Well at least I gave it my best shot who knows maybe I'll meet you once again."`);
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    dynamicGoto(st, 'loc', 'loc_arg');
  } },
...
function enterDatebar(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"How about going to a bar?" you ask ${((s as any).npcdesc ?? 0)}.`);
  ...
  scene.actions([
    ...
    scene.text(`As you sit down ${((s as any).npcdesc ?? 0)} asks you, "Would you like to drink something?"`);
    scene.actions([
    ...
    scene.text(`"My kind of girl." ${((s as any).npcdesc ?? 0)} tells you.`);
    scene.actions([
...
function enterBartalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} goes to the bar counter and after a while returns with the drinks.`);
  ...
  scene.actions([
    ...
    scene.text(`The band steps out and starts playing. You're amazed over how well they play and turn to ${((s as any).npcdesc ?? 0)} "They are pretty good don't you think?"`);
    // TODO-QSP: dynamic text: <<$npcdesc>> nods her head. "Yeah they are, do you want another drink?"
    scene.text(`${((s as any).npcdesc ?? 0)} nods her head. "Yeah they are, do you want another drink?"`);
    scene.actions([
      { label: 'Sure', handler: (st: GameState) => {
    qspCall(s, 'drugs', 'alcohol', 'vodka');
    (s as any).minut = ((s as any).minut ?? 0) + 20;
    ...
    scene.text(`"Well, I'm having a great time so why not." ${((s as any).npcdesc ?? 0)} goes over to the counter and returns with two new drinks.`);
    ...
    scene.text(`"Would you like to dance?" you ask ${((s as any).npcdesc ?? 0)}.`);
    ...
    scene.text(`You get up from the chair and walk up near the stage and begin moving sensually, teasing ${((s as any).npcdesc ?? 0)}.`);
    scene.actions([
    ...
    scene.text(`Recognizing ${((s as any).npcdesc ?? 0)} voice, you come to your senses and open the door, quickly pulling her in.`);
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} doesn't seem to mind your brashness. She begins grabbing you all over your body, adding even more to your arousal.`);
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} arrives shortly after you to the table. The whole situation is too awkward now and you begin to blush as you're really ashamed of your actions. You're starting to panic a bit…`);
    scene.actions([
    ...
    scene.text(`You and ${((s as any).npcdesc ?? 0)} continue talking even if it's hard to hear what she says at some times because of the loud music.`);
