...
function enterAfricanStudiesDjibrilNushBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Djibril gets up and lays down on the floor, before grabbing Anushka and pulling her suddenly over to him, she sequels a moment in surprise as he manhandles her until she is straddling him and then pushes her down. As she lowers herself down, she grabs his ${((st as any).dick ?? '')}cm ${((st as any).dick_girth ?? '')} cock with one hand and lines it up with her pussy. She slowly impales herself, you watch as his black cock slowly disappears inside of her white pussy. She moans loudly as she takes more and more of his cock inside of her. "OH FUCK!… Your so big…" She looks at you, she looks so turned on, as she leans a bit towards you, while she rides Djibril.`);
    ...
    qspCall(st, 'arousal', 'bj', 1, ((st as any).npcID ?? 0));
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterAfricanStudiesDjibrilNushRidehim(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', 2, ((st as any).npcID ?? 0));
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Watch what happens next', handler: (st: GameState) => {
...
function enterAfricanStudiesDjibrilNushGetfucked(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', 2, ((st as any).npcID ?? 0));
    qspCall(st, 'arousal', 'cuni', (-2), ((st as any).npcID1 ?? 0));
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'hj', 1, ((st as any).npcID ?? 0));
    qspCall(st, 'arousal', 'end');
    qspCall(st, 'cum_call', 'face', ((st as any).npcID ?? 0), 1);
    qspCall(st, 'cum_cleanup', 'head');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterAfricanStudiesDjibrilNushCleanup(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`A few minutes later, Anushka finishes getting herself all cleaned up and and clothes back in order. She gives you a knowing little smile. "Well that was fun, at least I had fun. Did you have fun ${((st as any).pcs_nickname ?? '')}?"`);
