...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('<center><b>Apartment manager Ilyushkin</b></center>');
  scene.img('images/locations/shared/apartment/podezdhr.jpg');
  scene.text('Ilyushkin\'s apartment door is made of steel and held in place with strong locks.');
  if (((s as any).start_type ?? 0)?.['magic'] !== 'nomagic') {
    if (((s as any).poverKAR ?? 0) === 1) {
    ...
    qspCall(st, 'stat', '');
    scene.img('images/system/1_openings/shared/npc_tatiana.jpg');
    scene.text('Together with Tatiana you leave the apartment and Ilyushkin closes the door behind you. Tatiana angrily slammed her fist into palm. "Well damn it to hell that was a complete failure and a huge waste of time."');
    scene.actions([
      { label: 'Go out of the house', goto: ['city_industrial', ''] },
    ]);
  } },
    ]);
  } },
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/industrial/aptmanagers/sex/threesome3.jpg');
    scene.text('Tatiana pulls Ilyushkin down onto his back and, tearing off her clothes, jumps onto his cock. While Tatiana furiously rides his dick, you caress his balls and stroke Tatiana\'s back and ass.');
    scene.actions([
      { label: 'Sit on Ilyushkin\'s face', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/industrial/aptmanagers/sex/threesome4.jpg');
    scene.text('While Tatiana continues to ride his dick, you expose your pussy and sit down on Ilyushkin\'s face. Ilyushkin begins to lick your pussy, and incredible bliss envelops you. After several minutes and several small orgasms, you climb off Ilyushkin\'s face, pushing Tatiana off Ilyushkin. You push her down on the floor next to Ilyushkin and lower yourself onto her.');
    qspCall(st, 'arousal', 'cuni', 5, ((st as any).npcID ?? 0), 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: '69 Tatiana', handler: (st: GameState) => {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).cumface = ((st as any).cumface ?? 0) + (1);
    scene.img('images/shared/sex/group/cum/threesome6.jpg');
    scene.text('You kneel together with Tatiana as Ilyushkin begins to jerk off in front of your faces. Suddenly, Ilyushkin clenches up, and cum flies out of his cock, hot jets of sperm hitting you both in the face. Ilyushkin, finally dried out, retreats to the couch trying to catch his breath. You take a few minutes to lick the cum off of each other\'s faces. Tatiana whispers to you, cameras ready.');
    qspCall(st, 'cum_call', 'face', ((st as any).npcID ?? 0), 1);
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Interrogate Ilyushkin', handler: (st: GameState) => {
    (st as any).softKAR = 10;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWEvidence'] = 1;
    ...
    if (((s as any).softKAR ?? 0) === 1) {
      scene.text('You arrive at Ilyushkin\'s apartment and see Tanya is already there. She adjusts her glasses and says, "Ilyushkin is in his apartment. Well, ready?"');
      scene.actions([
        { label: 'What\'s the plan?', handler: (st: GameState) => {
    (st as any).pcs_horny = 0;
    qspCall(st, 'stat', '');
    scene.img('images/system/1_openings/shared/npc_tatiana.jpg');
    scene.text('Tanya starts to explain, "In your disguise, Ilyushkin won\'t recognize you. We have to seduce him. For that, I have the means. This aphrodisiac is strong enough he just has to smell it and immediately he\'ll be horny. The main thing is we need to keep ourselves out of the vapor. Then when he is ready, we\'ll offer him a drink. That should untie his tongue. We\'ll record his confession on camera, and that\'s it."');
    scene.actions([
      { label: 'Let\'s begin', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (10);
    qspCall(st, 'stat', '');
    scene.img('images/system/1_openings/shared/npc_tatiana.jpg');
    scene.text('Tanya says, "Here, take this bottle, it\'s the aphrodisiac. Careful with that. Remember it will affect us as well." Tanya gives you the bottle, and you splash a little on the frame of the door, trying not to inhale the aroma. Tanya rings the bell and waits a few minutes for Ilyushkin to answer it. Appearing on the threshold, he looks a little perplexed at you. Tanya smiles sweetly at Ilyushkin, "We are conducting a social survey. Can we ask you a few questions?" Ilyushkin stands aside, inviting you to go into the apartment.');
    scene.actions([
      { label: 'Go into the apartment', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (50);
    qspCall(st, 'stat', '');
    scene.img('images/system/1_openings/shared/npc_tatiana.jpg');
    scene.text('Together with Tanya, you enter the apartment and follow Ilyushkin into his living room. The room is clean and comfortable. Ilyushkin asks you to sit down on the sofa and sits down in front of you in another chair. Tanya begins to ask Ilyushkin questions about sex. Ilyushkin looks surprisingly calm for a man under the impact of an aphrodisiac. You try not to breathe and can still feel its effects. Tanya nudges you with her elbow and looks toward your bottle.');
    scene.actions([
      { label: 'Drizzle secretly', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (25);
    qspCall(st, 'stat', '');
    scene.img('images/system/1_openings/shared/npc_tatiana.jpg');
    scene.text('While Tanya is distracting Ilyushkin showing him her neckline, you discretely spread the aphrodisiac in the air and feel yourself become excited. Tanya, all the more tempting and seductive, talks with Ilyushkin, trying to seduce him. You even noticed her cheeks flush. But Ilyushkin reacts rather calmly to what is happening. He looks at you with interest, but you do not notice any signs of excitement. Meanwhile, Tanya again jabs you with an elbow in the ribs.');
    scene.actions([
      { label: 'Splash again', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (25);
    ...
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/industrial/aptmanagers/sex/threesome3.jpg');
    scene.text('Tanya pulls Ilyushkin down onto his back and, tearing off her clothes, jumps onto his cock. While Tanya furiously rides his dick, you caress his balls and stroke Tanya\'s back and ass.');
    scene.actions([
      { label: 'Sit on Ilyushkin\'s face', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/industrial/aptmanagers/sex/threesome4.jpg');
    scene.text('While Tanya continues to ride his dick, you expose your pussy and sit down on Ilyushkin\'s face. Ilyushkin begins to lick your pussy, and incredible bliss envelops you. After several minutes and several small orgasms, you climb off Ilyushkin\'s face, pushing Tanya off Ilyushkin. You push her down on the floor on her back next to Ilyushkin and lower yourself onto her.');
    qspCall(st, 'arousal', 'cuni', 5, ((st as any).npcID ?? 0), 'group');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: '69 Tanya', handler: (st: GameState) => {
    qspCall(st, 'arousal_funcs', 'stretch', 'vaginal');
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/industrial/aptmanagers/sex/threesome5.jpg');
    ...
    (st as any).cumface = ((st as any).cumface ?? 0) + (1);
    scene.img('images/shared/sex/group/cum/threesome6.jpg');
    scene.text('You kneel together with Tanya as Ilyushkin begins to jerk off in front of your faces. Suddenly, Ilyushkin clenches up and cum flies out of his cock, hot jets of sperm hitting you both in the face. Ilyushkin, finally dried out, retreats to the couch trying to catch his breath. You take a few minutes to lick the cum off of each other\'s faces. Tanya whispers to you, get the camera ready.');
    qspCall(st, 'cum_call', 'face', ((st as any).npcID ?? 0), 1);
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Interrogate Ilyushkin', handler: (st: GameState) => {
...
export const ilyroom: LocationDef = {
  name: 'ilyroom',
  title: 'Apartment manager Ilyushkin',
  region: 'other',
  description: ['Ilyushkin\'s apartment door is made of steel and held in place with strong locks.'],
  enter: enter,
};
