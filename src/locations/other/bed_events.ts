...
function enterRab2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'bj', 5, 'sub', 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    qspCall(s, 'arousal', 'bj', 5, 'sub', 'lesbian', 'rough');
    qspCall(s, 'stat', '');
    scene.actions([
...
function enterVor2(s: GameState, scene: SceneBuilder): void {
  if (((s as any).rex ?? 0)?.['owned'] === 0) {
    ...
    if (((s as any).dog_bravery ?? 0) < 50) {
      ...
      // TODO-QSP: dynamic text: You're lying in bed when you suddenly hear a commotion out in the hallway. You g...
    ...
    // TODO-QSP: dynamic text: You kneel down and hug <<$rex['name']>>, your heart pounding at the thought of w...
    scene.text(`You kneel down and hug ${((s as any).rex ?? 0)?.['name']}, your heart pounding at the thought of what could have happened if he wasn't here.`);
  } },
...
function enterVor3(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_apprnc ?? 0) < 120) {
    ...
    qspCall(s, 'npcgeneratec', '', 0, 'Rapist 1', Math.floor(Math.random() * 23) + 18, 0, 1);
    qspCall(s, 'npcStat', '', ((s as any).npclastgenerated ?? 0));
    qspCall(s, 'npcgeneratec', '', 0, 'Rapist 2', Math.floor(Math.random() * 23) + 18, 0, 1);
...
function enterVorend(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).remsitr ?? 0) === 1) {
    // TODO-QSP: dynamic text: <center><img <<$set_imgh>> src="images/locations/city/residential/apartment/sitr...
    scene.text(`<center><img ${((s as any).set_imgh ?? 0)} src="images/locations/city/residential/apartment/sitr2.jpg"></center>`);
  }
  scene.text('Relieved that it\'s over, you check your apartment to see what\'s missing.');
  if (((s as any).stolmoney ?? 0) > 0) {
    // TODO-QSP: dynamic text: They took <<$func('money', 'format', stolmoney)>> from your living room table.
    scene.text(`They took ${qspFunc(s, 'money', 'format', ((s as any).stolmoney ?? 0))} from your living room table.`);
    (s as any).stolmoney = 0;
  }
  if (((s as any).money ?? 0) > 0) {
    // TODO-QSP: dynamic text: They took the <<$func('money', 'format', money)>> that you had on you.
    scene.text(`They took the ${qspFunc(s, 'money', 'format', ((s as any).money ?? 0))} that you had on you.`);
  ...
  if (((s as any).husID ?? 0) === ''  ||  (('bedr;bedr2x;korr;korr2x').indexOf((((s as any).loc ?? 0)))) + 1 <= 0  ||  ((s as any).spouseVars ?? 0)?.['drink'] === 10) {
    ...
    if (((s as any).mc_inventory ?? 0)?.['dildo_small'] === 0) {
      if (((s as any).week ?? 0) < 6  &&  ((s as any).hour ?? 0) < 16) {
        ...
        qspCall(s, 'bed_events', 'event_end');
      }
    } else {
      if (((s as any).week ?? 0) < 6  &&  ((s as any).hour ?? 0) < 16) {
...
function enterMast2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) >= 23  ||  ((s as any).hour ?? 0) < 7) {
    ...
    scene.text(`${((s as any).boydesc ?? 0)} looks at you, a little dumbfounded.`);
  } else {
    // TODO-QSP: dynamic text: Once you get into it, <<$boydesc>> enters the room.
    scene.text(`Once you get into it, ${((s as any).boydesc ?? 0)} enters the room.`);
  }
  if (((s as any).npc_gentle ?? 0)?.[String((s as any).boy ?? 0)] === 1) {
    ...
    scene.text(`You pull the dildo out of your pussy and, winking at your husband, insert it into your ass. ${((s as any).boydesc ?? 0)} watches closely as the dildo disappears into your ass.`);
    ...
    // TODO-QSP: dynamic text: <<$boydesc>> gets his dick out and starts masturbating. It's not what you expect...
    scene.text(`${((s as any).boydesc ?? 0)} gets his dick out and starts masturbating. It's not what you expected and you feel a little sad that he isn't fucking you as you start to masturbate.`);
    ...
    scene.text(`${((s as any).boydesc ?? 0)} is still masturbating, looking at you stretched out on the table before you take the dildo out of your ass and start lick it while looking into his eyes.`);
    ...
    if (((s as any).npc_rough ?? 0)?.[String((s as any).boy ?? 0)] === 0) {
      ...
      if (((s as any).npc_pervert ?? 0)?.[String((s as any).boy ?? 0)] === 0) {
        ...
        scene.text(`${((s as any).boydesc ?? 0)} stands up and playfully claps. "Bravo! Encore?" he laughs.`);
        // TODO-QSP: dynamic text: "Anything for such a lovely audience, but first an intermission," you laugh and ...
        scene.text(`"Anything for such a lovely audience, but first an intermission," you laugh and ${((s as any).boydesc ?? 0)} helps you down from the table.`);
        scene.actions([
          { label: 'Finish', handler: (st: GameState) => {
    qspCall(st, 'bed_events', 'event_end');
  } },
    ...
    scene.img('images/characters/city/husband/sex/m22.jpg');
    // TODO-QSP: dynamic text: You pull the dildo out of your pussy and, winking at your husband, insert it int...
    scene.text(`You pull the dildo out of your pussy and, winking at your husband, insert it into your ass. ${((s as any).boydesc ?? 0)} watches closely as you slide the dildo in and out of your ass.`);
    scene.text('You see how his pants swell. "Anything else you want, dear?" you ask while smiling.');
    ...
    scene.text(`When his cock has finally hardened, ${((s as any).boydesc ?? 0)} stops you. "Now I'll take this hole."`);
    ...
    scene.text(`Still using the dildo on your pussy and ass, you offer ${((s as any).boydesc ?? 0)} your mouth. He gets up and walks over to you.`);
