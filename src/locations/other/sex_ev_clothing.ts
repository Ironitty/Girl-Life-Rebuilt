...
function enterUndressMenu(s: GameState, scene: SceneBuilder): void {
  if (((s as any).date_ev ?? 0)?.['at_home'] === 0) {
    ...
    scene.text(`${((s as any).npcdesc ?? '')} opens the door up to his ` + ((((s as any).sex_ev ?? 0)?.['loc'] === 'npc_home') ? ('place.') : ('room.')));
  } else {
    scene.text(`${((s as any).npcdesc ?? '')} leads you ` + ((((s as any).npc_apt_type_label ?? 0)?.[String((s as any).npcID ?? 0)] === 'studio') ? ('to his bed.') : ('down the hall to his bedroom.')));
  }
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterUndressFunction(s, scene); (s as any).locArgs = __savedLocArgs; }
...
function enterTopUndress(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).lastwornpantytype ?? 0)?.['sex_ev'] !== 'none') {
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterPantyMenu(s, scene); (s as any).locArgs = __savedLocArgs; }
  } else {
    if (((s as any).sex_ev ?? 0)?.['bottom_noun'] === 'skirt') {
      scene.actions([
        { label: 'Show him what\'s under your skirt (nothing)', goto: ['sex_ev_clothing', 'pantyless_skirt'] },
...
function enterDressLoop(s: GameState, scene: SceneBuilder): void {
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterDressLoopCode(s, scene); (s as any).locArgs = __savedLocArgs; }
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterDressImage(s, scene); (s as any).locArgs = __savedLocArgs; }
  qspCall(s, 'stat', '');
  scene.build();
}
