...
function enterSetCoffeeHoleChats(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).NatbelQW ?? 0)?.['uni_nightclub_date'] === 0) {
    if (((s as any).NatbelQW ?? 0)?.['blocked'] === 1) {
      if (((s as any).daystart ?? 0) > ((s as any).NatbelQW ?? 0)?.['block_day']) {
        if (((s as any).NatbelQW ?? 0)?.['block_reason'] === 'uni') {
          ...
          scene.actions([
            { label: 'You spot Natasha looking very sorry for herself', handler: (st: GameState) => {
    if (qspFunc(s, 'money', 'can_afford', 240) === 0) {
      s.scene = { ...s.scene, mainText: String((st as any).noMoney || ''), curActs: [] };
    } else {
      qspGoto(st, 'natbel_uni_chat', 'coffee_hole_chat_brokenup_1');
    }
  } },
          ]);
        }
    ...
    if (qspFunc(s, 'money', 'can_afford', 240) === 0) {
      ...
      qspGoto(st, 'natbel_uni_chat', 'coffee_hole_chat_gf_1');
    }
  } },
        ]);
      } else {
    ...
    if (qspFunc(s, 'money', 'can_afford', 1362) === 0) {
      ...
      qspGoto(st, 'natbel_uni_chat', 'coffee_hole_chat_strangers_1');
    }
  } },
          ]);
        }
    ...
    if (((s as any).NatbelQW ?? 0)?.['uni_nightclub_date'] === 1) {
      if (((s as any).week ?? 0) === 6  &&  ((s as any).hour ?? 0) >= 19) {
        if (((s as any).NatbelQW ?? 0)?.['FriendLover'] >= 3) {
          (s as any).natbel_here = 1;
          scene.actions([
            { label: 'Go to The Coffee Hole to meet Natasha', handler: (st: GameState) => {
    if (qspFunc(s, 'money', 'can_afford', 660) === 0) {
      s.scene = { ...s.scene, mainText: String((st as any).noMoney || ''), curActs: [] };
    } else {
      qspGoto(st, 'natbel_uni_chat', 'coffee_hole_nightclubdate_gf_1');
    }
  } },
          ]);
        } else {
    ...
    if (qspFunc(s, 'money', 'can_afford', 660) === 0) {
      ...
      qspGoto(st, 'natbel_uni_chat', 'coffee_hole_nightclubdate_strangers_1');
    }
  } },
            ]);
          }
  ...
  if (((s as any).natbelQW ?? 0)?.['designs_unlock_day'] > 0  &&  ((s as any).natbelQW ?? 0)?.['designs_chat_done'] === 0) {
    if (((s as any).daystart ?? 0) >= ((s as any).natbelQW ?? 0)?.['designs_unlock_day'] + 30) {
      (s as any).natbel_here = 1;
      scene.actions([
        { label: 'Ask Natasha how her design work is progressing', handler: (st: GameState) => {
    if (qspFunc(s, 'money', 'can_afford', 500) === 0) {
      s.scene = { ...s.scene, mainText: String((st as any).noMoney || ''), curActs: [] };
    } else {
      qspGoto(st, 'natbel_uni_dates_2', 'designs_chat');
    }
  } },
      ]);
    }
...
function enterCoffeeHoleChatGf_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You can see she wasn't aware of your presence at all as she jumps slightly. She looks at you and you can see her eyes are a bit red. "Oh hi ${((st as any).pcs_nickname ?? '')}!"`);
    scene.text('Grinning you respond. "Well I\'m going to have a coffee. Anyway, is that any way to greet your girlfriend?"');
    // TODO-QSP: dynamic text: "Sorry <<$pcs_nickname>>, I didn''t mean to be rude; I''ve just had a bit of a s...
    scene.text(`"Sorry ${((st as any).pcs_nickname ?? '')}, I didn't mean to be rude; I've just had a bit of a shitty day but you being here has already cheered me up." She smiles at you and wipes her eyes.`);
    ...
    if (((st as any).month ?? 0) === 8) {
      ...
      scene.text(`She nods. "Yes, now you say it I guess I should do both. Okay, thank you for the prompt ${((st as any).pcs_nickname ?? '')}. I'll get myself registered for both."`);
    } else {
      // TODO-QSP: dynamic text: "Yes I agree <<$pcs_nickname>>; it took me ages to decide but I''ve enrolled for...
      scene.text(`"Yes I agree ${((st as any).pcs_nickname ?? '')}; it took me ages to decide but I've enrolled for both or them."`);
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['uni_nightclub_date'] = 1;
    qspCall(st, 'stat', '');
    ...
    // TODO-QSP: dynamic text: A now smiling Natasha looks at you and replies. "Oh that''d be great <<$pcs_nick...
    scene.text(`A now smiling Natasha looks at you and replies. "Oh that'd be great ${((st as any).pcs_nickname ?? '')} I love dancing but haven't had the courage to go by myself as yet."`);
...
function enterCoffeeHoleNightclubdateGf_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`As you arrive she greets you and tells you that she bought you a Cappuccino. "Here you go ${((s as any).pcs_nickname ?? '')}, I hope it's still warm."`);
  ...
  scene.actions([
    ...
    qspCall(st, 'beverage', 'vodka_martini_stats');
    scene.actions([
    ...
    qspCall(st, 'beverage', 'beer_stats');
    qspCall(st, 'beverage', 'beer_stats');
    scene.actions([
    ...
    scene.text(`Natasha looks like she's going to start crying. "I'm so sorry ${((st as any).pcs_nickname ?? '')}, I've never got like this before; I rarely drink and when I do it's just the odd glass of wine."`);
    ...
    // TODO-QSP: dynamic text: Once you''ve booked it Natasha stops you paying for it. "No <<$pcs_nickname>>, t...
    scene.text(`Once you've booked it Natasha stops you paying for it. "No ${((st as any).pcs_nickname ?? '')}, this is my fault so at least let me pay for it."`);
    ...
    qspGoto(st, 'natbel_uni_chat', 'coffee_hole_nightclubdate_gf_1_after');
  } },
    ]);
  } },
    ]);
...
function enterCoffeeHoleNightclubdateGf_1After(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Before leaving Natasha looks at you with concern and raises the subject of the Starlets. "${((st as any).pcs_nickname ?? '')}, were'nt you a Starlet with Albina?"`);
    // TODO-QSP: dynamic text: Unsure of how to reply you pause to consider your response. Natasha takes this a...
    scene.text(`Unsure of how to reply you pause to consider your response. Natasha takes this as a bad sign and looks downcast. "Sorry, ${((st as any).pcs_nickname ?? '')} I shouldn't have raised it. Don't answer if you don't want to.`);
    ...
    scene.text(`With that she gets up and gives you a hug before sitting back down then gives your hand a friendly squeeze. "Thank you for telling me ${((st as any).pcs_nickname ?? '')}, I know it wouldn't have been an easy thing to do."`);
    scene.actions([
      { label: 'Leave the café', goto: ['natbel_uni_chat', 'coffee_hole_nightclubdate_1_cafe_leave'] },
    ]);
  } },
...
function enterCoffeeHoleNightclubdate_1CafeLeave(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Okay ${((s as any).pcs_nickname ?? '')}, that was a nice meal and now I have a treat for us which I hope you'll like.`);
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'erotic_nudity', 4);
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'massage_give', 5);
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    (st as any).orgasm_msg = '';
    qspCall(st, 'arousal', 'cuni', 5, 'no_orgasm_msg');
    qspCall(st, 'arousal', 'cuni_give', (-5));
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 30;
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['uni_dorm_access'] = 1;
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['uni_nightclub_date'] = 2;
...
function enterCoffeeHoleChatBrokenup_1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).university ?? 0)?.['student'] === 1) {
    ...
    // TODO-QSP: dynamic text: You can see she wasn''t aware of your presence at all as she jumps slightly. She...
    scene.text(`You can see she wasn't aware of your presence at all as she jumps slightly. She looks at you and you can see her eyes are a bit red. "${((st as any).pcs_nickname ?? '')}! What are you doing here?!"`);
    ...
    scene.text(`"Sorry ${((st as any).pcs_nickname ?? '')}, I didn't mean to be rude but the people who come here are nearly all university students so I'm just surprised to see you here."`);
    ...
    qspCall(st, 'natbel_friend', 'reconcile');
    qspCall(st, 'npcStat', 'A16');
    ...
    // TODO-QSP: dynamic text: A now smiling Natasha looks at you and replies. "Oh that''d be great <<$pcs_nick...
    scene.text(`A now smiling Natasha looks at you and replies. "Oh that'd be great ${((st as any).pcs_nickname ?? '')}."`);
    scene.text('You give her a smile in return. "Right, well we\'ll have to arrange a date then…"');
    ...
    // TODO-QSP: dynamic text: "Erm… Well… It''s just that…" After a sigh she finally says it. "So <<$pcs_nickn...
    scene.text(`"Erm… Well… It's just that…" After a sigh she finally says it. "So ${((st as any).pcs_nickname ?? '')} does this mean you want us to get back together then? I didn't want to break up with you it's just that…"`);
    ...
    // TODO-QSP: dynamic text: A now smiling Natasha looks at you and replies. "Oh that''d be great <<$pcs_nick...
    scene.text(`A now smiling Natasha looks at you and replies. "Oh that'd be great ${((st as any).pcs_nickname ?? '')}, I've really missed you… I love dancing but haven't had the courage to go by myself as yet."`);
    scene.text('You give her a smile in return. "Right, well we\'ll have to arrange a date then…"');
    scene.text('Natasha nods and interrupts. "Yes I\'ll look forward to that." She pauses and looks nervously at you.');
    scene.text('"OK Natasha, I know that look. Come on what is it you want to say?"');
    // TODO-QSP: dynamic text: "Erm… Well… It''s just that…" After a sigh she finally says it. "So <<$pcs_nickn...
    scene.text(`"Erm… Well… It's just that…" After a sigh she finally says it. "So ${((st as any).pcs_nickname ?? '')} does this mean you want us to get back together then? I didn't want to break up with you it's just that…" She pauses.`);
    scene.text('Now it\'d your turn to interrupt her. "I wasn\'t sure you\'d want to but yes if that\'s what you really want. I loved spending time with you and would like to continue doing so."');
    scene.text('With that she gives you a big smile jumps up and kisses you. After breaking the kiss she holds you close.');
    ...
    if (((st as any).month ?? 0) === 8) {
      ...
      // TODO-QSP: dynamic text: "Yes I agree <<$pcs_nickname>>; it took me ages to decide but I''ve enrolled for...
      scene.text(`"Yes I agree ${((st as any).pcs_nickname ?? '')}; it took me ages to decide but I've enrolled for both the art and computer classes. I didn't fancy the psychology though."`);
      scene.text('You reply. "Good I think that the art class is pretty much a must for you and the computer class will assist too especially given your job with the fashion blog."');
      scene.text('She nods in agreement and gives you a warm smile.');
...
function enterCoffeeHoleChatStrangers_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: Natasha has a deep sigh and looks at you, albeit briefly. "I''m so sorry <<$pcs_...
    scene.text(`Natasha has a deep sigh and looks at you, albeit briefly. "I'm so sorry ${((st as any).pcs_nickname ?? '')}; I'm just having a really shitty day."`);
    scene.text('"Look I can leave you alone if you prefer?"');
    // TODO-QSP: dynamic text: She shakes her head. "No, <<$pcs_nickname>> actually I''d really like some compa...
    scene.text(`She shakes her head. "No, ${((st as any).pcs_nickname ?? '')} actually I'd really like some company if you don't mind and I apologise for my rudeness; I'm not normally like that."`);
    ...
    if (((st as any).NatbelQW ?? 0)?.['FriendLover'] > 0  ||  ((st as any).NatbelQW ?? 0)?.['QWstage'] > 0) {
    ...
    // TODO-QSP: dynamic text: She looks at you uncertainly. "Well only if you''re sure… Thank you <<$pcs_nickn...
    scene.text(`She looks at you uncertainly. "Well only if you're sure… Thank you ${((st as any).pcs_nickname ?? '')}. You taking the time to talk to me has already helped a bit." She gives you a weak smile and gets up. "Where do you want to go?"`);
    ...
    scene.text(`When you enter the hairdresser Natasha looks a bit confused. "I thought we were just going to have a browse ${((st as any).pcs_nickname ?? '')}, what are we doing here, do you need a haircut of something?"`);
...
function enterCoffeeHoleNightclubdateStrangers_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.actions([
      { label: 'Wake up', handler: (st: GameState) => {
    qspCall(st, 'sleep_simple', 'sleep_until', 8, 0);
    qspGoto(st, 'natbel_uni_chat', 'coffee_hole_nightclubdate_strangers_1_after');
  } },
    ]);
  } },
    ]);
...
function enterCoffeeHoleNightclubdateStrangers_1After(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if ((!(((st as any).runnerQW ?? 0)?.['bronze_medals'] + ((st as any).runnerQW ?? 0)?.['silver_medals'] + ((st as any).runnerQW ?? 0)?.['gold_medals']))) {
      ...
      scene.text(`After a moment she gets up and comes over to give you a big hug before getting a bit embarrassed and sitting down again and giving your hand a squeeze. "Yes ${((st as any).pcs_nickname ?? '')}. I'd love to see you race. Erm… Sorry, I guess I got a bit carried away."`);
      ...
      scene.text(`"Stop it ${((st as any).pcs_nickname ?? '')}." She says blushing even more. After gathering herself she continues. "Thank you again for taking me out to the nightclub, I enjoyed that despite the way the night ended. Also for looking after me; that was so nice of you."`);
...
function enterCoffeeHoleNightclubdateStrangers_1JobHub(s: GameState, scene: SceneBuilder): void {
  ...
  if (Object.keys((s as any).temp_city_work ?? {}).length === 0  ||  Object.keys((s as any).temp_city_work ?? {}).length === 1  &&  ((s as any).temp_city_work ?? 0)[0] === 'runner') {
    ...
    scene.text(`She smiles at you and responds happily. "Well, it's not my blog ${((st as any).pcs_nickname ?? '')}, I just help out but yeah it's going fine and I'm really enjoying it. Making a bit of money from it is also really nice."`);
    ...
    scene.text(`Natasha clearly has something else on her mind and fidgets while looking at you with concern and raises the subject of your friendship with Albina. "${((st as any).pcs_nickname ?? '')}, are'nt you close friends with Albina and part of her group which got caught up in that big scandal last year?"`);
    // TODO-QSP: dynamic text: Unsure of how to reply you pause to consider your response. Natasha takes this a...
    scene.text(`Unsure of how to reply you pause to consider your response. Natasha takes this as a bad sign and looks downcast. "Sorry, ${((st as any).pcs_nickname ?? '')} I shouldn't have raised it. Don't answer if you don't want to. I… I'm just concerned for you." She trails off.`);
...
function enterCoffeeHoleNightclubdateStrangers_1JobHubPorn(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Natasha starts fidgeting more then looks at you and says in a very quiet voice "Erm… ${((s as any).pcs_nickname ?? '')} are you ${((s as any).pfname ?? '')}?"`);
  ...
  scene.actions([
    ...
    if ((Array.isArray((st as any).temp_city_work) ? ((st as any).temp_city_work as any[]).indexOf('runner') : -1) >= 0) {
      ...
      scene.text(`Natasha gives you a hug. "Don't worry ${((st as any).pcs_nickname ?? '')} I'd never say anything whatever happens between us.`);
    } else {
