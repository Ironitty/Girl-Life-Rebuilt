...
function enterVicArt_Ev1_Cash(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).billwin ?? 0) > 12) {
    ...
    scene.text(`"Take care, ${((s as any).pcs_firstname || '')}," Victor bids you farewell and you return to the main room.`);
    scene.actions([
      { label: 'Return to the bar', goto: ['qwBarPolet', 'bar'] },
    ]);
  } },
    ...
    if (qspFunc(s, 'money', 'can_afford', 1000) === 0) {
      ...
      qspGoto(s, 'qwBarBilliard2', 'VicArt_Ev1_Cash');
    }
  } },
    ]);
  }
...
function enterVicArt_Ev1_Broke(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_inhib ?? 0) >= 40  &&  ((s as any).Jaska_Friend_Bill ?? 0) >= 3) {
    ...
    scene.text(`"Just having fun with our little ${((s as any).pcs_firstname || '')}."`);
    ...
    scene.text(`"And we usually wouldn't," Victor agrees, "but ${((s as any).pcs_firstname || '')} is such wonderful company that we just couldn't help ourselves."`);
    ...
    scene.text(`Victor smiles triumphantly. "Oh, I thought that maybe Artyom and I will occasionally play a little billiards with ${((s as any).pcs_firstname || '')} or other girls in here and allow them to make up for their losses with… services." Artyom chuckles at that.`);
    ...
    scene.text(`"Because that's how you keep your partners happy. He didn't demand anything we weren't going to give him anyway and 10 percent of a blowjob isn't anything, really, is it, ${((s as any).pcs_firstname || '')}?"`);
