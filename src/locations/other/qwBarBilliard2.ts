...
function enterVicArt_Ev1_Cash(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('"Why not."');
  // TODO-QSP: dynamic text: You try to ignore the voice in the back of your head that insists <<$func('money...
  ...
  if (((s as any).billwin ?? 0) > 12) {
    ...
    scene.text(`"Take care, ${((s as any).pcs_firstname ?? 0)}," Victor bids you farewell and you return to the main room.`);
    scene.actions([
      { label: 'Return to the bar', goto: ['qwBarPolet', 'bar'] },
    ]);
  } },
...
function enterVicArt_Ev1_Broke(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).Jaska_Friend_Bill ?? 0) === 1) {
    ...
    // TODO-QSP: dynamic text: "That's a pity." Victor gives you a long, pensive look. "You know… You are a ver...
    scene.text(`"That's a pity." Victor gives you a long, pensive look. "You know… You are a very pretty girl, ${((s as any).pcs_firstname ?? 0)}. Isn't she, Artyom?"`);
    ...
    scene.text('"Well, our offer still stands," Artyom reminds you. "You could bet doing a striptease instead.\' + iif(Jaska_Friend_Bill >= 2, \'… maybe even more than that."\', \') + \'"');
  ...
  if (((s as any).pcs_inhib ?? 0) >= 40  &&  ((s as any).Jaska_Friend_Bill ?? 0) >= 3) {
    ...
    scene.text(`"Just having fun with our little ${((s as any).pcs_firstname ?? 0)}."`);
    ...
    // TODO-QSP: dynamic text: "And we usually wouldn't," Victor agrees, "but <<$pcs_firstname>> is such wonder...
    scene.text(`"And we usually wouldn't," Victor agrees, "but ${((s as any).pcs_firstname ?? 0)} is such wonderful company that we just couldn't help ourselves."`);
    ...
    scene.text(`Victor smiles triumphantly. "Oh, I thought that maybe Artyom and I will occasionally play a little billiards with ${((s as any).pcs_firstname ?? 0)} or other girls in here and allow them to make up for their losses with… services." Artyom chuckles at that.`);
    ...
    // TODO-QSP: dynamic text: "Because that's how you keep your partners happy. He didn't demand anything we w...
    scene.text(`"Because that's how you keep your partners happy. He didn't demand anything we weren't going to give him anyway and 10 percent of a blowjob isn't anything, really, is it, ${((s as any).pcs_firstname ?? 0)}?"`);
