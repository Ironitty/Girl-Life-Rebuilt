...
function enterCardGameGroupSex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`${((s as any).boydesc || '')} says: "I've given it some thought, and I've decided that it is bad to be selfish. Therefore, as my wish: I will share your body with my friends, and we will all fuck you good!"`);
    ...
    scene.text(`"Exactly! I get a wish, and you have to do what I want. My wish is to share your body with my friends", ${((s as any).boydesc || '')} tells you, while he plays with your boobs. "Now stop complaining and get on your back."`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
...
function enterCardGameRoughSex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"I want your ass next, bitch", ${((s as any).boydesc || '')} says. "Get on your hands and knees, and offer yourself to me."`);
    ...
    if (((s as any).analPlugIn ?? 0) === 1) {
      ...
      scene.text(`${((s as any).boydesc || '')} laughs: "This would only get in the way. Out you go!", and he tugs the plug out of your ass and tosses it aside.`);
    }
    ...
    scene.text(`You were about to get up once ${((s as any).boydesc3 || '')} finished in you, but his friends stop you. "Everyone gets to have a go, slut. I won your ass, so I decide when you're done. Stay down", he orders you.`);
    ...
    scene.text(`Luckily, ${((s as any).boydesc3 || '')}'s cum acts as a lubricant to an extent, and when someone else pushes their dick inside your already opened sphincter, it doesn't hurt quite as much any more. In fact… it actually feels kind of nice! You moan softly, trying to hide how much you're enjoying this from the guys. They'd just get the wrong idea…`);
    ...
    scene.text(`"Almost done, whore. Show us your ass", ${((s as any).boydesc3 || '')} smirks with contempt.`);
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.text(`Then ${((s as any).boydesc3 || '')} slaps your ass cheek and grins: "Alright, slut. We've trained your hole enough for today. Go freshen up."`);
    scene.actions([
...
function enterCardGameWonExposeGirl(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`${((s as any).loss_npc_name || '')} exits the house, and the rest of you follow him. He does as you ask, and can barely escape when the furious lady chases him all the way down the street.`);
    ...
    scene.text(`"Why is the camera shaking so much!? This looks like shit!" ${((s as any).loss_npc_name || '')} asks, clearly frustrated now.`);
...
function enterCardGameWonCumOnGirl(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`When you lick ${((s as any).loss_npc_name || '')} close to his orgasm, he runs to his victim.`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    qspCall(s, 'stat', '');
    ...
    scene.text(`While you idly wipe your lips with the back of your hand and watch ${((s as any).loss_npc_name || '')} run, his friends are shooting everything on film with their mobiles.`);
    ...
    scene.text(`${((s as any).loss_npc_name || '')} comes back, his face as red as a lobster. As he gasps for breath, he groans: "I'm never doing that again! ${((s as any).pcs_nickname || '')}, the next time you lose… I'm going to make you pay!"`);
  ...
  scene.actions([
    ...
    if (((s as any).pcs_horny ?? 0) >= 70) {
      ...
      if (((s as any).pcs_horny ?? 0) < 70) {
        ...
        scene.text(`${((s as any).boydesc || '')} does his best to get you to orgasm. He's doing all the right things and you feel very aroused, but in the end you can't quite reach an orgasm this time. After a while you can tell his tongue is starting to feel exhausted, and you tell him he can stop.`);
        qspCall(s, 'arousal', 'cuni', 2);
      }
    }
    qspCall(s, 'arousal', 'end');
...
function enterCardGameWonFuckMyAss(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`${((s as any).boydesc || '')} fucks you in the ass, like you demanded of him. He's surprisingly careful with you, using plenty of lube and ensuring that he's making you feel good as he does it. What a nice change from the normal way the boys treat you…`);
    ...
    qspCall(s, 'arousal', 'end');
    scene.img(`images/pc/body/cum/${(String(((s as any).temp2 || '')).slice((4)-1, ((4)-1)+(parseFloat((String(((s as any).temp1 || '')).slice((3)-1, ((3)-1)+(2))))-3)))}.jpg`);
    ...
    scene.text(`"Mmmm… delicious, babe!" you muse, more to make him feel good than anything. You smile at the proud grin ${((s as any).boydesc || '')} has on his face afterwards, and feel slightly better for having made his day.`);
    scene.actions([
      { label: 'Finish', goto: ['vasilyhome', 'livingroom'] },
    ]);
  } },
