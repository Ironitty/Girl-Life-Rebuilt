...
function enterPregnancyVisibility(s: GameState, scene: SceneBuilder): void {
  (s as any).RESULT = 0;
  (s as any).pregchemTemp = ((Number((s as any).locArgs?.[1] ?? 0) === 0) ? (((s as any).pregChem ?? 0)) : (((s as any).locArgs?.[1] ?? 0)));
  (s as any).clothingworntypeTemp = ((Number((s as any).locArgs?.[2] ?? 0) === '') ? (((s as any).clothingworntype ?? 0)) : (((s as any).locArgs?.[2] ?? 0)));
  (s as any).clothingwornnumberTemp = ((Number((s as any).locArgs?.[3] ?? 0) === 0) ? (((s as any).clothingwornnumber ?? 0)) : (((s as any).locArgs?.[3] ?? 0)));
