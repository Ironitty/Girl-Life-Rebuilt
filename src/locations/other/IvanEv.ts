...
function enterBoxingLessons(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).ivanQW ?? 0)?.['boxing_lessons'] <= 3) {
    ...
    qspCall(st, 'exercise', 'tier2', 30, 'stren', 'jab', 'punch', 'def');
    ...
    if (((s as any).ivanQW ?? 0)?.['boxing_lessons'] < 8) {
      ...
      scene.actions([
        { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'exercise', 'tier3', 30, 'stren', 'jab', 'punch', 'def');
    ...
    scene.actions([
      ...
      scene.actions([
        { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'exercise', 'tier3', 15, 'stren', 'jab', 'punch', 'def');
...
function enterLockerRoom(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).temprand = (Math.floor(Math.random() * 4) + 1);
    if (((st as any).temprand ?? 0) === 1  &&  ((st as any).ivanQW ?? 0)?.['sex'] >= 1) {
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A3', 'dislike');
    ...
    qspCall(st, 'arousal', 'flash', 1, 'humiliation');
    qspCall(st, 'stat', '');
    qspCall(st, 'willpower', 'anal', 'resist', 'medium');
    if (((st as any).will_cost ?? 0) <= ((st as any).pcs_willpwr ?? 0)) {
      scene.actions([
        { label: 'Tell him to go away', handler: (st: GameState) => {
    qspCall(st, 'npc_relationship', 'modify', 'A3', 'hate');
    qspCall(st, 'willpower', 'pay', 'resist');
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/school/boy/ivan/sex/community/locker/coverup_go.jpg');
    ...
    scene.actions([
      ...
      qspGoto(st, 'IvanEv', 'locker_room_dressed');
    }
  } },
  ]);
  scene.build();
...
function enterLockerRoomFuck(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('With that, he walks out and leaves.');
    qspCall(st, 'arousal', 'foreplay', 1, 'sub');
    qspCall(st, 'cum_call', 'mouth_swallow', 'A3', 1);
    qspCall(st, 'arousal', 'end');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Get dressed', goto: ['IvanEv', 'locker_room_dressed'] },
...
function enterIvanBored(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).pcs_buttsize ?? 0) <= 21) {
    ...
    qspCall(st, 'arousal', 'foreplay', 1, 'rough');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterIvanBoredDisco(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 2, 'sub', 'rough', 'deepthroat');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterLockerRoomDressed(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).clothingworntype ?? 0) !== 'nude') {
    ...
    alert(qspFunc(st, 'wrap', 'neg b', 'You need to get dressed before leaving.'));
  } },
    ]);
  }
  scene.actions([
