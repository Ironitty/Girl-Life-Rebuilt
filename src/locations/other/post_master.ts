...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).job_status ?? 0)?.['pav_mailgirl'] === '') {
    ...
    scene.actions([
      ...
      scene.actions([
        { label: '', labelFn: (s: GameState) => String(qspFunc(s, 'wrap', 'v_neg', 'Resign from your job') ?? ''), handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'jobs', 'set_terminated', 'pav_mailgirl');
    qspCall(st, 'stat', '');
    scene.img('images/locations/shared/postoffice/postmaster.jpg');
    scene.text(`"${((st as any).pcs_nickname ?? '')}," the postmaster says, looking up from his desk. What can I do for you?"`);
    ...
    scene.text(`"Well alright then; good luck with your future plans ${((st as any).pcs_nickname ?? '')}"`);
...
function enterVisitAfterRefusing2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).temp_prev_job_last_work_day = undefined;
