...
function enterShowertalk1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).npc_rel ?? 0)?.['A34'] <50) {
      qspCall(st, 'npc_relationship', 'modify', 'A34', 'like');
    }
    if (((st as any).pcs_inhib ?? 0) >= 40  ||  ((st as any).npc_rel ?? 0)?.['A34'] >= 90) {
      scene.actions([
        { label: '"You can look at me you know"', handler: (st: GameState) => {
    qspCall(st, 'npc_relationship', 'modify', 'A34', 'love');
    ((st as any).brotherQW = (st as any).brotherQW ?? {})['shower_look'] = 1;
    if (((st as any).brotherQW ?? 0)?.['Sex'] < 2) {
      ((st as any).brotherQW = (st as any).brotherQW ?? {})['Sex'] = 2;
    }
    ...
    if (((st as any).bro_ass_compliment ?? 0) === 1) {
      scene.actions([
    ...
    if (((st as any).pcs_pubes ?? 0) >= 4  ||  ((st as any).pcs_leghair ?? 0) >= 4) {
      scene.actions([
        { label: 'Continue', goto: ['brother_voyeur', 'shave'] },
      ]);
    } else {
...
function enterShowertalk2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Continue', handler: (st: GameState) => {
    if (((st as any).brotherQW ?? 0)?.['shower_look'] >= 1) {
      qspGoto(st, 'brother_voyeur', 'brothershowerlook');
    }
    if (((st as any).npc_rel ?? 0)?.['A34'] < 50) {
      qspCall(st, 'npc_relationship', 'modify', 'A34', 'like');
    }
    ...
    qspCall(st, 'arousal', 'clit_vibe', (-5), 'incest', 'masturbate');
    (st as any).orgasm_or = 'no';
    qspCall(st, 'arousal', 'clit_vibe', (-5), 'incest', 'masturbate');
    ...
    ((st as any).brother_shower = (st as any).brother_shower ?? {})['mast_look'] = (Math.floor(Math.random() * 3) + 1);
    scene.img('images/locations/pavlovsk/resident/apartment/home/vanrpar.jpg');
    if (((st as any).brother_shower ?? 0)?.['mast_look'] !== 1) {
    ...
    if (((st as any).bro_ass_compliment ?? 0) === 1) {
      ...
      { label: 'Ask him about his day', handler: (st: GameState) => {
    scene.img('images/shared/home/bathroom/brothershowertalk.mp4');
    if (((st as any).brotherQW ?? 0)?.['shower_talk'] < 6) {
      scene.text('You casually talk to him about his day while you shower. He sits on the toilet seat and faces away from you, awkwardly answering all your questions and queries, but you still see him occasionally stealing glances at your naked body. Not that it bothers you any, as you simply smile to yourself, amused that he thinks he\'s still being sneaky.');
    } else {
...
function enterBrothershowerlook(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Ask him about his day', handler: (st: GameState) => {
    (st as any).brothershowerdaytalk = (Math.floor(Math.random() * 3) + 1);
    scene.img('images/shared/home/bathroom/brothershowertalk.mp4');
    ...
    (st as any).bro_ass_compliment = (Math.floor(Math.random() * 3) + 1);
    if (((st as any).bro_ass_compliment ?? 0) === 1) {
      qspGoto(st, 'brother_voyeur', 'ass_compliment');
    } else {
      qspGoto(st, 'brother_voyeur', 'shave');
    }
  } },
    ]);
  } },
    { label: 'Ask how school\'s going', handler: (st: GameState) => {
    (st as any).brothershowerschooltalk = (Math.floor(Math.random() * 3) + 1);
    scene.img('images/shared/home/bathroom/brothershowertalk.mp4');
    if (((st as any).brotherQW ?? 0)?.['shower_talk'] < 10) {
    ...
    { label: 'Ask him about his friends', handler: (st: GameState) => {
    (st as any).brothershowerfriendtalk = (Math.floor(Math.random() * 3) + 1);
    scene.img('images/shared/home/bathroom/brothershowertalk.mp4');
    if (((st as any).brotherQW ?? 0)?.['shower_talk'] < 10) {
...
function enterBrotherShowerTogetherFirst(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_inhib ?? 0) >= 50) {
    ...
    scene.img('images/shared/home/bathroom/brothershower.jpg');
    scene.text(`You look straight at his hard ${((st as any).dick ?? '')}cm cock, completely unashamed, saying, "Hey, you're pretty big for a little brother, you know that?"`);
...
function enterBrotherShowerTogether(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).brotherQW ?? 0)?.['shower_times'] > 2) {
    ...
    if (((st as any).brotherQW ?? 0)?.['fuck'] < 1) {
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    scene.img('images/shared/home/bathroom/wash2.mp4');
    if (((st as any).brotherQW ?? 0)?.['fuck'] < 1  &&  ((st as any).brotherQW ?? 0)?.['bj'] < 1) {
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A34', 'love');
    if (((st as any).brotherQW ?? 0)?.['bj'] < 1  &&  ((st as any).brotherQW ?? 0)?.['fuck'] < 1) {
      ...
      scene.text(`Looking at Kolka, he glances back at you to see a mischievous twinkle in your eye. Without a second's thought or warning, you get on your knees and wrap your lips around his ${((st as any).dick ?? '')} cm cock.`);
      scene.actions([
        { label: 'Blow him', goto: ['brother_shower_sex', 'brother_shower_bj'] },
      ]);
    }
  ...
  scene.actions([
    ...
    if (((st as any).brotherQW ?? 0)?.['shower_times'] < 3  ||  ((st as any).brotherQW ?? 0)?.['Sex'] < 1) {
      ...
      if (((st as any).brotherQW ?? 0)?.['Sex'] >= 4  &&  ((st as any).brother_shower ?? 0)?.['grope'] === 1) {
        ((st as any).brother_shower = (st as any).brother_shower ?? {})['grope'] = 2;
    ...
    if (((st as any).brotherQW ?? 0)?.['shower_times'] < 3  ||  ((st as any).brotherQW ?? 0)?.['Sex'] < 1) {
      ...
      if (((st as any).brotherQW ?? 0)?.['Sex'] >= 4  &&  ((st as any).brother_shower ?? 0)?.['grope'] === 1) {
        ((st as any).brother_shower = (st as any).brother_shower ?? {})['grope'] = 3;
...
function enterBrotherWash(s: GameState, scene: SceneBuilder): void {
  ...
  if ((((s as any).pcs_leghair ?? 0) > 3  ||  ((s as any).pcs_pubes ?? 0) > 3)  &&  ((s as any).brotherQW ?? 0)?.['shave_day'] < ((s as any).daystart ?? 0)) {
    ...
    ((st as any).brotherQW = (st as any).brotherQW ?? {})['shave_day'] = ((st as any).daystart ?? 0) + 2;
    ...
    if (((st as any).brotherQW ?? 0)?.['Sex'] > 0  ||  ((st as any).brotherQW ?? 0)?.['fuck'] > 0  ||  ((st as any).brotherQW ?? 0)?.['anal'] > 0  ||  ((st as any).brotherQW ?? 0)?.['bj'] > 0  ||  ((st as any).brotherwash ?? 0) > 5) {
      scene.actions([
    ...
    ((st as any).brother_shower = (st as any).brother_shower ?? {})['shave_ask1'] = ((st as any).daystart ?? 0);
    qspGoto(st, 'brother_voyeur', 'brother_shower_shave');
  } },
        ]);
      }
      scene.actions([
    ...
    ((st as any).brotherQW = (st as any).brotherQW ?? {})['shave_day'] = ((st as any).daystart ?? 0) + 6;
    scene.img('images/shared/home/bathroom/brotherwash.jpg');
  ...
  if (((s as any).brotherQW ?? 0)?.['Sex'] > 0  ||  ((s as any).brotherQW ?? 0)?.['fuck'] > 0  ||  ((s as any).brotherQW ?? 0)?.['anal'] > 0  ||  ((s as any).brotherQW ?? 0)?.['bj'] > 0  ||  ((s as any).brotherwash ?? 0) > 5) {
    ...
    ((st as any).brother_shower = (st as any).brother_shower ?? {})['grope'] = 4;
    qspGoto(st, 'brother_voyeur', 'showerfun');
  } },
    ]);
  }
  scene.actions([
...
function enterShaveBoth(s: GameState, scene: SceneBuilder): void {
  if (((s as any).pcs_leghair ?? 0) > 3  &&  ((s as any).pcs_pubes ?? 0) > 3) {
    if (((s as any).mc_inventory ?? 0)?.['razor'] < ((s as any).razors_to_use ?? 0)?.['all']) {
      alert('As you pick up your razor, you realize that it\'s probably only sharp enough for one more use. You\'ll have to decide between a ' + (((s as any).pc_desc ?? 0)?.['pubes']) + ' pussy or ' + (((s as any).pc_desc ?? 0)?.['legs']) + ' legs.');
    } else {
      scene.actions([
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    if (((st as any).brotherQW ?? 0)?.['fuck'] > 0  ||  ((st as any).brotherQW ?? 0)?.['anal'] > 0  ||  ((st as any).brotherQW ?? 0)?.['bj'] > 0  ||  ((st as any).brotherwash ?? 0) > 5) {
      qspCall(st, 'arousal', 'vaginal_finger', 1, 'incest');
      qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'vaginal_finger', 5, 'incest');
    qspCall(st, 'stat', '');
...
function enterShaveLegs(s: GameState, scene: SceneBuilder): void {
  if (((s as any).pcs_leghair ?? 0) > 3  &&  ((s as any).mc_inventory ?? 0)?.['razor'] >= ((s as any).razors_to_use ?? 0)?.['legs']) {
    ...
    if (((st as any).pcs_pubes ?? 0) > 3) {
      scene.text('Kolka frowns. "You\'re not going to shave your ' + ((((st as any).pcs_pubes ?? 0) <= 20) ? ('pussy?"') : ('bush?"')));
      if (((st as any).mc_inventory ?? 0)?.['razor'] < ((st as any).razors_to_use ?? 0)?.['pussy']) {
        scene.actions([
    ...
    scene.actions([
      { label: 'Finish showering together', goto: ['brother_voyeur', 'brother_shower_end'] },
        ...
        if (((st as any).pcs_pubes ?? 0) < 21  ||  ((st as any).shave_length ?? 0) > 0) {
          scene.actions([
    ...
    if (((st as any).brother_shower ?? 0)?.['shave_ask1'] === ((st as any).daystart ?? 0)) {
      ((st as any).brother_shower = (st as any).brother_shower ?? {})['shave_ask2'] = ((st as any).daystart ?? 0);
    ...
    ((st as any).brother_shower = (st as any).brother_shower ?? {})['shave_request'] = 1;
  }, goto: ['brother_voyeur', 'brother_shower_shave'] },
        { label: 'Oh alright', handler: (st: GameState) => {
    ((st as any).brother_shower = (st as any).brother_shower ?? {})['shave_request'] = 2;
  }, goto: ['brother_voyeur', 'brother_shower_shave'] },
      ]);
    } else {
...
function enterShavePussy(s: GameState, scene: SceneBuilder): void {
  if (((s as any).pcs_pubes ?? 0) >= ((s as any).shave_trigger ?? 0)  &&  ((s as any).mc_inventory ?? 0)?.['razor'] >= ((s as any).razors_to_use ?? 0)?.['pussy']) {
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A34', 'love');
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    if (((st as any).pubestyle ?? 0) > 0  ||  ((st as any).pubestyle ?? 0) === 10) {
    ...
    if (((st as any).pcs_leghair ?? 0) > 3) {
      ...
      if (((st as any).mc_inventory ?? 0)?.['razor'] < ((st as any).razors_to_use ?? 0)?.['legs']) {
        scene.actions([
          { label: '"I can\'t. Razor\'s gone dull"', handler: (st: GameState) => {
    scene.text('"I couldn\'t even if I wanted to," you say to him wistfully. He looks at you in confusion. "Razor\'s gone dull." You thumb the razor to show him. Need a sharp razor to shave. <i>Especially</i> down there," you smile.');
    scene.text('"Oh." He looks disappointed.');
    ...
    scene.text('Once you\'re finished you gently glide your hands over your smooth pussy and legs, enjoying the feeling.');
    if (((st as any).brotherQW ?? 0)?.['shave'] === 1) {
...
function enterBrotherShowerEnd(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).pcs_inhib ?? 0) >= 60  ||  ((st as any).npc_rel ?? 0)?.['A34'] > 85) {
      scene.actions([
    ...
    qspCall(st, 'brother_shower_sex', 'brother_shower_finishdryoff');
  } },
      ]);
    }
    scene.actions([
    ...
    qspCall(st, 'brother_shower_sex', 'brother_shower_finish');
  } },
  ]);
  scene.build();
}
  ...
  if (((s as any).brotherQW ?? 0)?.['shower_look'] === 1  &&  ((s as any).brotherQW ?? 0)?.['shave_day'] < ((s as any).daystart ?? 0)) {
    ...
    scene.actions([
      ...
      if (((s as any).pcs_leghair ?? 0) > 3  &&  ((s as any).pcs_pubes ?? 0) > 3  &&  ((s as any).mc_inventory ?? 0)?.['razor'] < ((s as any).razors_to_use ?? 0)?.['all']) {
        alert('As you pick up your razor, you realize that it\'s probably only sharp enough for shaving one set of things. If you want to shave you\'ll have to decide between keeping a ' + (((s as any).pc_desc ?? 0)?.['pubes']) + ' pussy or ' + (((s as any).pc_desc ?? 0)?.['legs']) + ' legs.');
      }
    ...
    ((st as any).brotherQW = (st as any).brotherQW ?? {})['shave_day'] = ((st as any).daystart ?? 0) + 6;
    scene.text('"Nope! I\'m thinking about growing it out," you say cheerfully, smiling at him.');
    if (((st as any).pcs_pubes ?? 0) > 3  &&  ((st as any).pcs_pubes ?? 0) < 11) {
      scene.text('"Hmph. I still think you\'d look better if you just kept it smooth…" he mutters.');
    } else {
      if (((st as any).pcs_pubes ?? 0) >= 11  &&  ((st as any).pcs_pubes ?? 0) < 21) {
    ...
    if (((st as any).shave_length ?? 0) === 2) {
      ...
      if ((((st as any).pubestyle ?? 0) >= 2  &&  ((st as any).pubestyle ?? 0) <= 8)  ||  ((st as any).pubestyle ?? 0) >= 12) {
        if (((st as any).pcs_pubes ?? 0) >= ((st as any).shave_trigger ?? 0) + 5) {
        ...
        if (((st as any).pubestyle ?? 0) === 9) {
          (st as any).shave_act = 'Trim your bush';
...
function enterShaveNotDone(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).brother_shower ?? 0)?.['together'] === ((st as any).daystart ?? 0)) {
      if (((st as any).brotherQW ?? 0)?.['Sex'] > 0  ||  ((st as any).brotherQW ?? 0)?.['fuck'] > 0  ||  ((st as any).brotherQW ?? 0)?.['bj'] > 0  ||  ((st as any).brotherwash ?? 0) > 5) {
        scene.actions([
          { label: 'Get frisky', handler: (st: GameState) => {
    ((st as any).brother_shower = (st as any).brother_shower ?? {})['grope'] = 4;
    qspGoto(st, 'brother_voyeur', 'showerfun');
