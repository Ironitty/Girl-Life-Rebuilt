...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    scene.text(`<center><b>${(((st as any).npc_firstname ?? 0)?.['A13'] ?? '')} ${(((st as any).npc_lastname ?? 0)?.['A13'] ?? '')}</b></center>`);
    scene.img('images/characters/shared/headshots_main/big13.jpg');
    ...
    if (((st as any).LariskaQW ?? 0)?.['story'] === 5) {
      ...
      ((st as any).LariskaQW = (st as any).LariskaQW ?? {})['story'] = 6;
    } else {
