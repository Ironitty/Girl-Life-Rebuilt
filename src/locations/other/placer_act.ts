...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).placerParameter ?? 0)?.['player_act'] === 1) {
    ...
    (s as any).temp_time_elapsed = 30 / ((s as any).placerParameter ?? 0)?.['number_of_man'];
    (s as any).i = 0;
    // TODO-QSP: :parkbj2
    if (((s as any).i ?? 0) > 0) {
      qspCall(s, 'npcgeneratec', '', 0, 'Stranger', Math.floor(Math.random() * 18) + 18, 0, 1);
    ...
    (s as any).minut = ((s as any).minut ?? 0) + (30 - ((s as any).temp_time_elapsed ?? 0) * ((s as any).placerParameter ?? 0)?.['number_of_man']);
    qspCall(s, 'stat', '');
