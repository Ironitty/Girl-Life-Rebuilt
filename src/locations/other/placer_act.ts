...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).placerParameter ?? 0)?.['player_act'] === 0) {
    ...
    if (((s as any).placerParameter ?? 0)?.['recognize_status'] === 2) {
      ...
      // TODO-QSP: dynamic text: You stop and look at the guy who approached you. You don''t think you''ve ever s...
      scene.text(`You stop and look at the guy who approached you. You don't think you've ever seen him before. He introduces himself: "My name is ${((s as any).boydesc || '')}."`);
      ...
      scene.text(`${((s as any).boydesc || '')} invites you: "Pleased to meet you, ${((s as any).pcs_nickname || '')}! Would you like to have a beer with me?"`);
      if (((s as any).placerParameter ?? 0)?.['friend_index'] > 0) {
        // TODO-QSP: dynamic text: <<$ev_name>> whispers to you: "I''m not so sure about this, <<$pcs_nickname>>! I...
        scene.text(`${((s as any).ev_name || '')} whispers to you: "I'm not so sure about this, ${((s as any).pcs_nickname || '')}! I think we should go…"`);
      }
    } else {
      if (((s as any).month ?? 0) >= 11  ||  ((s as any).month ?? 0) < 3) {
        scene.img('images/locations/pavlovsk/park/skver_znacom_1.jpg');
      ...
      if (((s as any).placerParameter ?? 0)?.['number_of_man'] === 1  &&  ((s as any).placerParameter ?? 0)?.['friend_index'] > 0) {
        ...
        scene.text(`${((s as any).ev_name || '')} whispers to you: "Who do you think he likes more, ${((s as any).pcs_nickname || '')}? You or me?"`);
      } else {
        if (((s as any).placerParameter ?? 0)?.['number_of_man'] > 1) {
          // TODO-QSP: dynamic text: Vying for the guys'' attention, you happily accept <<$boydesc>>''s invitation an...
          scene.text(`Vying for the guys' attention, you happily accept ${((s as any).boydesc || '')}'s invitation and introduce yourself to everyone.`);
          if (((s as any).placerParameter ?? 0)?.['friend_index'] > 0) {
            // TODO-QSP: dynamic text: <<$ev_name>> whispers to you: "They''re cute, <<$pcs_nickname>>! Let''s stay for...
            scene.text(`${((s as any).ev_name || '')} whispers to you: "They're cute, ${((s as any).pcs_nickname || '')}! Let's stay for a while and see what happens?"`);
          }
        }
      }
    }
    ...
    qspCall(st, 'stat', '');
    if (((st as any).placerParameter ?? 0)?.['friend_slut'] < 80) {
      qspGoto(st, 'placer_end', '');
    }
    if (((st as any).placerParameter ?? 0)?.['friend_slut'] >= 80) {
      // TODO-QSP: dynamic text: <<$ev_name>> whispers to you: "Don''t be such a bore, <<$pcs_nickname>>! I want ...
      scene.text(`${((st as any).ev_name || '')} whispers to you: "Don't be such a bore, ${((st as any).pcs_nickname || '')}! I want to stay, he's cute! Come on, stay with me and live a little!"`);
  ...
  if (((s as any).placerParameter ?? 0)?.['player_act'] === 1) {
    ...
    scene.text(`You kiss ${((st as any).boydesc || '')} on his lips and embrace him, and soon his hands are exploring your body. He whispers to you: "Come on, you cute little thing… let's go somewhere a bit more private" while he roughly kneads your ass through your clothes. His friends are watching you intently.`);
    ...
    scene.text(`${((st as any).boydesc || '')} takes you by the hand, and you walk down the street with the group of Gopniks. They lead you to a shabby house, and let you in.`);
    // TODO-QSP: dynamic text: The boys quickly clear the empty beer bottles from the table, and put out some v...
    scene.text(`The boys quickly clear the empty beer bottles from the table, and put out some vodka and snacks. The vodka is poured into glasses, and ${((st as any).boydesc || '')} offers you one with a generous smile.`);
    ...
    scene.text(`You push ${((st as any).boydesc || '')}'s hands away from your body, and quietly whisper to him: "Please, stop touching me like that! Do you treat all girls like this?"`);
    ...
    scene.text(`When your cup is nearly empty, ${((st as any).boydesc || '')} generously pours more in your glass with a confident smile. The guys persuade you to keep drinking, saying how much they are enjoying you spending time with them.`);
    ...
    scene.actions([
      ...
      if ((!(Math.floor(Math.random() * 2) + 0))) {
        // TODO-QSP: dynamic text: One of the guys suggests: "Hey guys, we don''t have to hang around in the park a...
        scene.text(`One of the guys suggests: "Hey guys, we don't have to hang around in the park all day! The house should be available right now, maybe we can show ${((st as any).pcs_nickname || '')} around there and not have people stare us down all day!"`);
    ...
    scene.actions([
      { label: 'Continue', goto: ['podval_rape', ''] },
          ...
          if (((s as any).placerParameter ?? 0)?.['know_slut'] === 1  &&  ((s as any).BjBeerQW ?? 0) > 1  &&  ((s as any).bjgangrand ?? 0) === 1) {
            // TODO-QSP: dynamic text: One of the guys suddenly laughs: "Wait a minute, I know her! That''s <<$pcs_nick...
            scene.text(`One of the guys suddenly laughs: "Wait a minute, I know her! That's ${((s as any).pcs_nickname || '')}, everyone knows she's a ${((s as any).gnikname || '')}. She gives guys blowjobs just to get a beer! My buddy showed me a video where they gave like three guys a blowjob, and then happily said hi to her mother with the cum still on her face!"`);
    ...
    (st as any).blowbangrand = (Math.floor(Math.random() * 2) + 3);
    if (((st as any).pcs_makeup ?? 0) > ((st as any).makeup ?? 0)?.['base']) {
      (st as any).pcs_makeup = 0;
    }
    (st as any).minut = ((st as any).minut ?? 0) + (30 - ((st as any).temp_time_elapsed ?? 0) * (((st as any).placerParameter ?? {})?.['number_of_man'] ?? 0));
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Wipe the cum from your eye', handler: (st: GameState) => {
    scene.img(`images/shared/sex/dom/blowbang${((st as any).blowbangrand || '')}.jpg`);
    ...
    qspCall(st, 'cum_cleanup', 'external');
    ...
    scene.actions([
      { label: 'Find your way back to the main streets', goto: ['pav_park', 'start'] },
            ...
            if (((s as any).placerParameter ?? 0)?.['know_slut'] === 1) {
              ...
              scene.text(`One of the guys tells you: "${((s as any).pcs_nickname || '')}, we all know you're a ${((s as any).gnikname || '')}". One of his friends pitches in: "In other words: we're going to some place quiet and we're going to fuck you. Cause trouble and we'll knock your teeth out. Understood?"`);
              scene.actions([
                { label: 'Go with the guys', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
        ...
        if (((s as any).placerParameter ?? 0)?.['friend_index'] > 0) {
          if (((s as any).placerParameter ?? 0)?.['know_slut'] === 0) {
            ...
            if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
              scene.actions([
                { label: 'Grab your friend and leave', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
              ]);
            } else {
    ...
    scene.text(`You can't do this! You break free from the Gopniks and quickly take ${((st as any).ev_name || '')}'s hand. You drag her along as you walk away from the Gopniks. She's a bit reluctant to leave them at first, but trusts your judgment enough to follow you without objections.`);
    scene.actions([
...
{ label: 'Kiss him', handler: (st: GameState) => {
    ...
    scene.actions([
      { label: 'Go with the guys', goto: ['placer_gop_orgy', ''] },
          ...
          if (((s as any).placerParameter ?? 0)?.['know_slut'] === 1) {
            // TODO-QSP: dynamic text: One of the guys says: "<<$pcs_nickname>>, we all know you''re a whore. If your l...
            scene.text(`One of the guys says: "${((s as any).pcs_nickname || '')}, we all know you're a whore. If your little girlfriend is with you, we can only assume she is, too."`);
...
{ label: 'Try to leave with your friend', handler: (st: GameState) => {
    ...
    // TODO-QSP: dynamic text: Several of them surround <<$ev_name>>, and threaten you: "We weren''t asking, bi...
    scene.text(`Several of them surround ${((st as any).ev_name || '')}, and threaten you: "We weren't asking, bitch. Either you're coming with us to share the load, or your cute little friend here is going to have to satisfy all of us by herself. You probably won't see her again today in that case, we're going to need her services for quite a while if she has to do it all alone."`);
    // TODO-QSP: dynamic text: You can see <<$ev_name>>''s eyes widen, giving you a begging look. It''s quite o...
    scene.text(`You can see ${((st as any).ev_name || '')}'s eyes widen, giving you a begging look. It's quite obvious she is feeling very intimidated, and really doesn't want to go with them by herself.`);
    qspCall(st, 'willpower', 'sex', 'resist', 'hard');
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
    ...
    qspCall(st, 'arousal', 'kiss', 5);
    qspCall(st, 'stat', '');
    qspCall(st, 'skverdin', 'd_park_placer_bj');
    scene.actions([
    ...
    // TODO-QSP: dynamic text: The guy just grins at you: "Everyone knows you''re a whore, <<$pcs_nickname>>. W...
    scene.text(`The guy just grins at you: "Everyone knows you're a whore, ${((st as any).pcs_nickname || '')}. We've all seen the video… it's way too late for you to start being a prude now. Come on, it'd only take a minute. What do you say?"`);
    qspCall(st, 'skverdin', 'd_park_placer_bj');
    ...
    scene.text(`The guy smiles: "Look around, ${((st as any).pcs_nickname || '')}! We're alone, we'll be fine!"`);
    qspCall(st, 'skverdin', 'd_park_placer_bj');
    scene.actions([
    ...
    qspCall(st, 'stat', '');
    (st as any).tiperand = (Math.floor(Math.random() * 2) + 0);
    if ((!((st as any).tiperand ?? 0))) {
      qspGoto(st, 'fbHouse', '');
    }
    if (((st as any).tiperand ?? 0) === 1) {
      qspGoto(st, 'fbDorm', '');
    }
    if (((st as any).tiperand ?? 0) === 2) {
      qspGoto(st, 'fbGar', '');
    }
  } },
    ]);
      ...
      qspGoto(st, 'fbGar', '');
          ...
          if (((s as any).placerParameter ?? 0)?.['friend_index'] > 0  &&  ((s as any).placerParameter ?? 0)?.['number_of_man'] === 1) {
            if ((!((s as any).tiploverand ?? 0))) {
              // TODO-QSP: dynamic text: You''re having fun, drinking beers with your friend and the guy you just met. Af...
              scene.text(`You're having fun, drinking beers with your friend and the guy you just met. After a while you realize that he is mostly just paying attention to you though, while pretending your friend ${((s as any).ev_name || '')} isn't there.`);
              scene.actions([
                { label: 'Kiss the guy', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    // TODO-QSP: dynamic text: <<$boydesc>> whispers in your ear: "Come on <<$pcs_nickname>>, let''s go to my p...
    scene.text(`${((st as any).boydesc || '')} whispers in your ear: "Come on ${((st as any).pcs_nickname || '')}, let's go to my place…"`);
    if (((st as any).placerParameter ?? 0)?.['friend_slut'] < 50  ||  (((st as any).placerParameter ?? 0)?.['friend_index'] === 14  &&  ((st as any).katjaQW ?? 0)?.['park_sex'] < (Math.floor(Math.random() * 2) + 1))) {
      // TODO-QSP: dynamic text: <<$ev_name>> seems to understand where this is going, and nods at you: "Okay, <<...
      scene.text(`${((st as any).ev_name || '')} seems to understand where this is going, and nods at you: "Okay, ${((st as any).pcs_nickname || '')}… I guess I should leave the two of you alone. I'll see you later?"`);
      qspCall(st, 'willpower', 'sex', 'resist', 'hard');
      if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
        scene.actions([
          { label: 'Leave with your friend', handler: (st: GameState) => {
    ...
    if (((st as any).tiperand ?? 0) === 2) {
      ...
      // TODO-QSP: dynamic text: <<$ev_name>> smiles as she licks her lips: "He''s cute, <<$pcs_nickname>>… take ...
      scene.text(`${((st as any).ev_name || '')} smiles as she licks her lips: "He's cute, ${((st as any).pcs_nickname || '')}… take me with you?"`);
      scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
              ...
              if (((s as any).tiploverand ?? 0) === 1) {
                // TODO-QSP: dynamic text: You''re having fun, drinking beers with your friend and the guy you just met. Af...
                scene.text(`You're having fun, drinking beers with your friend and the guy you just met. After a while you realize that he is mostly just paying attention to ${((s as any).ev_name || '')} though, while pretending you are not there.`);
                scene.actions([
                  { label: 'See what happens', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
    if (((st as any).placerParameter ?? 0)?.['friend_slut'] < 30) {
      // TODO-QSP: dynamic text: <<$ev_name>> kisses the guy for a while, but when he whispers something to her s...
      scene.text(`${((st as any).ev_name || '')} kisses the guy for a while, but when he whispers something to her she apologizes to him: "Sorry, ${((st as any).boydesc || '')}… I can't just leave ${((st as any).pcs_nickname || '')} hanging. We should be going."`);
      scene.actions([
        { label: 'Leave with your friend', goto: ['placer_end', ''] },
      ]);
    }
    if (((st as any).placerParameter ?? 0)?.['friend_slut'] >= 30  &&  ((st as any).placerParameter ?? 0)?.['friend_slut'] < 60) {
      // TODO-QSP: dynamic text: <<$ev_name>> kisses the guy, and he begins to grope her body. She quickly whispe...
      scene.text(`${((st as any).ev_name || '')} kisses the guy, and he begins to grope her body. She quickly whispers something in his ear, and you see him nod. Then ${((st as any).ev_name || '')} turns to you: "Sorry ${((st as any).pcs_nickname || '')}, ${((st as any).boydesc || '')} promised me he'd show me his cool rock collection. His apartment is small though, there'd be no room for the both of us… I'll see you later?"`);
      ...
      if (((st as any).placerParameter ?? 0)?.['friend_slut'] >= 60) {
        // TODO-QSP: dynamic text: <<$ev_name>> kisses the guy, and he begins to grope her body. She quickly whispe...
        scene.text(`${((st as any).ev_name || '')} kisses the guy, and he begins to grope her body. She quickly whispers something in his ear, and you see him nod. Then ${((st as any).ev_name || '')} turns to you and asks innocently: "Hey ${((st as any).pcs_nickname || '')}… ${((st as any).boydesc || '')} asks if you want to come with us for some fun. What do you say?"`);
        qspCall(st, 'willpower', 'sex', 'resist', 'hard');
        if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
          scene.actions([
    ...
    scene.text(`When you don't object, he pulls away and asks you: "Hey, ${((st as any).pcs_nickname || '')}… you're cute! We're staying in a hostel just minutes away from the park. Want to come party with us there?"`);
    scene.actions([
      { label: 'Go with the guys', handler: (st: GameState) => {
    ...
    // TODO-QSP: dynamic text: The guys cheer when you say you''ll come with them, and <<$boydesc>> takes your ...
    scene.text(`The guys cheer when you say you'll come with them, and ${((st as any).boydesc || '')} takes your hand while he guides you out of the park. You make a quick stop at a liquor store, so one of the guys can buy some booze for the party.`);
    // TODO-QSP: dynamic text: When you enter the hostel room, you see it has nothing but a few iron beds for f...
    scene.text(`When you enter the hostel room, you see it has nothing but a few iron beds for furniture. ${((st as any).boydesc || '')} pulls you down onto a bed with him, and begins to make out with you again while another guy pours the booze into some glasses and hands everyone a glass.`);
    ...
    scene.actions([
      { label: 'Continue', goto: ['placer_sex', 'meet'] },
              ...
              // TODO-QSP: dynamic text: <<$boydesc>> comes up to you, and aggressively kisses you on your mouth while he...
              scene.text(`${((s as any).boydesc || '')} comes up to you, and aggressively kisses you on your mouth while he hugs you.`);
              scene.text('His lips taste like the beer you\'ve both been drinking. You ease up against him and kiss him back, happy to surrender to him for just one moment. The guy feels your body relax and begins to grope your ass greedily, hoping he can get you to put out more.');
              // TODO-QSP: dynamic text: When you don''t object, he pulls away and asks you: "Hey, <<$pcs_nickname>>… you...
              scene.text(`When you don't object, he pulls away and asks you: "Hey, ${((s as any).pcs_nickname || '')}… you're cute! We're staying in a hostel just minutes away from the park. Want to come party with us there?"`);
              scene.actions([
                { label: 'Go with the guys', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 30;
    qspCall(st, 'drugs', 'alcohol', 'vodka', 3);
    qspCall(st, 'stat', '');
    scene.img('images/shared/sex/sleep/drunk.jpg');
    // TODO-QSP: dynamic text: The guys cheer when you say you''ll come with them, and <<$boydesc>> takes your ...
    scene.text(`The guys cheer when you say you'll come with them, and ${((st as any).boydesc || '')} takes your hand while he guides you out of the park. The group makes a quick stop at a liquor store, so one of the guys can buy some booze for the party.`);
    // TODO-QSP: dynamic text: When you enter the hostel room, you see it has nothing but a few iron beds for f...
    scene.text(`When you enter the hostel room, you see it has nothing but a few iron beds for furniture. ${((st as any).boydesc || '')} pulls you down onto a bed with him, and begins to make out with you again while another guy pours the booze into some glasses and hands everyone a glass.`);
    scene.text('The guys can stomach their alcohol a lot better than you, and before you know it you\'re absolutely hammered. They look like they\'re still feeling fine.');
    ...
    qspCall(st, 'stat', '');
    // TODO-QSP: dynamic text: You kiss the guy passionately, and just try to enjoy him groping your body. In t...
    scene.text(`You kiss the guy passionately, and just try to enjoy him groping your body. In the corner of your eye you see ${((st as any).ev_name || '')} receiving a similar treatment, several more guys gathering around the two of you. They're all pawing at your ass and boobs greedily, and suggest you go with them to their home for some more fun. ${((st as any).boydesc || '')} whispers in your ear: "Want to go to my place, babe? We can get some alone time there… or we can bring your friend along and all go there together?"`);
    scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
                ...
                scene.text(`You see one of the guys move up behind your friend, closing his arms around her in a bear grip as he kisses her neck. Another guy does the same to you, and kisses your neck quite aggressively while he has a tight grip on your waist. ${((s as any).boydesc || '')} whispers in your ear: "Hey babe… how about you and your friend come back to my place? I bet we could have some fun together…"`);
                scene.actions([
                  { label: 'Go with the guys', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'stat', '');
    // TODO-QSP: dynamic text: You leave the park with <<$ev_name>> and the guys, and chat with them while they...
    scene.text(`You leave the park with ${((st as any).ev_name || '')} and the guys, and chat with them while they lead you to their apartment.`);
    scene.actions([
    ...
    qspCall(st, 'stat', '');
    if (((st as any).placerParameter ?? 0)?.['know_slut'] === 0) {
      if (((st as any).placerParameter ?? 0)?.['slut_visual'] === 0) {
        qspGoto(st, 'pav_park', 'start');
      }
      if (((st as any).placerParameter ?? 0)?.['slut_visual'] > 0) {
        if (((st as any).placerParameter ?? 0)?.['recognize_status'] === 2) {
          // TODO-QSP: dynamic text: <<$boydesc>> disappointedly remarks: "We know you''re a whore… why are you being...
          scene.text(`${((st as any).boydesc || '')} disappointedly remarks: "We know you're a whore… why are you being such a prude?"`);
      ...
      if (((st as any).placerParameter ?? 0)?.['know_slut'] === 1) {
        if (((st as any).placerParameter ?? 0)?.['recognize_status'] === 2) {
