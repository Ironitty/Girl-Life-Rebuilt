...
function enterHome3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Fedor leans over toward you and begins kissing and licking your neck, obviously ready for some fun. You lay back enjoying his tongue on your neck which turns you on as well as feeling his strong hands caressing your breasts. Fedor continues licking and feeling up your body as you both start undressing each other. Fedor whispers in your ear, "${((s as any).pcs_firstname ?? 0)}, I want you right now."`);
...
function enterFedorSleeping(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Hey ${((s as any).pcs_firstname ?? 0)}, I'm a bit too tired to walk you home right now but you can climb into bed and spend the night if you like. Just don't let my mother see you leave.`);
...
function enterReginaChat(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_health ?? 0) < ((s as any).healthmax ?? 0)) {
    scene.actions([
      { label: 'Let Regina help you [+$func(\'money\', \'get_cost_string\', 20)]', handler: (st: GameState) => {
...
function enterFedorLivingroom(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_health ?? 0) < 100) {
    ...
    scene.text(`Fedor looks at you and frowns. "${((s as any).pcs_firstname ?? 0)} you're hurt. My mother works as a nurse, if she is home then you should have her take a look at you."`);
  }
  if (((s as any).clothingworntype ?? 0) === 'nude') {
    // TODO-QSP: dynamic text: Fedor reaches over, then begins rubbing your ass, "You have a body to die for <<...
    scene.text(`Fedor reaches over, then begins rubbing your ass, "You have a body to die for ${((s as any).pcs_nickname ?? 0)}."`);
  }
...
function enterFedorKitchen(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).FedorKoz_Daystart ?? 0) !== ((s as any).daystart ?? 0)) {
    ...
    scene.text(`"${((s as any).pcs_firstname ?? 0)} would you like something to eat?"`);
  }
  if (((s as any).pcs_health ?? 0) < 100) {
    // TODO-QSP: dynamic text: Fedor looks at you and frowns. "<<$pcs_firstname>> you're hurt. My mother works ...
    scene.text(`Fedor looks at you and frowns. "${((s as any).pcs_firstname ?? 0)} you're hurt. My mother works as a nurse, if she is home then you should have her take a look at you."`);
  ...
  if (((s as any).clothingworntype ?? 0) === 'nude') {
    ...
    scene.text(`Fedor reaches over, then begins feeling up your legs, "You are so hot, ${((s as any).pcs_nickname ?? 0)}."`);
  }
  if (((s as any).FedorKoz_Daystart ?? 0) !== ((s as any).daystart ?? 0)) {
    scene.actions([
...
function enterFedorBedroom(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_health ?? 0) < 100) {
    ...
    scene.text(`Fedor looks at you then frowns. "${((s as any).pcs_firstname ?? 0)} you're hurt. My mother works as a nurse. You should have her take a look at you."`);
  }
  if (((s as any).pcs_hairbsh ?? 0) < 1) {
    scene.text('Fedor notices your messy hair and pulls out his <a href="exec:gt \'FedorMisc\', \'Comb\'">comb</a>.');
  }
...
function enterFedorBedroom2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Fedor is watching videos on his computer. As you approach, Fedor turns to you and asks "Hey ${((s as any).pcs_nickname ?? 0)}, wanna check out some videos?"`);
  if (((s as any).pcs_health ?? 0) < 100) {
    // TODO-QSP: dynamic text: Fedor looks at you and frowns. "<<$pcs_firstname>> you're hurt. My mother works ...
    scene.text(`Fedor looks at you and frowns. "${((s as any).pcs_firstname ?? 0)} you're hurt. My mother works as a nurse, if she is home then you should have her take a look at you."`);
  }
