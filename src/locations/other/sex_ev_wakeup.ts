...
function enterSleepFuckWake(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"No promises," ${((s as any).npcdesc || '')} smirks mischievously.`);
    ...
    scene.text(`"Somebody had fun last night," you muse, looking over at ${((s as any).npcdesc || '')} who is also waking.`);
    if (((s as any).npc_sleep_sex_okay ?? 0)?.[String((s as any).npcID ?? 0)] === 0) {
...
function enterLateForSchool(s: GameState, scene: SceneBuilder): void {
  if (((s as any).kanikuli ?? 0) === 0  &&  ((s as any).start_type ?? 0)?.['loc'] === 'sg'  &&  ((s as any).gschoolVars ?? 0)?.['school_diploma'] === 0  &&  ((s as any).gschoolVars ?? 0)?.['block'] === 0  &&  ((s as any).week ?? 0) < 6  &&  ((s as any).hour ?? 0) >= 7) {
    ...
    if (((s as any).sex_ev ?? 0)?.['loc'] !== 'pc_home') {
      { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterLateSchoolNotAtHome(s, scene); (s as any).locArgs = __savedLocArgs; }
    } else {
      scene.actions([
    ...
    scene.text(`"I need to go," you say, hurriedly hauling ${((s as any).npcdesc || '')} out of your bed. "And so do you."`);
    if (((s as any).npc_latesleeper ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
      ...
      scene.text(`${((s as any).npcdesc || '')} groggily starts gathering his clothes as you impatiently try to get him to go faster, constantly checking the clock to see how much time he's wasting. When he's <i>finally</i> dressed you rush ${((s as any).npcdesc || '')} to the door and shove him out and slam it shut, racing to get ready for class.`);
    } else {
      (s as any).minut = ((s as any).minut ?? 0) + 1;
      // TODO-QSP: dynamic text: At your command, <<$npcdesc>> starts throwing his clothes on while you impatient...
      scene.text(`At your command, ${((s as any).npcdesc || '')} starts throwing his clothes on while you impatiently keep checking the clock. Thankfully it's not more than a minute before he's fully dressed and you rush him out the door and slam it behind him, racing to get ready for class.`);
    }
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
...
function enterLateSchoolNotAtHome(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Hey, what's wrong?" ${((s as any).npcdesc || '')} says, turning over and squinting at you with sleepy eyes.`);
    scene.actions([
    ...
    scene.text(`With one last quick check, you make sure all your clothes are in place and rush out the door, leaving ${((s as any).npcdesc || '')} and your night together behind you.`);
    scene.actions([
...
function enterYawnWake(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([{ label: 'Continue', goto: ['sex_ev_morning', 'morning_menu1'] }]);
  // TODO-QSP: end
  scene.build();
}
...
function enterKissWake(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] !== 1) {
    scene.actions([
      { label: 'Kiss <<$npcdesc>>', handler: (st: GameState) => {
    ...
    scene.text(`You roll over onto ${((s as any).npcdesc || '')}, resting your breasts on his chest as you straddle him.`);
    ...
    qspCall(s, 'sex_ev_morning', 'morning_menu1');
    scene.actions([
      { label: 'Give <<$npcdesc>> a blowjob', handler: (st: GameState) => {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterBjWake(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ...
    // TODO-QSP: gs 'arousal', 'bj', 1, 'no_orgasm_msg', $sex_ev['prostitution_flag']
    scene.img('images/shared/sex/blowjob/bj47.mp4');
    // TODO-QSP: dynamic text: You break the kiss from <<$npcdesc>>'s lips and start trailing them down his nec...
    scene.text(`You break the kiss from ${((s as any).npcdesc || '')}'s lips and start trailing them down his neck and then his chest and then his stomach and soon you have your lips wrapped around his morning wood.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterBjWake(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
      ]);
    }
...
function enterBjWake(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_end_free_time ?? 0) <= ((s as any).hour ?? 0) + 2) {
    ...
    scene.text(`You give ${((s as any).npcdesc || '')} one more good suck before you withdraw your lips.`);
    ...
    scene.text(`You give ${((s as any).npcdesc || '')} one more good suck before you withdraw your lips to speak.`);
    ...
    scene.text(`You pull your lips away with a pop and stare ${((s as any).npcdesc || '')} straight in the eye.`);
    scene.text('"Guess."');
...
function enterAfterSleepfuckWake(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['sleep_fuck'] === 2) {
    ...
    scene.text(`"That was nice," you smile sleepily at ${((s as any).npcdesc || '')}. "Our little mid-night romp I mean. Slept like a baby after. How about you?"`);
...
function enterCumshotWakeup1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_cumshot_wake ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterCumshotWakeupClean1(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  if (!(s as any).npc_cumshot_wake) (s as any).npc_cumshot_wake = {}; (s as any).npc_cumshot_wake[String((s as any).npcID ?? 0)] = ((s as any).npc_cumshot_wake[String((s as any).npcID ?? 0)] ?? 0) + (1);
  // TODO-QSP: end
  scene.actions([
    ...
    scene.text(`"You were so hot lying there," ${((s as any).npcdesc || '')} snickers, a lewd grin smeared across his face. "I got hard just looking at you."`);
    ...
    scene.img('images/shared/sex/sleep/cumshot_wake2.mp4');
    // TODO-QSP: dynamic text: You sputter briefly as some of the salty sperm slips past your lips, but can't h...
    scene.text(`You sputter briefly as some of the salty sperm slips past your lips, but can't help but feel amusement when you see the lewd satisfaction on ${((s as any).npcdesc || '')}'s face.`);
    scene.actions([
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterCumshotWakeupClean2(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enterCumshotWakeupClean1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'cum_call', 'mouth_swallow', ((s as any).npcID ?? 0), 1, 0, 5);
    // TODO-QSP: dynamic text: Before you can make any more protest, <<$npcdesc>>'s cock pushes into your mouth...
    scene.text(`Before you can make any more protest, ${((s as any).npcdesc || '')}'s cock pushes into your mouth. Your eyes accuse him of being a jerk but your lips suck obediently, drawing the last drops of cum from his shaft.`);
    qspCall(s, 'sex_ev_morning', 'morning_menu1');
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterCumshotWakeupBj(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterCumshotWakeupClean2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'bj', 1, 'sub');
    qspCall(s, 'cum_call', 'mouth_swallow', ((s as any).npcID ?? 0), 1, 0, 5);
    // TODO-QSP: dynamic text: "Oh fiiiine," you say, mock frowning as <<$npcdesc>>'s cock pushes past your lip...
    scene.text(`"Oh fiiiine," you say, mock frowning as ${((s as any).npcdesc || '')}'s cock pushes past your lips and you subserviently suck out the last drops of cum from it.`);
    qspCall(s, 'sex_ev_morning', 'morning_menu1');
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterCumshotWakeupBj(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
...
function enterForgotBcPill(s: GameState, scene: SceneBuilder): void {
  if (((s as any).pilldaychk ?? 0) < ((s as any).daystart ?? 0) - 1  &&  ((s as any).birth_control ?? 0)?.['using_bc'] > 0) {
    ...
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['forgot_bc'] = 1;
    if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] !== 1) {
    ...
    if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 3) {
      ...
      scene.text(`Not only did you forget your birth control, but ${((s as any).npcdesc || '')} came inside you last night too. <i>A lot.</i> How could you fuck up like this?`);
    } else {
      if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 0) {
        // TODO-QSP: dynamic text: Not only did you forget your birth control, but <<$npcdesc>> came inside you las...
        scene.text(`Not only did you forget your birth control, but ${((s as any).npcdesc || '')} came inside you last night too. How could you fuck up like this?`);
      } else {
        if (((s as any).pharmacy_timers ?? 0)?.['birth_control'] > 0) {
          ...
          scene.text(`${((s as any).npcdesc || '')} didn't come inside you yesterday, but this breaks your streak. Aunt Luda said you need to take it every day or it won't work. So if you take it now, does that mean you're still not safe?`);
        } else {
          // TODO-QSP: dynamic text: <<$npcdesc>> didn't come inside you yesterday, but this breaks your streak. Are ...
          scene.text(`${((s as any).npcdesc || '')} didn't come inside you yesterday, but this breaks your streak. Are you still safe if you skip a day?`);
        }
      }
    }
  ...
  }, goto: ['sex_ev_wakeup', 'forgot_bc_pill2'] },
    ...
    if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 5) {
      ...
      scene.text(`Okay, so ${((s as any).npcdesc || '')} came inside you last night... <i>A lot.</i> But you can still take a morning after pill. Maybe it'll be okay.`);
    } else {
      if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 1) {
        ...
        scene.text(`Okay, so ${((s as any).npcdesc || '')} came inside you last night... More than once... A <i>few</i> times. But you can still take a morning after pill. Maybe it'll be okay.`);
      } else {
        if (((s as any).sex_ev ?? 0)?.['creampie_count'] === 1) {
          // TODO-QSP: dynamic text: Okay, so <<$npcdesc>> came inside you last night... But only once. That's not to...
          scene.text(`Okay, so ${((s as any).npcdesc || '')} came inside you last night... But only once. That's not too bad, right? Maybe you can still take a morning after pill. Maybe it'll be okay.`);
        }
      }
    }
    if (((s as any).mc_inventory ?? 0)?.['morning_after_pill'] > 0) {
      ...
      if (((s as any).sex_ev ?? 0)?.['no_condom'] === 0) {
        if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['forgot_bc_act'] = 'You used condoms';
      } else {
        if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['forgot_bc_act'] = '' + qspUntranslated(s, "npcdesc>", { location: "sex_ev_wakeup" }) + ' didn\'t come inside you';
      }
      scene.actions([
    ...
    if (((s as any).pharmacy_timers ?? 0)?.['birth_control'] > 0) {
      ...
      scene.text(`${((s as any).npcdesc || '')} didn't come inside you yesterday, so you're not in any real danger of getting pregnant right this second.' + iif(sex_ev['no_condom'] = 0, ' Besides, you used condoms.', ' ') + 'But this breaks your streak. Aunt Luda said you need to take it every day or it won't work. So if you take it now, does that mean you're still not safe?`);
    } else {
      // TODO-QSP: dynamic text: <<$npcdesc>> didn't come inside you yesterday, so you're not in any real danger ...
      scene.text(`${((s as any).npcdesc || '')} didn't come inside you yesterday, so you're not in any real danger of getting pregnant right this second.' + iif(sex_ev['no_condom'] = 0, ' Besides, you used condoms.', ' ') + 'But does this mean you'll need to wait a few days to build up the birth control in your system again? If you take your next pill now, does that mean you're still not safe?`);
    }
  }, goto: ['sex_ev_wakeup', 'forgot_bc_pill2'] },
      ]);
    }
    ...
    if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 0) {
      if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 5) {
        ...
        scene.text(`Okay, so ${((s as any).npcdesc || '')} came inside you last night... <i>A lot.</i> But it's not the fertile stage of your cycle. You should be fine, right?`);
      } else {
        if (((s as any).sex_ev ?? 0)?.['creampie_count'] > 1) {
          // TODO-QSP: dynamic text: Okay, so <<$npcdesc>> came inside you last night... More than once... A <i>few</...
          scene.text(`Okay, so ${((s as any).npcdesc || '')} came inside you last night... More than once... A <i>few</i> times. But it's not the fertile stage of your cycle. You should be fine, right?`);
        } else {
          if (((s as any).sex_ev ?? 0)?.['creampie_count'] === 1) {
            // TODO-QSP: dynamic text: Okay, so <<$npcdesc>> came inside you last night... But only once. And it's not ...
            scene.text(`Okay, so ${((s as any).npcdesc || '')} came inside you last night... But only once. And it's not the fertile stage of your cycle. You should be fine, right?`);
          }
        }
      }
    } else {
      if (((s as any).pharmacy_timers ?? 0)?.['birth_control'] > 0) {
        // TODO-QSP: dynamic text: <<$npcdesc>> didn't come inside you yesterday, so you're not in any real danger ...
        scene.text(`${((s as any).npcdesc || '')} didn't come inside you yesterday, so you're not in any real danger of getting pregnant right this second. But this breaks your streak. Aunt Luda said you need to take it every day or it won't work. So if you take it now, does that mean you're still not safe?`);
      } else {
        // TODO-QSP: dynamic text: <<$npcdesc>> didn't come inside you yesterday, so you're not in any real danger ...
        scene.text(`${((s as any).npcdesc || '')} didn't come inside you yesterday, so you're not in any real danger of getting pregnant right this second. But does this mean you'll need to wait a few days to build up the birth control in your system again? If you take your next pill now, does that mean you're still not safe?`);
      }
    }
  }, goto: ['sex_ev_wakeup', 'forgot_bc_pill2'] },
      ]);
  ...
  scene.actions([
    ...
    if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
      ...
      if (((s as any).npc_latesleeper ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
        ...
        scene.text(`${((s as any).npcdesc || '')} is still asleep next to you snoring softly, completely ignorant to the situation.`);
        qspCall(s, 'sex_ev_morning', 'morning_menu1');
      } else {
...
function enterCheatNoGuilt1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).lover_stat ?? 0)?.['fighting_flag'] === 1) {
    ...
    scene.text('<i>Serves </i>\' + iif($sex_ev[\'cheat\'] = \'boyfriend\', \'<i>him</i>\', \'<i>her</i>\') + \'<i>right,</i> you think, taking a perverse satisfaction in cheating on \' + iif($sex_ev[\'cheat\'] = \'boyfriend\', \'him\', \'her\') + \' after your recent fight.');
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterKissWake(s, scene); (s as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: '<i>Yawn</i>', goto: ['sex_ev_wakeup', 'yawn_wake'] },
    ]);
  ...
  scene.actions([
    ...
    // TODO-QSP: dynamic text: <i>Oops,</i> you think. <i>I think I just cheated on my <<$sex_ev['cheat']>>.</i...
    scene.text(`<i>Oops,</i> you think. <i>I think I just cheated on my ${((s as any).sex_ev ?? 0)?.['cheat'] ?? ''}.</i>`);
...
function enterGuiltInit(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['guilt_check'] === 0) {
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['guilt_check'] = 1;
    if (((s as any).pcs_lover ?? 0) + ((s as any).pcs_girlfriends ?? 0) > 0) {
      if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['guilt_count'] = ((s as any).sex_ev['guilt_count'] ?? 0) + (1);
    }
...
function enterGuiltStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['guilt_count'] > 0) {
    ...
    if (!(s as any).sex_ev) (s as any).sex_ev = {}; (s as any).sex_ev['guilt'] = 1;
    if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] !== 1) {
      scene.img('images/shared/sex/after/wakeup_shock1.mp4');
    ...
    if (((s as any).npcID ?? 0) !== 'one_of_svetas_lovers') {
      if (((s as any).stat ?? 0)?.['boyfriends_current'] > 0) {
        scene.actions([
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterGuiltProstitution1(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ]);
  }
  // TODO-QSP: end
  ...
  scene.actions([
    ...
    if (((s as any).stat ?? 0)?.['bf_cheated_on'] <= 10) {
      ...
      if (((s as any).sex_ev ?? 0)?.['cheat'] === 'boyfriend'  ||  ((s as any).sex_ev ?? 0)?.['cheat'] === 'girlfriend') {
        ...
        scene.text(`<i>I just cheated on my ${((s as any).sex_ev ?? 0)?.['cheat'] ?? ''}...</i>`);
      } else {
        // TODO-QSP: dynamic text: <i>I just cheated on <<$sex_ev['cheat']>>...</i> you think.
        scene.text(`<i>I just cheated on ${((s as any).sex_ev ?? 0)?.['cheat'] ?? ''}...</i> you think.`);
      }
      // TODO-QSP: dynamic text: Your thoughts wander through your night with <<$npcdesc>>...
      scene.text(`Your thoughts wander through your night with ${((s as any).npcdesc || '')}...`);
      { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterGuiltActRecount(s, scene); (s as any).locArgs = __savedLocArgs; }
      if (((s as any).sex_ev ?? 0)?.['prostitution_flag'] === 'prostitution') {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterCheatGuiltExcuses(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterCheatGuiltExcuses(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.img('images/shared/sex/after/wakeup_shock2.mp4');
    if (((s as any).sex_ev ?? 0)?.['cheat'] === 'Julia'  ||  ((s as any).sex_ev ?? 0)?.['cheat'] === 'girlfriend') {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterCheatGuilt2(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    { label: 'It\'s just sex', handler: (st: GameState) => {
    scene.img('images/shared/sex/after/wakeup_shock2.mp4');
    if (((s as any).sex_ev ?? 0)?.['cheat'] === 'Julia') {
      // TODO-QSP: dynamic text: <i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in lo...
      scene.text(`<i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in love with ${((s as any).npcdesc || '')} or anything. I just really needed some cock, which ${((s as any).sex_ev ?? 0)?.['cheat'] ?? ''} obviously doesn't have. Strap-ons and dildos aren't the same. It's not cheating. It's just sex.</i>.`);
    } else {
      if (((s as any).sex_ev ?? 0)?.['cheat'] === 'girlfriend') {
        // TODO-QSP: dynamic text: <i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in lo...
        scene.text(`<i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in love with ${((s as any).npcdesc || '')} or anything. I just really needed some cock, which my girlfriend obviously doesn't have. Strap-ons and dildos aren't the same. It's not cheating. It's just sex.</i>.`);
      } else {
        // TODO-QSP: dynamic text: <i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in lo...
        scene.text(`<i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in love with ${((s as any).npcdesc || '')} or anything. It's not cheating. It's just sex.</i>.`);
      }
    }
    ...
    if (Object.keys((s as any).fuckbuddy ?? {}).length > 5) {
      ...
      if (((s as any).npc_selfish ?? 0)?.[String((s as any).npcID ?? 0)] !== 1  &&  ((s as any).npc_abusive ?? 0)?.[String((s as any).npcID ?? 0)] !== 1  &&  (Math.floor(Math.random() * 3) + 1) === 3) {
        ...
        if (((s as any).npc_know_cheat ?? 0)?.[String((s as any).npcID ?? 0)] === 0) {
          ...
          if (((s as any).npc_know_cheat ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
            scene.actions([
              { label: 'Thinking about your <<$sex_ev[\'cheat\']>>' }, // TODO-QSP: empty action body
            ]);
          } else {
            scene.text('"Hey," he stretches, looking over at you. "You just wake up too?"');
            scene.actions([
...
function enterGuiltVirginity1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('Something so important and you just gave it away...');
    scene.text('For what? Money...?');
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterGuiltVirginity2(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
  ]);
...
function enterGuiltVirginity2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterGuiltVirginityEnd(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
  ...
  scene.actions([
    ...
    if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
      ...
      if (((s as any).npc_latesleeper ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
        ...
        scene.text(`You feel movement in the bed and turn to see ${((s as any).npcdesc || '')} stirring next to you.`);
