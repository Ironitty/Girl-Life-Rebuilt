  ...
  if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
    if (((s as any).sex_ev ?? 0)?.['boy_make_breakfast'] === 1) {
      if (((s as any).npc_apt_type ?? 0)?.[String((s as any).npcID ?? 0)] === 2  &&  ((s as any).sex_ev ?? 0)?.['loc'] !== 'pc_home'  &&  ((s as any).sex_ev ?? 0)?.['loc'] !== 'family_home') {
        ...
        scene.text(`${((s as any).npcdesc ?? 0)} is already up, doing something in his kitchen area and the smell of coffee and fresh food wafts over to you in bed.`);
      } else {
      ...
      if (((s as any).sex_ev ?? 0)?.['boy_in_shower'] === 1) {
        ...
        scene.text(`"Hey sleepyhead," ${((s as any).npcdesc ?? 0)} smirks as he comes back into the room, a towel around his waist, hair still damp from the shower.`);
      }
    }
  } else {
    if (((s as any).npc_latesleeper ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
      // TODO-QSP: dynamic text: <<$npcdesc>> is still completely passed out beside you, your movement not interr...
      scene.text(`${((s as any).npcdesc ?? 0)} is still completely passed out beside you, your movement not interrupting his snoring in the slightest. Groping around for your phone, the time reads:`);
      ...
      scene.text(`Your movement causes ${((s as any).npcdesc ?? 0)} to stir as well, blinking his eyes open. Reaching down, you grab your phone and switch on the display.`);
    }
  }
...
function enterKissWake(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] !== 1) {
    ...
    scene.text(`You roll over onto ${((s as any).npcdesc ?? 0)}, resting your breasts on his chest as you straddle him.`);
    ...
    scene.text(`You break the kiss from ${((s as any).npcdesc ?? 0)}'s lips and start trailing them down his neck and then his chest and then his stomach and soon you have your lips wrapped around his morning wood.`);
    qspCall(s, 'sex_ev_wakeup', 'bj_wake');
  } },
      ]);
    }
...
function enterBjWake(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_end_free_time ?? 0) <= ((s as any).hour ?? 0) + 2) {
    ...
    scene.text(`You give ${((s as any).npcdesc ?? 0)} one more good suck before you withdraw your lips.`);
    ...
    scene.text(`You give ${((s as any).npcdesc ?? 0)} one more good suck before you withdraw your lips to speak.`);
    ...
    scene.text(`You pull your lips away with a pop and stare ${((s as any).npcdesc ?? 0)} straight in the eye.`);
    scene.text('"Guess."');
...
function enterAfterSleepfuckWake(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['sleep_fuck'] === 2) {
    ...
    scene.text(`"That was nice," you smile sleepily at ${((s as any).npcdesc ?? 0)}. "Our little mid-night romp I mean. Slept like a baby after. How about you?"`);
...
function enterCumshotWakeup1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You gasp as you are suddenly awakened by the shock of something wet spattering across your face. Your eyes flutter open just in time to see another spurt of cum exit ${((s as any).npcdesc ?? 0)}'s cock and hit you square in the face.`);
  if (((s as any).npc_cumshot_wake ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
    ...
    qspCall(s, 'sex_ev_wakeup', 'cumshot_wakeup_clean1');
  } },
    ]);
  }
  ...
  scene.actions([
    ...
    scene.text(`"You were so hot lying there," ${((s as any).npcdesc ?? 0)} snickers, a lewd grin smeared across his face. "I got hard just looking at you."`);
    ...
    scene.text(`You sputter briefly as some of the salty sperm slips past your lips, but can't help but feel amusement when you see the lewd satisfaction on ${((s as any).npcdesc ?? 0)}'s face.`);
    scene.actions([
    ...
    qspCall(s, 'sex_ev_wakeup', 'cumshot_wakeup_clean2');
  } },
...
function enterCumshotWakeupClean1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Before you can make any more protest, ${((s as any).npcdesc ?? 0)}'s cock pushes into your mouth. Your eyes accuse him of being a jerk but your lips suck obediently, drawing the last drops of cum from his shaft.`);
    qspCall(s, 'sex_ev_morning', 'morning_menu1');
    qspCall(s, 'sex_ev_wakeup', 'cumshot_wakeup_bj');
  } },
  ]);
  scene.build();
}
...
function enterCumshotWakeupClean2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Oh fiiiine," you say, mock frowning as ${((s as any).npcdesc ?? 0)}'s cock pushes past your lips and you subserviently suck out the last drops of cum from it.`);
    qspCall(s, 'sex_ev_morning', 'morning_menu1');
    qspCall(s, 'sex_ev_wakeup', 'cumshot_wakeup_bj');
  } },
  ]);
  ...
  scene.actions([
    ...
    if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
      ...
      if (((s as any).npc_latesleeper ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
        ...
        scene.text(`${((s as any).npcdesc ?? 0)} is still asleep next to you snoring softly, completely ignorant to the situation.`);
        qspCall(s, 'sex_ev_morning', 'morning_menu1');
      } else {
        // TODO-QSP: dynamic text: <<$npcdesc>> stirs next to you.
        scene.text(`${((s as any).npcdesc ?? 0)} stirs next to you.`);
...
function enterGuiltStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['guilt_count'] > 0) {
    ...
    qspCall(s, 'sex_ev_wakeup', 'guilt_prostitution1');
  } },
    ]);
  }
  // TODO-QSP: end
...
function enterCheatGuilt1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).stat ?? 0)?.['bf_cheated_on'] <= 10) {
      ...
      scene.text(`Your thoughts wander through your night with ${((s as any).npcdesc ?? 0)}...`);
      qspCall(s, 'sex_ev_wakeup', 'guilt_act_recount');
      if (((s as any).sex_ev ?? 0)?.['prostitution_flag'] === 'prostitution') {
    ...
    qspCall(s, 'sex_ev_wakeup', 'cheat_guilt_excuses');
  } },
  ]);
  scene.build();
}
...
function enterCheatGuiltExcuses(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'sex_ev_wakeup', 'cheat_guilt2');
  } },
    ...
    if (((s as any).sex_ev ?? 0)?.['cheat'] === 'Julia') {
      ...
      scene.text(`<i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in love with ${((s as any).npcdesc ?? 0)} or anything. I just really needed some cock, which ${((s as any).sex_ev ?? 0)?.['cheat']} obviously doesn't have. Strap-ons and dildos aren't the same. It's not cheating. It's just sex.</i>.`);
    } else {
      if (((s as any).sex_ev ?? 0)?.['cheat'] === 'girlfriend') {
        // TODO-QSP: dynamic text: <i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in lo...
        scene.text(`<i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in love with ${((s as any).npcdesc ?? 0)} or anything. I just really needed some cock, which my girlfriend obviously doesn't have. Strap-ons and dildos aren't the same. It's not cheating. It's just sex.</i>.`);
      } else {
        // TODO-QSP: dynamic text: <i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in lo...
        scene.text(`<i>It's just sex,</i> you try to rationalize. <i>It's not like I'm falling in love with ${((s as any).npcdesc ?? 0)} or anything. It's not cheating. It's just sex.</i>.`);
      }
    }
  ...
  if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
    ...
    if (((s as any).npc_latesleeper ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
      if (((s as any).sex_ev ?? 0)?.['type'] === 'hookup') {
        ...
        scene.text(`${((s as any).npcdesc ?? 0)}, the guy from last night, is still asleep next to you, completely ignorant to your internal strife, snoring softly.`);
      } else {
        // TODO-QSP: dynamic text: <<$npcdesc>> is still asleep next to you, completely ignorant to your internal s...
        scene.text(`${((s as any).npcdesc ?? 0)} is still asleep next to you, completely ignorant to your internal strife, snoring softly.`);
      }
      qspCall(s, 'sex_ev_morning', 'morning_menu1');
    } else {
      // TODO-QSP: dynamic text: <<$npcdesc>> stirs next to you.
...
function enterGuiltVirginity1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('Something so important and you just gave it away...');
    scene.text('For what? Money...?');
    qspCall(s, 'sex_ev_wakeup', 'guilt_virginity2');
  } },
    ]);
  } },
  ]);
...
function enterGuiltVirginity2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'sex_ev_wakeup', 'guilt_virginity_end');
  } },
  ...
  scene.actions([
    ...
    if (((s as any).npc_earlyriser ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
      ...
      if (((s as any).npc_latesleeper ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
        ...
        scene.text(`You feel movement in the bed and turn to see ${((s as any).npcdesc ?? 0)} stirring next to you.`);
