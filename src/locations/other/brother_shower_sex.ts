...
function enterBrotherShowerBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterEndingRouteRoutine(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      ]);
    }
    scene.actions([
    ...
    if (((st as any).pcs_horny ?? 0) < 90) {
      ...
      if ((!((st as any).succubusflag ?? 0))) {
        if (((st as any).birth_control ?? 0)?.['think_safe'] === 1  ||  ((st as any).tabletkishot ?? 0) >= 1) {
          // TODO-QSP: dynamic text: <i>Well,</i> you think to yourself, <i>It''s a good thing '+iif(birth_control['i...
...
function enterBrotherShowerAnalVagvAnalv(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterBrotherShowerFinish(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ]);
...
function enterBrotherShowerAnalVagvAnalnv(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "Kolka, ' + iif (brotherQW['anal'] > 0, 'I still can''t let you have my pussy, b...
...
function enterBrotherShowerFuckEnd(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).brotherQW ?? 0)?.['club'] === 1  &&  ((s as any).brotherQW ?? 0)?.['virgin_talk'] === 0) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterBrotherShowerFinishdryoff(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ]);
...
function enterBrotherShowerFinish(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterBrotherShowerFinishEnd(s, scene); (st as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
