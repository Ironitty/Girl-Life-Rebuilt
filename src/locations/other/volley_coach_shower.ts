...
function enter0(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).voltrenerVoyeur = ((st as any).voltrenerVoyeur ?? 0) + (1);
...
function enter1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    qspCall(st, 'npc_relationship', 'modify', 'A69', 2);
    ((st as any).vballVars = (st as any).vballVars ?? {})['coach_lust'] = ((st as any).vballVars['coach_lust'] ?? 0) + (2);
  ...
  if ((((s as any).pcs_horny ?? 0) >= 70  ||  ((s as any).fame ?? 0)?.['pav_slut'] > 50)) {
    ...
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
      ...
      scene.actions([
        { label: 'Masturbate', handler: (st: GameState) => {
    qspCall(st, 'willpower', 'pay', 'self');
    qspCall(st, 'npc_relationship', 'modify', 'A69', 1);
    ((st as any).vballVars = (st as any).vballVars ?? {})['coach_lust'] = ((st as any).vballVars['coach_lust'] ?? 0) + (4);
    ...
    qspCall(st, 'arousal', 'clit_finger', 5, 'masturbate', 'exhibitionism');
...
function enter3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).orgasm_txt = 'The sudden feeling of being watched, was new, acute, the orgasm intense. You drop to the floor of the shower and lay there helplessly for some time.';
    (st as any).orgasm_or = 'custom';
    qspCall(st, 'arousal', 'anal', 10, 'exhibitionism');
    qspCall(st, 'cum_call', 'anus', 'A3');
    qspCall(st, 'arousal', 'end');
    scene.actions([
    ...
    qspCall(st, 'willpower', 'cum_outside', 'resist');
...
function enter4(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'rimming_give', 5, 'sub', 'exhibitionism');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enter5(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'boyStat', 'A69');
    qspCall(st, 'npc_relationship', 'modify', 'A69', 3);
    (st as any).voltrenerVoyeur = ((st as any).voltrenerVoyeur ?? 0) + (1);
    (st as any).ShowerIvan = 3;
    ...
    if (((st as any).vballVars ?? 0)?.['coachsex'] < 1) {
      ((st as any).vballVars = (st as any).vballVars ?? {})['coachsex'] = 1;
    ...
    if (((st as any).stat ?? 0)?.['think_virgin'] === 1) {
      ...
      if (((st as any).pcs_vag ?? 0) <= 15) {
        ...
        if (((st as any).pcs_vag ?? 0) > 15  &&  ((st as any).mesec ?? 0) <= 0) {
    ...
    qspCall(st, 'arousal', 'anal', 15, 'sub');
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Leave', goto: ['gdksport', 'start'] },
    ]);
