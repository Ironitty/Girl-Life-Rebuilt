...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).Enc_Rand ?? 0) === 1  &&  (!((s as any).Jaska_Ev ?? 0))) {
    ...
    if (((st as any).boozeVar ?? 0) === 'beer') {
      ...
      if (((st as any).boozeVar ?? 0) === 'vodka') {
        qspCall(st, 'exp_gain', 'stren', (Math.floor(Math.random() * 2) + 0));
        qspCall(st, 'exp_gain', 'vital', (Math.floor(Math.random() * 2) + 0));
        qspCall(st, 'drugs', 'alcohol', 'vodka');
      } else {
        if (((st as any).boozeVar ?? 0) === 'wine') {
          ...
          if (((st as any).boozeVar ?? 0) === 'spirit') {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 30;
    qspCall(st, 'stat', '');
    (st as any).pcs_billiard = ((st as any).pcs_billiard ?? 0) + ((Math.floor(Math.random() * 3) + 1));
    ...
    if (qspFunc(s, 'money', 'can_afford', 2500) === 0  ||  ((st as any).pcs_intel ?? 0) >= 60  ||  ((st as any).pcs_persuas ?? 0) >= 60) {
      scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal_finger', 5, 'sub', 'exhibitionism');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'stat', '');
    (st as any).orgasm_txt = 'Your fingers on your needy cunt and the sensation of blowing a stranger in the middle of a bar prove too much to you, and you cum, shaking all over, your moans muffled by his ' + ((st as any).dick_girth ?? 0) + ' cock.';
    scene.actions([
    ...
    scene.text('Once the last spurt is out, he wipes the tip of dick against your lips to get the last drops off cum off. Then he tucks his cock back in his pants, orders another drink for each of you and, before you\'re even fully back on your feet, takes his drink and walks off.');
    qspCall(st, 'arousal', 'end');
    qspCall(st, 'cum_call', 'face', ((st as any).npcID ?? 0), 0, '', '', 20);
    qspCall(st, 'cum_call', 'mouth', ((st as any).npcID ?? 0), 0, '', '', 20);
    qspCall(st, 'fame', 'city', 'sex', 1);
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Finish up', goto: ['qwBarPolet', 'bar'] },
