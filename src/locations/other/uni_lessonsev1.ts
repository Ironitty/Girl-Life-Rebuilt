  ...
  if ((!((s as any).temp ?? 0))) {
    ...
    scene.actions([
      ...
      if (((s as any).temp ?? 0) === 2) {
        ...
        scene.text('The second girl plays along, moaning and pretending she\'s getting fucked, but the noise quickly draws Professor Kovalyov\'s attention, who walks over and gives the humping girl some pointers with her technique, which makes her blush. The two girls share a grin at his comments and burst out laughing before they quickly stop and retake their seats as Professor Kovalyov walks back to the front of the classroom with an amused smile on his face.');
      } else {
...
function enterTeachingMethods(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).temp ?? 0))) {
    ...
    if (((s as any).temp ?? 0) === 1) {
      ...
      scene.text('During the halftime break, you relax as the class was pretty mentally exhausting and look around to see how others are handling all the information you\'re getting bombarded with. Two girls notice some of the boys checking them out and start whispering back and forth while giggling before one of them grabs her friend, bends her over the table and starts humping her hard and rough.');
      scene.text('The second girl plays along, moaning and pretending she\'s getting fucked, but the noise quickly draws Professor Pasternak\'s attention, who loudly clears his throat. The girls giggle but stop what they\'re doing and take their seats again.');
    } else {
      if (((s as any).temp ?? 0) === 2) {
        ...
        if (((s as any).temp ?? 0) === 3) {
          ...
          scene.text('As you take a sneak peek, you can see it\'s a special day because she has decided to wear panties this time.');
        } else {
          if (((s as any).temp ?? 0) === 4) {
    ...
    if (((s as any).temp ?? 0) === 1) {
      ...
      if (((s as any).temp ?? 0) === 2) {
        ...
        if (((s as any).temp ?? 0) === 34) {
          scene.img('images/locations/city/island/university/classroom/general_education/girls_bj.jpg');
          scene.text('When a teacher leaves a classroom at school, chaos strikes and everybody starts to act like a child. It seems university is no different. As soon as Professor Marinova leaves the classroom, you see different students talking to each other and taking selfies.');
          scene.text('However, Mia and Yana are on top of their desks, one kneeling before the other. One of them takes the lead and starts to shove her imaginary dick down the other\'s throat. This goes on for nearly a minute before the door opens and Professor Marinova returns. Everyone quickly returns to their seats and the class quietens down.');
        } else {
          if (((s as any).temp ?? 0) === 4) {
            scene.img('images/locations/city/island/university/classroom/teaching_methods/sleeping.jpg');
            scene.text('It\'s hard to stay awake during the class and you\'re not alone with this thought. You see someone has already fallen asleep on their desk.');
  ...
  if ((!((s as any).temp ?? 0))) {
    ...
    if (((s as any).temp ?? 0) === 1) {
      ...
      if (((s as any).temp ?? 0) === 2) {
        ...
        if (((s as any).temp ?? 0) === 3) {
          scene.img('images/locations/city/island/university/classroom/learning_theories/girls_kissing.jpg');
          scene.text('During the break, most of the students just relax, but a few quietly fool around so as not to draw too much attention. A pair of girls, perhaps feeling extra brave and cheeky, start slowly but passionately kissing near the front of the class and a number of girls enjoy the show. They keep kissing until Professor Borisov walks up to them and awkwardly asks them to stop. The two girls ignore him and, unsure what to do, he stands in silence as the two girls continue making out before giggling and taking their seats. A rather flustered looking Professor Borisov then starts the second part of the class.');
        } else {
          if (((s as any).temp ?? 0) === 4) {
            scene.img('images/locations/city/island/university/classroom/general_education/girls_dryhump.jpg');
            scene.text('During the halftime break, you relax as the class was pretty mentally exhausting and look around to see how others are handling all the information you\'re getting bombarded with. The twins start giggling before one of them grabs the other, bends her over the table and starts humping her hard and rough.');
...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  switch (arg) {
    ...
    case 'anatomy_and_physiology':
      ...
      break;
    case 'examination_and_treatment':
      enterExaminationAndTreatment(s, scene);
      break;
    default:
...
export const uni_lessonsev1: LocationDef = {
  name: 'uni_lessonsev1',
