  ...
  if (((s as any).sex_ev ?? 0)?.['locat'] === 'npc_home') {
    ...
    if (((s as any).sex_ev ?? 0)?.['locat'] === 'hotel') {
      ...
      if (((s as any).sex_ev ?? 0)?.['locat'] === 'pc_home') {
        ...
        alert('Error 1: Wrong $ARGS[2] for sex_ev_start, initiate_pre. Given \'' + ((s as any).locArgs?.[2] ?? 0) + '\', not \'npc_home\', \'pc_home\', or \'hotel\' ');
      }
    }
  }
    ...
    if (((s as any).date_ev ?? 0)?.['npc_home_sex'] === 1) {
      ...
      if (((s as any).date_ev ?? 0)?.['pc_home_sex'] === 1) {
        ...
        if (((s as any).date_ev ?? 0)?.['hotel_sex'] === 1) {
          ...
          alert('There is an error here! Let hornguy6 know that date_ev[\'hotel_sex\'], date_ev[\'pc_home_sex\'], and date_ev[\'npc_home_sex\'] are all zero. You will now be failsafed to the NPC\'s home.');
          qspGoto(s, 'sex_ev_start', 'date_npc_home_start');
        }
      }
    }
  ...
  if (Object.keys((s as any).date_ev ?? {}).length > 0) {
    ...
    dynamicGoto(st, 'prevLoc', 'prevArg');
        ...
        if (String((s as any).locArgs?.[1] ?? '') === 'hotel_room') {
          ...
          alert('There is an error here! Let hornguy6 and anjuna know that $ARGS[1] of sex_ev_start, lover_initiate is \'' + ((s as any).locArgs?.[1] ?? 0) + '\' and not \'npc_home\' or \'hotel_room\'. You will now be failsafed to the street.');
          { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterNeighborhoodImage(s, scene); (s as any).locArgs = __savedLocArgs; }
          scene.text(`You knock on ${((s as any).npcdesc ?? '')}'s door but there is no answer.`);
          scene.actions([
            { label: 'Leave', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'prevArg');
      ...
      alert('There is an error here! Let hornguy6 and anjuna know that ' + (((s as any).sex_ev ?? 0)?.['type']) + ' of sex_ev_start, lover_initiate is \'' + (((s as any).sex_ev ?? 0)?.['type']) + '\' and not \'booty_call\'. You will now be failsafed to the street.');
      { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterNeighborhoodImage(s, scene); (s as any).locArgs = __savedLocArgs; }
      scene.text(`You knock on ${((s as any).npcdesc ?? '')}'s door but there is no answer.`);
      scene.actions([
        { label: 'Leave', handler: (st: GameState) => {
...
function enterHookupInitiate(s: GameState, scene: SceneBuilder): void {
  ...
  qspFunc(s, 'npc_preferences[$npcID]');
  ((s as any).npc_rel_type = (s as any).npc_rel_type ?? {})[String((s as any).npcID ?? 0)] = 'hookup';
  ...
  if ((Array.isArray((s as any).hookup_partners) ? ((s as any).hookup_partners as any[]).indexOf(((s as any).npcID ?? 0)) : -1) < 0) {
    (s as any).hookup_partners = [...((s as any).hookup_partners ?? []), ((s as any).npcID ?? 0)];
  }
...
function enterStartingRoute(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_alko ?? 0)?.[String((s as any).npcID ?? 0)] >= ((s as any).npc_alko_limit ?? 0)?.[String((s as any).npcID ?? 0)]  &&  ((s as any).npc_alko_limit ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
    ...
    if (((s as any).sex_ev ?? 0)?.['loc'] !== 'pc_home'  ||  ((s as any).home ?? 0)?.['current'] !== 'parents_home') {
      scene.actions([
        { label: 'Go to the bed', goto: ['sex_ev_foreplay', 'bed_start'] },
      ]);
    } else {
      scene.actions([
        { label: 'Go to your bed', goto: ['sex_ev_foreplay', 'bed_start'] },
        { label: 'Go to Anya\'s bed', handler: (st: GameState) => {
    (st as any).sex_ev[bed_choice] = 'anya_bed';
    ((st as any).sisterQW = (st as any).sisterQW ?? {})['sex_bed_count'] = (((st as any).sisterQW = (st as any).sisterQW ?? {})['sex_bed_count'] ?? 0) + (1);
    ((st as any).sisterQW = (st as any).sisterQW ?? {})['sex_bed'] = ((st as any).daystart ?? 0);
  }, goto: ['sex_ev_foreplay', 'bed_start'] },
      ]);
    }
  }
  scene.build();
