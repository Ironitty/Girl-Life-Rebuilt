...
{ label: 'Try to run away', handler: (st: GameState) => {
    ...
    (st as any).locbegrand = (Math.floor(Math.random() * 5) + 0);
    if ((!((st as any).locbegrand ?? 0))) {
      qspGoto(st, 'city_park', 'start');
    }
    if (((st as any).locbegrand ?? 0) === 1) {
      qspGoto(st, 'city_residential', '');
    }
    if (((st as any).locbegrand ?? 0) === 2) {
      qspGoto(st, 'city_center', '');
    }
    if (((st as any).locbegrand ?? 0) === 3) {
      qspGoto(st, 'city_industrial', '');
    }
    if (((st as any).locbegrand ?? 0) === 4) {
...
{ label: 'Ask him how to do it', handler: (st: GameState) => {
    ...
    qspCall(st, 'npcStat', 'A121');
    (st as any).parkday = ((st as any).daystart ?? 0);
    if ((!((st as any).parkslut ?? 0))) {
      (st as any).parkslut = 1;
...
function enter3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).mc_inventory ?? 0)?.['tech_hidden_cam'] === 1  ||  ((st as any).mc_inventory ?? 0)?.['tech_camera'] === 1) {
      scene.actions([
        { label: 'Take a photo', handler: (st: GameState) => {
    qspCall(st, 'exp_gain', 'photoskl', 2);
    if (((st as any).mc_inventory ?? 0)?.['tech_hidden_cam'] === 1) {
      (st as any).fotoparkrand = (Math.floor(Math.random() * 11) + 0);
    }
    if (((st as any).mc_inventory ?? 0)?.['tech_hidden_cam'] === 0) {
      (st as any).fotoparkrand = (Math.floor(Math.random() * 2) + 0);
    }
    if (((st as any).fotoparkrand ?? 0) > 0) {
    ...
    if (((st as any).mc_inventory ?? 0)?.['tech_hidden_cam'] === 1) {
      ((st as any).mc_inventory = (st as any).mc_inventory ?? {})['tech_hidden_cam'] = 0;
      scene.text('One of the other guys takes your camera and smashes it against a rock');
    } else {
      scene.text('One of the other guys takes your camera and smashes it against a rock');
      ((st as any).mc_inventory = (st as any).mc_inventory ?? {})['tech_camera'] = 0;
    }
    scene.actions([
                ...
                if (((st as any).pcs_run ?? 0) < 130) {
                  ...
                  if (((st as any).pcs_run ?? 0) < 150) {
                    ...
                    if (((st as any).pcs_run ?? 0) < 170) {
                      ...
                      (st as any).peshimraperand = (Math.floor(Math.random() * 101) + 0);
                    }
                  }
                }
              }
...
{ label: 'Agree and join them', handler: (st: GameState) => {
    ...
    qspCall(st, 'willpower', 'rape', 'resist', 'hard');
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
    ...
    if ((!((st as any).parkblackmail ?? 0))) {
      (st as any).guy = ((st as any).guy ?? 0) + (2);
      (st as any).parkblackmail = 1;
      (st as any).parkday = ((st as any).daystart ?? 0);
    }
...
{ label: '"I\'m listening"', handler: (st: GameState) => {
    ...
    (st as any).yaq = ((st as any).yaq ?? 0) + (1);
...
function enter6a(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal_finger', 5, 'sub', 'humiliation');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 5, 'sub', 'humiliation', 'rough', 'rape');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enter6b(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', 5, 'sub', 'humiliation', 'rough', 'rape', 'deepthroat');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'clit_finger', 5, 'sub', 'humiliation');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Keep going', handler: (st: GameState) => {
    ...
    qspCall(st, 'arousal', 'clit_finger', (-5), 'sub', 'humiliation');
    qspCall(st, 'arousal', 'vaginal', 5, 'sub', 'humiliation', 'rough', 'rape');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', 10, 'sub', 'rough', 'humiliation', 'rape');
    qspCall(st, 'cum_call', '');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enter9(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_had_sex ?? 0)?.[String((s as any).npcID ?? 0)] === 0  ||  (!((s as any).SUB ?? 0))) {
    ...
    if (((st as any).npc_gender ?? 0)?.[String((st as any).boy ?? 0)] === 1) {
...
function enterRun(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).PSport ?? 0))) {
    ...
    if (((st as any).daybegskver ?? 0) === ((st as any).daystart ?? 0)) {
      qspCall(st, 'exercise', 'tier1', 30, 'run');
    } else {
    ...
    (st as any).daybegskver = ((st as any).daystart ?? 0);
    ...
    if ((Math.floor(Math.random() * 25) + 0) < 7  &&  ((st as any).cheatVars ?? 0)?.['random_rapists'] !== 1) {
      scene.actions([
        { label: 'Continue', handler: (st: GameState) => {
    ((st as any).temp = (st as any).temp ?? {})['rape_type'] = 'gang';
...
function enterRunEnd(s: GameState, scene: SceneBuilder): void {
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterRunCooldownImage(s, scene); (s as any).locArgs = __savedLocArgs; }
...
function enterFindBaby(s: GameState, scene: SceneBuilder): void {
  if (((s as any).hour ?? 0) >= 8  &&  ((s as any).hour ?? 0) <= 20  &&  ((s as any).park_baby_found ?? 0) < 1) {
    ...
    (st as any).temp_var = qspFunc(s, 'lact_lib', '$get_breastmilk_time', 7, 30);
    if (((st as any).temp_var ?? 0) > 10) {
      ...
      (st as any).milkedvolume = qspFunc(s, 'lact_lib', '$get_breastmilk', 7, 10);
      (st as any).milkedvolume = 0;
      (st as any).minut = ((st as any).minut ?? 0) + (((st as any).temp_var ?? 0));
      if (((st as any).pcs_mood ?? 0) < 20) {
        scene.actions([
    ...
    (st as any).temp_var = qspFunc(s, 'lact_lib', '$get_breastmilk_time', 2, 120);
    if (((st as any).temp_var ?? 0) > 30) {
      ...
      (st as any).milkedvolume = qspFunc(s, 'lact_lib', '$get_breastmilk', 2, 30);
      (st as any).milkedvolume = 0;
      (st as any).minut = ((st as any).minut ?? 0) + 30;
      if (((st as any).pcs_mood ?? 0) < 20) {
        scene.actions([
    ...
    scene.actions([
      ...
      (st as any).milkedvolume = qspFunc(s, 'lact_lib', '$get_breastmilk', 2, ((st as any).temp_var ?? 0));
      (st as any).milkedvolume = 0;
      (st as any).minut = ((st as any).minut ?? 0) + (((st as any).temp_var ?? 0));
      if (((st as any).pcs_mood ?? 0) < 20) {
        scene.actions([
