...
function enterRaise(s: GameState, scene: SceneBuilder): void {
  (s as any).temp_val = ((((s as any).locArgs?.[1] ?? 0) !== 0) ? (((s as any).locArgs?.[1] ?? 0)) : (qspFunc(s, 'mood', 'interpret', ((s as any).locArgs?.[1] ?? 0))));
...
function enterRaiseDisposition(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === 'max') {
    ...
    (s as any).temp_val = ((((s as any).locArgs?.[1] ?? 0) !== 0) ? (((s as any).locArgs?.[1] ?? 0)) : (qspFunc(s, 'mood', 'interpret', ((s as any).locArgs?.[1] ?? 0)) / 5));
  }
