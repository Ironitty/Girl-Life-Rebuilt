...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) > 20) {
    ...
    qspCall(s, 'exp_gain', 'chess', Math.floor(Math.random() * 2) + 0);
    qspCall(s, 'exp_gain', 'intel', Math.floor(Math.random() * 2) + 0);
    (s as any).minut = ((s as any).minut ?? 0) + 60;
    qspCall(s, 'stat', '');
    scene.img('images/locations/pavlovsk/community/chess/play\' + rand(1, 2) + \'.jpg');
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 30;
    qspCall(s, 'exp_gain', 'chess', 2);
    qspCall(s, 'npc_relationship', 'modify', 'A151', 5);
    qspCall(s, 'stat', '');
