...
function enterWineInvite(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.img('images/locations/pavlovsk/resident/albinahome/kitchen.jpg');
    if (((st as any).AlbinaQW ?? 0)?.['wine'] === 0) {
...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).AlbinaQW ?? 0)?.['wine'] === 0) {
    ...
    ((st as any).AlbinaQW = (st as any).AlbinaQW ?? {})['wine_start'] = 1;
...
function enterPornTalk(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).stat ?? 0)?.['porn'] === 0) {
    ...
    if (((st as any).AlbinaQW ?? 0)?.['kolka_know'] < 1) {
...
function enterTellRaped(s: GameState, scene: SceneBuilder): void {
  if (((s as any).stat ?? 0)?.['rape_count'] > 0) {
    ...
    if (((st as any).stat ?? 0)?.['rape_count'] === 1) {
      if (((st as any).pav_swimpool ?? 0)?.['rape'] === 1  ||  ((st as any).pav_swimpool ?? 0)?.['virgin_rape'] === 1) {
      ...
      if (((st as any).memory ?? 0)?.['city_res_rape'] === 1  ||  ((st as any).memory ?? 0)?.['city_center_rape'] === 1) {
        scene.text('"In the city. A man pulled me into an alley, fucked me silly, and just left me there leaking his cum."');
      } else {
        if (((st as any).memory ?? 0)?.['city_park_rape'] === 1) {
          ...
          if (((st as any).memory ?? 0)?.['pav_comm_rape'] === 1) {
          } else {
            if (((st as any).memory ?? 0)?.['pav_res_rape'] === 1) {
            } else {
              if (((st as any).memory ?? 0)?.['pav_park_rape'] === 1) {
              }
            }
          }
        }
