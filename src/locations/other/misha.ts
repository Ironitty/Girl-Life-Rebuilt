...
function enter1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    scene.text(`Fortunately, he's not upset with you at all. The smug grin on Uncle Misha's face never leaves, his cock still hanging out of his pants. "No problem, ${((st as any).pcs_nickname ?? '')}. I recognized that look in your eyes when I pulled my dick out. You'll be back. They all come back." You ignore his laughter as you pull the door shut behind you.`);
    scene.actions([
  ...
  scene.actions([
    ...
    (st as any).dmishaday = ((st as any).daystart ?? 0);
    qspCall(st, 'stat', '');
    scene.text(`He's content just letting you suck him off for a bit, especially when you gather your courage and start to apply your skills. Then, after a while, he pulls back, saying, "Alright, ${((st as any).pcs_nickname ?? '')}, that was nice, but not what I had in mind for today. It's time for the main course!"`);
    if (((st as any).mc_inventory ?? 0)?.['equipped_condoms'] > 0) {
      qspGoto(st, 'misha', '1_2nd_part');
      scene.actions([
        { label: 'You put your condom on his cock', handler: (st: GameState) => {
    (st as any).temp = qspUntranslated(st, "{gs 'dinsex','pc_puts_condom'}", { location: "misha" });
  } },
      ]);
    }
    scene.actions([
      { label: 'Uncle Misha puts his condom on', handler: (st: GameState) => {
    (st as any).temp = qspUntranslated(st, "{gs 'dinsex','boy_puts_his_condom'}", { location: "misha" });
  }, goto: ['misha', '1_2nd_part'] },
...
function enter1_2ndPart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).mish_rand1 ?? 0) <= 60) {
    scene.text(`<center><img ${((s as any).set_imgh ?? '')} src="images/characters/pavlovsk/resident/misha/sex/misha1,${(Math.floor(Math.random() * 8) + 0)}.jpg"></center>`);
  }
  if (((s as any).mish_rand1 ?? 0) > 60) {
  ...
  scene.actions([
    ...
    scene.text(`Uncle Misha takes a minute to catch his breath and admire his handiwork on your face before leaning down and kissing you on the forehead. He throws a couple of crumpled bills on the floor. "Very nice, ${((st as any).pcs_nickname ?? '')}. Here, take these for your efforts," he says as you scramble to pick them up, indicating he's done with you now and expects you to leave.`);
...
function enter2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Your stepfather instructs you, "Well, ${((s as any).pcs_nickname ?? '')}, given your history, I'm sure you know what to do with these. Get started."`);
  qspCall(s, 'stat', '');
  scene.actions([
    { label: 'Get on your knees and suck them off', handler: (st: GameState) => {
    qspCall(st, 'boyStat', 'A28', '1');
    qspCall(st, 'boyStat', 'A28', '2');
    qspCall(st, 'boyStat', 'A54', '' + (Math.floor(Math.random() * 2) + 1) + '');
    (st as any).fatherandmisha = 1;
    ...
    ((st as any).npc_rel = (st as any).npc_rel ?? {})['A28'] = ((st as any).npc_rel['A28'] ?? 0) + (3);
      ...
      if ((!((st as any).stepmisha ?? 0))) {
        ...
        if (((st as any).stepmisha ?? 0) === 1) {
          ...
          if (((st as any).stepmisha ?? 0) === 2) {
            ...
            if ((((st as any).stepmisha ?? 0) === 3  ||  ((st as any).stepmisha ?? 0) === 4)) {
...
function enterTakeWallet(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).mishaQW = (st as any).mishaQW ?? {})['caught'] = 1;
    qspCall(st, 'stat', '');
    ...
    (st as any).agreeText = 'You shake your head. "I didn\'t mean that. Ok, I will be your maid. I am very sorry for what I did. I don\'t know what came over me."';
    qspGoto(st, 'misha', 'maid_agree');
  } },
    ]);
  } },
      { label: 'Agree to be his maid', handler: (st: GameState) => {
    (st as any).agreeText = 'You nod your head. "Yes, thank you, I am very sorry for what I did. I don\'t know what came over me."';
    qspGoto(st, 'misha', 'maid_agree');
  } },
    ]);
  } },
    ...
    qspCall(st, 'mood', 'lower', 'small');
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).mishaQW = (st as any).mishaQW ?? {})['maid'] = 1;
    qspCall(st, 'stat', '');
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/misha/sex/wallet/wallet6.jpg');
...
function enterMaidAgree(s: GameState, scene: SceneBuilder): void {
  ...
  s.scene = { ...s.scene, mainText: String((s as any).agreeText || ''), curActs: [] };
  (s as any).agreeText = undefined;
  ...
  scene.actions([
    ...
    alert('Unfortunately it\'s currently not actually possible to be a maid for Misha - missing content.');
