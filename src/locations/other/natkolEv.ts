...
function enterKolkabj(s: GameState, scene: SceneBuilder): void {
  ((s as any).NatbelQW = (s as any).NatbelQW ?? {})['KolkaFuckDay'] = ((s as any).daystart ?? 0);
  if (((s as any).NatbelQW ?? 0)?.['QWstage'] === 8) {
    ((s as any).NatbelQW = (s as any).NatbelQW ?? {})['QWstage'] = 9;
  }
  qspCall(s, 'stat', '');
  ...
  scene.actions([
    ...
    if (((st as any).NatbelQW ?? 0)?.['KolkaSex'] === 0) {
    ...
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['bj'] = ((st as any).NatbelQW['bj'] ?? 0) + (1);
    scene.text(`<center><video autoplay loop ${((st as any).set_imgh ?? '')} src="images/characters/pavlovsk/school/girl/natasha/sex/natbj` + (Math.floor(Math.random() * 3) + 1) + '.mp4"></video></center>');
    ...
    qspCall(st, 'arousal', 'voyeur_sex', 5, 'dom');
...
function enterSoftsex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['KolkaSex'] = ((st as any).NatbelQW['KolkaSex'] ?? 0) + (1);
    qspCall(st, 'arousal', 'voyeur_sex', 5, 'dom');
    qspCall(st, 'arousal', 'end');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'tits']; enterCumeater(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
    ...
    qspCall(st, 'arousal', 'voyeur_sex', 5, 'dom');
    qspCall(st, 'arousal', 'end');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'labia']; enterCumeater(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Continue', goto: ['natbelapt', 'natroom'] },
    ]);
  } },
    ...
    if (((st as any).NatbelQW ?? 0)?.['KolkaFootjob'] === 0) {
      ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['KolkaFootjob'] = 1;
...
function enterPenetration(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['vaginal'] = ((st as any).NatbelQW['vaginal'] ?? 0) + (1);
    ...
    scene.text('After a few more minutes Kolka says "I\'m going to cum."');
    qspCall(st, 'natbelEv', 'natcumresponse');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 1]; enterKolkacum(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ]);
    ...
    scene.text(`"I won't ${((st as any).pcs_nickname ?? '')}. She can ride my dick all she wants." He tells you as he guides his dick inside her wet pussy.`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    ...
    scene.text('After a few more minutes Kolka says "I\'m going to cum."');
    qspCall(st, 'natbelEv', 'natcumresponse');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 2]; enterKolkacum(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ]);
    ...
    scene.text(`Once she's positioned properly, he urges her on. "Come on Natasha you heard ${((st as any).pcs_nickname ?? '')}."`);
    ...
    scene.text('After a few more minutes Kolka says "I\'m going to cum."');
    qspCall(st, 'natbelEv', 'natcumresponse');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 3]; enterKolkacum(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ]);
...
function enterNatkolkaanalprep(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).NatbelQW ?? 0)?.['anal'] === 0) {
      scene.text(`"Y-Yeah, I'll try it, ${((st as any).pcs_nickname ?? '')}." She says, still out of breath.`);
    } else {
      if (((st as any).NatbelQW ?? 0)?.['anal'] < 5) {
...
function enterNatkolkaanalsex(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).NatbelQW ?? 0)?.['anal'] === 0) {
      ...
      if (((st as any).NatbelQW ?? 0)?.['anal'] < 10) {
    ...
    if (((st as any).NatbelQW ?? 0)?.['anal'] === 0) {
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 4]; enterKolkacum(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      if (((st as any).NatbelQW ?? 0)?.['anal'] < 5) {
    ...
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['anal'] = ((st as any).NatbelQW['anal'] ?? 0) + (1);
  } },
    ]);
  } },
    ...
    if (((st as any).NatbelQW ?? 0)?.['anal'] === 0) {
      ...
      scene.text('You sit down on the bed next to Natasha and whisper in her ear. "Keep your asshole relaxed. You\'re doing great for your first time."');
      scene.text('After a few more minutes of slowly fucking her ass, Kolka says "I\'m going to cum."');
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 5]; enterKolkacum(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      if (((st as any).NatbelQW ?? 0)?.['anal'] < 5) {
        scene.text('"I\'m going to start moving now Natasha." Kolka says as he slowly pushes deeper into her ass, making her moan.');
    ...
    if (((st as any).NatbelQW ?? 0)?.['anal'] === 0) {
      ...
      scene.text('You sit down on the bed next to Natasha and whisper in her ear. "Keep your asshole relaxed. You\'re doing great for your first time."');
      scene.text('After a few more minutes of slowly fucking her ass, Kolka says "I\'m going to cum."');
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 6]; enterKolkacum(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      if (((st as any).NatbelQW ?? 0)?.['anal'] < 5) {
        scene.text('"I\'m going to start moving now Natasha." Kolka says as he slowly pushes deeper into her ass, making her moan.');
        scene.text('"Ok, just go slow so I can adjust." Natasha pleads again.');
...
function enterKolkacum(s: GameState, scene: SceneBuilder): void {
  ((s as any).NatbelQW = (s as any).NatbelQW ?? {})['KolkaSex'] = ((s as any).NatbelQW['KolkaSex'] ?? 0) + (1);
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 1) {
    ...
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['KolkaCP'] = ((st as any).NatbelQW['KolkaCP'] ?? 0) + (1);
    ...
    scene.text('"That\'s all for today Kolka. You can get dressed. I have a few things I want to discuss with Natasha."');
    if ((Math.floor(Math.random() * 100) + 1) >= 95  &&  ((st as any).NatbelQW ?? 0)?.['pregday'] === 0) {
      ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['pregday'] = ((st as any).daystart ?? 0);
    }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'pussy']; enterCumeater(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Continue', goto: ['natbelapt', 'natroom'] },
    ]);
  } },
    ...
    if ((!(Math.floor(Math.random() * 10) + 0))) {
      ...
      scene.text('"That\'s all for today Kolka. You can get dressed. I have a few things I want to discuss with Natasha."');
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'belly']; enterCumeater(st, scene); (st as any).locArgs = __savedLocArgs; }
    }
    scene.actions([
      { label: 'Continue', goto: ['natbelapt', 'natroom'] },
    ]);
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 2) {
    ...
    if ((!(Math.floor(Math.random() * 10) + 0))) {
      ...
      scene.text('"That\'s all for today Kolka. You can get dressed. I have a few things I want to discuss with Natasha."');
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'butt']; enterCumeater(st, scene); (st as any).locArgs = __savedLocArgs; }
    }
    scene.actions([
      { label: 'Continue', goto: ['natbelapt', 'natroom'] },
    ]);
  ...
  if (String((s as any).locArgs?.[1] ?? '') === 4) {
    ...
    scene.text('"That\'s all for today Kolka. You can get dressed. I have a few things I want to discuss with Natasha."');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'anus']; enterCumeater(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Continue', goto: ['natbelapt', 'natroom'] },
    ]);
  } },
  ...
  scene.actions([
    ...
    scene.text('"That\'s all for today Kolka. You can get dressed. I have a few things I want to discuss with Natasha."');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'face']; enterCumeater(st, scene); (st as any).locArgs = __savedLocArgs; }
    scene.actions([
      { label: 'Continue', goto: ['natbelapt', 'natroom'] },
    ]);
  } },
...
function enterCumeater(s: GameState, scene: SceneBuilder): void {
  if (((s as any).trait_vars ?? 0)?.['cumeater'] === 1  &&  ((s as any).brotherQW ?? 0)?.['Sex'] >= 6) {
    ...
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
      ...
      scene.actions([
        { label: 'Resist the urge', handler: (st: GameState) => {
    (st as any).temp_loc = undefined;
    qspCall(st, 'willpower', '');
  }, goto: ['natbelapt', 'natroom'] },
      ]);
    }
    scene.actions([
      { label: '', labelFn: (s: GameState) => String(((((s as any).temp_loc ?? 0) === 'pussy'  ||  ((s as any).temp_loc ?? 0) === 'anus') ? ('Eat Kolka\'s cum out of Natasha\'s ' + ((s as any).temp_loc ?? '') + '') : ('Lick Kolka\'s cum off Natasha\'s ' + ((s as any).temp_loc ?? '') + '')) ?? ''), handler: (st: GameState) => {
    if (((st as any).temp_loc ?? 0) === 'labia'  ||  ((st as any).temp_loc ?? 0) === 'pussy'  ||  ((st as any).temp_loc ?? 0) === 'anus') {
      qspCall(st, 'npcStat', 'A16');
      scene.img(`images/shared/sex/cum/eat/eat_cum_${((st as any).temp_loc ?? '')}.mp4`);
      scene.text('After Kolka closes the door you move closer to Natasha who\'s still recovering her breath on the bed. His cum is slowly starting to drip ' + ((((st as any).temp_loc ?? 0)==='labia') ? ('off') : ('out of')) + ` her ${((st as any).temp_loc ?? '')}, which simply becomes too much for you to bear. You jump on the bed, startling her` + ((((st as any).temp_loc ?? 0)==='labia') ? ('') : (' and flip her over on all fours')) + ', then dive mouth first towards her ' + ((((st as any).temp_loc ?? 0)==='labia') ? ('cum glazed') : ('cream filled')) + ' hole. Natasha is in no condition to stop you so all you hear are strained moans of protest and pleasure as you happily slurp and lick the sticky mixture of her juices and Kolka\'s spunk from ' + ((((st as any).temp_loc ?? 0)==='anus') ? ('her stretched anal ring.') : ('between her pink pussy petals.')));
      scene.text('This goes on for about three minutes until you\'re satisfied you got most of your snack ' + ((((st as any).temp_loc ?? 0)==='labia') ? ('off') : ('out of')) + ' the Natasha-shaped cum dispenser who is now lying there, chest heaving, with eyes glazed over and only able to produce incoherent groaning noises. If you weren\'t so absorbed in getting to the cum, you would have noticed she came twice in these last three minutes. You feel a tinge of pity for the poor girl after you ' + ((((st as any).temp_loc ?? 0)==='labia') ? ('get up') : ('push her off you')) + ' and see the state she\'s in. Pulling the bed covers over her naked body, you give her a small kiss on the lips and leave her room, hoping she has some nice dreams in her orgasm-induced coma.');
      qspCall(st, 'arousal', '', ((((st as any).temp_loc ?? 0)==='anus') ? ('rimming_give') : ('cuni_give')), 3, 'lesbian', 'dom');
      qspCall(st, 'arousal', 'kiss', 1, 'lesbian', 'dom');
      qspCall(st, 'arousal', 'end');
      (st as any).minut = ((st as any).minut ?? 0) + 1;
      qspCall(st, 'npc_relationship', 'modify', 'A16', 'love');
      scene.actions([
        { label: 'Leave the apartment', goto: ['pod_ezd', 'etaj_4'] },
      ]);
    } else {
      scene.img(`images/shared/sex/cum/eat/eat_cum_${((st as any).temp_loc ?? '')}.mp4`);
      scene.text(`After Kolka closes the door you move closer to Natasha who's still recovering her breath on the bed. His cum is slowly starting to slide down her skin, which simply becomes too much for you to bear. You jump on the bed, startling her and dive mouth first towards her cum covered ${((st as any).temp_loc ?? '')}. Natasha is in no condition to stop you so all you hear are strained moans of protest and pleasure as you happily slurp and lick the spunk off her body.`);
      ...
      qspCall(st, 'arousal', 'foreplay_give', 2, 'lesbian', 'dom');
...
function enterShowerspy(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).loc = 'natbelapt';
  (s as any).locM_arg = 'bathroom';
  (s as any).locM = 'natbelapt';
  qspCall(s, 'stat', '');
  (s as any).location_type = 'bathroom';
  scene.img('images/locations/pavlovsk/resident/apartment/natbelapt/sex/natkolkashowerbj1.jpg');
  scene.text('You open the bathroom door as quietly as you can and peer through the crack to see Natasha on her knees sucking Kolka off while in the shower.');
  qspCall(s, 'arousal', 'voyeur_sex', 5);
  if (((s as any).NatbelQW ?? 0)?.['QWstage'] < 5) {
    scene.text('Not wanting to intrude any further you decide to leave them be and close the door again.');
    return;
  }
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    scene.text('They\'re so absorbed in each other that they don\'t even notice your entry.');
    scene.text('"Taking private lessons Kolka?" You say with a grin on your face.');
    scene.text('At the sound of your voice, they both startle visibly. Natasha\'s face instantly turns crimson.');
    scene.text('"Don\'t stop on my account. I love watching you guys get each other off."');
    qspCall(st, 'arousal', 'voyeur_sex', 5);
    scene.actions([
      { label: 'Make Kolka fuck her', handler: (st: GameState) => {
    scene.img('images/locations/pavlovsk/resident/apartment/natbelapt/sex/natkolkashowerfuck.jpg');
    scene.text('"It looks like Natasha needs a dick in her pussy. Give her what she wants Kolka."');
    scene.text('Obedience seems to be well trained into Natasha by now as she immediately gets up and offers her pussy to Kolka.');
    scene.text('Kolka doesn\'t waste any time and immediately plunges his full length inside her, making her moan loudly. He starts to move gently at first, but Natasha almost immediately starts begging him: "Please fuck me hard and fast Kolka. Yes… Like that… Keep going."');
    scene.text('The bathroom is suddenly filled with the sounds of Natasha\'s moaning and flesh smacking against flesh.');
    scene.text('It doesn\'t take long before Natasha\'s moaning turns into pleading again, "Please… Yes… Almost…" before her legs start trembling in Kolka\'s arms.');
    scene.text('You squirm a bit yourself as you watch Natasha having what looks to be one hell of an orgasm.');
    scene.text('It seems to take her forever to stop cumming, but eventually, she just can\'t stay on her feet any longer and sinks down to the floor. Kolka looks like he\'s on the edge himself.');
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'arousal', 'voyeur_sex', (-20));
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Let him cum on her face', handler: (st: GameState) => {
    scene.img('images/locations/pavlovsk/resident/apartment/natbelapt/sex/natkolkashowercum.jpg');
    scene.text('"Looks like you did her in Kolka." You say. "Come lie down here Natasha and let him cum on your face."');
    scene.text('She looks dazed as she crawls out of the shower and lies down on her back. Kolka follows her out and straddles her chest. He immediately starts jerking off furiously over her face.');
    scene.text('"Open your mouth. Here it comes." He tells her and seconds after that he starts shooting a huge load of his cum into her mouth and over her face.');
    scene.text('By the time he\'s done cumming Natasha\'s mouth is overflowing, and half of her face is covered in cum.');
    scene.text('"A true cum shower. Fitting." You say, unable to hold in your laughter. "Well, Kolka looks like it\'s time for you to get going. I\'ll help her get to her feet."');
    scene.text('Kolka quickly gets dressed before he slips out of the bathroom. After he\'s gone you pull Natasha to her feet and help her over to the sink to clean up.');
    qspCall(st, 'arousal', 'voyeur_sex', 5);
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['vaginal'] = ((st as any).NatbelQW['vaginal'] ?? 0) + (1);
    ((st as any).NatbelQW = (st as any).NatbelQW ?? {})['KolkaSex'] = ((st as any).NatbelQW['KolkaSex'] ?? 0) + (1);
    scene.actions([
      { label: 'Go to Natasha\'s room', goto: ['natbelapt', 'natroom'] },
    ]);
  } },
    ]);
  } },
