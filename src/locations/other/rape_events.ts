...
function enterUrbanFightRape(s: GameState, scene: SceneBuilder): void {
  ...
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterUrbanRapeStruggle(s, scene); (s as any).locArgs = __savedLocArgs; }
...
function enterUrbanRape(s: GameState, scene: SceneBuilder): void {
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterRapistGenerate(s, scene); (s as any).locArgs = __savedLocArgs; }
  (s as any).temp_rape_angry = undefined;
  ...
  scene.actions([
    ...
    ((st as any).temp = (st as any).temp ?? {})['skip'] = 0;
    qspCall(st, 'stat', '');
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanRapeStruggle(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanRapeLimp(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    { label: 'Scream', handler: (st: GameState) => {
    ((st as any).temp = (st as any).temp ?? {})['skip'] = 0;
    qspCall(st, 'stat', '');
    scene.img('images/locations/shared/street/sex/rape2.jpg');
    qspCall(st, 'pain', '7', 'head', 'slam');
...
function enterUrbanRapeStruggle(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if ((Math.floor(Math.random() * 2) + 0) === 1) {
      ((st as any).temp = (st as any).temp ?? {})['rape'] = 'vaginal';
      scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', (Math.floor(Math.random() * 6) + 5), 'rough', 'rape');
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Endure it', handler: (st: GameState) => {
    qspCall(st, 'cum_call', '', 'rapist');
    qspCall(st, 'stat', '');
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'vaginal']; enterUrbanRapeStruggleAfter(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
      ]);
    } else {
      ((st as any).temp = (st as any).temp ?? {})['rape'] = 'anal';
      scene.actions([
        { label: '<i><b>MMMMMRRMMPH-!!!</b></i>', handler: (st: GameState) => {
    scene.img('images/locations/shared/street/sex/rape.mp4');
    qspCall(st, 'arousal', 'anal', (Math.floor(Math.random() * 6) + 5), 'rough', 'rape');
    qspCall(st, 'stat', '');
    scene.text('<i><b>"MMMMMRRMMPH-!!!"</b></i>');
    ...
    qspCall(st, 'cum_call', 'anus', 'rapist');
    qspCall(st, 'stat', '');
    scene.img('images/locations/shared/street/sex/rape_cum.mp4');
    scene.text('After what feels like an eternity, the man thrusts into you several times, much harder than before before completely burying his cock inside you.');
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', 'anal']; enterUrbanRapeStruggleAfter(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
      ]);
...
function enterUrbanRapeLimp(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    (st as any).orgasm_or = 'no';
    qspCall(st, 'arousal', '$temp[\'rape\']', (Math.floor(Math.random() * 6) + 5), 'rough', 'rape');
    qspCall(st, 'stat', '');
    scene.actions([
    ...
    if (((st as any).temp ?? 0)?.['rape'] === 'vaginal') {
      scene.text('Something warm trickles from between your legs…');
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanVaginalCry(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      if (((st as any).temp ?? 0)?.['rape'] === 'anal') {
        scene.text('Something warm trickles from your ass…');
        { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanAnalCry(st, scene); (st as any).locArgs = __savedLocArgs; }
      }
    }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanRepress(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
    ]);
...
function enterUrbanVaginalCry(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).pantyworntype ?? 0) !== 'none') {
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanPantiesAfter(st, scene); (st as any).locArgs = __savedLocArgs; }
    } else {
      if (((st as any).PCloSkirt ?? 0) > 0) {
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanRapeEnd(st, scene); (st as any).locArgs = __savedLocArgs; }
    }
  } },
  ]);
  scene.build();
...
function enterUrbanRepress(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    ((st as any).temp = (st as any).temp ?? {})['rape_repress'] = 1;
    scene.img('images/locations/shared/street/alleynight.jpg');
    ...
    if (((st as any).pantyworntype ?? 0) !== 'none') {
      ...
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterUrbanRepressEnd(st, scene); (st as any).locArgs = __savedLocArgs; }
    }
  } },
  ]);
  scene.build();
...
function enterUrbanPantiesAfter(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if (((st as any).temp ?? 0)?.['rape_repress'] === 1) {
      if (((st as any).PCloSkirt ?? 0) > 0) {
...
function enterUrbanRapeEnd(s: GameState, scene: SceneBuilder): void {
  ...
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterUrbanPoliceAfter(s, scene); (s as any).locArgs = __savedLocArgs; }
  scene.actions([
...
function enterParkRapeStruggleStart(s: GameState, scene: SceneBuilder): void {
  ...
  { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterParkGangrapeStruggle1(s, scene); (s as any).locArgs = __savedLocArgs; }
  scene.build();
}
...
function enterParkGangrapeFightLoss(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'vaginal', 1, (((st as any).rapist ?? 0)?.[0] ?? 0), 'rough', 'rape');
    qspCall(st, 'stat', '');
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkGangrapeStruggle2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterParkGangrapeStruggle2(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', (Math.floor(Math.random() * 3) + 3), (((st as any).rapist ?? 0)?.[0] ?? 0), 'rough', 'rape');
    qspCall(st, 'stat', '');
    scene.img('images/shared/sex/rape/park_rape1.jpg');
    if (((st as any).stat ?? 0)?.['think_virgin'] === 1) {
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkGangrapeTp(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterParkGangrapeTp(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkGangrapeEnd1(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterParkGangrapeCum(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'cum_call', '', (((s as any).rapist ?? 0)?.[0] ?? 0));
  qspCall(s, 'cum_call', 'anus', (((s as any).rapist ?? 0)?.[1] ?? 0));
  qspCall(s, 'cum_call', 'mouth_swallow', (((s as any).rapist ?? 0)?.[2] ?? 0));
...
function enterParkGangrapeEnd1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkGangrapeCum(st, scene); (st as any).locArgs = __savedLocArgs; }
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkGangrapeEnd2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    { label: 'Pass out', handler: (st: GameState) => {
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkGangrapeCum(st, scene); (st as any).locArgs = __savedLocArgs; }
    ...
    if (((st as any).cheatVars ?? 0)?.['abduction_chance'] === 1) {
      (st as any).temp_thresh = 0;
    } else {
      if (((st as any).cheatVars ?? 0)?.['abduction_chance'] === 0) {
        (st as any).temp_thresh = 3;
      } else {
        if (((st as any).cheatVars ?? 0)?.['abduction_chance'] === 2) {
          (st as any).temp_thresh = 1;
        } else {
          if (((st as any).cheatVars ?? 0)?.['abduction_chance'] === 3) {
            (st as any).temp_thresh = 5;
          } else {
            if (((st as any).cheatVars ?? 0)?.['abduction_chance'] === 4) {
              (st as any).temp_thresh = 8;
            }
          }
        }
      }
    }
    if ((Math.floor(Math.random() * 40) + 1) <= ((st as any).temp_thresh ?? 0)) {
      (st as any).temp_thresh = undefined;
      qspCall(st, 'abduction', 'abdCarTrunk');
    } else {
      (st as any).temp_thresh = undefined;
      { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkWakeup(st, scene); (st as any).locArgs = __savedLocArgs; }
    }
  } },
    ]);
  } },
...
function enterParkGangrapeEnd2(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkWalkaway1(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    { label: 'Pass out', handler: (st: GameState) => {
    scene.img('images/shared/sex/rape/park_rape5.jpg');
    scene.text('In the end, it\'s too much to bear.');
...
function enterParkWalkaway1(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterParkWalkaway2(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
  ]);
  scene.build();
}
...
function enterRapeReport(s: GameState, scene: SceneBuilder): void {
  if (((s as any).region ?? 0) === 'pav') {
    ...
    if (((st as any).loc ?? 0) === 'pav_commercial') {
      ...
      if (((st as any).loc ?? 0) === 'pav_residential') {
        scene.text('"In th-the commercial district. But he pulled me away. I… I don\'t remember how far…"');
      }
    }
