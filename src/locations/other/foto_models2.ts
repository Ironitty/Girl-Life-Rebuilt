...
function enterEmily_1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).emily = (st as any).emily ?? {})['hate'] = ((st as any).emily['hate'] ?? 0) + (1);
    scene.img('images/locations/city/citycenter/photo/portraits/emily_r/talking/face.jpg');
...
function enterEmily_2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).emily ?? 0)?.['hate'] === 1) {
    ...
    // TODO-QSP: dynamic text: "Uhh, yeah… I never introduced myself the last time we saw each other. I''m <<$m...
...
function enterEmilyChat(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).emily ?? 0)?.['model_exp'] !== ((s as any).daystart ?? 0)) {
    ...
    ((st as any).emily = (st as any).emily ?? {})['model_exp'] = ((st as any).daystart ?? 0);
    scene.img('images/locations/city/citycenter/photo/portraits/emily_r/talking/face.jpg');
...
function enterJeff_2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Anyways, gotta run! Got dirty pics to take and money to get paid! Bye ${(((st as any).model ?? 0)?.['nickname'] ?? '')}!" she says and runs off.`);
    scene.actions([
      { label: 'Leave', goto: ['foto', 'studio'] },
    ]);
  } },
    ...
    scene.text(`"Anyways, gotta run! Got dirty pics to take and money to get for it! Bye ${(((st as any).model ?? 0)?.['nickname'] ?? '')}!" she says and runs off.`);
    scene.actions([
      { label: 'Leave', goto: ['foto', 'studio'] },
    ]);
  } },
...
function enterNushChat1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).nush_chat ?? 0)?.['ask_anastasia'] === 0) {
    ...
    qspCall(st, 'stat', '');
    ((st as any).nush_chat = (st as any).nush_chat ?? {})['ask_anastasia'] = 1;
    scene.text('<center><b>Photography Studio</b></center>');
    scene.img('images/characters/pavlovsk/school/girl/anushka/nush_model/nush_talk.jpg');
    if (((st as any).nush_chat ?? 0)?.['been_here_awhile'] === 0) {
      ((st as any).nush_chat = (st as any).nush_chat ?? {})['been_here_awhile'] = 1;
  ...
  if (((s as any).nush_chat ?? 0)?.['ask_krystal'] === 0) {
    ...
    if (((st as any).pcs_hotcat ?? 0) >= 6  &&  ((st as any).npc_rel ?? 0)?.['A144'] >= 50  &&  ((st as any).anushkaQW ?? 0)?.['sex'] >= 1) {
    ...
    ((st as any).nush_chat = (st as any).nush_chat ?? {})['ask_krystal'] = 1;
    qspCall(st, 'arousal', 'voyeur', 5);
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
  ...
  if (((s as any).nush_chat ?? 0)?.['ask_emily'] === 0) {
    ...
    qspCall(st, 'stat', '');
    ((st as any).nush_chat = (st as any).nush_chat ?? {})['ask_emily'] = 1;
    scene.text('<center><b>Photography Studio</b></center>');
    scene.img('images/characters/pavlovsk/school/girl/anushka/nush_model/nush_talk.jpg');
    if (((st as any).nush_chat ?? 0)?.['been_here_awhile'] === 0) {
      ((st as any).nush_chat = (st as any).nush_chat ?? {})['been_here_awhile'] = 1;
  ...
  if (((s as any).nush_chat ?? 0)?.['ask_jeff'] === 0) {
    ...
    qspCall(st, 'stat', '');
    ((st as any).nush_chat = (st as any).nush_chat ?? {})['ask_jeff'] = 1;
    scene.text('<center><b>Photography Studio</b></center>');
    scene.img('images/characters/pavlovsk/school/girl/anushka/nush_model/nush_talk.jpg');
    if (((st as any).nush_chat ?? 0)?.['been_here_awhile'] === 0) {
      ((st as any).nush_chat = (st as any).nush_chat ?? {})['been_here_awhile'] = 1;
  ...
  if (((s as any).nush_chat ?? 0)?.['ask_mari'] === 0) {
    ...
    qspCall(st, 'stat', '');
    ((st as any).nush_chat = (st as any).nush_chat ?? {})['ask_mari'] = 1;
    scene.text('<center><b>Photography Studio</b></center>');
    scene.img('images/characters/pavlovsk/school/girl/anushka/nush_model/nush_talk.jpg');
    if (((st as any).nush_chat ?? 0)?.['been_here_awhile'] === 0) {
      ((st as any).nush_chat = (st as any).nush_chat ?? {})['been_here_awhile'] = 1;
...
function enterNushModeling(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).anushkaQW ?? 0)?.['model'] < 6) {
    ...
    qspGoto(s, 'foto_models2', qspUntranslated(s, "nush_photoshoot(Math.floor(Math.random() * 5) + 1)", { location: "foto_models2" }));
  }
  // TODO-QSP: end
  scene.build();
}
