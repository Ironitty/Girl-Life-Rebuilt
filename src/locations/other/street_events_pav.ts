...
function enterMinorNakedHawker(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('She has some cosmetics for ' + qspFunc(s, 'money', 'string_price', ((st as any).tovpay1 ?? '')) + '.');
    (st as any).tovpay2 = (Math.floor(Math.random() * (9 - 3 + 1)) + (3));
    scene.text('She has lip balm for ' + qspFunc(s, 'money', 'string_price', ((st as any).tovpay2 ?? '')) + '.');
    (st as any).tovpay3 = (Math.floor(Math.random() * (5 - 2 + 1)) + (2));
    scene.text('She has some vitamins for ' + qspFunc(s, 'money', 'string_price', ((st as any).tovpay3 ?? '')) + '.');
...
function enterMinorImprovisedMarket(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).tovpay1 = (Math.floor(Math.random() * (8 - 3 + 1)) + (3));
    scene.text('They have some cosmetics for ' + qspFunc(s, 'money', 'string_price', ((st as any).tovpay1 ?? '')) + '.');
    (st as any).tovpay2 = (Math.floor(Math.random() * (9 - 3 + 1)) + (3));
    scene.text('They have lip balm for ' + qspFunc(s, 'money', 'string_price', ((st as any).tovpay2 ?? '')) + '.');
    (st as any).tovpay3 = (Math.floor(Math.random() * (5 - 2 + 1)) + (2));
    scene.text('They have some vitamins for ' + qspFunc(s, 'money', 'string_price', ((st as any).tovpay3 ?? '')) + '.');
