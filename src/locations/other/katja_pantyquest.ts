...
function enterKitchenShowPussy(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Really, ${((s as any).pcs_nickname ?? 0)}! This again? It's like you don't trust me," Katja says, rolling her eyes.`);
      ...
      if (((s as any).katjaQW ?? 0)?.['slut'] < 50  &&  ((s as any).katjaQW ?? 0)?.['pantiesQWstage'] < 2) {
        ...
        if (((s as any).katjaQW ?? 0)?.['slut'] < 100  &&  ((s as any).katjaQW ?? 0)?.['pantiesQWstage'] < 4) {
          ...
          if (((s as any).katjaQW ?? 0)?.['pantiesQWstage'] >= 5) {
            ...
            scene.text(`"Very nice, ${((s as any).pcs_nickname ?? 0)}. ' + iif( pcs_pubes > 3 , 'But you really need to shave that bush off', 'Perfectly smooth, how a pussy should be') + '," Katja says as her eyes devour your exposed pussy.`);
          } else {
            // TODO-QSP: dynamic text: "<<$pcs_nickname>>, you naughty girl! You should put some panties on," Katja say...
            scene.text(`"${((s as any).pcs_nickname ?? 0)}, you naughty girl! You should put some panties on," Katja says, not at all convincingly, while her eyes devour your exposed pussy.`);
...
function enterUniExit(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).trait_vars ?? 0)?.['panty_preference'] > 0  &&  ((s as any).pantyworntype ?? 0) === 'none'  &&  ((s as any).npc_had_sex ?? 0)?.['A14']  &&  ((s as any).katjaQW ?? 0)?.['no_panties_day'] !== ((s as any).daystart ?? 0)) {
    ...
    if (((s as any).katjaQW ?? 0)?.['slut'] < 50  &&  ((s as any).katjaQW ?? 0)?.['pantiesQWstage'] === 1) {
      // TODO-QSP: dynamic text: "<<$pcs_nickname>>! You can't just go around flashing your pussy; put some panti...
      scene.text(`"${((s as any).pcs_nickname ?? 0)}! You can't just go around flashing your pussy; put some panties on!" Katja scolds, not really angry.`);
...
function enterCoffeeHole(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).katjaQW ?? 0)?.['slut'] < 30  &&  ((s as any).katjaQW ?? 0)?.['pantiesQWstage'] === 0) {
    // TODO-QSP: dynamic text: "<<$pcs_nickname>>! You can't just go around flashing your pu… You aren't wearin...
    scene.text(`"${((s as any).pcs_nickname ?? 0)}! You can't just go around flashing your pu… You aren't wearing panties! What are you doing, put some panties on!" Katja says, very shocked.`);
    ...
    if (((s as any).katjaQW ?? 0)?.['slut'] < 50  &&  ((s as any).katjaQW ?? 0)?.['pantiesQWstage'] < 2) {
      // TODO-QSP: dynamic text: "<<$pcs_nickname>>! You can't just go around flashing your pussy. Put some panti...
      scene.text(`"${((s as any).pcs_nickname ?? 0)}! You can't just go around flashing your pussy. Put some panties on!" Katja says, not really angry.`);
      scene.text('"I flash who I want to, and I don\'t wear panties since it\'s much nicer not to. Now, show me if you wear panties or not," you demand as you close your legs, covering your pussy again.');
    } else {
      if (((s as any).katjaQW ?? 0)?.['slut'] < 100  &&  ((s as any).katjaQW ?? 0)?.['pantiesQWstage'] < 4) {
        // TODO-QSP: dynamic text: "Stop it, <<$pcs_nickname>>! You should really put some panties on!" Katja says,...
