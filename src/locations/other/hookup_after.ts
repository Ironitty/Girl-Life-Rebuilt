...
function enterCreampieMad(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(String(((st as any).npc_apt_bedroom ?? {})[String((st as any).npcID ?? '')] || ''));
...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(String(((st as any).npc_apt_bathroom ?? {})[String((st as any).npcID ?? '')] || ''));
...
function enterNpcFuckbuddyRequest(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((st as any).hookup = (st as any).hookup ?? {})['text_message'] = (Math.floor(Math.random() * 2) + 1);
    if (((st as any).hookup ?? 0)?.['text_message'] === 1) {
      scene.text('  ' + qspFunc(s, 'wrap', 'accent b', 'hey fuck buddy'));
      scene.text('"Hey yourself," you say aloud, looking up at him. "Anyways, see you around."');
    } else {
      if (((st as any).hookup ?? 0)?.['text_message'] === 2) {
        scene.text('  ' + qspFunc(s, 'wrap', 'accent b', 'send nudes'));
