...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).kameraKPZ ?? 0) < 3) {
    ...
    (st as any).kameraKPZ = ((st as any).kameraKPZ ?? 0) + (1);
    qspCall(st, 'stat', '');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 260;
    qspCall(st, 'stat', '');
    scene.img('images/characters/city/ilyushkin/event/kpz.jpg');
    ...
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWPoliceDayCount'] = 1;
    ((st as any).BurgerQW = (st as any).BurgerQW ?? {})['IlyQWTatianaHelp'] = 1;
    qspCall(st, 'stat', '');
