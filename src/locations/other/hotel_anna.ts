...
function enterDefault(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 4) {
    ...
    scene.text(`You knock at Anna's room. "${((s as any).pcs_nickname ?? 0)} come in, the door is open!" You hear her shouting.`);
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 5) {
    ...
    scene.text(`"${((s as any).pcs_nickname ?? 0)} my dear! I'm totally fine now that you are here! I was waiting for you, well I wasn't alone but my guest knew I was waiting for someone, anyway tell me all about yourself. I miss our chit chat."`);
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 6) {
    ...
    scene.text(`She walks over to the woman, "As for you Jeanie, you've got another hour of this while I entertain my client. Maybe spend that time reconsidering betting against ${((s as any).pcs_firstname ?? 0)} again?"`);
    ...
    scene.text(`"Hi, I'm ${((s as any).pcs_firstname ?? 0)}" You say as introduction and think it might have been polite to have done so before tearing gaffer tape off her face.`);
    scene.actions([
    ...
    scene.text(`It doesn't take long as the knots are so poorly tied. "Thank you ${((s as any).pcs_nickname ?? 0)} this slut's name is Jeanine."`);
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 7) {
    ...
    scene.text(`"${((s as any).pcs_nickname ?? 0)} Move, move… it's started!" You have no time to realize that you are dragged by Anna in the main room.`);
    scene.text('"Err… Hi Anna!"');
    // TODO-QSP: dynamic text: "Oh sorry! Hi <<$pcs_nickname>>, take a seat. I'm watching a film and its just g...
    scene.text(`"Oh sorry! Hi ${((s as any).pcs_nickname ?? 0)}, take a seat. I'm watching a film and its just getting to the good bit, come in we can still talk."`);
...
export const hotel_anna: LocationDef = {
  ...
  region: 'other',
  locationType: 'event',
  description: ['You decide to check for Anna Igorevna. You think it\'s the better option, there\'s no harm in visiting her and you don\'t want to be disrespectful, especially if not doing so lead her to say something to Mr. Leonidovich. You reach the floor and to your surprise Anna Igorevna seems to waiting for you.'],
  enter: enter,
};
