...
function enterMeeting(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 1) {
    ...
    scene.text(`Both of you enter the room and go to sit on the sofa. "Well ${((s as any).pcs_nickname || '')}, we shouldn't be disturbed… or peeped." She says with a little smile.`);
    ...
    scene.text(`"${((s as any).pcs_nickname || '')}, I'm not going to say anything about your peeking to anyone whatever happens. I should have locked the door so that is my fault, I was playing a game of risk. Knowing someone might open the door at any time it is thrilling and dangerous." She pauses. "You can come to visit me here on Mondays and Tuesdays at 20:00 if you decide you want to learn. We'll chit-chat a little and maybe come to some other arrangements in the future to satisfy your curiosity.".`);
    ...
    scene.text(`"I'm glad you came and please, call me Anna. Now I have things to do so you'll have to leave for now." Anna Igorevna leads you to the door. "See you soon ${((s as any).pcs_nickname || '')}."`);
    scene.actions([
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 2) {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enter2a(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ...
    scene.text(`"Hahaha, I'm joking ${((s as any).pcs_nickname || '')}, don't worry. I don't like to judge, and don't care if someone else judges me, so feel free to speak openly with me: what you saw is a lifestyle."`);
    scene.text('"And this lifestyle is…?"');
    scene.text('"The BDSM lifestyle. BDSM stands for: Bondage, Domination or Discipline, Submission or Sadism and Masochism. It comes in many forms, and there are a lot of disciplines; possibilities are nearly endless. BDSM can be both fictional and practical, mental and physical. From an erotic spanking or a vanilla roleplay, from giving or accepting verbal commands or a simple dirty talk, through to a complex and intense full session. Limits are discussed and although they may be pushed, hard limits are never overstepped; as a matter of fact if something goes wrong we use a "safe word", usually "Red" to stop immediately. In short, we look out for each other."');
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enter2a(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enter2a(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 3) {
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enter3a(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
    ...
    scene.text(`"Don't mind me ${((s as any).pcs_nickname || '')}, the main thing is that you listen"`);
    ...
    scene.text(`[Snap snap] (She clicks her fingers) "${((s as any).pcs_nickname || '')}, my eyes are up here, I don't mind you looking at me but we don't have the whole day."`);
...
function enter3a(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).annaToy ?? 0) === 3) {
      ...
      scene.text(`${((s as any).pcs_nickname || '')}, you seem a little distracted… should I cover up?"`);
...
function enter3b(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 4) {
    ...
    // TODO-QSP: dynamic text: You knock at Anna's room. "<<$pcs_nickname>> come in, the door is open!" You hea...
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enter4a(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enter4a(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 5) {
    ...
    // TODO-QSP: dynamic text: "<<$pcs_nickname>> my dear! I'm totally fine now that you are here! I was waitin...
    scene.text(`"${((s as any).pcs_nickname || '')} my dear! I'm totally fine now that you are here! I was waiting for you, well I wasn't alone but my guest knew I was waiting for someone, anyway tell me all about yourself. I miss our chit chat."`);
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    qspCall(s, 'stat', '');
    ((s as any).AnnaQW = (s as any).AnnaQW ?? {})['dress'] = 1;
    ((s as any).AnnaQW = (s as any).AnnaQW ?? {})['trust'] = ((s as any).AnnaQW['trust'] ?? 0) + (1);
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 6) {
    ...
    // TODO-QSP: dynamic text: She walks over to the woman, "As for you Jeanie, you've got another hour of this...
    scene.text(`She walks over to the woman, "As for you Jeanie, you've got another hour of this while I entertain my client. Maybe spend that time reconsidering betting against ${((s as any).pcs_firstname || '')} again?"`);
    ...
    // TODO-QSP: dynamic text: "Hi, I'm <<$pcs_firstname>>" You say as introduction and think it might have bee...
    scene.text(`"Hi, I'm ${((s as any).pcs_firstname || '')}" You say as introduction and think it might have been polite to have done so before tearing gaffer tape off her face.`);
    scene.actions([
    ...
    // TODO-QSP: dynamic text: It doesn't take long as the knots are so poorly tied. "Thank you <<$pcs_nickname...
    scene.text(`It doesn't take long as the knots are so poorly tied. "Thank you ${((s as any).pcs_nickname || '')} this slut's name is Jeanine."`);
...
function enterSpank(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[0] ?? 0) === 7) {
    ...
    // TODO-QSP: dynamic text: "<<$pcs_nickname>> Move, move… it's started!" You have no time to realize that y...
    ...
    // TODO-QSP: dynamic text: "Oh sorry! Hi <<$pcs_nickname>>, take a seat. I'm watching a film and its just g...
