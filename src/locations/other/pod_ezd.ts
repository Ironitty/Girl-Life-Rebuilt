...
function enterEtaj_2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).hour ?? 0) >= 0  &&  ((st as any).hour ?? 0) < 5) {
      ...
      if (((st as any).hour ?? 0) >= 5  &&  ((st as any).hour ?? 0) < 10) {
        ...
        if (((st as any).week ?? 0) < 6  &&  ((st as any).hour ?? 0) >= 10  &&  ((st as any).hour ?? 0) < 19) {
          scene.text('You ring the doorbell, but no one answers the door.');
        } else {
          if (((st as any).mishahouse_count ?? 0) === 1  &&  ((st as any).dyadyamishaevent ?? 0) >= 1  &&  ((st as any).week ?? 0) < 6  &&  ((st as any).hour ?? 0) >= 19  &&  ((st as any).hour ?? 0) <= 23  ||  ((st as any).mishahouse_count ?? 0) === 1  &&  ((st as any).dyadyamishaevent ?? 0) >= 1  &&  ((st as any).week ?? 0) >= 6  &&  ((st as any).hour ?? 0) >= 10  &&  ((st as any).hour ?? 0) <= 23) {
            qspCall(st, 'stat', '');
            scene.text('The door opens and Uncle Misha invites you into the apartment.');
            scene.actions([
              { label: 'Enter', handler: (st: GameState) => {
    (st as any).mishahouse_day = ((st as any).daystart ?? 0);
    (st as any).mishahouse_count = ((st as any).mishahouse_count ?? 0) - (1);
    qspGoto(st, 'misha', '1');
  } },
            ]);
          } else {
...
function enterEtaj2Paint(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).paint_blue ?? 0) <= 0) {
    ...
    (st as any).pod_whore_countQW = 0;
    (st as any).minut = ((st as any).minut ?? 0) + 40;
    (st as any).paint_blue = ((st as any).paint_blue ?? 0) - (1);
    qspCall(st, 'stat', '');
    scene.img('images/locations/pavlovsk/resident/apartment/stairs/etaj2.jpg');
...
function enterFloor5Event_1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    qspCall(st, 'willpower', 'pay', 'self');
    qspCall(st, 'stat', '');
    qspGoto(st, 'pod_ezd', 'home_movie1');
  } },
    ]);
  }
  // TODO-QSP: end
...
function enterFloor5Event_2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    qspCall(st, 'willpower', 'pay', 'self');
    qspCall(st, 'stat', '');
    qspGoto(st, 'pod_ezd', 'home_movie2');
  } },
    ]);
  }
  // TODO-QSP: end
...
function enterFloor5Event_3(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    qspCall(st, 'willpower', 'pay', 'self');
    qspCall(st, 'stat', '');
    qspGoto(st, 'pod_ezd', 'home_movie3');
  } },
    ]);
  }
  // TODO-QSP: end
...
function enterFloor5Event_4(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    qspCall(st, 'willpower', 'pay', 'self');
    qspCall(st, 'stat', '');
    qspGoto(st, 'pod_ezd', 'home_movie4');
  } },
    ]);
  }
  // TODO-QSP: end
...
function enterBoysSmoke(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).fame ?? 0)?.['pav_slut'] < 250) {
      ...
      scene.actions([
        { label: 'Leave', handler: (st: GameState) => { qspGoto(st, 'pod_ezd', ((st as any).smoke_loc ?? '')); } },
      ]);
    } else {
    ...
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
          ...
          scene.actions([
            { label: 'Refuse and get away from them', handler: (st: GameState) => {
    qspCall(st, 'willpower', 'pay', 'resist');
    qspGoto(st, 'pod_ezd', ((st as any).smoke_loc ?? ''));
  } },
          ]);
        }
        scene.actions([
...
function enterDanvasSmokeBj(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'hj', (-3), ((st as any).npcID1 ?? 0), 'group');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterSexEv1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).mc_inventory ?? 0)?.['tampons'] === 1) {
...
function enterSexEv1Bj(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'arousal', 'bj', 2, (((s as any).npcID ?? 0)?.[2] ?? 0));
  qspCall(s, 'arousal', 'bj', 1, (((s as any).npcID ?? 0)?.[1] ?? 0));
  qspCall(s, 'cum_call', 'face', (((s as any).boy ?? 0)?.[1] ?? 0), 1, '', '', 20);
...
function enterSexEv2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'bj', (-2), (((st as any).npcID ?? 0)?.[2] ?? 0), 'sub');
    qspCall(st, 'arousal', 'vaginal', 3, (((st as any).npcID ?? 0)?.[1] ?? 0), 'sub');
    qspCall(st, 'gopsex', 'gg_sex_orgasm');
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'hj', 3, (((st as any).npcID ?? 0)?.[2] ?? 0));
    qspCall(st, 'cum_call', 'face', (((st as any).boy ?? 0)?.[1] ?? 0), 1, '', '', 20);
    qspCall(st, 'cum_call', 'mouth', (((st as any).boy ?? 0)?.[2] ?? 0), 1, '', '', 20);
...
function enterSexEv3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).mesec ?? 0) > 0) {
      ...
      if (((st as any).mesec ?? 0) <= 0  &&  ((st as any).pcs_horny ?? 0) >=70) {
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'hj', 2, (((st as any).npcID ?? 0)?.[2] ?? 0));
    qspCall(st, 'cum_call', 'face', (((st as any).boy ?? 0)?.[2] ?? 0), 1);
    (st as any).pcs_hairbsh = 0;
    qspCall(st, 'arousal', 'bj', 2, (((st as any).npcID ?? 0)?.[1] ?? 0));
