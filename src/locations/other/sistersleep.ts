...
function enterSisterSleep_2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"What… What's going on? ${((s as any).pcs_nickname ?? 0)}? What are you doing?" she asks as she sleepily sits up.`);
...
function enterSisterSleep_3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'cuni_give', 5, 'incest', 'lesbian');
    qspCall(s, 'arousal', 'vaginal_finger_give', (-5), 'incest', 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    (s as any).sisterLesb = 7;
    qspCall(s, 'npc_relationship', 'modify', 'A33', 'adore');
    scene.img('images/characters/pavlovsk/resident/anya/sex/les8.jpg');
...
function enterSisterStrapon(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).sisterQW ?? 0)?.['strapon'] === 0) {
      ...
      scene.text(`"Don't be so dramatic, ${((s as any).pcs_nickname ?? 0)}! I would never hurt you. You see guys like this. It's called 'deepthroating.' I'm just helping you train your mouth and throat so you know how to give boys proper blowjobs."`);
