...
function enterShower(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).belgangPay ?? 0) <= 0) {
      ...
      scene.text(`Vadim looks at you and says, "That was a good fuck, ${((st as any).pcs_nickname ?? '')}. Here, this is for you," and with those words he hands you ` + qspFunc(s, 'money', 'string_profit', ((st as any).belsexpayment ?? '')) + '.');
    } else {
