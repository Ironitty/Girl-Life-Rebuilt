...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterFashion(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterBikini(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterLingerie(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterSexy(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
      { label: '"I\'d like to practice glamour shoots"', handler: (st: GameState) => {
    scene.img('images/locations/city/citycenter/photo/fotograph.jpg');
    scene.text('"Alright, we can do that. What would you like to do?"');
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTransparent(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTransLing(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterTopless(st, scene); (st as any).locArgs = __savedLocArgs; }
    { const __savedLocArgs = (st as any).locArgs; (st as any).locArgs = ['', ]; enterNude(st, scene); (st as any).locArgs = __savedLocArgs; }
  } },
    ]);
  } },
  ]);
...
function enterFashion(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    (st as any).modelpayfin = 0;
    qspCall(st, 'exp_gain', 'mdlng', (Math.floor(Math.random() * 3) + 3));
...
function enterBikini(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    scene.img('images/locations/city/citycenter/photo/modelbts2.mp4');
    scene.text('The practice shoot ends and he calls you over to his computer to show you what you need to work on.');
    if (((st as any).pcs_mdlng ?? 0) >= 40  &&  (!((st as any).commercial ?? 0))) {
      scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 120;
    (st as any).modelpayfin = 0;
    qspCall(st, 'exp_gain', 'mdlng', (Math.floor(Math.random() * 5) + 6));
    if (((st as any).pcs_inhib ?? 0) < 50) {
...
function enterLingerie(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    qspCall(st, 'exp_gain', 'mdlng', 1);
    if (((st as any).pcs_inhib ?? 0) < 50) {
      (st as any).inhib_exp = ((st as any).inhib_exp ?? 0) + (0);
    }
    qspCall(st, 'stat', '');
...
function enterSexy(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    (st as any).modelpayfin = 0;
    qspCall(st, 'exp_gain', 'mdlng', 1);
    (st as any).inhib_exp = ((st as any).inhib_exp ?? 0) + ((Math.floor(Math.random() * 4) + 1));
    qspCall(st, 'stat', '');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 120;
    (st as any).modelpayfin = 0;
    qspCall(st, 'exp_gain', 'mdlng', (Math.floor(Math.random() * 4) + 5));
    (st as any).inhib_exp = ((st as any).inhib_exp ?? 0) + (2);
    qspCall(st, 'stat', '');
    scene.img(`images/locations/city/citycenter/photo/sexy/${(Math.floor(Math.random() * 12) + 1)}.jpg`);
...
function enterNude(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'exp_gain', 'mdlng', (Math.floor(Math.random() * 4) + 7));
    (st as any).inhib_exp = ((st as any).inhib_exp ?? 0) + (1);
    qspCall(st, 'stat', '');
    scene.img('images/locations/city/citycenter/photo/nudepractice3.mp4');
    ...
    if (((st as any).pcs_inhib ?? 0) >= 60) {
      ...
      qspCall(st, 'mood', 'raise', 'tiny');
      (st as any).pcs_stam = ((st as any).pcs_stam ?? 0) - (20);
      (st as any).inhib_exp = ((st as any).inhib_exp ?? 0) + (10);
      qspCall(st, 'stat', '');
