...
function enterZoyaMeet(s: GameState, scene: SceneBuilder): void {
  ((s as any).AlbinaQW ?? {})['visiting'] = 1;
  ((s as any).AlbinaQW ?? {})['meet_zoya'] = 1;
  scene.img('images/locations/pavlovsk/resident/albinahome/zoya.jpg');
  ...
  scene.text(`"You must be ${((s as any).pcs_firstname ?? 0)}," she smiles. "Albina has told me quite a bit about you. All good things, I assure you," she adds with a laugh.`);
...
function enterZoyaSunbathingSolo1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).AlbinaQW ?? 0)?.['pool'] === 0) {
    ...
    scene.text(`"Hello ${((s as any).pcs_firstname ?? 0)}," she says, carefully sitting up on her elbows, the floatie wobbling beneath her.`);
    ...
    scene.text(`You blink and realize that Zoya is looking sternly at you, seemingly annoyed. "It's rude to stare at someone's body like that, ${((s as any).pcs_firstname ?? 0)}."`);
    ...
    scene.text(`"Hello again, ${((s as any).pcs_firstname ?? 0)}. Waiting for Albina again?"`);
    ...
    if (((s as any).AlbinaQW ?? 0)?.['check_out_zoya'] > 0) {
      ...
      scene.text(`You blink and realize that Zoya is glaring at you. "I believe we talked about this before, ${((s as any).pcs_firstname ?? 0)}…`);
      ...
      scene.text(`You blink and realize that Zoya is looking sternly at you, seemingly a bit annoyed. "It's rude to stare at someone's body like that, ${((s as any).pcs_firstname ?? 0)}."`);
      scene.text('"Oh, sorry…" you gulp.');
      scene.text('"Perhaps you should wait for Albina inside?"');
...
function enterZoyaSunbathingSolo2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).AlbinaQW ?? 0)?.['pool'] === 0) {
    ...
    scene.text(`"Hello ${((s as any).pcs_firstname ?? 0)}," she smiles, carefully sitting up on her elbows, the floatie wobbling beneath her.`);
    scene.text('"I hope I\'m not intruding?" you reply politely.');
    ...
    scene.text(`"Hello ${((s as any).pcs_firstname ?? 0)}."`);
...
function enterZoyaSunbathingAlbina(s: GameState, scene: SceneBuilder): void {
  ((s as any).AlbinaQW ?? {})['mother_daughter_sunbathe'] = ((s as any).daystart ?? 0);
  (s as any).minut = ((s as any).minut ?? 0) + 2;
  qspCall(s, 'stat', '');
  ...
  scene.text(`"Hello Miss ${((s as any).pcs_firstname ?? 0)}," she smiles. "Ms. Zoya and Miss Albina are out by the pool."`);
  ...
  scene.actions([
    ...
    scene.text(`"Hey ${((s as any).pcs_nickname ?? 0)}," she says. "Did Milena let you in?"`);
    ...
    scene.text(`Albina snorts. "Why not? It's nothing ${((s as any).pcs_nickname ?? 0)} hasn't seen before. We shower together all the time at school. Besides, you're topless too, Mama."`);
...
function enterZoyaYogaAlbina(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Hello Miss ${((s as any).pcs_firstname ?? 0)}," she smiles. "Ms. Zoya and Miss Albina are in the gym."`);
  ...
  scene.text(`"Is that you, ${((s as any).pcs_nickname ?? 0)}?" you hear Albina call out from the room and enter to find her and Zoya doing some yoga.`);
  ...
  scene.text(`"Hello ${((s as any).pcs_firstname ?? 0)}. I'd love to stay and chat, but I need to shower and get some paperwork done for work," Zoya says and excuses herself from the room, leaving you with Albina.`);
...
function enterZoyaWorkoutSolo(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Noticing your reflection in the mirror, she turns her head over her shoulder to smile at you while continuing to squat. "Hello ${((s as any).pcs_firstname ?? 0)}. What can I do for you?"`);
  scene.text('"I hope I\'m not intruding?" you reply politely.');
  scene.text('"No no, not at all. Were you looking for Albina? I think she\'s out right now, but you\'re welcome to make yourself at home until she comes back."');
...
function enterPeep2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if ((!(Math.floor(Math.random() * 3) + 0))) {
      ...
      scene.text(`"${((s as any).pcs_nickname ?? 0)}? What are you doing?" she asks, noticing the flushed appearance on your face.`);
      ...
      scene.text(`"Are you okay, Miss ${((s as any).pcs_firstname ?? 0)}?" she asks, noticing the flushed appearance on your face.`);
