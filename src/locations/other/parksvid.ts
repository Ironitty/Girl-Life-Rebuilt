...
function enter1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`As you walk through the park, you notice a man in a kilt. ${((s as any).telsob ?? '')} is interested in why he is wearing a kilt and what he's up to.`);
  // TODO-QSP: end
  scene.actions([
    ...
    // TODO-QSP: dynamic text: "That''s interesting, but do you think he is wearing anything under his skirt?" ...
    scene.text(`"That's interesting, but do you think he is wearing anything under his skirt?" said ${((st as any).telsob ?? '')}.`);
    scene.text('"How should I know, come on and let\'s ask him."');
    // TODO-QSP: dynamic text: "I''m not going to ask him, you ask him." said <<$telsob>>, making plaintive eye...
    scene.text(`"I'm not going to ask him, you ask him." said ${((st as any).telsob ?? '')}, making plaintive eyes.`);
    scene.actions([
    ...
    scene.text(`${((st as any).telsob ?? '')} and you walk up to the guy, she leans against you and whispers in your ear. "Well ask."`);
    ...
    scene.text(`"Do you want to take a look ${((st as any).pcs_nickname ?? '')}?" asks ${((st as any).telsob ?? '')}`);
    ...
    scene.text(`"Fine, I'll look." Says ${((st as any).telsob ?? '')} defiantly pouting and squatting down.`);
    ...
    if (((st as any).pcs_willpwr ?? 0) < ((st as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Decline and continue your walk', handler: (st: GameState) => {
    ...
    scene.text(`Emboldened ${((st as any).telsob ?? '')} immediately kneels, clasps her lips around his cock and begins to suck his dick. You crouch down behind her and start sliding her panties down, so you can lick and tongue fuck her pussy.`);
...
function enter2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`After you finish your walk, ${((s as any).telsob ?? '')} says she needs to go, she gives you a kiss on the cheek and leaves.`);
...
function enter3(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).telsob ?? 0) === 'Alla'  ||  ((s as any).telsob ?? 0) === 'Masha'  ||  ((s as any).telsob ?? 0) === 'Kate') {
    ...
    scene.text(`After you finish your walk, ${((s as any).telsob ?? '')} looks at you and waits for your next move.`);
    if (((s as any).home ?? 0)?.['town'] === 'pavlovsk') {
      scene.actions([
        { label: 'Kiss her and say goodbye', goto: ['pav_park', 'start'] },
      ]);
  ...
  scene.actions([
    ...
    if (((st as any).telsob ?? 0) === 'Alla') {
      ...
      if (((st as any).alla ?? 0) >= 20) {
        (st as any).parksvidrand = (Math.floor(Math.random() * 6) + 0);
        if (((st as any).parksvidrand ?? 0) > 0  &&  ((st as any).parksvidrand ?? 0) < 3) {
          qspGoto(st, 'parksvid', '1');
        } else {
          qspGoto(st, 'parksvid', '3');
        }
      } else {
      ...
      if (((st as any).telsob ?? 0) === 'Masha') {
        ...
        if (((st as any).masha ?? 0) >= 20) {
          (st as any).parksvidrand = (Math.floor(Math.random() * 11) + 0);
          if (((st as any).parksvidrand ?? 0) > 0  &&  ((st as any).parksvidrand ?? 0) < 3) {
            qspGoto(st, 'parksvid', '1');
          } else {
            qspGoto(st, 'parksvid', '3');
        ...
        if (((st as any).telsob ?? 0) === 'Kate') {
          ...
          if (((st as any).telsob ?? 0) === ((st as any).npcdesc ?? 0)) {
            ...
            if (((st as any).npc_rel ?? 0)?.[String((st as any).npcID ?? 0)] < 50) {
              qspGoto(st, 'parksvid', '3');
            } else {
              (st as any).parksvidrand = (Math.floor(Math.random() * 11) + 0);
              if (((st as any).parksvidrand ?? 0) <= 1) {
                qspGoto(st, 'parksvid', '1');
              } else {
                if (((st as any).parksvidrand ?? 0) <= 3) {
                  qspGoto(st, 'parksvid', '2');
                } else {
                  qspGoto(st, 'parksvid', '3');
                }
              }
