...
function enter2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).temp_rand ?? 0) <= 15  &&  ((s as any).alko ?? 0) < 6) {
    scene.text(`The guys suggest: "Hey ${((s as any).pcs_nickname ?? '')}, show us your tits!" (<a href="#" onclick="window.__gameStore.getState().doGoto(/u0027zsoft_gopskverGorSlut/u0027, /u0027siski/u0027); return false;">Show them.</a>)`);
  }
  ...
  scene.actions([
    ...
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).poza_text = 'You get into a comfortable pose, that also shows off your body to the boys.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).poza_text = 'You strike a ' + ((st as any).text_sveta_obol ?? 0) + ' pose for the guys.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).poza_text = 'You get into a comfortable pose. Without realizing it, you\'re also giving the guys a nice show.';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).poza_text = 'You shift your weight and get into a different pose while you talk with the boys. Without meaning to, you end up looking quite ' + ((st as any).text_sveta_obol ?? 0) + '.';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).poza_text = 'The boys have been so nice to you, you want to do something back for them and show off your body. When they look at you, you give them a cute wink.';
    }
    if (((st as any).temp_rand ?? 0) === 5) {
      (st as any).poza_text = 'You calmly put down your empty bottle and strike a sexy pose for the boys, hoping they\'ll give you another.';
    }
    (st as any).temp_rand = (Math.floor(Math.random() * 8) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).poza_gop_text3 = 'successfully';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).poza_gop_text3 = 'sexually';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).poza_gop_text3 = 'for fucking';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).poza_gop_text3 = 'excitingly';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).poza_gop_text3 = 'vulgarly';
    }
    if (((st as any).temp_rand ?? 0) === 5) {
      (st as any).poza_gop_text3 = 'handsomely';
    }
    if (((st as any).temp_rand ?? 0) === 6) {
      (st as any).poza_gop_text3 = 'good';
    }
    if (((st as any).temp_rand ?? 0) === 7) {
      (st as any).poza_gop_text3 = 'perfectly';
    }
    (st as any).temp_rand = (Math.floor(Math.random() * 6) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).poza_gop_text2 = 'Looks like our ' + ((st as any).shultextrand2 ?? 0) + ' is in the mood!';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).poza_gop_text2 = 'You know, ' + ((st as any).shultextrand2 ?? 0) + '… if you keep doing that, you\'re getting us all worked up! What are you going to do about it?';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).poza_gop_text2 = 'Looks like our ' + ((st as any).shultextrand2 ?? 0) + ' is in the mood for a fucking, boys!';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).poza_gop_text2 = 'You\'re suck a slut, ' + ((st as any).pcs_nickname ?? 0) + '… you want us to fuck you, don\'t you? Don\'t worry, we\'ll take good care of you.';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).poza_gop_text2 = 'Looks like her cunt is ready, guys!';
    }
    if (((st as any).temp_rand ?? 0) === 5) {
      (st as any).poza_gop_text2 = 'Haha, that\'s a nice look for you slut! Just stay in that position for a while…';
    }
    (st as any).temp_rand = (Math.floor(Math.random() * 5) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).poza_gop_text = 'Well, guys… who\'s going to fuck her first?';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).poza_gop_text = 'Alright, who gets to go first this time?';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).poza_gop_text = 'You were first last time, I think it\'s my turn to use this slut first!';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).poza_gop_text = 'Dibs on her mouth… I bet she gives a great blowjob!';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
    ...
    scene.text(`${((st as any).shultextrand3 ?? '')} ${((st as any).shultextrand6 ?? '')} says "${((st as any).poza_gop_text2 ?? '')}"`);
    scene.text(`${((st as any).shultextrand4 ?? '')}.`);
    scene.text(`${((st as any).shultextrand33 ?? '')} speaks "${((st as any).poza_gop_text ?? '')}"`);
    (st as any).temp_rand = (Math.floor(Math.random() * 5) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).horny_orgazm_text2 = 'You\'re enjoying their treatment more than you probably should. You frantically rub your clit, trying to get off while you please the guys.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).horny_orgazm_text2 = 'You groan loudly while you get fucked, and beg the guy to fuck you harder.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).horny_orgazm_text2 = 'It feels great, and you beg your partner to fuck you faster.';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).horny_orgazm_text2 = 'You feel close to your orgasm and groan at the guy: "I\'m not made of sugar! Stop treating me like I\'m fragile and fuck me like a man!"';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).horny_orgazm_text2 = 'You moan at your partner: "I\'m a whore… I\'m your slut, I\'ll do whatever you want… just keep fucking me!"';
    }
    (st as any).temp_rand = (Math.floor(Math.random() * 5) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).horny_orgazm_text3 = 'You were not satisfied.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).horny_orgazm_text3 = 'Unfortunately, you did not reach your orgasm on time.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).horny_orgazm_text3 = 'You couldn\'t have an orgasm.';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).horny_orgazm_text3 = 'Your partner finishes before you, leaving you unsatisfied.';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).horny_orgazm_text3 = 'You were close to your orgasm, but didn\'t quite get there.';
    }
    (st as any).temp_rand = (Math.floor(Math.random() * 5) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).horny_orgazm_text4 = 'You moan loudly, and cum all over his cock.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).horny_orgazm_text4 = 'Your body begins to shake violently and you shamelessly orgasm.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).horny_orgazm_text4 = 'A loud groan escapes your lips, and your cunt spasms around his cock when you cum violently.';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).horny_orgazm_text4 = 'You reach your orgasm loudly, and cover up your mouth afterwards in embarrassment. Oh my…';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).horny_orgazm_text4 = 'Your breathing grows more rapid, and you experience an explosive orgasm while pleasing him.';
    }
    (st as any).temp_rand = (Math.floor(Math.random() * 7) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).oral_den_text = 'His cock is not fully hard yet, and he smiles at you: "Go on slut, get it hard for me." You nod and obediently close your lips around it, giving him the best blowjob you can.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
    ...
    if ((Math.floor(Math.random() * 100) + 1) <= 15) {
      (st as any).temp_randB = (Math.floor(Math.random() * 3) + 0);
      if ((!((st as any).temp_randB ?? 0))) {
        scene.img(`images/locations/pavlovsk/park/sex/gop/bjbeer${((st as any).DoubleRand ?? '')}.jpg`);
        scene.text(`By now you know what they want, and humbly open your mouth as wide as possible. Vitek immediately guides his cock towards your mouth, and impatiently pushes it down your ${((st as any).throattipe ?? '')} throat.`);
        ...
        if (((st as any).temp_randB ?? 0) === 1) {
          ...
          scene.text(`Vasily continues: "${((st as any).pcs_nickname ?? '')}, I've been told that you're doing this, for a beer! Do you really like beer, or do you just like sucking cocks so much?"`);
          scene.text(`Dan grabs your ${((st as any).pcs_haircolor ?? '')} ${((st as any).curly_txt ?? '')} hair in his fist, and moves your head so it looks like you're nodding yes.`);
          scene.text(`Vasily piles it on some more: "There you have it, dear viewers! ${((st as any).pcs_nickname ?? '')} ${((st as any).pcs_lastname ?? '')} is happily sucking three cocks, for the low low price of one beer! Where else can you find such a cheap slut?"`);
          scene.text('You try to protest and turn away from the camera, but Dan\'s firm grip on your hair makes it so you cannot move.');
          scene.text(`You can only imagine how incredibly foolish and humiliating you must look in the video: on your knees, with two thick cocks in your cute little mouth wedged in so tightly that they make your cheeks bulge. "Open wider, ${((st as any).shultextrand2 ?? '')}" Dan tells you once more before unceremoniously cramming himself further down your throat, stretching your mouth to the limit.`);
          ...
          scene.text(`Luckily the guys don't last long, and finish fairly quickly. They both pull out and drop their loads on your face, and wipe their dicks on your ${((st as any).pcs_haircolor ?? '')} ${((st as any).curly_txt ?? '')} hair. They make you lick their cocks clean, but don't let go of your hair just yet.`);
          scene.text(`Vasily grins as he tells the camera: "And that was the premiere presentation of ${((st as any).pcs_nickname ?? '')} ${((st as any).pcs_lastname ?? '')} in her very first production called "Gagging Whore"! You delivered an excellent performance ${((st as any).pcs_nickname ?? '')}, say hi to your mother!" He zooms in on your face, and continues to film.`);
          if ((!(Math.floor(Math.random() * 2) + 0))) {
      ...
      (st as any).temp_randC = undefined;
...
function enterPiss(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'arousal', 'vaginal', 3, 'rough', 'sub', 'gangbang', 'humiliation');
    qspCall(st, 'arousal', 'anal', 2, 'rough', 'sub', 'gangbang', 'humiliation');
    qspCall(st, 'arousal', 'bj', 2, 'rough', 'sub', 'gangbang', 'humiliation');
    qspCall(st, 'stat', '');
    ...
    qspCall(st, 'arousal', 'bj', 1, 'rough', 'sub', 'gangbang', 'humiliation');
    qspCall(st, 'arousal', 'anal', 1, 'rough', 'sub', 'gangbang', 'humiliation');
    qspCall(st, 'stat', '');
    ...
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    qspCall(st, 'arousal', 'bj', 2, 'rough', 'sub', 'gangbang', 'humiliation', 'deepthroat');
    ...
    qspCall(st, 'arousal', 'vaginal_dildo', 4, 'masturbate', 'exhibitionism', 'humiliation');
    qspCall(st, 'arousal', 'end');
    scene.actions([
      { label: 'Leave', goto: ['pav_park', 'start'] },
...
function enterSiski(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_horny ?? 0) >= 50) {
    ...
    if (((st as any).temp_randB ?? 0) === 1) {
      (st as any).rnd_droch_text = 'You gently rub your fingers over your incredibly wet slit, and nearly orgasm at the first touch. You carefully touch yourself for a few minutes, making sure you don\'t orgasm in front of the guys.';
    }
    if (((st as any).temp_randB ?? 0) === 2) {
      (st as any).rnd_droch_text = 'You spread your legs and expose your bare pussy to the guys, before you slowly begin to rub your clitoris. The guys watch you intently, not wanting to miss a single moment of your show.';
    }
    if (((st as any).temp_randB ?? 0) === 3) {
      (st as any).rnd_droch_text = 'You bite your lip as you gently massage your clitoris, fondling your tits with your available hand. The guys are looking at your show with wide eyes, unsure what they did to convince you to actually do this for them.';
    }
    (st as any).temp_randB = undefined;
    scene.text(`${((st as any).rnd_droch_text ?? '')}`);
    qspCall(st, 'arousal', 'clit_finger', 4, 'masturbate', 'exhibitionism');
    qspCall(st, 'arousal', 'end');
    qspCall(st, 'stat', '');
    scene.actions([
...
function enterCumface(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).cum_face_text = 'He cums all over your face, shooting his hot jets of jizz all over you. You lick your lips obediently, and wink at him lewdly once he\'s finished.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).cum_face_text = 'His warm and slightly sweet jets of cum hit your face. After he\'s mostly finished, you massage his cock for a little while to squeeze out the last drops, and you lick his shrinking cock clean obediently.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).cum_face_text = 'When he\'s about to orgasm, he pulls his cock away from you and begins to jerk off. His sperm flies quite far, splattering against your face with quite a bit of force. The second and third blast hit your cheeks, while the fourth and fifth end up in your hair. Eventually he\'s done, and groans loudly when you lick him clean.';
    }
    (st as any).temp_rand = undefined;
    scene.text(`${((st as any).cum_face_text ?? '')}`);
    scene.actions([
      { label: 'Leave', goto: ['pav_park', 'start'] },
...
function enterCumfaceGroup(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    (st as any).temp_rand = (Math.floor(Math.random() * 5) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).cum_face_textgr = 'You fall back, feeling exhausted. Your face is covered in a mixture of the guys\' cum and your tears and saliva.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).cum_face_textgr = 'You fall back, feeling exhausted. Your face is covered in a mixture of the guys\' cum and your tears and saliva. ' + ((st as any).shultextrand3 ?? 0) + ' grins and says: "She\'s got a great mouth on her… it\'s like she was made for sucking cocks!" You blush and look away with your head held down, trying to ignore their degrading comments while you clean yourself up a little.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).cum_face_textgr = 'When the guys approach their orgasms, they aim their cocks at your face and begin to masturbate furiously. You immediately move up to kiss and lick their dicks, not wanting to have anyone feel like they were left out while they cum all over your face.\n' + ((st as any).shultextrand33 ?? 0) + ' spits on your face and gives you a few slaps, right before he cums on your cheeks. The other guys laugh and shoot their loads onto you as well.';
    }
    // TODO-QSP: <<$shultextrand33>> spits on your face and gives you a few slaps, right before he cums on your cheek...
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).cum_face_textgr = 'The guys all cum at roughly the same time, shooting their loads all over your face. When they\'re done, you look like a glazed donut.';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).cum_face_textgr = 'Luckily, you had the guys all pent up and they finish fairly quickly. After they shoot their hot loads of cum all over your face, they carefully wipe their dirty cocks off on your ' + ((st as any).pcs_haircolor ?? 0) + ' ' + ((st as any).curly_txt ?? 0) + ' before letting go of your head.';
    }
    (st as any).temp_rand = undefined;
    scene.text(`${((st as any).cum_face_textgr ?? '')}`);
    scene.actions([
      { label: 'Leave', goto: ['pav_park', 'start'] },
      { label: 'Return to the guys', goto: ['zsoft_gopskverGorSlut', '2'] },
    ]);
...
function enterCummouth(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    (st as any).temp_rand = (Math.floor(Math.random() * 6) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).cum_mouth_text = 'You feel his hot jets of cum shooting down your throat, and choke in surprise. You manage to recover though, and obediently swallow every drop of his load.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).cum_mouth_text = 'He grins: "Bon appetit, sperm slut" and cums down your throat. Then he wipes his limp cock against your cheeks and walks away, not paying you any more attention.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).cum_mouth_text = 'After a few quick hand movements, he pulls your face towards his cock and shoots his cum all over your tongue. Then he puts his hand on your jaw and forces it shut, making you swallow his load.';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).cum_mouth_text = 'You can\'t do much besides swallow his load, and involuntarily wince at the horrible taste of it. He doesn\'t care, and tells you to lick him clean before he pulls his cock from your mouth.';
    }
    if (((st as any).temp_rand ?? 0) === 4) {
      (st as any).cum_mouth_text = 'Vasily grins at you and says: "Get used to it, whore. This is the way we\'ll be feeding you from now on" while you carefully lick the remnants of sperm from the shrinking cock. You don\'t feel bold enough to refute his statement right now, and just let it slide.';
    }
    if (((st as any).temp_rand ?? 0) === 5) {
      (st as any).cum_mouth_text = 'He forces his cock down your throat, and he cums while his balls are touching your lips. You nearly suffocate, but he grabs hold of your hair and keeps forcing you down against his groin until he\'s done. When he finally shot all his cum down your throat he lets you go, and you gasp for air when you can finally breathe again.';
    }
    (st as any).temp_rand = undefined;
    scene.text(`${((st as any).cum_mouth_text ?? '')}`);
    scene.actions([
      { label: 'Leave', goto: ['pav_park', 'start'] },
      { label: 'Return to the guys', goto: ['zsoft_gopskverGorSlut', '2'] },
    ]);
...
function enterCummouthGroup(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).cum_mouth_textgr = 'The guys line up, and the cock you are currently sucking on begins to spasm in your mouth, soon filling it with jets of slightly bitter cum. You choke in surprise and do your best to hide your disgust while you swallow every drop, while the second guy approaches you. Two more loads later, you sit back in the grass and idly lick your lips while the guys grin happily and pull up their pants.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).cum_mouth_textgr = 'He grins: "Bon appetit, sperm slut" and cums down your throat. Then he wipes his limp cock against your cheeks and walks away, not paying you any more attention. You\'re not given any time to respond, since a second guy immediately moves in to take his place. You dutifully swallow the loads of all three guys, and sit back in the grass licking your lips while the guys grin and pull up their pants again.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).cum_mouth_textgr = 'You can\'t do much besides swallow his load, and involuntarily wince at the horrible taste of it. He doesn\'t care, and tells you to lick him clean before he pulls his cock from your mouth.';
    }
    if (((st as any).temp_rand ?? 0) === 3) {
      (st as any).cum_mouth_textgr = 'Dan forces his cock down your throat, and he cums while his balls are touching your lips. You nearly suffocate, but he grabs hold of your hair and keeps forcing you down against his groin until he\'s done. When he finally shot all his cum down your throat he lets you go, and you gasp for air when you can finally breathe again. Vitek immediately takes his place though, and after a few quick gasps of breath your throat is filled again. Vitek and then Vasily fuck your throat for several more minutes, and eventually both cum in your mouth as well.';
    }
    (st as any).temp_rand = undefined;
    scene.text(`${((st as any).cum_mouth_textgr ?? '')}`);
    scene.actions([
      { label: 'Leave', goto: ['pav_park', 'start'] },
      { label: 'Return to the guys', goto: ['zsoft_gopskverGorSlut', '2'] },
    ]);
...
function enterCumass(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    (st as any).temp_rand = (Math.floor(Math.random() * 3) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).cumass_text = 'You feel his body grow tense against yours, and soon after you feel a hot liquid spreading inside you. When he\'s nearly finished, he pulls out his cock and squeezes the last drops out over your ass, before rubbing it against your pussy, rubbing the cum that is slowly running out of your sphincter over your snatch too.';
    }
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).cumass_text = 'He shoots his load inside your ass, groaning loudly. Then he pulls out his cock and pushes your ass cheeks aside, slowly watching his cum run out of your stretched hole.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
...
function enterCumpussy(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('iif(func(\'pregriskeval\', eff_contra) < 2 and succubusflag ! 1, \'You squat down and try to push his sperm out of you, while\', \'Done with you for now,\') + \' he pulls his pants back up and returns to drinking beer with his buddies, not giving you another thought.\'');
...
function enterVaginalsex(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    (st as any).temp_rand = (Math.floor(Math.random() * 3) + 0);
    if ((!((st as any).temp_rand ?? 0))) {
      (st as any).vag_gop_text = 'He turns to you and grins: "On your hands and knees, ' + ((st as any).shultextrand2 ?? 0) + '." You do as he says and assume the position, your eyes fixated on his ' + ((st as any).dick ?? 0) + 'cm cock. He walks over to you and rests it on your ass cheeks, grinning at the greedy look in your eyes.\nHe puts one hand on your throat and pulls your body up, and whispers in your ear: "Don\'t worry, slut… it\'s all yours."\nThen you feel his cock rubbing against your pussy lips, and them slowly parting to let him in. You\'re quite wet, and his cock slides in easily.\nAt first his thrusts are slow and careful, but he soon speeds up and rams his cock in and out of you furiously, the sound of your buttocks slapping against his hips quite audible. Hopefully no one walks by…\nYou suddenly feel the gaze of the other guys upon you, and you blush while you look in their direction. They\'re still sitting back drinking beer, watching you get fucked.\nSoon enough, the fucking makes you feel so good that you forget all about them, and just close your eyes and give in to the sensations.';
    }
    // TODO-QSP: Soon enough, the fucking makes you feel so good that you forget all about them, and just close your ...
    if (((st as any).temp_rand ?? 0) === 1) {
      (st as any).vag_gop_text = 'Your cunt is quite wet by now, and when he moves his ' + ((st as any).dick ?? 0) + 'cm cock against your pussy it slides in easily. Realizing he doesn\'t have to take it slow with you, he just begins to ram his cock inside you, and you reach down to massage your clit while you get fucked like a whore in the park.';
    }
    if (((st as any).temp_rand ?? 0) === 2) {
      (st as any).vag_gop_text = 'In an instant he is upon you, and roughly shoves you against the ground. He rapidly pulls your clothes aside and slams his cock inside you, bucking his hips against yours while he groans like an animal.\nIt feels great and you moan silently, begging for him to not stop fucking you. Then you hear a hoarse voice behind you: "Stop begging, slut… it makes you look desperate."\nOh crap, did you just say that out loud!? He redoubles his efforts and fucks you even harder, while you can do little more than relax your body and let him use you. Not that you\'d have it any other way… you just hope your loud moans don\'t attract any attention.';
    }
...
function enterAnalsex(s: GameState, scene: SceneBuilder): void {
  scene.actions([
    ...
    if ((!((st as any).temp_rand ?? 0))) {
      ...
      scene.text(`He slaps your ass cheeks playfully and reminds you: "You should really relax, ${((st as any).pcs_nickname ?? '')}. This will hurt a lot more if you resist."`);
      ...
      if (((st as any).temp_rand ?? 0) === 1) {
        ...
        if (((st as any).temp_rand ?? 0) === 2) {
          ...
          scene.text(`He silently slides his cock inside your ass, and asks with a lewd grin on his face: "You want this… don't you, ${((st as any).pcs_nickname ?? '')}?"`);
