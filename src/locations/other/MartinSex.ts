...
function enterDefault(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).MartinSexTalk ?? 0) === 0  &&  (!((s as any).MartinSex ?? 0))) {
    ...
    qspCall(st, 'mood', 'lower', 'tiny');
    scene.text('$OpenInnerThought + \'Too young? Really?\' + $CloseInnerThought');
    ...
    scene.text('$OpenInnerThought + \'I want to talk about this! I really, really do!\' + $CloseInnerThought + \' You tell yourself.\'');
...
function enterMartinTease1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    (st as any).MartinStripVar = undefined;
    scene.actions([
      { label: 'Let Martin fuck you', handler: (st: GameState) => {
    (st as any).martinpos = 'missionary';
...
function enterMartinTease2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.actions([
      { label: 'Let Martin fuck you', handler: (st: GameState) => {
    (st as any).martinpos = 'doggystyle';
  }, goto: ['MartinSex2', 'MartinSexDyn'] },
    ]);
  } },
    ]);
