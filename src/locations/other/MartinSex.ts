...
function enterDefault(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).MartinSexTalk ?? 0) === 0  &&  (!((s as any).MartinSex ?? 0))) {
    ...
    scene.text(`"Good," Martin gives a sigh of relief. You could tell that he was holding himself back, but the sigh gives you an impression of how much of an effort that really was for him. "I don't want this to stand between us, ${((s as any).pcs_firstname || '')}, I just… Uh…" He trails off, unsure what to say.`);
    ...
    scene.text(`"${((s as any).pcs_firstname || '')}," Martin begins, his voice taking on his most calming tone, but instead of calming you, it is the pinprick needed to set you off.`);
    // TODO-QSP: dynamic text: In the blink of an eye, you are face to face with him again, yelling at him unre...
    scene.text(`In the blink of an eye, you are face to face with him again, yelling at him unrestrainedly. "No! Apparently, you think I'm too young and too dumb to know what I want, so you don't get to '${((s as any).pcs_firstname || '')}' me! I don't need another stepdad who knows what's best for me!"`);
    ...
    scene.text(`"Hello ${((s as any).pcs_firstname || '')}. Still here?"`);
    ...
    scene.text(`When you break the kiss, Martin looks at in silence for a moment. "${((s as any).pcs_firstname || '')}, we talked about this. It-"`);
...
function enterMartinTease1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"${((s as any).pcs_firstname || '')}, I'm trying to be <b>sensible</b> here. This is not a good idea. You should cover yourself and go."`);
    ...
    scene.text(`His hands are roaming your body, pawing at your breasts, brushing against your ass and pussy. Then, all of a sudden, Martin pulls you closer to the edge, his pants miraculously already undone, his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} dick revealed in all its glory.`);
...
function enterMartinTease2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"${((s as any).pcs_firstname || '')}, stop." Martin has found his voice again, it seems - not that he's using it to say something helpful. "This is wrong."`);
    ...
    scene.text(`"${((s as any).pcs_firstname || '')}-" But you cut him off.`);
    ...
    scene.text(`You don't know if his zipper exploded at your last words or if he was just that damn good with his hands that you didn't even notice when he opened it to reveal his ${((s as any).dick || '')}cm ${((s as any).dick_girth || '')} dick, but you cannot wait to feel him inside you and, judging by how he's now taking position behind you, neither can he.`);
    qspCall(s, 'arousal', 'foreplay', 1, 'exhibitionism');
