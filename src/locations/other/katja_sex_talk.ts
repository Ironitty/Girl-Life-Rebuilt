...
function enterSextalk(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).kattalkrand ?? 0))) {
    ...
    if (((st as any).loc ?? 0) === 'katja_dorm') {
      ...
      if (((st as any).loc ?? 0) === 'mey_home'  &&  ((st as any).loc_arg ?? 0) === 'garden') {
        qspGoto(st, 'katja_pool', 'sunbating_chat_finish');
      } else {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 3;
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['horny'] = ((st as any).katjaQW['horny'] ?? 0) + (5);
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['knows_cuni'] = 1;
    qspCall(st, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    ...
    if (((st as any).katjaQW ?? 0)?.['horny'] > 50  &&  ((st as any).katjaQW ?? 0)?.['slut'] >= 40  &&  (Math.floor(Math.random() * (30 - -30 + 1)) + (-30)) < ((st as any).katjaQW ?? 0)?.['dom']  &&  ((st as any).npc_had_sex ?? 0)?.['A14']) {
    ...
    scene.text(`You're a little confused at first before you notice her pulling her panties off. Once she has them off, she opens her legs wide. "Go on, ${((st as any).pcs_nickname ?? '')}. Show me how you do it, please."`);
    ...
    scene.text(`"Sorry ${((st as any).pcs_nickname ?? '')}, maybe next time. We need to finish our homework," she says sweetly. You pause before nodding in understanding. You watch the new confident look intensify on her face as she smiles and the two of you finish your homework.`);
    ...
    qspCall(st, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    if (((st as any).katjaQW ?? 0)?.['simultanous_girls'] > 0) {
    ...
    if (((st as any).katjaQW ?? 0)?.['horny'] >= 50  &&  ((st as any).katjaQW ?? 0)?.['QWstage'] > 2  &&  ((st as any).katjaQW ?? 0)?.['slut'] >= 20) {
    ...
    scene.actions([
      { label: 'Continue talking', handler: (st: GameState) => {
    if (((st as any).katjaQW ?? 0)?.['fisted_vag'] === 0  &&  ((st as any).npc_vag ?? 0)?.['A14'] > 3  &&  ((st as any).katjaQW ?? 0)?.['slut'] >= 70  &&  ((st as any).katjaQW ?? 0)?.['horny'] >= 50  &&  ((st as any).npc_had_sex ?? 0)?.['A14']) {
      qspGoto(st, 'katjaHomeTalk', 'fisting_ask');
    } else {
      (st as any).minut = ((st as any).minut ?? 0) + 5;
      if (((st as any).loc ?? 0) === 'katja_dorm') {
        qspGoto(st, 'katja_dorm', 'talk');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 3;
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['horny'] = ((st as any).katjaQW['horny'] ?? 0) + (10);
    if (((st as any).katjaQW ?? 0)?.['knows_self_fisting_anal'] === 0) {
      ((st as any).katjaQW = (st as any).katjaQW ?? {})['knows_self_fisting_anal'] = 1;
    }
    qspCall(st, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 3;
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['horny'] = ((st as any).katjaQW['horny'] ?? 0) + (5);
    ((st as any).katjaQW = (st as any).katjaQW ?? {})['knows_masturbation'] = 1;
    qspCall(st, 'stat', '');
    scene.img('images/characters/shared/headshots_main/big14.jpg');
  ...
  scene.actions([
    ...
    scene.text(`You refuse to talk about such topics with her and she looks down in confusion and mumbles. "Sorry, ${((st as any).pcs_nickname ?? '')}. I didn't mean to pry."`);
    scene.actions([
      { label: 'Continue talking', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    if (((st as any).loc ?? 0) === 'katja_dorm') {
