...
function enterFirstOrgyObserve1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You remove your hand from ${(((st as any).katja_first_orgy_temp ?? 0)?.['first_orgy_name_tatoo_guy'] ?? '')}'s cock and move so that his hand slips out of your ` + ((((st as any).PCloPants ?? 0) > 0) ? ('pants.') : ('skirt.')));
...
function enterFirstOrgyObserve2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('Seeing Katja ' + ((((st as any).katjaQW ?? 0)?.['horny'] === 0) ? ('getting off') : ('enjoying herself')) + ' makes you wonder how the shy girl ' + ((((st as any).katjaQW ?? 0)?.['first_time_in_nightclub'] === 0) ? ('you taught the ropes to is going farther than you are.') : ('from school turned out to be such a slut.')));
    scene.actions([
...
function enterFirstOrgyObserve5(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text('So she can have fun without a dick in her pussy... You were beginning to wonder just what kind of slut ' + ((((st as any).katjaQW ?? 0)?.['first_time_in_nightclub'] === 0) ? ('you have created...') : ('your friend is...')));
    scene.actions([
