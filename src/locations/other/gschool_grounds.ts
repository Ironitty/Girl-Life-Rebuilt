...
function enterMain(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) <= 8  &&  ((s as any).kanikuli ?? 0) === 6) {
    ...
    if (((s as any).class ?? 0)?.['school_grade_average'] >= 90) {
      ...
      scene.text(`"I'd like to honor our best student of the year," she announces. "Miss ${((s as any).pcs_lastname || '')} here performed exceptionally well in her studies and I think that deserves some recognition."`);
      ...
      if (((s as any).class ?? 0)?.['school_grade_average'] >= 70) {
        ...
        scene.text(`"Well done, Miss ${((s as any).pcs_lastname || '')}!" she says before you return to your place.`);
      } else {
        scene.text('You walk up to the podium and the principal shakes your hand before giving you your diploma.');
  ...
  if (((s as any).gschoolVars ?? 0)?.['school_diploma'] === -1  &&  ((s as any).week ?? 0) < 6  &&  ((s as any).hour ?? 0) >= 6  &&  ((s as any).hour ?? 0) < 14) {
    ...
    if (((s as any).class ?? 0)?.['school_grade_average'] >= 70) {
      ...
      scene.text(`"Well done, Miss ${((s as any).pcs_lastname || '')}! Good luck in your future!" she says as she shakes your hand.`);
    } else {
      scene.text('The principal hands you your diploma.');
  ...
  if (((s as any).home ?? 0)?.['current'] === 'parents_home') {
    ...
    if (((s as any).npc_rel ?? 0)?.['A29'] > 50) {
      ...
      if (((s as any).npc_rel ?? 0)?.['A33'] > 50) {
        ...
        scene.text(`You see Anya coming over behind Kolka. "Well done, ${((s as any).pcs_nickname || '')}. Why didn't you say something? That way we'd get to know each other better?"`);
      ...
      scene.text(`Your stepfather now joins the conversation. "Come on ${((s as any).pcs_nickname || '')}, get showered and changed and we'll meet you outside. We're all going out for dinner to celebrate."`);
    ...
    (st as any).pavhome_enter = 1;
      ...
      scene.text(`"${((s as any).pcs_nickname || '')}, you won! Well done!" your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} shouts as she approaches.`);
    ...
    scene.text(`"Of course I'd come, ${((s as any).pcs_nickname || '')}! I just want the best for you and I'm proud of you. Come on, let's go out for a bite to eat. I think we need to have a chat."`);
    ...
    if (((s as any).npc_QW ?? 0)?.['A29'] > 0) {
      ...
      scene.text(`Your mother gets up and comes over to hug you. "Thanks ${((s as any).pcs_nickname || '')}, that's all I can ask. I'll try to be more supportive too. Just please talk to me if you're worried about anything."`);
    ...
    scene.text(`Rejoining ${((s as any).npcdesc || '')}, you both have a passionate embrace as most folks have gone.`);
    // TODO-QSP: dynamic text: <<$npcdesc>> then says "Well done again <<$pcs_nickname>>, but I have something ...
    scene.text(`${((s as any).npcdesc || '')} then says "Well done again ${((s as any).pcs_nickname || '')}, but I have something I have to do that won't wait. See you soon."`);
    (s as any).minut = ((s as any).minut ?? 0) + 10;
    qspCall(s, 'stat', '');
    scene.actions([
      { label: 'Go home', handler: (st: GameState) => {
