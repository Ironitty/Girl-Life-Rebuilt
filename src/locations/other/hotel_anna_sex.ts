...
function enterLesSex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"You are embarrassing me in front of my friend and that is not right. You deserve a small punishment, don't you agree ${((s as any).pcs_nickname || '')}?"`);
    ...
    scene.text(`The woman immediately strips and starts to put on the outfit which is nothing more than a corset, gloves boots. She can't fasten the corset up on her own but struggles trying to, Anna looks to you and smiles, ${((s as any).pcs_nickname || '')} will help you with your corset seeing as you are too usless to do it yourself."`);
    ...
    scene.text(`"This is old school punishment ${((s as any).pcs_nickname || '')}. I start easy and then a bit hard until soon I will be slapping her very hard. These kinds of slut need to be disciplined or they will become disobedient." Anna says this while slapping the women's butt noticeable harder each stroke while the woman counts each strike.`);
    qspCall(s, 'arousal', 'voyeur', 5, 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.text(`"This is just like the punishments you might get at school ${((s as any).pcs_nickname || '')}. Except I start easy and then a bit hard until soon I will be hitting her very hard. These kinds of slut need to be disciplined or they will become disobedient." Anna says this while caning the women's butt noticeable harder each stroke while the woman counts each strike.`);
    qspCall(s, 'arousal', 'voyeur', 5, 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
      { label: '…', handler: (st: GameState) => {
...
function enterSlaveM(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Anna stretches her leg towards the man, and puts her heel on his crotch. "This is worm, worm She is ${((s as any).pcs_nickname || '')}, a new Mistress". She smiles at you.`);
    ...
    scene.text(`"Exactly ${((s as any).pcs_nickname || '')}. She smiles.`);
...
function enterSlaveM2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Very good ${((s as any).pcs_nickname || '')}, now the fleshier bits near his butt and middle of his back either side of his spine are less sensitive."`);
...
function enterSlaveM3(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Bend Worm" The man bends forwards legs slightly apart. "OK ${((s as any).pcs_nickname || '')} grab his balls in your hand."`);
    ...
    scene.text(`"Mistress ${((s as any).pcs_nickname || '')} owns them as much as I do. Do you understand Worm?" The man nods obediently.`);
...
function enterSlaveF(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Anna lies next to you nearest the tied up woman, looking at er she says: "Nice butt, almost a shame to whip it. Let's admire it for a minute while we relax and have a smoke, what do you think ${((s as any).pcs_nickname || '')}?"`);
...
function enterSlaveF1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"${((s as any).pcs_nickname || '')} why don't you sit on that chair?" She suggests as she removes the outer mask revealing a second mask that covers all but the woman's mouth and eyes.`);
    ...
    scene.text(`"Very good ${((s as any).pcs_nickname || '')}, that's part of it. I'm also very good at this" She laughs as she says this. "Now suck my toe bitch!" She barks.`);
    qspCall(s, 'arousal', 'BDSM_give', 5);
    qspCall(s, 'stat', '');
    scene.actions([
...
function enterAnnaPathChoice(s: GameState, scene: SceneBuilder): void {
  if (((s as any).IgorevnaBDSM_session_librarian ?? 0) === 1) {
    ...
    scene.text(`<center><b> "Hehehe…${((s as any).pcs_nickname || '')}, it's not the we are on the cafe'…" </b></center>`);
    scene.actions([
...
function enterLibrarian1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).Anna_librarian_brat ?? 0) === 2) {
      scene.text(`<center><b>"${((s as any).pcs_nickname || '')}! What are you doing? Are you trying to be the "brat" of the year?"</b></center>`);
    } else {
      scene.text(`<center><b>"${((s as any).pcs_nickname || '')}! Do you mind to pay attention on what you are doing?"</b></center>`);
    }
    scene.actions([
...
function enterLibrarianEnd(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).Anna_librarian_brat ?? 0) <= 1) {
    ...
    scene.text(`Anna help you lay on the sofa with your belly down…  "I'll be gentle ${((s as any).pcs_nickname || '')}…you aren't the only one with a thin skin<br>"N-no?…and… who?…AHHHHHAAAAHHHNNNNA!…"<br>"…I'm sorry ${((s as any).pcs_nickname || '')}…i had to expose your butt…" Anna distract your attention to expose your butt, knowing it could be really sensible…"It's cold at the beginning… it will end soon…" <br>You feel something slowly falling along your skin…"…SSSSHHH… AAHHH… AHHH… AAHH… Aah… aa…"<br>"I'm sure you feel better soon… it's a good cream<br>"…I-i…"The cold change to heat… but it's not pain… it's similar to pleasure; you wonder how's that possible, but you are mainly focused on Anna's hands…"…A-anna…"<br>"Ssssh… You make your part in that too Once your butt finish to catch the cream Anna help you to dress and lead you to the exit.`);
    scene.actions([
      { label: 'Leave', goto: ['pav_hotel', ''] },
    ]);
  } },
...
function enterLibrarianBratEnd(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).Anna_librarian_brat ?? 0))) {
    ...
    scene.text(`<center><b> "…OTK: over the knee… Put your belly on my knees, ${((s as any).pcs_nickname || '')}, we practically finished." </b></center>`);
    scene.actions([
      { label: '…', goto: ['hotel_anna_sex', 'librarian_end'] },
    ]);
  } },
    ...
    qspCall(s, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/Anna/sessionpracticend/librarian/annalibrarian9.jpg');
    scene.text(`<center><b> "…OTK: over the knee… Put your belly on my knees, ${((s as any).pcs_nickname || '')}." </b></center>`);
    scene.actions([
      { label: '…', goto: ['hotel_anna_sex', 'librarian_end'] },
    ]);
  } },
    ...
    scene.text(`<center><b> "Ssshh… Don't say it ${((s as any).pcs_nickname || '')}, there's no need; we finished. Stay silent, I need silence right now…"</b></center>`);
    scene.actions([
      { label: '…', goto: ['hotel_anna_sex', 'librarian_end'] },
    ]);
  } },
...
function enterAnnaDomSession4(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`<center><b> "${((s as any).pcs_nickname || '')}! What did I told you!"</b></center>`);
    scene.actions([
      { label: '…', handler: (st: GameState) => {
    qspCall(s, 'stat', '');
