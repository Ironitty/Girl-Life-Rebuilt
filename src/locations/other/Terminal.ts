...
function enterDefault(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).AboutBussines = 0;
    (st as any).AboutDocs = 0;
    (st as any).AboutDocsManager = 0;
    (st as any).YouNotOpenDoorDir = 0;
    (st as any).YouNotOpenDoorBuh = 0;
    qspGoto(st, 'TerminalOffice', '00');
  } },
  ]);
  scene.build();
}
...
function enterAskfather(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    ((st as any).trfatherQW = (st as any).trfatherQW ?? {})['day'] = ((st as any).daystart ?? 0);
    qspCall(st, 'stat', '');
