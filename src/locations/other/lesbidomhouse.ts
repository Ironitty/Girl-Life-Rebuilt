...
function enterQW1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'I\'d rather not [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    ...
    scene.text(`She leads you to another room when she sees you nod. She had already prepared a bottle of wine, two glasses and some snacks, and invites you to take a seat. "Have a seat, ${((s as any).pcs_nickname ?? 0)}, I'll be with you soon."`);
    scene.actions([
    ...
    // TODO-QSP: dynamic text: She leads you to another room, where you find she's already prepared a bottle of...
    scene.text(`She leads you to another room, where you find she's already prepared a bottle of wine, two glasses and some snacks. "Have a seat, ${((s as any).pcs_nickname ?? 0)}. I'll be with you soon."`);
    scene.actions([
    ...
    scene.text(`Mistress puts her hand on yours reassuringly. "There's no need to be nervous, ${((s as any).pcs_nickname ?? 0)}. We don't start until next time. Today is just for getting to know one another, so why don't you tell me a little about yourself?"`);
...
function enterQW2(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Ask why you have to be naked [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'foreplay', 30, 'sub', 'lesbian');
...
function enterQW3(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).MistressResist ?? 0) === 1) {
    ...
    qspCall(s, 'arousal', 'foreplay', 10, 'sub', 'bound');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    qspCall(s, 'arousal', 'foreplay', 10, 'sub', 'bound', 'humiliation');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    qspCall(s, 'arousal', 'foreplay', 20, 'sub', 'bound');
    qspCall(s, 'arousal', 'end');
    scene.actions([
    ...
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Refuse to take your clothes off [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
...
function enterQW4(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).MistressResist ?? 0) === 2) {
    ...
    qspCall(s, 'arousal', 'kiss', 4, 'dom', 'lesbian');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Leave her apartment', goto: ['city_residential', ''] },
    ]);
    ...
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Refuse to show your pussy [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'foreplay', 60, 'sub', 'bound');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Leave her apartment', goto: ['city_residential', ''] },
    ]);
...
function enterQW5(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    scene.actions([
      { label: 'Beg her not to do it [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
    ]);
  } else {
    ...
    (s as any).minut = ((s as any).minut ?? 0) + 35;
    qspCall(s, 'arousal', 'anal_finger', 20, 'sub', 'lesbian', 'bound', 'rough');
    qspCall(s, 'arousal', 'anal_fist', (-20), 'sub', 'lesbian', 'bound', 'rough');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.actions([
      ...
      scene.text('Ignoring your reaction, she forces her whole hand past your sphincter and starts mercilessly fisting your asshole. She occasionally mutters to herself about how much of a disappointment you\'ve been so far, and that you have a long way to go.');
      qspCall(s, 'arousal', 'anal_finger', 20, 'sub', 'lesbian', 'bound');
      qspCall(s, 'arousal', 'anal_fist', (-20), 'sub', 'lesbian', 'bound');
      qspCall(s, 'stat', '');
      scene.actions([
        { label: 'Break down and cry', handler: (st: GameState) => {
    qspCall(s, 'sweat', 'add', 10);
...
function enterQW6(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).temp ?? 0) === 1) {
      ...
      qspCall(s, 'arousal', 'foreplay', 100, 'sub', 'lesbian', 'bound');
      qspCall(s, 'stat', '');
      scene.actions([
        { label: 'Try to apologize', handler: (st: GameState) => {
    (s as any).minut = ((s as any).minut ?? 0) + 5;
    ...
    qspCall(s, 'arousal', 'anal_finger', 30, 'sub', 'lesbian', 'bound', 'rough');
    qspCall(s, 'arousal', 'anal_fist', 30, 'sub', 'lesbian', 'bound', 'rough');
    qspCall(s, 'arousal', 'vaginal_finger', 30, 'sub', 'lesbian', 'bound', 'rough');
    qspCall(s, 'arousal', 'vaginal_fist', (-30), 'sub', 'lesbian', 'bound', 'rough');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Leave her apartment', goto: ['city_residential', ''] },
    ]);
    ...
    qspCall(s, 'arousal', 'bj', 15, 'lesbian', 'sub', 'deepthroat', 'rough');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Leave her apartment', goto: ['city_residential', ''] },
    ]);
...
function enterQW7(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Her face then breaks out into a smile. "${((s as any).pcs_nickname ?? 0)}! I'm so glad you came. I owe you an apology, come here."`);
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'vaginal_vibe', 20, 'sub', 'lesbian');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    // TODO-QSP: dynamic text: "Erm, hi. I'm <<$pcs_nickname>>…" you reply softly. Your mind is still a bit haz...
    scene.text(`"Erm, hi. I'm ${((s as any).pcs_nickname ?? 0)}…" you reply softly. Your mind is still a bit hazy from the stimulation.`);
    ...
    (s as any).pcs_horny = ((s as any).pcs_horny ?? 0) + (75);
    (s as any).lesbiQW = 8;
    (s as any).MistressAdoration = ((s as any).MistressAdoration ?? 0) + (3);
    qspCall(s, 'stat', '');
    ...
    scene.text(`"${((s as any).pcs_nickname ?? 0)} is becoming a very good friend of mine very quickly, Masha! I'm sure you'll see more of her…" Mistress says ambiguously. You blush when you realize what she means… she's going to show you off to her friend!`);
    ...
    scene.text(`When you return home, Mistress gives you a kiss on the cheek and gives you the remote. "Here, you earned it. That was a fun session, ${((s as any).pcs_nickname ?? 0)}! Don't get too used to it though. Next session, we'll be focusing on your training again."`);
    qspCall(s, 'arousal', 'end');
    scene.actions([
...
function enterQW8(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    scene.actions([
      { label: 'Hesitate and stay where you are [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
    ]);
  } else {
    ...
    if (((s as any).pcs_vag ?? 0) < 20  &&  ((s as any).pcs_vag ?? 0) >= 10) {
    ...
    qspCall(s, 'arousal', 'anal_strap', 20, 'sub', 'lesbian', 'rough');
    ...
    scene.actions([
      { label: 'Do as she says', handler: (st: GameState) => {
    qspCall(s, 'arousal', 'pee', 20, 'sub', 'humiliation', 'lesbian');
    qspCall(s, 'arousal', 'end');
  ...
  scene.actions([
    ...
    if ((!((s as any).temp ?? 0))) {
      ...
      qspCall(s, 'arousal', 'foreplay', 40, 'sub', 'exhibitionism', 'bound', 'humiliation');
      qspCall(s, 'stat', '');
    } else {
      if (((s as any).temp ?? 0) === 1) {
        ...
        if (((s as any).temp ?? 0) === 2) {
          ...
          qspCall(s, 'arousal', 'bj', 40, 'sub', 'exhibitionism', 'humiliation');
          qspCall(s, 'stat', '');
        }
      }
    }
...
function enterQW10a(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).temp ?? 0))) {
    ...
    qspCall(s, 'arousal', 'cuni_give', 15, 'sub', 'lesbian', 'bound');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Get dressed and leave', goto: ['city_residential', ''] },
    ]);
...
function enterQW10b1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).temp ?? 0) === 1) {
    ...
    scene.text('Mistress caresses your scalp for a second before snapping back into her regular role as she undoes your binds. "You know the drill, slave. Get dressed in the hallway and find your own way out." When she spots her juices on your chin and breasts, she grins. "Oh… and leave that on."');
    scene.text('You blush a little as you get dressed before leaving.');
    qspCall(s, 'arousal', 'cuni_give', 20, 'sub', 'lesbian', 'bound');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Get dressed and leave', goto: ['city_residential', ''] },
    ]);
...
function enterQW10b2(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).temp ?? 0))) {
    ...
    qspCall(s, 'arousal', 'vaginal_dildo_give', 40, 'sub', 'bound', 'lesbian');
    qspCall(s, 'stat', '');
...
function enterQW10b3(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).temp ?? 0) === 1) {
    ...
    qspCall(s, 'arousal', 'anal_dildo', 40, 'sub', 'lesbian', 'bound');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Get dressed and leave', goto: ['city_residential', ''] },
    ]);
...
function enterQW10c(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).temp ?? 0))) {
    ...
    (s as any).MistressAdoration = ((s as any).MistressAdoration ?? 0) + (4);
    qspCall(s, 'stat', '');
    scene.img('images/characters/city/natalya/sex/sub13.jpg');
...
function enterQW12(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).mistresscontract ?? 0))) {
    ...
    scene.text(`She smiles benevolently at you. "${((s as any).pcs_nickname ?? 0)}, I know this is a lot to take in, so I'll give you a chance to read the contract in full and think it over. I don't want you to sign something you don't believe is right." She then stands up and walks to her bedroom, lightly squeezing your shoulder as she walks past you.`);
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    ...
    scene.text(`You mull it over for another minute, after which Mistress returns to the table and smiles at you confidently. "So, ${((s as any).pcs_nickname ?? 0)}… do you have any questions?"`);
    scene.actions([
    ...
    // TODO-QSP: dynamic text: She smiles at you. "I'll put this in my safe. I'm really glad we're taking this ...
    scene.text(`She smiles at you. "I'll put this in my safe. I'm really glad we're taking this step together, ${((s as any).pcs_nickname ?? 0)}. Let's celebrate! Would you like some wine?"`);
    ...
    scene.text(`She stands up and signals for you to do the same. "I guess this is where we part ways, then. Take care, ${((s as any).pcs_nickname ?? 0)}."`);
    scene.actions([
    ...
    if (((s as any).mistresscontract ?? 0) === -1) {
      ...
      // TODO-QSP: dynamic text: "<<$pcs_nickname>>, you're back!" she says and invites you to sit down. "Did you...
      scene.text(`"${((s as any).pcs_nickname ?? 0)}, you're back!" she says and invites you to sit down. "Did you get a chance to think about my proposal?"`);
    ...
    (s as any).lesbiQW = 13;
    (s as any).mistresscontract = ((s as any).daystart ?? 0) + 182;
    ...
    scene.text(`Mistress is clearly disappointed. "That's too bad, ${((s as any).pcs_nickname ?? 0)}. I really thought we would be able to work it out, but I guess you don't not want this as much as I do."`);
    scene.text('She breaks the awkward silence that follows. "You know, I wasn\'t lying when I said I see potential in you. There\'s a BDSM club near the city, where people with interest in our lifestyle can go to explore their interests. I think you would enjoy going there."');
    scene.text('She offers you a luxurious black business card with a gold trim. "If you\'re interested, this will get you in."');
    // TODO-QSP: dynamic text: She stands up and signals for you to do the same. "I guess this is where we part...
    scene.text(`She stands up and signals for you to do the same. "I guess this is where we part ways, then. Take care, ${((s as any).pcs_nickname ?? 0)}."`);
