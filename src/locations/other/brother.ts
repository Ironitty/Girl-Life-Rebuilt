...
function enterDefault(s: GameState, scene: SceneBuilder): void {
  if (!(s as any).brotherQW) (s as any).brotherQW = {}; (s as any).brotherQW['Age'] = ((s as any).year ?? 0) - (((((s as any).npc_dob ?? {})?.['A34'] ?? 0) - ((((s as any).npc_dob ?? {})?.['A34'] ?? 0) % 10000)) / 10000);
...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).loc ?? 0) === 'sitrPar') {
    ...
    if (((s as any).brotherQW ?? 0)?.['Sex'] === 0) {
      ...
      scene.text(`After a minute, he releases your ${((s as any).titsize || '')} boobs, saying, "I really like your new additions."`);
      qspCall(s, 'arousal', 'foreplay', 1, 'incest');
      qspCall(s, 'stat', '');
    }
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterBrotherSexStart(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
...
function enterPlayfootball(s: GameState, scene: SceneBuilder): void {
  ...
  if ((Math.floor(Math.random() * 2) + 0) === 0  &&  (!((s as any).seepornofut ?? 0))) {
    ...
    dynamicGoto(st, 'loc', 'loc_arg');
        ...
        scene.text(`Kolka hesitates for a moment before giving you a funny look, "Actually, ${((s as any).pcs_nickname || '')}, we want to go watch some porn but we weren't sure with having you around… But I think a girl like you could handle it."`);
        scene.actions([
          { label: 'Leave', handler: (st: GameState) => {
    dynamicGoto(st, 'loc', 'loc_arg');
  } },
...
function enterKissTalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Kolka breaks the silence, "It's okay, ${((s as any).pcs_nickname || '')}."`);
    ...
    scene.text(`"It's okay, ${((s as any).pcs_nickname || '')}," he repeats as he turns back towards you. "Really, it is. You're a beautiful girl," he says, causing you to blush even harder. "Anyone would be made happy if you kissed them like that. Even your brother. And if that's wrong…"`);
...
function enterBrotherSexStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).brotherQW ?? 0)?.['Sex'] >= 5) {
    ...
    scene.text(`It isn't long before you hear your brother moan, "I'm gonna cum, ${((s as any).pcs_nickname || '')}!"`);
    if (((s as any).brotherQW ?? 0)?.['Sex'] >= 6) {
      scene.actions([
