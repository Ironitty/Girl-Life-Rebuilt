...
function enterStartDoor(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(st, 'stat', '');
    if (((st as any).petersexday ?? 0) < 1) {
      (st as any).petersexday = ((st as any).petersexday ?? 0) + (1);
...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).peter ?? 0) > 0) {
    ...
    (st as any).petersex = ((st as any).petersex ?? 0) + 1;
    if (((st as any).analPlugIn ?? 0) === 1) {
      qspGoto(st, 'peterroom', 'klap');
    }
    if (((st as any).peterrand ?? 0) < 2) {
      qspGoto(st, 'peterroom', 'd1');
    }
    if (((st as any).peterrand ?? 0) > 1) {
      qspGoto(st, 'peterroom', 'd2');
    }
  } },
    ]);
  } else {
...
function enterKlap(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    (st as any).temp = 'Your moans are muffled by the gag as Peter bears down on you from above. "Let\'s see how well your sweet ass takes me…" he says ';
    if (((st as any).analPlugIn ?? 0) === 1) {
      (st as any).analPlugIn = 0;
      (st as any).analPlugOut = 1;
    ...
    if (((st as any).pcs_horny ?? 0) >= 100) {
      ...
      qspCall(st, 'arousal', 'anal', (-10), 'sub');
      (st as any).orgasm_txt = 'A pleasant sensation surges within you and an orgasm washes over you, juices spraying from your pussy as you moan loudly into the gag.';
    }
