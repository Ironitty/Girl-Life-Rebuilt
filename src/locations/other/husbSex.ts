...
function enterHusbDebtEvent(s: GameState, scene: SceneBuilder): void {
  (s as any).spouseVars['husb_debt_event'] = 1;
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} is about to enter you as he gets pulled back and you spot two other men in the room. One is standing by the bed holding your husband's hair, the second one is holding a gun in his hand.`);
  ...
  scene.text(`"I told, I will pay you. I have no money now, but in a month I can give it to you" Says ${((s as any).npcdesc ?? 0)}.`);
  ...
  scene.text(`"But I don't have the money. I need more time." Pleads ${((s as any).npcdesc ?? 0)}.`);
  ...
  if (((s as any).npc_rough ?? 0)?.[String((s as any).npcID ?? 0)] === 0) {
    // TODO-QSP: dynamic text: "Don't touch her! I'll get you the money. You will have it tomorrow." Says <<$np...
    scene.text(`"Don't touch her! I'll get you the money. You will have it tomorrow." Says ${((s as any).npcdesc ?? 0)}.`);
    ...
    scene.text(`"Alright you can have her. But this is part of my payment, Kral." Says ${((s as any).npcdesc ?? 0)}.`);
  ...
  scene.actions([
    ...
    if (qspFunc(s, 'money', 'can_afford', 100000) === 0) {
      ...
      (s as any).spouseVars['husb_debt_event'] = 2;
...
function enterDolg(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'vaginal', 10, ((s as any).npcID ?? 0), 'group', 'humiliation', 'rape');
    qspCall(s, 'arousal', 'vaginal', 10, ((s as any).npcID1 ?? 0), 'group', 'humiliation', 'rape');
    qspCall(s, 'arousal', 'anal', (-10), ((s as any).npcID1 ?? 0), 'group', 'humiliation', 'rape');
    qspCall(s, 'arousal', 'anal', (-10), ((s as any).npcID ?? 0), 'group', 'humiliation', 'rape');
  ...
  if (((s as any).pcs_apprnc ?? 0) >= 60) {
    ...
    scene.actions([
      ...
      if (((s as any).spouseVars ?? 0)?.['husb_debt_event'] === 2) {
        (s as any).spouseVars['husb_debt_event'] = 3;
...
function enterVariant(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} He is looking at you and waiting for what you say`);
...
function enterMinet(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`Your husband's ${((s as any).dick ?? 0)} centimeter member enters your mouth.`);
  ...
  scene.actions([
    ...
    scene.text(`A jet of hot sperm hits you in the mouth. You begin to swallow his sperm as ${((s as any).npcdesc ?? 0)} continues to cum in your mouth. Finally, the flow of hot, pungent tasting liquid stops.`);
    scene.actions([
    ...
    if ((!((s as any).tiprand ?? 0))) {
      ...
      scene.text(`You take his member out and at this point ${((s as any).npcdesc ?? 0)} discharges directly on your face, covering it with warm and sticky sperm.`);
    } else {
      qspCall(s, 'npc_relationship', 'modify', ((s as any).npcID ?? 0), (-1));
      ...
      scene.text(`You take his member out of your mouth and ${((s as any).npcdesc ?? 0)} finishes onto the palm of your hand.`);
    }
    scene.actions([
      { label: 'Leave', goto: ['vanr', ''] },
    ]);
...
function enterHand(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You take the ${((s as any).dick_desc ?? 0)} cock and begin to caress it with your hands, moving the skin of the purple head.`);
...
function enterKuni(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} lies down between your legs and begins to lick your clit and pussy.`);
...
function enterStrapon(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You get the strap-on from the box and put it on. ${((s as any).npcdesc ?? 0)} kneels in the doggy position on the bed.`);
...
function enterAnal(s: GameState, scene: SceneBuilder): void {
  if (((s as any).analPlugIn ?? 0) === 1) {
    ...
    if (((s as any).pcs_ass ?? 0) < 16) {
      ...
      scene.text(`${((s as any).npcdesc ?? 0)} pulls the anal plug out of your narrow and tight ass.`);
    } else {
      // TODO-QSP: dynamic text: <<$npcdesc>> pulls out the anal plug and oozes flush out of your ass. Soon you f...
      scene.text(`${((s as any).npcdesc ?? 0)} pulls out the anal plug and oozes flush out of your ass. Soon you feel his hard cock enter your hole.`);
    }
    ...
    if (((s as any).pcs_ass ?? 0) + ((s as any).analPlugInbonus ?? 0) < ((s as any).dick_length ?? 0)) {
      ...
      scene.text(`You scream and bite your lip from the sharp pain when ${((s as any).dick_desc ?? 0)} member inches apart your ass as he enters you.`);
    } else {
      // TODO-QSP: dynamic text: You groan when his full <<$dick_desc>> member inches apart your ass as he enters...
      scene.text(`You groan when his full ${((s as any).dick_desc ?? 0)} member inches apart your ass as he enters you.`);
    }
    // TODO-QSP: dynamic text: <<$npcdesc>> tightly grabs your buttcheeks as he drives is cock deep into your a...
    scene.text(`${((s as any).npcdesc ?? 0)} tightly grabs your buttcheeks as he drives is cock deep into your aching ass one last time, releasing the contents of his balls.`);
  ...
  if (((s as any).mc_inventory ?? 0)?.['lubricant'] === 0) {
    ...
    scene.text(`You don't have an lubricants, so ${((s as any).npcdesc ?? 0)} spits on his fingers and uses the saliva to lubricate your ass.`);
  } else {
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} puts you on you stomach and parts your buttocks, massaging your anus with his fingers, first one, then two.`);
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} Pulls his fingers out of your ass, and you feel his hard cock slide into your hole.`);
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} freezes for a moment giving your ass time to adapt to his member. Then he begins to drive it in, all the more insistent.`);
  qspCall(s, 'arousal', 'anal', 10, 'sub');
  qspCall(s, 'stat', '');
  if (((s as any).pcs_ass ?? 0) + ((s as any).anal_slip ?? 0) + 2 < ((s as any).dick_length ?? 0)) {
    ...
    scene.text(`You cry and sink your teeth into the pillow when his ${((s as any).dick_desc ?? 0)} cock begins to fuck your ass at full power.`);
  } else {
  ...
  if (((s as any).klismaday ?? 0) !== ((s as any).daystart ?? 0)) {
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} takes his member out of your ass and seeing shit on his dick tells you, "Damn. ${((s as any).pcs_nickname ?? 0)} clean your ass before sex."`);
  } else {
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} takes his member out of you, kisses you and goes to the bathroom.`);
  }
  if (((s as any).pcs_ass ?? 0) < ((s as any).dick_length ?? 0)) {
    qspCall(s, 'arousal_funcs', 'stretch', 'anal', 1);
  }
...
function enterMiss(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} spreads your legs and uses his fingers to caress your pussy.`);
...
function enterDog(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`${((s as any).npcdesc ?? 0)} turns you around into the doggy style position and spreads your buttcheeks before he enters you.`);
  qspCall(s, 'dinsex', 'boy_puts_condom');
...
function enterHusbMastrVtor(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) > 22  ||  ((s as any).hour ?? 0) < 7) {
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} looks at you, obviously a little dumbfounded.`);
  } else {
    // TODO-QSP: dynamic text: Just as you start getting into it, <<$npcdesc>> walks in.
    scene.text(`Just as you start getting into it, ${((s as any).npcdesc ?? 0)} walks in.`);
  }
  if (((s as any).npc_gentle ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
    ...
    scene.text(`You pull the dildo out of your pussy and, winking at your husband, insert it into your anus. ${((s as any).npcdesc ?? 0)} closely watches as the dildo disappears in your ass and then reappears. You see a swelling in his pants.`);
    ...
    scene.text(`${((s as any).npcdesc ?? 0)} takes out his member and begins to masturbate. This is not what you expected and at first you even feel a little sorry that he did not enter you, but then you start to masturbate again. You look at your husband, who masturbates looking at you, as you drive the dildo in your ass. Soon you feel yourself coming and you cry out in delight as you fall on the table, orgasming, the dildo still sticking out of your ass. ${((s as any).npcdesc ?? 0)} furiously masturbates looking at you stretched out on the table. You remov the dildo from your ass and start licking it in front of your husband's eyes. Clearly excited from such a spectacle he soon starts cumming, spraying his sperm on the floor. You reach out to him sticking out your lips. A moment later he passionately kisses you.`);
    qspCall(s, 'arousal', 'anal_dildo', 5, 'masturbate');
    qspCall(s, 'arousal', 'end');
    scene.actions([
      { label: 'Finish', handler: (st: GameState) => {
    ...
    if (((s as any).npc_rough ?? 0)?.[String((s as any).npcID ?? 0)] === 0) {
      ...
      if (((s as any).npc_pervert ?? 0)?.[String((s as any).npcID ?? 0)] === 0) {
        ...
        scene.text(`"No, don't. This is enough" He says, and sits down on a chair watching you. You continue to satisfy yourself rapidly, and soon finish in front of husband. ${((s as any).npcdesc ?? 0)} stands up and begins to applaud. "Bravo, can I have an encore?" He says laughing.`);
        // TODO-QSP: dynamic text: "For such a wonderful audience I will do anything, but only after the intermissi...
        scene.text(`"For such a wonderful audience I will do anything, but only after the intermission," You laugh. ${((s as any).npcdesc ?? 0)} helps you getting off the table.`);
        qspCall(s, 'arousal', 'end');
        scene.actions([
          { label: 'Finish', handler: (st: GameState) => {
    dynamicGoto(st, 'loc', 'loc_arg');
    ...
    scene.img('images/characters/city/husband/sex/m22.jpg');
    // TODO-QSP: dynamic text: You pull the dildo out of your pussy and, winking at your husband, insert it int...
    scene.text(`You pull the dildo out of your pussy and, winking at your husband, insert it into your anus. ${((s as any).npcdesc ?? 0)} watches closely as the dildo disappears in your ass and then reappears. You see a swelling in his pants.`);
    scene.text('"Is there anything else you want, dear?" You ask with a smile.');
    scene.text('"I would like to join" He says, unbuttoning his pants and exposing his member to your face.');
    qspCall(s, 'arousal', 'anal_dildo', 10, 'masturbate');
    qspCall(s, 'stat', '');
    scene.actions([
    ...
    scene.text(`You take his slightly flaccid penis and start sucking, continuing to drive the dildo in your anus. You quickly get him hard and ${((s as any).npcdesc ?? 0)} stops you. "Wait, I think I'll take another hole."`);
    ...
    scene.img('images/characters/city/husband/sex/m24.jpg');
    // TODO-QSP: dynamic text: You take his slightly flaccid penis and start sucking, continuing to drive the d...
    scene.text(`You take his slightly flaccid penis and start sucking, continuing to drive the dildo in your anus. You quickly get him hard and ${((s as any).npcdesc ?? 0)} stops you. "Take the dildo out. It is time for me to fuck your ass."`);
    qspCall(s, 'arousal', 'bj', 10, 'sub');
    qspCall(s, 'arousal', 'anal_dildo', (-10), 'sub');
    qspCall(s, 'stat', '');
    scene.actions([
