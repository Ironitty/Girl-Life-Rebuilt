...
function enterModelTalk(s: GameState, scene: SceneBuilder): void {
  if (((s as any).anastasia ?? 0)?.['model_exp'] !== ((s as any).daystart ?? 0)) {
    ...
    qspCall(s, 'model_anastasia', 'model_talk');
  } },
    ]);
  }
  scene.actions([
...
function enterSexTalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'model_anastasia', 'sex_talk');
  } },
  ]);
  scene.build();
}
...
function enterAnastasiasex(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"I've got big tits too," you grin, looking down meaningfully at your ${((s as any).titsize ?? 0)} breasts. "Boys wouldn't be boys if they didn't drool over the thought of fucking busty girls like me.`);
