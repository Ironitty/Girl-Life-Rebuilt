...
function enterTransition1Desc(s: GameState, scene: SceneBuilder): void {
  if (((s as any).sex_ev ?? 0)?.['loc'] === 'player home') {
    ...
    scene.text(`You pant tiredly, laying back on your bed as your orgasm slowly winds down. Suddenly ${((s as any).npcdesc ?? 0)}'s cock enters your view, dangling above your head, and he bends over you, trying to push it into your panting mouth.`);
  } else {
    if (((s as any).sex_ev ?? 0)?.['loc'] === 'npc_home') {
      // TODO-QSP: dynamic text: You pant tiredly, laying back on <<$npcdesc>>'s bed as your orgasm slowly winds ...
      scene.text(`You pant tiredly, laying back on ${((s as any).npcdesc ?? 0)}'s bed as your orgasm slowly winds down. Suddenly ${((s as any).npcdesc ?? 0)}'s cock enters your view, dangling above your head, and he bends over you, trying to push it into your panting mouth.`);
    } else {
