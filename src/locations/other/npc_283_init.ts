...
function enterExamInterview(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`“I did not give you permission to sit.”, Rudolph says in an icy cold voice without looking up from the document he is reading. “${((s as any).pcs_firstname ?? 0)} ${((s as any).pcs_lastname ?? 0)}, ${((s as any).age ?? 0)} years old, lives in Pavlovsk. Your mother is ${((s as any).npc_firstname ?? 0)?.['A29']} ${((s as any).pcs_lastname ?? 0)} who split from her husband after being caught having affairs. Yet despite being poor you aspired to be a ballerina, and despite all your efforts to get selected you showed a complete lack of commitment or skill.”`);
  ...
  scene.actions([
    ...
    ((s as any).rudolphqw ?? {})['counter'] = ((s as any).daystart ?? 0) + 7;
    ...
    scene.text(`Rudolph glares at you for a moment and then his expression changes to being more fatherly, “Do not worry ${((s as any).pcs_nickname ?? 0)}, I will not lay a finger on you. I do not rape my pupils, and all my relationships have been consensual. However I still get to decide what you owe me and how you pay me back. Understand?” Something in his voice feels slightly threatening but you can't see what that could be.`);
    ...
    scene.text(`“I am glad you see it my way. You may attend the classes and rehearsals but I will withhold the privilege to perform at the winter performance.”, you start to speak, “Do not try me ${((s as any).pcs_nickname ?? 0)}, you have already pushed my patience and you are walking on thin ice already. Now leave, I have other matters to attend to and you have wasted too much of my time. Remember I will be watching your training and your actions from now on.” His voice makes it clear that he owns you now as you leave the room with each heavy step.`);
    scene.actions([
      { label: 'Return', handler: (st: GameState) => {
    dynamicGoto(st, 'loc', 'loc_arg');
  } },
