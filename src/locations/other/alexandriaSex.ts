...
function enterNice1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Bossing me around, ${((s as any).pcs_lastname || '')}? Not a good idea, and for that you're going to have to wait for me to be ready."`);
    ...
    scene.text(`"No ${((s as any).pcs_lastname || '')}. Slow… with care."`);
    ...
    scene.text(`"Ordering me again ${((s as any).pcs_lastname || '')}? What am I supposed to do with you? I think I must meditate on it, but until then, don't you think that your bosom needs some attention?"`);
    ...
    scene.text(`"Isn't it better when you listen to me ${((s as any).pcs_lastname || '')}?"`);
    scene.actions([
    ...
    scene.text(`"Okay ${((s as any).pcs_lastname || '')}, let's work on our magic."`);
    ...
    scene.text(`"That's the end of this lesson ${((s as any).pcs_lastname || '')}."`);
    ...
    scene.text(`"That was strange. I wasn't expecting Rikudo's magic to force us into this situation ${((s as any).pcs_lastname || '')}. However, I must say that is fascinating! I have not had this sort of insight into Rikudo's work in centuries. I assume, that it has been as… enlightening to you as it was to me?"`);
    ...
    scene.text(`"${((s as any).pcs_lastname || '')}, this is an opportunity I can't refuse. With your approval, we will continue with more of these <i>teaching sessions</i>, but I want to emphasize that this has been enjoyable, and I am sure that neither of us regrets what just happened. However, this can't be anything more than an exchange for my knowledge`);
...
function enterGrumpy1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"No, no, no ${((s as any).pcs_lastname || '')}! This is a lesson you MUST learn."`);
    ...
    scene.text(`"Don't move ${((s as any).pcs_lastname || '')}, you are doing it right"`);
    ...
    scene.text(`"Well ${((s as any).pcs_lastname || '')}, time to teach you some magic."`);
    ...
    scene.text(`"${((s as any).pcs_lastname || '')} You are a wizard, a literal <i>willworker</i> that can make the world cry like a little girl. So why should pain, fatigue or mental duress be allowed to have power over you?"`);
    ...
    scene.text(`"It is evident that I'm a great teacher ${((s as any).pcs_lastname || '')}, so how will you thank me?"`);
    scene.actions([
      { label: 'It\'s his turn', handler: (st: GameState) => {
    qspCall(s, 'arousal', 'BDSM', 5, 'sub', 'lesbian');
    ...
    scene.text(`"${((s as any).pcs_lastname || '')}, I suppose that this lesson is over. It seems that the curse is stronger than I thought, but… We have learned something isn't it ${((s as any).pcs_lastname || '')}? so… our deal is unchanged. I must clear up some facts, however. The truth is that this is only the beginning. I will escalate our methodology."`);
...
function enterNice2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"Enough ${((s as any).pcs_lastname || '')}, is time to continue your training."`);
    ...
    scene.text(`"Fascinating! Isn't it ${((s as any).pcs_lastname || '')}? Your magic interacts with my curse, in truly interesting ways! Beyond altering your thought patterns and senses, now we have <i>Creatio ex nihilo</i>! All of this without true volition!"`);
    ...
    scene.text(`"Well, it has been illuminating, but I think that this <i>teaching lesson</i> has come to an end, and I must think on how to <i>reward</i> you the next time ${((s as any).pcs_lastname || '')}"`);
...
function enterGrumpy2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"${((s as any).pcs_lastname || '')} I want to get something clear. I am not a sadist, and this, doesn't give me any pleasure, but… you on the other hand… ARE enjoying this isn't it? That is what I find fascinating, MY curse is working through OUR link, to make YOU a masochist!"`);
    ...
    scene.text(`"There… There… ${((s as any).pcs_lastname || '')}, you are clean and ready."`);
    ...
    scene.text(`"SLAP! Wake up ${((s as any).pcs_lastname || '')}! SLAP!"`);
    ...
    scene.text(`"No ${((s as any).pcs_lastname || '')}… No… Remember… when I say so."`);
    scene.actions([
    ...
    scene.text(`"Don't worry ${((s as any).pcs_lastname || '')}! Just relax! We have all the time in the world!"`);
    ...
    scene.text(`"Good ${((s as any).pcs_lastname || '')}, good, now you can have your release."`);
    ...
    scene.text(`"Now ${((s as any).pcs_lastname || '')}, let's see if you can learn something."`);
    ...
    scene.text(`"Nice ${((s as any).pcs_lastname || '')}… very nice…"`);
    ...
    scene.text(`"What are you waiting ${((s as any).pcs_lastname || '')}? You are a will-worker! Just come here!"`);
    ...
    scene.text(`"Only a little more ${((s as any).pcs_lastname || '')}!"`);
    ...
    scene.text(`"Hm… ¨You are learning ${((s as any).pcs_lastname || '')}."`);
    ...
    scene.text(`"${((s as any).pcs_lastname || '')}, It's evident to me, that you have enjoyed this lesson beyond whatever the curse made you feel… What do you think about that? Hm? Well, at the end of the day it's unimportant, as it seems that you can't learn without discipline."`);
