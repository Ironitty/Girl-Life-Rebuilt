...
function enterClientTalk(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).whore_money_bjsex ?? 0) < 50) {
    ...
    qspCall(s, 'stat', '');
    qspGoto(s, 'pod_whore', 'hide', 'pod_client_talk_no');
  } },
      ]);
    }
    scene.actions([
    ...
    qspCall(s, 'willpower', 'pay', 'resist');
    qspCall(s, 'stat', '');
    qspGoto(s, 'pod_whore', 'hide', 'pod_whore_no');
  } },
        ]);
      }
      scene.actions([
...
function enterHide(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === 'pod_whore_analPlugIn') {
    ...
    qspCall(s, 'willpower', 'pay', 'resist');
    qspCall(s, 'stat', '');
    qspGoto(s, 'pod_whore', 'hide', 'pod_client_talk_bazar7');
  } },
          ]);
        }
        scene.actions([
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', 'text_pod_whore_groupsex']; enterHide(s, scene); (s as any).locArgs = __savedLocArgs; }
    qspCall(s, 'gopsex', 'gg_sex_orgasm');
    qspCall(s, 'gopsex', 'hide', 'accview_ggsex');
    scene.actions([
      { label: 'Continue', handler: (st: GameState) => {
    ...
    scene.text(`Soon enough, the hot jets of cum splatter down on your face, mostly on your exposed tongue. A few drips run down your chin and drop onto your ${((s as any).titsize || '')} boobs. You absent-mindedly rub them, spreading their cum all over them.`);
    ...
    scene.text(`One guy produces the money you were promised, and hands you the ${qspFunc(s, 'money', 'string_profit', ((s as any).total_money || ''))}. He smiles happily: "You were great, girl. We'll definitely be back."`);
    scene.actions([
    ...
    scene.text(`Then one guy turns to you and gives you the money: "You're a pretty decent fuck, ${((s as any).pcs_nickname || '')}. Here are the ${qspFunc(s, 'money', 'string_profit', ((s as any).total_money || ''))} we agreed upon. See you around."`);
    scene.actions([
      { label: 'Thank them and leave', handler: (st: GameState) => {
    qspCall(st, 'pod_whore', 'hide', 'clean');
  }, goto: ['pod_ezd', 'etaj_2'] },
