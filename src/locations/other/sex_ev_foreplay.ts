...
function enterBjAsk2(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    { label: 'Can\'t we just fuck again? (complain)', handler: (st: GameState) => {
    scene.text('"Ugh, do I have to?" you complain. "Can\'t we just fuck again?"');
    if (((s as any).npc_selfish ?? 0)?.[String((s as any).npcID ?? 0)] === 1) {
    } else {
...
function enterForeplayChoose(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).sex_ev ?? 0)?.['position'] !== 'blowjob') {
    // TODO-QSP: act 'Play with his cock (mouth)': gt 'sex_ev_foreplay', 'blowjob_tease_dom'
  }
...
function enterBjDom1Cum(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).sex_ev ?? {})['cum_face'] = 1;
  qspCall(s, 'sex_ev_foreplay', 'bj_dom_facial1');
  qspCall(s, 'sex_ev_sex', 'sex_end');
  // TODO-QSP: end}
  scene.actions([
...
function enterBjDom2CumAct(s: GameState, scene: SceneBuilder): void {
  scene.img('images/shared/sex/blowjob/bj47.mp4');
  scene.text('You give it a go for a good few minutes before giving him one last mighty slurp and pull back.');
  scene.text('"How about we do something else?" you ask.');
