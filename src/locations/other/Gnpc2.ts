...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
    ...
    if (!(s as any).otnBoyFrend) (s as any).otnBoyFrend = {}; (s as any).otnBoyFrend[String((s as any).numnpc ?? 0)] = ((s as any).otnBoyFrend[String((s as any).numnpc ?? 0)] ?? 0) - (10);
    qspCall(s, 'stat', '');
  ...
  if (((s as any).pcs_horny ?? 0) >= 75) {
    ...
    if (!(s as any).kissBoyFrend) (s as any).kissBoyFrend = {}; (s as any).kissBoyFrend[String((s as any).numnpc ?? 0)] = ((s as any).kissBoyFrend[String((s as any).numnpc ?? 0)] ?? 0) + (1);
    qspCall(s, 'stat', '');
    ...
    scene.text(`You turn your head to the guy and your lips meet. ${((s as any).boydesc || '')} starts to passionately caress your body with his hands as he kisses you.`);
  } },
      ]);
    }
  }
  ...
  scene.actions([
    ...
    scene.text(`You accompany ${((s as any).boydesc || '')} and when you reach the club he pays the entrance fee and you both walk in. Together with your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} you head to the dance floor and start dancing.`);
    if (((s as any).diskoIvrand ?? 0) >= 80) {
      ...
      if ((!((s as any).diskoIvTiperand ?? 0))) {
        ...
        scene.text(`While you are dancing with ${((s as any).boydesc || '')} a drunk kid in sweatpants and sweatshirt approaches.`);
        ...
        if (((s as any).fame ?? 0)?.['pav_slut'] < 150) {
          ...
          scene.text(`The Gopnik pushes ${((s as any).boydesc || '')} to the side and grabs you by the waist. "Let's dance, babe!"`);
          qspCall(s, 'willpower', 'misc', 'resist', 'easy');
          if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
            scene.actions([
    ...
    scene.text(`You escape from the clutches of the Gopnik. ${((s as any).boydesc || '')} pushes the drunk kid away who immediately goes to his Gopnik buddies who were standing nearby. It didn't take them long to approach your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} and an argument starts. The Gopnik raises his fists, showing your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} that he wants to fight.`);
    if (((s as any).tiprand ?? 0) <= 70) {
      // TODO-QSP: dynamic text: Fortunately, <<$boydesc>> manages to win the fight verbally, accusing the Gopnik...
      scene.text(`Fortunately, ${((s as any).boydesc || '')} manages to win the fight verbally, accusing the Gopnik of grabbing you, his girlfriend. But the evening was spoiled and finally ${((s as any).boydesc || '')} suggests that it would be better if you both head home.`);
      scene.actions([
        { label: 'Go home', goto: ['pav_residential', ''] },
      ]);
    } else {
      if (((s as any).tiprand ?? 0) <= 85) {
        ...
        scene.text(`The Gopnik quickly pounces on your guy and punches him in the face. It seems that ${((s as any).boydesc || '')} doesn't stand a chance against the three Gopniks and before he gets completely beaten up he offers all his money to the angry Gopniks and apologizes repeatedly before all of them. You quickly leave the dance floor and ${((s as any).boydesc || '')} suggests that you both head home.`);
        scene.actions([
          { label: 'Go home', goto: ['pav_residential', ''] },
        ]);
      } else {
    ...
    if (((s as any).pcs_agil ?? 0) >= 60) {
      ...
      scene.text(`At this point ${((s as any).boydesc || '')} comes from the side and punches the hooligan hard on his already broken nose. The poor guy flies backwards and comes crashing down to the floor. Two other Gopniks used the distraction to grab onto your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} and try to pin his arms.`);
      scene.actions([
    ...
    if (!(s as any).GopWin) (s as any).GopWin = {}; (s as any).GopWin[String((s as any).numnpc ?? 0)] = ((s as any).GopWin[String((s as any).numnpc ?? 0)] ?? 0) + (1);
    scene.actions([
      { label: 'Go home', goto: ['pav_residential', ''] },
    ]);
  } },
    ...
    scene.text(`${((s as any).boydesc || '')} can't escape the attacks of the three Gopniks and soon falls under a barrage of punches. The Gopniks drag him into the streets where they steal all his money, take off his shoes, and throw him into the bushes.`);
    ...
    if (!(s as any).otnBoyFrend) (s as any).otnBoyFrend = {}; (s as any).otnBoyFrend[String((s as any).numnpc ?? 0)] = 0;
    (s as any).DPtipe = 2;
    qspCall(s, 'stat', '');
    // TODO-QSP: dynamic text: You decide to dance with the Gopnik. For a moment <<$boydesc>> is staring at you...
    scene.text(`You decide to dance with the Gopnik. For a moment ${((s as any).boydesc || '')} is staring at you in disbelieve, then he approaches the guy and grabs his shoulder. "Hey you! That's my girl!" Suddenly a couple of Gopniks grab your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} and keep him from interrupting the hooligan dancing with you.`);
    // TODO-QSP: dynamic text: After a few minutes <<$boydesc>> stops struggling and the Gopniks let him free. ...
    scene.text(`After a few minutes ${((s as any).boydesc || '')} stops struggling and the Gopniks let him free. He curses and glares angrily at you, while you are still dancing with the Gopnik, and then stomps out of the club.`);
    ...
    scene.actions([
      { label: 'Leave', goto: ['pav_commcenter', ''] },
          ...
          scene.text(`The Gopnik takes one look at you and then starts laughing, "Listen, buddy. Why are you wasting your time dancing with her? ${((s as any).pcs_nickname || '')} is a ${((s as any).gnikname || '')}. Just take her into the bushes out back, but don't hurt her mouth. That sweet set of lips can really smack when sucking dick." You blush scarlet red from the Gopnik's words, deeply embarrassed. ${((s as any).boydesc || '')} rushes up to the guy, "What did you say? Come again!" But the hooligan just went back to join his friends. "Listen, boy, no need to argue. Everyone in the neighborhood knows she is a ${((s as any).gnikname || '')}." ${((s as any).boydesc || '')} looks at you in surprise. "Is this true?" The boys laugh "Well, everyone except you." ${((s as any).boydesc || '')} glares angrily at you and then he slaps you hard across the face. "Go fuck yourself, slut!" He abruptly leaves the dance floor.`);
          scene.actions([
            { label: 'Rub your cheek', goto: ['pav_disco', ''] },
          ]);
        }
      } else {
        if (((s as any).diskoIvTiperand ?? 0) === 1) {
          // TODO-QSP: dynamic text: You enjoy dancing with your <<$bfTitle[numnpc]>>. After a while <<$boydesc>> say...
          scene.text(`You enjoy dancing with your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''}. After a while ${((s as any).boydesc || '')} says that he could do with a beer and invites you to a drink.`);
    ...
    if (((s as any).fame ?? 0)?.['pav_slut'] >= 150) {
      ...
      scene.text(`Someone walks up to you "Hey, I've been watching you having fun with your boyfriend here. He still does't know that you are a ${((s as any).gnikname || '')}? Maybe I should tell him?"`);
    ...
    scene.text(`You angrily spit at him, "Go fuck yourself, asshole!", and storm out of the restroom. Just as you reach the dance floor you see that ${((s as any).boydesc || '')} is returning from the bar. You quickly grab his hands and begin dancing with him, still a little furious about the incident.`);
    // TODO-QSP: dynamic text: <<$boydesc>> notices your bad mood and asks, "What's wrong, <<$pcs_nickname>>?"....
    scene.text(`${((s as any).boydesc || '')} notices your bad mood and asks, "What's wrong, ${((s as any).pcs_nickname || '')}?". At this moment, you see the guy from the restrooms returning. He walks up to your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} and pulls him to the side. After a while, ${((s as any).boydesc || '')} returns and says "We're through. Based on the shit I was just told, I won't waste another minute on the likes of a whore like you!"`);
    scene.actions([
      { label: 'Watch him leave', goto: ['pav_disco', ''] },
    ]);
  } },
    ...
    scene.text(`You try to ignore the guys as you slip out of the restroom and walk back to the dance floor. ${((s as any).boydesc || '')} has been looking for you. He comes up to you and asks, "Where have you been? I looked everywhere for you." You say the first thing that comes to your mind, "I went to the toilet." ${((s as any).boydesc || '')} asks "Why are you wiping your mouth?"`);
    ...
    scene.text(`You suggest to your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} to leave the club. ${((s as any).boydesc || '')} shrugs and you both walk out to the exit.`);
    scene.actions([
      { label: 'Leave',  },
    ]);
  } },
    ...
    scene.text(`You continue to dance with your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''}. A few minutes pass and then you see the guys that watched your whole blowjob scene coming out of the toilet. They also see you and guffaw, noisily discussing something. ${((s as any).boydesc || '')} looks at them puzzled, "What's with them?" The boys shout to him, "While you were wandering around the place, your girlfriend was in the toilet sucking on a dick!" ${((s as any).boydesc || '')} comes up to them, "Are you crazy?"`);
    // TODO-QSP: dynamic text: The Boys start to tell him how you just sucked off a guy in front of them. <<$bo...
    scene.text(`The Boys start to tell him how you just sucked off a guy in front of them. ${((s as any).boydesc || '')} rushes back over to you and slaps you across the face "Get the fuck out, slut! I don't want to see you ever again!"`);
    scene.actions([
      { label: 'Leave', goto: ['pav_commcenter', ''] },
    ]);
  } },
      ...
      scene.text(`While ${((s as any).boydesc || '')} went for a beer another guy comes up to you and asks for a dance.`);
      qspCall(s, 'willpower', 'misc', 'resist', 'easy');
      if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
        scene.actions([
    ...
    scene.text(`You politely refuse his invitation. The guy looks dejected but leaves without making any fuss. A few minutes later ${((s as any).boydesc || '')} returns to the dance floor, a beer in his hand. You have a fun time dancing with your ${((s as any).bfTitle ?? 0)?.[String((s as any).numnpc ?? 0)] ?? ''} and soon after he finishes his beer he suggests to leave.`);
    scene.actions([
      { label: 'Leave',  },
    ]);
  } },
    ...
    scene.text(`You start to dance with him. A few minutes later ${((s as any).boydesc || '')} comes up to you and pulls you by the hand, dragging you away from the other guy. "What the fuck is wrong with you? I leave for a minute, and you're already dancing with someone else!" He angrily leaves the club.`);
    scene.actions([
    ...
    scene.text(`You head to the bar and ${((s as any).boydesc || '')} orders your drinks. When you finally get them you move to the outdoor area and find a secluded place to enjoy your drinks.`);
    qspCall(s, 'willpower', 'kiss', 'self');
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
    ...
    scene.text(`You tenderly embrace ${((s as any).boydesc || '')} and began to kiss his lips.`);
    if (((s as any).gnpcSex ?? 0)?.[String((s as any).numnpc ?? 0)] > 0) {
      // TODO-QSP: dynamic text: <<$boydesc>> whispered in your ear, "<<$pcs_nickname>> want to do something righ...
      scene.text(`${((s as any).boydesc || '')} whispered in your ear, "${((s as any).pcs_nickname || '')} want to do something right here?"`);
      scene.actions([
        { label: 'Jerk guy', handler: (st: GameState) => {
    if (((s as any).gnpcSex ?? 0)?.[String((s as any).numnpc ?? 0)] === 0) {
      if (!(s as any).gnpcSex) (s as any).gnpcSex = {}; (s as any).gnpcSex[String((s as any).numnpc ?? 0)] = 1;
    ...
    if (!(s as any).GboyBalabol) (s as any).GboyBalabol = {}; (s as any).GboyBalabol[String((s as any).numnpc ?? 0)] = 1;
    qspCall(s, 'stat', '');
    ...
    scene.text(`Soon ${((s as any).boydesc || '')} groaned "${((s as any).pcs_nickname || '')}, I'm going to come."`);
    scene.actions([
    ...
    scene.text(`${((s as any).boydesc || '')} pulls your ${((s as any).titsize || '')} breast out of your top and begins to knead your boobs and kiss on them. He takes your hand and puts it against his groin, and you feel that he's got a hard cock.`);
    ...
    scene.text(`You cease to resist and just relax feeling the pleasant warmth as your guy caresses your pussy. ${((s as any).boydesc || '')} begins to undress you and soon you are naked.`);
    ...
    scene.actions([
      { label: 'Leave the club',  },
          ...
          if (((s as any).diskoIvTiperand ?? 0) === 2) {
            ...
            if (((s as any).diskoIvTiperand ?? 0) === 3) {
            }
          }
        }
      }
    ...
    scene.text(`${((s as any).boydesc || '')} pulls your ${((s as any).titsize || '')} breasts outside and begins to knead your breasts and kiss them. Guy takes your hand and puts it on the groin, you feel that he's got a hard cock.`);
    qspCall(s, 'willpower', 'mast', 'resist', 'easy');
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Resist', handler: (st: GameState) => {
    ...
    scene.text(`You cease to resist and just relax feeling that pleasant warmth of a guy caressing your pussy with his fingers. ${((s as any).boydesc || '')} begins to undress you and soon you are completely naked.`);
    qspCall(s, 'willpower', 'anal', 'self');
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Substitute ass', handler: (st: GameState) => {
    ...
    scene.text(`You pretend not to notice his hand and ${((s as any).boydesc || '')} doesn't hesitate to pull your thigh closer to his. ${((s as any).boydesc || '')} tilted towards your face trying to kiss you.`);
    qspCall(s, 'willpower', 'sex', 'resist', 'easy');
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Remove his hand', handler: (st: GameState) => {
    ...
    scene.text(`${((s as any).boydesc || '')} takes you to his home. You came to a small two-story house, in which there are several apartments and ${((s as any).boydesc || '')} guides you to his apartment. He opened the door with a key and lets you inside.`);
    ...
    scene.text(`You settled comfortably on the sofa and ${((s as any).boydesc || '')} rummages in the drawers, finds a movie, and puts it in the VCR and starts it, then sits down next to you.`);
    ...
    scene.text(`You got up from the couch upset with the guy claiming that you are not interested in watching that type of trash. ${((s as any).boydesc || '')} gets embarrassed and blushing began stuttering an apology.`);
    scene.actions([
      { label: 'Go home', goto: ['pav_residential', ''] },
    ]);
  } },
    ...
    scene.text(`You continue watching the porn and what you saw was gradually exciting you. You feel ${((s as any).boydesc || '')} put his hand on your knee and began to stroke it.`);
  } },
      ]);
    } else {
      if (((s as any).boyvideorand ?? 0) > 0) {
