...
function enterSummerInviteSms(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['a2'] = ':-) Great! I\'ll see you on ' + (((s as any).weekName ?? 0)?.[parseFloat((String((((s as any).nerd_game ?? 0)?.['lot'])).slice((1)-1, ((1)-1)+(1))))] ?? 0) + ' at ' + qspFunc(s, 'time', 'get_time_string', 20, 0) + ' in the library then! Be ready to get your nerd on, because we\'re going to rock it so hard!';
  ...
  ((s as any).SMSTree = (s as any).SMSTree ?? {})['b2'] = ':-) Great! I\'ll see you on ' + (((s as any).weekName ?? 0)?.[parseFloat((String((((s as any).nerd_game ?? 0)?.['lot'])).slice((2)-1, ((2)-1)+(1))))] ?? 0) + ' at ' + qspFunc(s, 'time', 'get_time_string', 20, 0) + ' in the library then! Be ready to get your nerd on, because we\'re going to rock it so hard!';
