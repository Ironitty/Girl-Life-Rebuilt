  ...
  if (((s as any).gopnikbandQW ?? 0)?.['nushob'] === 1  &&  ((s as any).gopnikbandQW ?? 0)?.['lena_lara_present'] === 1) {
    ...
    if (((s as any).gopnikbandQW ?? 0)?.['nushob'] === 2  &&  ((s as any).gopnikbandQW ?? 0)?.['roman_present'] === 1) {
      ...
      if (((s as any).gopnikbandQW ?? 0)?.['nushob'] === 3) {
        ...
        scene.text('"Oh dude… Lav had us out partying until like, \'+func(\'time\', \'get_time_string\', 5, 0)+\'. I don\'t know what the fuck he gave us, but I think I\'m still hearing colors." The two of you talk, mostly about the drug hazed party they had last night, which she seems to not fully remember.');
        scene.text('Your conversation with Anushka starts winding down and you seem to be running out of small talk. Now seems like a good time to ask her about something specific.');
        scene.actions([
          { label: 'Ask her some questions', goto: ['praiders_garage_chat', 'chat_anushka1'] },
        ]);
...
function enterChatAnushka1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"You're looking a little tense, ${((s as any).pcs_nickname ?? 0)}," Anushka says as she eyes you up and down. She has a devious sparkle in her eyes that makes you worried about what her intentions are.`);
    ...
    scene.actions([
      { label: 'Ask him some questions', goto: ['praiders_garage_chat', 'chat_valentin1'] },
        ...
        if (((s as any).gopnikbandQW ?? 0)?.['valob'] === 4  &&  ((s as any).gopnikbandQW ?? 0)?.['roman_present'] === 1) {
          ...
          scene.text(`"Did you enjoy the music, ${((s as any).pcs_nickname ?? 0)}?" Valentin eagerly asks.`);
...
function enterChatValentin1(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).anushkaQW ?? 0)?.['rad_date'] >= 1) {
    ...
    if (((s as any).npc_rel ?? 0)?.['A158'] < 60) {
      ...
      scene.text(`He shakes his head. "Look, you seem like a nice girl ${((s as any).pcs_nickname ?? 0)}, but we're not exactly friends. I'm not going to tell a friend's story to someone I barely know. If you want to know, then ask them."`);
    ...
    // TODO-QSP: gt 'praiders_garage', $hangout_rand
      ...
      scene.text(`He sighs. "Look, it isn't really my story to tell. You seem like a cool chick and all ${((s as any).pcs_nickname ?? 0)}, but you should really ask them."`);
...
function enterChatArkadi(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).gopnikbandQW ?? 0)?.['arkob'] === 1  &&  ((s as any).gopnikbandQW ?? 0)?.['roman_present'] === 1) {
    ...
    scene.text(`"Yeah, he's my brother in arms. No one likes to fight as much as that kid. I'm telling you ${((s as any).pcs_nickname ?? 0)}, he would fit right in with the crowd I hung out with in the city. He's mean, he can fight and he can take a beating."`);
    ...
    if (((s as any).gopnikbandQW ?? 0)?.['arkob'] === 2  &&  ((s as any).gopnikbandQW ?? 0)?.['lavrenti_present'] === 1) {
      ...
      scene.text(`"Listen here, ${((s as any).pcs_nickname ?? 0)}," He grabs you by the arms. "I need that stuff to function properly. There's nothing wrong with that…" He squeezes your arm even harder.`);
...
function enterChatLenalera(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).gopnikbandQW ?? 0)?.['leleob'] === 1  &&  ((s as any).gopnikbandQW ?? 0)?.['lavrenti_present'] === 1) {
    ...
    scene.text(`"I can't wait for their next gig. The last one was a blast. You remember it, Lera?" Lena says and Lera nods, but before she can respond, Lena continues. "Pity you missed it, ${((s as any).pcs_nickname ?? 0)}," she says in a slightly smug tone.`);
...
function enterChatLavrenti(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).pcs_willpwr ?? 0) < ((s as any).will_cost ?? 0)) {
      scene.actions([
        { label: 'Convince him to talk about it [+$func(\'willpower\', \'get_willcost_string\'...]', handler: (st: GameState) => {
    st.scene = { ...st.scene, mainText: String((st as any).noWillpower || ''), curActs: [] };
  } },
      ]);
    } else {
...
function enterChatRoman(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).gopnikbandQW ?? 0)?.['roman_chat'] === 1) {
    ...
    scene.text(`Roman is ecstatic as you slide over to him. "You should've seen it, ${((s as any).pcs_nickname ?? 0)}! That was one of the greatest moments in my life!"`);
...
function enterChatRoman1(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"What's with all these questions, ${((s as any).pcs_nickname ?? 0)}? I don't know and I don't really care, okay?! So either change the subject or I'll shut you up with my cock!" He starts laughing.`);
    scene.actions([
      { label: 'Hang out', handler: (st: GameState) => {
    // TODO-QSP: gt 'praiders_garage', $hangout_rand
  } },
...
function enterChatNiko1(s: GameState, scene: SceneBuilder): void {
      ...
      if (((s as any).fedorKozlovQW ?? 0) >= 5) {
        ...
        if (((s as any).artemQW ?? 0)?.['bf'] === 1) {
          ...
          if (((s as any).rex_love ?? 0) === 1) {
            ...
            scene.text(`Niko's eyes glimmer with hope as his smile widens. "Simply considering is enough to keep me hopeful. I can take very good care of you, ${((s as any).pcs_nickname ?? 0)}. You need only place your trust in me."`);
  ...
  if (((s as any).gopnikbandQW ?? 0)?.['niko_talk_4'] === 0) {
    ...
    if (((s as any).NikoVolkovQW ?? 0) >= 5) {
      ...
      scene.text(`"There's no beauty more refined than yours, ${((s as any).pcs_nickname ?? 0)}," he seductively replies. "You have nothing to fear. My eyes are glued to you, and you alone."`);
    } else {
  ...
  if (((s as any).gopnikbandQW ?? 0)?.['niko_talk_7'] === 0  &&  ((s as any).gopnikbandQW ?? 0)?.['lena_lara_present'] === 1) {
    ...
    scene.text(`Niko makes an exaggerated emote of being stabbed in the head and falling over. "Oh, you wound me, ${((s as any).pcs_nickname ?? 0)}! But in all honesty, she seems to be obsessed with Lena, and does does everything she wants to do, even if it means fucking the guys together."`);
    ...
    if (((s as any).NikoVolkovQW ?? 0) >= 5) {
      ...
      scene.text(`He brings his face closer to yours. "Of course you are, ${((s as any).pcs_firstname ?? 0)}. There's no one more perfect than you, and no girl that I'd rather be with." He then gently grips your cheek before giving you a passionate kiss as the party around you begins to wrap up.`);
    } else {
      ...
      if (((s as any).VK ?? 0) === 2) {
        ...
        if (((s as any).VK ?? 0) === 3) {
          ...
          if (((s as any).NikoPayback ?? 0) === 0  &&  ((s as any).NikoDinerSlut ?? 0) > 0) {
            ...
            scene.text(`Niko slowly nods his head. "You're probably right. I'm just weighing our options for now. Nothing's set in stone, but trust me ${((s as any).pcs_nickname ?? 0)}, I <i>will</i> restore your reputation and make things right. I promise." He then leans over and gives you a soft kiss on the lips before handing you his beer to share.`);
          } else {
