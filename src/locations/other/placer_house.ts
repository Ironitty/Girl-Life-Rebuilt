...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).placerParameter ?? 0)?.['number_of_man'] === 1) {
    ...
    scene.text(`You and ${((s as any).ev_name || '')} both get down on your knees in front of ${((s as any).boydesc || '')} and share his cock, giving him a blowjob in turns. When you don't have his cock in your mouth, you spend the idle time by playing with his balls, licking and sucking on them while softly whispering words of encouragement to ${((s as any).ev_name || '')}, challenging her to take the cock deeper down her throat.`);
    ...
    scene.text(`When you get down on your hands and knees for him, ${((s as any).boydesc || '')} wastes no time and immediately gets behind you, ready to fuck you. Just as he slams his cock in and begins to fuck you in earnest, ${((s as any).ev_name || '')} is very turned on by your horny mewling and scoots over in front of you with her legs spread wide. Without a second thought, you immediately begin to lap up her pussy juices from her soaked snatch.`);
    ...
    scene.text(`You're not about to let her show you up! You quickly scoot down on the bed until your mouth is near his erection. The musky and manly smell of it is impossible to ignore, and actually arouses you a quite a bit. You eagerly take his cock in your mouth and suck on the head, trying to compete with ${((s as any).ev_name || '')}. However, she is one step ahead of you and already has her guy's cock all the way down her throat. The guy has his hand behind her head but realizes he doesn't need to do anything, ${((s as any).ev_name || '')} is giving him the blowjob of a lifetime! You try to smile with ${((s as any).boydesc || '')}'s cock still in your mouth… guess she wins this one. Then you decide to focus solely on making ${((s as any).boydesc || '')} feel good instead, and forget about ${((s as any).ev_name || '')} and the other guy.`);
    ...
    scene.text(`You quickly brush up a little together with ${((s as any).ev_name || '')}, and get ready to leave the apartment.`);
    scene.actions([
      { label: 'Leave', goto: ['placer_end', ''] },
    ]);
  } },
    ...
    scene.text(`You sheepishly smile at the guy as ${((s as any).ev_name || '')} takes the other two guys by the hand and leaves the room. Then you move in and kiss him. The guy quickly answers your kiss, and roughly paws at your body hoping to get you naked.`);
    ...
    if ((!((s as any).tipsexrand ?? 0))) {
      ...
      if (((s as any).tipsexrand ?? 0) === 1) {
        ...
        scene.text(`The guy whispers: "${((s as any).pcs_nickname || '')}, I want you…"`);
        scene.actions([
    ...
    scene.text(`${((s as any).ev_name || '')} comes up to you and whispers: "${((s as any).boydesc || '')} said he had something really cool to show me, in private… you'll be fine here by yourself for a while, right?", before leaving to the next room without waiting for an answer.`);
    ...
    scene.text(`Well, you are feeling great right now, and these boys are kinda cute… might as well have some fun while you wait for ${((s as any).ev_name || '')}!`);
