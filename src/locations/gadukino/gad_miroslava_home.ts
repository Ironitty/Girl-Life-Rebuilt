...
function enterMirabrother(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspGoto(st, 'gad_miroslava_home', 'mirabrother');
  } },
        ]);
      } else {
    ...
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + ((Math.floor(Math.random() * 11) + 15));
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'cum_call', 'mouth_swallow', ((st as any).boy ?? 0), 1);
    (st as any).bja = ((st as any).bja ?? 0) + (1);
    if ((!((st as any).mirabrosextime ?? 0))) {
      (st as any).mirabrosextime = 1;
