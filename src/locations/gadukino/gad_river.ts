...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).temp_rand ?? 0) === 1  &&  ((s as any).hour ?? 0) < 5  &&  ((s as any).month ?? 0) >= 5  &&  ((s as any).month ?? 0) <= 9  &&  ((s as any).MiraVars ?? 0)?.['QW'] >= 20) {
    ...
    if ((!((st as any).mesec ?? 0))) {
      ...
      qspCall(st, 'dinsex', 'vaginal_sex', 20, 'sub', 'gangbang');
      qspCall(st, 'dinsex', 'sexcum');
    } else {
      qspCall(st, 'dinSex', 'boy_wants_anal', '', 'lubri');
      qspCall(st, 'dinsex', 'analsex', 20, 'sub', 'gangbang');
    }
    ...
    qspCall(st, 'arousal', 'anal', 20, 'gangbang', 'sub');
    qspCall(st, 'arousal', 'anal', 20, 'gangbang', 'sub');
    qspCall(st, 'arousal', 'anal', 20, 'gangbang', 'sub');
...
function enterFishers(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).fishersday ?? 0) === ((s as any).daystart ?? 0)) {
    ...
    (st as any).fishersday = ((st as any).daystart ?? 0);
    (st as any).minut = ((st as any).minut ?? 0) + 3;
    if (((st as any).fishersQW ?? 0) < 3) {
      (st as any).fishersQW = ((st as any).fishersQW ?? 0) + (1);
    }
    qspCall(st, 'stat', '');
...
function enterFishing(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) < 22) {
    ...
    if (((st as any).hour ?? 0) < 12) {
      (st as any).temp_rand = (Math.floor(Math.random() * 16) + 0);
    } else {
      if (((st as any).hour ?? 0) >= 12  &&  ((st as any).hour ?? 0) < 18) {
        (st as any).temp_rand = (Math.floor(Math.random() * 21) + 0);
      } else {
        (st as any).temp_rand = (Math.floor(Math.random() * 11) + 0);
      }
    }
    qspCall(st, 'stat', '');
    ...
    if (((st as any).temp_rand ?? 0) < (Math.floor(Math.random() * 5) + 1)) {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    (st as any).temp_randB = (Math.floor(Math.random() * 121) + 0);
    if (((st as any).temp_randB ?? 0) < 50) {
      qspCall(st, 'exp_gain', 'react', 1);
    }
    qspCall(st, 'stat', '');
    if (((st as any).pcs_react ?? 0) > ((st as any).temp_randB ?? 0)) {
