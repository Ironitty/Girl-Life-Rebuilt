...
function enterCollectionPoint(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "Buying: mushrooms <<$func('money', 'string_profit', 25)>>/kg, berries <<$func('...
...
function enterMiraEvents(s: GameState, scene: SceneBuilder): void {
  if (((s as any).npc_known ?? 0)?.['A60'] === 0) {
    ...
    qspCall(s, 'arousal', 'erotic', 2, 'lesbian', 'dom');
    qspCall(s, 'stat', '');
    ...
    if (((s as any).MiraVars ?? 0)?.['Mitka_fucks'] < 50) {
      ...
      qspCall(s, 'willpower', 'exhib', 'force');
      qspCall(s, 'willpower', 'pay', 'force');
      qspCall(s, 'arousal', 'erotic', 5, 'lesbian', 'dom');
      qspCall(s, 'arousal', 'end');
      scene.actions([
        { label: 'Finish your walk', goto: ['gadukino', ''] },
      ]);
      ...
      qspCall(s, 'willpower', 'exhib', 'force');
      qspCall(s, 'willpower', 'pay', 'force');
      qspCall(s, 'arousal', 'erotic_nudity', 10, 'erotic', 5, 'lesbian', 'dom');
      qspCall(s, 'arousal', 'end');
      scene.actions([
        { label: 'Finish your walk', goto: ['gadukino', ''] },
      ]);
