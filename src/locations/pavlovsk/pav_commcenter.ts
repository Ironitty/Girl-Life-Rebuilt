...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).hour ?? 0) === 19  &&  ((s as any).minut ?? 0) >= 40  &&  ((s as any).week ?? 0) >= 5  &&  ((s as any).week ?? 0) < 7) {
    ...
    scene.actions([
      { label: 'Wait in line for the disco to open (0:<<iif(60-minut < 10, "0<<60-minut>>", 60-minut)>>) [+$func(\'money\', \'get_cost_string\', 25)]' }, // TODO-QSP: empty action body
    ]);
  }
...
export const pav_commcenter: LocationDef = {
  ...
  description: ['The entrance to the community center is full of young people. The guys are hanging around smoking and joking while the girls are in small groups, all dressed up and gossiping. You see the bushes moving - Pavlovsk\'s famous make out garden.'],
  enter: enter,
};
