...
function enterOldmanHouse(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (!(s as any).npc_dick) (s as any).npc_dick = {}; (s as any).npc_dick[String((s as any).npclastgenerated ?? 0)] = 2;
...
function enterEatlera(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).park ?? 0)?.['girl_eatlera'] >= 1) {
    ...
    scene.text(`A few minutes later, she lets out a cry and shudders against you, then lets go of your head and steps back. "Fuck, that was good. You're great at eating pussy, ${((s as any).pcs_nickname ?? 0)}." As she puts her pants back on, she bites her lip and gives you a once over.`);
    ...
    scene.text(`"Look at her. She never cared about the beer, she just loves pussy. She looks like she's about to cum herself," Alyona comments as she lays back with her pants pulled off and her panties pulled aside. "Come on over and give my kitty a tongue bath, ${((s as any).pcs_nickname ?? 0)}."`);
    ...
    scene.text(`Lena glances over at Alyona. "Maybe we should make this a regular thing? ${((s as any).pcs_nickname ?? 0)} seems to love it." She then turns her head to look at you. "Well? come on, you've got one left."`);
...
function enterPressuredForBra(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You're really starting to get wasted as Dan speaks up. "Hey, ${((s as any).pcs_nickname ?? 0)}, what colour is your bra?"`);
...
function enterPressuredForPanties(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You're really starting to get wasted as Dan speaks up. "${((s as any).pcs_nickname ?? 0)}, what colour are your panties?"`);
  scene.text('You look at him, a little confused. "What?"');
...
function enterPressuredForTits(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You're really starting to get wasted now. As you do, Dan speaks up. "Hey, ${((s as any).pcs_nickname ?? 0)}, show us your tits."`);
  scene.text('You look at him, a little confused. "What?"');
...
function enterPressuredForPussy(s: GameState, scene: SceneBuilder): void {
  ...
  scene.img('images/locations/pavlovsk/park/event/gopnik/fall_strip1.jpg');
  // TODO-QSP: dynamic text: You're really starting to get wasted as Dan speaks up. "Hey, <<$pcs_nickname>>, ...
  scene.text(`You're really starting to get wasted as Dan speaks up. "Hey, ${((s as any).pcs_nickname ?? 0)}, show us your pussy."`);
  scene.text('You look at him, a little confused. "What?"');
...
function enterPressuredForGangbang(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`Once they're all finished, you hear Pauline speaking. "Give us a smile. This is going to make you famous. Just wait until all our classmates see the adventures of ${((s as any).pcs_nickname ?? 0)} the slut."`);
...
function enterRadomirWalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`You suddenly feel his hand on the back of your head and the next thing you know, he forces your head down. His ${((s as any).dick ?? 0)}cm ${((s as any).dick_girth ?? 0)} dick slides down your throat, gagging you. From that moment on, he keeps his hand in place for the next few minutes, forcing your head all the way down. "Ah fuck yeah… Now that's how you suck cock!"`);
...
function enterValNushWalk(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`"OH… fuck Nush… Okay, I can't wait anymore!" He points over at the rock he was sitting on. "Sit there, ${((s as any).pcs_nickname ?? 0)}."`);
...
function enterFuckedByVal(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    scene.text(`He stops licking you just when you're about to tell him to fuck you and glance back to see him getting in place behind you. Moments later, you feel his ${((s as any).dick ?? 0)}cm ${((s as any).dick_girth ?? 0)} dick sliding into your wet slit, filling you up as you moan loudly.`);
...
function enterFuckedAnalByVal(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`He stops for a moment to give you a chance to relax before he slowly starts sliding his ${((s as any).dick ?? 0)}cm ${((s as any).dick_girth ?? 0)} cock up your ass, filling you as it stretches you out.`);
