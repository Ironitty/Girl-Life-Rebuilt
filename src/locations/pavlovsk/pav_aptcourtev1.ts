...
function enterFollownush(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).spellKnown ?? 0)?.['penisenvy'] === 1  &&  ((s as any).PCLoSkirt ?? 0) > 0) {
      scene.actions([
...
function enterSuck3stooges(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    qspCall(s, 'arousal', 'bj', 5, ((s as any).npcID1 ?? 0), 'group', 'exhibitionism');
    qspCall(s, 'arousal', 'bj', 5, ((s as any).npcID2 ?? 0), 'group', 'exhibitionism');
    qspCall(s, 'arousal', 'hj', (-7), ((s as any).npcID ?? 0), 'group', 'exhibitionism');
    qspCall(s, 'arousal', 'hj', (-7), ((s as any).npcID1 ?? 0), 'group', 'exhibitionism');
    qspCall(s, 'arousal', 'hj', (-7), ((s as any).npcID2 ?? 0), 'group', 'exhibitionism');
    qspCall(s, 'stat', '');
    scene.actions([
