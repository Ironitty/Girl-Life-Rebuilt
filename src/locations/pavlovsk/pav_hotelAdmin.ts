...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).job_hiring_step ?? 0)?.['pav_hotel_maid'] >= 3  &&  ((s as any).job_status ?? 0)?.['pav_hotel_maid'] !== 'employed') {
    ...
    scene.text(`"${((s as any).pcs_nickname ?? 0)}! I didn't expect to see you back here. What can I do for you?"`);
    ...
    scene.text(`"Same terms as before: working between '+func('time', 'get_time_string', 16, 0)+' and '+func('time', 'get_time_string', 19, 0)+', give or take, ${qspFunc(s, 'money', 'string_profit', 375)} per shift. Sound good?"`);
    scene.actions([
    ...
    scene.text('Herman nods. "Your uniform should still be in the locker room. Come by between \'+func(\'time\', \'get_time_string\', 16, 0)+\' and \'+func(\'time\', \'get_time_string\', 17, 0)+\' whenever you\'re ready to start a shift."');
    scene.actions([
    ...
    scene.text('You can feel his eyes exploring your body for a moment before he responds. "Yes, we are! We currently only have a part-time vacancy for a maid. You would be working between \'+func(\'time\', \'get_time_string\', 16, 0)+\' and \'+func(\'time\', \'get_time_string\', 19, 0)+\', give or take. Most of the hotel rooms are empty then, before the new guests come in."');
    ...
    scene.text('He continues. "If you want to work a shift, you need to be here between \'+func(\'time\', \'get_time_string\', 16, 0)+\' and \'+func(\'time\', \'get_time_string\', 17, 0)+\'. There\'s no point in you getting changed for just 20 minutes of work."');
