...
function enterInviteChat(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_usedname ?? 0)?.[String((s as any).npcID ?? 0)] !== ((s as any).npc_nickname ?? 0)?.[String((s as any).npcID ?? 0)]) {
    ...
    scene.text(`"I'm ${((s as any).pcs_firstname || '')}," you smile back.`);
    qspGoto(s, 'pav_disco_outside', 'chat_menu');
  } },
    ...
    scene.text(`"I'm ${((s as any).pcs_nickname || '')}," you smile back.`);
    qspGoto(s, 'pav_disco_outside', 'chat_menu');
  } },
    ]);
  } else {
    ...
    if (((s as any).npc_finance ?? 0)?.[String((s as any).npcID ?? 0)] === 2) {
      scene.actions([
        { label: '', labelFn: (s: GameState) => 'Go back to ' + String(((s as any).npcdesc || '') ?? '') + '\'s', goto: ['sex_ev_start', 'hotel_start'] },
      ]);
    } else {
      scene.actions([
        { label: '', labelFn: (s: GameState) => 'Go back to ' + String(((s as any).npcdesc || '') ?? '') + '\'s', goto: ['sex_ev_start', 'npc_home_start'] },
      ]);
    }
  } },
        ]);
...
function enterChatMenu(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_usedname ?? 0)?.[String((s as any).npcID ?? 0)] !== ((s as any).npc_nickname ?? 0)?.[String((s as any).npcID ?? 0)]) {
    ...
    scene.text(`"I'm ${((s as any).npc_firstname || '')}," he tells you.`);
    qspGoto(s, 'pav_disco_outside', 'chat_menu');
  } },
    ]);
  }
...
function enterChatEnd(s: GameState, scene: SceneBuilder): void {
  ...
  if ((Array.isArray((s as any).lover) ? ((s as any).lover as any[]).indexOf(((s as any).npcID ?? 0)) : -1) < 0) {
    ...
    scene.text(`You leave ${((s as any).npcdesc || '')} in the dark and head back inside.`);
    scene.actions([
    ...
    qspGoto(s, 'pav_disco_outside', 'exit_options');
  } },
    ...
    scene.text(`"I wouldn't mind seeing you again, if you want my number?" you offer ${((s as any).npcdesc || '')}.`);
...
function enterMoveMenu(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).temp ?? 0)?.['curr_action'] !== 'kissing') {
      ((s as any).temp = (s as any).temp ?? {})['curr_action'] = 'kissing';
      ...
      if (((s as any).npc_rel_goal ?? 0)?.[String((s as any).npcID ?? 0)] === 'sex') {
        ...
        scene.text(`${((s as any).npcdesc || '')} stiffens beneath you in momentary surprise but quickly recovers, responding to your kiss with equal enthusiasm.`);
      }
    } else {
      // TODO-QSP: dynamic text: You keep making out with <<$npcdesc>>.
      scene.text(`You keep making out with ${((s as any).npcdesc || '')}.`);
    }
    qspGoto(s, 'pav_disco_outside', 'move_menu');
  } },
    ...
    if (((s as any).temp ?? 0)?.['curr_action'] === 'kissing') {
      ...
      scene.text(`Reach down into ${((s as any).npcdesc || '')}'s pants, you feel a stiff, ${((s as any).npc_dick_desc ?? 0)?.[String((s as any).npcID ?? 0)] ?? ''} shaft inside. You pull, releasing it from its confines and stroking it to diamond hardness without ever breaking the kiss.`);
    } else {
      // TODO-QSP: dynamic text: You reach down into <<$npcdesc>>'s pants, feeling a stiff, <<$npc_dick_desc[$npc...
      scene.text(`You reach down into ${((s as any).npcdesc || '')}'s pants, feeling a stiff, ${((s as any).npc_dick_desc ?? 0)?.[String((s as any).npcID ?? 0)] ?? ''} shaft inside. You pull, releasing it from its confines and stroking it to diamond hardness`);
    }
...
function enterToomuchExit(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_shy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
    ...
    scene.text(`"Yeah," ${((s as any).npcdesc || '')} nods emphatically.`);
    scene.actions([
    ...
    scene.text(`"Fucking typical cocktease," ${((s as any).npcdesc || '')} grumbles as he pulls his pants up. "Blueball bitch. Get the fuck out of here."`);
    ...
    scene.text(`"That might be better," you say and ${((s as any).npcdesc || '')} grins back at you.`);
    scene.text('"Come on then. Let\'s go."');
    if (((s as any).npc_residence ?? 0)?.[String((s as any).npcID ?? 0)] === 'pav_residential') {
      scene.actions([
        { label: '', labelFn: (s: GameState) => 'Go back to ' + String(((s as any).npcdesc || '') ?? '') + '\'s place', handler: (st: GameState) => {
...
function enterCallitanight(s: GameState, scene: SceneBuilder): void {
  ...
  if ((Math.floor(Math.random() * 3) + 1) + ((s as any).npc_assertive ?? 0)?.[String((s as any).npcID ?? 0)] > 1) {
    ...
    scene.text(`"I hope I'll see you again soon, ${((s as any).pcs_firstname || '')}," he smiles back as you head back to the disco.`);
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterExitOptions(s, scene); (s as any).locArgs = __savedLocArgs; }
  } },
      ]);
    }
...
function enterPcAsknumber(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_rel_goal ?? 0)?.[String((s as any).npcID ?? 0)] === 'sex') {
    ...
    scene.text(`"No...?" you reply, bewildered. ${((s as any).npcdesc || '')} shrugs.`);
...
function enterPcAskcoffee(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).npc_residence ?? 0)?.[String((s as any).npcID ?? 0)] === 'pav_residential') {
    ...
    qspGoto(s, 'date_hangout', 'start', 'disco');
  } },
      ]);
    }
  } else {
...
function enterBoysexOptions(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).npc_gentle_lover ?? 0)?.[String((s as any).npcID ?? 0)] + ((s as any).npc_sensual_lover ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
      qspGoto(s, 'pav_disco_outside', 'boysex_titslip');
    } else {
      qspGoto(s, 'pav_disco_outside', 'boysex_handjob');
    }
  } },
  ]);
  scene.build();
    ...
    if (((s as any).npc_fav_pos ?? 0)?.[String((s as any).npcID ?? 0)] === 'blowjob') {
      if (((s as any).temp ?? 0)?.['curr_action'] === 'kissing') {
        ...
        if (((s as any).temp ?? 0)?.['curr_action'] === 'titsuck') {
          ...
          // TODO-QSP: dynamic text: You hear the sound of <<$npcdesc>>'s buckle coming loose. As you glance downward...
...
function enterBlowjobPre(s: GameState, scene: SceneBuilder): void {
  if (((s as any).home ?? 0)?.['current'] === 'mey_home') {
    ...
    scene.text(`"Wait," you stop suddenly, looking around. "I don't want to do this here. Would you wanna come back to my place instead?" you ask. With a sultry smirk, you lean in to whisper in ${((s as any).npcdesc || '')}'s ear.`);
    scene.text('"<i>My parents aren\'t home tonight...</i>"');
    scene.text('The look in his eyes is all you need to know and the two of you rush back to the estate together.');
    scene.actions([
      { label: 'Continue', goto: ['sex_ev_start', 'pc_home_start', 'hookup'] },
  ...
  scene.actions([
    ...
    { const __savedLocArgs = (s as any).locArgs; (s as any).locArgs = ['', ]; enterOutsideImg(s, scene); (s as any).locArgs = __savedLocArgs; }
    // TODO-QSP: dynamic text: "Woah!" you yelp, stepping back to get out from under <<$npcdesc>>'s hand. "What...
    scene.text(`"Woah!" you yelp, stepping back to get out from under ${((s as any).npcdesc || '')}'s hand. "What do you think you're doing?!"`);
    if (((s as any).npc_shy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
      ...
      if (((s as any).npc_pushy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
        ...
        scene.text(`"Er..." ${((s as any).npcdesc || '')} seems a slightly embarrassed. "Sorry, I just thought things were going well and that you would be fine with giving me some head...?"`);
      }
    }
    scene.actions([
...
function enterBendOver(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((s as any).npc_selfish ?? 0)?.[String((s as any).npcID ?? 0)] === 1  ||  ((s as any).pav_disco_bj ?? 0)?.[String((s as any).npcID ?? 0)] > ((s as any).totminut ?? 0) - 20) {
...
function enterSlutInvite(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    ((s as any).temp = (s as any).temp ?? {})['prostitution_offer'] = 500;
    if (((s as any).npc_prostitution_count ?? 0)?.[String((s as any).npcID ?? 0)] >= 5) {
      if (((s as any).npc_assertive ?? 0)?.[String((s as any).npcID ?? 0)] + ((s as any).npc_pushy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
        ...
        scene.text(`"You let me fuck you for money a whole lot more than 'one' time," ${((s as any).npcdesc || '')} grins, stepping closer.`);
      } else {
        if (((s as any).npc_shy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
          // TODO-QSP: dynamic text: "Well, I'm pretty sure it was a lot more than one time," <<$npcdesc>> stammers.
          scene.text(`"Well, I'm pretty sure it was a lot more than one time," ${((s as any).npcdesc || '')} stammers.`);
        } else {
          // TODO-QSP: dynamic text: "Come on, who are you kidding? It's been a lot more than 'one time', <<$pcs_nick...
          scene.text(`"Come on, who are you kidding? It's been a lot more than 'one time', ${((s as any).pcs_nickname || '')}."`);
        }
      }
    } else {
      if (((s as any).fame ?? 0)?.['pav_slut'] >= 200) {
        if (((s as any).npc_assertive ?? 0)?.[String((s as any).npcID ?? 0)] + ((s as any).npc_pushy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
          // TODO-QSP: dynamic text: "Please, don't try to pretend," <<$npcdesc>> says, stepping closer. "Your name i...
          scene.text(`"Please, don't try to pretend," ${((s as any).npcdesc || '')} says, stepping closer. "Your name is all over town. I have it on good authority from friends you've fucked."`);
        } else {
          if (((s as any).npc_shy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
            // TODO-QSP: dynamic text: "Er, I kinda think you are," <<$npcdesc>> mumbles. "I mean, I know a lot of peop...
            scene.text(`"Er, I kinda think you are," ${((s as any).npcdesc || '')} mumbles. "I mean, I know a lot of people who told me specifically how you let them fuck you for money..."`);
          } else {
        ...
        if (((s as any).npc_assertive ?? 0)?.[String((s as any).npcID ?? 0)] + ((s as any).npc_pushy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
          ...
          scene.text(`"If you say so," ${((s as any).npcdesc || '')} grins, stepping closer. "But we can fix that."`);
        } else {
          if (((s as any).npc_shy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
            // TODO-QSP: dynamic text: "Oh! Uhm..." <<$npcdesc>> turns bright red. "Well... even so I really need to, u...
            scene.text(`"Oh! Uhm..." ${((s as any).npcdesc || '')} turns bright red. "Well... even so I really need to, uhh, y'know. I could pay you anyways...?"`);
          } else {
            // TODO-QSP: dynamic text: "Huh," <<$npcdesc>> blinks. "Well, I have some money you could earn regardless.
            scene.text(`"Huh," ${((s as any).npcdesc || '')} blinks. "Well, I have some money you could earn regardless.`);
          }
        }
      }
    }
    // TODO-QSP: dynamic text: He holds out <<$func('money', 'string_profit', temp['prostitution_offer'])>> in ...
    scene.text(`He holds out ${qspFunc(s, 'money', 'string_profit', ((s as any).temp ?? 0)?.['prostitution_offer'] ?? '')} in notes.`);
    ...
    qspGoto(s, 'pav_disco_outside', 'slut_accept_route', 'paid');
  } },
    ...
    if (((s as any).npc_assertive ?? 0)?.[String((s as any).npcID ?? 0)] + ((s as any).npc_pushy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
      ...
      scene.text(`"But-!" ${((s as any).npcdesc || '')} starts, before you suddenly step close to him.`);
    } else {
      if (((s as any).npc_shy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
        // TODO-QSP: dynamic text: "But-!" <<$npcdesc>> squeaks, looking extremely embarrassed before you suddenly ...
        scene.text(`"But-!" ${((s as any).npcdesc || '')} squeaks, looking extremely embarrassed before you suddenly step close to him.`);
      } else {
        // TODO-QSP: dynamic text: "But-" <<$npcdesc>> starts, looking confused as you suddenly step close to him.
        scene.text(`"But-" ${((s as any).npcdesc || '')} starts, looking confused as you suddenly step close to him.`);
      }
    }
    ...
    qspGoto(s, 'pav_disco_outside', 'slut_accept_route');
  } },
    ]);
  } },
  ]);
  ...
  if (((s as any).npc_fav_pos ?? 0)?.[String((s as any).npcID ?? 0)] === 'blowjob') {
    if (((s as any).npc_assertive ?? 0)?.[String((s as any).npcID ?? 0)] + ((s as any).npc_pushy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
      ...
      if (((s as any).npc_shy ?? 0)?.[String((s as any).npcID ?? 0)] > 0) {
        ...
        // TODO-QSP: dynamic text: "I wouldn't say no to a blowjob right now," <<$npcdesc>> says, unzipping his tro...
