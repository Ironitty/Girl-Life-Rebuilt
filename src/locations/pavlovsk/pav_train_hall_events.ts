...
function enterManager(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: '"Cleaner wanted! 2 hours per day at any time, salary '+$func('money', 'string_p...
  ...
  // TODO-QSP: dynamic text: '"Office visiting hours: '+func('time', 'get_time_string', 8, 0)+' - '+func('tim...
  ...
  if (((s as any).hour ?? 0) >= 8  &&  ((s as any).hour ?? 0) <= 16) {
    ...
    qspCall(st, 'jobs', 'get_job_definition', 'pav_train_cleaner');
    if (((st as any).job_status ?? 0)?.['pav_train_cleaner'] === '') {
      scene.actions([
        { label: 'Apply for the cleaning job', handler: (st: GameState) => {
    qspCall(st, 'jobs', 'set_employed', 'pav_train_cleaner');
...
function enterVera(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).verakassir ?? 0) === 3  &&  ((s as any).hour ?? 0) >= 8  &&  ((s as any).hour ?? 0) <= 20  &&  (!((s as any).verarand ?? 0))) {
    ...
    if (((st as any).npc_rel ?? 0)?.['A27'] > 0) {
      ...
      (st as any).veratalkrand = (Math.floor(Math.random() * 4) + 0);
      if ((!((st as any).veratalkrand ?? 0))) {
...
{ label: 'Suck up to Vera', handler: (st: GameState) => {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
      ...
      if (((st as any).npc_rel ?? 0)?.['A27'] >= 40) {
        if (((st as any).veratalkrand ?? 0) === 1) {
          ...
          if (((st as any).npc_rel ?? 0)?.['A27'] >= 50) {
            scene.actions([
              { label: 'Ask for details', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (10);
    qspCall(st, 'stat', '');
    if (((st as any).veraendtalker ?? 0) === 1) {
      (st as any).veraboyrand = ((st as any).veraboyrand ?? 0) + (1);
      if (((st as any).veraboyrand ?? 0) >= 9) {
        (st as any).veraendtalker = 2;
      }
    } else {
      if ((!((st as any).veraendtalker ?? 0))) {
        (st as any).veraendtalker = 1;
        (st as any).veraboyrand = 0;
      } else {
        if (((st as any).veraendtalker ?? 0) === 2) {
          (st as any).veraboyrand = (Math.floor(Math.random() * 9) + 0);
        }
      }
    }
    if (((st as any).veraboyrand ?? 0) > 8) {
      scene.img('images/characters/pavlovsk/resident/vera/verasmoke.jpg');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
              ...
              if (((st as any).veraboyrand ?? 0) === 5) {
                ...
                // TODO-QSP: dynamic text: '"They continued fucking me throughout that whole night. It was '+func('time', '...
    ...
    if (((s as any).verakassir ?? 0) === 2  &&  ((s as any).hour ?? 0) >= 8  &&  ((s as any).hour ?? 0) <= 20  &&  (!((s as any).verarand ?? 0))) {
      ...
      scene.actions([
        { label: 'Chat with Vera', handler: (st: GameState) => {
    (st as any).verakassir = 3;
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    qspCall(st, 'npc_relationship', 'modify', 'A27', 10);
    qspCall(st, 'stat', '');
    scene.img('images/characters/pavlovsk/resident/vera/verasmoke.jpg');
    ...
    // TODO-QSP: dynamic text: 'She nods, fully understanding your problem before her eyes suddenly light up. "...
...
function enterToiletEvent(s: GameState, scene: SceneBuilder): void {
  ...
  if ((!((s as any).startevtoirand ?? 0))) {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    (st as any).seepeenach = ((st as any).seepeenach ?? 0) + (1);
    (st as any).tot_seepee = ((st as any).tot_seepee ?? 0) + (1);
    if (((st as any).tot_seepee ?? 0) > 5) {
        ...
        if (((s as any).vokmantoitipev ?? 0) === 2) {
          ...
          if (((s as any).pcs_horny ?? 0) >= 50) {
            scene.actions([
              { label: 'Try to sneak a peek', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    (st as any).seepeecop = ((st as any).seepeecop ?? 0) + (1);
    (st as any).tot_seepee = ((st as any).tot_seepee ?? 0) + (1);
    if (((st as any).tot_seepee ?? 0) > 5) {
      (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (5);
    }
          ...
          if (((s as any).vokmantoitipev ?? 0) === 3) {
            ...
            if (((s as any).pcs_horny ?? 0) >= 50) {
              scene.actions([
                { label: 'Try to sneak a peek', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 1;
    (st as any).seepeeman = ((st as any).seepeeman ?? 0) + (1);
    (st as any).tot_seepee = ((st as any).tot_seepee ?? 0) + (1);
    if (((st as any).tot_seepee ?? 0) > 5) {
      (st as any).pcs_horny = ((st as any).pcs_horny ?? 0) + (5);
    }
...
function enterTrinkets(s: GameState, scene: SceneBuilder): void {
  ...
  scene.actions([
    ...
    if (((st as any).pcs_intel ?? 0) + ((st as any).pcs_prcptn ?? 0) > 80  &&  (Math.floor(Math.random() * 6) + 0) === 5) {
...
{ label: 'Leave', handler: (st: GameState) => {
    ...
    if (qspFunc(s, 'money', 'can_afford', 500, 'cash') === 0) {
      ...
      // TODO-QSP: dynamic text: 'You look around and, when you''re sure the coast is clear, stick out your hand ...
    ...
    if ((!((st as any).katalkinSexOnce ?? 0))) {
    ...
    ((st as any).mc_inventory = (st as any).mc_inventory ?? {})['trinkets_home'] = 0;
