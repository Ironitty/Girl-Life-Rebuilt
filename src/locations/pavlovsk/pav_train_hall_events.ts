...
function enterManager(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('"Office visiting hours: \'+func(\'time\', \'get_time_string\', 8, 0)+\' - \'+func(\'time\', \'get_time_string\', 17, 0)+\'"');
  ...
  if (((s as any).verakassir ?? 0) === 3  &&  ((s as any).hour ?? 0) >= 8  &&  ((s as any).hour ?? 0) <= 20  &&  (!((s as any).verarand ?? 0))) {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
      ...
      if (((s as any).veraboyrand ?? 0) <0) {
        ...
        scene.text(`Vera lets out a wistful sigh and shrugs. "I don't know, ${((s as any).pcs_nickname ?? 0)}. I don't feel like talking about the past right now."`);
        scene.actions([
          { label: 'Leave', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 1;
  }, goto: ['pav_train_hall', ''] },
    ...
    scene.text(`Vera just gives you a disappointed look before she grins. "Oh dear… ${((s as any).pcs_nickname ?? 0)}, you're such a prude!"`);
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 1;
  }, goto: ['pav_train_hall', ''] },
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
              ...
              if (((s as any).veraboyrand ?? 0) === 5) {
                ...
                scene.text('"They continued fucking me throughout that whole night. It was \'+func(\'time\', \'get_time_string\', 4, 0)+\' by the time they were spent. Leszek liked it so much that he kept inviting his friend over from then on. The following years Leszek would invite him more and more often, and they\'d both fuck me any way they pleased. Towards the end, they were spitroasting and double penetrating me every day."');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
                    ...
                    if (((s as any).veraboyrand ?? 0) === 8) {
                      ...
                      scene.text(`"I figured I would only do it for a little while, earn some money to find my own place, get a good job and stop there. That's not what happened. I ended up in an illegal brothel where the owner took my passport and wouldn't even let me out to buy cigarettes. Several people would come to fuck me every day. Saturdays were the worst. He'd rent me out to a group of criminals regularly who would fuck me all night and stick beer bottles in my pussy and anus by the end of the night, when they no longer wanted me. It was hell, ${((s as any).pcs_nickname ?? 0)}!"`);
    ...
    scene.text(`She hangs her head in shame for a moment, but then rebounds. "I was young and naive, ${((s as any).pcs_nickname ?? 0)}! A stupid girl who thought her pussy could conquer the world. It was foolish of me, but I've paid my dues and ended up just fine. Things could've been a lot worse."`);
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 1;
  }, goto: ['pav_train_hall', ''] },
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
      ...
      if (((s as any).npc_rel ?? 0)?.['A27'] <= 0) {
        ...
        scene.text(`"Leave me alone, ${((s as any).pcs_nickname ?? 0)}! I don't want to talk to you."`);
        scene.actions([
    ...
    if (((s as any).npc_rel ?? 0)?.['A27'] <= 0) {
      ...
      if (((s as any).npc_rel ?? 0)?.['A27'] > 0) {
        ...
        scene.text(`Vera nods. "Thanks ${((s as any).pcs_nickname ?? 0)}, I appreciate the apology. I guess I should forgive you."`);
      }
    }
    scene.actions([
      { label: 'Leave', handler: (st: GameState) => {
    ...
    scene.text('She nods, fully understanding your problem before her eyes suddenly light up. "Oh! You know how there are long-distance trains arriving at the station at \'+func(\'time\', \'get_time_string\', 11, 0)+\' and \'+func(\'time\', \'get_time_string\', 18, 0)+\' every day? The station is always packed with tourists then. I bet you could make some good money selling trinkets and baubles to them!"');
    ...
    scene.text(`"Thanks for the tip, maybe I will!" you reply. "By the way, my name is ${((s as any).pcs_nickname ?? 0)}."`);
...
function enterTrinkets(s: GameState, scene: SceneBuilder): void {
  ...
  if (!(s as any).mc_inventory) (s as any).mc_inventory = {}; (s as any).mc_inventory['trinkets_home'] = ((s as any).mc_inventory['trinkets_home'] ?? 0) - (((s as any).tovarand ?? 0));
  ...
  scene.actions([
    ...
    scene.actions([
      { label: 'Bribe them [+$func(\'money\', \'get_cost_string\', 500, \'...]', handler: (st: GameState) => {
    if (qspFunc(s, 'money', 'can_afford', 500, 'cash') === 0) {
      ...
      scene.text('You look around and, when you\'re sure the coast is clear, stick out your hand with a \' + $func(\'money\', \'string_price\', 500) + \' note in it.');
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
        ...
        if (((s as any).katalkinSexDay ?? 0) + 7 < ((s as any).daystart ?? 0)) {
          ...
          scene.text(`Captain Katalkin suddenly recognizes you and frowns. "${((s as any).pcs_nickname ?? 0)}? You haven't come to see me in a long time… I thought we had an agreement. Are you avoiding me?"`);
          scene.actions([
    ...
    if (!(s as any).mc_inventory) (s as any).mc_inventory = {}; (s as any).mc_inventory['trinkets_home'] = 0;
