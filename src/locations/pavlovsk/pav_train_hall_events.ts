...
function enterManager(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('"Cleaner wanted! 2 hours per day at any time, salary ' + qspFunc(s, 'money', 'string_profit', 250) + ' Ask the station manager for more information."');
...
{ label: 'Suck up to Vera', handler: (st: GameState) => {
    ...
    (st as any).minut = ((st as any).minut ?? 0) + 1;
      ...
      (st as any).veratalkrand = undefined;
      scene.actions([
        { label: 'Leave', handler: (st: GameState) => {
    (st as any).minut = ((st as any).minut ?? 0) + 1;
  }, goto: ['pav_train_hall', ''] },
...
function enterTrinkets(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You manage to sell ${((s as any).tovarand ?? '')} trinkets to passengers today, earning ` + qspFunc(s, 'money', 'string_profit', ((s as any).tovpay ?? '')) + '.');
  scene.actions([
    { label: 'Leave', handler: (st: GameState) => {
    qspCall(st, 'stat', '');
