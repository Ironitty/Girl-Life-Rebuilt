...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('While the commercial hub of Pavlovsk is better maintained and a bit cleaner looking than other areas, it is still very much rural like the rest of the town. Even from here, you can easily access the old <a href="exec:view\'images/locations/pavlovsk/palace/pav_palace.jpg\'">Imperial Palace</a> built by Catherine the Great, and the remodeled <a href="exec:view\'images/locations/pavlovsk/fortress/bip_hotel.jpg\'">Bip fortress</a>, which now operates as a luxury hotel and restaurant.');
  // TODO-QSP: dynamic text: A short distance away are the golden domes of the <a href="exec:minut += 10 & gt...
  scene.text('A short distance away are the golden domes of the <a href="exec:minut += 10 & gt \'pav_church\',\'start\'">Church of St. Nikolas</a>. It\'s only a 10 minute walk to get there from here.');
  // TODO-QSP: dynamic text: The nearby residential area is next to the lake, while the old palace grounds ar...
  scene.text('The nearby residential area is next to the lake, while the old palace grounds are now a large <a href="exec:minut += 5 & gt \'pav_park\', \'start\'">public park</a>.');
  ...
  scene.text('<br>Centered along the main street is the <a href="exec:minut += 10 & gt \'pav_market\'">Market square</a> with the <a href="exec:minut += 10 & gt \'pav_train_hall\'">Train station</a> that is always open and running. For those with a car, the end of the street leads to a <a href="exec:minut += 5 & gt \'fuelstation\', \'start\'">gas station</a>, which is usually the last stop people make before heading out onto the nearby highway.');
  ...
  if (((s as any).leonidSlave ?? 0) === 1) {
    ...
    scene.text('<br>Near the town hall is an old <a href="exec:minut += 15 & gt \'pav_commercial_offices\'">office building</a>.');
  }
