...
function enterRules(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('2) All residents must return to the block by \'+func(\'time\', \'get_time_string\', 21, 0)+\'. No exceptions. If you are late, you will have to report to the duty warden.');
  ...
  scene.text('4) Breakfast is from \'+func(\'time\', \'get_time_string\', 6, 0)+\' to \'+func(\'time\', \'get_time_string\', 7, 0)+\'.');
...
function enterMakeUp(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).pcs_makupskl ?? 0) <= 30) {
    ((s as any).mc_inventory ?? {})['cosmetics'] = (((s as any).mc_inventory ?? {})['cosmetics'] ?? 0) - (9);
    (s as any).pcs_makeup = 0;
  } else {
    if (((s as any).pcs_makupskl ?? 0) <= 69) {
      ((s as any).mc_inventory ?? {})['cosmetics'] = (((s as any).mc_inventory ?? {})['cosmetics'] ?? 0) - (5);
...
function enterWarden(s: GameState, scene: SceneBuilder): void {
  if ((((s as any).hour ?? 0) > 20  ||  ((s as any).hour ?? 0) < 6)  &&  ((s as any).ballet_awol ?? 0) === 1) {
    if (((s as any).school_curfew ?? 0) > 0) {
      ((s as any).ballet_grade_discipline ?? {})[String((s as any).week ?? 0)] = (((s as any).ballet_grade_discipline ?? {})[String((s as any).week ?? 0)] ?? 0) - (2);
    }
    ...
    scene.text(`"${((s as any).pcs_firstname ?? 0)} ${((s as any).pcs_lastname ?? 0)}, can I be let in please?", there's a few seconds pause.`);
    // TODO-QSP: dynamic text: "<<$pcs_firstname>> <<$pcs_lastname>>, you will see me in my office. Now." You c...
    scene.text(`"${((s as any).pcs_firstname ?? 0)} ${((s as any).pcs_lastname ?? 0)}, you will see me in my office. Now." You can swear the temperature around you just plummeted a few degrees and the door was buzzed open.`);
    ...
    if ((((s as any).hour ?? 0) >= 17  ||  ((s as any).hour ?? 0) < 21 )  &&  ((s as any).ballet_awol ?? 0) === 1) {
      ...
      // TODO-QSP: dynamic text: "<<$pcs_firstname>> <<$pcs_lastname>>, can I be let in please?", there's a few s...
      scene.text(`"${((s as any).pcs_firstname ?? 0)} ${((s as any).pcs_lastname ?? 0)}, can I be let in please?", there's a few seconds pause and you hear the door being buzzed open.`);
      ...
      if (((s as any).ballet_awol ?? 0) === 1) {
        ...
        scene.text(`"${((s as any).pcs_firstname ?? 0)} ${((s as any).pcs_lastname ?? 0)}, where the hell have you been? Get in here. Now.", you gulp at her voice and with trepidation make your way to her office.`);
        scene.actions([{ label: 'Continue', goto: ['pushkin_ballet_res', 'warden_room'] }]);
      } else {
