...
function enterStart(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).setloc = (s as any).setloc ?? {})['imagepath'] = 'images/' + 'locations/pushkin/ballet_school/';
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Terpsichore Wing';
  scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/hall.jpg');
  ((s as any).balletEv = (s as any).balletEv ?? {})['trigger'] = '';
  ...
  if ((((s as any).month ?? 0) === 7  &&  ((s as any).day ?? 0) === 9)  &&  (((s as any).hour ?? 0) === 6  ||  ((s as any).hour ?? 0) === 7)) {
    ...
    ((st as any).AlbinaQW = (st as any).AlbinaQW ?? {})['StarletsJoined'] = 0;
    qspGoto(st, 'pav_commcenter', '');
  } },
      ]);
    } else {
      if (((s as any).balletqw ?? 0)?.['school'] !== 1  &&  ((s as any).ballet_day ?? 0) === 1) {
        // TODO-QSP: dynamic text: You are chaperoned to your school by <<$npc_firstname[''A284'']>>. After a short...
    ...
    dynamicGoto(st, 'prevLoc', 'pilates');
  } },
        ]);
      } else {
        if (((s as any).balletqw ?? 0)?.['school'] !== 1  &&  ((s as any).ballet_day ?? 0) > 1) {
    ...
    dynamicGoto(st, 'prevLoc', 'pilates');
          ...
          if (((s as any).month ?? 0) !== 7  ||  ((s as any).month ?? 0) === 7  &&  ((s as any).day ?? 0) === 9  &&  ((s as any).hour ?? 0) >= 8) {
            ((s as any).ballet_grade_score = (s as any).ballet_grade_score ?? {})['class'] = ((s as any).ballet_grade_score['class'] ?? 0) - (500);
...
function enterPilates(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Pilates';
  scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/pilates_2.jpg');
  qspCall(s, 'core_library', 'setloc', 'pushkin_ballet_class', ((s as any).locArgs?.[0] ?? 0));
  (s as any).nclass = 1;
  ...
  if (((s as any).ballet_day ?? 0) === 1) {
    ...
    qspCall(s, 'stat', '');
    qspCall(s, 'core_library', 'stage_title');
    // TODO-QSP: dynamic text: You''re led to the class by <<$npc_firstname[''A284'']>> Volkova, who tells you ...
    ...
    qspCall(st, 'exercise', 'tier1', 30, 'agil', 'stren');
    qspCall(st, 'pushkin_ballet_init', 'exercise_pain');
    qspCall(st, 'stat', '');
    ...
    // TODO-QSP: dynamic text: "You are?" <<$npc_firstname[''A284'']>>asks. " <<$npc_firstname[''A279'']>> <<$n...
    scene.text(`"You are?" ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')}asks. " ${(((st as any).npc_firstname ?? 0)?.['A279'] ?? '')} ${(((st as any).npc_lastname ?? 0)?.['A279'] ?? '')}," the girl replies.`);
    // TODO-QSP: dynamic text: " <<$npc_firstname[''A284'']>>''s daughter?"
    scene.text(`" ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')}'s daughter?"`);
    scene.text('"Yes, Miss."');
    // TODO-QSP: dynamic text: "So, what are the differences?", <<$npc_firstname[''A284'']>>asks brusquely.
    scene.text(`"So, what are the differences?", ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')}asks brusquely.`);
    ...
    // TODO-QSP: dynamic text: With that, <<$npc_firstname[''A284'']>> leads the class through strenuous exerci...
    scene.text(`With that, ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')} leads the class through strenuous exercises designed to strengthen and stretch your muscles. After an hour, your body slightly aches from the warm-up exercises.`);
    scene.actions([
      { label: 'Next class', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'first_lesson');
  } },
    ]);
  } },
    ]);
...
function enterFirstLesson(s: GameState, scene: SceneBuilder): void {
  ...
  (s as any).location_type = 'public_indoors';
  qspCall(s, 'themes', 'indoors');
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Dance Class';
  scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/ballet_class_002.jpg');
  ...
  if (((s as any).ballet_day ?? 0) === 1) {
    ...
    qspCall(s, 'stat', '');
    qspCall(s, 'core_library', 'stage_title');
    // TODO-QSP: dynamic text: "Listen up!" shouts <<$npc_firstname[''A284'']>>, "I want you to introduce yours...
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname[''A284'']>> points to the student that''s furthest from you. Se...
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname[''A284'']>> frowns. "You need to work on your confidence and pr...
    ...
    // TODO-QSP: dynamic text: "You." <<$npc_firstname[''A284'']>> points to another student. "I''m <<$npc_firs...
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname[''A284'']>> points to another student. " <<$npc_firstname[''A27...
    ...
    // TODO-QSP: dynamic text: For the next hour, <<$npc_firstname[''A284'']>> puts you through a series of dem...
  ...
  if (((s as any).ballet_day ?? 0) === 3) {
    ...
    qspCall(s, 'pushkin_ballet_init', 'exercise_pain');
    qspCall(s, 'stat', '');
    scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/ballet_class_003.jpg');
    qspCall(s, 'core_library', 'stage_title');
  ...
  if (((s as any).ballet_day ?? 0) === 4) {
    ...
    qspCall(s, 'stat', '');
    qspCall(s, 'core_library', 'stage_title');
    // TODO-QSP: dynamic text: After yesterday''s rehearsals, you sense an air of both anticipation and dread f...
    ...
    // TODO-QSP: dynamic text: You split into your two groups, and you follow <<$npc_firstname[''A274'']>>. She...
    ...
    // TODO-QSP: dynamic text: As you finish, <<$npc_firstname[''A284'']>>tells the class, "Tomorrow, you will ...
  ...
  if (((s as any).ballet_day ?? 0) === 5) {
    ...
    qspCall(s, 'pushkin_ballet_init', 'exercise_pain');
    qspCall(s, 'stat', '');
    scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/ballet_class_005.jpg');
    qspCall(s, 'core_library', 'stage_title');
    ...
    scene.text(`"Well done, class, you have done me proud. You have done exactly what I asked." It was rare praise from ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')}. "Now I want you to give me your best. I want you to dance like you never danced."`);
    // TODO-QSP: dynamic text: With that, she claps her hands. "Take your positions." <<$npc_firstname[''A285''...
    scene.text(`With that, she claps her hands. "Take your positions." ${(((st as any).npc_firstname ?? 0)?.['A285'] ?? '')} starts to play. You lose yourself in the performance of all the lessons coming together from the last week, and you flow through the performance.`);
    // TODO-QSP: dynamic text:  <<$npc_firstname[''A284'']>> watches all the girls'' performances critically, t...
    scene.text(` ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')} watches all the girls' performances critically, taking notes as she goes. The minutes fly by, and before you know it, the first rehearsal is over. "And again," she calls out.`);
    scene.actions([
      { label: 'Continue rehearsals', handler: (st: GameState) => {
    scene.img('images/' + '' + (((st as any).setloc ?? 0)?.['imagepath']) + 'daily/ballet_teach_1.jpg');
    qspCall(st, 'core_library', 'stage_title');
    // TODO-QSP: dynamic text: You rehearse several times before she''s happy, and then she hands critique note...
    scene.text(`You rehearse several times before she's happy, and then she hands critique notes to all the girls. When she gives you yours, she compliments you before gathering ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} and ${(((st as any).npc_firstname ?? 0)?.['A275'] ?? '')} aside for a private talk.`);
    // TODO-QSP: dynamic text: '"Tomorrow your assessment will start at ' + func('time', 'get_time_string', 9, ...
    scene.text('"Tomorrow your assessment will start at 9:00. You will enter the room in First Position and wait for the examiners\' instructions. Once you have performed the Entrance of the Swans, you will be asked to perform a Pas de Deux with your dance partner."');
    ...
    scene.text(`"Any questions? If you have any tonight or need assistance, contact ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')}, and she will advise or help as needed."`);
    scene.actions([
      { label: 'Reverence', handler: (st: GameState) => {
    scene.img('images/' + '' + (((st as any).setloc ?? 0)?.['imagepath']) + '/events/reverence.jpg');
    qspCall(st, 'core_library', 'stage_title');
    // TODO-QSP: dynamic text: You and the girls all look at each other and start to perform a deep curtsy as p...
    scene.text(`You and the girls all look at each other and start to perform a deep curtsy as part of the great reverence you agreed to the previous evening with the girls to pay respect to ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')} and ${(((st as any).npc_firstname ?? 0)?.['A285'] ?? '')}for their teaching this week.`);
    // TODO-QSP: dynamic text: Both teachers look taken aback and have a smile on their faces. "Thank you, clas...
    scene.text(`Both teachers look taken aback and have a smile on their faces. "Thank you, class. It's been a pleasure to teach you this week," ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')} says, with ${(((st as any).npc_firstname ?? 0)?.['A285'] ?? '')} nodding her agreement and repeating her words.`);
    scene.actions([
      { label: 'Go to the showers', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'shower');
  } },
...
function enterShower(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'core_library', 'setloc', 'pushkin_ballet_class', ((s as any).locArgs?.[0] ?? 0));
  qspCall(s, 'stat', '');
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'School Showers';
  scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/shower_1.jpg');
  if (((s as any).nclass ?? 0) > 1) {
    ...
    dynamicGoto(st, 'prevLoc', 'makeup');
      ...
      if (((s as any).nclass ?? 0) === 4) {
        scene.actions([
          { label: 'Go to next lesson', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'music_theory');
  } },
        ]);
      }
    }
    ...
    ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Residential Bathroom';
    scene.img('images/' + 'locations/pushkin/ballet_residence/residential_bathroom.jpg');
    ...
    if (((st as any).clothingworntype ?? 0) === 'nude') {
      ...
      qspGoto(st, 'pushkin_ballet_res', 'hallway');
    }
  } },
    ]);
  }
...
function enterGradeCheck(s: GameState, scene: SceneBuilder): void {
  (s as any).popup_advise = '<center><h3>Appearance</h3></center>';
...
function enterMakeup(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Theatre Make Up';
  scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/first_mua.jpg');
  (s as any).location_type = 'public_indoors';
  qspCall(s, 'themes', 'indoors');
  ...
  if (((s as any).ballet_day ?? 0) === 1) {
    ...
    // TODO-QSP: dynamic text:  <<$npc_firstname[''A274'']>> slides in beside you and whispers, "A lot to take ...
    ...
    if (((s as any).ballet_day ?? 0) === 2) {
      scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/second_mua.jpg');
      qspCall(s, 'core_library', 'stage_title');
      ...
      if (((s as any).ballet_day ?? 0) === 3) {
        ...
        // TODO-QSP: dynamic text: You quickly glance at <<$npc_firstname[''A274'']>>, who gives you an encouraging...
...
function enterLunch(s: GameState, scene: SceneBuilder): void {
  qspCall(s, 'core_library', 'setloc', 'pushkin_ballet_class', ((s as any).locArgs?.[0] ?? 0));
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'School Cafeteria';
  scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/cafeteria.jpg');
  (s as any).location_type = 'public_indoors';
  qspCall(s, 'themes', 'indoors');
  qspCall(s, 'core_library', 'stage_title');
...
function enterSecondLesson(s: GameState, scene: SceneBuilder): void {
  ...
  qspCall(s, 'pushkin_ballet_init', 'daily_assessment');
  qspCall(s, 'stat', '');
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Dance Practice';
  if (((s as any).mayaqw ?? 0)?.['grave'] === 4) {
    scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/mirror_ballet.jpg');
  } else {
    scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/class_three.jpg');
  }
  qspCall(s, 'core_library', 'stage_title');
  if (((s as any).ballet_day ?? 0) === 1) {
    ...
    // TODO-QSP: dynamic text: This doesn''t go unnoticed by <<$npc_firstname[''A284'']>>, and once the movemen...
    ...
    // TODO-QSP: dynamic text: She changes the image on the projector and lets <<$npc_firstname[''A274'']>> stu...
    ...
    // TODO-QSP: dynamic text: The following five minutes of performance pushed <<$npc_firstname[''A274'']>> ha...
    ...
    if (((s as any).ballet_day ?? 0) === 2) {
      ...
      // TODO-QSP: dynamic text: Before you can ask another question, <<$npc_firstname[''A284'']>> calls an end t...
      scene.text(`Before you can ask another question, ${(((s as any).npc_firstname ?? 0)?.['A284'] ?? '')} calls an end to the class.`);
    } else {
      if (((s as any).ballet_day ?? 0) === 3) {
        // TODO-QSP: dynamic text:  <<$npc_firstname[''A274'']>> doesn''t seem focused on the lesson today. She see...
        ...
        // TODO-QSP: dynamic text: "Yes, I''m fine." <<$npc_firstname[''A274'']>> replies, but she''s making too ma...
        ...
        // TODO-QSP: dynamic text:  <<$npc_firstname[''A274'']>> winces, but does as <<$npc_firstname[''A284'']>> a...
        ...
        // TODO-QSP: dynamic text: Before <<$npc_firstname[''A274'']>> can protest, <<$npc_firstname[''A284'']>> le...
  ...
  scene.actions([
    ...
    ((st as any).setloc = (st as any).setloc ?? {})['StageTitle'] = 'Reverance';
    scene.img('images/' + '' + (((st as any).setloc ?? 0)?.['imagepath']) + 'daily/ballet_class_006.jpg');
    qspCall(st, 'core_library', 'stage_title');
    // TODO-QSP: dynamic text: You perform a reverence to <<$npc_firstname[''A274'']>> in thanks for her help, ...
    scene.text(`You perform a reverence to ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} in thanks for her help, she returns the curtsey, and you head to the showers together.`);
    (st as any).nclass = 4;
    scene.actions([
      { label: 'Go to the showers', handler: (st: GameState) => {
    dynamicGoto(st, 'prevLoc', 'shower');
...
function enterMusicTheory(s: GameState, scene: SceneBuilder): void {
  ...
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Music Theory';
  scene.img('images/' + '' + (((s as any).setloc ?? 0)?.['imagepath']) + 'daily/ballet_class_004.jpg');
  (s as any).location_type = 'public_indoors';
  qspCall(s, 'themes', 'indoors');
  qspCall(s, 'exp_gain', 'intel', (Math.floor(Math.random() * 2) + 1));
  ...
  (s as any).nclass = 0;
  if (((s as any).ballet_day ?? 0) === 1) {
    // TODO-QSP: dynamic text: You enter the classroom, and you see <<$npc_firstname[''A285'']>> standing besid...
    ...
    // TODO-QSP: dynamic text: You and your classmates murmur a yes, unsure how to react. <<$npc_firstname[''A2...
    ...
    if (((s as any).ballet_day ?? 0) === 2) {
      ...
      // TODO-QSP: dynamic text:  <<$npc_firstname[''A285'']>>''s eyebrow rises slightly. "Excellent answer, Tais...
      ...
      // TODO-QSP: dynamic text:  <<$npc_firstname[''A285'']>> nods in approval to Taiya''s response spends the r...
      ...
      if (((s as any).ballet_day ?? 0) === 3) {
        // TODO-QSP: dynamic text: Today, you''re given a sharp shock. <<$npc_firstname[''A285'']>> claps her hands...
        ...
        // TODO-QSP: dynamic text: You continue the lesson. <<$npc_firstname[''A285'']>> keeps you all on your toes...
        ...
        if (((s as any).ballet_day ?? 0) === 4) {
          // TODO-QSP: dynamic text: "Positions!" <<$npc_firstname[''A285'']>> calls out before you can relax as you ...
          ...
          // TODO-QSP: dynamic text: The difference is <<$npc_firstname[''A285'']>> is creating audible cues with the...
          ...
          if (((s as any).ballet_day ?? 0) === 5) {
            ...
            // TODO-QSP: dynamic text:  <<$npc_firstname[''A285'']>> and <<$npc_firstname[''A284'']>> walk into the cla...
            ...
            // TODO-QSP: dynamic text: <<$npc_firstname[''A284'']>> claps her hands, and on the count of three, you hea...
            ...
            // TODO-QSP: dynamic text: "That wasn''t a complete disaster. From the top once more." You complete the dan...
            ...
            // TODO-QSP: dynamic text:  <<$npc_firstname[''A285'']>> comes over and starts handing out booklets to each...
            ...
            // TODO-QSP: dynamic text: "Remember, keep your muscles warm and do light stretches tonight. You don''t wan...
...
function enterAssessment(s: GameState, scene: SceneBuilder): void {
  ((s as any).setloc = (s as any).setloc ?? {})['StageTitle'] = 'Assessment Day';
  (s as any).location_type = 'public_indoors';
  qspCall(s, 'themes', 'indoors');
  qspCall(s, 'pushkin_ballet_init', 'daily_assessment');
  qspCall(s, 'exercise', 'tier4', 90, 'agil', 'stren', 'danc');
  ...
  // TODO-QSP: dynamic text: With minutes to spare, you''re outside the door to the examination. You feel you...
  ...
  scene.actions([
    ...
    scene.img('images/' + '' + (((st as any).setloc ?? 0)?.['imagepath']) + 'exams/exam_1.jpg');
    qspCall(st, 'core_library', 'stage_title');
    qspCall(st, 'exercise', 'tier4', 30, 'agil', 'stren', 'danc');
    qspCall(st, 'stat', '');
    ...
    // TODO-QSP: dynamic text: You recognise <<$npc_firstname[''A284'']>>, two other teachers you''ve not seen ...
    scene.text(`You recognise ${(((st as any).npc_firstname ?? 0)?.['A284'] ?? '')}, two other teachers you've not seen before, and ${(((st as any).npc_firstname ?? 0)?.['A283'] ?? '')}, are all assessing you. ${(((st as any).npc_firstname ?? 0)?.['A283'] ?? '')} seems to have his eyes mostly on yourself, but you think you are imagining it.`);
    ...
    qspCall(st, 'exercise', 'tier3', 30, 'agil', 'stren', 'danc');
    scene.img('images/' + '' + (((st as any).setloc ?? 0)?.['imagepath']) + 'exams/exam_2.jpg');
    qspCall(st, 'core_library', 'stage_title');
    qspCall(st, 'stat', '');
    // TODO-QSP: dynamic text: You wait outside the classroom. You''ve put on your feet warmers to ensure they ...
    scene.text(`You wait outside the classroom. You've put on your feet warmers to ensure they remain pliable and take the opportunity to drink some water and eat an energy bar. ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} sits down beside you, lost in thought.`);
    // TODO-QSP: dynamic text: "How do you think we did?" you ask her. "Hmm?" <<$npc_firstname[''A274'']>> look...
    scene.text(`"How do you think we did?" you ask her. "Hmm?" ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} looks up. "Yeah, we did brilliantly. I didn't see many faults from any of us, but it went so quickly, and I couldn't watch everything."`);
    // TODO-QSP: dynamic text: "I know," you reply. Before you can say anything else, you and <<$npc_firstname[...
    scene.text(`"I know," you reply. Before you can say anything else, you and ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} are called in.`);
    // TODO-QSP: dynamic text: You and <<$npc_firstname[''A274'']>> are given your assessment instructions. Fin...
    scene.text(`You and ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} are given your assessment instructions. Finally, after your ten minutes, you both finish your Pas de Deux performance with a curtsy of reverence to your examiners. All the while, during movements, you've felt ${(((st as any).npc_firstname ?? 0)?.['A283'] ?? '')}'s eyes still watching you.`);
    ...
    qspCall(st, 'food', 'healthy_meal_stats');
    ...
    qspCall(st, 'exercise', 'tier3', 30, 'agil', 'stren', 'danc');
    qspCall(st, 'stat', '');
    scene.img('images/' + '' + (((st as any).setloc ?? 0)?.['imagepath']) + 'exams/solo_2.jpg');
    qspCall(st, 'core_library', 'stage_title');
    // TODO-QSP: dynamic text: Back outside, you and <<$npc_firstname[''A274'']>>do checks on your outfits. You...
    scene.text(`Back outside, you and ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')}do checks on your outfits. You notice a small tear on ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')}'s tights and get your needle and thread out to repair it. You then check each other's hair and makeup, with ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} applying some hairspray to tame your flyaway hairs.`);
    // TODO-QSP: dynamic text: There''s some fruit juice and some snacks on a table to eat whilst you wait for ...
    scene.text(`There's some fruit juice and some snacks on a table to eat whilst you wait for your examination performance. Then, you start doing some light exercises with ${(((st as any).npc_firstname ?? 0)?.['A274'] ?? '')} to remain supple.`);
    ...
    qspCall(st, 'pushkin_ballet_init', 'daily_assessment', 'grade');
    ((st as any).balletqw = (st as any).balletqw ?? {})['school'] = 1;
    scene.actions([
