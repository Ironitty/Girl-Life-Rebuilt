...
function enterLetter(s: GameState, scene: SceneBuilder): void {
  if (((s as any).balletqw ?? 0)?.['letter'] !== 1  &&  ((s as any).balletqw ?? 0)?.['blocker'] !== 1) {
    ...
    // TODO-QSP: dynamic text: "<<$pcs_nickname>>," your mother calls your name, getting your attention. You've...
    scene.text(`"${((s as any).pcs_nickname ?? 0)}," your mother calls your name, getting your attention. You've received a letter from the school, and she hands it over to you.`);
    // TODO-QSP: dynamic text: You take the letter from your <<$npc_nickname['A29']>>, and your hands start sha...
    ...
    scene.text(`"Open it, ${((s as any).pcs_nickname ?? 0)}," she encourages you excitedly.`);
    scene.actions([
    ...
    scene.text(`"Did you get the place, ${((s as any).pcs_nickname ?? 0)}?" your ${((s as any).npc_nickname ?? 0)?.['A29']} asks.`);
    ...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29']} pulls you into her arms. "I knew you could do it, ${((s as any).pcs_nickname ?? 0)}. Well done!"`);
    ...
    scene.text(`"When do you go?" your ${((s as any).npc_nickname ?? 0)?.['A29']} asks.`);
  ...
  if (((s as any).balletqw ?? 0)?.['school'] > 1  &&  ((s as any).balletqw ?? 0)?.['letter'] === 1) {
    ...
    if (((s as any).balletqw ?? 0)?.['school'] === 5) {
      ...
      // TODO-QSP: dynamic text: Your <<$npc_nickname['A29']>> picks up the letter you dropped and reads it. "You...
      scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29']} picks up the letter you dropped and reads it. "You've done it, ${((s as any).pcs_nickname ?? 0)}. You're in. I'm proud of you, daughter." It's a rare moment between you and your mother.`);
      // TODO-QSP: dynamic text: "You've got to be at the School at 0900 next Sunday for induction. There will be...
      ...
      // TODO-QSP: dynamic text: "You've made it, <<$pcs_nickname>>. All that hard work has paid off." your <<$np...
      scene.text(`"You've made it, ${((s as any).pcs_nickname ?? 0)}. All that hard work has paid off." your ${((s as any).npc_nickname ?? 0)?.['A29']} says proudly.`);
      // TODO-QSP: dynamic text: "No, <<$npc_nickname['A29']>>, it's only the first step but it's an important on...
    ...
    if (((s as any).balletqw ?? 0)?.['school'] === 4) {
      ...
      scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29']} picks up the letter you tossed down and reads it. "You've done it, ${((s as any).pcs_nickname ?? 0)}. You're in. I'm proud of you, daughter." It's a rare moment between you and your mother.`);
      // TODO-QSP: dynamic text: "You've got to be at the School at 0900 next Sunday for induction. There will be...
      scene.text(`"You've got to be at the School at 0900 next Sunday for induction. There will be a bus waiting at the community center, or you can find the way to the school on your own," your ${((s as any).npc_nickname ?? 0)?.['A29']} says, still reading the contents.`);
...
function enterStarlets(s: GameState, scene: SceneBuilder): void {
  if (((s as any).balletqw ?? 0)?.['letter'] === 1) {
    ...
    scene.text(`"Albina, I need to have a talk with you." She turns towards you with a frown, "What's wrong, ${((s as any).pcs_nickname ?? 0)}." You wince at her frosty tone; you know she's a hard taskmaster with an even sharper tongue.`);
...
function enterResEvents(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === 'introductions') {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'kitchen_birthday') {
    ...
    // TODO-QSP: dynamic text: "No, <<$pcs_nickname>>, all's fine. I'm just finishing off the last of the food ...
    scene.text(`"No, ${((s as any).pcs_nickname ?? 0)}, all's fine. I'm just finishing off the last of the food to be baked."`);
    scene.text('"Gasha is going to be overwhelmed with all this! Have you asked everyone to come?" you inquire.');
    // TODO-QSP: dynamic text: "Yeah, well, everyone except <<$npc_firstname['A276']>>. I've asked my mum to ke...
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'birthday') {
    if (((s as any).locArgs?.[2] ?? 0) === 'card') {
      ...
      // TODO-QSP: dynamic text: "It's <<$npc_firstname['A280']>>'s birthday today, and I've got her a card. Can ...
      ...
      // TODO-QSP: dynamic text: "Don't worry about it. <<$npc_firstname['A277']>> is taking her out to Pushkin u...
      ...
      // TODO-QSP: dynamic text: You raise your eyebrow; <<$npc_firstname['A277']>> would have been the last pers...
      ...
      if (qspFunc(s, 'money', 'can_afford', 500) === 0) {
        ...
        (s as any).minut = ((s as any).minut ?? 0) + 10;
        qspCall(s, 'npc_relationship', 'modify', 'A279', 'like');
        ...
        scene.text(`You dig out your purse and put ' + $func('money', 'string_price', 500) + ' inside the card, and hand it back to ${((s as any).npc_firstname ?? 0)?.['A279']}. She notices you've put something inside and opens the card to find the money.`);
        // TODO-QSP: dynamic text: "Why <<$pcs_nickname>>? I'm aware your parents are poor, and I didn't expect you...
        scene.text(`"Why ${((s as any).pcs_nickname ?? 0)}? I'm aware your parents are poor, and I didn't expect you to give her a gift," she says with a surprised voice.`);
        // TODO-QSP: dynamic text: "It's fine, <<$npc_firstname['A279']>>. Tell her to buy a treat to cheer herself...
        scene.text(`"It's fine, ${((s as any).npc_firstname ?? 0)?.['A279']}. Tell her to buy a treat to cheer herself up. Just don't mention it's from me, please. How do you know I'm from a poor family?" you ask, frowning.`);
    ...
    // TODO-QSP: gt 'pushkin_ballet_evt', 'res_events', 'kitchen_birthday'
      ...
      scene.actions([
        { label: 'Gift [+$func(\'money\', \'get_cost_string\', 500)]' }, // TODO-QSP: empty action body
      ]);
    }
    scene.actions([
    ...
    scene.text(`You hand the card back to ${((s as any).npc_firstname ?? 0)?.['A279']} after signing it, and she looks at you. "Thank you, ${((s as any).pcs_nickname ?? 0)}, hopefully this helps her feel welcome here. She's been struggling to cope with being away from home for the first time."`);
    // TODO-QSP: dynamic text: "Think nothing of it, <<$npc_firstname['A279']>>. If it makes her feel better, t...
    scene.text(`"Think nothing of it, ${((s as any).npc_firstname ?? 0)?.['A279']}. If it makes her feel better, that's great," you say.`);
    // TODO-QSP: dynamic text: "Yeah, look, I need to start cooking the food for tonight. Will you come to the ...
    scene.text('"Yeah, look, I need to start cooking the food for tonight. Will you come to the communal room after \' + func(\'time\', \'get_time_string\', 18, 30) + \'?" You nod your head in a non-committal way as she leaves.');
    scene.actions([
      { label: 'Return to the hallway', goto: ['pushkin_ballet_res', 'hallway'] },
    ]);
  } },
    ...
    (s as any).setloc['StageImage'] = '<<$setloc[\'imagepath\']>>/ballet_residence/communal.jpg';
    qspCall(s, 'core_library', 'stage_title');
    (s as any).minut = ((s as any).minut ?? 0) + 20;
    qspCall(s, 'stat', '');
    // TODO-QSP: dynamic text: You help <<$npc_firstname['A279']>> with the plates and glasses in the common ro...
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname['A279']>> looks at you for a moment pausing to gather her thoug...
    ...
    // TODO-QSP: dynamic text: "Sorry hear that <<$npc_firstname['A279']>>", <<$npc_firstname['A279']>> holds u...
    scene.text(`"Sorry hear that ${((s as any).npc_firstname ?? 0)?.['A279']}", ${((s as any).npc_firstname ?? 0)?.['A279']} holds up her hand up, "I don't need sympathy ${((s as any).pcs_nickname ?? 0)}, I learned to deal with this years ago, like many here dancing is our refuge and passion in our lives. I barely knew my father and mother never found anyone else even here in the city"`);
    // TODO-QSP: dynamic text: "You sound wistful?", <<$npc_firstname['A279']>> looks at you sharply, and gives...
    ...
    // TODO-QSP: dynamic text: "She's our mother hen then?", <<$npc_firstname['A279']>> giggles, "Whatever you ...
    scene.text(`"She's our mother hen then?", ${((s as any).npc_firstname ?? 0)?.['A279']} giggles, "Whatever you do ${((s as any).pcs_nickname ?? 0)}, never and I mean never say that in front of mother."`);
    // TODO-QSP: dynamic text: <<$npc_firstname['A279']>> vanishes into the kitchen to get more food and items ...
    ...
    if (((s as any).locArgs?.[2] ?? 0) === 'party') {
      (s as any).setloc['StageTitle'] = 'Communal Room';
      (s as any).setloc['StageImage'] = '<<$setloc[\'imagepath\']>>/ballet_residence/gasha_birthday.jpg';
      qspCall(s, 'core_library', 'stage_title');
      ...
      // TODO-QSP: dynamic text: “What's going on <<$npc_firstname['A279']>>?” <<$npc_firstname['A280']>> sounds ...
      scene.text(`“What's going on ${((s as any).npc_firstname ?? 0)?.['A279']}?” ${((s as any).npc_firstname ?? 0)?.['A280']} sounds nervous.`);
      // TODO-QSP: dynamic text: “It's a little something for you podruzhka.”, she lifts her hands away from <<$n...
      ...
      // TODO-QSP: dynamic text: “Stay <<$npc_firstname['A280']>>, we're here to celebrate your birthday with you...
      ...
      // TODO-QSP: dynamic text: “Thank you…," you hear her murmur into <<$npc_firstname['A279']>>'s shoulder. Af...
      ...
      // TODO-QSP: dynamic text: “Thank you <<$pcs_nickname>> for being here”, <<$npc_firstname['A280']>> says as...
      scene.text(`“Thank you ${((s as any).pcs_nickname ?? 0)} for being here”, ${((s as any).npc_firstname ?? 0)?.['A280']} says as she hugs you.`);
      scene.text('“Can\'t miss the excuse for a party can we?”, you say jokingly.');
      scene.text('“No, and this is amazing. Did you help?”');
      // TODO-QSP: dynamic text: “A little but most of it's <<$npc_firstname['A279']>>'s handiwork, I think she's...
      ...
      // TODO-QSP: dynamic text: “She succeeded, she's even got some of my favourite snacks.” She blushes and loo...
      ...
      // TODO-QSP: dynamic text: You watch with amusement as you see <<$npc_firstname['A280']>> itching to try ea...
      ...
      // TODO-QSP: dynamic text: “See, I told you they were delicious!” <<$npc_firstname['A280']>> says with a gr...
      ...
      // TODO-QSP: dynamic text: “Yes, and I suspect that <<$npc_firstname['A284']>> is going to burn all the cal...
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'homesick') {
    // TODO-QSP: dynamic text: You enter the communal area and see <<$npc_firstname['A280']>> holding a textboo...
    ...
    // TODO-QSP: dynamic text: She looks up to you from the book, "I'm okay, <<$pcs_nickname>>, I'm just not us...
    scene.text(`She looks up to you from the book, "I'm okay, ${((s as any).pcs_nickname ?? 0)}, I'm just not used to being away from home or being in the city."`);
    // TODO-QSP: dynamic text: "You're a friend of <<$npc_firstname['A279']>>?" you ask her, trying to distract...
    ...
    // TODO-QSP: dynamic text: <<$npc_firstname['A280']>> looks at you with more interest, and you start to gos...
    ...
    scene.text(`"Thank you, ${((s as any).pcs_nickname ?? 0)}, for telling me your stories and cheering me up. I'm glad I'm not the only one!" Gasha says with a broad grin on her face.`);
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'tanis_bully') {
    scene.text('WIP - tbc');
  }
  if (((s as any).locArgs?.[1] ?? 0) === 'tanis_gasha') {
    scene.text('WIP - tbc');
  }
  if (((s as any).locArgs?.[1] ?? 0) === 'tanis_showdown') {
    scene.text('WIP - tbc');
  }
  if (((s as any).locArgs?.[1] ?? 0) === 'kima_gossip') {
    scene.text('WIP - tbc');
  }
  scene.build();
}
...
function enterIntro_274(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "I live Pushkin now but from Moscow originally.", <<$npc_firstname['A274']>> rep...
...
function enterIntro_275(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`You notice girl writing in a small book. She notices you looking at it, and makes a small attempt to try and avoid you. "Yes?", she asks, "I'm just introducing myself, ${((s as any).pcs_nickname ?? 0)}". "Ah you're the dancer from Pavlovsk." she said, looking at you critically. `);
...
function enterIntro_276(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: Glancing around the room you spot another girl and quickly approach her, "Hi I'm...
...
function enterIntro_277(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "I'm <<$pcs_nickname>>, are you alright?", you ask concerned about her behaviour...
  scene.text(`"I'm ${((s as any).pcs_nickname ?? 0)}, are you alright?", you ask concerned about her behaviour.`);
  // TODO-QSP: dynamic text: "I am <<$npc_firstname['A277']>>.", her answer is short and abrupt in a quiet vo...
...
function enterIntro_278(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text(`"Hi!", you call out as you approach her, "I'm ${((s as any).pcs_nickname ?? 0)}"`);
  // TODO-QSP: dynamic text: The girl gives you a warm smile as she gracefully turns, her eyes sparkle as she...
  scene.text(`The girl gives you a warm smile as she gracefully turns, her eyes sparkle as she greets you in a warm and entrancing voice, "Hi I'm ${((s as any).pcs_nickname ?? 0)}", I'm ${((s as any).npc_firstname ?? 0)?.['A278']}. Such a pleasure to meet you.`);
  ...
  // TODO-QSP: dynamic text: "Don't worry <<$pcs_nickname>>, I am used to that reaction.", she gives you a sm...
  scene.text(`"Don't worry ${((s as any).pcs_nickname ?? 0)}, I am used to that reaction.", she gives you a smile to reassure you, "I am a late bloomer as it were, and I've been studying as a Thespian performer and I have been invited to this class to expand my abilities."`);
  ...
  scene.text(`"So this is a great opportunity for you ${((s as any).pcs_nickname ?? 0)}? I wish you all the best. Don't fear of any competition from me, I'm more interested in pursuing my career as an actress then a ballerina. I want to become famous and go to Hollywood."`);
  ...
  scene.text(`"My agent said it would be a great experience for an upcoming role about a ballerina. Oh, Madam ${((s as any).npc_lastname ?? 0)?.['A284']} is staring at us. I think we need to be more sociable!", she takes your hand gracefully, "I know what it is like to struggle ${((s as any).pcs_nickname ?? 0)}, and by the will of God, I really do wish you all the best and you achieve your dreams."`);
  // TODO-QSP: dynamic text: "With this <<$npc_firstname['A278']>> leaves you at the table staring after her ...
...
function enterIntro_279(s: GameState, scene: SceneBuilder): void {
  scene.img(`${((s as any).npc_img_path ?? 0)?.['\'A279\'']}/279.jpg`);
  // TODO-QSP: dynamic text: You see a girl talking animatedly to Madam <<$npc_lastname['A284']>> as you appr...
  ...
  scene.text(`"Hello, you must be ${((s as any).pcs_nickname ?? 0)}", she says with a cheeky grin."`);
  scene.text('"Yes, how did you know?"');
  // TODO-QSP: dynamic text: "It's not really a secret at the school that people are saying you're the next u...
  ...
  // TODO-QSP: dynamic text: "I'm being spoken about?", you reply surprised at being recognised, "I saw you t...
  ...
  // TODO-QSP: dynamic text: "Thank you, I will keeep that in mind. Madam <<$npc_lastname['A284']>> seems int...
...
function enterIntro_280(s: GameState, scene: SceneBuilder): void {
  ...
  // TODO-QSP: dynamic text: "No", you reply, "just saw you alone and wanted to meet you. I'm <<$pcs_nickname...
  scene.text(`"No", you reply, "just saw you alone and wanted to meet you. I'm ${((s as any).pcs_nickname ?? 0)}", the girl gives you a shy smile, "I'm ${((s as any).npc_firstname ?? 0)?.['A280']} pleased to meet you!"`);
  scene.text('"Likewise, are you looking forward to this week?", you ask.');
  // TODO-QSP: dynamic text: "Not really, but I have <<$npc_firstname['A279']>> supporting me." She looks acr...
  ...
  // TODO-QSP: dynamic text: "<<$npc_firstname['A280']>> gives another of her shy smiles, "I know what you me...
...
function enterResEventsArray(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === '0') {
    ...
    (s as any).setloc['StageImage'] = '<<$setloc[\'imagepath\']>>ballet_events/chess.jpg';
    qspCall(s, 'core_library', 'stage_title');
    ...
    scene.text(`You feel teaching the girls has deepened your own understanding and appreciation for ${((s as any).npc_firstname ?? 0)?.['A286']}'s own abilities.`);
    scene.actions([
      { label: 'Leave room', goto: ['pushkin_ballet_res', 'communal_area'] },
    ]);
  } },
    ...
    scene.actions([
      ...
      if (((s as any).locArgs?.[1] ?? 0) === '4') {
        ...
        // TODO-QSP: dynamic text: "No why?", you ask. <<$npc_firstname['A280']>> looks nervously at the door, "She...
        ...
        // TODO-QSP: dynamic text: "Let me guess, <<$npc_firstname['A276']>>?", <<$npc_firstname['A280']>> nods fea...
        ...
        // TODO-QSP: dynamic text: <<$npc_firstname['A280']>>'s eyes open even wider, "Are you crazy? She'll just m...
        ...
        // TODO-QSP: dynamic text: "You can't let her get away with this, She'll just get worse and bully someone e...
        scene.text(`"You can't let her get away with this, She'll just get worse and bully someone else", you say, "Please ${((s as any).pcs_nickname ?? 0)}, don't make this worse for me."`);
        scene.actions([
    ...
    scene.text(`You grab a blanket to cover yourself and settle down to watch TV beside. After a moment ${((s as any).npc_firstname ?? 0)?.['A280']} shivers a bit and you pull your blanket up over her. This seems to help relax her and you sit in comfortable silence enjoying the show.`);
    // TODO-QSP: dynamic text: When the show finishes, you get up to leave. <<$npc_firstname['A280']>> stirs fr...
    scene.text(`When the show finishes, you get up to leave. ${((s as any).npc_firstname ?? 0)?.['A280']} stirs from her position and looks at you but doesn't say anything, but you can tell she appreciated your company.`);
    scene.actions([
      { label: 'Leave room', goto: ['pushkin_ballet_res', 'communal_area'] },
    ]);
  } },
          { label: 'Let <<$npc_firstname[\'A280\']>> handle it', handler: (st: GameState) => {
    qspCall(s, 'npc_relationship', 'modify', 'A280', 'dislike');
    qspCall(s, 'stat', '');
    // TODO-QSP: dynamic text: "If you're sure, but promise me that'll you talk to <<$npc_firstname['A286']>> i...
    scene.text(`"If you're sure, but promise me that'll you talk to ${((s as any).npc_firstname ?? 0)?.['A286']} if it gets out of hand?", ${((s as any).npc_firstname ?? 0)?.['A280']} nods too fearful to say anything else.`);
    // TODO-QSP: dynamic text: You leave the room still uncertain if you should leave the matter to <<$npc_firs...
    scene.text(`You leave the room still uncertain if you should leave the matter to ${((s as any).npc_firstname ?? 0)?.['A280']} or if you should go and see ${((s as any).npc_firstname ?? 0)?.['A286']}.`);
    scene.actions([
      { label: 'Leave room', goto: ['pushkin_ballet_res', 'communal_area'] },
    ]);
  } },
