...
function enterLetter(s: GameState, scene: SceneBuilder): void {
  if (((s as any).balletqw ?? 0)?.['letter'] !== 1  &&  ((s as any).balletqw ?? 0)?.['blocker'] !== 1) {
    ...
    scene.text(`"Did you get the place, ${((s as any).pcs_nickname || '')}?" your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} asks.`);
    ...
    scene.text(`Your ${((s as any).npc_nickname ?? 0)?.['A29'] ?? ''} pulls you into her arms. "I knew you could do it, ${((s as any).pcs_nickname || '')}. Well done!"`);
...
function enterResEvents(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).locArgs?.[1] ?? 0) === 'birthday') {
    ...
    if (qspFunc(s, 'money', 'can_afford', 500) === 0) {
      ...
      scene.text(`You dig out your purse and put 500₽ inside the card, and hand it back to ${((s as any).npc_firstname ?? 0)?.['A279'] ?? ''}. She notices you've put something inside and opens the card to find the money.`);
      // TODO-QSP: dynamic text: "Why <<$pcs_nickname>>? I'm aware your parents are poor, and I didn't expect you...
      scene.text(`"Why ${((s as any).pcs_nickname || '')}? I'm aware your parents are poor, and I didn't expect you to give her a gift," she says with a surprised voice.`);
    ...
    scene.text(`You hand the card back to ${((s as any).npc_firstname ?? 0)?.['A279'] ?? ''} after signing it, and she looks at you. "Thank you, ${((s as any).pcs_nickname || '')}, hopefully this helps her feel welcome here. She's been struggling to cope with being away from home for the first time."`);
    ...
    scene.actions([
      ...
      if (((s as any).locArgs?.[2] ?? 0) === 'prep') {
        ...
        scene.img('images/' + '' + qspUntranslated(s, "setloc['imagepath']>", { location: "pushkin_ballet_evt" }) + '/ballet_residence/communal.jpg');
        qspCall(s, 'core_library', 'stage_title');
        ...
        if (((s as any).locArgs?.[2] ?? 0) === 'party') {
          if (!(s as any).setloc) (s as any).setloc = {}; (s as any).setloc['StageTitle'] = 'Communal Room';
          scene.img('images/' + '' + qspUntranslated(s, "setloc['imagepath']>", { location: "pushkin_ballet_evt" }) + '/ballet_residence/gasha_birthday.jpg');
          qspCall(s, 'core_library', 'stage_title');
...
function enterResEventsArray(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === '0') {
    ...
    scene.img('images/' + '' + qspUntranslated(s, "setloc['imagepath']>", { location: "pushkin_ballet_evt" }) + 'ballet_events/chess.jpg');
    qspCall(s, 'core_library', 'stage_title');
