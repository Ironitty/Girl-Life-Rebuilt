...
function enterPayments(s: GameState, scene: SceneBuilder): void {
  ...
  scene.text('You pay ' + qspFunc(s, 'money', 'string_price', ((s as any).locArgs?.[2] ?? '')) + ` for the ${((s as any).locArgs?.[1] ?? '')} course and are now booked in for your lesson. Please refer to the noticeboard for class times and rooms.`);
  qspCall(s, 'stat', '');
  scene.actions([
...
function enterMasseuse(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).license ?? 0)?.['masseuse_enrolled'] === 0) {
    if (qspFunc(s, 'money', 'can_afford', 10000) === 1) {
      ...
      scene.text('You nod and she quickly gives you the course details and a pamphlet to read.');
      scene.text('The basic course is 10 lessons, and the full set of courses costs ' + qspFunc(s, 'money', 'string_price', 10000));
      scene.actions([
        { label: 'Pay for the course', handler: (st: GameState) => {
