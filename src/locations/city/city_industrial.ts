...
function enterExit(s: GameState, scene: SceneBuilder): void {
  if (((s as any).locArgs?.[1] ?? 0) === 'pavlovsk') {
    ...
    if (((s as any).locArgs?.[0] ?? 0) === ''  ||  ((s as any).locArgs?.[0] ?? 0) === 'start'  ||  ((s as any).locArgs?.[0] ?? 0) === 'main') {
    ...
    // TODO-QSP: gt 'city_industrial', 'exit', 'pavlovsk'
        ...
        { label: 'Walk to the Red Light District (0:05)', handler: (st: GameState) => {
    // TODO-QSP: gt 'city_industrial', 'exit', 'redlight'
  } },
        { label: 'Walk to the central park (1:00)', handler: (st: GameState) => {
    // TODO-QSP: gt 'city_industrial', 'exit', 'citypark'
  } },
        { label: 'Walk to the industrial train station (0:15)', handler: (st: GameState) => {
    // TODO-QSP: gt 'city_industrial', 'exit', 'train'
  } },
        { label: 'Walk to the Metro (0:05)', handler: (st: GameState) => {
    // TODO-QSP: gt 'city_industrial', 'exit', 'metro'
  } },
        { label: 'Walk to the canals (0:15)', handler: (st: GameState) => {
    // TODO-QSP: gt 'city_industrial', 'exit', 'canals'
  } },
      ]);
    }
  }
