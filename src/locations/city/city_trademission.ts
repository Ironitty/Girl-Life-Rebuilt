...
function enter(s: GameState, scene: SceneBuilder): void {
  ...
  if (((s as any).svQW ?? 0) === 2) {
    ...
    (st as any).TorgPredZ = 3;
    (st as any).minut = ((st as any).minut ?? 0) + 5;
    (st as any).TorgPredDay = ((st as any).daystart ?? 0);
    qspCall(st, 'stat', '');
    ...
    (st as any).TPmainQW = 2;
    ((st as any).npc_height = (st as any).npc_height ?? {})['A50'] = 181;
    ((st as any).npc_weight = (st as any).npc_weight ?? {})['A50'] = 58;
    ((st as any).npc_haircol = (st as any).npc_haircol ?? {})['A50'] = 3;
    ((st as any).npc_finance = (st as any).npc_finance ?? {})['A50'] = 1;
    ((st as any).npc_pervert = (st as any).npc_pervert ?? {})['A50'] = 0;
    (st as any).figurBoy = (Math.floor(Math.random() * 3) + 0);
    (st as any).titBoy = (Math.floor(Math.random() * 3) + 0);
    (st as any).hairBoy = (Math.floor(Math.random() * 4) + 0);
    qspCall(st, 'npcStat', 'A50');
    (st as any).npcheight = 'high';
    (st as any).npcbuild = 'thin';
    (st as any).npchair = 'blond';
    (st as any).vneshBoy = 1;
    (st as any).npcClo = 'jeans and a sweater';
    qspCall(st, 'lover', 'add_boyfriend', 'A50');
    (st as any).stopboy = 0;
    (st as any).boyonceA = 1;
    qspCall(st, 'stat', '');
    scene.actions([
      { label: 'Leave', goto: ['city_smalloffice', 'start'] },
    ]);
