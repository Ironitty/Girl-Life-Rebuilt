...
- **Save/Load**: IndexedDB via `src/core/save.ts` (10 slots)
  ...
  - **Batch policy for location ports**: Port **3 locations** per batch, then run the full test cycle (steps 2–5) once. Do NOT run tests after each individual location within a batch. Only run LSP/typecheck per location to catch immediate errors. After the 3rd location is written, run build + smoke test + Firefox together.
   - **Comprehensive audit**: `npx tsx scripts/comprehensive-audit.ts` — 3-phase audit (static analysis, render, interaction). Requires `npx vite build` first (serves `dist/index.html` on port 4174). Takes ~1–2 hours. Always give it a **2 hour timeout** (7200000 ms). CLI flags: `--skip-static`, `--skip-render`, `--skip-interaction`, `--filter <regex>`, `--check-todo`, `--verbose`. Exclusion sets at the top of the file (`EXCLUDE_EXEC_DATA`, `EXCLUDE_FUNC_LITERAL`, `EXCLUDE_EXPR`, `EXCLUDE_BG`, `EXCLUDE_NO_ACTIONS`, `EXCLUDE_JS_ERRORS`, `EXCLUDE_UNTRANSLATED`) — before adding a location to an exclusion, verify against the QSP source (`.qsps` file) that it's not a transpiler bug. **When the audit finds a bug, fix the root cause** (transpiler, generated code, or audit logic) instead of adding the location to an exclusion set. Exclusions are a last resort for genuinely special cases (e.g., location has no `act` commands in QSP source, or actions are conditional on game state that isn't set in a fresh save).
